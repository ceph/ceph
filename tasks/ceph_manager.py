    ...
    def kill_osd(self, osd=None, mark_down=False, mark_out=False):
        ...
        if self.ceph_objectstore_tool:
            ...
            if 'keyvaluestore_backend' in self.ceph_manager.ctx.ceph.conf['osd']:
                prefix = "sudo ceph-objectstore-tool --data-path {fpath} --journal-path {jpath} --type keyvaluestore --log-file=/var/log/ceph/objectstore_tool.\\$pid.log ".format(fpath=FSPATH, jpath=JPATH)
            else:
