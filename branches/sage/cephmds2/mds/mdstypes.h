...
struct inode_caps_reconnect_t {
  ...
  inode_caps_reconnect_t(int c, int s, int w, int i) : 
    caps(c), seq(s), wanted(w), issued(i), size(0) {}
  inode_caps_reconnect_t(int c, int s, int w, int i, off_t sz, utime_t mt, utime_t at) : 
    caps(c), seq(s), wanted(w), issued(i), size(sz), mtime(mt), atime(at) {}
};
