...
Requirements are INTERFACE for interface or imported targets, and PUBLIC for
ordinary build targets so their own sources receive them as well.
...
GIT_REPOSITORY and GIT_TAG. HEADER_ONLY_INCLUDE_DIR downloads the package without
executing its CMake project, then exposes only that include directory:

    require_cpp_feature(__cpp_lib_generator cpp_generator
        TARGET rgw_fdb
        GIT_REPOSITORY https://github.com/lewissbaker/generator
        GIT_TAG 6ef0cca50072409a5898f52770bcfbaf418eece8
        HEADER_ONLY_INCLUDE_DIR include)
...
fetches lewissbaker/generator, its own build and tests are not added to Ceph; only its headers are
made available where rgw_fdb needs them.
...
function(cpp_feature_target_scope_detail out_name target)

  get_target_property(target_type ${target} TYPE)
  get_target_property(imported_target ${target} IMPORTED)

  if(imported_target OR target_type STREQUAL "INTERFACE_LIBRARY")
    set(scope INTERFACE)
  else()
    set(scope PUBLIC)
  endif()

  set(${out_name} ${scope} PARENT_SCOPE)

endfunction()

function(cpp_feature_header_only_alternative_detail out_name feature_tag_name include_directory)

  set(source_directory_variable "${feature_tag_name}_SOURCE_DIR")
  set(source_directory "${${source_directory_variable}}")

  if(NOT source_directory)
    message(FATAL_ERROR "CPM did not provide a source directory for ${feature_tag_name}")
  endif()

  set(include_path "${source_directory}/${include_directory}")

  if(NOT IS_DIRECTORY "${include_path}")
    message(FATAL_ERROR "C++ feature include directory does not exist: ${include_path}")
  endif()

  string(MAKE_C_IDENTIFIER "cpp_feature_${feature_tag_name}_alternative" alternative_target)

  if(NOT TARGET ${alternative_target})
    add_library(${alternative_target} INTERFACE)
    target_include_directories(${alternative_target} SYSTEM INTERFACE
...
# (MIN_VALUE and HEADER_ONLY_INCLUDE_DIR are used by this helper; the extra
# arguments are passed to CPM.)
function(obtain_cpp_feature_detail cpp_feature_macro feature_tag_name our_name)

  cmake_parse_arguments(PARSED "" "MIN_VALUE;TARGET;HEADER_ONLY_INCLUDE_DIR" "USE_ALTERNATIVE" ${ARGN})
  set(MIN_VALUE_ARG ${PARSED_MIN_VALUE})
  set(TARGET_ARG ${PARSED_TARGET})
  set(HEADER_ONLY_INCLUDE_DIR_ARG ${PARSED_HEADER_ONLY_INCLUDE_DIR})
  ...
  if(NOT feature_found AND UNPARSED_ARGS)
    ...
    if(HEADER_ONLY_INCLUDE_DIR_ARG)
      CPMAddPackage(NAME ${feature_tag_name} ${UNPARSED_ARGS} DOWNLOAD_ONLY YES)
    else()
    ...
    if(NOT feature_alternative AND HEADER_ONLY_INCLUDE_DIR_ARG)
      cpp_feature_header_only_alternative_detail(feature_alternative
        ${feature_tag_name} ${HEADER_ONLY_INCLUDE_DIR_ARG})
    endif()
  ...
  if(feature_found AND TARGET_ARG)
    cpp_feature_target_scope_detail(target_scope ${TARGET_ARG})
    target_compile_definitions(${TARGET_ARG} ${target_scope} ${our_name})

    if(feature_alternative)
      cpp_feature_alternative_macro_detail(alternative_name ${our_name})
      target_compile_definitions(${TARGET_ARG} ${target_scope} ${alternative_name})
      target_link_libraries(${TARGET_ARG} ${target_scope} ${feature_alternative})
    endif()
  endif()
...
# MIN_VALUE, TARGET, USE_ALTERNATIVE, HEADER_ONLY_INCLUDE_DIR, and CPM package
# arguments are handled by obtain_cpp_feature_detail(). Fail if the feature
