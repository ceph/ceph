...
To access stored data in Ceph, one can achieve this in three ways, which the first being most relevant to this project: radosgw (RGW), librados, and RADOS block device (RBD). With RGW, data can be accessed using HTTP with the Ceph Object Gateway daemon providing interfaces compatible with Amazon S3 and OpenStack Swift.
