...
The runtime performance of a ``Debug`` build may be as little as 20% of that of a non-debug build.
Pass ``-DCMAKE_BUILD_TYPE=RelWithDebInfo`` to ``do_cmake.sh`` to create a
non-debug build.
The default build type is ``RelWithDebInfo`` when `.git` does not exist.
