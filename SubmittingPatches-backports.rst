...
* all fixes should land in main branch first
* commits to stable branches should be cherry-picked from the main branch
* before starting to cherry-pick a set of commits from main, grep the main branch git history for the SHA1 of each commit (using ``git log --grep``) to check for follow-up fixes. Include any follow-up fixes found in the set of commits to be cherry-picked.
* when backporting a main branch PR to a stable branch, double-check that the backport PR contains cherry-picks of all of the main branch PR's commits. If any commit needs to be omitted, declare and explain this in the PR.
* cherry-picks must be done using ``git cherry-pick -x``
* if a cherry-pick from the main branch is not feasible and a direct fix is being undertaken, this must be explained
...
What does "all fixes should land in the main branch first" mean? What if I just need to
fix the issue in <release>?

As the person fixing the issue, you are required to first check whether the
issue exists in the main branch. If it does, then the proper course of action is to
create a main branch tracker (see `Tracker workflow`_) and fix the issue in the main branch,
first, and only then cherry-pick the fix to the stable branches that have the
issue.

If the issue exists in the stable branch, but not in the main branch, there are several
possibilities:

1. it's a regression introduced into the stable branch by a bad backport
2. the issue was fixed in the main branch by some massive refactoring that cannot be backported
3. the issue was already fixed in the main branch by a cherry-pickable commit
...
the main branch, the commit message must explain why that was not possible". Once the
...
For fixes already merged to the main branch, this may have already been done - see the
``Fixes:`` line in the main branch PR. If the main branch PR has already been merged and
...
The stable branches to which the main branch changes are to be cherry-picked should
be listed in the "Backport" field. For example::

    Backport: mimic, nautilus

Once the PR targeting the main branch is open, add the PR number assigned by GitHub to
the tracker issue. For example, if the PR number is 99999::

    Pull request ID: 99999

Once the main branch PR has been merged, after checking that the change really needs
to be backported and the Backport field has been populated, change the main branch
...
Once the entire `Tracker workflow`_ has been completed for the main branch issue,
issues can be created in the Backport tracker for tracking the backporting work.

Under ordinary circumstances, the developer who merges the main branch PR will flag
...
most recent version from the main branch.
...
the commits from the main branch PR, opening the GitHub backport PR, and
...
local ceph clone on every call::
    ...
    alias ceph-backport="bash <(git --git-dir=$pathToCephClone/.git --no-pager show upstream/main:src/script/ceph-backport.sh)"
...
Naturally, the ``Fixes`` line points to the main branch issue. You might be tempted
to modify it so it points to the backport issue, but - please - don't do that.
First, the main branch issue points to all the backport issues, and second, *any*
...
Next, check which component label was applied to the main branch PR corresponding to
this backport, and double-check that that label is applied to the backport PR as
well. For example, if the main branch PR carries the component label "core", the
