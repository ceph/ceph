...
accordance with obsreved and expected pool utilization.
...
There is also a central config default ``pg_autoscale_mode`` option pools that are created
...
- **RATE** is space amplification factor for the pool that indicates how much raw storage
...
- **NEW PG_NUM** (if present) is the value that the system recommends that the
  ...
  more than the default factor of ``3``. This avoids thrashing, like an old car's
  automatic transmission might experience on a rollercoaster road.
