...
Unable to Mount with Kernel Client
----------------------------------

If you are unable to use the kernel client to mount images or a filesystem while
``pg-upmap-primary`` mappings are in your cluster, this is because ``pg-upmap-primary``
is not yet supported by the kernel client (as of 2025-09-05).

You may verify this scenario by:

1. Confirming that your cluster contains ``pg-upmap-primary`` mappings:

.. prompt:: bash $

   ceph osd dump

2. Checking for this error message in the kernel client log:
...
Those details simply confirm that the cluster is using features that the kernel client doesn't support.
