...
A Ceph Storage Cluster that deployes the :term:`Ceph File System` also runs
...
Each Ceph daemon, process, and library pulls configuration option values from one or more
of the sources listed below. Values set via sources that occur later in the list
override those that occur earlier in the list.
...
no different options set in a node-local configuration file, subject to the above
source precedence.
...
.. note:: The Ceph releas versions of *running* daemons may be reported by
	  running the ``ceph versions`` commands.  If your cluster is not
	  in the process of an upgrade, all daemons should show the same
	  version. If multiple versions are reported outside of an upgrade,
	  a prior upgrade may have failed or manual changes may have been executed,
	  and the circumstances should be examined and an upgrade to harmonize
...
without requiring that the daemon be restartedn. This might be used for
...
To see all settings (including those with default values), for a specific daemon, run
