...
Override Max IOPS Capacity of an OSD
------------------------------------

The steps in this section may be performed to override the max IOPS capacity of
an OSD that was automatically set during its initialization.
The option ``osd_mclock_max_capacity_iops_[hdd, ssd]`` for an OSD can be
overridden by running a command of the following form:
