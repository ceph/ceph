...
- **Clustered storage manager.** Ceph is a clustered storage manager. That
  means that the storage manager is installed not just on a single machine but
  on several machines that work together as a system.
- **Distributed storage manager.** Ceph is a clustered and distributed storage
  ...
  it might be helpful to describe what it is not: it is not a system like
  iSCSI, which is a system that exposes a single logical disk over the network
  in a 1:1 (one-to-one) mapping.
