...
extra copies of that data, so that the loss of one disk or one server loses
nothing.
...
.. list-table::
   ...
   * - :term:`CephFS<Ceph File System>`
     - A shared, POSIX-compatible file system
...
A Ceph cluster is run by a small set of daemons: Monitors, Managers, OSDs,
and, depending on the storage interfaces in use, Metadata Servers and Object
Gateways. See :ref:`ceph-cluster-components` for what each daemon does and how
...
- :doc:`Developer Quick Guide </dev/quick_guide>`, for developers who want to
  build Ceph from source and run a throwaway ``vstart`` cluster
