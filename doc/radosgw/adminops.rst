...
:Description: A number of bucket list entries to return, max is
              enforced to rgw_list_buckets_max_chunk config option.
:Type: Integer
:Example: 100
:Required: No
...
:Description: Number of returned buckets, only if ``max-entries`` is used.
:Type: Integer

``truncated``

:Description: If the response is truncated, only if ``max-entries`` is used.
:Type: Boolean
