...
In a multi-object delete request, Lua scripts apply per delete operation for the ``postauth`` and ``postrequest`` contexts, while the ``prerequest`` context is skipped. Returning `RGW_ABORT_REQUEST` in one delete operation will abort this operation, but will not abort remaining deletions in the same bulk request.
