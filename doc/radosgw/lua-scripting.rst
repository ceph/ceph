...
In a multi-object delete request, Lua scripts apply per delete operation for the ``prerequest``, ``postauth``, and ``postrequest`` contexts. Returning `RGW_ABORT_REQUEST` in one delete operation will abort this operation, but will not abort remaining deletions in the same bulk request.
