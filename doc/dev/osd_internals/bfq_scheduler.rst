...
* ``osd_bfq_cost_per_op`` -- the fixed OSD pipeline overhead of any op
  ...
  16-64 KiB).
* ``osd_bfq_cost_per_seek`` -- the positioning (seek) cost in
  byte-equivalents, charged on rotational devices only.  Flash pays no
  positioning cost, so the value is ignored there and one cluster-wide
  setting is safe for a mixed fleet; on HDDs roughly the seek-bandwidth
  product (512 KiB to 1 MiB for 7200 rpm), which charges a 4 KiB and a
  256 KiB random read nearly the same, as the platter does.

When either constant is in effect every op is charged ``cost_per_op +
cost_per_seek + bytes`` (the seek term counting only on rotational
media) and the ``osd_bfq_min_cost`` floor is ignored.
