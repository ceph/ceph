...
Primaries can request a temp acting set mapping in order to
...
type like a pair<shard_t, pg_t> in order to distinguish different pg
...
blocks within a stripe that is being written, i.e., W <= K.
...
the commit (sorry, overloaded term) and remove the rollback information.
