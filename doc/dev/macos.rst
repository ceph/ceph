...
::
  ...
  brew install cmake ninja boost ccache openssl@3 snappy lz4 zstd nss \
      icu4c sqlite libevent utf8proc c-ares lua pkgconf autoconf automake \
      libtool uv gawk

``lua`` is needed because ``cls_lua`` builds on every platform but Windows.
``autoconf``, ``automake`` and ``libtool`` are needed because ISA-L, which the
erasure code plugin and the zlib compressor use on arm64, is built from its
submodule with autotools. ``gawk`` is only needed by the ``qa/standalone``
test helpers, which macOS awk cannot run.
...
Only a few of the submodules are needed::
  ...
  git submodule update --init src/fmt src/BLAKE3 src/xxHash src/rocksdb \
      src/zstd src/googletest src/utf8proc src/isa-l \
...
If CMake's first compiler check fails with ``ld: tapi error: malformed file
... libSystem.tbd: unknown architecture arm64e.x1``, the default SDK is newer
than the linker in the Command Line Tools, typically because a beta SDK was
left behind by an earlier install. Point ``SDKROOT`` at the SDK that ships
with the installed Command Line Tools, e.g.
