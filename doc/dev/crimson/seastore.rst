...
design an on-device layout that is friendly to GC at lower layers and
drive garbage collection at higher layers.

In principle a fine-grained discard could communicate our intent to
the device, but in practice discard operations are sometimes coded poorly
implemented in the device and intervening software layers.
...
.. flowchart TD
  ...
  class OnodeTree,OnodeN,Omap1,Omap2,Omap3,Extents logical;
...
**Extents**:
  ...
  Logically addressed extents do not store physical offsets, allowing them to be relocated freely
  without affecting the data structures they belong to. The vast majority of extents fall into this category.
...
- **backend_type_t::RANDOM_BLOCK**:
  ...
  * ``device_type_t::RANDOM_BLOCK_SSD``
  ...
  Preferred for fast NVMe devices.
...
dirty blocks (See: ``JournalTrimmerImpl::trim_dirty()``).
Note, we can do journal checkpoints relatively infrequently, and they needn’t block the write stream.

**Journal Deltas**:

Deltas are logical mutations to existing physical extents.
