...
.. note::
   ...
   due to ``CRIMSON_OSDS_DOWN``, explaining further that "Crimson is not configured for making pools."
   There is also the additional ``CRIMSON_FLAGS_OFF`` warning which is triggered when
   these three flags are not set. 
