...
(triggering PG peering), if the primary does not change then the client
need not resend.
...
Ordinarily the OSD will simply queue any requests it cannot immediately
...
read off the network socket, causing back pressure through the network.
