...
To enable CpuTrace in the Ceph codebase you need to build with the
``WITH_CPUTRACE`` flag:
...
CpuTrace is using linux ``perf_event_open`` syscall. You can use the tool
...
The hardware context (``HW_ctx``) must be initialized once before creating any
...
Each ``HW_named_guard`` automatically starts measurement at construction  
and stops when leaving scope.
...
before any guards can be used.

Named guards provide a simple and consistent way to track performance metrics.

To later access the collected measurement for a given name, use:  
...
.. code-block:: cpp
  ...
  HWProfileFunctionF(profile, __FUNCTION__,
...
Operators ``|`` and ``&`` are overloaded so that multiple flags can be combined easily.
...
The most common usage pattern is to profile an entire function.
This is done with the RAII-style `HW_profile` object:

.. code-block:: cpp

  void myfunction()
  {
    HW_profile profiler("my_code", 0,
                        HW_PROFILE_CYC  | HW_PROFILE_CMISS |
                        HW_PROFILE_INS  | HW_PROFILE_BMISS |
                        HW_PROFILE_SWI);
    // my code starts
...
When the function is entered, profiling begins.
When the function exits (normally or via exception), profiling ends automatically.
