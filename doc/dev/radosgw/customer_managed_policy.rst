...
NOTE: users/groups/roles can be renamed by the Update APIs like iam:UpdateUser. if we rename an entity or change its path, we should also update the name/ARN for all of its attachments
