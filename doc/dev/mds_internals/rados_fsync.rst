...
- okay, when does the Client actually share change_attr/stx_version with users?
  It increments change_attr on every write, but that means if there are multiple
  writers they are doing the same increment. But it tries to only share it in
  certain circumstances...so when is Ganesha even getting to see the change_attr
  at all?
- so Client independently updates change_attr on every write, but it looks like
  it only exposes the change_attr when it has all the caps on calling fill_statx?
