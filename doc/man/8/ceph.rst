...
Subcommand The subcommand ``ok-to-upgrade`` determines a safe set of OSD(s)
found within the specified CRUSH bucket to upgrade simultaneously without
impacting cluster data availability and with all data remaining readable
and writeable. Data redundancy may be reduced with some PGs in degraded (but
active) state. The command checks the Ceph version running on the OSDs against
the specified version and filters those still needing upgrade. The command
returns a success code if it finds a safe set of OSD(s) to upgrade and shows
the list of OSD(s) in the response, or an error code and informative message
otherwise or if no conclusion can be drawn.
...
``mgr_osd_upgrade_check_reduction_factor`` to progressively reduce the set of
OSDs to check until a safe set is found. Note that its default value is on the
higher side to help determine an optimal set of OSDs to upgrade. Therefore, a
higher reduction factor will help maximize the number of OSDs to upgrade at the
cost of higher number of iterations and time taken to find the set. The converse
is true if a lower reduction factor is used. A lower value should be used only
if the command is sluggish to respond.
