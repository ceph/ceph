...
* Objecter: check the 'initialized' atomic_t safely & init constant with correct type (#9617 Josh Durgin)
* CrushWrapper: pick a ruleset same as rule_id (#9675 Xiaoxi Chen)
* DBObjectMap: lock header_lock on sync() (#9891 Samuel Just)
* osd: cache tiering: fix the atime logic of the eviction (#9915 Zhiqiang Wang)
* osd/ReplicatedPG: fix compile error (#9985 Sage Weil)
* Get the currently atime of the object in cache pool for eviction (#9985 Xinze Chi)
* osdc/Objecter: Fix a bug of dead looping in Objecter::handle_osd_map (#9986 Ding Dinghua)
* PG: always clear_primary_state on new interval, but only clear pg temp if not primary (#10059 Samuel Just)
* PG: always clear_primary_state when leaving Primary (#10059 Samuel Just)
* SimpleMessenger: allow RESETSESSION whenever we forget an endpoint (#10080 Greg Farnum)
* FileJournal: fix journalq population in do_read_entry() (#6003 Samuel Just)
* rgw: flush xml header on get acl request (#10106 Yehuda Sadeh)
* ReplicatedPG::scan_range: an object can disappear between the list and the attr get (#10150 Samuel Just)
* common: do not unlock rwlock on destruction (#10153 Federico Simoncelli)
* mon: Paxos: reset accept timeout before submiting work to the store (#10220 Joao Eduardo Luis)
* mon: MonitorDBStore: allow randomly injecting random delays on writes (#10220 Joao Eduardo Luis)
* WorkQueue: make wait timeout on empty queue configurable (#10817 Samuel Just)
* PGLog: include rollback_info_trimmed_to in (read|write)_log (#10157 Samuel Just)
* mon: MonCap: take EntityName instead when expanding profiles (#10844 Joao Eduardo Luis)
* ReplicatedPG: fail a non-blocking flush if the object is being scrubbed (#8011 Samuel Just)
...
* rados.py: keep reference to python callbacks (#10775 Josh Durgin)
* Fix memory leak in python rados bindings (#10723 Billy Olsen)
...
* mount.ceph: avoid spurious error message (#10351 Yan, Zheng)
* msg/Pipe: discard delay queue before incoming queue (#9910 Sage Weil)
* osd_types: op_queue_age_hist and fs_perf_stat should be in osd_stat_t::operator== (Samuel Just)
* mon: PGMonitor: skip zeroed osd stats on get_rule_avail() (#10257 Joao Eduardo Luis)
* mon: PGMonitor: available size 0 if no osds on pool's ruleset (Joao Eduardo Luis)
* mon: PGMonitor: fix division by zero on stats dump (Joao Eduardo Luis)
* doc: fixes to ceph command manpage (#10676 David Zafman, Nilayamdyuti Goswami)
* mds: fix assertion caused by system clock backwards (#11053 Yan, Zheng)
* Fix libstriprados::stat, use strtoll insdead of strtol (#11078 Dongmao Zhang)
* Fix libstriprados::remove, use strtoll insdead of strtol (#11078 Dongmao Zhang)
* Objecter::_op_submit_with_budget: add timeout before call (Samuel Just)
* osd: fix PG leak in SnapTrimWQ._clear() (#10421 Kefu Chai)
