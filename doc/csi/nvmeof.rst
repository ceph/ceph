...
through a direct RBD client interface on each node. Worker nodes attach volumes with
...
The gateway exposes RBD images from a RADOS pool, so the Ceph-side pool
preparation matches that of the RBD driver: see :ref:`csi-rbd` for
