...
* RGW: The radosgw-admin sub-commands dealing with orphans – ``radosgw-admin
  orphans find``, ``radosgw-admin orphans finish``, ``radosgw-admin orphans
  list-jobs –`` have been deprecated. They have not been actively maintained
  and they store intermediate results on the cluster, which could fill a
  nearly-full cluster.  They have been replaced by a tool, currently considered
  experimental, ``rgw-orphan-list``.

* Now when ``noscrub`` and/or ``nodeep-scrub`` flags are set globally or per
  pool, scheduled scrubs of the type disabled will be aborted. All user
  initiated scrubs are NOT interrupted.

* Fixed a ceph-osd crash in committed osd maps when there is a failure to
  encode the first incremental map. `issue#46443 <http://tracker.ceph.com/issues/46443>`_
...
* cephfs: client: fix snap directory atime (`pr#36169 <https://github.com/ceph/ceph/pull/>`_, Luis Henriques)
...
* cephfs: mds: cleanup uncommitted fragments before mds goes to active (`pr#35397 <https://github.com/ceph/ceph/pull/>`_, “Yan, Zheng”)
...
* rgw: cls_bucket_list_(un)ordered should clear results collection (`pr#36163 <https://github.com/ceph/ceph/pull/36163>`_, J. Eric Ivancich)
