...
host into maintenance mode no matter what. The ``--force`` and ``--offline`` flags on the exit command
can be used to to have cephadm mark a host that is in maintenance mode and offline as no longer
in maintenance mode. Note in this case if the host was to come back online, the ceph daemons
on the host would still be in the stopped state. The ``--force`` and ``--offline`` flags in the exit
