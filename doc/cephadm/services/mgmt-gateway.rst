...
service itself can be configured for high availability, ensuring that the entire system remains resilient
even if certain core components for the service fail including the `mgmt-gateway`.
...
run ``mgmt-gateway`` in HA mode, user can use either cephadm command line as in the following example:
...
the `mgmt-gateway`. Below is a configuration example for the ingress service:
