...
* ``monitor_ssl_cert``
    Monitor SSL certificate, if monitor SSL is enabled and ``monitor_certificate_source``
    is not 'cephadm-signed'. This should have the certificate .pem format.
* ``monitor_ssl_key``
    Monitor SSL key, if monitor SSL is enabled and ``monitor_certificate_source`` is not
