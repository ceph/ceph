...
.. code-block:: yaml
    ...
    spec:
      ...
      enable_stats: true
      monitor_ssl: <bool>
      monitor_certificate_source: inline                # optional: default is reuse_service_cert
...
* ``ssl_cert``
    SSL certificate, if SSL is enabled. This should have the certificate .pem format.
* ``ssl_key``
    SSL key, if SSL is enabled. This should have the key .pem format.
...
* ``monitor_ssl_cert``
    Monitor SSL certificate, if monitor SSL is enabled. This should have the certificate .pem format.
* ``monitor_ssl_key``
    Monitor SSL key, if monitor SSL is enabled. This should have the key .pem format.
* ``monitor_ip_addrs``
