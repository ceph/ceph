...
High availability
==============================
`oauth2-proxy` is designed to integrate with an external IDP hence login high availability is not responsibility of this service.
In squid release high availability for the service itself is not supported yet.
...
Before deploying `oauth2-proxy` service please remember to deploy the `mgmg-gateway` service turning on the `--enable_auth` flag. i.e:
...
Fields specific to the ``spec`` section of the `oauth2-proxy` service are described below.
...
the `mgmt-gateway` service adapting its configuration to redirect the authencation to the new deployed `oauth2-service`.
