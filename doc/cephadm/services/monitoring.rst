...
. note::
  ...
  The config keyname for the Grafana certificate has been changed from ``grafana_crt`` to ``grafana_cert``.
...
.. prompt:: bash #
  ...
  ceph config-key set mgr/cephadm/{hostname}/grafana_cert -i $PWD/certificate.pem
