...
In this case we distinguish between two different options:
...
See ``Certificate Health Monitoring`` for details on how certmgr handles user-provided certificate expiration.  
...
- If the certificate is approaching its expiration date, as determined by the
...
user-provided certificates are never renewed automatically. In either case, Cephadm alerts
...
Use `<entity-name>` as listed by `ceph orch certmgr entity ls`. For certificates with host or service scope,
...
Replace `<entity-name>` with the actual certificate name from `ceph orch certmgr entity ls`.
...
for internal modules that require automatic TLS provisioning (e.g., dashboard).