...
command, you can pass these details yourself too::
    ...
    mount -t ceph cephuser@cephfs=/ /mnt/mycephfs -o mon_host=192.168.0.1:6789,secret=AQATSKdNGBnwLhAAnNDKnH65FmVKpXZJVasUeQ==
...
and pass the file by using the option ``secretfile`` instead of ``secret``::
    ...
    mount -t ceph cephuser@cephfs=/ /mnt/mycephfs -o mon_host=192.168.0.1:6789,secretfile=/etc/ceph/cephuser.secret
...
Multiple monitor hosts can be passed as follows::
  ...
  mount -t ceph cephuser@cephfs=/ /mnt/mycephfs -o mon_host=192.168.0.1:6789/192.168.0.2:6789,secretfile=/etc/ceph/cephuser.secret

In case CephX is disabled, you can omit any credential related options::

    mount -t ceph cephuser@cephfs=/ /mnt/mycephfs -o mon_host=192.168.0.1:6789

To mount a subtree of the CephFS root, append the path to the device string::

    mount -t ceph cephuser@cephfs=/subvolume/dir1/dir2 /mnt/mycephfs -o mon_host=192.168.0.1:6789
...
following to ``/etc/fstab``::
    ...
    {name}@{fs_name}=/ {mount}/{mountpoint} ceph [mon_host={ipaddress},secret=secretkey|secretfile=/path/to/secretfile],[{mount.options}]

For example::

    cephuser@cephfs=/     /mnt/ceph    ceph    mon_host=192.168.0.1:6789,noatime,_netdev    0       2
