...
.. prompt:: bash $ auto
  ...
  $ ceph fs subvolume quiesce --set-id myset1 <vol_name> <[group_name/]sub_name...> --await
  # perform actions while the IO pause is active, like taking snapshots
  $ ceph fs subvolume quiesce --set-id myset1 --release --await
  # if successful, all members of the set were confirmed as still in pause and released from such

The ``subvolume quiesce`` functionality is itself based on top of a lower level QuiesceDb service maintained by the MDS
...
.. prompt:: bash $ auto
  ...
  $ ceph fs subvolume quiesce a sub1 --set-id=unique-id
  {
      "epoch": 3,
      "db_version": 1,
      "sets": {
...
.. prompt:: bash $ auto
  ...
  $ ceph fs subvolume quiesce a --set-id=unique-id --timeout=10 > /dev/null
  Error EPERM:  
...
.. prompt:: bash $ auto
  ...
  $ ceph fs subvolume quiesce a sub1 --timeout 60
  {
      "epoch": 3,
      "db_version": 2,
      "sets": {
...
.. prompt:: bash $ auto
  ...
  $ ceph fs subvolume quiesce a --set-id 8988b419 --include sub2 sub3
  {
      "epoch": 3,
      "db_version": 3,
      "sets": {
...
.. prompt:: bash $ auto
  ...
  $ time ceph fs subvolume quiesce a sub1 --timeout=10 --await-for=2
  {
      "epoch": 6,
      "db_version": 3,
      "sets": {
  ...
  Error EINPROGRESS: 
  ceph fs subvolume quiesce a sub1 --timeout=10 --await-for=2  0.41s user 0.04s system 17% cpu 2.563 total
...
.. prompt:: bash $ auto
  ...
  $ set -e   # (1)
  $ ceph fs subvolume quiesce a sub1 sub2 sub3 --timeout=30 --expiration=10 --set-id="snapshots" --await # (2)
  $ ceph fs subvolume snapshot create a sub1 snap1-sub1  # (3)
  $ ceph fs subvolume quiesce a --set-id="snapshots" --await  # (4)
  $ ceph fs subvolume snapshot create a sub2 snap1-sub2  # (3)
  $ ceph fs subvolume quiesce a --set-id="snapshots" --await  # (4)
  $ ceph fs subvolume snapshot create a sub3 snap1-sub3  # (3)
  $ ceph fs subvolume quiesce a --set-id="snapshots" --release --await  # (5)
...
.. prompt:: bash $ auto
  ...
  $ ceph fs subvolume quiesce a sub1 sub2 sub3 --timeout=30 --expiration=60 --set-id="snapshots" --await  # (1)
...
.. prompt:: bash $ auto
  ...
  $ ceph fs subvolume quiesce a --set-id="snapshots" --exclude sub3
...
.. prompt:: bash $ auto
  ...
  $ ceph fs subvolume quiesce a --set-id="snapshots" --release --await --if-version=13 # (5)
...
.. prompt:: bash $ auto
  ...
  $ ceph fs subvolume quiesce a sub1 sub2 sub3 --set-id="external-id" --if-version=0
