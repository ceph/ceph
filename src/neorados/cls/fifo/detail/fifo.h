...
private:
  ...
  auto push_entries(const DoutPrefixProvider* dpp,
    ...
    return asio::async_initiate<CompletionToken, void(sys::error_code, int)>
	 ...
	 try {
	   ...
	   // ldpp_dout_fmt(dpp, 10, "{}:{}: push entries failed: ",
    ...
    return asio::async_initiate<
	 ...
	 try {
	   ...
	   if (e.code() != sys::errc::no_such_file_or_directory) {
	     // ldpp_dout_fmt(dpp, 10, "{}:{} list entries failed: {}",
	     // 		   __PRETTY_FUNCTION__, __LINE__, e.what());
	   }
  ...
  auto read_meta(const DoutPrefixProvider* dpp,
    ...
    return asio::async_initiate<CompletionToken, void(sys::error_code)>
	 ...
	 // ldpp_dout_fmt(dpp, 20, "{}:{} entering", __PRETTY_FUNCTION__,
	 // 	       __LINE__);
	 ...
	 // ldpp_dout_fmt(dpp, 5, "{}:{} read_meta failed: {}",
	 // 	       __PRETTY_FUNCTION__, __LINE__, e.what());
	 co_return e.code();
       }
       co_return sys::error_code{};
  ...
  void apply_update(const DoutPrefixProvider *dpp,
    ...
    // ldpp_dout_fmt(dpp, 20, "{}:{} entering", __PRETTY_FUNCTION__, __LINE__);
    std::unique_lock l(m);
    if (objv != info->version) {
      // ldpp_dout_fmt(dpp, 10, "{}:{}: version mismatch, canceling",
  ...
  auto update_meta(const DoutPrefixProvider* dpp,
    ...
    return asio::async_initiate<CompletionToken, void(sys::error_code, bool)>
	 ...
	 if (canceled) {
           // ldpp_dout_fmt(dpp, 20, "{}:{} canceled", __PRETTY_FUNCTION__,
	   // 		 __LINE__);
	 }
	 co_return {sys::error_code{}, canceled};
       } catch (const sys::system_error& e) {
	 // ldpp_dout_fmt(dpp, 10, "{}:{} failed with error: {}",
	 // 	       __PRETTY_FUNCTION__, __LINE__, e.what());
  ...
  auto process_journal(const DoutPrefixProvider* dpp,
    ...
    return asio::async_initiate<CompletionToken, void(sys::error_code)>
	 ...
	 for (auto& entry : tmpjournal) {
	   // ldpp_dout_fmt(dpp, 20, "{}:{} processing entry: entry=",
	   // 		 __PRETTY_FUNCTION__, __LINE__, entry);
	   switch (entry.op) {
	     using enum fifo::journal_entry::Op;
	   case create:
	     // ldpp_dout_fmt(dpp, 10, "{}:{} Creating part {}",
	     // 		   __PRETTY_FUNCTION__, __LINE__
	     // 		   << entry.part_num);
	     ...
	     break;
	   case set_head:
	     // ldpp_dout_fmt(dpp, 10, "{}:{} Setting head to {}",
	     // 		   __PRETTY_FUNCTION__, __LINE__, entry.part_num);
	   ...
	   case remove:
	     try {
	       // ldpp_dout_fmt(dpp, 10, "{}:{}  Removing part {}",
	       // 		     __PRETTY_FUNCTION__, __LINE__, entry.part_num);
	     ...
	     break;
	   default:
	     // ldpp_dout_fmt(dpp, 1, "{}:{} unknown journaled op: entry={}",
	     // 		   __PRETTY_FUNCTION__, __LINE__, entry);
	 ...
	 for (auto i = 0; canceled && i < MAX_RACE_RETRIES; ++i) {
	   // ldpp_dout_fmt(dpp, 20, "{}:{} postprocessing: i={}",
	   ...
	   std::optional<std::int64_t> max_part_num;
	     ...
	     max_part_num = new_max;
           ...
           if (processed.empty() && !tail_part_num && !max_part_num) {
             // ldpp_dout_fmt(dpp, 20, "{}:{} nothing to update any more: i={}",
	     // 		   __PRETTY_FUNCTION__, __LINE__, i);
             canceled = false;
             break;
	   }
	   ...
	   // ldpp_dout_fmt(dpp, 10, "{}:{} Calling update_meta: update=",
	   // 		 __PRETTY_FUNCTION__, __LINE__, u);

	   canceled = co_await f->update_meta(dpp, u, objv, asio::deferred);
	   if (canceled) {
	     std::vector<fifo::journal_entry> new_processed;
	     std::unique_lock l(f->m);
	     // ldpp_dout_fmt(dpp, 20, "{}:{} update canceled, retrying: i=",
	     // 		   __PRETTY_FUNCTION__, __LINE__, i);
	 ...
	 if (canceled) {
	   // ldpp_dout_fmt(dpp, 5, "{}:{} canceled too many times, giving up",
	   // 		 __PRETTY_FUNCTION__, __LINE__);
	   throw sys::system_error(ECANCELED, sys::generic_category());
	 }
       } catch (const sys::system_error& e) {
	 // ldpp_dout_fmt(dpp, 3, "{}:{} process_journal failed: {}",
	 // 	       __PRETTY_FUNCTION__, __LINE__, e.what());
	 co_return e.code();
       }
       co_return sys::error_code{};
    ...
    return asio::async_initiate<CompletionToken, void(sys::error_code)>
	 ...
	 try {
	   ...
	   if (f->info.journal.contains({create, new_part_num}) &&
	     ...
	     l.unlock();
	     // ldpp_dout_fmt(dpp, 5, "{}:{} new part journaled, but not processed",
	     // 		   __PRETTY_FUNCTION__, __LINE__);
	   ...
	   if (is_head) {
	     // ldpp_dout_fmt(dpp, 20, "{}:{} needs new head", __PRETTY_FUNCTION__,
	     // 		   __LINE__);
	   ...
	   for (auto i = 0; canceled && i < MAX_RACE_RETRIES; ++i) {
	     canceled = false;
	     // ldpp_dout_fmt(dpp, 20, "{}:{} updating metadata: i={}",
	     // 		   __PRETTY_FUNCTION__, __LINE__, i);
	   ...
	   // ldpp_dout_fmt(dpp, 5, "{}:{} prepare_new_part failed: {}",
	   // 		 __PRETTY_FUNCTION__, __LINE__, e.what());
	   co_return e.code();
	 }
       }, rados.get_executor()),
    ...
    return asio::async_initiate<CompletionToken, void(sys::error_code)>
	 ...
	 try {
	   ...
	   if (max_push_part_num < new_head_part_num) {
	     // ldpp_dout_fmt(dpp, 20, "{}:{} need new part",
	     // 		   __PRETTY_FUNCTION__, __LINE__);
	   ...
	   // ldpp_dout_fmt(dpp, 3, "{}:{} prepare_new_head failed: {}",
	   // 		 __PRETTY_FUNCTION__, __LINE__, e.what());
	   co_return e.code();
	 }
       }, rados.get_executor()),
    ...
    return asio::async_initiate<CompletionToken,
	 ...
	 try {
	   ...
	   if (!f->info.journal.empty()) {
	     // ldpp_dout_fmt(dpp, 20, "{}:{} processing leftover journal",
	     // 		   __PRETTY_FUNCTION__, __LINE__);
	     co_await f->process_journal(dpp, asio::deferred);
	   }
	   co_return sys::error_code{};
	   ...
	   if (!probe ||
	     ...
	     // ldpp_dout_fmt(dpp, 5, "{}:{} open failed: {}:", __PRETTY_FUNCTION__,
	     // 		   __LINE__, e.what());
	   }
	   co_return e.code();
	 }
