...
make_bi_shards_mgr(std::string_view& composed_marker, int shard_id) noexcept {
  ...
  for (auto shard : ceph::split(composed_marker, SHARDS_SEPARATOR)) {
	...
	add(shard_id, shard);
      ...
      auto shardno = ceph::parse<int>(shard_str);
      if (!shardno) {
	return std::nullopt;
      }
...
void obj_check_mtime(WriteOp& o, real_time mtime, bool high_precision_time,
...
void obj_check_attrs_prefix(WriteOp& o, std::string_view prefix,
			    bool fail_if_exist)
{
  bufferlist in;
  rgw_cls_obj_check_attrs_prefix call;
  call.check_prefix = std::string(prefix);
  call.fail_if_exist = fail_if_exist;
  encode(call, in);
  o.exec(RGW_CLASS, RGW_OBJ_CHECK_ATTRS_PREFIX, in);
}

// TODO: Do not use std::list for this.

void remove_obj(WriteOp& o, std::list<string>&& keep_attr_prefixes)
{
  cb::list in;
...
void obj_store_pg_ver(WriteOp& o, std::string_view attr)
{
  bufferlist in;
