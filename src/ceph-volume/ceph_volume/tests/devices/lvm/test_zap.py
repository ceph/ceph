...
from mock.mock import patch, call, Mock, MagicMock
...
class TestZap:
    ...
    def test_no_lvs_and_raw_found_that_match_id(self, monkeypatch, device_info):
    ...
    @patch('ceph_volume.devices.raw.list.List.filter_lvm_osd_devices', Mock(return_value='/dev/sdb'))
    @patch('ceph_volume.process.call', Mock(side_effect=process_call))
    def test_no_lvs_and_raw_found_that_match_fsid(self, monkeypatch):
    ...
    @patch('ceph_volume.devices.raw.list.List.filter_lvm_osd_devices', Mock(return_value='/dev/sdb'))
    @patch('ceph_volume.process.call', Mock(side_effect=process_call))
    def test_no_lvs_and_raw_found_that_match_id_fsid(self, monkeypatch):
        tags = 'ceph.osd_id=9,ceph.osd_fsid=asdf-lkjh,ceph.journal_uuid=x,'+\
               'ceph.type=data'
        osd = api.Volume(lv_name='volume1', lv_uuid='y', vg_name='vg',
                         lv_tags=tags, lv_path='/dev/VolGroup/lv')
    ...
    def test_no_ceph_lvs_and_no_ceph_raw_found(self, monkeypatch):
    ...
    @patch('ceph_volume.process.call', Mock(side_effect=process_call))    
    def test_lv_is_matched_id(self, mock_zap, monkeypatch):
    ...
    @patch('ceph_volume.devices.raw.list.List.filter_lvm_osd_devices', Mock(return_value='/dev/sdb'))
    @patch('ceph_volume.process.call', Mock(side_effect=process_call))
    def test_raw_is_matched_id(self, mock_zap, monkeypatch):
        volumes = []
        monkeypatch.setattr(zap.api, 'get_lvs', lambda **kw: volumes)
    ...
    @patch('ceph_volume.devices.lvm.zap.Zap.zap')
    def test_lv_is_matched_fsid(self, mock_zap, monkeypatch):
    ...
    @patch('ceph_volume.devices.raw.list.List.filter_lvm_osd_devices', Mock(return_value='/dev/sdb'))
    @patch('ceph_volume.process.call', Mock(side_effect=process_call))
    def test_raw_is_matched_fsid(self, mock_zap, monkeypatch):
        volumes = []
        monkeypatch.setattr(zap.api, 'get_lvs', lambda **kw: volumes)
    ...
    @patch('ceph_volume.devices.lvm.zap.Zap.zap')
    def test_lv_is_matched_id_fsid(self, mock_zap, monkeypatch):
        tags = 'ceph.osd_id=0,ceph.osd_fsid=asdf-lkjh,ceph.journal_uuid=x,' +\
               'ceph.type=data'
        osd = api.Volume(lv_name='volume1', lv_uuid='y', vg_name='',
                         lv_path='/dev/VolGroup/lv', lv_tags=tags)
    ...
    @patch('ceph_volume.devices.raw.list.List.filter_lvm_osd_devices', Mock(return_value='/dev/sdb'))
    @patch('ceph_volume.process.call', Mock(side_effect=process_call))
    def test_raw_is_matched_id_fsid(self, mock_zap, monkeypatch):
        volumes = []
        monkeypatch.setattr(zap.api, 'get_lvs', lambda **kw: volumes)
    ...
    @patch('ceph_volume.process.call', Mock(side_effect=process_call))
    def test_raw_multiple_devices(self, mock_zap, monkeypatch):
        volumes = []
        monkeypatch.setattr(zap.api, 'get_lvs', lambda **kw: volumes)
        z = zap.Zap(['--osd-id', '5'])
        z.main()
