...
class TestCephLuks2:
    ...
    @patch.object(encryption.CephLuks2, 'get_label', Mock(return_value='ceph;abcd'))
    ...
    def test_config_luks2_invalid_config(self) -> None:
        with pytest.raises(AssertionError):
            encryption.CephLuks2('/dev/foo').config_luks2({'label': 'ceph;1234-abcd', 'subsystem': 'foo', 'foo': 'bar'})

    @patch('ceph_volume.util.encryption.process.call', Mock(return_value=MagicMock()))
    def test_config_luks2_invalid_config_keys(self) -> None:
        with pytest.raises(AssertionError):
            encryption.CephLuks2('/dev/foo').config_luks2({'fake': 'ceph;1234-abcd', 'subsystem': 'foo'})

    @patch('ceph_volume.util.encryption.process.call')
    def test_config_luks2_ok(self, m_call: Any) -> None:
        m_call.return_value = ('', '', 0)
        encryption.CephLuks2('/dev/foo').config_luks2({'label': 'ceph;1234-abcd', 'subsystem': 'foo'})
        assert m_call.mock_calls == [call(['cryptsetup', 'config', '/dev/foo', '--label', 'ceph;1234-abcd', '--subsystem', 'foo'], verbose_on_failure=False)]
