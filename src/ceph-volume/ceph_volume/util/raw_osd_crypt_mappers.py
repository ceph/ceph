...
class RawOsdCryptMappers:
    def __init__(
        ...
        self.cluster_name = cluster_name or conf.cluster or 'ceph'
        self._cluster_context_applied = False
        self._mapper_names = self._build_mapper_names()

    def _ensure_cluster_context(self) -> None:
        if self._cluster_context_applied:
            return
        self.credentials.apply_cluster_context(self.cluster_name)
        self._cluster_context_applied = True
    ...
    def close(self) -> None:
        for role, _device in self._role_devices():
