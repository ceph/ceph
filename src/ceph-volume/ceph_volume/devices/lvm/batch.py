...
class Batch(object):
    ...
    def get_deployment_layout(self) -> List["OSD"]:
        ...
        if fast_devices:
            ...
            for osd in plan:
                if self.args.block_db_size is not None and not self.args.db_devices:
                    for i, fast_alloc in enumerate(fast_allocations):
                        if osd.data.path == fast_alloc[0]:
...
def get_physical_osds(devices: List[device.Device], args: argparse.Namespace) -> List[Batch.OSD]:
    ...
    for dev in devices:
        ...
        if dev.available_lvm:
            ...
            if args.block_db_size is not None and not args.db_devices:
            ...
            if not args.db_devices and args.block_db_size is not None:
