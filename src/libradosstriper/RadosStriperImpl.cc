 ...
 */
struct CompletionData : RefCountedObjectSubType<CompletionData> {
  /// complete method
  void complete(int r);
...
(libradosstriper::RadosStriperImpl* striper,
 ...
 librados::AioCompletionImpl *userCompletion) :
  RefCountedObjectSubType<CompletionData>(striper->cct()),
 ...
 */
struct ReadCompletionData : RefCountedObjectInstance<ReadCompletionData, CompletionData> {
...
(libradosstriper::RadosStriperImpl* striper,
 ...
 std::vector<ObjectExtent>* extents,
 std::vector<bufferlist>* resultbl) :
  RefCountedObjectInstance<ReadCompletionData, CompletionData>(striper, soid, lockCookie, userCompletion),
 ...
 */
struct WriteCompletionData : RefCountedObjectInstance<WriteCompletionData, CompletionData> {
...
(libradosstriper::RadosStriperImpl* striper,
 ...
 const std::string& lockCookie,
 librados::AioCompletionImpl *userCompletion) :
  RefCountedObjectInstance<WriteCompletionData, CompletionData>(striper, soid, lockCookie, userCompletion),
...
struct RemoveCompletionData : RefCountedObjectInstance<RemoveCompletionData, CompletionData> {
  /// removal flags
  int flags;
...
private:
  ...
  RemoveCompletionData(libradosstriper::RadosStriperImpl * striper,
		       ...
		       librados::AioCompletionImpl *userCompletion,
		       int flags = 0) :
  RefCountedObjectInstance<RemoveCompletionData, CompletionData>(striper, soid, lockCookie, userCompletion), flags(flags) {}
};
 ...
 */
struct TruncateCompletionData : RefCountedObjectInstance<TruncateCompletionData> {
  /// striper to be used
  libradosstriper::RadosStriperImpl *m_striper;
...
private:
  ...
  TruncateCompletionData(libradosstriper::RadosStriperImpl* striper,
			 const std::string& soid,
			 uint64_t size) :
    RefCountedObjectInstance<TruncateCompletionData>(striper->cct()),
 ...
 */
struct RadosReadCompletionData : RefCountedObjectInstance<RadosReadCompletionData> {
...
private:
  ...
  RadosReadCompletionData(MultiAioCompletionImplPtr multiAioCompl,
			  ...
			  bufferlist *bl,
			  CephContext *context) :
    RefCountedObjectInstance<RadosReadCompletionData>(context),
 ...
 */
struct BasicStatCompletionData : RefCountedObjectSubType<BasicStatCompletionData, CompletionData> {
...
protected:
  ...
  BasicStatCompletionData(libradosstriper::RadosStriperImpl* striper,
			  ...
			  uint64_t *psize) :
    RefCountedObjectSubType<BasicStatCompletionData, CompletionData>(striper, soid, "", userCompletion),
...
struct StatCompletionData : RefCountedObjectInstance<StatCompletionData<TimeType>, BasicStatCompletionData> {
  // where to store the file time
  TimeType *m_pmtime;
private:
  friend typename RefCountedObjectInstance<StatCompletionData<TimeType>, BasicStatCompletionData>::factory;
  /// constructor
  StatCompletionData<TimeType>(libradosstriper::RadosStriperImpl* striper,
		     ...
		     uint64_t *psize,
		     TimeType *pmtime) :
    RefCountedObjectInstance<StatCompletionData<TimeType>, BasicStatCompletionData>(striper, soid, userCompletion, multiCompletion, psize),
    m_pmtime(pmtime) {};
};
 ...
 */
struct RadosRemoveCompletionData : RefCountedObjectInstance<RadosRemoveCompletionData> {
  /// the multi asynch io completion object to be used
  MultiAioCompletionImplPtr m_multiAioCompl;
private:
  friend factory;
  ...
  RadosRemoveCompletionData(MultiAioCompletionImplPtr multiAioCompl,
			    CephContext *context) :
    RefCountedObjectInstance<RadosRemoveCompletionData>(context),
...
{
  auto cdata = ReadCompletionData::ref(static_cast<ReadCompletionData*>(arg), false);
...
{
  auto data = RadosReadCompletionData::ref(static_cast<RadosReadCompletionData*>(arg), false);
						...
						uint64_t off)
{
  ...
  auto cdata = ReadCompletionData::create(this, soid, lockCookie, c, bl, extents, resultbl);
  c->is_read = true;
  c->io = m_ioCtxImpl;
  ...
  for (vector<ObjectExtent>::iterator p = extents->begin(); p != extents->end(); ++p) {
    ...
    // will release one
    auto data = RadosReadCompletionData::create(nc, p->length, oid_bl, cct());
...
static void striper_stat_aio_stat_complete(rados_completion_t c, void *arg) {
  auto data = BasicStatCompletionData::ref(static_cast<BasicStatCompletionData*>(arg), false);
  int rc = rados_aio_get_return_value(c);
  if (rc == -ENOENT) {
 ...
 typename libradosstriper::RadosStriperImpl::StatFunction<TimeType>::Type statFunction)
{
  ...
  std::string firstObjOid = getObjectId(soid, 0);
  auto cdata = StatCompletionData<TimeType>::create(this, firstObjOid, c, multi_completion.get(), psize, pmtime);
...
{
  auto cdata = RadosRemoveCompletionData::ref(static_cast<RadosRemoveCompletionData*>(arg), false);
  int rc = rados_aio_get_return_value(c);
  // in case the object did not exist, it means we had a sparse file, all is fine
  if (rc == -ENOENT) {
    rc = 0;
...
{
  auto cdata = RemoveCompletionData::ref(static_cast<RemoveCompletionData*>(arg), false);
  libradosstriper::MultiAioCompletionImpl *comp =
    reinterpret_cast<libradosstriper::MultiAioCompletionImpl*>(c);
						  ...
						  int flags)
{
  ...
  auto cdata = RemoveCompletionData::create(this, soid, lockCookie, c, flags);
  MultiAioCompletionImplPtr multi_completion{
    new libradosstriper::MultiAioCompletionImpl, false};
 ...
 int flags)
{
  ...
  try {
    ...
    for (int i = nb_objects-1; i >= 1; i--) {
      ...
      auto data = RadosRemoveCompletionData::create(multi_completion, cct());
      librados::AioCompletion *rados_completion =
...
{
  auto cdata = WriteCompletionData::ref(static_cast<WriteCompletionData*>(arg), false);
  libradosstriper::MultiAioCompletionImpl *comp =
    reinterpret_cast<libradosstriper::MultiAioCompletionImpl*>(c);
  cdata->complete_unlock(comp->rval);
}
...
int libradosstriper::RadosStriperImpl::write_in_open_object(const std::string& soid,
  ...
  auto cdata = WriteCompletionData::create(this, soid, lockCookie, nullptr);
...
int libradosstriper::RadosStriperImpl::aio_write_in_open_object(const std::string& soid,
  ...
  // we need 3 references as striper_write_aio_req_complete will release two and
  // striper_write_aio_req_safe will release one
  auto cdata = WriteCompletionData::create(this, soid, lockCookie, c);
  m_ioCtxImpl->get();
  c->io = m_ioCtxImpl;
  // create a completion object for the unlocking of the striped object at the end of the write
  librados::AioCompletion *unlock_completion =
...
{
  auto cdata = TruncateCompletionData::ref(static_cast<TruncateCompletionData*>(arg), false);
  libradosstriper::MultiAioCompletionImpl *comp =
    reinterpret_cast<libradosstriper::MultiAioCompletionImpl*>(c);
  if (0 == comp->rval) {
...
int libradosstriper::RadosStriperImpl::truncate(const std::string& soid,
						...
						ceph_file_layout &layout) 
{
  auto cdata = TruncateCompletionData::create(this, soid, size);
