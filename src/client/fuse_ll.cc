...
static void fuse_ll_write(fuse_req_t req, fuse_ino_t ino, const char *buf, size_t size, off_t off, struct fuse_file_info *fi) {
	CephFuse::Handle *cfuse = fuse_ll_req_prepare(req);
	Fh *fh = reinterpret_cast<Fh*>(fi->fh);
...
static void fuse_ll_ioctl(fuse_req_t req, fuse_ino_t ino, int cmd, void *arg, struct fuse_file_info *fi, unsigned flags, const void *in_buf, size_t in_bufsz, size_t out_bufsz) {
	CephFuse::Handle *cfuse = fuse_ll_req_prepare(req);
...
static void fuse_ll_setlk(fuse_req_t req, fuse_ino_t ino, struct fuse_file_info *fi, struct flock *lock, int sleep) {
	...
	if (!cfuse->client->cct->_conf->fuse_multithreaded && sleep && lock->l_type != F_UNLCK) {
...
static void dentry_invalidate_cb(void *handle, vinodeno_t dirino, vinodeno_t ino, string& name) {
	CephFuse::Handle *cfuse = (CephFuse::Handle *) handle;
...
static void do_init(void *data, fuse_conn_info *conn) {
...
#if !defined(DARWIN)
	if (!client->cct->_conf->fuse_default_permissions && client->ll_handle_umask()) {
	...
	if (cfuse->fd_on_success) {
		...
		if (err) {
			derr
						<< "fuse_ll: do_init: safe_write failed with error " << cpp_strerror(err) << dendl;ceph_abort();
		}
...
const static struct fuse_lowlevel_ops fuse_ll_oper = { init: do_init, destroy: 0, lookup: fuse_ll_lookup, forget: fuse_ll_forget, getattr: fuse_ll_getattr,
		setattr: fuse_ll_setattr, readlink: fuse_ll_readlink, mknod: fuse_ll_mknod, mkdir: fuse_ll_mkdir, unlink: fuse_ll_unlink, rmdir: fuse_ll_rmdir, symlink: fuse_ll_symlink,
		rename: fuse_ll_rename, link: fuse_ll_link, open: fuse_ll_open, read: fuse_ll_read, write: fuse_ll_write, flush: fuse_ll_flush, release: fuse_ll_release, fsync
				: fuse_ll_fsync, opendir: fuse_ll_opendir, readdir: fuse_ll_readdir, releasedir: fuse_ll_releasedir, fsyncdir: fuse_ll_fsyncdir, statfs: fuse_ll_statfs, setxattr
				: fuse_ll_setxattr, getxattr: fuse_ll_getxattr, listxattr: fuse_ll_listxattr, removexattr: fuse_ll_removexattr, access: fuse_ll_access, create: fuse_ll_create,
		getlk: fuse_ll_getlk, setlk: fuse_ll_setlk, bmap: 0,
#if FUSE_VERSION >= FUSE_MAKE_VERSION(2, 8)
#ifdef FUSE_IOCTL_COMPAT
		ioctl: fuse_ll_ioctl,
#else
...
#endif
#if FUSE_VERSION >= FUSE_MAKE_VERSION(2, 9)
		write_buf: 0, retrieve_reply: 0, forget_multi: 0, flock: fuse_ll_flock,
#endif
#if FUSE_VERSION > FUSE_MAKE_VERSION(2, 9)
		fallocate: fuse_ll_fallocate
                                                                                                        ...
                                                                                                        (Client *c, int fd) :
		fd_on_success(fd), client(c), ch(NULL), se(NULL), mountpoint(NULL), stag_lock("fuse_ll.cc stag_lock"), last_stag(0) {
...
int CephFuse::Handle::init(int argc, const char *argv[]) {
	...
	derr
				<< "init, newargv = " << newargv << " newargc=" << newargc << dendl;
...
int CephFuse::Handle::start() {
	...
	if (!ch) {
		derr
					<< "fuse_mount(mountpoint=" << mountpoint << ") failed." << dendl;
		return EIO;
	}
	...
	struct client_callback_args args = { handle: this, ino_cb: client->cct->_conf->fuse_use_invalidate_cb ? ino_invalidate_cb : NULL, dentry_cb: dentry_invalidate_cb,
