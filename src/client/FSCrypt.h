...
static inline uint64_t fscrypt_block_end(uint64_t ofs) {
  return (ofs & ~(FSCRYPT_BLOCK_SIZE - 1)) + FSCRYPT_BLOCK_SIZE;
}
...
public:
  ...
  int find(const struct ceph_fscrypt_key_identifier& id, FSCryptKeyHandlerRef& key, bool create_if_not_found);
...
struct FSCryptKeyValidator {
  ...
  FSCryptKeyValidator(FSCryptKeyHandlerRef& kh, int64_t e) : handler(kh), epoch(e) {}
