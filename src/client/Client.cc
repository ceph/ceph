...
static const auto locale = boost::locale::generator()("");
...
bool Client::_wrap_name(const Inode& diri, std::string& dname, std::string& alternate_name)
{
  ...
  if (diri.has_charmap()) {
    ...
    if (normalization.size()) {
      ...
      normalized = boost::locale::normalize(encoded, norm_type, locale);
    ...
    if (is_insensitive) {
      ...
      folded = boost::locale::fold_case(normalized, locale);
