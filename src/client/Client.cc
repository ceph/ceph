...
/*
 * The physical (on-disk) end of a write extent.  For fscrypt enabled
 * files in->size and in->max_size are encrypted (physical) sizes, i.e.
 * the logical size rounded up to the fscrypt block size, while the
 * write offsets are logical.  Round the logical extent end up, so that
 * it can be compared against, and fed into, max_size.
 */
static uint64_t write_extent_end(Inode *in, loff_t endoff)
{
  if (in->is_fscrypt_enabled()) {
#if defined(__linux__)
    return fscrypt_next_block_start(endoff);
#endif
  }
  return (uint64_t)endoff;
}
  ...
  while (1) {
    ...
    if (have & need & CEPH_CAP_FILE_WR) {
	 ...
	 if ((endoff >= (loff_t)in->max_size ||
	   ...
	   in->wanted_max_size = write_extent_end(in, endoff);
	 }
	               ...
	               Context *onfinish, bool do_fsync, bool syncdataonly)
{
  ...
  if (is_append) {
    ...
    in->wanted_max_size = std::max(in->wanted_max_size,
				   std::max(write_extent_end(in, endoff),
					    in->max_size + 1));
    if (in->wanted_max_size > in->max_size &&
        in->wanted_max_size > in->requested_max_size)
      check_caps(in, 0);
    ...
    if (eof != (uint64_t)offset) {
       ...
       */
      uint64_t extent_end = write_extent_end(in, endoff);
