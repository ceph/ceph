...
{
  // TODO change API to disallow setting alternate_name by the application
  ldout(cct, 20) << __func__ << ": (" << dname << ", " << alternate_name << ") on " << diri << dendl;
  if (diri.has_casesensitivity()) {
    auto& cs = diri.get_casesensitivity();
    ldout(cct, 25) << __func__ << ":  " << cs << dendl;
    if (cs.is_insensitive()) {
      auto&& folder = cs.get_casefolder();
      std::string folded = dname;
      if (folder == "locale"sv) {
...
int Client::statxat(int dirfd, const char *relpath,
  ...
  InodeRef in;
  r = path_walk(cwd, filepath(relpath), &in, perms, !(flags & AT_SYMLINK_NOFOLLOW), mask);
  if (r < 0) {
    return r;
  }
  r = _getattr(in, mask, perms);
		   ...
		   const UserPerm& perms, InodeRef *inp)
{
  ...
  if (should_check_perms()) {
    int r = may_create(wdr.diri.get(), perms); // FIXME make sure other callsites use wdr.diri; but diri may be nullptr :/
    if (r < 0)
      return r;
  }
