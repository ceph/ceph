...
int Client::snap_metadata_update(const char* path,
  const map<string, string>& new_metadata, const UserPerm &perms)
{
  RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
  if (!mref_reader.is_state_satisfied())
    return -ENOTCONN;
  ...
  ldout(cct, 10) << "here123 printing snap metadata passed by the user" << dendl;
  for (auto& [key, value] : new_metadata) {
    ldout(cct, 10) << "here123 key " << key << dendl;
    ldout(cct, 10) << "here123 value " << value << dendl;
  }

  MetaRequest *req = new MetaRequest(CEPH_MDS_OP_SNAP_METADATA_UPDATE);
  ...
  ldout(cct, 10) << "_snap_metadata_update: making request" << dendl;
  int res = make_request(req, perms, &wdr.target);
  ldout(cct, 10) << "_snap_metadata_update: result is " << res << dendl;

  trim_cache();

  ldout(cct, 8) << "_snap_metadata_update(" << wdr.getpath() << ", " << perms
                << ") = " << res << dendl;
