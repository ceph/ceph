...
Context *SnapshotRollbackRequest<I>::handle_rollback_objects(int *result) {
  ...
  return send_set_snap_location();
}

template <typename I>
Context *SnapshotRollbackRequest<I>::send_set_snap_location() {
  I &image_ctx = this->m_image_ctx;
  ...
  librados::ObjectWriteOperation op;
  cls_client::set_snapshot_location(&op, m_snap_id);

  using klass = SnapshotRollbackRequest<I>;
  librados::AioCompletion *comp =
    create_rados_safe_callback<klass, &klass::handle_set_snap_location>(this);
...
template <typename I>
Context *SnapshotRollbackRequest<I>::handle_set_snap_location(int *result) {
  I &image_ctx = this->m_image_ctx;
  CephContext *cct = image_ctx.cct;
  ldout(cct, 5) << this << " " << __func__ << ": r=" << *result << dendl;
