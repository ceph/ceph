...
struct ImageRequestWQ<I>::C_ThrottlingTask : public Context {
  ...
  C_ThrottlingTask(ImageRequestWQ *work_queue, 
		           ImageQosStatus *qos_status)
    : work_queue(work_queue), qos_status(qos_status) {
  }
  void finish(int r) override {
    work_queue->update_throttling(r, qos_status);
  }
};
...
template <typename I>
void ImageRequestWQ<I>::schedule_throttling_task(ImageQosStatus* qos_status, utime_t ts) {
  Mutex::Locker timer_locker(*m_image_ctx.m_timer_lock);
  C_ThrottlingTask* throttling_task = new C_ThrottlingTask(this, qos_status);
  qos_status->set_throttling_flag(true);
...
template <typename I> 
void ImageRequestWQ<I>::update_throttling(int r, ImageQosStatus* qos_status) {
  // disable throttling
  qos_status->set_throttling_flag(false);
...
bool ImageRequestWQ<I>::reserve_throttle_iops(bool is_write) {
  bool res = false;
  uint64_t max_iops = 0;
  ImageQosStatus* qos_status = NULL;  

  if (is_write) {
    max_iops = m_image_ctx.max_write_iops;
    qos_status = &write_qos_status;
  } else {
    max_iops = m_image_ctx.max_read_iops;
    qos_status = &read_qos_status;
  }
  
  if (max_iops && (qos_status->get_throttling_flag() || check_throttle_iops(qos_status, max_iops))) {
    res = true;
  }
...
template <typename I>
bool ImageRequestWQ<I>::check_throttle_iops(ImageQosStatus* qos_status, uint64_t max_iops) {
  bool res = false;
  utime_t now = ceph_clock_now();
  utime_t cur_limit = qos_status->get_prev_limit();
  cur_limit += (double)(1.0 / max_iops);
  cur_limit = (cur_limit > now ? cur_limit : now);
  if (cur_limit > now) {
    ldout(m_image_ctx.cct, 0) << "we ahead by "
                               << cur_limit - now
                               << ", active throttling" << dendl;
    schedule_throttling_task(qos_status, cur_limit);
    res = true;
  }
  return res;
}
