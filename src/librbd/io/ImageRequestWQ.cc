...
bool ImageRequestWQ::is_lock_request_needed() const {
  RWLock::RLocker locker(m_lock);
  return (m_queued_writes.load() > 0 ||
          (m_require_lock_on_read && m_queued_reads.load() > 0));
}
...
void ImageRequestWQ::block_writes(Context *on_blocked) {
...

  {
    ...
    if (!m_write_blocker_contexts.empty() || m_in_progress_writes.load() > 0) {
...
void ImageRequestWQ::finish_queued_op(ImageRequest<> *req) {
  RWLock::RLocker locker(m_lock);
  if (req->is_write_op()) {
    assert(m_queued_writes.load() > 0);
    m_queued_writes--;
  } else {
    assert(m_queued_reads.load() > 0);
    m_queued_reads--;
  }
}
...
void ImageRequestWQ::finish_in_progress_write() {
  ...
  {
    RWLock::RLocker locker(m_lock);
    assert(m_in_progress_writes.load() > 0);
