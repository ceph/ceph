...
void mirror_group_status_cpp_to_c(
    const librbd::mirror_group_status_t &cpp_status,
    rbd_mirror_group_status_t *c_status) {
  c_status->name = strdup(cpp_status.name.c_str());
...
#pragma GCC diagnostic pop
  ...
  int RBD::mirror_group_status_list(
      IoCtx& io_ctx, const std::string &start_id, size_t max,
  ...
  int RBD::group_remove(IoCtx& io_ctx, const char *group_name, uint32_t flags)
  {
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
    tracepoint(librbd, group_remove_enter, io_ctx.get_pool_name().c_str(),
	       io_ctx.get_id(), group_name);
    int r = librbd::api::Group<>::remove(io_ctx, group_name, flags);
  ...
  int RBD::mirror_group_create_snapshot(IoCtx& group_ioctx,
    ...
    return librbd::api::Mirror<>::group_create_snapshot(group_ioctx, group_name,
                                                        flags, snap_id);
  }
  ...
  int RBD::mirror_group_get_status(IoCtx& group_ioctx, const char *group_name,
                                   ...
                                   size_t status_size) {
    if (sizeof(mirror_group_status_t) != status_size) {
      return -ERANGE;
    }
...
extern "C" int rbd_mirror_group_status_list(rados_ioctx_t p,
    const char *start_id, size_t max, char **group_ids,
    rbd_mirror_group_status_t *groups, size_t *len) {
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
  std::map<std::string, librbd::mirror_group_status_t> cpp_groups;
...
extern "C" void rbd_mirror_group_status_cleanup(
    rbd_mirror_group_status_t *status) {
  free(status->name);
...
extern "C" void rbd_mirror_group_status_list_cleanup(
                                ...
                                uint32_t flags)
{
  ...
  tracepoint(librbd, group_remove_enter, io_ctx.get_pool_name().c_str(),
             io_ctx.get_id(), name);
  int r = librbd::api::Group<>::remove(io_ctx, name, flags);
  tracepoint(librbd, group_remove_exit, r);
  return r;
}
                                                ...
                                                char **snap_id)
{
  ...
  int r = librbd::api::Mirror<>::group_create_snapshot(group_ioctx, group_name,
                                                       flags, &cpp_snap_id);
  if (r < 0) {
    return r;
  }
...
extern "C" int rbd_mirror_group_get_status(
    rados_ioctx_t group_p, const char *group_name,
    rbd_mirror_group_status_t *status, size_t status_size) {
  librados::IoCtx group_ioctx;
  librados::IoCtx::from_rados_ioctx_t(group_p, group_ioctx);

  if (sizeof(rbd_mirror_group_status_t) != status_size) {
    return -ERANGE;
  }

  librbd::mirror_group_status_t cpp_status;
