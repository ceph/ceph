...
template <typename I>
void GroupPromoteRequest<I>::unregister_images_watchers() {
  ldout(m_cct, 10) << dendl;

  auto ctx = create_context_callback<GroupPromoteRequest<I>,
        &GroupPromoteRequest<I>::handle_unregister_images_watchers>(this);

  auto gather_ctx = new C_Gather(m_cct, ctx);
  for (size_t i = 0; i < m_image_ctxs.size(); i++) {
     auto req = UnregisterImageWatchersRequest<I>::create(
...
template <typename I>
void GroupPromoteRequest<I>::handle_unregister_images_watchers(int r) {
  ldout(m_cct, 10) << "r=" << r << dendl;
...
void GroupPromoteRequest<I>::handle_acquire_exclusive_locks(int r) {
  ...
  if (r < 0) {
    ...
    for (size_t i = 0; i < m_image_ctxs.size(); ++i) {
      ...
      if (m_image_ctxs[i]->exclusive_lock != nullptr &&
          !m_image_ctxs[i]->exclusive_lock->is_lock_owner()) {
        lderr(m_image_ctxs[i]->cct) << "failed to acquire exclusive lock" << dendl;
