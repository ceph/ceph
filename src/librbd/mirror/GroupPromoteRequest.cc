...
template <typename I>
void GroupPromoteRequest<I>::remove_image_from_group() {
  ldout(m_cct, 10) << dendl;

  auto gather = new C_Gather(m_cct,
    create_context_callback<GroupPromoteRequest<I>,
      &GroupPromoteRequest<I>::handle_remove_image_from_group>(this));
...
template <typename I>
void GroupPromoteRequest<I>::handle_remove_image_from_group(int r) {
  ldout(m_cct, 10) << "r=" << r << dendl;
...
template <typename I>
void GroupPromoteRequest<I>::remove_image_request() {
  ldout(m_cct, 10) << dendl;

  auto ctx = create_context_callback<GroupPromoteRequest<I>,
    &GroupPromoteRequest<I>::handle_remove_image_request>(this);
...
template <typename I>
void GroupPromoteRequest<I>::handle_remove_image_request(int r) {
  ldout(m_cct, 10) << "r=" << r << dendl;

  if (r < 0) {
    lderr(m_cct) << "failed to remove image(s): "
                 << cpp_strerror(r) << dendl;
    release_exclusive_locks();
    return;
  }
