...
template <typename I>
void GroupPromoteRequest<I>::get_mirror_peer_list() {
  ldout(m_cct, 10) << dendl;
  ...
  auto comp = create_rados_callback<
      GroupPromoteRequest<I>,
      &GroupPromoteRequest<I>::handle_get_mirror_peer_list>(this);
  m_out_bl.clear();
...
template <typename I>
void GroupPromoteRequest<I>::handle_get_mirror_peer_list(int r) {
  ldout(m_cct, 10) << "r=" << r << dendl;
...
template <typename I>
void GroupPromoteRequest<I>::open_images() {
  ldout(m_cct, 10) << dendl;

  auto ctx = create_context_callback<
    GroupPromoteRequest<I>, &GroupPromoteRequest<I>::handle_open_images>(this);
...
template <typename I>
void GroupPromoteRequest<I>::handle_open_images(int r) {
  ldout(m_cct, 10) << "r=" << r << dendl;
  ...
  // Resize all per-image state vectors here after m_images count is known
  m_rollback_snap_ids.resize(num_images);
...
template <typename I>
void GroupPromoteRequest<I>::get_images_mirror_info() {
  ldout(m_cct, 10) << dendl;
  auto ctx = create_context_callback<
    GroupPromoteRequest<I>,
    &GroupPromoteRequest<I>::handle_get_images_mirror_info>(this);
  auto gather_ctx = new C_Gather(m_cct, ctx);
...
template <typename I>
void GroupPromoteRequest<I>::handle_get_images_mirror_info(int r) {
  ldout(m_cct, 10) << "r=" << r <<  dendl;
...
template <typename I>
void GroupPromoteRequest<I>::update_images_read_only_mask() {

  auto ctx = create_context_callback<
    GroupPromoteRequest<I>,
    &GroupPromoteRequest<I>::handle_update_images_read_only_mask>(this);
  auto gather_ctx = new C_Gather(m_cct, ctx);
...
template <typename I>
void GroupPromoteRequest<I>::handle_update_images_read_only_mask(int r) {
  ldout(m_cct, 10) << "r=" << r << dendl;
...
void GroupPromoteRequest<I>::promote_group() {
  ...
  for (size_t i = 0; i < m_images.size(); i++) {
    ...
    not_create_orphan = not_create_orphan &&
                        (m_rollback_snap_ids[i] == CEPH_NOSNAP && !requires_orphan);
  }
  if (not_create_orphan) {
    create_primary_group_snapshot();
  } else {
...
void GroupPromoteRequest<I>::handle_create_group_orphan_snapshot(int r) {
  ...
  if (r < 0) {
    lderr(m_cct) << "failed to create group orphan snapshot: "
                 << cpp_strerror(r) << dendl;
    m_ret_val = r;
    remove_incomplete_orphan_group_snapshot();
    return;
  }
  create_images_orphan_snapshots();
}
...
void GroupPromoteRequest<I>::create_images_orphan_snapshots() {
  ...
  for (size_t i = 0; i < m_image_ctxs.size(); i++) {
    auto req = snapshot::CreateNonPrimaryRequest<I>::create(
      m_image_ctxs[i], false, "", "", CEPH_NOSNAP, {}, {}, nullptr, gather_ctx->new_sub());
    req->send();
  }
  gather_ctx->activate();
}
...
void GroupPromoteRequest<I>::handle_rollback(int r) {
  ...
  if (r < 0) {
    // needs update according to suggestions
...
template <typename I>
void GroupPromoteRequest<I>::remove_incomplete_orphan_group_snapshot() {
  // needs update to remove image orphan snapshots according to suggestions
  if (!m_need_rollback) {
    close_images();
    return;
  }
  ...
  auto ctx = create_context_callback<
    GroupPromoteRequest<I>,
    &GroupPromoteRequest<I>::handle_remove_incomplete_orphan_group_snapshot>(this);

  auto req = snapshot::RemoveGroupSnapshotRequest<I>::create(m_group_ioctx,
     m_group_id, &m_orphan_group_snap, &m_image_ctxs, ctx);

  req->send();
}

template <typename I>
void GroupPromoteRequest<I>::handle_remove_incomplete_orphan_group_snapshot(int r) {
  ldout(m_cct, 10) << "r=" << r << dendl;
