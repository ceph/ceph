...
void GroupUnlinkPeerRequest<I>::process_snapshot(cls::rbd::GroupSnapshot group_snap,
  ...
  if (ns.mirror_peer_uuids.empty() ||
    ...
    // Note: avoid calling remove_peer_uuid_from_image_snaps() for CREATING
...
void GroupUnlinkPeerRequest<I>::remove_peer_uuid_from_group_snap(
  ...
  auto aio_comp = create_rados_callback(
    new LambdaContext([this, group_snap](int r) {
      handle_remove_peer_uuid_from_group_snap(r, group_snap);
    }));
...
template <typename I>
void GroupUnlinkPeerRequest<I>::handle_remove_peer_uuid_from_group_snap(
    int r, cls::rbd::GroupSnapshot group_snap) {
  ldout(m_cct, 10) << "r=" << r << dendl;
