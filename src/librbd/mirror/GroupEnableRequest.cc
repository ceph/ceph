...
void GroupEnableRequest<I>::handle_get_mirror_group(int r) {
  ...
  if (r == 0) {
    if (m_mirror_group.mirror_image_mode != m_mode) {
      ...
      lderr(m_cct) << "mirroring either in progress or was interrupted"
                   << dendl;
      r = -EINVAL;
    } else if (m_mirror_group.state == cls::rbd::MIRROR_GROUP_STATE_CREATING) {
      lderr(m_cct) << "in-progress group replication" << dendl;
      r = -EINVAL;
    } else if (
	m_mirror_group.state == cls::rbd::MIRROR_GROUP_STATE_DISABLING) {
      ...
      lderr(m_cct) << "unexpected group mirror state" << dendl;
      r = -EINVAL;
    }
