...
GroupRemoveImageRequest<I>::GroupRemoveImageRequest(I* image_ctx,
  ...
  : m_image_ctx(image_ctx), m_group_id(group_id),
    m_group_io_ctx(group_io_ctx), m_force(force), m_on_finish(on_finish),
...
void GroupRemoveImageRequest<I>::handle_get_mirror_info(int r) {
  ...
  if (!m_is_primary && !m_force) {
    lderr(m_cct) << "image not primary and force not set" << dendl;
    finish(-EINVAL);
    return;
  }
...
void GroupRemoveImageRequest<I>::handle_set_mirror_image_disabling(int r) {
  ...
  if (r == -ENOENT) {
    ldout(m_cct, 10) << "mirror already removed" << dendl;
    remove_group_from_image();
    return;
  }
...
template <typename I>
void GroupRemoveImageRequest<I>::promote_if_needed() {
  ldout(m_cct, 10) << dendl;
  ...
  auto ctx = create_context_callback<GroupRemoveImageRequest<I>,
    &GroupRemoveImageRequest<I>::handle_promote>(this);
...
template <typename I>
void GroupRemoveImageRequest<I>::handle_promote(int r) {
  ldout(m_cct, 10) << "r=" << r << dendl;
...
template <typename I>
void GroupRemoveImageRequest<I>::refresh_image() {
  ldout(m_cct, 10) << dendl;
  ...
  auto ctx = create_context_callback<GroupRemoveImageRequest<I>,
    &GroupRemoveImageRequest<I>::handle_refresh>(this);

  m_image_ctx->state->refresh(ctx);
}

template <typename I>
void GroupRemoveImageRequest<I>::handle_refresh(int r) {
  ldout(m_cct, 10) << "r=" << r << dendl;

  if (r < 0) {
    lderr(m_cct) << "refresh failed: "
                 << cpp_strerror(r) << dendl;
    finish(r);
    return;
  }
...
template <typename I>
void GroupRemoveImageRequest<I>::remove_group_from_image() {
  ldout(m_cct, 10) << dendl;
  ...
  auto comp = create_rados_callback<GroupRemoveImageRequest<I>,
    &GroupRemoveImageRequest<I>::handle_remove_group_from_image>(this);
...
template <typename I>
void GroupRemoveImageRequest<I>::handle_remove_group_from_image(int r) {
  ldout(m_cct, 10) << "r=" << r << dendl;
...
template <typename I>
void GroupRemoveImageRequest<I>::remove_snapshot_mirror_metadata() {
  ldout(m_cct, 10) << dendl;

  if (!m_mirror_uuid.empty()) {
    int r = m_image_ctx->operations->metadata_remove(
        snapshot::util::get_image_meta_key(m_mirror_uuid));

    if (r < 0 && r != -ENOENT) {
      lderr(m_cct) << "failed removing snapshot metadata: "
                   << cpp_strerror(r) << dendl;
      finish(r);
      return;
    }
