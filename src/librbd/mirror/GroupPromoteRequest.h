...
private:
   ...
   * MARK_GROUP_ORPAHN_SNAPSHOT_COMPLETE                |
   *    |                                               |
   *    v            (no watchers for all images)       |
   * CHECK_WATCHERS------------------------------->\    |
   *    |     ^                                    |    |
   *    |     |                                    |    |
   *    |     \----------------------------------\ |    |
  ...
  public:
    UpdateWatchCtx(GroupPromoteRequest *promote_request, size_t index)
      : promote_request(promote_request), m_index(index) {
    }

    void handle_notify() override {
      promote_request->handle_update_notify(m_index);
    }

  private:
    GroupPromoteRequest *promote_request;
    size_t m_index;
  };

  std::vector<UpdateWatchCtx*> m_update_watch_ctxs;
  std::vector<std::list<obj_watch_t>> m_watchers;
  std::vector<uint64_t> m_update_watcher_handles;
  std::vector<uint64_t> m_scheduler_ticks;
  ...
  void check_watchers();
  void list_watchers(size_t index);
  void handle_list_watchers(int r);

  void wait_update_notify(size_t index);
  void handle_update_notify(size_t index);
  void scheduler_unregister_update_watcher(size_t index);

  void unregister_update_watcher(size_t index);
  ...
  void remove_incomplete_orphan_group_snapshot();
  void handle_remove_incomplete_orphan_group_snapshot(int r);
