...
namespace librbd {
  ...
  void AioObjectRead::read_from_parent(
      const vector<pair<uint64_t,uint64_t> >& parent_extents)
  {
  ...
  AbstractAioObjectWrite::AbstractAioObjectWrite(
      ImageCtx *ictx, const std::string &oid, uint64_t object_no,
