...
int validate_pool(IoCtx &io_ctx, CephContext *cct) {
  if (!cct->_conf->get_val_generic("rbd_validate_pool")) {
    return 0;
  }
...
#define READ_SIZE 4096
             ...
             bool skip_mirror_enable)
  {
    ...
    if (opts.get(RBD_IMAGE_OPTION_FORMAT, &format) != 0)
      format = cct->_conf->get_val_generic("rbd_default_format");
    ...
    if (opts.get(RBD_IMAGE_OPTION_ORDER, &order) != 0 || order == 0) {
      order = cct->_conf->get_val_generic("rbd_default_order");
    }
		       ...
		       void *arg)
  {
    ...
    ZTracer::Trace trace;
    if (ictx->cct->_conf->get_val_generic("rbd_blkin_trace_all")) {
