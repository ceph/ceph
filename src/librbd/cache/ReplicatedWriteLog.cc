...
void ReplicatedWriteLog<I>::schedule_complete_op_log_entries(GenericLogOperationsT &&ops, const int result)
{
  ...
  m_async_op_tracker.start_op();
  Context *complete_ctx = new LambdaContext([this, ops=move(ops), result](int r) {
...
void ReplicatedWriteLog<I>::aio_discard(uint64_t offset, uint64_t length,
  ...
  auto *discard_req =
    new C_DiscardRequestT(*this, now, std::move(discard_extents), discard_granularity_bytes, on_finish);
  ...
  discard_req->op->on_write_persist = new LambdaContext(
...
void ReplicatedWriteLog<I>::flush_new_sync_point(C_FlushRequestT *flush_req, DeferredContexts &later) {
  ...
  to_append->m_sync_point_persist->
    set_finisher(new LambdaContext([this, flush_req](int r) {
	  if (RWL_VERBOSE_LOGGING) {
...
void ReplicatedWriteLog<I>::aio_writesame(uint64_t offset, uint64_t length,
  ...
  auto *ws_req =
    new C_WriteSameRequestT(*this, now, std::move(ws_extents), std::move(bl), fadvise_flags, on_finish);
...
void ReplicatedWriteLog<I>::aio_compare_and_write(Extents &&image_extents,
   ...
   * succeeds. */
  auto *cw_req =
    new C_CompAndWriteRequestT(*this, now, std::move(image_extents), std::move(adjusted_cmp_bl), std::move(bl),
...
void ReplicatedWriteLog<I>::new_sync_point(DeferredContexts &later) {
  ...
  new_sync_point->m_prior_log_entries_persisted->
    set_finisher(new LambdaContext([this, new_sync_point, sync_point_persist_ready](int r) {
	  if (RWL_VERBOSE_LOGGING) {
