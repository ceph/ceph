...
ImageCacheStateRWL<I>::ImageCacheStateRWL(I *image_ctx): ImageCacheState<I>(image_ctx) {
  ...
  this->m_cache_type = IMAGE_CACHE_TYPE_RWL;
...
template <typename I>
void ImageCacheStateRWL<I>::dump(Formatter *f) const {
  ImageCacheState<I>::dump(f);
  ::encode_json("rwl_host", m_host, f);
  ::encode_json("rwl_path", m_path, f);
  ::encode_json("rwl_size", m_size, f);
  ::encode_json("rwl_invalidate_on_flush", m_invalidate_on_flush, f);
}
...
ReplicatedWriteLog<I>::~ReplicatedWriteLog() {
  ldout(m_image_ctx.cct, 15) << "enter" << dendl;
  {
    ...
    std::lock_guard dispatch_locker(m_deferred_dispatch_lock);
    std::lock_guard append_locker(m_log_append_lock);
    std::lock_guard locker(m_lock);
    std::lock_guard bg_locker(m_blockguard_lock);
    std::lock_guard bl_locker(m_entry_bl_lock);
...
void ReplicatedWriteLog<I>::arm_periodic_stats() {
  ...
  if (m_periodic_stats_enabled) {
    m_timer.add_event_after(LOG_STATS_INTERVAL_SECONDS, new LambdaContext(
      [this](int r) {
