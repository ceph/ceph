...
#include "librbd/neorbdrados/RADOSImpl.h"
...
namespace std {
size_t hash<neorbdrados::Object>::operator ()(
  const neorbdrados::Object& r) const {
  static constexpr const hash<object_t> H;
  return H(*reinterpret_cast<const object_t*>(&r.impl));
}

size_t hash<neorbdrados::IOContext>::operator ()(
  const neorbdrados::IOContext& r) const {
  static constexpr const hash<int64_t> H;
  static constexpr const hash<std::string> G;
  const auto l = reinterpret_cast<const neorbdrados::IOContextImpl*>(&r.impl);
