...
public:
  ...
  inline int operator()(const cls::rbd::GroupSnapshotNamespaceMirror& snap_namespace) {
    ...
    group_snap_info2_t* group_snap)
{
  ...
  if (std::holds_alternative<cls::rbd::GroupSnapshotNamespaceMirror>(
	cls_group_snap.snapshot_namespace)) {
    group_snap->namespace_type = RBD_GROUP_SNAP_NAMESPACE_TYPE_MIRROR;
  } else {
...
template <typename I>
int Group<I>::snap_get_mirror_namespace(librados::IoCtx& group_ioctx,
                                        const std::string& group_id, const std::string& snap_id,
                                        group_snap_mirror_namespace_t* mirror_namespace) {
  std::vector<cls::rbd::GroupSnapshot> cls_group_snaps;
  int r = group_snap_list<I>(group_ioctx, group_id, false, false, &cls_group_snaps);
  if (r < 0) {
    return r;
  }
  ...
  r = std::visit(visitor, cls_group_snap_ptr->snapshot_namespace);
  if (r < 0) {
    return -ENOENT;
  }
