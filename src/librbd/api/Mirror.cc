...
int Mirror<I>::group_enable(IoCtx& group_ioctx, const char *group_name,
  ...
  if (r == -EOPNOTSUPP) {
    ldout(cct, 10) << "mirroring on group is not supported by OSD" << dendl;
    return r;
  } else if (r == 0 && !mirror_group.global_group_id.empty() &&
             (mirror_group.state == cls::rbd::MIRROR_GROUP_STATE_CREATING ||
              mirror_group.state == cls::rbd::MIRROR_GROUP_STATE_ENABLING)) {
    ldout(cct, 10) << "mirroring on group is in-progress of enabling" << dendl;
    return r;
  } else if (r == 0) {
