  ...
  struct CompressWQ : public ThreadPool::WorkQueue<Job> {
    ...
    Job* _dequeue() override {
      ...
      while (!job_queue.empty()) {
        ...
        status_t expect { status_t::WAIT };
        if (item->status.compare_exchange_weak(expect, status_t::WORKING)) {
          break;
        } else {
    ...
    void _process(Job *item, ThreadPool::TPHandle &) override {
      assert(item->status.load() == status_t::WORKING);
      bufferlist out;
      int r;
      if (item->is_compress)
      ...
      if (!r) {
        item->data.swap(out);
        status_t expected = status_t::WORKING;
        assert(item->status.compare_exchange_weak(expected, status_t::DONE));
      } else {
        item->status.store(status_t::ERROR);
      }
    }
