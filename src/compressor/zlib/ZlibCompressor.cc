...
int ZlibCompressor::decompress(bufferlist::const_iterator &p, size_t compressed_size, bufferlist &out, std::optional<int32_t> compressor_message)
{
  ...
  if (qat_enabled && (!compressor_message || *compressor_message == GZIP_WRAPPER + MAX_WBITS ) && g_ceph_context->_conf.get_val<std::string>("qat_compressor_data_fmt").compare("QZ_DEFLATE_GZIP_EXT") == 0)
