...
void PgScrubber::downgrade_on_op_abort(
    Scrub::delay_cause_t issue,
    Scrub::SchedTarget& targ)
{
  if (issue == Scrub::delay_cause_t::operator_abort) {
...
void PgScrubber::on_mid_scrub_abort(Scrub::delay_cause_t issue)
{
  ...
  // any operator-requested urgency from the aborted target
  downgrade_on_op_abort(issue, aborted_target);
  ...
  if (!sister.queued) {
    downgrade_on_op_abort(issue, sister);
