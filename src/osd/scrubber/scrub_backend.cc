  ...
  if (auth_selection.auth_oi.version != eversion_t() &&
    ...
    for (const auto& [srd, smap] : this_chunk->received_maps) {
      if (smap.objects.contains(ho)) {
        ...
        // The digest map below and the decode check that consumes
        // available_shards are both in relative shard space, so a multi-zone
        // pool must record the relative id here rather than the absolute one.
        available_shards.insert(m_pg.get_ec_sinfo().get_rel_shard(srd.shard));
