...
std::string ScrubBackend::extract_crcs_from_map(shard_id_map<bufferlist> map) {
...
std::string ScrubBackend::extract_crc_from_bufferlist(bufferlist crc_buffer) {
  std::string crc_string;
  constexpr size_t digest_length = sizeof(uint32_t);
  for (size_t i = 0; i < digest_length; i++) {
    crc_string += fmt::format("{:02x}", crc_buffer.c_str()[digest_length - i]);
  }
  return crc_string;
}
  ...
  stringstream& errstream)
{
  ...
  if (!m_is_replicated && m_pg.get_ec_supports_crc_encode_decode()) {
    ...
    ceph_assert(incorrectly_decoded_shards.size() <=
                m_pg.get_ec_sinfo().get_k());
    if (incorrectly_decoded_shards.size() == 1) {
      ...
      dout(10) << fmt::format(
                      ...
                      extract_crcs_from_map(digests))
               << dendl;
    } else if (incorrectly_decoded_shards.size() > 0 &&
