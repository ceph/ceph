...
void BfqScheduler::refresh_config()
{
  ...
  // the seek term models device positioning cost, which flash does
  // not pay: charge it on rotational media only, so one cluster-wide
  // value is safe for a mixed fleet
  cost_per_seek = is_rotational ?
    conf.get_val<Option::size_t>("osd_bfq_cost_per_seek") : 0;
  // floor budgets so a round always covers at least one scaled item
  // and rounds cannot degenerate into per-item switching
  const uint64_t fixed_cost = cost_per_op + cost_per_seek;
...
uint64_t BfqScheduler::calc_scaled_cost(int item_cost) const
{
  ...
  if (fixed_cost > 0) {
    ...
    // on rotational media, the seek-time equivalent (cost_per_seek),
    // plus transfer bytes.  When either constant is in effect it
    // replaces the min_cost floor.
...
void BfqScheduler::end_round(expire_reason reason)
{
  ...
  root_tree.expire(idx(g), served, max_budget,
