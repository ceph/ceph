...
public:
  ...
  SchedulerClass get_scheduler_class() const final {
    ...
    /**
     * SubOp reads/writes for mClock are now classed based on their priority.
     ...
     * of client and other classes of operation as they are no longer
     * unnecessarily preempted by subOps in the 'immediate' queue. SubOps
     * are now handled as follows:
     *
     *  - SubOps generated during recovery will either go into the
     *    'background_recovery' or 'background_best_effort' class based on
     *    the recovery priority set for the op.
     *
     *  - SubOp writes generated as a result of client operations will
     *    be classified as 'client' class.
     *
     *  - SubOp replies are considered high priority and therefore continue
     *    to be classed as 'immediate'.
     *
     *  Note: The 'cost' for subOp read and write operation is set to 1 as
     *  before so that they don't spend time in the scheduler queue.
     */
