...
void mClockScheduler::set_osd_capacity_params_from_config()
{
  ...
  osd_bandwidth_cost_per_io_inv = 1 / osd_bandwidth_cost_per_io;
...
void mClockScheduler::enqueue(OpSchedulerItem&& item)
{
  ...
  if (op_scheduler_class::immediate == id.class_id) {
    ...
    if (qos_profile_params.qos_profile_id != 0) {
      ...
      /**
       * Convert delta & rho into 'Bytes' representation before adding it to the
       * dmClock server request queue:
       *     delta = delta (#IOs) * cost_per_io (Bytes/IO)
       *     rho = rho (#IOs) * cost_per_io (Bytes/IO)
       */
      qos_req_params.delta *= cost;
      qos_req_params.rho *= cost;
	...
	0 == "Impossible, must have checked empty() first");
      ...
      if (_op.has_value()) {
        ...
        if (req->get_type() == CEPH_MSG_OSD_OP) {
         ...
         /**
          * Convert the cost in terms of number of IOs as shown below:
          *    cost = cost (bytes) / cost_per_io (bytes/io)
          *
          * The conversion to number of IOs is to enable the
          * ServiceTracker associated with clients' QoS profile to
          * calculate the new delta and rho params in the IO
          * response path (MOSDOpReply). The new delta and rho are
          * sent via MOSDOp when the client issues the next Op.
          */
          (*_op)->qos_cost = std::max<uint64_t>(1,
            std::floor(retn.cost * osd_bandwidth_cost_per_io_inv));
          dout(20) << __func__ << " qos_cost: " << (*_op)->qos_cost << dendl;
        }
      }
