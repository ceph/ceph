...
void OSDService::adjust_pg_priorities(const vector<PGRef>& pgs, int newflags)
{
  ...
  set<spg_t> did;
  bool is_cancel = newflags & OFR_CANCEL;
  if (newflags && (OFR_BACKFILL || OFR_RECOVERY)) {
    for (auto& pg : pgs) {
      if (pg->set_force_backfill(!(is_cancel)) ||
          pg->set_force_recovery(!(is_cancel))) {
        did.insert(pg->pg_id);
      }
    }
  }
  ...
  dout(10) << __func__ << " "
    << (is_cancel ? "cleared" : "set") << " "
    << "force_" << ((newflags & OFR_BACKFILL) ? "backfill" : "_recovery")
