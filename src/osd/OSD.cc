...
bool OSD::network_rebind_and_check()
{
  ...
  bool need_rebind = true;
  if (!cluster_messenger->rebind(avoid_ports))
    if (!hb_back_server_messenger->rebind(avoid_ports))
      if (!hb_front_server_messenger->rebind(avoid_ports))
	need_rebind = false;
  if (need_rebind)
  ...
  if (cct->_conf->osd_support_ethtool_link_check){
    ...
    if (cluster_messenger->is_linked())
      if (hb_back_server_messenger->is_linked())
	if (hb_front_server_messenger->is_linked())
	  is_linked = true;
    if (!is_linked)
