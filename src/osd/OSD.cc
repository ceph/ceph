  ...
  set<pair<spg_t,epoch_t>> *merge_pgs)
{
  ...
  auto pool_pg_num_history_map = osd->pg_num_history.pg_nums[pgid.pool()];
  dout(20) << __func__ << " " << pgid << " e" << old_map->get_epoch()
  ...
  while (!queue.empty()) {
    ...
    for (auto map_iter = pool_pg_num_history_map.lower_bound(old_map->get_epoch());
	 ...
	 ++map_iter) {
      auto& [new_epoch, new_pgnum] = *map_iter;
...
void OSD::handle_osd_map(MOSDMap *m)
{
  ...
  track_pools_and_pg_num_changes(added_maps, t, first, last);
...
/*
 *  Compare between the previous last_map to the current one
...
void OSD::track_pools_and_pg_num_changes(
  ...
  epoch_t last)
{
  OSDMapRef current_lastmap = added_maps.at(last);
  OSDMapRef previous_lastmap;

  if (superblock.maps.empty()) {
    dout(10) << __func__ << " no maps stored, this is probably "
             << "the first start of this osd" << dendl;
    previous_lastmap = added_maps.at(first);
  } else {
    if (first > superblock.get_newest_map() + 1) {
  ...
  for (auto& [pool_id, pg_pool] : previous_lastmap->get_pools()) {
    if (!current_lastmap->have_pg_pool(pool_id)) {
      pg_num_history.log_pool_delete(last, pool_id);
      ...
      string name = previous_lastmap->get_pool_name(pool_id);
      encode(name, bl);
      map<string,string> profile;
      if (previous_lastmap->get_pg_pool(pool_id)->is_erasure()) {
        profile = previous_lastmap->get_erasure_code_profile(
        previous_lastmap->get_pg_pool(pool_id)->erasure_code_profile);
      }
      encode(profile, bl);
      t.write(coll_t::meta(), obj, 0, bl.length(), bl);

    // 2) For existing pools, check if pg_num was changed
    } else if (unsigned new_pg_num = current_lastmap->get_pg_num(pool_id);
               new_pg_num != pg_pool.get_pg_num()) {
      dout(10) << __func__ << " recording pool " << pool_id << " pg_num "
               << pg_pool.get_pg_num() << " -> " << new_pg_num << dendl;
      pg_num_history.log_pg_num_change(last, pool_id, new_pg_num);
    }
  }

  // 3) Check if a pool was created
  for (auto& [pool_id, pg_pool] : current_lastmap->get_pools()) {
    if (!previous_lastmap->have_pg_pool(pool_id)) {
        dout(10) << __func__ << " recording new pool " <<pool_id << " pg_num "
                 << pg_pool.get_pg_num() << dendl;
    pg_num_history.log_pg_num_change(last, pool_id, pg_pool.get_pg_num());
    }
  }
