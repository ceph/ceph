			     ...
			     const std::set <std::string> &changed)
{
  ...
  if (changed.count("osd_op_thread_timeout")) {
    op_shardedwq.set_timeout(ceph::make_timespan(cct->_conf->osd_op_thread_timeout));
  }
  if (changed.count("osd_op_thread_suicide_timeout")) {
    op_shardedwq.set_suicide_timeout(ceph::make_timespan(cct->_conf->osd_op_thread_suicide_timeout));
  }
}
