...
void ReplicatedPG::make_writeable(OpContext *ctx)
{
  ...
  if (ctx->obs->exists &&               // head exist(ed)
    ...
    if (is_primary()) {
      ctx->clone_obc = ObjectContextRef(new ObjectContext(static_snap_oi, true, NULL));
...
{
  for (map<hobject_t, WObjectContextRef>::iterator i = object_contexts.begin();
       i != object_contexts.end();
       ++i) {
    ObjectContextRef obc(i->second.lock());
    if (obc)
...
void ReplicatedPG::check_blacklisted_watchers()
{
  ...
  for (map<hobject_t, WObjectContextRef>::iterator i = object_contexts.begin();
       ...
       ++i) {
    if (!i->second.expired())
      check_blacklisted_obc_watchers(i->second.lock());
  }
  check_wake();
}
...
ObjectContextRef ReplicatedPG::create_object_context(const object_info_t& oi,
								 SnapSetContext *ssc)
{
  ObjectContextRef obc(new ObjectContext(oi, false, ssc));
  dout(10) << "create_object_context " << obc << " " << oi.soid << " " << dendl;
...
ObjectContextRef ReplicatedPG::get_object_context(const hobject_t& soid,
							      ...
							      bool can_create)
{
  map<hobject_t, WObjectContextRef>::iterator p = object_contexts.find(soid);
  ObjectContextRef obc;
  if (p != object_contexts.end() && !p->second.expired()) {
    obc = p->second.lock();
    ...
    obc = ObjectContextRef(new ObjectContext(oi, true, ssc));
    obc->obs.exists = true;
				      ...
				      snapid_t *psnapid)
{
  ...
  if (oid.snap == CEPH_SNAPDIR) {
    ...
    ObjectContextRef obc = get_object_context(head, oloc, can_create);
  ...
  if (oid.snap > ssc->snapset.seq) {
    if (ssc->snapset.head_exists) {
      ObjectContextRef obc = get_object_context(head, oloc, false);
  ...
  ObjectContextRef obc = get_object_context(soid, oloc, false);
  assert(obc);
...
void ReplicatedPG::finish_degraded_object(const hobject_t& oid)
{
  ...
  map<hobject_t, WObjectContextRef>::iterator i = object_contexts.find(oid);
  if (i != object_contexts.end() && !i->second.expired()) {
    ObjectContextRef obc(i->second.lock());
...
void ReplicatedPG::_applied_recovered_object(ObjectStore::Transaction *t, ObjectContextRef obc)
{
  lock();
							    ...
							    utime_t mtime, int what)
{
  ...
  ObjectContextRef obc = get_object_context(oid, oloc, true);
  ...
  while (p != missing.rmissing.end()) {
      ...
      case pg_log_entry_t::LOST_REVERT:
	{
	  if (item.have == latest->reverting_to) {
	    ...
	    ObjectContextRef obc = get_object_context(soid, oloc, true);
  ...
  const hobject_t& soid, eversion_t v, int prio)
{
  ...
  ObjectContextRef obc = get_object_context(soid, OLOC_BLANK, false);
  if (!obc) {
...
int ReplicatedPG::recover_backfill(int max)
{
  ...
  for (set<hobject_t>::iterator i = add_to_stat.begin();
       i != add_to_stat.end();
       ++i) {
    ObjectContextRef obc = get_object_context(*i, OLOC_BLANK, false);
    pg_stat_t stat;
...
void ReplicatedPG::push_backfill_object(hobject_t oid, eversion_t v, eversion_t have, int peer)
{
  ...
  ObjectContextRef obc = get_object_context(oid, OLOC_BLANK, false);
  obc->ondisk_read_lock();
