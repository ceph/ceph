...
)
{
    ...
    {
    //�ղ��Ǹ�����(Ҳ����������assert)�ܿ������������µģ�������Ϊpg������osd�ڵ�����pglog����һ���汾������˵11��
    //�ڸ���Ҳ��¼�а汾��������10.�ڻָ���ʱ������������ͬ�����󣬸�����Ϊ�ܵ��İ汾Ӧ����> 10�ġ�
    //����ʵ�����������������ݶ��ˣ������־û��İ汾ʵ������9
        assert(is_primary());
    ...
    ThreadPool::TPHandle &handle)
{
    ...
    case MOSDPGScan::OP_SCAN_GET_DIGEST:
    {
        ...
        // past m->begin
        //ɨ���õ�backfill��������Ϣ��primaryͨ���Աȿ��Լ���ʵ����Ҫbackfill�Ķ���
        scan_range(
...
void ReplicatedPG::make_writeable(OpContext *ctx)
{
    ...
    // use newer snapc?
    // snapc 就是客户端/osdmap 中最新的snap信息。
    // 这里应该是避免客户端没有更新snap信息的情况。
            ...
            snapc.snaps[0] > ctx->new_snapset.seq)    // existing object is old
    {
        ...
        //因为snaps是逆序排列的，通过这个循环找到第一个大于ctx->new_snapset.seq的snap seq。
        for (l=1; l<snapc.snaps.size() && snapc.snaps[l] > ctx->new_snapset.seq; l++) ;

        //这里就是把大于当前object seq的所有seq都记录下来
        ...
        //clone的是整个对象，因而只需要记录一个size即可，而不需要记录一个段
    ...
    // clone_overlap记录clone对象 与 next/head对象之间的差距。
    // 通过这个差距可以用来优化恢复，实现clones以及head之间的本地拷贝
    ...
    int *ops_started)
{
    ...
    //��������missing��unfound,�����Ͳ����Żָ��ˣ�����unfoundҲ�ָ�����
    ...
    if (recovering.empty() &&
            ...
            missing.num_missing() == 0 && //��backfill��ʱ��,���ص�missing�Ѿ��ǿյ���
...
boost::statechart::result ReplicatedPG::TrimmingObjects::react(const SnapTrim&)
{
    ...
    while (repops.size() < g_conf->osd_pg_max_concurrent_snap_trims)
    {
        ...
        assert(repop);
		//����osd_pg_max_concurrent_snap_trimsָ��������ops֮�󣬼�����һ��
