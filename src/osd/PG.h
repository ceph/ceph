...
protected:
  ...
  public:
    ...
    // these distinct sets, plus an "other".
    struct loc_count_t {
      int acting = 0;    //< acting
      int backfill = 0;  //< backfill,  not acting
      int up = 0;        //< up, not acting or backfill
      int other = 0;     //< not acting, up, or backfill

      friend bool operator<(const loc_count_t& l,
			    const loc_count_t& r) {
	return (l.acting < r.acting ||
		(l.acting == r.acting &&
		 (l.backfill < r.backfill ||
		  (l.backfill == r.backfill &&
		   (l.up < r.up ||
		    (l.up == r.up &&
		     (l.other < r.other)))))));
	...
	assert(l.up >= 0);
	assert(l.other >= 0);
	return out << "(" << l.acting << "+" << l.backfill
		   << "+" << l.up << "+" << l.other << ")";
      }
    };
  ...
  private:
    // we group the peer shards into distinct, non-overlapping sets:
    boost::container::flat_set<pg_shard_t> acting, backfill_not_acting, up_only;

    void _calc_sets() {
      acting.clear();
      for (auto s : pg->actingset) {
	acting.insert(s);
      }
      backfill_not_acting.clear();
      for (auto s : pg->actingbackfill) {
	if (acting.count(s) == 0) {
	  backfill_not_acting.insert(s);
	}
      }
      up_only.clear();
      for (auto s : pg->upset) {
	if (acting.count(s) == 0 && backfill_not_acting.count(s) == 0) {
	  up_only.insert(s);
	}
      }
    }
	...
	if (acting.count(s)) {
	  r.acting++;
	} else if (backfill_not_acting.count(s)) {
	  r.backfill++;
	} else if (up_only.count(s)) {
	  r.up++;
	} else {
	  r.other++;
	}
