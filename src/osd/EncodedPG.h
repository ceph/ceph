...
#include "PGBackend.h"
#include "EncodedBackend.h"

#include "ReplicatedPG.h" //FIXME: Replace later.

class MOSDSubOpReply;

class EncodedPG;
void intrusive_ptr_add_ref(EncodedPG *pg);
void intrusive_ptr_release(EncodedPG *pg);
uint64_t get_with_id(EncodedPG *pg);
void put_with_id(EncodedPG *pg, uint64_t id);

#ifdef PG_DEBUG_REFS
  typedef TrackedIntPtr<EncodedPG> EncodedPGRef;
#else
  typedef boost::intrusive_ptr<EncodedPG> EncodedPGRef;
#endif

class EncodedPG : public PG, public PGBackend::Listener {
  friend class OSD;
  friend class Watch;
...
public:
  ...
  class BlessedGenContext : public GenContext<T> {
    ...
    epoch_t e;
  public:
    BlessedGenContext(EncodedPG *pg, GenContext<T> *c, epoch_t e)
      : pg(pg), c(c), e(e) {}
    void finish(T t) {
      pg->lock();
  ...
  class BlessedContext : public Context {
    ...
    epoch_t e;
  public:
    BlessedContext(EncodedPG *pg, Context *c, epoch_t e)
      : pg(pg), c(c), e(e) {}
    void finish(int r) {
      pg->lock();
      if (pg->pg_has_reset_since(e))
...
protected:
  ...
  struct C_PG_ObjectContext : public Context {
    EncodedPGRef pg;
    ObjectContext *obc;
    C_PG_ObjectContext(EncodedPG *p, ObjectContext *o) :
      pg(p), obc(o) {}
    void finish(int r) {
  ...
  struct C_OSD_AppliedRecoveredObject : public Context {
    EncodedPGRef pg;
    ObjectContextRef obc;
    C_OSD_AppliedRecoveredObject(EncodedPG *p, ObjectContextRef o) :
      pg(p), obc(o) {}
    void finish(int r) {
  ...
  struct C_OSD_CommittedPushedObject : public Context {
    ...
    C_OSD_CommittedPushedObject(
      EncodedPG *p, epoch_t epoch, eversion_t lc) :
  ...
  struct C_OSD_AppliedRecoveredObjectReplica : public Context {
    EncodedPGRef pg;
    C_OSD_AppliedRecoveredObjectReplica(EncodedPG *p) :
      pg(p) {}
    void finish(int r) {
...
private:
  ...
  struct SnapTrimmer : public boost::statechart::state_machine< SnapTrimmer, NotTrimming > {
    ...
    bool requeue;
    SnapTrimmer(EncodedPG *pg) : pg(pg), need_share_pg_info(false), requeue(false) {}
    ~SnapTrimmer();
...
inline ostream& operator<<(ostream& out, EncodedPG::RepGather& repop)
{
  out << "repgather(" << &repop
