...
void ECBackend::handle_sub_write(
  ...
  // One line per sub-write at 10.  Roll-forward-only sub-writes generated by
  // ECDummyOp (at_version 0'0, no log entries) are only interesting at 20.
  const int sub_write_log_level = (op.at_version == eversion_t()) ? 20 : 10;
  dout(ceph::dout::need_dynamic(sub_write_log_level)) << __func__
           << " tid=" << op.tid
           << " reqid=" << op.reqid
           << " " << op.soid
           << " v=" << op.at_version
