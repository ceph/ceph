    ...
    pg_shard_t from)
{
    ...
    //Õâº¯ÊýÖ÷Òª¾ÍÊÇ´¦Àídivergent log, ²¢¾ÝdivergentÀ´¸üÐÂpeer missing
    ...
    const set<pg_shard_t> &acting) const
{
	...
	//µ±Ç°½ÚµãÔÚhavingÁÐ±íÖÐ£¬Ôò·µ»Øtrue
	//have.count(whoami);
...
//Õâº¯Êý¾ÍÊÇÓÃÀ´¼ÆËãpast_intervals
    ...
    const map<pg_shard_t, pg_info_t> &infos) const
{
	...
	//ÕÒµ½°üº¬×îºóÒ»¸öpeering³É¹¦£¬¸üÐÂlast_epoch_startµÄ°æ±¾
	//ÒÔ¼°¸Ã°æ±¾Ëù¶ÔÓ¦osdÖÐ×îÐ¡µÄlast_update
    ...
    ostream &ss)
{
    ...
    // select primary
    //Ö»ÓÐÔÚup_primaryÎÞ·¨Ê¹ÓÃµÄÊ±ºò²ÅÑ¡Ôñanth_log_shard
 ...
 */
 //¾ÍÏñÆäÃû×ÖÒ»Ñù£¬Õâ¸öº¯ÊýÊÇ¸ù¾Ýpg info¼ÆËãdesired acting
bool PG::choose_acting(pg_shard_t &auth_log_shard_id)
{
    ...
    {
        if (want[i] != CRUSH_ITEM_NONE)//Õâ¸ö»ù±¾Ò²ÊÇ¸øECÓÃµÄ
            have.insert(
                pg_shard_t(
                    want[i],
                    pool.info.ec_pool() ? shard_id_t(i) : shard_id_t::NO_SHARD));
    }
	//Õâ¸ö¶ÔÓÚreplicate PGÒ²ºÜ¼òµ¥£¬Ö»Òªhave²»Îª¿Õ¼´¿É
	...
	//¸ù¾Ýpg info¼ÆËã³öÀ´µÄactingÓëcrush²»Ò»ÖÂ
    if (want != acting)
    {
    ...
    else
    {
        ...
        for (set<pg_shard_t>::iterator i = want_backfill.begin();
                ...
                ++i)
        {
            //ºÃ°É »»ÁËÒ»ÖÖÐ´·¨£¬Êµ¼Ê»¹ÊÇËµstrayµÄ²»»á×öbackfill
    ...
    if (is_primary())
    {
                ...
                ++i)
        {
                ...
                (backfill_targets.count(*i) && pi.last_backfill.is_max()))
            {
                ...
                //Èç¹ûÒª×öbackfill,ÄÇÃ´peer_missing¿ÉÒÔÇå¿ÕÁË
                pm.clear();
            }
            else
            {
            ...
            //¸ù¾ÝÉÏÃæ¼ÆËãµÄpglog,²¹³äreplicaÉÐÎ´Í¬²½pglog¶ÔÏóµÄmissingÐÅÏ¢
        ...
        if (needs_recovery())
        {
			...
			//ËäÈ»ÕâÀïºÜ¿ì¿ªÆôÁËrecovery,µ«ÊÇÃ»ÓÐbackfill_reserveÒ²ÊÇ×ö²»ÁËbackfillµÄ¡£
			//È»¶øÕâ²¢Ã»Ê²Ã´ÂÑÓÃ£¬»¹ÊÇÒª×ßrecovery reserve£¬Ö»ÒªÓÐ
			//¶ÔÏóÐèÒª»Ö¸´¡£¶øÇÒÒ²ÎÞ·¨¿ªÊ¼»Ö¸´,ÔÚstart_recovery_opsÖÐ»áÅÐ¶ÏÊÇ·ñÎªRecovering»ò
			//Backfilling×´Ì¬
...
//¿´ÏÂÕâ¸öº¯Êýµ÷ÓÃµÄµØ·½¾ÍÖªµÀÔõÃ´»Ö¸´»úÖÆÊÇÔõÃ´´¥·¢µÄ
//ÒÔ¼°ÔõÃ´½áÊøµÄ£¬×îÖÕÊÇÔÚclean×´Ì¬µ÷ÓÃfinish_recoveryº¯Êý½áÊø
...
void PG::share_pg_log()
{
    ...
    while (a != end)
    {
        ...
        //shareµÄÍ¬Ê±¸üÐÂpeer missing£¬ÕâÐ©pglogÈ«²¿¶¼ÊÇ¸±±¾È±ÉÙµÄ£¬Òò¶ø¶¼µÃ¼Óµ½missing
...
//Õâ¸öÊÂ¼þºÍÉÏÃæÊÇÏà¸¨µÄ¡£
//Èç¹ûPGÃ»ÓÐ½øÈëReset×´Ì¬£¬ÄÇÃ´ÕâÀïÒ²¾ÍÊÕ²»µ½ActMapÊÂ¼þ
...
boost::statechart::result PG::RecoveryState::Active::react(const ActMap&)
{
    ...
    //Õâ¶Î·Ç³£ÖØÒª£¬ÓÐÐ©Çé¿ö£¬osd×´Ì¬µÄ±ä»¯²»»áÒýÆðactingºÍupµÄ±ä»¯¡£
    //µ«ÊÇÕâÐ©osdÉÏÈ´ÓÐ¸ÃpgÐèÒªµÄmissing object£¬ÕâÊ±±ØÐëÆô¶¯»Ö¸´¡£
    if (!pg->is_clean() &&
...
boost::statechart::result PG::RecoveryState::Active::react(const AllReplicasActivated &evt)
{
	...
	//µ½´ËËùÓÐpeer¶¼µÃµ½pglogÁË£¬¿ÉÒÔ¸üÐÂlast_epoch_startedÁË
...
boost::statechart::result PG::RecoveryState::Stray::react(const MLogRec& logevt)
{
    ...
    if (msg->info.last_backfill == hobject_t())
    {
        ...
        //¶ÔÓÚÐèÒª×öbackfillµÄreplica£¬ÊÇ²»ÐèÒªmerge pglogµÄ
      ...
      NamedState(context< RecoveryMachine >().pg->cct, "Started/Primary/Peering/GetInfo")
{
    ...
    //prior_setÖ ¸ù¾Ýpg infoºÍpast_intervals¼ÆËã
    // 1£¬È·¶¨get_infosµÄosd 2,¸üÐÂheart_beatµÄosd
    if (!prior_set.get())
      ...
      msg(0)
{
    ...
    // adjust acting?
    //ÔÚGetInfoÖ®ºó,Ë­×÷Îªprimary,Ë­ÊÇacting,Ë­backfill¶¼Çå³þÁË
    //logÖ»ÊÇÓÃÀ´È·¶¨missing,ÎªºóÃæµÄrecoveryÆÌµæ
    ...
    //ÏÔÈ»auth_log_shardÖ»ÓÐÒ»¸ö,¿ÉÒÔquery from stray,µ«ÊÇstray²»»á½øÈëReplicaActive×´Ì¬
    // how much?
