...
void PG::_update_calc_stats()
{
  ...
  if ((is_degraded() || is_undersized() || !is_clean()) && is_peered()) {
    ...
    for (set<pg_shard_t>::iterator i = actingbackfill.begin();
	 ...
	 ++i) {
      ...
      if (!is_backfill_targets(p)) {
        ...
        osd_objects = std::max(0L, num_objects - osd_missing);
    ...
    misplaced = std::max(0L, misplaced - adjust_misplaced);
    ...
    int64_t degraded = std::max(0L, info.stats.stats.sum.num_object_copies - object_copies);
...
void PG::chunky_scrub(ThreadPool::TPHandle &handle)
{
  ...
  while (!done) {
	...
	scrubber.seed = -1;
	   ...
	   */
	  int64_t min = std::max(3L, cct->_conf->osd_scrub_chunk_min);
	  ...
	  ret = get_pgbackend()->objects_list_partial(
	    start,
	    min,
	    std::max(min, cct->_conf->osd_scrub_chunk_max),
	    &objects,
	    &candidate_end);
	  assert(ret >= 0);
