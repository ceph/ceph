...
int PrimaryLogPG::do_osd_ops(OpContext *ctx, vector<OSDOp>& ops)
{
  ...
  for (auto p = ops.begin(); p != ops.end(); ++p, ctx->current_osd_subop_num++, ctx->processed_subop_count++) {
	...
	// the RDMA write is a side effect the resend machinery knows
	// nothing about: a resent op (peering change, lost reply) could
	// double-execute against client memory while the superseded
	// attempt's write is still in flight on another OSD. Refuse
	// retries so the caller falls back to a single-writer path.
	if (ctx->op->get_req<MOSDOp>()->get_retry_attempt() > 0) {
	  result = -EOPNOTSUPP;
	  break;
	}
	[[maybe_unused]] std::string rdma_token;
	[[maybe_unused]] uint64_t rdma_client_offset = 0;
	{
	  uint32_t rdma_flags = 0;
	  try {
	    uint8_t payload_ver;
	    decode(payload_ver, bp);
	    if (payload_ver != 1) {
	      result = -EINVAL;
	      break;
	    }
	    decode(rdma_token, bp);
	    decode(rdma_client_offset, bp);
	    decode(rdma_flags, bp);
	  } catch (const ceph::buffer::error& e) {
	    result = -EINVAL;
	    break;
	  }
	  // reserved: refuse flag bits we do not implement so future
	  // clients get a clean fallback instead of silent misbehavior
	  if (rdma_flags != 0) {
	    result = -EOPNOTSUPP;
	    break;
	  }
	...
	// read the extent exactly as CEPH_OSD_OP_READ would (same
	// clamping, digest verification and read accounting), then
	// push it out of band instead of returning it inline
	result = do_read(ctx, osd_op);
	if (result >= 0) {
	  ssize_t pushed = osd->cuobj->rdma_write(
	    soid.oid.name, osd_op.outdata, rdma_token, rdma_client_offset);
	  osd_op.outdata.clear();
	  if (pushed < 0) {
	    result = pushed;
	  } else {
	    // the client-read throughput counters are fed from outdata
	    // length, which is now only the 8-byte count; account the
	    // bytes actually pushed out of band
	    ctx->bytes_read += pushed;
	    encode(static_cast<uint64_t>(pushed), osd_op.outdata);
	    result = 0;
	  }
	}
#else
