...
void EncodedPG::on_local_recover_start(
  const hobject_t &oid,
...
void EncodedPG::on_local_recover(
...
void EncodedPG::on_global_recover(
...
void EncodedPG::on_peer_recover(
  int peer,
  const hobject_t &soid,
...
void EncodedPG::begin_peer_recover(
  int peer,
  const hobject_t soid)
{
...
bool EncodedPG::is_missing_object(const hobject_t& soid)
{
  return pg_log.get_missing().missing.count(soid);
}

void EncodedPG::wait_for_missing_object(const hobject_t& soid, OpRequestRef op)
{
...
void EncodedPG::wait_for_all_missing(OpRequestRef op)
{
  waiting_for_all_missing.push_back(op);
}

bool EncodedPG::is_degraded_object(const hobject_t& soid)
{
...
void EncodedPG::wait_for_degraded_object(const hobject_t& soid, OpRequestRef op)
{
...
void EncodedPG::wait_for_blocked_object(const hobject_t& soid, OpRequestRef op)
{
...
void EncodedPG::wait_for_backfill_pos(OpRequestRef op)
{
  waiting_for_backfill_pos.push_back(op);
}

void EncodedPG::release_waiting_for_backfill_pos()
{
  requeue_ops(waiting_for_backfill_pos);
}

bool EncodedPG::pgls_filter(PGLSFilter *filter, hobject_t& sobj, bufferlist& outdata)
{
  bufferlist bl;
...
int EncodedPG::get_pgls_filter(bufferlist::iterator& iter, PGLSFilter **pfilter)
{
  string type;
  PGLSFilter *filter;
...
bool EncodedPG::pg_op_must_wait(MOSDOp *op)
{
...
void EncodedPG::do_pg_op(OpRequestRef op)
{
...
EncodedPG::EncodedPG(OSDService *o, OSDMapRef curmap,
  ...
  pgbackend(new EncodedBackend(this, coll_t(p), o)),
  snapset_contexts_lock("EncodedPG::snapset_contexts"),
  temp_seq(0),
...
void EncodedPG::get_src_oloc(const object_t& oid, const object_locator_t& oloc, object_locator_t& src_oloc)
{
  src_oloc = oloc;
  if (oloc.key.empty())
...
bool EncodedPG::maybe_handle_cache(OpRequestRef op, ObjectContextRef obc,
                                      int r)
{
...
void EncodedPG::do_cache_redirect(OpRequestRef op, ObjectContextRef obc)
{
  MOSDOp *m = static_cast<MOSDOp*>(op->get_req());
...
void EncodedPG::execute_ctx(OpContext *ctx)
{
...
void EncodedPG::reply_ctx(OpContext *ctx, int r)
{
  osd->reply_op_error(ctx->op, r);
  close_op_ctx(ctx);
}

void EncodedPG::reply_ctx(OpContext *ctx, int r, eversion_t v, version_t uv)
{
  osd->reply_op_error(ctx->op, r, v, uv);
  close_op_ctx(ctx);
}

void EncodedPG::log_op_stats(OpContext *ctx)
{
  OpRequestRef op = ctx->op;
  MOSDOp *m = static_cast<MOSDOp*>(op->get_req());
...
void EncodedBackend::_do_push(OpRequestRef op)
{
...
struct C_EncodedBackend_OnPullComplete : GenContext<ThreadPool::TPHandle&> {
  EncodedBackend *bc;
  list<ObjectContextRef> to_continue;
  int priority;
  C_EncodedBackend_OnPullComplete(EncodedBackend *bc, int priority)
    : bc(bc), priority(priority) {}

  void finish(ThreadPool::TPHandle &handle) {
    EncodedBackend::RPGHandle *h = bc->_open_recovery_op();
...
void EncodedBackend::_do_pull_response(OpRequestRef op)
{
  MOSDPGPush *m = static_cast<MOSDPGPush *>(op->get_req());
  assert(m->get_header().type == MSG_OSD_PG_PUSH);
  int from = m->get_source().num();
  ...
  if (!to_continue.empty()) {
    C_EncodedBackend_OnPullComplete *c =
      new C_EncodedBackend_OnPullComplete(
	this,
	m->get_priority());
...
void EncodedBackend::do_pull(OpRequestRef op)
{
...
void EncodedBackend::do_push_reply(OpRequestRef op)
{
...
EncodedPG::RepGather *EncodedPG::trim_object(const hobject_t &coid)
{
  // load clone info
  bufferlist bl;
  ObjectContextRef obc;
...
int EncodedPG::do_xattr_cmp_u64(int op, __u64 v1, bufferlist& xattr)
{
  __u64 v2;
  if (xattr.length())
...
int EncodedPG::do_xattr_cmp_str(int op, string& v1s, bufferlist& xattr)
{
  const char *v1, *v2;
  v1 = v1s.data();
  string v2s;
...
int EncodedPG::do_tmapup_slow(OpContext *ctx, bufferlist::iterator& bp, OSDOp& osd_op,
				    bufferlist& bl)
{
  // decode
  bufferlist header;
...
int EncodedPG::do_tmapup(OpContext *ctx, bufferlist::iterator& bp, OSDOp& osd_op)
{
...
int EncodedPG::do_osd_ops(OpContext *ctx, vector<OSDOp>& ops)
{
  int result = 0;
...
int EncodedPG::_get_tmap(OpContext *ctx,
			    map<string, bufferlist> *out,
			    bufferlist *header)
{
  vector<OSDOp> nops(1);
...
inline int EncodedPG::_delete_head(OpContext *ctx)
{
...
int EncodedPG::_rollback_to(OpContext *ctx, ceph_osd_op& op)
{
  SnapSet& snapset = ctx->new_snapset;
  ObjectState& obs = ctx->new_obs;
  object_info_t& oi = obs.oi;
...
void EncodedPG::_make_clone(ObjectStore::Transaction& t,
...
void EncodedPG::make_writeable(OpContext *ctx)
{
...
void EncodedPG::write_update_size_and_usage(object_stat_sum_t& delta_stats, object_info_t& oi,
...
void EncodedPG::add_interval_usage(interval_set<uint64_t>& s, object_stat_sum_t& delta_stats)
{
...
void EncodedPG::do_osd_op_effects(OpContext *ctx)
{
...
coll_t EncodedPG::get_temp_coll(ObjectStore::Transaction *t)
{
  return pgbackend->get_temp_coll(t);
}

hobject_t EncodedPG::generate_temp_object()
{
  ostringstream ss;
...
int EncodedPG::prepare_transaction(OpContext *ctx)
{
...
struct C_Copyfrom : public Context {
  ...
  tid_t tid;
  C_Copyfrom(EncodedPG *p, hobject_t o, epoch_t lpr)
...
int EncodedPG::start_copy(CopyCallback *cb, ObjectContextRef obc,
...
void EncodedPG::_copy_some(ObjectContextRef obc, CopyOpRef cop)
{
...
void EncodedPG::process_copy_chunk(hobject_t oid, tid_t tid, int r)
{
...
void EncodedPG::_write_copy_chunk(CopyOpRef cop, ObjectStore::Transaction *t)
{
...
void EncodedPG::_build_finish_copy_transaction(CopyOpRef cop,
                                                  ObjectStore::Transaction& t)
{
...
int EncodedPG::finish_copyfrom(OpContext *ctx)
{
...
void EncodedPG::cancel_copy(CopyOpRef cop)
{
...
void EncodedPG::cancel_copy_ops()
{
...
public:
  EncodedPGRef pg;
  EncodedPG::RepGather *repop;

  C_OSD_OpApplied(EncodedPG *p, EncodedPG::RepGather *rg) :
    pg(p), repop(rg) {
    repop->get();
  }
  void finish(int r) {
...
public:
  ...
  C_OSD_OpCommit(EncodedPG *p, EncodedPG::RepGather *rg) :
    pg(p), repop(rg) {
    repop->get();
  }
  void finish(int r) {
...
void EncodedPG::apply_repop(RepGather *repop)
{
...
void EncodedPG::op_applied(RepGather *repop)
{
  lock();
...
void EncodedPG::op_commit(RepGather *repop)
{
  lock();
  if (repop->ctx->op)
...
void EncodedPG::eval_repop(RepGather *repop)
{
  MOSDOp *m = NULL;
  if (repop->ctx->op)
...
void EncodedPG::issue_repop(RepGather *repop, utime_t now)
{
...
EncodedPG::RepGather *EncodedPG::new_repop(OpContext *ctx, ObjectContextRef obc,
						 tid_t rep_tid)
{
  if (ctx->op)
...
void EncodedPG::remove_repop(RepGather *repop)
{
  release_op_ctx_locks(repop->ctx);
...
void EncodedPG::repop_ack(RepGather *repop, int result, int ack_type,
...
void EncodedPG::get_watchers(list<obj_watch_item_t> &pg_watchers)
{
...
void EncodedPG::get_obc_watchers(ObjectContextRef obc, list<obj_watch_item_t> &pg_watchers)
{
...
{
  dout(20) << "EncodedPG::check_blacklisted_watchers for pg " << get_pgid() << dendl;
  pair<hobject_t, ObjectContextRef> i;
  while (object_contexts.get_next(i.first, &i))
    check_blacklisted_obc_watchers(i.second);
}

void EncodedPG::check_blacklisted_obc_watchers(ObjectContextRef obc)
{
  dout(20) << "EncodedPG::check_blacklisted_obc_watchers for obc " << obc->obs.oi.soid << dendl;
...
void EncodedPG::populate_obc_watchers(ObjectContextRef obc)
{
  assert(is_active());
...
ObjectContextRef EncodedPG::create_object_context(const object_info_t& oi,
						     SnapSetContext *ssc)
{
...
ObjectContextRef EncodedPG::get_object_context(const hobject_t& soid,
						  bool can_create,
...
void EncodedPG::context_registry_on_change()
{
  pair<hobject_t, ObjectContextRef> i;
  while (object_contexts.get_next(i.first, &i)) {
    ObjectContextRef obc(i.second);
...
int EncodedPG::find_object_context(const hobject_t& oid,
...
void EncodedPG::object_context_destructor_callback(ObjectContext *obc)
{
  if (obc->ssc)
    put_snapset_context(obc->ssc);
}

void EncodedPG::add_object_context_to_pg_stat(ObjectContextRef obc, pg_stat_t *pgstat)
{
...
void EncodedPG::kick_object_context_blocked(ObjectContextRef obc)
{
  const hobject_t& soid = obc->obs.oi.soid;
...
SnapSetContext *EncodedPG::create_snapset_context(const object_t& oid)
{
...
SnapSetContext *EncodedPG::get_snapset_context(
  const object_t& oid,
  const string& key,
  ps_t seed,
  bool can_create,
...
void EncodedPG::put_snapset_context(SnapSetContext *ssc)
{
  Mutex::Locker l(snapset_contexts_lock);
  --ssc->ref;
  if (ssc->ref == 0) {
...
void EncodedPG::sub_op_modify(OpRequestRef op)
{
  MOSDSubOp *m = static_cast<MOSDSubOp*>(op->get_req());
  assert(m->get_header().type == MSG_OSD_SUBOP);
...
void EncodedPG::sub_op_modify_applied(RepModify *rm)
{
  lock();
...
void EncodedPG::sub_op_modify_commit(RepModify *rm)
{
  lock();
...
void EncodedPG::sub_op_modify_reply(OpRequestRef op)
{
  MOSDSubOpReply *r = static_cast<MOSDSubOpReply*>(op->get_req());
  assert(r->get_header().type == MSG_OSD_SUBOPREPLY);
...
void EncodedBackend::calc_head_subsets(
...
void EncodedBackend::calc_clone_subsets(
...
void EncodedBackend::prepare_pull(
  const hobject_t& soid,
...
void EncodedPG::send_remove_op(const hobject_t& oid, eversion_t v, int peer)
{
 ...
 */
void EncodedBackend::prep_push_to_replica(
...
void EncodedBackend::prep_push(ObjectContextRef obc,
...
int EncodedBackend::send_pull_legacy(int prio, int peer,
					const ObjectRecoveryInfo &recovery_info,
...
void EncodedBackend::submit_push_data(
  ObjectRecoveryInfo &recovery_info,
  bool first,
  bool complete,
...
void EncodedBackend::submit_push_complete(ObjectRecoveryInfo &recovery_info,
					     ObjectStore::Transaction *t)
{
...
ObjectRecoveryInfo EncodedBackend::recalc_subsets(
  const ObjectRecoveryInfo& recovery_info,
  SnapSetContext *ssc)
{
...
bool EncodedBackend::handle_pull_response(
...
struct C_OnPushCommit : public Context {
  EncodedPG *pg;
  OpRequestRef op;
  C_OnPushCommit(EncodedPG *pg, OpRequestRef op) : pg(pg), op(op) {}
  void finish(int) {
...
void EncodedBackend::handle_push(
...
void EncodedBackend::send_pushes(int prio, map<int, vector<PushOp> > &pushes)
{
...
void EncodedBackend::send_pulls(int prio, map<int, vector<PullOp> > &pulls)
{
...
int EncodedBackend::build_push_op(const ObjectRecoveryInfo &recovery_info,
...
int EncodedBackend::send_push_op_legacy(int prio, int peer, PushOp &pop)
{
  tid_t tid = osd->get_tid();
  osd_reqid_t rid(osd->get_cluster_msgr_name(), 0, tid);
...
void EncodedBackend::prep_push_op_blank(const hobject_t& soid, PushOp *op)
{
...
void EncodedBackend::sub_op_push_reply(OpRequestRef op)
{
...
bool EncodedBackend::handle_push_reply(int peer, PushReplyOp &op, PushOp *reply)
{
...
void EncodedPG::finish_degraded_object(const hobject_t& oid)
{
 ...
 */
void EncodedBackend::sub_op_pull(OpRequestRef op)
{
  MOSDSubOp *m = static_cast<MOSDSubOp*>(op->get_req());
  assert(m->get_header().type == MSG_OSD_SUBOP);
...
void EncodedBackend::handle_pull(int peer, PullOp &op, PushOp *reply)
{
  const hobject_t &soid = op.soid;
  struct stat st;
...
void EncodedPG::_committed_pushed_object(
...
void EncodedPG::_applied_recovered_object(ObjectContextRef obc)
{
  lock();
...
void EncodedPG::_applied_recovered_object_replica()
{
  lock();
...
void EncodedPG::recover_got(hobject_t oid, eversion_t v)
{
 ...
 */
void EncodedBackend::trim_pushed_data(
 ...
 * NOTE: called from opqueue.
 */
void EncodedBackend::sub_op_push(OpRequestRef op)
{
  op->mark_started();
  MOSDSubOp *m = static_cast<MOSDSubOp *>(op->get_req());
...
void EncodedPG::failed_push(int from, const hobject_t &soid)
{
  assert(recovering.count(soid));
  recovering.erase(soid);
...
void EncodedBackend::_failed_push(int from, const hobject_t &soid)
{
...
void EncodedPG::sub_op_remove(OpRequestRef op)
{
  MOSDSubOp *m = static_cast<MOSDSubOp*>(op->get_req());
  assert(m->get_header().type == MSG_OSD_SUBOP);
...
eversion_t EncodedPG::pick_newest_available(const hobject_t& oid)
{
  eversion_t v;
 ...
 */
ObjectContextRef EncodedPG::mark_object_lost(ObjectStore::Transaction *t,
...
struct C_PG_MarkUnfoundLost : public Context {
  ...
  C_PG_MarkUnfoundLost(EncodedPG *p) : pg(p) {}
  void finish(int r) {
 ...
 */
void EncodedPG::mark_all_unfound_lost(int what)
{
...
void EncodedPG::_finish_mark_all_unfound_lost(list<ObjectContextRef>& obcs)
{
  lock();
...
void EncodedPG::_split_into(pg_t child_pgid, PG *child, unsigned split_bits)
{
  assert(repop_queue.empty());
}
...
void EncodedPG::apply_and_flush_repops(bool requeue)
{
...
void EncodedPG::cancel_pull(const hobject_t &soid)
{
  assert(recovering.count(soid));
  recovering.erase(soid);
  finish_recovery_op(soid);
...
void EncodedPG::check_recovery_sources(const OSDMapRef osdmap)
{
  /*
...
int EncodedPG::start_recovery_ops(
 ...
 */
int EncodedPG::recover_primary(int max, ThreadPool::TPHandle &handle)
{
  assert(is_primary());
...
int EncodedPG::prep_object_replica_pushes(
  const hobject_t& soid, eversion_t v,
  PGBackend::RecoveryHandle *h)
{
...
int EncodedBackend::start_pushes(
  const hobject_t &soid,
  ObjectContextRef obc,
  RPGHandle *h)
{
...
int EncodedPG::recover_replicas(int max, ThreadPool::TPHandle &handle)
{
 ...
 */
int EncodedPG::recover_backfill(
  int max,
  ThreadPool::TPHandle &handle)
{
...
void EncodedPG::prep_backfill_object_push(
...
void EncodedPG::_scrub(ScrubMap& scrubmap)
{
...
void EncodedPG::_scrub_clear_state()
{
  scrub_cstat = object_stat_collection_t();
}

void EncodedPG::_scrub_finish()
{
  bool repair = state_test(PG_STATE_REPAIR);
  bool deep_scrub = state_test(PG_STATE_DEEP_SCRUB);
  const char *mode = (repair ? "repair": (deep_scrub ? "deep-scrub" : "scrub"));
...
EncodedPG::SnapTrimmer::~SnapTrimmer()
{
  while (!repops.empty()) {
    (*repops.begin())->put();
...
void EncodedPG::SnapTrimmer::log_enter(const char *state_name)
{
  dout(20) << "enter " << state_name << dendl;
}

void EncodedPG::SnapTrimmer::log_exit(const char *state_name, utime_t enter_time)
{
...
/* NotTrimming */
EncodedPG::NotTrimming::NotTrimming(my_context ctx)
  : my_base(ctx), 
...
void EncodedPG::NotTrimming::exit()
{
  context< SnapTrimmer >().requeue = true;
  context< SnapTrimmer >().log_exit(state_name, enter_time);
}

boost::statechart::result EncodedPG::NotTrimming::react(const SnapTrim&)
{
  EncodedPG *pg = context< SnapTrimmer >().pg;
...
/* TrimmingObjects */
EncodedPG::TrimmingObjects::TrimmingObjects(my_context ctx)
  : my_base(ctx),
...
void EncodedPG::TrimmingObjects::exit()
{
  context< SnapTrimmer >().log_exit(state_name, enter_time);
}

boost::statechart::result EncodedPG::TrimmingObjects::react(const SnapTrim&)
{
...
EncodedPG::WaitingOnReplicas::WaitingOnReplicas(my_context ctx)
  : my_base(ctx),
...
void EncodedPG::WaitingOnReplicas::exit()
{
  context< SnapTrimmer >().log_exit(state_name, enter_time);
...
boost::statechart::result EncodedPG::WaitingOnReplicas::react(const SnapTrim&)
{
...
void intrusive_ptr_add_ref(EncodedPG *pg) { pg->get("intptr"); }
void intrusive_ptr_release(EncodedPG *pg) { pg->put("intptr"); }

#ifdef PG_DEBUG_REFS
uint64_t get_with_id(EncodedPG *pg) { return pg->get_with_id(); }
void put_with_id(EncodedPG *pg, uint64_t id) { return pg->put_with_id(id); }
#endif
