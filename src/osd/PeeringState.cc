...
bool PeeringState::should_restart_peering(
  ...
  OSDMapRef osdmap)
{
  const vector<int> newacting = osdmap->pgtemp_undo_primaryfirst(pool.info, info.pgid.pgid, _newacting);
...
{
  actingset.clear();
  acting = get_osdmap()->pgtemp_undo_primaryfirst(pool.info, info.pgid.pgid, newacting);
