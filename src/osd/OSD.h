...
private:
  ...
  class ShardedOpWQ: public ShardedThreadPool::ShardedWQ < pair <PGRef, OpRequestRef> > {
    ...
    public:
      ...
      void _process(uint32_t shard_index, ThreadPool::TPHandle tphandle) {
        ...
        if (NULL != sdata) {
          ...
          if (!(item.first)->deleting){
            (item.first)->do_request(item.second, tphandle);
          }
          (item.first)->unlock();
        } else {
          assert(0);
      ...
      void _enqueue(pair <PGRef, OpRequestRef> item, uint32_t shard_index) {
      ...
      void _enqueue_front(pair <PGRef, OpRequestRef> item, uint32_t shard_index) {
      ...
      void _lock_shard(uint32_t shard_index) {
        ShardData* sdata = shard_list[shard_index];
        if (NULL != sdata) {
          sdata->sdata_lock.Lock();
        } else {
      ...
      void _unlock_shard(uint32_t shard_index) {
        ShardData* sdata = shard_list[shard_index];
        if (NULL != sdata) {
          sdata->sdata_lock.Unlock();
        } else {
      ...
      void _wait_on_shard(uint32_t shard_index) {
        ShardData* sdata = shard_list[shard_index];
        if (NULL != sdata) {
      ...
      void _signal_shard_threads(uint32_t shard_index) {
        ShardData* sdata = shard_list[shard_index];
        if (NULL != sdata) {
          sdata->sdata_lock.Lock();
          sdata->sdata_cond.SignalOne();
      ...
      uint32_t calculate_shard_index(pair <PGRef, OpRequestRef> item) {

        return (((item.first)->get_pgid().ps())% shard_list.size()) ;
      }
