...
void ObjectCacher::flush(ZTracer::Trace *trace, loff_t amount, loff_t max_bhs)
{
  ceph_assert(trace != nullptr);
  ceph_assert(ceph_mutex_is_locked(lock));
  ceph::real_time cutoff = ceph::real_clock::now();

  ldout(cct, 10) << "flush " << amount << " bytes, max bufferheads " << max_bhs << dendl;
  ...
  while ((amount == 0 && max_bhs == 0) || left > 0 || left_bhs > 0) {
    ...
    if (scattered_write) {
      bh_write_adjacencies(bh, cutoff, amount > 0 ? &left : NULL, max_bhs > 0 ? &left_bhs : NULL);
    } else {
      left -= bh->length();
      left_bhs--;
      bh_write(bh, *trace);
...
void ObjectCacher::flusher_entry()
{
  ...
  uint64_t target_dirty_bh = target_dirty >> BUFFER_MEMORY_WEIGHT;
  ...
  while (!flusher_stop) {
    ...
    if (dirty_or_tx_bh.size() > target_dirty_bh) {
      loff_t max_bhs = dirty_or_tx_bh.size() - target_dirty_bh;
      ldout(cct, 10) << "flusher " << dirty_or_tx_bh.size() << " dirty/tx bh > "
                     << target_dirty_bh << " target dirty bh, "
                     << "flushing some dirty bhs" << dendl;
      flush(&trace, 0, max_bhs);
    }
