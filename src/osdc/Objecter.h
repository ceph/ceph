...
private:
  std::atomic<int64_t> last_tid;
  std::atomic<unsigned> inflight_ops;
  std::atomic<unsigned> client_inc;
  uint64_t max_linger_id;
  std::atomic<unsigned> num_in_flight;
  std::atomic<unsigned> global_op_flags; // flags which are applied to each IO op
...
public:
 ...
 private:
  ...
  std::atomic<unsigned> num_homeless_ops;
...
public:
  ...
  bool is_active() {
    shared_lock l(rwlock);
    return !((!inflight_ops.load()) && linger_ops.empty() &&
  ...
  int get_client_incarnation() const { return client_inc.load(); }
...
public:
  ...
  Op *prepare_mutate_op(
    ...
    Op *o = new Op(oid, oloc, op.ops, flags | global_op_flags.load() |
  ...
  Op *prepare_pg_read_op(
    ...
    Op *o = new Op(object_t(), oloc,
		   op.ops,
		   flags | global_op_flags.load() | CEPH_OSD_FLAG_READ |
  ...
  Op *prepare_stat_op(
    ...
    Op *o = new Op(oid, oloc, ops, flags | global_op_flags.load() |
  ...
  ceph_tid_t read_full(const object_t& oid, const object_locator_t& oloc,
		       ...
		       ObjectOperation *extra_ops = NULL) {
    return read(oid, oloc, 0, 0, snap, pbl, flags | global_op_flags.load() |
  ...
  ceph_tid_t create(const object_t& oid, const object_locator_t& oloc,
    ...
    Op *o = new Op(oid, oloc, ops, global_flags | global_op_flags.load() |
		   CEPH_OSD_FLAG_WRITE, oncommit, objver);
    o->mtime = mtime;
    o->snapc = snapc;
    ceph_tid_t tid;
