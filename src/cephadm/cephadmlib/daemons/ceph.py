...
class Ceph(ContainerDaemonForm):
    ...
    def setup_qat_args(self, ctx: CephadmContext, args: List[str]) -> None:
        try:
            ...
            logger.error(f'[QAT] Could not list /dev/vfio/devices: {e}')
            devices = []
    ...
    def customize_container_args(
        ...
        if qat_raw is None:
            ...
            raise Error(f'Invalid qat config: expected dict got {type(qat_raw.__name__)}')

        if self.identity.daemon_type == 'rgw' and qat_config.get('compression') == 'hw':
