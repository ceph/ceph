...
class Tracing(ContainerDaemonForm):
    ...
    components: Dict[str, Dict[str, Any]] = {
        'elasticsearch': {
            'image': DefaultImages.ELASTICSEARCH.value.name,
            'envs': ['discovery.type=single-node'],
        },
        'jaeger-agent': {
            'image': DefaultImages.JAEGER_AGENT.value.name,
        },
        'jaeger-collector': {
            'image': DefaultImages.JAEGER_COLLECTOR.value.name,
        },
        'jaeger-query': {
            'image': DefaultImages.JAEGER_QUERY.value.name,
        },
    }  # type: ignore
