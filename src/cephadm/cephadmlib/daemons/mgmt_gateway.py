...
class MgmtGateway(ContainerDaemonForm):
    ...
    def create_daemon_dirs(self, data_dir: str, uid: int, gid: int) -> None:
        ...
        conf_files = {fname: content for fname, content in self.files.items() if fname.endswith('.conf')}
        cert_files = {fname: content for fname, content in self.files.items() if fname.endswith('.crt') or fname.endswith('.key')}
