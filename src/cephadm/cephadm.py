...
class OSD(object):
    ...
    def rotate_osd_lv_keyring(
        ...
        if not osd_lv_data:
            logger.info(
                'No lv data found for OSD.%s, checking for raw OSD', self.daemon_id
            )
            c_v_container = CephContainer(
                ctx,
                image=ctx.image,
            ...
            for osd_data in raw_list_data.values():
                if str(osd_data.get('osd_id', -1)) == str(self.daemon_id):
                    logger.info('Found data for osd.%s in raw list', str(self.daemon_id))
        ...
        call(
            ctx,
            [ctx.container_engine.path, 'stop', 'ceph-%s-osd-%s' % (self.fsid, self.daemon_id)],
            verbosity=CallVerbosity.QUIET_UNLESS_ERROR,
        )
...
""" % (
            ...
            logger.info(
                f'Opening osd.{self.daemon_id} with crypsetup'
            )
            out, err, ret = call(
                ctx,
        ...
        logger.info(
            f'Rotating osd.{self.daemon_id} key with ceph-bluestore-tool'
        )
        ...
        for i in [2, 5, 10, 30, 0]:
            ...
            if ret:
                ...
                logger.info(f'Successfully rotated osd.{self.daemon_id} keyring')
                break
        ...
        call(ctx, ['systemctl', 'reset-failed', get_unit_name(self.fsid, 'osd', self.daemon_id)],
             verbosity=CallVerbosity.QUIET_UNLESS_ERROR)
        call(
            ctx,
            ['systemctl', 'start', get_unit_name(self.fsid, 'osd', self.daemon_id)],
            verbosity=CallVerbosity.QUIET_UNLESS_ERROR,
        )
