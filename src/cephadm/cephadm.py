...
def command_check_mtu(ctx: CephadmContext) -> int:
    ...
    iface = get_route_interface(ctx, target_ip, ipv6)
    if iface == 'lo':
    ...
    check_mtu_ping(ctx, target_ip, payload_size, ipv6)
    logger.info('MTU check OK for %s via %s (mtu=%s)', target_ip, iface, mtu)
    return 0


def get_route_interface(ctx: CephadmContext, addr: str, ipv6: bool) -> str:
    cmd = ['ip', '-6', '-j', 'route', 'get', addr] if ipv6 else ['ip', '-j', 'route', 'get', addr]
    out, err, code = call(ctx, cmd, verbosity=CallVerbosity.QUIET_UNLESS_ERROR)
    if code:
        raise Error(
            f'Unable to determine source interface to reach {addr}: '
            f'ip route command failed: {err or out}')
    try:
        routes = json.loads(out)
    except (json.JSONDecodeError, TypeError) as e:
        raise Error(
            f'Unable to determine source interface to reach {addr}: '
            f'invalid JSON from ip route: {e}')
    ...
    if not dev:
        raise Error(
            f'Unable to determine source interface to reach {addr}: '
            'route does not contain a network interface')
    if route.get('type') == 'local' or dev == 'lo':
        return 'lo'
    return dev


def get_interface_mtu(iface: str) -> int:
    try:
...
def check_mtu_ping(ctx: CephadmContext, addr: str, payload_size: int, ipv6: bool) -> None:
    try:
