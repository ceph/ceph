...
def check_sanity():
    ...
    with open(tmp_file, 'w') as fp:
        ...
        #include <stddef.h>
        #include "rgw/librgw_admin_user.h"

        int main(void) {
            rgw_admin_user_version(NULL, NULL, NULL);
            return 0;
        }
        """)
        fp.write(dummy_prog)
    ...
    try:
        ...
        compiler.link_executable(
            ...
            libraries=['rgw_admin_user'],
            output_dir=tmp_dir,
        )

    except CompileError:
        print('\nCompile Error: RGW admin user development headers not found', file=sys.stderr)
        return False
    except LinkError:
        print('\nLink Error: RGW admin user library not found', file=sys.stderr)
        return False
    else:
        return True
    finally:
...
try:
    ...
    if not os.path.isfile('rgw_admin_user.c'):
        print('ERROR: Cannot find Cythonized file rgw_admin_user.c', file=sys.stderr)
        sys.exit(1)
    else:
        def cythonize(x, **kwargs):
            return x

        source = "rgw_admin_user.c"
else:
    source = "rgw_admin_user.pyx"
...
setup(
    name='rgw_admin_user',
    version=__version__,
    description="Python bindings for the RGW admin user library",
    long_description=(
        "This package contains Python bindings for interacting with the "
        "RGW admin library, a library of functions typically run before "
        "the RGW admin rest interface can be used."
    ),
    url='https://github.com/ceph/ceph/tree/master/src/pybind/rgw_admin_user',
    license='LGPLv2+',
    platforms='Linux',
    ext_modules=cythonize(
        [
            Extension(
                ...
                libraries=['rgw_admin_user'] + flags['ldflags']['l'],
