from typing import Any, Dict, Protocol, Union, TypeVar
from orchestrator import OrchestratorError


class CredentialException(OrchestratorError):
    pass
...
class CredentialProtocol(Protocol):
    ...
    @classmethod
    def from_json(cls, data: Dict[str, Any]) -> 'CredentialProtocol':
        ...


class Cert(CredentialProtocol):
    STORAGE_PREFIX = 'cert'
    ...
    def from_json(cls, data: Dict[str, Union[str, bool]]) -> 'Cert':
        ...
        if not isinstance(cert, str):
            raise CredentialException('Tried to make Cert object with non-string cert')
        if any(k not in ['cert', 'user_made'] for k in data.keys()):
            raise CredentialException(f'Got unknown field for Cert object. Fields: {data.keys()}')
        ...
        if not isinstance(user_made, bool):
            ...
            try:
                ...
                raise CredentialException(f'Expected user_made field in Cert object to be bool but got {type(user_made)}')
        return cls(cert=cert, user_made=user_made)


class PrivKey(CredentialProtocol):
    STORAGE_PREFIX = 'key'
    ...
    def from_json(cls, data: Dict[str, str]) -> 'PrivKey':
        ...
        if not isinstance(key, str):
            raise CredentialException('Tried to make PrivKey object with non-string key')
        if any(k not in ['key', 'user_made'] for k in data.keys()):
            raise CredentialException(f'Got unknown field for PrivKey object. Fields: {data.keys()}')
        user_made: Union[str, bool] = data.get('user_made', False)
        if not isinstance(user_made, bool):
            if isinstance(user_made, str):
                if user_made.lower() == 'true':
            ...
            try:
                ...
                raise CredentialException(f'Expected user_made field in PrivKey object to be bool but got {type(user_made)}')
