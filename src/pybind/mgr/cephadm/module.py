    ...
    def _process_ls_output(self, host: str, ls: List[Dict[str, Any]]) -> None:
        ...
        for d in ls:
            ...
            if daemon_type in REQUIRES_POST_ACTIONS:
                ...
                if host in self.cache.daemons.keys() and sd.name() in self.cache.daemons[host].keys():
                    cached_dd = self.cache.daemons[host][sd.name()]
                    sd.pending_daemon_config = cached_dd.pending_daemon_config
            dm[sd.name()] = sd
