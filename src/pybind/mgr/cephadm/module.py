...
from .ceph_volume import CephVolumeLvmList
...
class CephadmOrchestrator(orchestrator.Orchestrator, MgrModule,
    ...
    def __init__(self, *args: Any, **kwargs: Any):
        ...
        self.ceph_volume_list: CephVolumeLvmList = CephVolumeLvmList(self)
...
> ssh -F ssh_config -i ~/cephadm_private_key {self.ssh_user}@{addr}'''
    ...
    def replace_device(self,
        ...
        self.ceph_volume_list.get_data(hostname=hostname)

        if device not in self.ceph_volume_list.all_devices():
            raise OrchestratorError(f"{device} doesn't appear to be used for an OSD, not a valid device in {hostname}.")

        device_osd_mapping = self.ceph_volume_list.device_osd_mapping()
        osds_to_zap = device_osd_mapping[device]['osd_ids']

        if self.ceph_volume_list.is_shared_device(device):
            if not yes_i_really_mean_it:
        ...
        if not self.to_remove_osds.rm_util.safe_to_destroy([int(osd_id) for osd_id in osds_to_zap]):
            raise OrchestratorError(f"Destroying OSD(s) {osds_to_zap} would cause some PGs to be undersized/degraded.\n"
                                    'Refusing to proceed.')
        replace_block: bool = self.ceph_volume_list.is_block_device(device)
        replace_db: bool = self.ceph_volume_list.is_db_device(device)
        replace_wal: bool = self.ceph_volume_list.is_wal_device(device)
        ...
        return f'Scheduled to destroy osds: {osds_to_zap} and mark {device} as being replaced.'
