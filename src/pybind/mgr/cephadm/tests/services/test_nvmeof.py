...
class TestNVMEOFService:
    ...
    def test_ok_to_stop_force_bypasses_error_but_warns(self):
        """The --force flag should not block the stop but should still emit a warning."""
        self.mgr.cache._daemons = [
            self._make_daemon('pool.grp1.node0.aaa', 'node0', 'nvmeof.pool.grp1',
                              status=DaemonDescriptionStatus.stopped),
            self._make_daemon('pool.grp1.node1.bbb', 'node1', 'nvmeof.pool.grp1',
        ...
        assert result.retval == 0
        assert 'nvmeof.pool.grp1' in result.stdout
        assert 'no running nvmeof daemons' in result.stdout
