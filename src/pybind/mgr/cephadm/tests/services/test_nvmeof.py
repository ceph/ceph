...
timeout = 1.0\n"""
    ...
    def test_validate_no_group_duplicate_on_apply(self, cephadm_module: CephadmOrchestrator):
        cephadm_module.mock_store_set('_ceph_get', 'osd_map', {
            'pools': [
                {'pool': 1, 'pool_name': 'testpool', 'type': 1},
                {'pool': 2, 'pool_name': 'testpool2', 'type': 1},
            ]
        })
    ...
    def test_validate_service_id_matches_group_on_apply(self, cephadm_module: CephadmOrchestrator):
        cephadm_module.mock_store_set('_ceph_get', 'osd_map', {
            'pools': [
                {'pool': 1, 'pool_name': 'pool1', 'type': 1},
                {'pool': 2, 'pool_name': 'pool2', 'type': 1},
                {'pool': 3, 'pool_name': 'pool3', 'type': 1},
                {'pool': 4, 'pool_name': 'pool4', 'type': 1},
            ]
        })
    ...
    @patch("cephadm.serve.CephadmServe._run_cephadm", _run_cephadm('{}'))
    def test_nvmeof_reject_pool_not_in_osd_map(self, cephadm_module: CephadmOrchestrator):
        cephadm_module.mock_store_set('_ceph_get', 'osd_map', {
            'pools': [
                {'pool': 1, 'pool_name': 'otherpool', 'type': 1},
            ]
        })
        nvmeof_spec = NvmeofServiceSpec(
            service_id='missingpool.testgroup',
            group='testgroup',
            pool='missingpool'
        )
        with with_host(cephadm_module, 'test'):
            with pytest.raises(
                OrchestratorError,
                match='Pool "missingpool" was not found in the OSD map'
            ):
                cephadm_module._apply_service_spec(nvmeof_spec)
