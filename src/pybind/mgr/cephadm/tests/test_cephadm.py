...
Traceback (most recent call last):
  ...
  File "<stdin>", line 1061, in call_throws"""
    ...
    @mock.patch("cephadm.serve.CephadmServe._run_cephadm")
    def test_prometheus_module_enable(self, _run_cephadm, cephadm_module: CephadmOrchestrator):
        _run_cephadm.return_value = ('{}', '', 0)

        cephadm_module._store['mock_store/_ceph_get/mgr_map'] = {'modules': []}
        cephadm_module.notify('mgr_map', None)

        assert cephadm_module.prometheus_service.get_mgr_scrape_list() == []

        with with_host(cephadm_module, 'test'):
            with with_service(cephadm_module, ServiceSpec(service_type='prometheus')) as d_names:
                [daemon_name] = d_names

                p_yaml = """# This file is generated by cephadm.
global:
  scrape_interval: 10s
...
"""
                ...
                j = {
                    'files': {
                        'prometheus.yml': p_yaml
                    }
                }

                _run_cephadm.assert_called_with('test', 'prometheus.test', 'deploy', [
                    '--name', 'prometheus.test',
                    '--meta-json', '{"service_name": "prometheus"}',
                    '--config-json', '-'], image='', stdin=json.dumps(j))

                cephadm_module._store['mock_store/_ceph_get/mgr_map'] = {
                    'modules': [], 'services': {'prometheus': 'http://test'}}
                cephadm_module.notify('mgr_map', None)
