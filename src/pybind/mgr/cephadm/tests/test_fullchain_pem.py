...
class TestSplitFullchainPem:
    ...
    def test_raises_on_encrypted_private_key(self):
        """Encrypted private keys cannot be used — cephadm has no passphrase prompt."""
        from cephadm.ssl_cert_utils import SSLConfigException
        with pytest.raises(SSLConfigException, match='Encrypted private keys are not supported'):
            self.split(_encrypted_fullchain())

    def test_raises_on_unsupported_private_key(self):
        from cephadm.ssl_cert_utils import SSLConfigException
        blob = "\n".join([
            "-----BEGIN DSA PRIVATE KEY-----\nunsupported\n-----END DSA PRIVATE KEY-----",
            LEAF_CERT,
        ])
        with pytest.raises(SSLConfigException, match='Unsupported private key PEM type'):
            self.split(blob)
...
class TestPemDetectors:
    ...
    def test_contains_private_key_encrypted(self):
        from cephadm.ssl_cert_utils import contains_private_key
        assert contains_private_key(_encrypted_fullchain()) is True
    ...
    def test_encrypted_private_key_raises(self, _set_store, cephadm_module):
        ...
        with pytest.raises(SSLConfigException, match='Encrypted private keys are not supported'):
            cm.save_cert_key_from_pem(
                'rgw_ssl_cert', 'rgw_ssl_key', _encrypted_fullchain(), service_name='rgw.x'
            )
...
class TestGetCertificatesFromSpecFullchain:
    ...
    def test_spec_encrypted_private_key_returns_empty(self, _set_store, cephadm_module):
        ...
        spec.ssl_cert = _encrypted_fullchain()
        spec.ssl_key = None
