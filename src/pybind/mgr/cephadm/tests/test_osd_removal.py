...
class TestOSDRemoval:
    ...
    @mock.patch('cephadm.services.osd.OSD.exists', return_value=True)
    def test_load(self, osd_exists, cephadm_module):
        data = json.dumps([
            {
                "osd_id": 35,
                "started": True,
        ...
        assert repr(cephadm_module.to_remove_osds.queue) == '{<OSD>(osd_id=35, draining=True)}'
