...
class TestReplaceDevice:
    def test_invalid_device(self, cephadm_module) -> None:  # type: ignore
    ...
    def test_invalid_hostname(self, cephadm_module) -> None:  # type: ignore
        with patch("cephadm.serve.CephadmServe._run_cephadm", _run_cephadm('[]')):
            with with_host(cephadm_module, 'test'):
                CephadmServe(cephadm_module)._refresh_host_daemons('test')
                with patch('cephadm.serve.CephadmServe._run_cephadm', _run_cephadm(data)):
    ...
    def test_block_device(self, cephadm_module) -> None:  # type: ignore
        with patch("cephadm.serve.CephadmServe._run_cephadm", _run_cephadm('[]')):
            with with_host(cephadm_module, 'test'):
                CephadmServe(cephadm_module)._refresh_host_daemons('test')
                with patch('cephadm.serve.CephadmServe._run_cephadm', _run_cephadm(data)):
    ...
    def test_shared_db_device_no_ireallymeanit_flag(self, cephadm_module) -> None:  # type: ignore
        with patch("cephadm.serve.CephadmServe._run_cephadm", _run_cephadm('[]')):
            with with_host(cephadm_module, 'test'):
                CephadmServe(cephadm_module)._refresh_host_daemons('test')
                with patch('cephadm.serve.CephadmServe._run_cephadm', _run_cephadm(data)):
    ...
    def test_shared_db_device(self, cephadm_module) -> None:  # type: ignore
        with patch("cephadm.serve.CephadmServe._run_cephadm", _run_cephadm('[]')):
            with with_host(cephadm_module, 'test'):
                CephadmServe(cephadm_module)._refresh_host_daemons('test')
                with patch('cephadm.serve.CephadmServe._run_cephadm', _run_cephadm(data)):
                    c = cephadm_module.replace_device('test', '/dev/vdk', True)
                    result = wait(cephadm_module, c)
