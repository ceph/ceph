...
import re
import logging
from typing import TYPE_CHECKING, Iterator, Optional, Dict, Any, List, Union
import copy
...
class Migrations:
    ...
    def migrate_5_6(self) -> bool:
        ...
        for s in self.rgw_migration_queue:
            ...
            self.rgw_ssl_migration_queue.append(rgw_spec)
        self.mgr.set_store('rgw_ssl_migration_queue', json.dumps(self.rgw_ssl_migration_queue))
        ...
        for grafana_daemon in self.mgr.cache.get_daemons_by_type('grafana'):
            ...
            if grafana_cert:
                ...
                if org == 'Ceph':
                    ...
                    self.mgr.cert_mgr.save_self_signed_cert_key_pair('grafana', grafana_cert, grafana_key, host=hostname)
                else:
