...
class NFSService(CephService):
    ...
    def _register_grpc_certs(self) -> None:
        """Register the five gRPC cert/key names with the cert and key stores."""
        ...
        self.mgr.cert_mgr.cert_store.register_object_name(NFS_GRPC_CLIENT_CERT, TLSObjectScope.SERVICE)
        self.mgr.cert_mgr.key_store.register_object_name(NFS_GRPC_CLIENT_KEY, TLSObjectScope.SERVICE)
    ...
    def _get_grpc_certs(self, spec: NFSServiceSpec,
        ...
        # cephadm-signed: generate once per service, store at SERVICE scope
        server_cert, server_key = self._get_or_generate_grpc_service_cert(
            NFS_GRPC_SERVER_CERT, NFS_GRPC_SERVER_KEY, svc_name, svc_name)
        client_cert, client_key = self._get_or_generate_grpc_service_cert(
            NFS_GRPC_CLIENT_CERT, NFS_GRPC_CLIENT_KEY, svc_name + '-grpc-client', svc_name)
    ...
    def generate_config(self, daemon_spec: CephadmDaemonDeploySpec) -> Tuple[Dict[str, Any], List[str]]:
        ...
        def get_cephadm_config() -> Dict[str, Any]:
            ...
            config['files'].update({
                ...
                'grpc_client.crt': client_cert,
                'grpc_client.key': client_key,
                'grpc_ca.crt': ca_cert,
            })
            config.update(
