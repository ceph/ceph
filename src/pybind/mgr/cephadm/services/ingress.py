...
from ceph.deployment.service_spec import ServiceSpec, IngressSpec, MonitorCertificateSource
...
class IngressService(CephService):
    ...
    def haproxy_generate_config(
        ...
        cert_ips = [ip]
        if spec.monitor_ssl:
            if spec.monitor_certificate_source == MonitorCertificateSource.REUSE_SERVICE_CERT.value:
        ...
        if spec.ssl:
            cert, key = self.get_certificates(daemon_spec, ips=cert_ips)
            config_files['files']['haproxy.pem'] = cert
            config_files['files']['haproxy.pem.key'] = key

        if spec.monitor_ssl and spec.monitor_certificate_source != MonitorCertificateSource.REUSE_SERVICE_CERT.value:
    ...
    def get_stats_certs(
        ...
        return self.get_certificates_generic(
            ...
            cert_source_attr='monitor_certificate_source',
