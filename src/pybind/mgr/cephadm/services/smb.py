...
class SMBService(CephService):
    ...
    def prepare_create(
        ...
        logger.warning('smb prepare_create')
    ...
    def generate_config(
        ...
        logger.warning('smb generate_config')
        assert self.TYPE == daemon_spec.daemon_type
        smb_spec = cast(
            SMBSpec, self.mgr.spec_store[daemon_spec.service_name].spec
        )
        # assert isinstance(smb_spec, SMBSpec)
        ...
        logger.warning('smb generate_config: %r', config_blobs)
        return config_blobs, []
