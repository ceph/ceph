...
class NvmeofService(CephService):
    ...
    def prepare_create(self, daemon_spec: CephadmDaemonDeploySpec) -> CephadmDaemonDeploySpec:
        ...
        self.mgr.log.info(f"gateway address: {addr} from {map_addr=} {spec.addr=}")
        discovery_addr = map_discovery_addr or spec.discovery_addr or host_ip
        self.mgr.log.info(f"discovery address: {discovery_addr} from {map_discovery_addr=} {spec.discovery_addr=}")
        context = {
            'spec': spec,
            'name': name,
            'addr': addr,
