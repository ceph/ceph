...
class CephadmUpgrade:
    UPGRADE_ERRORS = [
        ...
        'UPGRADE_FAILED_SETTING_MON_FLAGS'
    ]
    ...
    def upgrade_start(self, image: str, version: str, daemon_types: Optional[List[str]] = None,
                      ...
                      limit: Optional[int] = None, no_mon_flags: bool = False) -> str:
        ...
        mon_flags = None if no_mon_flags else [f.strip() for f in self.mgr.upgrade_mon_flags.split(',')]
    ...
    def _do_upgrade(self):
        ...
        if self.upgrade_state.mon_flags:
            try:
                ...
                self._fail_upgrade('UPGRADE_FAILED_SETTING_MON_FLAGS', {
                    'severity': 'error',
                    'summary': 'Upgrade: Failed to set mon flags for upgrade',
                    'count': 1,
                    'detail': [f'Upgrade: Failed to set mon flags for upgrade: {str(e)}'],
                })
        ...
        # Upgrade is complete. Unset mon flags
        if self.upgrade_state.mon_flags:
            try:
                currently_set_flags = self.mgr.get_set_global_mon_flags()
                for flag in self.upgrade_state.mon_flags:
                ...
                self.mgr.log.error(f'Failed to unset MON flags at end of upgrade: {str(e)}')
