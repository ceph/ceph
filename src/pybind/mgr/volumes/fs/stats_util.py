...
class TrashStats:
    ...
    def are_laggy(self):
        return self.total_files < self.files_left or \
               self.total_subvols < self.subvols_left or \
               self.total_size < self.size_left
...
class PurgeProgressBar(VolumesProgressBar):
    ...
    def are_rstats_only_slightly_laggy(self):
        return True if self.laggy_count < self.LAGGY_NOTE_LIMIT else False

    def are_rstats_too_laggy(self):
        return True if self.laggy_count >= self.LAGGY_NOTE_LIMIT else False

    def should_print_note_for_lag(self):
        '''
        Are rstats laggy enough for printing note along with progress bar but
        not laggy enough to disable it?
        '''
        if self.are_rstats_too_laggy() and not self.should_disable_prog_bar():
            return True
        else:
            return False

    def should_disable_prog_bar(self):
        '''
        Are rstats laggy enough to disable printing the progress bar?
        '''
        if self.laggy_count >= self.LAGGY_DISABLE_LIMIT:
            return True
        else:
            return False
    ...
    def _update_progress_bars(self):
        ...
        if self.trash_stats.are_laggy():
            ...
            if self.are_rstats_only_slightly_laggy():
                return
            elif self.should_print_note_for_lag() and self.prev_msg and \
                 self.prev_fraction:
                ...
                self._update_progress_bar_event(self.pev_id, self.prev_msg,
                                                    self.prev_fraction)
                return
            elif self.should_disable_prog_bar():
