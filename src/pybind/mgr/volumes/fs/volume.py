...
class VolumeClient(CephfsClient["Module"]):
    ...
    def _create_purge_status_report(self, volname):
        ...
        if not total_subvols and not total_files and not total_size:
            return {'status': {'progress_report': {'note':
        ...
        if self.purge_progress_bar.are_rstats_too_laggy():
        ...
        if subvols_purged_percent == 'NA':
            percent_purged['subvols'] = subvols_purged_percent
        else:
            percent_purged['subvols'] = f'{subvols_purged_percent}%'
        if files_purged_percent == 'NA':
            percent_purged['files'] = files_purged_percent
        else:
            percent_purged['files'] = f'{files_purged_percent}%'
        if size_purged_percent == 'NA':
            percent_purged['size'] = size_purged_percent
        else:
