...
class AsyncJobs(threading.Thread):
    ...
    def pause(self):
        ...
        log.info('pause() cancelling ongoing jobs now and respective worker '
                 'threads...')
        self.cancel_all_jobs(update_queue=False)

        # There are 2 major issues here -
        #
        # First, cancel_all_jobs() will set jobthread.cancel_event causing all
        # ongoing jobs to cancel. But if there are no jobs (that is self.q is
        # empty), cancel_all_jobs() will return without doing anything and
        # jobthread.cancel_event won't be set. Resulting in future jobs to be
        # executed even when config option to pause are already set. Therefore,
        # set it explicitly.
        #
        # Second, if, let's say, there's only one ongoing job, calling
        # cancel_all_jobs() will cancel and set jobthread.cancel_event but it
        # will not set the same on other threads, since no job is executing on
        # them. This means new jobs can be picked and executed by other threads.
        # Therefore, set jobthread.cancel_event for other threads as well.
        log.info('pause() pausing rest of worker threads')
        for t in self.threads:
            if not t.cancel_event.is_set():
                t.cancel_event.set()
        log.info('pause() all jobs cancelled and cancel_event have been set for '
    ...
    def resume(self):
        ...
        log.info('resume() enabling worker threads')
        for t in self.threads:
            t.cancel_event.clear()

        self.run_event.set()
        log.info('resume() run_event has been set, queue and threads have been'
                  'resumed')
