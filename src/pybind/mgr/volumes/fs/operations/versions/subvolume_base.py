...
class SubvolumeBase(object):
    ...
    def snapshot_visibility_set(self, value):
        ...
        # changes are not passed on to the children SnapRealms, so, for
        # subvolumes mounted as NFS shares, just updating the snapshot
        # visibility vxattr for subvolume root is not enough to invalidate the
        # NFS side cache. This leads to an anomalous behaviour where the old
