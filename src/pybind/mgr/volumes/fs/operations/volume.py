...
def create_fs_pools(mgr, volname, data_pool, meta_pool):
    if not data_pool and not meta_pool:
        ...
        meta_pool = gen_pool_names(volname)[0]
    ...
    if ',' in data_pool:
        pools = tuple(data_pool.split(',') + meta_pool)
    else:
        pools = (data_pool, meta_pool)
    non_existent_pools = get_non_existent_pools(mgr, pools)

    if meta_pool in non_existent_pools:
        r, outb, outs = create_pool(mgr, meta_pool)
        if r != 0:
            return r, outb, outs
        cleanup_meta_pool = True
        non_existent_pools.remove(meta_pool)
    else:
        cleanup_meta_pool = False
    ...
    # cleanup
    if r != 0:
        if cleanup_meta_pool:
            non_existent_pools.append(meta_pool)
