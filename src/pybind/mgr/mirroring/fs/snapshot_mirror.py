...
class FSSnapshotMirror(object):
    ...
    def set_mirror_info(self, local_cluster_id, local_fsid, remote_fs):
        ...
        try:
            ...
            if e.errno == errno.EEXIST:
                raise MirrorException(-errno.EEXIST, f'peer mirrored by another cluster')
            else:
    ...
    def init_mapping(self, fspolicy):
        # TODO: make async if required (its just bootup anyway)x
        ...
        with self.lock:
            ...
            try:
                ...
                if filesystem in self.pool_policy:
                    raise MirrorException(-errno.EEXIST, f'filesystem {filesystem} is already mirrored')
                fspolicy = self.init_pool_policy(filesystem)
                create_mirror_object(fspolicy.ioctx)
                ...
                if fspolicy:
                    fspolicy.shutdown()
                return e.args[0], '', 'failed to enable mirroring'
