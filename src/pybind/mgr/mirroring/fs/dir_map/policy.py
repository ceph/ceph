...
class DirectoryState(object):
    def __init__(self, instance_id=None, mapped_time=None):
        ...
        self.state = State.STATE_UNASSOCIATED
        self.stalled = False
        self.transition = Transition(ActionType.ACTION_TYPE_NONE)
...
class Policy(object):
    ...
    def is_instance_action(action_type):
        return action_type in (ActionType.ACTION_TYPE_ACQUIRE,
                               ActionType.ACTION_TYPE_RELEASE)
    ...
    def is_shuffling(self, dir_path):
        ...
        return self.is_state_scheduled(self.dir_states[dir_path], State.STATE_SHUFFLING)
    ...
    def init(self, dir_mapping):
        with self.lock:
            for dir_path, dir_map in dir_mapping.items():
                ...
                dir_state = self.dir_states[dir_path]
                state = State.STATE_INITIALIZING if instance_id else State.STATE_ASSOCIATING
                if instance_id:
                    purging = dir_mapping.get('purging', False)
                    if purging:
                        dir_state.purging = True
                        state = State.STATE_DISASSOCIATING
                    else:
                        state = State.STATE_INITIALIZING
                else:
                    state = State.STATE_ASSOCIATING
    ...
    def execute_policy_action(self, dir_path, dir_state, policy_action):
        ...
        if policy_action == PolicyAction.POLICY_ACTION_MAP:
            done = self.map(dir_path, dir_state)
        elif policy_action == PolicyAction.POLICY_ACTION_UNMAP:
            self.unmap(dir_path, dir_state)
        elif policy_action == PolicyAction.POLICY_ACTION_REMOVE:
            if dir_state.state == State.STATE_UNASSOCIATED:
        ...
        with self.lock:
            ...
            if dir_state.transition.start_policy_action:
                ...
                if stalled:
                    ...
                    return ActionType.ACTION_TYPE_NONE
    ...
    def finish_action(self, dir_path, r):
        ...
        with self.lock:
            ...
            if dir_state.transition.final_state:
                ...
                dir_state.transition = Transition(ActionType.ACTION_TYPE_NONE)
            ...
            pending = not dir_state.transition.action_type == ActionType.ACTION_TYPE_NONE
    ...
    def add_dir(self, dir_path):
        ...
        with self.lock:
            ...
            if dir_state.state == State.STATE_INITIALIZING:
                return False
            return self.set_state(dir_state, State.STATE_ASSOCIATING)
    ...
    def remove_dir(self, dir_path):
        ...
        with self.lock:
            ...
            dir_state.purging = True
            return self.set_state(dir_state, State.STATE_DISASSOCIATING)
        ...
        with self.lock:
            if initial_update:
                ...
                for dir_path in shuffle_dirs_ss:
                    dir_state = self.dir_states[dir_path]
                    if self.set_state(dir_state, State.STATE_SHUFFLING):
                        shuffle_dirs.append(dir_path)
    ...
    def _remove_instances(self, instance_ids):
        ...
        for instance_id in instance_ids:
            ...
            for dir_path in dir_paths:
                dir_state = self.dir_states[dir_path]
                if self.is_state_scheduled(dir_state, State.STATE_DISASSOCIATING):
                    log.debug(f'dir_path {dir_path} is disassociating, ignoring...')
                    continue
                log.debug(f'shuffling dir_path {dir_path}')
                if self.set_state(dir_state, State.STATE_SHUFFLING, True):
                    shuffle_dirs.append(dir_path)
    ...
    def dir_status(self, dir_path):
        with self.lock:
            ...
            elif dir_state.state == State.STATE_ASSOCIATING:
                res['state'] = 'mapping'
            else:
                state = None
                dstate = dir_state.state
                if dstate == State.STATE_ASSOCIATING:
                    state = 'mapping'
                elif dstate == State.STATE_DISASSOCIATING:
                    state = 'unmapping'
                elif dstate == State.STATE_SHUFFLING:
                    state = 'shuffling'
                elif dstate == State.STATE_ASSOCIATED:
                    state = 'mapped'
                elif dstate == State.STATE_INITIALIZING:
                    state = 'resolving'
                res['state'] = state
