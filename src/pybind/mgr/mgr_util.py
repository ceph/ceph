...
print("######")
print(crypto.dump_privatekey(crypto.FILETYPE_PEM, pkey))""".format(dname)
...
if x509.has_expired():
    ...
    if no_after is not None:
        ...
        print(msg)""".format(base64.b64encode(crt.encode('ascii')).decode("ascii"))

        import subprocess
        result = subprocess.run(["/usr/bin/python3", "-c", _verify_cacrt_content], capture_output=True, text=True)
...
try:
    ...
    print(f'Invalid cert/key pair: %s' % e)""".format(
            base64.b64encode(crt.encode('ascii')).decode("ascii"),
