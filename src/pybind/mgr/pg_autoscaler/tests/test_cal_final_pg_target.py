...
class TestPgAutoscaler(object):
    ...
    def create_group(self,
        ...
        for pool_name, p in pools.items():
            ...
            pool_key = (pg_target, p['size'], bias, bulk)
    ...
    def helper_test(self,
        ...
        for root_id in root_map:
            final_ratios, pool_pg_targets, final_pg_targets, out_pools = self.autoscaler._calc_final_pg_target(
                root_map, root_id, even_pools, bulk_pools, 'first', pool_group[root_id])
            i = 0
            for pool_name, p in pools.items():
        ...
        if bulk_pools:
            ...
            for root_id in root_map:
                final_ratios, pool_pg_targets, final_pg_targets, out_pools = self.autoscaler._calc_final_pg_target(
                    root_map, root_id, even_pools, bulk_pools, 'second', pool_group[root_id])
                i = 0
        ...
        if even_pools:
            ...
            for root_id in root_map:
                final_ratios, pool_pg_targets, final_pg_targets, out_pools = self.autoscaler._calc_final_pg_target(
                    root_map, root_id, even_pools, bulk_pools, 'third', pool_group[root_id])
                i = 0
