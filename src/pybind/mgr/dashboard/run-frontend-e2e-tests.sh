...
# Check if package jq is already installed. RHEL only
packages=(jq); # Here an array variable, we can add more packages like packages=(jq docker chrome) etc
if ! rpm -q "${packages[0]}"; then yum install -y "${packages[0]}"; fi
