...
export class MotdNotificationService implements OnDestroy {
  public motd$: Observable<Motd | undefined>;
  public motdSource = new BehaviorSubject<Motd | undefined>(undefined);
  ...
  constructor(private motdService: MotdService) {
    ...
    this.subscription = of(true)
      .pipe(
        ...
        tap((motd: Motd | undefined) => this.processResponse(motd)),
        delay(60000),
        repeat()
      )
      .subscribe();
  ...
  hide() {
    ...
    this.motdSource.next(undefined);
  }

  processResponse(motd: Motd | undefined) {
