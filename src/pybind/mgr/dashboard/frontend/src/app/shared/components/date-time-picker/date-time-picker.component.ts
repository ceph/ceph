  ...
  onModelChange(event?: any) {
    if (event) {
      if (Array.isArray(event)) {
        ...
        const updatedMoment = initialMoment.set('hour', initialMoment.hour() % 12 + (event === 'PM' ? 12 : 0));
        this.datetime.time = moment(updatedMoment).format('HH:mm:ss');
        this.datetime.ampm = event;
      } else {
        const time = event
        this.datetime.time  = moment(`${this.datetime.date} ${time} ${this.datetime.ampm}`).format('HH:mm:ss');
      }
    }
    if (this.datetime) {
