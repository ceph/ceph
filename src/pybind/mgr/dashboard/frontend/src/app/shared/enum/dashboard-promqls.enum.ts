...
export enum MultiClusterPromqls {
  ...
  POOL_CAPACITY_UTILIZATION = 'topk(2, ceph_pool_bytes_used * on(pool_id, cluster) group_left(instance, name) ceph_pool_metadata)',
  POOL_IOPS_UTILIZATION = 'topk(2, sum by (cluster, pool_id) (irate(ceph_pool_rd[1m])) * on(pool_id, cluster) group_left(instance, name) ceph_pool_metadata) \n+ sum by (cluster, pool_id) (irate(ceph_pool_wr[1m])) * on(pool_id, cluster) group_left(instance, name) ceph_pool_metadata',
  POOL_THROUGHPUT_UTILIZATION = 'topk(2, sum by (cluster, pool_id) (irate(ceph_pool_rd_bytes[1m])) * on(pool_id, cluster) group_left(instance, name) ceph_pool_metadata) \n+ sum by (cluster, pool_id) (irate(ceph_pool_wr_bytes[1m])) * on(pool_id, cluster) group_left(instance, name) ceph_pool_metadata',
}
