...
describe('CephfsSnapshotScheduleService', () => {
  ...
  it('checkScheduleExists should match subvolume schedules when /.. has a trailing slash', () => {
    const path = '/volumes/Group1/subvol1/uuid';
    let exists: boolean | undefined;
  ...
  it('checkScheduleExists should treat legacy uppercase Y as equivalent to lowercase y', () => {
    const path = '/volumes/Group1/subvol1/uuid';
    let exists: boolean | undefined;

    service.checkScheduleExists(path, 'fs1', 1, RepeatFrequency.Yearly, false).subscribe((result) => {
      exists = result;
    });
  ...
  it('checkScheduleExists should not match sibling subvolume paths with a common prefix', () => {
    const path = '/volumes/g1/sv1';
    let exists: boolean | undefined;
    ...
    const req = httpTesting.expectOne(
      (request) =>
        request.method === 'GET' &&
        request.url.includes('api/cephfs/snapshot/schedule/fs1') &&
        request.url.includes('path=/volumes/g1/sv1')
    );
    req.flush([
      {
        path: '/volumes/g1/sv10',
        schedule: '3M',
        active: true,
        retention: {}
      }
  ...
  it('parseScheduleCopy should format legacy yearly schedules with uppercase Y', () => {
    expect(service.parseScheduleCopy('7Y')).toBe('Every 7 years');
    expect(service.parseScheduleCopy('1Y')).toBe('Every year');
  });
