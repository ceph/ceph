...
export class TableComponent implements AfterViewInit, OnInit, OnChanges, OnDestroy {
  ...
  */
  ...
  set tableColumns(value: CdTableColumn[]) {
    // In case a name is not provied check set it to the prop name if present or set it to an empty string
    const valuesWithNames = value.map((col) =>
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes?.data?.currentValue && _.isNil(this.expanded)) {
      this.useData();
    }
  }
