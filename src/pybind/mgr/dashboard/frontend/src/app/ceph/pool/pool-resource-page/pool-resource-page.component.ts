...
type PoolResourceResult = [Pool, RbdConfigurationEntry[], ErasureCodeProfile[], Pool[]];
type RatePoint = [number, number] | number;
...
export class PoolResourcePageComponent implements OnInit, OnDestroy {
    ...
    this.sub.add(
      this.route.parent?.paramMap
        ...
        .subscribe((result: PoolResourceResult | null) => {
          ...
          const [poolData, poolConf, ecProfiles, lightweightPoolList] = result;
  ...
  private buildOverviewModel(pool?: Pool, erasureProfile?: ErasureCodeProfile): PoolOverviewModel {
    ...
    return {
      ...
      pgStatus: transformPgStatus(poolData.pg_status),
  ...
  private buildOverviewFields(pool?: Pool): OverviewField[] {
    ...
    return [
      ...
      },
      {
        ...
        value: transformPgStatus(poolData.pg_status)
      }
    ];
  }
  ...
  private getRateChartData(stat: PoolStat | undefined, groupLabel: string): ChartPoint[] {
    ...
    return stat.rates.reduce((points: ChartPoint[], point: RatePoint, index: number) => {
