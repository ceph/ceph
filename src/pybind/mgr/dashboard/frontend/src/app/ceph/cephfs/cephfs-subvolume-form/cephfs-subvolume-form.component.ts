...
export class CephfsSubvolumeFormComponent extends CdForm implements OnInit {
  ...
  constructor(
    ...
    @Inject('fsName') public fsName: string,
    @Inject('subVolumeName') public subVolumeName: string,
    @Inject('subVolumeGroupName') public subVolumeGroupName: string,
    @Inject('pools') public pools: Pool[],
    @Inject('isEdit') public isEdit = false
  ) {
    super();
