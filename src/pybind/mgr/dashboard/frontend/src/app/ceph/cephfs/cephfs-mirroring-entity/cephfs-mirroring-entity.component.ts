import { Component, OnInit, ViewChild, Output, EventEmitter, Input, OnChanges, SimpleChanges, inject } from '@angular/core';
...
import { CephAuthUser, FilesystemRow, MirroringEntityRow } from '~/app/shared/models/cephfs.model';
...
export class CephfsMirroringEntityComponent extends CdForm implements OnInit, OnChanges {
      ...
      switchMap(() =>
        this.clusterService.listUser().pipe(
          switchMap((users) => {
            ...
            const rows: MirroringEntityRow[] = filteredUsers.map((user) => {
              const caps = user.caps || {};
              const mdsCaps = caps.mds || '-';
  ...
  submitAction(): void {
    ...
    this.taskWrapperService
      .wrapTaskAroundCall({
        task: new FinishedTask(`mirroring/cephUser/create`, {
      ...
      .subscribe({
        complete: () => {
          ...
          this.loadEntities(this.context);
          this.entitySelected.emit(fullEntity);
