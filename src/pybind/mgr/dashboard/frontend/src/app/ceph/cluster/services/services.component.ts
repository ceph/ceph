...
export class ServicesComponent extends ListWithDetails implements OnChanges, OnInit {
  ...
  icons = Icons;
  service_urls = { grafana: '', prometheus: '', alertmanager: '' };
  ...
  ngOnInit() {
    ...
    this.configureServiceUrl('api/prometheus/url', 'prometheus');
    this.configureServiceUrl('api/prometheus/alertmanager_url', 'alertmanager');
  }
  ...
  private configureServiceUrl(url: string, serviceType: string) {
    this.settingsService.ifSettingConfigured(url, (url) => {
      this.service_urls[serviceType] = url;
    });
  }
}
