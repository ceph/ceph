...
export class ServiceFormComponent extends CdForm implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.sub = forkJoin(observables).subscribe(
      (multisiteInfo: [object, object, object]) => {
        ...
        this.serviceForm.get('zonegroup_name').setValue(this.defaultsInfo['defaultZonegroupName']);
  ...
  getDefaultsEntities(
    ...
    if (defaultZonegroupName === 'default' && defaultZoneName === 'default') {
      this.zoneList.push({ name: 'default' });
      this.zonegroupList.push({ name: 'default' });
    }
    return {
  ...
  onSubmit() {
    ...
    if (serviceType === 'rgw') {
      ...
      serviceSpec['rgw_zonegroup'] = values['zonegroup_name'] ? values['zonegroup_name'] : null;
      serviceSpec['rgw_zone'] = values['zone_name'] ? values['zone_name'] : null;
