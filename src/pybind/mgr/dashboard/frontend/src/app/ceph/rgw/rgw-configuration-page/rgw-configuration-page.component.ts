...
export class RgwConfigurationPageComponent extends ListWithDetails implements OnInit {
  ...
  getBackend(encryptionData: any, encryptionType: string) {
  ...
  flattenData(data: any) {
    ...
    for (const kmsData of Object.values(data.kms)) {
      combinedArray.push(kmsData);
    }
    for (const s3Data of Object.values(data.s3)) {
  ...
  fetchData() {
    this.rgwBucketService.getEncryptionConfig().subscribe((data: any) => {
      ...
      const kmsBackends = this.getBackend(data, 'kms');
      const s3Backends = this.getBackend(data, 's3');
