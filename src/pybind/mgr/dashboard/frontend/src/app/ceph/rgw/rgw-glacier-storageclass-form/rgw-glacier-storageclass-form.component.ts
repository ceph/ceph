...
export class RgwGlacierStorageclassFormComponent extends CdForm implements OnInit {
  ...
  ngOnInit() {
    console.log(this.storageClassType, "storageClassType");
    ...
    if(this.storageClassType === 'cloud-s3-glacier') {
      ...
      console.log(this.glacierStorageClassForm, "form")
    }
  }

  createForm() {
    this.glacierStorageClassForm = this.formBuilder.group({
      glacier_restore_tier_type: new FormControl('', this.storageClassType == 'cloud-s3-glacier' ? Validators.required : null )
            ,
      glacier_restore_days: new FormControl(1, Validators.required),
      restore_storage_class: new FormControl('', Validators.required),
