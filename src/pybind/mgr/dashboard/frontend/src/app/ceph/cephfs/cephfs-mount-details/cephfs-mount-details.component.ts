...
export class CephfsMountDetailsComponent extends BaseModal implements OnInit {
  ...
  getUser() {
    this.cephUserService.getCephUser().subscribe((cephUsers: CephUser[]) => {
      this.clients = cephUsers.filter((user) => user.entity.startsWith('client.')).map((user) => ({
        entity: user.entity,
        key: user.key
      }));
      this.userChangeHandler();
  ...
  userChangeHandler() {
    ...
    const user = this.clients.find((user) => user.entity === this.selectedUser);
    if (user) {
  ...
  getMonitorData() {
    this.monitorService.getMonitor().subscribe({
      next: (monitorData: MonitorData) => {
        ...
        this.fuse = `sudo ceph-fuse  -n ${this.selectedUser} -m ${this.publicaddr} ${this.MOUNT_DIRECTORY}`;
      },
      error: () => {
