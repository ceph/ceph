...
export class SmbClusterFormComponent extends CdForm implements OnInit {
  ...
  private buildRequest() {
    ...
    if (rawFormValue.public_addrs?.length > 0) {
      requestModel.cluster_resource.public_addrs = [];
      for (let i = 0; i < rawFormValue.public_addrs.length; i++) {
        if (rawFormValue.public_addrs[i].destination) {
          requestModel.cluster_resource.public_addrs.push({
            address: rawFormValue.public_addrs[i].address,
            destination: rawFormValue.public_addrs[i].destination
          });
        } else {
          requestModel.cluster_resource.public_addrs.push({
            address: rawFormValue.public_addrs[i].address
