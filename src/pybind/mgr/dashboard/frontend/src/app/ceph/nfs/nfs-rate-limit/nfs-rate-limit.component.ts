...
export class NfsRateLimitComponent implements OnInit {
  ...
  showDisabledField() {
    ...
    this.qosTypeVal = 'PerSharePerClient';
    const fields = [
      'enable_qos',
      'max_export_read_bw',
      'max_export_write_bw',
  ...
  getbwTypeHelp(type: string) {
    const bwTypeItem = this.bwTypeArr.find((item:any) => {
      if (type === item.value) {
        return item;
      }
    });
  ...
  registerQosChange(val: string) {
    ...
    this.qosTypeVal = QOSType[val as keyof typeof QOSType];
    this.setValidation();
  ...
  getQosTypeHelper(qosType: string) {
    const qosTypeItem = this.qosType.find((currentQosTypeItem:any) => {
      if (qosType === currentQosTypeItem.value) {
        return currentQosTypeItem;
      }
    });
