...
export class NfsRateLimitComponent implements OnInit {
  ...
  showQOS() {
    this.allowQoS = this.type === nfsType.export && !this.nfsClusterData?.enable_bw_control;
  }

  loadConditionCheck() {
    this.showDisableWarning = false;
    const isCLusterType = this.type === nfsType.cluster;
    const isExportType = this.type === nfsType.export;
  ...
  registerQoSIOPSChange(val: string) {
    ...
    let enable_qos = this.rateLimitForm?.get('enable_qos').value;
    if (!enable_qos) {
    }
    this.rateLimitForm?.get('qos_type_ops').updateValueAndValidity();
    this.rateLimitForm?.get('qos_type').updateValueAndValidity();
  ...
  showMaxBwNote(val: boolean) {
    ...
    this.showDisableWarning = false;
    if (!val && this.isEdit && this.type == nfsType.cluster && !_.isEmpty(this.nfsClusterData)) {
