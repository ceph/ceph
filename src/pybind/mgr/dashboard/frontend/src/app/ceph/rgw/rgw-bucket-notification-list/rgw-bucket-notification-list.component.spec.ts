...
class MockRgwBucketService {
  getBucketNotificationList = jest.fn((bucket: string) => of([{ bucket, notifications: [] }]));
}
describe('RgwBucketNotificationListComponent', () => {
  ...
  beforeEach(() => {
    ...
    rgwnotificationListSpy = spyOn(
      rgwtbucketService,
      'getBucketNotificationList'
    ).and.callThrough();
  ...
  it('should call list', () => {
    rgwtbucketService.getBucketNotificationList('testbucket').subscribe((response) => {
