...
export class PoolFormComponent extends CdForm implements OnInit {
  ...
  private initEditFormData(pool: Pool) {
    ...
    this.selectedEcp = this.ecProfiles.find((ecp) => ecp.name === pool.erasure_code_profile);
    ...
    this.ecpIsUsedBy(this.selectedEcp.name);
  ...
  private setAvailableApps(apps: string[] = this.data.applications.default) {
    ...
    this.data.applications.available = _.uniq(apps.sort()).map(
      (x: string) => new SelectOption(selectedApps.includes(x), x, this.data.APP_LABELS[x] || x)
    );
  }
  ...
  private hideOpenTooltips() {
    // No longer needed - cds-tooltip doesn't require manual closing
  }
  ...
  appSelection(events: string[]) {
    this.data.applications.selected = events.map((e: any) => (typeof e === 'string' ? e : e.name));
  ...
  erasureProfileChange() {
    ...
    this.selectedEcp = this.ecProfiles.find((ecp) => ecp.name === selectedName);
    if (this.selectedEcp) {
      this.msrCrush =
