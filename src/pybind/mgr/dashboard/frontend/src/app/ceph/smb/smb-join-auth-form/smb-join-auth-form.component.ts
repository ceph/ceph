...
import { JOINAUTH_PATH } from '../smb-join-auth-list/smb-join-auth-list.component';
...
export class SmbJoinAuthFormComponent extends CdForm implements OnInit {
  ...
  constructor(
    ...
    super();
    this.editing = this.router.url.startsWith(`/${JOINAUTH_PATH}/${URLVerbs.EDIT}`);
  ...
  submit() {
    ...
    const self = this;
    let taskUrl = `${JOINAUTH_PATH}/${this.editing ? URLVerbs.EDIT : URLVerbs.CREATE}`;
