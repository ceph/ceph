...
export class RgwUserFormComponent extends CdForm implements OnInit {
      ...
      observableForkJoin(observables).subscribe(
        (resp: any[]) => {
          ...
          this._setRateLimitProperty(
            ...
            resp[2].user_ratelimit.max_read_bytes
          );
          this._setRateLimitProperty(
            value,
            'user_rate_limit_max_writeBytes',
            'user_rate_limit_max_writeBytes_unlimited',
            resp[2].user_ratelimit.max_write_bytes
          );
          this._setRateLimitProperty(
            value,
            'user_rate_limit_max_readOps',
            'user_rate_limit_max_readOps_unlimited',
            resp[2].user_ratelimit.max_read_ops
          );
          this._setRateLimitProperty(
            value,
            'user_rate_limit_max_writeOps',
            'user_rate_limit_max_writeOps_unlimited',
            resp[2].user_ratelimit.max_write_ops
          );
  ...
  onSubmit() {
    ...
    if (this._isUserRateLimitDirty()) {
      ...
      this.submitObservables.push(
        this.rgwUserService.updateUserRateLimit(
          this.userForm.getValue('user_id'),
          userRateLimitArgs
        )
      );
  ...
  private _getUserRateLimitArgs(): RgwRateLimit {
    ...
    if (!this.userForm.getValue('user_rate_limit_max_readOps_unlimited')) {
      result['max_read_ops'] =
        new FormatterService().toIopm(this.userForm.getValue('user_rate_limit_max_readOps')) + '';
    }
    if (!this.userForm.getValue('user_rate_limit_max_writeOps_unlimited')) {
      result['max_write_ops'] =
        new FormatterService().toIopm(this.userForm.getValue('user_rate_limit_max_writeOps')) + '';
    }
    if (!this.userForm.getValue('user_rate_limit_max_readBytes_unlimited')) {
      // Convert the given value to bytes.
      result['max_read_bytes'] =
        new FormatterService().toBytes(this.userForm.getValue('user_rate_limit_max_readBytes')) +
        '';
    }
    if (!this.userForm.getValue('user_rate_limit_max_writeBytes_unlimited')) {
      result['max_write_bytes'] =
        new FormatterService().toBytes(this.userForm.getValue('user_rate_limit_max_writeBytes')) +
        '';
    }
    return result;
  }
