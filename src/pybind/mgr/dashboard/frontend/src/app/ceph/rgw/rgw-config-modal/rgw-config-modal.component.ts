...
export class RgwConfigModalComponent implements OnInit {
  ...
  checkKmsProviders() {
    ...
    //  for kms
    if (selectedEncryptionType === ENCRYPTION_TYPE.SSE_KMS) {
      this.kmsProviders = [KMS_PROVIDER.VAULT, KMS_PROVIDER.KMIP];
    } else if (selectedEncryptionType === ENCRYPTION_TYPE.SSE_S3) {
      this.kmsProviders = [KMS_PROVIDER.VAULT];
    }
    if (
