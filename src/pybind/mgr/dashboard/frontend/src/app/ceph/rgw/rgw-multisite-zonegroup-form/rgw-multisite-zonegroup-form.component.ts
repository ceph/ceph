...
export class RgwMultisiteZonegroupFormComponent extends BaseModal implements OnInit {
  ...
  INVALID_TEXTS = {
    ...
    invalidURL: $localize`Please enter a valid URL.`,
    uniqueName: $localize`The chosen zonegroup name is already in use.`
  };
  ...
  ngOnInit(): void {
    ...
    if (this.action === this.actionLabels.EDIT) {
      ...
      const existingSet = new Set(this.zonegroupZoneNames);
      const availableZonesItems = this.allZoneNames
        .filter((zone) => !existingSet.has(zone))
        .map((zone) => ({
          content: zone,
          value: zone,
          selected: false
  ...
  removePlacementTarget(index: number) {
    ...
    this.placementTargets.markAsTouched();
    this.cdr.detectChanges();
  }

  showError(index: number, control: string, formDir: any, x: string) {
    const placementTargets = <any>this.multisiteZonegroupForm.controls.placementTargets;
    if (placementTargets && placementTargets.controls && placementTargets.controls[index]) {
      return placementTargets.controls[index].showError(control, formDir, x);
    }
    return false;
  }
