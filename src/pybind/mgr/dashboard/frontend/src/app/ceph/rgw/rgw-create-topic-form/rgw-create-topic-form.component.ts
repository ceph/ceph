...
export class RgwCreateTopicFormComponent extends CdForm implements OnInit, AfterViewChecked {
  ...
  ngOnInit(): void {
    ...
    this.label = this.selectedOption !== 'AMQP' ? 'Kafka Ack Level' : 'AMQP Ack Level';
    this.helperText = `Select the ${this.label}`;
    this.controlName = 'ack_level';
    this.selectId = this.selectedOption !== 'AMQP' ? 'kafka_ack_level' : 'amqp_ack_level';
    this.createForm();
  ...
  loadTopicData(topicId: string) {
    this.rgwTopicService.getTopic(topicId).subscribe((topic: Topic) => {
      ...
      if (hostname === 'amqp' || hostname === 'amqps') {
        endpointType = HostURLProtocol.AMQP;
      } else if (hostname === 'https' || hostname === 'http') {
  ...
  setDefaultValue(enableSSL: boolean, selectedValue: string) {
    if (selectedValue === HostURLProtocol.HTTP) {
      ...
      this.port = enableSSL ? URLPort.KAFKA_SSL : URLPort.KAFKA;
    }
    ...
    switch (this.selectedOption) {
      ...
      case HostURLProtocol.KAFKA:
        ...
        if (userKafka && passwordKafka && fqdnKafka && portKafka) {
          generatedEndpoint = `${kafkaProtocol}://${userKafka}:${passwordKafka}@${fqdnKafka}:${portKafka}`;
        } else if (kafkaBrokers) {
          generatedEndpoint = `${kafkaProtocol}://${kafkaBrokers}`;
        } else if (fqdnKafka && portKafka) {
          generatedEndpoint = `${kafkaProtocol}://${fqdnKafka}:${portKafka}`;
        }
  ...
  submitAction() {
    ...
    const notificationTitle = $localize`${
      this.editing
        ? $localize`Topic ${this.topicId} updated successfully`
        : $localize`Topic ${this.topicId} created successfully`
    }`;
  ...
  get helpText(): string {
    return this.selectedOption === 'AMQP'
