...
describe('CephFS mirroring page', () => {
  ...
  describe('bootstrap two clusters', () => {
    it('should generate a token on the secondary and import it on the primary', () => {
      mirroring.isFilesystemMirrored(fsName).then((alreadyMirrored) => {
        ...
        mirroring.createFilesystemApi(fsName);
        cy.ceph2Login();
        mirroring.createFilesystemApi(fsName, ceph2Url);
        ...
        cy.origin(ceph2Origin, { args }, ({ fsName, username, siteName }) => {
          ...
          expect(bootstrapToken, 'bootstrap token from secondary').to.be.a('string').and.not.equal('');
  ...
  describe('add mirror paths', () => {
    it('should add a mirrored path with a snapshot schedule', () => {
      ...
      mirroring.isFilesystemMirrored(fsName).then((fsMirrored) => {
        ...
        cy.then(() => mirroring.listMirroredPaths(fsName)).then((paths) => {
          if (paths.includes(mirrorPath)) {
            cy.log(`${mirrorPath} is already mirrored; skipping add wizard`);
            mirroring.expectMirroredPaths(fsName, [mirrorPath]);
            return;
          }
    ...
    it('should add a directory and a subvolume path together after selecting the filesystem', () => {
      ...
      mirroring.isFilesystemMirrored(fsName).then((fsMirrored) => {
        ...
        cy.then(() => mirroring.listMirroredPaths(fsName)).then((paths) => {
          const missing = [extraPath, subvolPath].filter((path) => !paths.includes(path));
          if (!missing.length) {
            cy.log('extra directory and subvolume are already mirrored; skipping add wizard');
          ...
          mirroring.addPathsWithHourlySchedule(
            missing.map((path) => path.replace(/^\//, '').split('/')),
            fsName
          );
          mirroring.expectMirroredPaths(fsName, [extraPath, subvolPath]);
        });
  ...
  describe('mirror path side panel', () => {
    it('should switch between the Details, Snapshots, and Schedule policy tabs', () => {
      mirroring.listMirroredPaths(fsName).then((paths) => {
        const path = [mirrorPath, extraPath, subvolPath].find((candidate) =>
          paths.includes(candidate)
        );
        if (!path) {
          cy.log('no mirrored e2e path yet; skipping side panel');
          return;
        }
