...
import { BucketsPage } from './bucketCreate.po';
import { BucketsCreatePage } from './bucketCreate.po';
...
describe('Create bucket from the Create page', () => {
  ...
  describe('Types name, sets owner, and creates bucket', () => {
    ...
    it('Cancel Button brings you back to Buckets page', () => {
      ...
      var bcrumb = $('.breadcrumb-item.active');
...
describe('Checks that bucket was made', () => {
  ...
  describe('Checks that the buckets name, owner, and count is correct', () => {
    ...
    it('Should check the name of the bucket is correct', () => {
      ...
      page.navigateTo();
      var EC = protractor.ExpectedConditions;
      var btable = page.getBucketTable();
    ...
    it('Should check the owner of the bucket is correct', () => {
      ...
      page.navigateTo();
      page.getBucketTable().getText().then(function(bucketList) {
...
describe('Checks that a bucket was deleted', () => {
  ...
  describe('Selects bucket, clicks dropdown, selects delete, checks box, and clicks delete ', () => {
    beforeEach(() => {
      page.navigateTo();
    });
    ...
    it('Should select the bucket and delete it', () => {
      ...
      var EC = protractor.ExpectedConditions;
      var numDigits;
      var allb = page.getAllBuckets();
      var count1;
      var toggle = page.getToggleMenu();
      var dtitle = page.getBoxTitle();
      var confirm = page.getConfirm();
      var delbutton = page.getDelButton();
      var count2;
      ...
      page.getBucketCount().getText().then( function(char) {
      ...
      browser.wait(function() {
        return page.getBucketCount().getText().then( function(c) {
          count2 = c.substr(13,numDigits);
          return count2 < count1;
        });
      }, 5000, 'Number of buckets never decreased. Deletion failed.').then( function (){
        page.getBucketTable().getText().then(function (txt) {
