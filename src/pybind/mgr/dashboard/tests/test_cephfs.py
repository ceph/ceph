...
class CephFSMirrorTest(ControllerTestCase):  # pylint: disable=too-many-public-methods
    ...
    @patch('dashboard.controllers.cephfs.CephFS_')
    def test_list_path_snapshots_missing_snapdir(self, mock_cephfs_cls, mock_cephfs):
        fs_name = 'test_fs'
        path = '/volumes/g1/sv1'
        mock_cephfs.ObjectNotFound = Exception
        mock_cephfs.PermissionError = Exception
        mock_cephfs_cls.return_value.ls_snapshots.side_effect = mock_cephfs.ObjectNotFound(
