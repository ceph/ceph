...
class RgwMultisite:
    ...
    def create_sync_pipe(self, group_id: str, pipe_id: str,
        ...
        if source_zones['removed'] or destination_zones['removed']:
            ...
            if source_zones['removed']:
                rgw_sync_policy_cmd += ['--source-zones', ','.join(source_zones['removed'])]

            if destination_zones['removed']:
                rgw_sync_policy_cmd += ['--dest-zones', ','.join(destination_zones['removed'])]

            logger.info("Removing sync pipe zones! %s", rgw_sync_policy_cmd)
            try:
                exit_code, _, err = mgr.send_rgwadmin_command(rgw_sync_policy_cmd)
                if exit_code > 0:
                    raise DashboardException(f'Unable to remove sync pipe zones: {err}',
                                             http_status_code=500, component='rgw')
            except SubprocessError as error:
                raise DashboardException(error, http_status_code=500, component='rgw')
    ...
    def create_dashboard_admin_sync_group(self, zonegroup_name: str = ''):
        ...
        selected_zones = {}
        selected_zones['added'] = zone_names
        self.create_sync_flow(_SYNC_GROUP_ID, _SYNC_FLOW_ID,
                              SyncFlowTypes.symmetrical.value,
                              zones={'added': zone_names})
        # create a sync pipe with source and destination zones
        self.create_sync_pipe(_SYNC_GROUP_ID, _SYNC_PIPE_ID, source_zones={'added': '*'},
                              destination_zones={'added': '*'}, source_bucket='*',
                              destination_bucket='*')
