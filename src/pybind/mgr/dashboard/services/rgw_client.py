...
class RgwMultisiteAutomation:
    ...
    def update_progress(self, step_title):
        ...
        self.update_queue.put((self.progress_title, progress))

    def _process_updates(self):
        while True:
            title, progress = self.update_queue.get()
            if title is None and progress is None:
                break
            try:
                # Simulate progress update call
                print(f"Updating progress: {title}, {progress}%")
                mgr.remote(
    ...
    def setup_multisite_replication(self, realm_name: str, zonegroup_name: str,
        ...
        try:
            ...
            rgw_multisite_instance.create_zonegroup(realm_name=realm_name, zonegroup_name=zonegroup_name,
                                                    default=True, master=True, endpoints=zonegroup_ip_url)
        except Exception as e:
            logger.error("Failed to create realm or zonegroup: %s", e)
            mgr.remote('progress', 'fail', self.progress_id, str(e))
            raise
        try:
            # Create the zone and system user, then modify the zone with user credentials
            logger.info("Creating zone: %s", zone_name)
            if rgw_multisite_instance.create_zone(zone_name=zone_name, zonegroup_name=zonegroup_name,
                ...
                if user_details:
                    ...
                    if access_key and secret_key:
                        rgw_multisite_instance.modify_zone(zone_name=zone_name, zonegroup_name=zonegroup_name,
                                                           default='true', master='true', endpoints=zone_ip_url,
                                                           access_key=keys['access_key'],
                                                           secret_key=keys['secret_key'])
                    else:
        ...
        try:
            ...
            if cluster_fsid and realm_token_info:
                ...
                self.update_progress("Realm Export Token fetched successfully")
        ...
        try:
            ...
            if cluster_token:
                ...
                try:
                    ...
                    self.progress_done += 1
                    self.update_progress(
                        f"Checking for user: {username} and restarting the rgw daemons in the selected cluster")
