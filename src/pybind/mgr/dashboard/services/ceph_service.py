...
from .. import mgr
from ..model.rgw import VaultConfig, KmipConfig, EncryptionConfig, KmsConfig, S3Config, EncryptionTypes, KmsProviders
...
class CephService(object):
    ...
    @classmethod
    def get_encryption_config(cls, daemon_name: str):
        full_daemon_name = f'rgw.{daemon_name}'

        encryption_config = EncryptionConfig(kms=[], s3=[])

        # Fetch configuration for KMS
        if EncryptionTypes.KMS.value:
            vault_config_data = _get_conf_keys(VaultConfig._fields, VaultConfig.required_fields(),
                                            EncryptionTypes.KMS.value, KmsProviders.VAULT.value, full_daemon_name)
            kmip_config_data = _get_conf_keys(KmipConfig._fields, KmipConfig.required_fields(),
                                            EncryptionTypes.KMS.value, KmsProviders.KMIP.value, full_daemon_name)

            kms_config = {}

            if vault_config_data:
                vault_config_data = _set_defaults_in_encryption_configs(vault_config_data,
                                                                        EncryptionTypes.KMS.value,
                                                                        KmsProviders.VAULT.value)
                kms_config["vault"] = VaultConfig(**vault_config_data)

            if kmip_config_data:
                kmip_config_data = _set_defaults_in_encryption_configs(kmip_config_data,
                                                                    EncryptionTypes.KMS.value,
                                                                    KmsProviders.KMIP.value)
                kms_config["kmip"] = KmipConfig(**kmip_config_data)

            if kms_config:
                encryption_config = encryption_config._replace(
                    kms=KmsConfig(**kms_config)
                )

        # Fetch configuration for S3
        if EncryptionTypes.S3.value:
            s3_config_data = _get_conf_keys(VaultConfig._fields, VaultConfig.required_fields(),
                                            EncryptionTypes.S3.value, KmsProviders.VAULT.value, full_daemon_name)

            if s3_config_data:
                s3_config_data = _set_defaults_in_encryption_configs(s3_config_data,
                                                                    EncryptionTypes.S3.value,
                                                                    KmsProviders.VAULT.value)
                s3_vault_config = VaultConfig(**s3_config_data)

                encryption_config = encryption_config._replace(
                    s3=S3Config(vault=s3_vault_config)
                )
    ...
    @classmethod
    def set_encryption_config(encryption_type, kms_provider, config: Union[VaultConfig, KmipConfig], daemon_name):
...
def _generate_key(encryption_type: str, backend: str, config_name: str):
    ...
    elif encryption_type == EncryptionTypes.S3.value:
        return f'rgw_crypt_sse_s3_{backend}_{config_name}'

def _get_conf_keys(fields: List[str], req_fields: List[str], encryption_type: str, backend: str, daemon_name: str) -> List[str]:
    config: Dict[str, Any] = {}
    for field in fields:
        key = _generate_key(encryption_type, backend, field)
        try:
            value = CephService.send_command('mon', 'config get', who=name_to_config_section(daemon_name), key=key)
...
def _set_defaults_in_encryption_configs(config: Dict[str, Any], encryption_type: str, kms_provider: str) -> Dict[str, Any]:
