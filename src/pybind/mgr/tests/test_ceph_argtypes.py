import pytest
from ceph_argparse import GB, KB, MB, PB, TB, B, G, K, M, P, T, CephSizeBytes

class TestConvertToBytes:
    def test_with_kb(self):
        assert CephSizeBytes._convert_to_bytes(f"100{KB}") == 102400
        assert CephSizeBytes._convert_to_bytes(f"100{K}") == 102400

    def test_with_mb(self):
        assert CephSizeBytes._convert_to_bytes(f"2{MB}") == 2 * 1024 ** 2
        assert CephSizeBytes._convert_to_bytes(f"2{M}") == 2 * 1024 ** 2

    def test_with_gb(self):
        assert CephSizeBytes._convert_to_bytes(f"1{GB}") == 1024 ** 3
        assert CephSizeBytes._convert_to_bytes(f"1{G}") == 1024 ** 3

    def test_with_tb(self):
        assert CephSizeBytes._convert_to_bytes(f"1{TB}") == 1024 ** 4
        assert CephSizeBytes._convert_to_bytes(f"1{T}") == 1024 ** 4

    def test_with_pb(self):
        assert CephSizeBytes._convert_to_bytes(f"1{PB}") == 1024 ** 5
        assert CephSizeBytes._convert_to_bytes(f"1{P}") == 1024 ** 5
    ...
    def test_b(self):
        assert CephSizeBytes._convert_to_bytes(f"500{B}") == 500

    def test_with_large_number(self):
        assert CephSizeBytes._convert_to_bytes(f"1000{GB}") == 1000 * 1024 ** 3
