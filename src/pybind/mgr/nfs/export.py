...
class ExportMgr:
    ...
    def create_export_from_dict(self,
        ...
        # When RDMA is enabled at cluster level, default transports only if omitted
        nfs_spec = get_nfs_spec_for_cluster(self.mgr, cluster_id)
        rdma_status = nfs_spec and getattr(nfs_spec, "enable_rdma", False)
        if "transports" not in ex_dict and rdma_status:
            ...
            raise NFSInvalidOperation(
                "Can't set Transport as RDMA as RDMA not enabled at NFS cluster level"
            )
