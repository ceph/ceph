...
def _validate_cmount_path(mgr: 'Module', cmount_path: str, path: str) -> None:
    cmount_path = os.path.abspath(os.path.normpath(cmount_path))
    path = os.path.abspath(os.path.normpath(path))

    # Check if path is within cmount_path
    if not path.startswith(cmount_path + os.sep):
        raise ValueError(
...
class ExportMgr:
    ...
    def _ensure_cephfs_export_user(self, export: Export) -> None:
        ...
        fsal.user_id = f"nfs.{generate_unique_id(export.cluster_id, fsal.fs_name, fsal.cmount_path)}"
    ...
    def create_export_from_dict(self,
        ...
        if fsal_type == NFS_GANESHA_SUPPORTED_FSALS[1]:
            ...
            if "cmount_path" in fsal and fsal["cmount_path"] != "/":
                _validate_cmount_path(self.mgr, fsal["cmount_path"], path)  # type: ignore

            user_id = f"nfs.{generate_unique_id(cluster_id, fs_name, fsal['cmount_path'])}"
    ...
    def _apply_export(
        ...
        if old_export.fsal.name == NFS_GANESHA_SUPPORTED_FSALS[0]:
            ...
            self._ensure_cephfs_export_user(new_export)
            if old_fsal.user_id == new_fsal.user_id and old_fsal.fs_name == new_fsal.fs_name and old_export.path == new_export.path and old_export.pseudo == new_export.pseudo:
...
def generate_unique_id(cluster_id: str, fs_name: str, cmount_path: str) -> str:
    """
