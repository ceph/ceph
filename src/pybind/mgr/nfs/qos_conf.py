...
def validate_clust_qos_msg_interval(msg_interval: Optional[str]) -> int:
    min_interval = 100  # ms
    max_interval = 300  # ms

    if msg_interval is None:
        return 0
    if not msg_interval.endswith('ms'):
        raise Exception(
            'Invalid cluster QoS message interval, message interval should be in ms, '
            f'provided value is {msg_interval}'
        )
    try:
        interval = int(msg_interval[:-2])
    except Exception:
        raise Exception('Invalid cluster QoS message interval')
...
class QOS(object):
    ...
    def from_qos_block(cls, qos_block: RawBlock, cluster_op: bool = False) -> "QOS":
        ...
        if cluster_op:
            ...
            msg_interval = qos_block.values.get(QOSParams.clust_qos_msg_interval.value)
            if msg_interval:
                kwargs['clust_qos_msg_interval'] = f'{msg_interval}ms'
    ...
    def to_dict(self, ret_bw_in_bytes: bool = False) -> Dict[str, Any]:
        ...
        if self.cluster_op:
            ...
            if self.clust_qos_msg_interval:
                r[QOSParams.clust_qos_msg_interval.value] = f'{self.clust_qos_msg_interval}ms'
