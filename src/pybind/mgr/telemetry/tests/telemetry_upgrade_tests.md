...
documentation) to the current stable Ceph release (_tentacle_) can complete
successfully without disrupting any ongoing workloads.
...
- Test Telemetry functions as expected after the upgrade
...
Based on the above example where X=squid, is possible to test the upgrades from
Quincy (X-2) or from Reef (X-1) to Squid (X).
...
A simple upgrade test contains these steps in order, divided into separate yaml files:
```
├── 0-start.yaml
├── 1-tasks.yaml
├── upgrade-sequence.yaml
...
correct collections after the upgrade. This integration test coverage is done
...
For instance, the relevant telemetry workunits for `squid` release are:
```
qa/workunits
...
- `test_telemetry_quincy.sh`, tests the presence of telemetry collection on a Quincy cluster before the upgrade.
- `test_telemetry_quincy_x.sh`, tests the presence of new telemetry collection on the X-version cluster after it has been upgrade from Quincy.
...
a Quincy or Reef cluster, then upgrade the cluster to Squid. In parallel, some
...
- If the collection is added only to the `main` branch or the current release (`tentacle`),
