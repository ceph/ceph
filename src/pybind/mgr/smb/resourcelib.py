...
and be a base for a cleaner looking & eaiser working way to accomplish things
...
- serilaization into JSON/YAML can be handled at highter levels but without
...
fields can be easily converted into a more strucutured python object
...
class Field:
    ...
    def takes(self, dest_type: Any) -> bool:
        """Returns true if the `dest_type` is a container type (List, Dict, ...)
        and the field's non-optional form a matches said container type.
        """
...
class Registry:
    ...
    def track(self, key: str, resource: Resource) -> None:
        ...
        union" so long as the resource type providies a condition function to
        ...
        if len(self.resources[key]) > 1:
            if any(not c.conditional for c in self.resources[key]):
                raise ResourceTypeError(
                    'mulitplexed resources must be conditional'
                )
