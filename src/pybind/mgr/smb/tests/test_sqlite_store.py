...
def test_set_object_writes_mirror_before_commit():
    store, mirror_target = _mirroring_store()
    key = ('join_auths', 'auth1')
    store.set_object(key, {'auth': {'username': 'foo', 'password': 'old'}})
...
def test_get_object_sees_write_in_transaction():
    store, _ = _mirroring_store()
    key = ('join_auths', 'auth1')
    store.set_object(
...
def test_set_object_rollback_leaves_reconcilable_mirror_entry():
    store, mirror_target = _mirroring_store()
    kept = ('join_auths', 'kept')
    key = ('join_auths', 'auth1')
    store.set_object(kept, {'auth': {'username': 'foo', 'password': 's1'}})

    with pytest.raises(RuntimeError):
        with store.transaction():
            store.set_object(
                key, {'auth': {'username': 'foo', 'password': 'secret'}}
            )
            raise RuntimeError('later step in the same batch failed')

    # sqlite rolled back, but the mirror was written first: an extra entry
    # is left behind rather than a row missing its secret
    with pytest.raises(KeyError):
        store.get_object(key)
    assert set(mirror_target) == {kept, key}

    store.reconcile()

    assert set(mirror_target) == {kept}
...
def test_reconcile_warns_once(caplog):
    store, _ = _mirroring_store()
    key = ('join_auths', 'auth1')
    smb.sqlite_store.SqliteStore.set_object(store, key, {'auth': {}})
    ...
    assert caplog.text.count('has no mirror entry') == 1
