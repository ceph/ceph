...
@pytest.mark.parametrize(
    ...
    [
        ...
        # conflict with share in store, tickle impossible state
        {
            'to_apply': [
                smb.resources.Share(
                    cluster_id='c2',
            ...
            'invalid_db': True,
            'error_msg': 'share name already in use',
            # one of these ids will conflict. we can't be certain which one
            # so we have to try and match both.
            'conflicts': {'soup', 'noway'},
        },
    ],
)
