...
class ClusterConfigHandler:
    ...
    def _find_modifications(self, updated: ResultGroup) -> Collection[str]:
        ...
        """
        # this iniitial version is going to take a simplistic appraoach and try
        # broadly collect anything that could be a change.
    ...
    def generate_smb_service_spec(self, cluster_id: str) -> SMBSpec:
        ...
        return _generate_smb_service_spec(
            cluster, config_entries, join_source_entries
        )
...
def _generate_share(
    ...
    # very annoyingly, samba's ceph module abosoluetely must NOT have the
...
def _save_pending_users_and_groups(
    ...
    for idx, ugsv in enumerate(checked(cluster.user_group_settings)):
        ...
        ugentry.set(ugvalues.to_simplified())
