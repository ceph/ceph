...
class Module(orchestrator.OrchestratorClientMixin, MgrModule):
    ...
    def cluster_create(
        ...
        # unlike AD domain_join_user_pass values, we don't permit creating user
        # and group configs on the cluster create command line as these are far
        # too nested & complex (unlike the simple split for un/pw).  Therefore
        # you must rely on the apply command to create the config reources and
        # then can refer to them (by id) when creating the cluster.
        if user_group_ref:
            user_group_settings = [
