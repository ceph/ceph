...
cdef class Group(object):
    ...
    def group_snap_get_mirror_namespace(self, snap_id):
        ...
        :returns: dict - contains the following keys:
            ...
            * ``type`` (int) - the group snapshot state(mirror or user)

            * ``state`` (int) - the group snapshot state

            * ``mirror_peer_uuids_count`` (int) - mirror peer uuids count
        ...
        with nogil:
            ret = rbd_group_snap_get_mirror_namespace(
                  self._ioctx, _group_id, _snap_id, &sn)
        if ret != 0:
            raise make_ex(ret, 'error getting group snapshot mirror '
        ...
        info = {
                'type' :sn.type,
                'state' : sn.state,
                'mirror_peer_uuids_count' : sn.mirror_peer_uuids_count,
