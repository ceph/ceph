...
cdef class LibCephFS(object):
    ...
    def cptree(self, src_path, dst_path, should_cancel, suppress_errors=False):
        ...
        if stat.S_ISDIR(stx_b['mode']):
            NonRecursiveCptree(self, src_path, dst_path, should_cancel,
                               suppress_errors).cptree()
...
class CptreeDir:
    ...
    def __init__(self, cptree_run_obj, src_rel_path, dst_rel_path):
        # source and destination path passed by the user.
        self.src_path = cptree_run_obj.src_path
        self.dst_path = cptree_run_obj.dst_path

        self.src_root_fd = cptree_run_obj.src_root_fd
        self.dst_root_fd = cptree_run_obj.dst_root_fd

        self.fs = cptree_run_obj.fs
