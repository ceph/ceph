  ...
  std::optional<pg_shard_t> new_primary)
{
  ...
  for (int i = 0; i < max_iterations; i++) {
    ...
    if (new_epoch(true)) {
      ...
      // Call advance_map on ALL shards that have peering states, including failed ones
      // Use the newly computed up_osds and acting_osds from the new OSDMap
      std::vector<int> up_osds_iter, acting_osds_iter;
      int up_primary_iter = -1, acting_primary_iter = -1;
      osdmap->pg_to_up_acting_osds(this->pgid, &up_osds_iter, &up_primary_iter, &acting_osds_iter, &acting_primary_iter);
      
      for (auto& [shard, ps] : shard_peering_states) {
        ps->advance_map(
          osdmap, old_osdmap, up_osds_iter, up_primary_iter, acting_osds_iter, acting_primary_iter,
          *get_peering_ctx(shard));
      }
      dispatch_all();
    }
