...
public:
  ...
  static void add_osd(OSDMap& osdmap, int osd_id)
  {
    ...
    // preserve whatever is set here for the lifetime of the OSD, so use the
    // full supported feature set rather than a handful of named releases.
    osd_xinfo_t xinfo;
    xinfo.features = CEPH_FEATURES_SUPPORTED_DEFAULT;
