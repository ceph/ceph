...
RealObjsConf ScrubGenerator::make_erasure_code_configuration(int8_t k, int8_t m)
{
...
CorruptFuncList ScrubGenerator::make_erasure_code_hash_corruption_functions(int num_osds)
{
  ...
  bool erasure_coded_pool)
{
  ...
  for (auto osd : active_osds) {
    ...
    {
      this_osd_fakes->objs.erase(std::remove_if(this_osd_fakes->objs.begin(), this_osd_fakes->objs.end(), [pool_id](RealObj& robj)
      {
