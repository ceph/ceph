...
typedef struct stopwatch {
  utime_t start;
  utime_t stop;
  utime_t realtime () const { return stop-start;}
} stopwatch_t;
...
template <typename T>
class ErasureCodeTest_LP : public ::testing::Test {
 public:
};

template <typename T>
class ErasureCodeTest_LP_3 : public ::testing::Test {
 public:
};

template <typename T>
class ErasureCodeTest_LP_CRC32C : public ::testing::Test {
 public:
};
...
TYPED_TEST_CASE(ErasureCodeTest_LP, JerasureTypes);
TYPED_TEST_CASE(ErasureCodeTest_LP_3, JerasureTypes);
TYPED_TEST_CASE(ErasureCodeTest_LP_CRC32C, JerasureTypes);
...
TYPED_TEST(ErasureCodeTest_LP, encode_decode) {
  TypeParam jerasure;
  map<std::string,std::string> parameters;
  parameters["erasure-code-k"] = "8";
  parameters["erasure-code-m"] = "2";
  ...
  timing[jerasure.technique]["encode-lp"].start = ceph_clock_now(0);
  EXPECT_EQ(0, jerasure.encode(set<int>(want_to_encode, want_to_encode+12),
                              in,
                              &encoded));

  timing[jerasure.technique]["encode-lp"].stop = ceph_clock_now(0);
  ...
  // two chunks are missing 
  {
    ...
    map<int, bufferlist> decoded;
    timing[jerasure.technique]["reco-lp"].start = ceph_clock_now(0);
    EXPECT_EQ(0, jerasure.decode(set<int>(want_to_decode, want_to_decode+2),
                                degraded,
                                &decoded));
    timing[jerasure.technique]["reco-lp"].stop = ceph_clock_now(0);
    // always decode all, regardless of want_to_decode
    EXPECT_EQ(12u, decoded.size()); 
    EXPECT_EQ(length, decoded[0].length());
...
TYPED_TEST(ErasureCodeTest_LP_3, encode_decode) {
  TypeParam jerasure;
  map<std::string,std::string> parameters;
  parameters["erasure-code-k"] = "8";
  parameters["erasure-code-m"] = "2";
  ...
  timing[jerasure.technique]["encode-lp-3"].start = ceph_clock_now(0);
  EXPECT_EQ(0, jerasure.encode(set<int>(want_to_encode, want_to_encode+12),
                              in,
                              &encoded));

  timing[jerasure.technique]["encode-lp-3"].stop = ceph_clock_now(0);
  EXPECT_EQ(12u, encoded.size());
  unsigned length =  encoded[0].length();
  EXPECT_EQ(0, memcmp(encoded[0].c_str(), in.c_str(), length));
  ...
  // three chunks are missing 
  {
    ...
    map<int, bufferlist> decoded;
    timing[jerasure.technique]["reco-lp-3"].start = ceph_clock_now(0);
    EXPECT_EQ(0, jerasure.decode(set<int>(want_to_decode, want_to_decode+3),
                                degraded,
                                &decoded));
    timing[jerasure.technique]["reco-lp-3"].stop = ceph_clock_now(0);
    // always decode all, regardless of want_to_decode
    EXPECT_EQ(12u, decoded.size()); 
    EXPECT_EQ(length, decoded[0].length());
    EXPECT_EQ(0, memcmp(decoded[0].c_str(), in.c_str(), length));
...
TYPED_TEST(ErasureCodeTest_LP_CRC32C, encode_decode) {
  TypeParam jerasure;
  map<std::string,std::string> parameters;
  parameters["erasure-code-k"] = "8";
  parameters["erasure-code-m"] = "2";
  ...
  timing[jerasure.technique]["encode-lp-crc32c"].start = ceph_clock_now(0);
  EXPECT_EQ(0, jerasure.encode(set<int>(want_to_encode, want_to_encode+12),
                              in,
                              &encoded));
  ...
  timing[jerasure.technique]["encode-lp-crc32c"].stop = ceph_clock_now(0);
  EXPECT_EQ(12u, encoded.size());
  unsigned length =  encoded[0].length();
  EXPECT_EQ(0, memcmp(encoded[0].c_str(), in.c_str(), length));
  EXPECT_EQ(0, memcmp(encoded[1].c_str(), in.c_str() + length, encoded[1].length()));
  ...
  // two chunks are missing 
  {
    ...
    int want_to_decode[] = { 0, 5 };
    map<int, bufferlist> decoded;
    timing[jerasure.technique]["reco-lp-crc32c"].start = ceph_clock_now(0);
    EXPECT_EQ(0, jerasure.decode(set<int>(want_to_decode, want_to_decode+2),
                                degraded,
                                &decoded));
    ...
    timing[jerasure.technique]["reco-lp-crc32c"].stop = ceph_clock_now(0);
    // always decode all, regardless of want_to_decode
    EXPECT_EQ(12u, decoded.size()); 
    EXPECT_EQ(length, decoded[0].length());
    EXPECT_EQ(0, memcmp(decoded[0].c_str(), in.c_str(), length));
