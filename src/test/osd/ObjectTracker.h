...
public:
  ...
  struct DataWrite {
    ...
    DataWrite(uint64_t off, const std::string& d, const eversion_t& v)
      : offset(off), data(d), version(v) {}
  };

  /**
   * Information about an attribute write operation. Keyed by attribute name
   * in ObjectState::attributes, so the name itself isn't stored here.
   */
  struct AttributeWrite {
    ...
    AttributeWrite(const std::string& v, const eversion_t& ver)
      : value(v), version(ver) {}
  };

  /**
   * Information about an omap operation. Keyed by omap key in
   * ObjectState::omap_entries, so the key itself isn't stored here.
   */
  struct OMapWrite {
    std::string value;
    eversion_t version;  // Version when this write occurred
    ...
    OMapWrite() = default;
    OMapWrite(const std::string& v, const eversion_t& ver)
      : value(v), version(ver) {}
  };
