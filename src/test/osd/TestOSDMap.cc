...
TEST_F(OSDMapTest, pgtemp_primaryfirst) {
  ...
  for (int seed = 2; seed < 64; seed += 2) {
	...
	ASSERT_TRUE(pool.is_nonprimary_shard(shard_id_t(encoded[osd])));
      ...
      // Per-shard forward transform should round-trip through the per-shard
      // undo transform.
      ASSERT_EQ(osdmap.pgtemp_undo_primaryfirst(pool, pgid,
		  osdmap.pgtemp_primaryfirst(pool, pgid, shard_id_t(osd))),
		shard_id_t(osd));
      // pgtemp_primaryfirst() partitions shards into "primary-eligible"
      // (kept first, in their original relative order) then "non-primary"
      // (kept last, in their original relative order); a shard only changes
      // position if that partitioning actually moves it.
      //
      ...
      //
      // seed 62 is 0b111110: bit 0 clear, bits 1-5 all set, so shards 1-5 are
      // *all* non-primary and shard 0 is the *only* primary shard. The
      // primary group is just {0}, already in position 0, and the
      // non-primary group is {1,2,3,4,5}, already contiguous and in order --
      // so the partition is a no-op for every shard, not just shard 0.  For
      // any other seed, shards 1-5 are a genuine mix of primary/non-primary,
      // so at least one of them must move to reach the partitioned order.
      if ((osd != 0) && (seed != 62)) {
