...
public:
  ...
  void send_message(int from_osd, int to_osd, Message* m) {
    ...
    // Extract spg_t from the message (now that it has been encoded above)
    // to determine the target TestPG.
    ...
    ceph_assert(test_pg_getter);
    {
      ...
      if (dpp) {
        ldpp_dout(dpp, 20) << "MockMessenger: Extracted spg=" << spgid
                           << " for OSD." << to_osd << ", target_test_pg="
                           << target_test_pg << dendl;
      }
