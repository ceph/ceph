...
public:
  ...
  // Overload without TestPG* context — used by MockPeeringListener which owns
  // ps/ctx directly and does not need a TestPG pointer for dispatch.
  void schedule_peering_event(int osd, GenericEvent callback) {
    ceph_assert(osd >= 0);
    events.emplace_back(EventType::PEERING_EVENT, -1, osd, std::move(callback), nullptr);
  }
  ...
  void run_in_pg(int osd, TestPG* test_pg, Func&& callback) {
    // Save previous context. This must cover every piece of ambient state
    // run_one() also manages -- current_executing_osd, current_test_pg, AND
    // current_osd (the change-detection member run_one() uses to decide
    // whether to clear current_test_pg for the next event) -- so a call here
    // can't leave current_osd stale relative to current_test_pg and cause a
    // later run_one() call to skip clearing the context when it should.
    int prev_osd = current_executing_osd;
    TestPG* prev_test_pg = current_test_pg;
    int prev_current_osd = current_osd;
