...
using SelectBlockSize =
    ProgramOptionSelector<uint64_t, io_sequence::tester ::block_size_array_size,
...
using SelectNumThreads =
    ProgramOptionSelector<int, io_sequence::tester ::thread_array_size,
...
class SelectErasureKM
 ...
 public:
  SelectErasureKM(ceph::util::random_number_generator<int>& rng,
                  ...
                  const std::optional<std::string>& technique, bool first_use);
...
class SelectErasureC : public ProgramOptionGeneratedSelector<uint64_t> {
 public:
  SelectErasureC(ceph::util::random_number_generator<int>& rng,
                 po::variables_map& vm, std::string_view plugin,
                 const std::optional<std::pair<int, int>>& km, bool first_use);
...
class SelectMappingAndLayers {
 public:
  SelectMappingAndLayers(ceph::util::random_number_generator<int>& rng,
                         po::variables_map& vm, bool first_use);
...
class SelectErasureChunkSize : public ProgramOptionGeneratedSelector<uint64_t> {
 public:
  SelectErasureChunkSize(ceph::util::random_number_generator<int>& rng,
                         po::variables_map& vm, ErasureCodeInterfaceRef ec_impl,
                         bool first_use);
  const std::vector<uint64_t> generate_selections() override;
...
class SelectErasurePool : public ProgramOptionReader<std::string> {
 public:
  ...
  std::string create();
  void configureServices(bool allow_pool_autoscaling, bool allow_pool_balancer,
...
class TestRunner {
 public:
  TestRunner(boost::intrusive_ptr<CephContext> cct, po::variables_map& vm,
             librados::Rados& rados);
  ~TestRunner();
