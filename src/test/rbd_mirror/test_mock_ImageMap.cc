...
TEST_F(TestMockImageMap, RemoveReaddRemoteImageBeforeRemoveAck) {
  ...
  MockUpdateRequest mock_update_request;
  std::map<std::string, Context*> peer_acquire_ack_ctxs;
  std::map<std::string, Context*> peer_release_ack_ctxs;
  std::map<std::string, Context*> peer_remove_ack_ctxs;
  ...
  // Expect the remove and release operations. Keep the remove ACK pending so
  // the image can be added again before that ACK is completed.
  listener_remove_images(mock_listener, "uuid1", global_image_ids,
                         &peer_remove_ack_ctxs);
  expect_add_event(mock_threads);
  listener_release_images(mock_listener, global_image_ids,
                          &peer_release_ack_ctxs);
  update_map_request(mock_threads, mock_update_request, global_image_ids, 0);

  // Remove uuid1, which is the image's last peer. ImageMap should erase the
  // peer-map entry and notify the listener right away.
  entities = make_image_entities(global_image_ids);
  mock_image_map->update_images("uuid1", {}, std::move(entities));
  ASSERT_TRUE(wait_for_listener_notify(2));

  // Add the image again before the old remove ACK arrives. Expect the map to
  // be updated and the image to be acquired for uuid1 again.
  update_map_and_acquire(mock_threads, mock_update_request, mock_listener,
                         global_image_ids, 0, &peer_acquire_ack_ctxs);
