...
TEST_P(seastore_test_t, clone_range)
{
  run_async([this] {
    ...
    test_obj2.clone_range(*sharded_seastore, test_obj1, 98304, 4096, 98304);
    std::cout << "seastore_test_t.clone_range 1 clone_range" << std::endl;
    test_obj2.read(*sharded_seastore, 0, 131072);

    test_obj2.clone_range(*sharded_seastore, test_obj1, 1 << 18, 4096, 1 << 18);
    std::cout << "seastore_test_t.clone_range 2 clone_range" << std::endl;
    test_obj2.read(*sharded_seastore, 0, (1 << 18) + (1 << 16));

    test_obj2.clone_range(
      *sharded_seastore,
      test_obj1,
      (1 << 18) + (1 << 16) - 4096,
      4096,
