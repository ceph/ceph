...
using namespace hdd_device;
using namespace rbm_device;

struct hdd_dev_test_t : seastar_test_suite_t {
  std::unique_ptr<HDDBlockDevice> device;
  std::string dev_path;
...
struct hdd_dev_test_block_t {
  uint8_t data[BUF_SIZE];

  DENC(hdd_dev_test_block_t, v, p) {
    DENC_START(1, 1, p);
...
WRITE_CLASS_DENC_BOUNDED(
  hdd_dev_test_block_t
)

TEST_F(hdd_dev_test_t, write_and_verify_test)
{
  run_async([this] {
    device = RBMDevice::create<HDDBlockDevice>();
    device->open(dev_path, seastar::open_flags::rw).unsafe_get();
    hdd_dev_test_block_t original_data;
    ...
    hdd_dev_test_block_t read_data;
    {
