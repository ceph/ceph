...
struct onode_item_t {
  ...
  void initialize(Transaction& t, Onode& value) const {
    ...
    auto oroot = omap_root_t(laddr_t::get_hint_from_offset(id * 4096), cnt_modify,
  ...
  void validate(Onode& value) const {
    ...
    ceph_assert(uint32_t(layout.size) == uint32_t{size});
    ceph_assert(layout.omap_root.get(value.get_metadata_hint(block_size)).addr
                == laddr_t::get_hint_from_offset(id * 4096));
