...
TEST_F(btree_lba_manager_test, conflict_policy_global_linear)
{
  run_async([this] {
    ...
    hint.condition = laddr_conflict_condition_t::object_content;
    hint.policy = laddr_conflict_policy_t::never;
    hint.block_size = laddr_t::UNIT_SIZE;
...
TEST_F(btree_lba_manager_test, conflict_policy_object_data)
{
  run_async([this] {
    ...
    auto hint = laddr_hint_t::create_fresh_object_data_hint(shard, pool, crush);
    {
...

    {
      ...
      hint.condition = laddr_conflict_condition_t::object_prefix;
      hint.policy = laddr_conflict_policy_t::gen_random;
      hint.block_size = laddr_t::UNIT_SIZE;
...

    {
      ...
      hint.condition = laddr_conflict_condition_t::clone_prefix;
      hint.policy = laddr_conflict_policy_t::gen_random;
      hint.block_size = laddr_t::UNIT_SIZE;
      auto addr = alloc(hint);
      ASSERT_EQ(base_addr.get_shard(), addr.get_shard());
