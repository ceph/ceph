...
class TestFcopyfilex:
    ...
    def _create_test_files(self, data_size, dst_initial_size=0):
        ...
        # Write source file
        fd_src_write = cephfs.open(b'file_src', 'w', self.PERMS)
        cephfs.write(fd_src_write, data, 0)
        cephfs.close(fd_src_write)

        # Create destination file with initial content
        fd_dst = cephfs.open(b'file_dst', 'w+', self.PERMS)
        cephfs.write(fd_dst, b'0' * dst_initial_size, 0)
        cephfs.close(fd_dst)

        return data

    def _write_file_and_test_fcopyfilex(self, testdir, data_size, src_offset, dst_offset, copy_length, expected_copied=None):
        '''
        ...
        fd_src = cephfs.open(b'file_src', 'r', self.PERMS)
        ...
        fd_verify = cephfs.open(b'file_dst', 'r', self.PERMS)
    ...
    def test_fcopyfilex_with_offset(self, testdir):
        ...
        '''
        self._write_file_and_test_fcopyfilex(testdir, 24, 5, 10, 10)
    ...
    def test_with_regfile_of_size_0_5MB(self, testdir):
        ...
        self._write_file_and_test_fcopyfilex(testdir, size, 0, 0, size)
    ...
    def test_fcopyfilex_source_offset_beyond_file(self, testdir):
        '''
        Test that fcopyfilex() handles source offset beyond file size.
        '''
        data_size = 100
        ...
        data = self._create_test_files(data_size, 50)
        ...
        # This should either fail or copy 0 bytes
        try:
            copied = cephfs.fcopyfilex(fd_src, src_offset, fd_dst, dst_offset, copy_length)
            # If it succeeds, should copy 0 bytes
            assert copied == 0
    ...
    def test_fcopyfilex_partial_copy_due_to_source_size(self, testdir):
        ...
        self._write_file_and_test_fcopyfilex(testdir, data_size, src_offset, dst_offset, copy_length, expected_copied)
    ...
    def test_fcopyfilex_destination_offset_beyond_size(self, testdir):
        ...
        self._write_file_and_test_fcopyfilex(testdir, data_size, src_offset, dst_offset, copy_length)
