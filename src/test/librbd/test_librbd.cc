...
TEST_F(TestLibRBD, GetId)
{
  ...
  if (!is_feature_enabled(0)) {
    ...
    std::array<char, 4096> image_name;
...
int test_ls_snaps(rbd_image_t image, int num_expected, ...)
{
  ...
  va_list ap;
  std::array<rbd_snap_info_t, max_size> snaps;
  auto max_snaps = max_size;
  num_snaps = rbd_snap_list(image, snaps.data(), &max_snaps);
...
static void test_list_children2(rbd_image_t image, int num_expected, ...)
{
  ...
  static constexpr int max_size = 10;
  va_list ap;
  std::array<rbd_child_info_t, max_size> children;
  auto max_snaps = max_size;
  num_children = rbd_list_children2(image, children.data(), &max_snaps);
...
TEST_F(TestLibRBD, FlushAio)
{
  ...
  std::string name = get_temp_image_name();
  uint64_t size = 2 << 20;
  static constexpr size_t num_aios = 256;
  ...
  std::array<rbd_completion_t, num_aios> write_comps;
...
TEST_F(TestLibRBD, TestPendingAio)
{
  ...
  static constexpr size_t num_aios = 256;
  std::array<rbd_completion_t, num_aios> comps;
...
TEST_F(TestLibRBD, SnapRemoveWithChildMissing)
{
  ...
  std::array<rbd_linked_image_spec_t, max_size> children;
  auto max_images = max_size;
  ASSERT_EQ(0, rbd_list_children3(parent, children.data(), &max_images));
