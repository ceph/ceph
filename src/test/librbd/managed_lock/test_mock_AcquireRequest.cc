...
template class librbd::managed_lock::AcquireRequest<librbd::managed_lock::MockLockWatcher>;
...
public:
  typedef AcquireRequest<MockLockWatcher> MockAcquireRequest;
  ...
  void expect_work_queue() {
    EXPECT_CALL(m_lock_watcher, work_queue()).WillRepeatedly(
