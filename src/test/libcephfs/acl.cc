...
TEST(ACL, SetACL) {
  ...
  ASSERT_EQ(ceph_fsetxattr(cmount, fd, POSIX_ACL_XATTR_DEFAULT, acl_buf, acl_buf_size, 0), -EACCES);
  ASSERT_EQ(ceph_fsetxattr(cmount, fd, POSIX_ACL_XATTR_ACCESS, acl_buf, acl_buf_size, 0), 0);
  ...
  ASSERT_EQ(ceph_fsetxattr(cmount, fd, POSIX_ACL_XATTR_ACCESS, acl_buf, acl_buf_size, 0), 0);
  // ACL was deleted
  ASSERT_EQ(ceph_fgetxattr(cmount, fd, POSIX_ACL_XATTR_ACCESS, NULL, 0), -ENODATA);
...
TEST(ACL, Chmod) {
  ...
  ASSERT_EQ(ceph_fgetxattr(cmount, fd, POSIX_ACL_XATTR_ACCESS, acl_buf, acl_buf_size), acl_buf_size);
  ASSERT_EQ(check_acl_and_mode(acl_buf, acl_buf_size, stbuf.st_mode), 0);
...
TEST(ACL, DefaultACL) {
  ...
  // set default acl
  ASSERT_EQ(ceph_setxattr(cmount, test_dir1, POSIX_ACL_XATTR_DEFAULT, acl1_buf, acl_buf_size, 0), 0);
  ...
  // inherit default acl
  ASSERT_EQ(ceph_getxattr(cmount, test_dir2, POSIX_ACL_XATTR_DEFAULT, acl2_buf, acl_buf_size), acl_buf_size);
  ASSERT_EQ(memcmp(acl1_buf, acl2_buf, acl_buf_size), 0);

  // mode and ACL are updated
  ASSERT_EQ(ceph_getxattr(cmount, test_dir2, POSIX_ACL_XATTR_ACCESS, acl2_buf, acl_buf_size), acl_buf_size);
  {
    struct stat stbuf;
  ...
  // no default acl
  ASSERT_EQ(ceph_fgetxattr(cmount, fd, POSIX_ACL_XATTR_DEFAULT, NULL, 0), -ENODATA);

  // mode and ACL are updated
  ASSERT_EQ(ceph_fgetxattr(cmount, fd, POSIX_ACL_XATTR_ACCESS, acl2_buf, acl_buf_size), acl_buf_size);
  {
    struct stat stbuf;
...
TEST(ACL, Disabled) {
  ...
  ASSERT_EQ(ceph_setxattr(cmount, test_dir, POSIX_ACL_XATTR_DEFAULT, acl_buf, acl_buf_size, 0), -EOPNOTSUPP);
  ASSERT_EQ(ceph_setxattr(cmount, test_dir, POSIX_ACL_XATTR_ACCESS, acl_buf, acl_buf_size, 0), -EOPNOTSUPP);
  ASSERT_EQ(ceph_getxattr(cmount, test_dir, POSIX_ACL_XATTR_DEFAULT, acl_buf, acl_buf_size), -EOPNOTSUPP);
  ASSERT_EQ(ceph_getxattr(cmount, test_dir, POSIX_ACL_XATTR_ACCESS, acl_buf, acl_buf_size), -EOPNOTSUPP);
