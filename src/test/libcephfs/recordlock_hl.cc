...
TEST(LibCephFS, BasicRecordLockingHL) {
  ...
  lock2.l_len = 1024;
  lock2.l_pid = getpid();
  ASSERT_EQ(-CEPHFS_EAGAIN, ceph_setlk(cmount, fd, &lock2, 43, false));
...
static void thread_ConcurrentRecordLockingHL(str_ConcurrentRecordLockingHL& s) {
  ...
  lock1.l_len = 1024;
  lock1.l_pid = getpid();
  ASSERT_EQ(-CEPHFS_EAGAIN, ceph_setlk(cmount, fd, &lock1, ceph_pthread_self(), false));
...
static void process_ConcurrentRecordLockingHL(str_ConcurrentRecordLockingHL& s) {
  ...
  lock1.l_len = 1024;
  lock1.l_pid = getpid();
  ASSERT_EQ(-CEPHFS_EAGAIN, ceph_setlk(cmount, fd, &lock1, mypid, false));
  PING_MAIN(1); // (1)
  lock1.l_type = F_WRLCK;
  lock1.l_whence = SEEK_SET;
  lock1.l_start = 0;
