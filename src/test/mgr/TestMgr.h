...
class ActivePyModulesTestHelper : public TestMgr {
public:
  LogChannelRef clog;
  LogChannelRef audit_clog;
  std::unique_ptr<PyModuleRegistry> py_registry;
  ...
  void SetUp() override {
    ...
    finisher = std::make_unique<Finisher>(cct.get());
    finisher->start();
  }
...
class StandbyPyModulesTestHelper : public TestMgr {
public:
  LogChannelRef clog;
  std::unique_ptr<PyModuleRegistry> py_registry;
...
// MetadataUpdate::finish() is defined in Mgr.cc. Providing a stub here avoids
// pulling in Mgr.cc (and its transitive deps) for tests that only need DaemonServer.
inline void MetadataUpdate::finish(int r)
{
  (void)r;
