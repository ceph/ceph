...
TEST_F(PyRuntime, PyFormatterRO_FreezesContainers) {
  PyFormatterRO ro; // default root is a dict
  ...
  ro.dump_pyobject("alist", src_list);
  ro.dump_pyobject("aset",  src_set);
  ro.dump_pyobject("adict", src_dict);
...
TEST_F(PyRuntime, PyFormatterRO_NestedFreezingAndJsonDump) {
  ...
  ro.dump_pyobject("outer", outer_list);
