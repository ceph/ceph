...
int TestMemIoCtxImpl::append(const std::string& oid, const bufferlist &bl,
  ...
  bufferlist_rw::from_ro_unsafe(file->data).begin(off).copy_in(bl.length(), bl);
  return 0;
}
...
int TestMemIoCtxImpl::write(const std::string& oid, bufferlist& bl, size_t len,
  ...
  bufferlist_rw::from_ro_unsafe(file->data).begin(off).copy_in(len, bl);
  return 0;
}
...
int TestMemIoCtxImpl::write_full(const std::string& oid, bufferlist& bl,
  ...
  bufferlist_rw::from_ro_unsafe(file->data).begin().copy_in(bl.length(), bl);
  return 0;
}
