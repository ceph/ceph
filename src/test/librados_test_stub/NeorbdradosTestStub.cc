...
class Client {
public:
  ceph::mutex mutex = ceph::make_mutex("NeorbdradosTestStub::Client");
      ...
      if (r >= 0 && snaps != nullptr) {
        *snaps = {};
        ...
        for (auto& clone : snap_set.clones) {
          neorbdrados::CloneInfo clone_info;
...
CephContext* neorbdrados::RADOS::cct() {
  return impl->test_rados_client->cct();
}

boost::asio::io_context& neorbdrados::RADOS::get_io_context() {
  return impl->io_context;
}

boost::asio::io_context::executor_type neorbdrados::RADOS::get_executor() const {
...
MockTestMemIoCtxImpl& get_mock_io_ctx(neorbdrados::RADOS& rados,
  ...
  auto& impl = *reinterpret_cast<std::unique_ptr<neorbdrados::detail::Client>*>(
    &rados);
