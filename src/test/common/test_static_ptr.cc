...
TEST(StaticPtr, CreationCall) {
  {
    static_ptr<base, sizeof(grandchild)> p(std::in_place_type<sibling1>);
    EXPECT_TRUE(p);
...
TEST(StaticPtr, CopyMove) {
  // Won't compile. Good.
  // static_ptr<base, sizeof(base)> p1(std::in_place_type<grandchild>, 3);

  static_ptr<base, sizeof(base)> p1(std::in_place_type<sibling1>);
  static_ptr<base, sizeof(grandchild)> p2(std::in_place_type<grandchild>, 3);
...
TEST(StaticPtr, ImplicitUpcast) {
  ...
  static_ptr<sibling2, sizeof(grandchild)> p2(std::in_place_type<grandchild>, 3);
...
TEST(StaticPtr, StaticCast) {
  static_ptr<base, sizeof(grandchild)> p1(std::in_place_type<grandchild>, 3);
...
TEST(StaticPtr, DynamicCast) {
  ...
  {
    static_ptr<base, sz> p1(std::in_place_type<grandchild>, 3);
...
TEST(StaticPtr, ConstCast) {
  static constexpr auto sz = sizeof(constable);
  {
    ...
    static_assert(std::is_const_v<decltype(p1)::element_type>,
                  "Things are not as const as they ought to be.");
    EXPECT_EQ(p1->foo(), 5);
    auto p2 = ceph::const_pointer_cast<constable, sz>(p1);
    static_assert(!std::is_const_v<decltype(p2)::element_type>,
...
TEST(StaticPtr, ReinterpretCast) {
  static constexpr auto sz = sizeof(grandchild);
  {
    ...
    static_assert(std::is_same_v<decltype(p2)::element_type, constable>,
                  "Reinterpret is screwy.");
    auto p3 = ceph::reinterpret_pointer_cast<grandchild, sz>(p2);
    static_assert(std::is_same_v<decltype(p3)::element_type, grandchild>,
                  "Reinterpret is screwy.");
    EXPECT_EQ(p3->func(), 9);
...
TEST(StaticPtr, Exceptional) {
  static_ptr<exceptional> p1(std::in_place_type<exceptional>);
