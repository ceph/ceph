...
TEST(formatter, ceph_json_parse_validation)
{
  constexpr ceph_json::parse_options permissive {
    .validate_trailing_whitespace = false
  };

  for (const string_view input : {
         "truegarbage", "nullgarbage", R"({"key":1}garbage)"}) {
    ceph_json::value parsed;
    EXPECT_FALSE(ceph_json::parse(input, parsed));
    EXPECT_THROW(ceph_json::parse(input), JSONDecoderErr);

    EXPECT_TRUE(ceph_json::parse(input, parsed, permissive));

    JSONParser strict_parser;
    EXPECT_FALSE(strict_parser.parse(input));

    JSONParser permissive_parser(permissive);
    EXPECT_TRUE(permissive_parser.parse(input));
  }

  ceph_json::value parsed;
  EXPECT_TRUE(ceph_json::parse("1234 \t\n", parsed));

  // Reusing the output must not carry object keys between documents:
  EXPECT_TRUE(ceph_json::parse(R"({"same":1})", parsed));
  EXPECT_TRUE(ceph_json::parse(R"({"same":2})", parsed));

  constexpr string_view duplicate = R"({"outer":{"key":1,"key":2}})";
  EXPECT_FALSE(ceph_json::parse(duplicate, parsed));
  EXPECT_THROW(ceph_json::parse(duplicate), JSONDecoderErr);

  JSONParser duplicate_parser;
  EXPECT_FALSE(duplicate_parser.parse(duplicate));

  buffer::list buffers;
  buffers.append(R"({"name":"with terminator"})");
  buffers.append("\0", 1);

  EXPECT_TRUE(ceph_json::parse(buffers, parsed));

  JSONParser parser;
  EXPECT_TRUE(parser.parse(buffers));
  EXPECT_FALSE(parser.parse("[]", -1));

  EXPECT_NO_THROW(JSONDecoder decoder(buffers));

  buffer::list empty;
  EXPECT_FALSE(ceph_json::parse(empty, parsed));
  EXPECT_FALSE(parser.parse(empty));
}
