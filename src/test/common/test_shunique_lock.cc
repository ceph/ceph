...
static void ensure_conflicts(SharedMutex& sm, ceph::acquire_unique_t) {
  auto ttl = &test_try_lock<std::shared_mutex>;
  auto ttls = &test_try_lock_shared<std::shared_mutex>;
...
TEST(ShuniqueLock, DefaultConstructor) {
  typedef ceph::shunique_lock<std::shared_mutex> shunique_lock;
...
TEST(ShuniqueLock, MoveConstruct) {
  ...
  std::shared_mutex sm;
  {
    ...
    ceph::shunique_lock<std::shared_mutex> l(std::move(ul));
    check_owns_lock(sm, l, ceph::acquire_unique);
    ensure_conflicts(sm, ceph::acquire_unique);
  }
  {
    std::unique_lock<std::shared_mutex> ul(sm, std::defer_lock);
    ensure_free(sm);
    ceph::shunique_lock<std::shared_mutex> l(std::move(ul));
    check_abjures_lock(sm, l);
    ensure_free(sm);
  ...
  }
  {
    ...
    ceph::shunique_lock<std::shared_mutex> l(std::move(sl));
...
TEST(ShuniqueLock, MoveAssign) {
  ...
  std::shared_mutex sm;
  {
    std::unique_lock<std::shared_mutex> ul(sm);
    ensure_conflicts(sm, ceph::acquire_unique);
    ceph::shunique_lock<std::shared_mutex> l;
    l = std::move(ul);
    check_owns_lock(sm, l, ceph::acquire_unique);
    ensure_conflicts(sm, ceph::acquire_unique);
  }
