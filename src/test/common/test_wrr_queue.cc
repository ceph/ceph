...
protected:
  ...
  void fill_queue(WQ &wq, LQ &strictq, LQ &normq,
    ...
    for (unsigned i = 1; i <= item_size; ++i) {
	  ...
	  wq.enqueue(Klass(k), std::make_tuple(p, k, o), p,
	    c, CEPH_OP_QUEUE_FRONT, CEPH_OP_CLASS_STRICT);
        } else {
	  //Queue to the back.
	  strictq[p][k].push_back(std::make_pair(
	    c, std::make_tuple(p, k, o)));
	  wq.enqueue(Klass(k), std::make_tuple(p, k, o), p,
	    c, CEPH_OP_QUEUE_BACK, CEPH_OP_CLASS_STRICT);
        }
        break;
      default:
        // Normal queue
	  ...
	  normq[p][k].push_front(std::make_pair(
	    c, std::make_tuple(p, k, o)));
	  wq.enqueue(Klass(k), std::make_tuple(p, k, o), p,
	    c, CEPH_OP_QUEUE_FRONT);
        } else {
	  //Queue to the back.
	  normq[p][k].push_back(std::make_pair(
	    c, std::make_tuple(p, k, o)));
	  wq.enqueue(Klass(k), std::make_tuple(p, k, o), p, c);
        }
        break;
      }
    }
...
TEST_F(WrrQueueTest, wrr_size){
  ...
  for (unsigned i = 1; i < 5; ++i) {
    wq.enqueue(Klass(i), std::make_tuple(i, i, i), i, 0,
  ...
  for (unsigned i = 5; i < 10; ++i) {
    wq.enqueue(Klass(i), std::make_tuple(i, i, i), i);
    EXPECT_FALSE(wq.empty());
    EXPECT_EQ(i, wq.length());
  }
...
TEST_F(WrrQueueTest, wrr_test_static) {
  test_queue(1000);
} 

TEST_F(WrrQueueTest, wrr_test_random) {
...
TEST_F(WrrQueueTest, wrr_test_remove_by_filter) {
  WQ wq(0, 0);
  LQ strictq, normq;
...
TEST_F(WrrQueueTest, wrr_test_remove_by_class) {
  WQ wq(0, 0);
  LQ strictq, normq;
  unsigned num_items = 1000;
  fill_queue(wq, strictq, normq, num_items);
