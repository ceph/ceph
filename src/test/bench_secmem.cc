...
// Standalone build:
// g++ --std=c++20 -g -O2 -o bench ../src/test/bench_secmem.cc -lbenchmark -lkeyutils
...
void BM_MemfdSecret_RandomRead(benchmark::State& state) {
  ...
  std::call_once(mem_initialized, [&]() {
    const int fd = syscall(SYS_memfd_secret, 0);
    if (fd == -1) {
    ...
    if (ret == -1) {
      state.SkipWithError(std::format(
          "ftruncate: {} errno:{} error:{}", ret, errno, std::strerror(errno)));
      return;
    }
    mem = ::mmap(
