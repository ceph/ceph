...
/* Scenario G: Feature Flag Disabled
 *
 * Verifies that when standby_enable_host_anti_affinity is false for the file system,
 * the selection algorithm ignores host placement and strictly honors mds_join_fs.
 */
TEST_F(StandbyAffinityTest, ScenarioG_Feature_Flag_Disabled) {
  // Disable the feature flag
  fs.get_mds_map().set_standby_enable_host_anti_affinity(false);

  FSMap local_fsmap = fsmap;

  // Standby 1: Co-located on Host A, but PERFECT join_fscid match
  MDSMap::mds_info_t s1;
  s1.global_id = mds_gid_t(701);
  s1.name = "standby_match_same_host";
  s1.state = MDSMap::STATE_STANDBY;
  s1.addrs = same_host_addrs; // Host A
  s1.join_fscid = test_fscid; // Base Score 6 (normally penalized to 3)
  local_fsmap.insert(s1);

  // Standby 2: Isolated on Host B, but NO join_fscid match
  MDSMap::mds_info_t s2;
  s2.global_id = mds_gid_t(702);
  s2.name = "standby_vanilla_diff_host";
  s2.state = MDSMap::STATE_STANDBY;
  s2.addrs = diff_host_addrs; // Host B
  s2.join_fscid = FS_CLUSTER_ID_NONE; // Base Score 5
  local_fsmap.insert(s2);

  // Invoke the placement scoring matrix routine, passing the active host address
  const auto* selected = local_fsmap.get_available_standby(fs, active_addrs);
  ASSERT_NE(selected, nullptr);

  // Verify that with the flag disabled, Standby 1 (GID 701) wins purely based on mds_join_fs
  ASSERT_EQ(static_cast<uint64_t>(selected->global_id), 701ULL);

  // Reset the feature flag so it doesn't pollute subsequent test runs (if fs was reused)
  fs.get_mds_map().set_standby_enable_host_anti_affinity(true);
}
