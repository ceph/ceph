...
TEST_F(QuiesceAgentTest, DbUpdates) {
  ...
  EXPECT_EQ(1, ack1->db_version);
  EXPECT_EQ(3, ack1->roots.size());
  EXPECT_EQ(QS_QUIESCING, ack1->roots.at("root2").state);
  EXPECT_EQ(QS_EXPIRED, ack1->roots.at("root3").state);
  EXPECT_EQ(QS_EXPIRED, ack1->roots.at("root4").state);
  EXPECT_TRUE(await_idle());

  EXPECT_EQ(1, agent->get_current_version());
  auto roots1 = agent->tracked_roots();
  EXPECT_EQ(5, roots1.size());
  EXPECT_EQ(QS_QUIESCING, roots1.at("root0")->committed_state);
  EXPECT_EQ(QS_QUIESCING, roots1.at("root1")->committed_state);
  EXPECT_EQ(QS_QUIESCED, roots1.at("root2")->committed_state);
  EXPECT_EQ(QS_RELEASING, roots1.at("root3")->committed_state);
  EXPECT_EQ(QS_RELEASED, roots1.at("root4")->committed_state);

  // manipulate root0 and root1 as if they were quiesced and root2 as if it was released
  {
    auto& root = *roots1.at("root0");
    root.quiesce_result = 0;
    EXPECT_EQ(QS_QUIESCED, root.get_actual_state());
  }

  {
    auto& root = *roots1.at("root1");
    root.quiesce_result = 0;
    EXPECT_EQ(QS_QUIESCED, root.get_actual_state());
  }

  {
    auto& root = *roots1.at("root2");
    root.quiesce_result = 0;
    root.cancel_result = 0;
    EXPECT_EQ(QS_RELEASED, root.get_actual_state());
  }
  ...
  EXPECT_EQ(2, ack2->db_version);
  EXPECT_EQ(3, ack2->roots.size());
  EXPECT_EQ(QS_QUIESCED, ack2->roots.at("root0").state);
  EXPECT_EQ(QS_QUIESCED, ack2->roots.at("root1").state);
  EXPECT_EQ(QS_RELEASED, ack2->roots.at("root2").state);

  EXPECT_TRUE(await_idle());

  auto roots2 = agent->tracked_roots();
  EXPECT_EQ(3, roots2.size());
  EXPECT_EQ(QS_RELEASING, roots2.at("root0")->committed_state);
  EXPECT_EQ(QS_QUIESCING, roots2.at("root1")->committed_state);
  EXPECT_EQ(QS_RELEASING, roots2.at("root2")->committed_state);
}
