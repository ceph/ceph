...
TEST(ContextGather, OneSub) {
  ...
  EXPECT_EQ(1, gather.get()->get_num());
  EXPECT_EQ(1, gather.get()->get_num_remaining());
  EXPECT_FALSE(gather.get()->empty());
...
TEST(ContextGather, ManySubs) {
  ...
  for (int i = 0; i < sub_count; ++i) {
    ...
    EXPECT_EQ(i+1, gather.get()->get_num());
    EXPECT_EQ(i+1, gather-get()->get_num_remaining());
  }
  ...
  for (int j = 0; j < sub_count - 1; ++j) {
    subs[j]->finish(0);
    delete subs[j];
    EXPECT_EQ(sub_count - j - 1, gather.get()->get_num_remaining());
    EXPECT_EQ(sub_count, gather.get()->get_num());
...
TEST(ContextGather, AlternatingSubCreateFinish) {
  ...
  for (int j = 0; j < sub_count / 2; ++j) {
    subs[j]->finish(0);
    delete subs[j];
    EXPECT_EQ(sub_count / 2 - 1, gather.get()->get_num_remaining());
    
    subs[sub_count / 2 + j] = gather.new_sub();
    EXPECT_EQ(sub_count / 2 + 1 + j, gather.get()->get_num());
    EXPECT_EQ(sub_count / 2, gather.get()->get_num_remaining());
    EXPECT_FALSE(gather.get()->empty());
  }

  //finish last half of subs
  for (int k = sub_count / 2; k < sub_count; ++k) {
    EXPECT_EQ(sub_count - k, gather.get()->get_num_remaining());
    EXPECT_EQ(sub_count, gather.get()->get_num());
    EXPECT_FALSE(gather.get()->empty());
    subs[k]->finish(0);
    delete subs[k];
