...
TEST_F(AsioRados, AsyncReadOperationCallback)
{
  boost::asio::io_context service;
  {
    ...
    librados::async_operate(service, io, "exist", &op, 0, {false, false}, success_cb);
  }
  {
    librados::ObjectReadOperation op;
    op.read(0, 0, nullptr, nullptr);
    ...
    librados::async_operate(service, io, "noexist", &op, 0, {false, false}, failure_cb);
  }
  service.run();
}
...
TEST_F(AsioRados, AsyncReadOperationFuture)
{
  ...
  std::future<bufferlist> f1;
  {
    librados::ObjectReadOperation op;
    op.read(0, 0, nullptr, nullptr);
    f1 = librados::async_operate(service, io, "exist", &op, 0, {false, false},
                                 boost::asio::use_future);
  }
  std::future<bufferlist> f2;
  {
    librados::ObjectReadOperation op;
    op.read(0, 0, nullptr, nullptr);
    f2 = librados::async_operate(service, io, "noexist", &op, 0, {false, false},
                                 boost::asio::use_future);
  }
  service.run();
...
TEST_F(AsioRados, AsyncReadOperationYield)
{
  ...
  auto success_cr = [&] (boost::asio::yield_context yield) {
    ...
    op.read(0, 0, nullptr, nullptr);
    boost::system::error_code ec;
    auto bl = librados::async_operate(service, io, "exist", &op, 0, {false, false},
                                      yield[ec]);
    EXPECT_FALSE(ec);
    EXPECT_EQ("hello", bl.to_str());
  };
  ...
  auto failure_cr = [&] (boost::asio::yield_context yield) {
    ...
    op.read(0, 0, nullptr, nullptr);
    boost::system::error_code ec;
    auto bl = librados::async_operate(service, io, "noexist", &op, 0, {false, false},
                                      yield[ec]);
    EXPECT_EQ(boost::system::errc::no_such_file_or_directory, ec);
  };
  boost::asio::spawn(service, failure_cr, rethrow);
...
TEST_F(AsioRados, AsyncWriteOperationCallback)
{
  ...
  }
  {
    ...
    librados::async_operate(service, snapio, "exist", &op, 0, {false, false}, failure_cb);
  }
  service.run();
}
...
TEST_F(AsioRados, AsyncWriteOperationFuture)
{
  ...
  std::future<void> f2;
  {
    librados::ObjectWriteOperation op;
    op.write_full(bl);
    f2 = librados::async_operate(service, snapio, "exist", &op, 0, {false, false},
                                 boost::asio::use_future);
  }
  service.run();
...
TEST_F(AsioRados, AsyncWriteOperationYield)
{
  ...
  auto success_cr = [&] (boost::asio::yield_context yield) {
    ...
    op.write_full(bl);
    boost::system::error_code ec;
    librados::async_operate(service, io, "exist", &op, 0, {false, false}, yield[ec]);
    EXPECT_FALSE(ec);
  };
  boost::asio::spawn(service, success_cr, rethrow);
  ...
  auto failure_cr = [&] (boost::asio::yield_context yield) {
    ...
    op.write_full(bl);
    boost::system::error_code ec;
    librados::async_operate(service, snapio, "exist", &op, 0, {false, false}, yield[ec]);
    EXPECT_EQ(boost::system::errc::read_only_file_system, ec);
  };
  boost::asio::spawn(service, failure_cr, rethrow);
