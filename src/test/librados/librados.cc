...
TEST(Librados, ConnectRetriesStopAtMountTimeout) {
  rados_t cluster = make_unconnectable_cluster();
  // enough retries and backoff to run for minutes if only the count bounded
  // them; the wall-clock budget has to cut them off first
  ASSERT_EQ(0, rados_conf_set(cluster, "rados_connect_retries", "1000"));
  ASSERT_EQ(0, rados_conf_set(cluster, "rados_connect_retry_interval", "0.05"));
  ASSERT_EQ(0, rados_conf_set(cluster, "client_mount_timeout", "1"));

  int err = 0;
  double elapsed = time_connect(cluster, &err);
  ASSERT_EQ(-ENOENT, err);
  ASSERT_GE(elapsed, 0.5);
