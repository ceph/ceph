  ...
  Context *on_commit)
{
  ...
  h.pool = 0;
  t->write(c, ghobject_t(h), offset, bl.length(), bl, 0, ObjectStore::Transaction::write_params_t());
  ...
  if (write_infos) {
    ...
    t->write(meta, ghobject_t(info), 0, bl2.length(), bl2, 0, ObjectStore::Transaction::write_params_t());
  }
