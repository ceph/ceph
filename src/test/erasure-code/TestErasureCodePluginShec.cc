...
TEST(ErasureCodePlugin, factory)
{
  ...
  parameters["directory"] = ".libs";
  {
    ...
    EXPECT_EQ(0, instance.factory("shec", parameters,
                                        &erasure_code, cerr));
...
TEST(ErasureCodePlugin, select)
{
  ...
  // happens
  parameters["shec-name"] = "test_shec";
  parameters["directory"] = ".libs";
  ...
  // all features are available, load the SSE4 plugin
  {
    ...
    EXPECT_EQ(sse4_side_effect, instance.factory("shec", parameters,
                                                 &erasure_code, cerr));
  }
  // pclmul is missing, load the SSE3 plugin
  {
    ...
    EXPECT_EQ(sse3_side_effect, instance.factory("shec", parameters,
                                                 &erasure_code, cerr));
  }
  // pclmul and sse3 are missing, load the generic plugin
  {
    ...
    EXPECT_EQ(generic_side_effect, instance.factory("shec", parameters,
						    &erasure_code, cerr));
  }
...
#define LARGE_ENOUGH 2048
  ...
  for (vector<string>::iterator sse_variant = sse_variants.begin();
    ...
    ErasureCodeInterfaceRef erasure_code;
    EXPECT_FALSE(erasure_code);
    EXPECT_EQ(0, instance.factory("shec_" + *sse_variant, parameters,
                                  &erasure_code, cerr));
    EXPECT_TRUE(erasure_code);
