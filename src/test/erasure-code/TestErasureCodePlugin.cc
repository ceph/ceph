...
TEST_F(ErasureCodePluginRegistryTest, all)
{
  ...
  PluginRegistry *instance = g_ceph_context->get_plugin_registry();
  {
    ...
    EXPECT_EQ(0, instance->load("erasure-code", "ec_example"));
  }
  ErasureCodePlugin* ecp = dynamic_cast<ErasureCodePlugin*>(instance->get_with_load("erasure-code", "ec_example"));
  EXPECT_TRUE(ecp);
  ...
  EXPECT_TRUE(erasure_code.get());
  {
    ...
    EXPECT_EQ(-EEXIST, instance->load("erasure-code", "ec_example"));
    EXPECT_EQ(-ENOENT, instance->remove("erasure-code", "does not exist"));
    EXPECT_EQ(0, instance->remove("erasure-code", "ec_example"));
    EXPECT_EQ(0, instance->load("erasure-code", "ec_example"));
  }
}
