...
public:
  ...
  unsigned int calculate_crc(const bufferlist& bl, int crc_seed)
  {
    bufferhash hash(crc_seed);
    hash << bl;
    return hash.digest();
  }
  unsigned int calculate_zero_buffer_crc(int crc_seed)
  {
    bufferlist zero_bl;
    generate_chunk(zero_bl, 0);
    return calculate_crc(zero_bl, crc_seed);
  }
  bufferptr create_buffer_from_crc(unsigned int crc)
  {
  ...
  unsigned int read_crc_from_bufferlist(bufferlist& bl, unsigned int offset = 0)
  {
    unsigned int crc = 0;
    std::size_t length = sizeof(unsigned int);
    for (std::size_t i = 0; i < length; i++) {
...
TEST_P(PluginTest,CRCEncodeDecodeSupport)
{
  ...
  int crc_seed = -1;
  unsigned int zero_data_crc = calculate_zero_buffer_crc(crc_seed);
  ...
  for (unsigned int i = 0; i < get_k(); ++i) {
    ...
    unsigned int crc = calculate_crc(data_shard_bl, crc_seed);
  ...
  for (shard_id_t shard_id : want_to_encode) {
    ...
    if (shard_id < get_k() + 1)
    {
      ...
      unsigned int calculated_crc = calculate_crc(encoded_data.at(shard_id), crc_seed);
      ...
      unsigned int unseeded_crc = read_crc_from_bufferlist(encoded_unseeded_hashes.at(shard_id));
    ...
    ECUtil::stripe_info_t sinfo{get_k(),
                                get_m(),
                                get_k() * chunk_size,
    ...
    for (raw_shard_id_t missing_raw_shard_id{0};
         missing_raw_shard_id < get_k(); ++missing_raw_shard_id) {
      ...
      unsigned int decoded_crc = read_crc_from_bufferlist(out_bls[missing_shard_id]);
      unsigned int original_crc = read_crc_from_bufferlist(hashes_bl, missing_shard_id.id * chunk_size);
