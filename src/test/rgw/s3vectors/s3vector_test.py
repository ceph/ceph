...
def verify_list_vectors_pagination(conn, bucket_name, index_name, expected_vectors, max_results,
    ...
    for idx in range(len(all_retrieved_vectors)):
        assert all_retrieved_vectors[idx]['key'] == expected_vectors[idx]['key']
        if return_data:
            vector_pairs = zip(all_retrieved_vectors[idx]['data']['float32'], expected_vectors[idx]['data']['float32'])
            assert all(abs(a - b) < 1e-6 for a, b in vector_pairs), \
                f"returned data don't match expected data {vector_pairs}"
