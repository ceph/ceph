...
TEST(S3VectorKeyNameTest, ValidNames) {
  for (const auto& name : {"genre", "underscore_is_ok", "my key", " genre ", "my-key",
                           ...
                           "$eq", "$and", "$or", "'genre'",
                           "my\"key", "\"genre\""}) {
    EXPECT_TRUE(validate_metadata_key_name(name)) << "name: " << name;
    EXPECT_TRUE(validate_declared_metadata_key_name(name)) << "name: " << name;
  }
}

TEST(S3VectorKeyNameTest, EmptyNameRejected) {
  EXPECT_FALSE(validate_metadata_key_name(""));
  EXPECT_FALSE(validate_declared_metadata_key_name(""));
}
...
TEST(S3VectorKeyNameTest, LengthValidation) {
  ...
  EXPECT_FALSE(validate_declared_metadata_key_name(too_long));
  EXPECT_TRUE(validate_metadata_key_name(too_long));
  EXPECT_TRUE(validate_declared_metadata_key_name(longest));
}

TEST(S3VectorKeyNameTest, UnderscorePrefixRejected) {
  for (const auto& name : {"_key", "_data", "_metadata", "_distance", "_", "_anything"}) {
    EXPECT_FALSE(validate_metadata_key_name(name)) << "name: " << name;
    EXPECT_FALSE(validate_declared_metadata_key_name(name)) << "name: " << name;
  }
}

TEST(S3VectorKeyNameTest, DotRejected) {
  EXPECT_FALSE(validate_metadata_key_name("user.name"));
  EXPECT_FALSE(validate_metadata_key_name("."));
  EXPECT_FALSE(validate_declared_metadata_key_name("user.name"));
  EXPECT_FALSE(validate_declared_metadata_key_name("."));
}
