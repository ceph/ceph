...
class KafkaReceiver(object):
    ...
    def __init__(self, topic_name, security_type, kafka_server):
        ...
        if remaining_retries == 0:
            raise Exception('Kafka admin failed to create topic - no retries left')
        ...
        while remaining_retries > 0:
            try:
                ...
                remaining_retries -= 1
                log.warning('Kafka consumer failed to connect to broker/s: %s. remaining reties: %d. error: %s',
                            endpoint , remaining_retries, str(error))
                time.sleep(1)

        if remaining_retries == 0:
            raise Exception('Kafka consumer failed to connect to kafka - no retries left')
...
def kafka_receiver_thread_runner(receiver):
    ...
    try:
        ...
        log.info('Kafka receiver ended unexpectedly. error: %s', str(error))
...
def stop_kafka_receiver(receiver, task):
    ...
    try:
        ...
        log.info('Kafka receiver failed to gracefully stop. error: %s', str(error))
...
def verify_kafka_receiver(receiver):
    ...
    remaining_retries = 10
    while len(events) == 0:
        log.info('Kafka receiver (in "%s" state) waiting for test event (at: %s). remaining retries: %d', receiver.status, datetime.datetime.now(), remaining_retries)
        print('Kafka receiver (in "%s" state) waiting for test event (at: %s). remaining retries: %d' % (receiver.status, datetime.datetime.now(), remaining_retries))
        time.sleep(1)
        events = receiver.get_and_reset_events()
        remaining_retries -= 1
        if remaining_retries == 0:
            raise Exception('kafka receiver did not receive test event in time')
    assert_equal(len(events), 1)
    assert_in('test', events[0])
    log.info('Kafka receiver tested ok')
