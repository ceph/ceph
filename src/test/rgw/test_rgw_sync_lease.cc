...
using MockWaiter = ceph::async::co_waiter<void, executor_type>;
...
class MockClient : public LockClient {
 ...
 public:
  ...
  asio::awaitable<void> acquire(ceph::timespan) override {
    events.push_back(event::acquire);
    co_await waiter.get();
  }
  asio::awaitable<void> renew(ceph::timespan) override {
