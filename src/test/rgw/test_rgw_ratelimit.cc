...
#define LIST_TYPE_2 "?list-type=2"
...
TEST(RGWRateLimit, reject_list_op_over_limit)
{
  ...
  bool success = ratelimit.should_rate_limit("GET", key, time, &info, LIST_TYPE_2);
  time = ceph::coarse_real_clock::now();
  success = ratelimit.should_rate_limit("GET", key, time, &info, LIST_TYPE_2);
  EXPECT_EQ(true, success);
}
...
TEST(RGWRateLimit, accept_list_op_after_giveback)
{
  ...
  std::string key = "uuser_list";
  bool success = ratelimit.should_rate_limit("GET", key, time, &info, LIST_TYPE_2);
  ratelimit.giveback_tokens("GET", key, LIST_TYPE_2);
  time = ceph::coarse_real_clock::now();
  success = ratelimit.should_rate_limit("GET", key, time, &info, LIST_TYPE_2);
  EXPECT_EQ(false, success);
}
