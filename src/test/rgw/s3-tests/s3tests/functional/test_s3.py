...
def test_object_content_disposition_duplicate_header_not_joined():
    """tracker #75304, the other half: signature verification must combine
    duplicate field-lines for every header, because that is what the client
    signed, but only headers that RFC-define a comma-separated list may have
    the combined value stored as the object attribute. Content-Disposition has
    no list form, so joining two field-lines would persist a malformed value.
    The request must still be accepted; the object keeps the last value."""
    bucket = get_new_bucket()
    key = 'dup-content-disposition'
    body = b'hello duplicate headers'
    ...
    # Sign the comma-joined form, exactly as for Content-Encoding above: RFC
    # 7230 equivalence applies to the canonical request regardless of whether
    # the field is a list.
    first = 'attachment; filename="a"'
    second = 'inline'
    url = '%s/%s/%s' % (config.default_endpoint, bucket, key)
    headers = {'Host': '%s:%d' % (host, port),
               'Content-Disposition': '%s,%s' % (first, second)}
    req = AWSRequest(method='PUT', url=url, data=body, headers=headers)
    S3SigV4Auth(creds, 's3', region).add_auth(req)
    signed = dict(req.headers)
    ...
    try:
        conn.putrequest('PUT', '/%s/%s' % (bucket, key),
                        skip_host=True, skip_accept_encoding=True)
        for name, value in signed.items():
            if name.lower() in ('content-disposition', 'content-length'):
                continue
            conn.putheader(name, value)
        conn.putheader('Content-Disposition', first)
        conn.putheader('Content-Disposition', second)
        conn.putheader('Content-Length', str(len(body)))
        conn.endheaders()
        conn.send(body)
        resp = conn.getresponse()
    ...
    # 200 proves verification combined the field-lines.
    assert status == 200

    # The stored attribute must be the last value alone. Joining would give
    # 'attachment; filename="a",inline', which is not a legal
    # Content-Disposition and would be echoed back on every GET.
    client = get_client()
    response = client.head_object(Bucket=bucket, Key=key)
    assert response['ContentDisposition'] == second
