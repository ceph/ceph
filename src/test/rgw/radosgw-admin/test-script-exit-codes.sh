...
# when a test fails. rgw_global_init consumes --log-to-stderr before the flag loop runs.
...
# missing required
check_cluster "put: missing --context"        22 -- \
  script put --infile /dev/null
check_cluster "put: missing --infile"         22 -- \
  script put --context prerequest
check_cluster "put: missing both"             22 -- \
  script put
...
# error or a warning.
check "put: --context missing value" 1 \
  script put --context
check "put: --infile missing value"  1 \
  script put --infile
...
check_cluster "put: --context and --tenant before script and again after put, no --infile"  22 -- \
  --context prerequest --tenant foo script put --context background --tenant bar
check_cluster "put: all three before script and again after put, --tenant with background context"  22 -- \
...
check_cluster "put: invalid lua syntax" 22 -- \
  script put --context prerequest --infile "$_bad_lua"
rm -f "$_bad_lua"

# non-existent infile
check_cluster "put: file not found" 2 -- \
...
check "script-package (incomplete command)" 1 script-package
check "script-package: unknown subcommand" 1 script-package banana
check "script-package: empty subcommand" 1 script-package ""
check "script-package: repeated command word" 1 script-package script-package list

# stray positional args
check "script-package: stray between the command words" 1 script-package extra list
...
# attempted and fails.
check_cluster "script-package add: --allow-compilation with no value" 22 -- script-package add --allow-compilation --package no-such-package
check_cluster "script-package add: --allow-compilation true (bool consumed)" 22 -- script-package add --allow-compilation true --package no-such-package
check_cluster "script-package add: --allow-compilation false (bool consumed)" 22 -- script-package add --allow-compilation false --package no-such-package
check_cluster "script-package add: --allow-compilation=true (=form)" 22 -- script-package add --allow-compilation=true --package no-such-package
...
check_cluster "script-package add: unrelated binary flag --fix accepted" 22 -- script-package add --fix --package no-such-package
check_cluster "script-package add: unrelated value flag --max-entries=5 (=form)" 22 -- script-package add --max-entries=5 --package no-such-package
check_cluster "script-package add: unrelated --max-entries 5 swallowed (space form)" 22 -- script-package add --max-entries 5 --package no-such-package
...
check_cluster "script-package list: --tenant" 22 -- script-package list --tenant t
...
check "script package: two words is not the command" 1 script package list
check "script-packages: not the command" 1 script-packages list
check "scriptpackage: not the command" 1 scriptpackage list

# a subcommand of one command is not a subcommand of the other
check "script-package get: not a script-package subcommand" 1 script-package get
check "script-package put: not a script-package subcommand" 1 script-package put
...
check_cluster "script-package add: --context accepted and ignored" 22 -- script-package add --context prerequest --package no-such-package
check_cluster "script get: --package accepted and ignored" 0 -- script get --context prerequest --package no-such-package
...
check_cluster "integration: get after rm"             0 -- \
  script get --context prerequest
...
check_cluster "integration: put for remove alias"     0 -- \
  script put --context prerequest --infile "$_script_file"
check_cluster "integration: get before remove"        0 -- \
  script get --context prerequest
check_cluster "integration: remove prerequest script" 0 -- \
  script remove --context prerequest
check_cluster "integration: get after remove"         0 -- \
  script get --context prerequest
...
check_cluster "integration: get after tenant rm"      0 -- \
  script get --context prerequest --tenant testenant
...
check "bare script"         1 script
check "unknown subcommand"  1 script banana
check "reversed put script" 1 put script
...
check_cluster "metadata list script" 2 -- metadata list script
check_cluster "metadata get script" 2 -- metadata get script
check_cluster "metadata rm script" 2 -- metadata rm script
check_cluster "metadata list script.instance" 2 -- metadata list script.instance
check_cluster "metadata list banana" 2 -- metadata list banana
...
# fail there.
check_cluster "script-package add: --package script" 22 -- script-package add --package script
check_cluster "script-package add: --package banana" 22 -- script-package add --package banana
...
check_cluster "user info: --access-key script" 22 -- user info --access-key script
...
check_cluster "script get: --access-key bucket" 22 -- script get --access-key bucket
check_cluster "script get: --access-key script" 22 -- script get --access-key script
check_cluster "script get: --access-key banana" 22 -- script get --access-key banana
check "script: --access-key script, no subcommand" 1 script --access-key script
check_cluster "script-package list: --access-key script" 22 -- script-package list --access-key script
check_cluster "script-package list: --access-key banana" 22 -- script-package list --access-key banana
check_cluster "script get: --access-key script before the command words" 22 -- --access-key script script get
check_cluster "script-package list: --access-key script before the command words" 22 -- --access-key script script-package list
check_cluster "metadata list user: --access-key script before the command words" 22 -- --access-key script metadata list user

# a flag the loop does know takes the word as its value first
check_cluster "user info: --uid script" 22 -- user info --uid script
check_cluster "user info: --bucket script" 22 -- user info --bucket script
check_cluster "metadata list: --bucket script" 0 -- metadata list --bucket script

# a global flag before the command words
check_cluster "metadata list script: --tenant before the command" 22 -- --tenant t metadata list script
check_cluster "script-package list: --tenant before the command" 22 -- --tenant t script-package list
