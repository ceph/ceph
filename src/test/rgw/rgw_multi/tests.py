...
def test_bucket_full_sync_when_the_bucket_is_deleted_in_the_meantime():
    num_objects_to_upload = (
        10000  # must be more than 1000 to have pagination at full sync
    )
    ...
    def trim_bilog(stop_event: threading.Event):
        log.info(f"starting bilog trim thread for bucket={bucket.name}")
        while not stop_event.is_set():
            try:
                primary_zone_cluster_conn.cluster.admin(
                    ["bilog", "trim", "--bucket", bucket.name]
                )
            except Exception as e:
                log.debug(f"ignore trim bilog failure for bucket={bucket.name}: {e}")
            time.sleep(1)
        log.info(f"stopping bilog trim thread for bucket={bucket.name}")

    try:
        # create a trim thread to avoid incremental sync from happenning
        trim_thread_stop = threading.Event()
        trim_thread = threading.Thread(
            target=trim_bilog, args=(trim_thread_stop,), daemon=False
        )
        trim_thread.start()

        log.info(
            "set rgw_inject_delay_sec and rgw_inject_delay_pattern to slow down bucket full sync"
        )
        secondary_zone_cluster_conn.cluster.ceph_admin(
            [
                ...
                str(bucket_full_sync_listing_inject_delay_sec),
            ]
        )
        secondary_zone_cluster_conn.cluster.ceph_admin(
            [
                ...
                bucket_full_sync_listing_inject_delay_pattern,
            ]
        )
        ...
        # Since bilog trimming cripples incremenetal sync and so does rgw_inject_delay_sec full sync,
        ...
        zonegroup_data_checkpoint(zonegroup_conns)
        deadline = config.checkpoint_retries * config.checkpoint_delay + time.time()
        while time.time() < deadline:
            if (
                get_oldest_incremental_change_not_applied_epoch(
                    secondary_zone_cluster_conn
                )
                is None
            ):
            ...
            raise Exception("bucket sync did not complete in time")
    except Exception as e:
        ...
        log.info("bilog trim thread stopped")
