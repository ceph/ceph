...
alt_user = None
config = None
def init_multi(_realm, _user, _alt_user, _non_account_user, _non_account_alt_user, _config=None):
    global realm
    realm = _realm
    global user
    user = _user
...
def test_assume_role_after_sync():
    ...
    role_name = gen_role_name()
    log.info('create role zone=%s name=%s', zonegroup_conns.master_zone.name, role_name)
    policy_document = "{\"Version\":\"2012-10-17\",\"Statement\":[{\"Effect\":\"Allow\",\"Principal\":{\"AWS\":[\"arn:aws:iam::RGW11111111111111111:user/alt_tester\"]},\"Action\":[\"sts:AssumeRole\"]}]}"
