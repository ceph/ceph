...
def remove_sync_group_pipe(cluster, group, pipe, bucket = None):
    cmd = ['sync', 'group', 'pipe', 'remove', '--group-id', group, '--pipe-id', pipe]
    if bucket:
        cmd += ['--bucket', bucket]
    (result_json, retcode) = cluster.admin(cmd)
    if retcode != 0:
        assert False, 'failed to remove sync pipe id=%s, bucket=%s' % (pipe, bucket)
    return json.loads(result_json)
