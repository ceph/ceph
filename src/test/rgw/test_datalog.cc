...
protected:
  ...
  asio::awaitable<std::unordered_map<std::string, uint64_t>>
  read_all_sems_all_shards() {
    std::unordered_map<std::string, uint64_t> all_sems;
  ...
  asio::awaitable<std::unordered_map<BucketGen, uint64_t>>
  read_all_log(const DoutPrefixProvider* dpp) {
    std::unordered_map<BucketGen, uint64_t> all_keys;
