...
def verify_dedup_ratio(expcted_dedup_stats, dedup_ratio):
    ...
    s3_dedup_bytes  = expcted_dedup_stats.dedup_bytes_estimate
    ...
    log.info("s3_dedup_bytes = %d", expcted_dedup_stats.dedup_bytes_estimate);
...
def exec_dedup(expcted_dedup_stats, dry_run, verify_stats=True):
    ...
    max_dedup_time = 5*60
    if expcted_dedup_stats.deduped_obj > 10000:
        max_dedup_time = 20 * 60
    elif expcted_dedup_stats.deduped_obj > 5000:
        max_dedup_time = 10 * 60
    elif expcted_dedup_stats.deduped_obj > 1000:
        max_dedup_time = 5 * 60
    ...
    verify_dedup_ratio(expcted_dedup_stats, dedup_ratio_estimate)
...
def threads_simple_dedup_with_tenants(files, conns, bucket_names, config, dry_run=False):
    ...
    start = time.time_ns()
    ret=procs_upload_objects(files, conns, bucket_names, indices, config)
    upload_time_sec = (time.time_ns() - start) / (1000*1000*1000)
    expected_results = ret[0]
    dedup_stats = ret[1]
    s3_objects_total = ret[2]

    exec_time_sec = exec_dedup(dedup_stats, dry_run)
    verify_time_sec=0
    if dry_run == False:
        log.info("Verify all objects")
        start = time.time_ns()
    ...
    log.warning("[%d] obj_count=%d, upload=%d(sec), exec=%d(sec), verify=%d(sec)",
...
def corrupt_etag(key):
    ...
    result = rados(['getxattr', rados_name, 'user.rgw.etag'])
    assert result[1] == 0
    
    log.warning("etag of % = %s", rados_name, result[1])
...
def test_dedup_etag_corruption():
    ...
    try:
        files=[]
        num_files = 1
        # generate a single object of 8MB with 2 identical copies
        gen_files_fixed_copies(files, num_files, 8*MB, 2)
...
def test_dedup_inc_loop_with_tenants():
    ...
    try:
        ...
        for idx in range(0, 9):
            log.warning("test_dedup_inc_loop_with_tenants: INC-STEP %d", idx)
            ret = inc_step_with_tenants(stats_base, files, conns, bucket_names, config)
            files=ret[0]
            stats_last=ret[1]
            log.warning("skip_src_record=%d/%d, set_shared_manifest=%d",
                        stats_base.skip_src_record,
                        stats_last.skip_src_record,
                        stats_last.set_shared_manifest)
            stats_base.skip_src_record     += stats_last.skip_src_record
            stats_base.skip_src_record     += stats_last.set_shared_manifest
...
def test_dedup_dry_multipart():
    ...
    min_size=MULTIPART_SIZE
    # create files in range [MULTIPART_SIZE, 4*MULTIPART_SIZE] aligned on RADOS_OBJ_SIZE
    # create files in range [MULTIPART_SIZE, 5GB] aligned on RADOS_OBJ_SIZE
    gen_files_in_range(files, num_files, min_size, 5*1024*MB)
