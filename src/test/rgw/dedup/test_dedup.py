...
#-------------------------------------------------------------------------------
def verify_dedup_ratio(expcted_dedup_stats, dedup_ratio):
    s3_bytes_before = expcted_dedup_stats.size_before_dedup
    s3_dedup_bytes  = expcted_dedup_stats.duplicate_bytes
    s3_bytes_after  = s3_bytes_before - s3_dedup_bytes
    skipped_bytes   = (expcted_dedup_stats.skip_too_small_bytes +
                       expcted_dedup_stats.non_default_storage_class_objs_bytes)
    ...
    log.info("s3_dedup_bytes = %d", expcted_dedup_stats.duplicate_bytes);
...
def read_dedup_stats(dry_run):
    ...
    if dedup_work_was_completed:
        dedup_ratio=read_dedup_ratio(jstats['dedup ratio'])
    else:
        log.info("Uncompleted!")

    return (dedup_work_was_completed, dedup_stats, dedup_ratio)


#-------------------------------------------------------------------------------
def exec_dedup(expcted_dedup_stats, dry_run, verify_stats=True):
    log.info("sending exec_dedup request: dry_run=%d", dry_run)
    if dry_run:
        result = admin(['dedup', 'restart', '--dry-run'])
        reset_full_dedup_stats(expcted_dedup_stats)
    else:
    ...
    verify_dedup_ratio(expcted_dedup_stats, dedup_ratio)
...
#-------------------------------------------------------------------------------
def small_single_part_objs_dedup(out_dir, conn, bucket_name, dry_run):
...
#-------------------------------------------------------------------------------
def simple_dedup(out_dir, conn, files, bucket_name, run_cleanup_after, config, dry_run):
...
#-------------------------------------------------------------------------------
def simple_dedup_with_tenants(out_dir, files, conns, bucket_names, config, dry_run=False):
    indices=[0] * len(files)
...
#-------------------------------------------------------------------------------
def dedup_basic_with_tenants_common(out_dir, files, max_copies_count, config, dry_run):
    try:
        ret=gen_connections_multi(max_copies_count)
        tenants=ret[0]
        bucket_names=ret[1]
        conns=ret[2]
        simple_dedup_with_tenants(OUT_DIR, files, conns, bucket_names, config, dry_run)
...
def check_full_dedup_state():
    ...
    log.warning("check_full_dedup_state:: sending FULL Dedup request")
    result = admin(['dedup', 'restart'])
    if result[1] == 0:
        full_dedup_state_disabled = False
        log.warning("check_full_dedup_state:: sending Dedup ABORT request")
...
def full_dedup_is_disabled():
    ...
    if full_dedup_state_disabled:
        log.warning("Full Dedup is DISABLED, skipping test...")
...
def test_dedup_small():
    ...
    conn = connection()
    small_single_part_objs_dedup(OUT_DIR, conn, bucket_name, False)
...
def test_dedup_inc_0():
    ...
    try:
        ...
        gen_files_in_range(OUT_DIR, files, num_files, 1*MB, 64*MB)
        # upload objects, dedup, verify, but don't cleanup
        ret = simple_dedup(OUT_DIR, conn, files, bucket_name, False, config, False)
        expected_results = ret[0]
        dedup_stats = ret[1]
        s3_objects_total = ret[2]
...
def test_dedup_inc_with_remove_multi_tenants():
    ...
    try:
        ...
        exec_dedup(dedup_stats, False, False)
...
def test_dedup_multipart_with_tenants():
    ...
    dedup_basic_with_tenants_common(OUT_DIR, files, max_copies_count, default_config, False)
...
def test_dedup_multipart():
    ...
    simple_dedup(OUT_DIR, conn, files, bucket_name, True, default_config, False)
...
def test_dedup_small_multipart_with_tenants():
    ...
    dedup_basic_with_tenants_common(OUT_DIR, files, max_copies_count, config, False)
...
def test_dedup_small_multipart():
    ...
    # create files in range [4KB-512KB] aligned on 4KB
    gen_files_in_range(OUT_DIR, files, num_files, min_size, max_size, min_size)
    simple_dedup(OUT_DIR, conn, files, bucket_name, True, config2, False)
...
def test_dedup_dry_multipart():
    ...
    simple_dedup(OUT_DIR, conn, files, bucket_name, True, default_config, True)
...
def test_dedup_dry_small_multipart():
    ...
    # create files in range [4KB-512KB] aligned on 4KB
    gen_files_in_range(OUT_DIR, files, num_files, min_size, max_size, min_size)
    simple_dedup(OUT_DIR, conn, files, bucket_name, True, config2, True)
...
def test_dedup_dry_small_large_mix():
    ...
    try:
        ...
        num_large_files=2
        files=[]
        gen_files_fixed_size(OUT_DIR, files, num_small_files,
                             small_file_size, max_copies_count)
        gen_files_fixed_size(OUT_DIR, files, num_large_files,
                             large_file_size, max_copies_count)
        small_obj_count=0
        large_obj_count=0
        small_obj_total=0
        small_obj_dup=0
        ...
        for f in files:
            ...
            num_copies=f[2]
            if obj_size == 1*MB:
                small_obj_count += num_copies
                small_obj_total += obj_size*num_copies
                if (num_copies > 1):
                    small_obj_dup += obj_size*(num_copies-1)
            else:
                large_obj_count += num_copies
                large_obj_total += obj_size*num_copies
                if (num_copies > 1):
                    large_obj_dup += obj_size*(num_copies-1)

        total_size=small_obj_total+large_obj_total
        log.info("Total size=%d, small=%d, large=%d, large_obj_dup=%d",
                 total_size, small_obj_total, large_obj_total, large_obj_dup)
        log.info("small_obj_count=%d, large_obj_count=%d",
                 small_obj_count, large_obj_count);
        size_after=(total_size-large_obj_dup)
        if size_after:
            log.info("dedup ratio = %f", (float(total_size)/size_after))
        else:
            log.info("size after is zero!!!");

        bucket = conn.create_bucket(Bucket=bucket_name)
        indices = [0] * len(files)
        ret = upload_objects(OUT_DIR, bucket_name, files, indices,
                             conn, default_config)
        expected_results = ret[0]
        dedup_stats = ret[1]
        s3_objects_total = ret[2]
        ...
        if dry_run == False:
            ...
            verify_objects(OUT_DIR, bucket_name, files, conn,
                           expected_results, default_config)
...
def test_dedup_dry_basic_with_tenants():
    ...
    log.info("test_dedup_basic_with_tenants: connect to AWS ...")
    gen_files_fixed_size(OUT_DIR, files, num_files, file_size, max_copies_count)
    dedup_basic_with_tenants_common(OUT_DIR, files, max_copies_count, default_config, True)
...
def test_dedup_dry_small_multipart_with_tenants():
    ...
    # create files in range [4KB-512KB] aligned on 4KB
    gen_files_in_range(OUT_DIR, files, num_files, min_size, max_size, min_size)
    dedup_basic_with_tenants_common(OUT_DIR, files, max_copies_count, config, True)
