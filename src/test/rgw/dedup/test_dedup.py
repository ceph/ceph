...
def delete_dup_objects(bucket_name, files, conn):
    ...
    if verify:
        log.info("=================================================")
        log.info("delete_dup_objects: verify delete_list_total")
...
def verify_objects_multi(files, conns, bucket_names, expected_results, config, delete_dups):
    ...
    for f in files:
        ...
        for i in range(1, num_copies):
            ...
            ten_id = i % max_tenants
            conns[ten_id].download_file(bucket_names[ten_id], key, tempfile,
                                        Config=config)
            result = bash(['cmp', tempfile, OUT_DIR + filename])
            assert result[1] == 0 ,"Files %s and %s differ!!" % (key, tempfile)
            os.remove(tempfile)
...
def test_dedup_split_head_with_tenants():
    ...
    for idx in range(0, 9):
        log.info("test_dedup_split_head_with_tenants: loop #%d", idx);
...
def test_dedup_split_head():
    ...
    for idx in range(0, 9):
        log.info("test_dedup_split_head: loop #%d", idx);
