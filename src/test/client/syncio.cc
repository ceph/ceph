...
TEST_F(TestClient, SyncIOLlreadvLlwritevSizeOutOfBound) {
    /* sync/async i/o system calls can transfer at most 2147483647(INT_MAX)
    bytes. Therefore, test that sync'd read/write calls returned bytes do not
    exceed this limit.*/
    ...
    bytes_written = client->ll_writev(file.fh, iov_out_over_limit, 3, 0);
    ...
    bytes_read = client->ll_readv(file.fh, iov_in_over_limit, 3, 0);
    ASSERT_EQ(bytes_read, READ_WRITE_LIMIT);

    client->ll_release(file.fh);
    ASSERT_EQ(client->ll_unlink(root, file.filename, myperm), 0);
}
