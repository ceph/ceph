...
public:
    ...
    void ll_write_n_bytes(struct Fh *fh, size_t to_write, size_t block_size,
      ...
      for (int i = 0; i < iov_cnt; ++i) {
        std::string out_buf(DATA_PER_BLOCK, 'a' + i);
        iov_buf_out[i].iov_base = out_buf.data();
      ...
      while (to_write > 0) {
        if (to_write >= block_size) {
          writefinish = new C_SaferCond("writefinish-upto-blocksize");
          ...
          *offset += bytes_written;
          ...
          std::string small_buf(to_write, 'z');
          struct iovec iov_small_buf_out[1] = {
              {small_buf.data(), to_write}
          };
          writefinish = new C_SaferCond("writefinish-small-buffer");
