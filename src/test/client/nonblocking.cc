...
TEST_F(TestClient, LlreadvLlwritevZeroSizeBufferFsync) {
  ...
  // do a buffered write
  rc = client->ll_preadv_pwritev(file.fh, iov_out_buffered, 2, 0, true,
  ...
  rc = client->ll_preadv_pwritev(file.fh, iov_in_fsync_sync_all, 2, 0, false,
  ...
  // async-fsync read should fail while flushing the file inode
  ASSERT_EQ(bytes_read, -EINVAL);
  ...
  // do a buffered write
  rc = client->ll_preadv_pwritev(file.fh, iov_out_buffered_dataonly, 2, (BUFSIZE * 2) + 1,
  ...
  rc = client->ll_preadv_pwritev(file.fh, iov_in_dataonly_fysnc, 2, (BUFSIZE * 2) + 1,
