...
TEST_F(TestClient, LlreadvLlwritevOverlimit) {
  ...
  */
  ...
  FileHandleInfo file = client->get_file(root, fname, myperm,
                                         O_RDWR | O_CREAT | O_TRUNC);
  ...
  rc = client->ll_preadv_pwritev(file.fh, iov_out_overlimit, IOV_SEG_OVERLIMIT,
                                 0, true, &writefinish, nullptr);
  ASSERT_EQ(rc, 0);
  ssize_t bytes_written = writefinish.wait();
  ASSERT_EQ(bytes_written, bytes_to_write);

  rc = client->ll_preadv_pwritev(file.fh, iov_in_overlimit, IOV_SEG_OVERLIMIT,
  ...
  client->ll_release(file.fh);
  ASSERT_EQ(0, client->ll_unlink(root, file.filename, myperm));
}
...
TEST_F(TestClient, LlreadvLlwritevNonContiguous) {
  ...
  C_SaferCond *writefinish = nullptr;
  C_SaferCond *readfinish = nullptr;
  ...
  for(int i = 0; i < NUM_BUF; ++i) {
    writefinish = new C_SaferCond("test-nonblocking-writefinish-non-contiguous");
    rc = client->ll_preadv_pwritev(file.fh, current_iov++, 1, i * NUM_BUF * 10,
  ...
  for(int i = 0; i < NUM_BUF; ++i) {
    ...
    readfinish = new C_SaferCond("test-nonblocking-readfinish-non-contiguous");
    rc = client->ll_preadv_pwritev(file.fh, current_iov++, 1, i * NUM_BUF * 10,
...
TEST_F(TestClient, LlreadvLlwritevWriteOnlyFile) {
  ...
  rc = client->ll_preadv_pwritev(file.fh, iov_out, 2, 0, true, &writefinish,
                                 nullptr);
  ASSERT_EQ(rc, 0);
  ssize_t total_bytes_written = writefinish.wait();
  ASSERT_EQ(total_bytes_written, bytes_to_write);

  rc = client->ll_preadv_pwritev(file.fh, iov_in, 2, 0, false, &readfinish,
                                 &bl);
  ASSERT_EQ(rc, 0);
...
TEST_F(TestClient, LlreadvLlwritevFsync) {
  ...
  writefinish = new C_SaferCond("test-nonblocking-writefinish-fsync-sync-all");
  readfinish = new C_SaferCond("test-nonblocking-readfinish-fsync-sync-all");
  ...
  rc = client->ll_preadv_pwritev(file.fh, iov_out_fsync_sync_all, 2, 1000, true,
                                 writefinish, nullptr, true, false);
  ASSERT_EQ(rc, 0);
  bytes_written = writefinish->wait();
  ASSERT_EQ(bytes_written, bytes_to_write);

  rc = client->ll_preadv_pwritev(file.fh, iov_in_fsync_sync_all, 2, 1000, false,
                                 readfinish, &bl);
  ASSERT_EQ(rc, 0);
  bytes_read = readfinish->wait();
  ASSERT_EQ(bytes_read, bytes_to_write);
  ...
  writefinish = new C_SaferCond("test-nonblocking-writefinish-fsync-syncdataonly");
  readfinish = new C_SaferCond("test-nonblocking-readfinish-fsync-syndataonly");


  bytes_to_write = iov_out_dataonly_fysnc[0].iov_len
                   + iov_out_dataonly_fysnc[1].iov_len;

  rc = client->ll_preadv_pwritev(file.fh, iov_out_dataonly_fysnc, 2, 100,
                                 true, writefinish, nullptr, true, true);
  ASSERT_EQ(rc, 0);
  bytes_written = writefinish->wait();
  ASSERT_EQ(bytes_written, bytes_to_write);

  bl.clear();
  rc = client->ll_preadv_pwritev(file.fh, iov_in_dataonly_fysnc, 2, 100,
                                 false, readfinish, &bl);
  ASSERT_EQ(rc, 0);
  bytes_read = readfinish->wait();
  ASSERT_EQ(bytes_read, bytes_to_write);
...
TEST_F(TestClient, LlreadvLlwritevQuotaFull) {
  ...
  FileHandleInfo file_a = client->get_file(diri, filename, myperm,
                                           O_RDWR | O_CREAT | O_TRUNC);
  ...
  C_SaferCond writefinish;
  rc = client->ll_preadv_pwritev(file_a.fh, iov_out, 2, 0, true,
                                 &writefinish, nullptr);
  ASSERT_EQ(rc, 0);
  ...
  C_SaferCond readfinish;
  bufferlist bl;
  rc = client->ll_preadv_pwritev(file_a.fh, iov_in, 2, 0, false,
                                 &readfinish, &bl);
  ASSERT_EQ(rc, 0);
