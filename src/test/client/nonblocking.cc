...
TEST_F(TestClient, LlreadvLlwritevOverlimit) {
  ...
  */
  ...
  rc = client->ll_preadv_pwritev(fh, iov_out_overlimit, IOV_SEG_OVERLIMIT, 1
...
TEST_F(TestClient, LlreadvLlwritevNonContiguous) {
  ...
  for(int i = 0; i < NUM_BUF; ++i) {
    ...
    rc = client->ll_preadv_pwritev(fh, current_iov++, 1, i * NUM_BUF * 10, 1
...
TEST_F(TestClient, LlreadvLlwritevWriteOnlyFile) { 1
