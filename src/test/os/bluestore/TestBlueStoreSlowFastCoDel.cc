...
public:
  std::unique_ptr<CephContext> ceph_context;
  std::unique_ptr<BlueStoreSlowFastCoDelMock> slow_fast_codel;
  ...
  void SetUp() override {
    ceph_context = std::make_unique<CephContext>(CEPH_ENTITY_TYPE_ANY);
  }

  void create_bluestore_slow_fast_codel() {
    slow_fast_codel = std::make_unique<BlueStoreSlowFastCoDelMock>(
            ceph_context,
