    ...
    events(td->o.iodepth)
{
  ...
  const uint64_t file_size = td->o.size / min(1u, td->o.nr_files);
  ...
  int r = engine->os->apply_transaction(&sequencer, t);
  if (r)
...
/// completion context combined with the Transaction for a single allocation
class TxComplete : public ObjectStore::Transaction, public UnitComplete {
 public:
  using UnitComplete::UnitComplete; // (io_u* u)
};
...
int fio_ceph_os_queue(thread_data* td, io_u* u)
{
  ...
  if (u->ddir == DDIR_WRITE) {
    ...
    auto t = new TxComplete{u};
    t->write(coll.cid, object.oid, u->offset, u->xfer_buflen, bl);
    os->queue_transaction(&coll.sequencer, t, t);
    return FIO_Q_QUEUED;
  }
