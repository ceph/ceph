    ...
    return replay_free_dump_and_apply(argv[1],
      [&](Allocator *a, const string &aname) {
        ...
        for(int i = 0; i < num_buckets; i++) {
          auto s = hist[i].get_min(i, num_buckets);
          auto e = hist[i].get_max(i, num_buckets);
          std::cout << "(" << s << ".." << e << "]"
                    << " -> " << hist[i].extent_count << " extents, "
                    << hist[i].aligned_extent_count << " aligned extents, "
                    << hist[i].usable_bytes << " usable bytes of total "
                    << hist[i].total_bytes
                    << std::endl;
        }
	return 0;
