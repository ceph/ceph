...
TEST_P(StoreTestSpecificAUSize, SpilloverFixedPartialTest) {
  ...
  // File size below is higher than in Reef+ releases
  // due to the fact that RocksDB settings differ.
  // Due to that volume selector permits ~1.6GB extra vs. 0.87GB
  // in later releases. Hence we write 2GB file instead of 1GB.
  // To be reduced if RocksDB settings are backported.
  do_bluefs_write(fs, "db.slow", "file2", 1ull << 31); // 2 GB
