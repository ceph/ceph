...
TEST_P(AllocTest, test_claim_range_zero_length)
{
  ...
  init_alloc(capacity, block_size);
  if (!alloc->supports_claim_range()) {
    GTEST_SKIP() << "claim_range() not implemented for this allocator";
  }
  alloc->init_add_free(0, capacity);
...
TEST_P(AllocTest, test_claim_range_geometry)
{
  ...
  // cheap probe instance, just to decide whether to skip this whole test;
  // every row below creates its own instance via init_alloc()
  init_alloc(32 * block_size, block_size);
  if (!alloc->supports_claim_range()) {
    GTEST_SKIP() << "claim_range() not implemented for this allocator";
  }
