...
class BlueFSTest : public ::testing::Test {
 protected:
  std::unique_ptr<BlueFS> fs;
  string fn;
  uint64_t size = 1048576 * 128;

  void SetUp() override {
    fn = get_temp_bdev(size);
    fs.reset(new BlueFS(g_ceph_context));
...
TEST_F(BlueFSTest, small_appends) {
  {
    BlueFS::FileWriter *h;
    ASSERT_EQ(0, fs->mkdir("dir"));
    ASSERT_EQ(0, fs->open_for_write("dir", "file", &h, false));
...
{
  std::for_each(begin(v), end(v), [](auto& t){ t.join(); });
}
...
TEST_F(BlueFSTest, test_flush_1) {
  {
...
TEST_F(BlueFSTest, test_flush_2) {
  {
    uint64_t effective_size = size - (32 * 1048576); // leaving the last 32 MB for log compaction
...
TEST_F(BlueFSTest, test_flush_3) {
  {
...
TEST_F(BlueFSTest, test_simple_compaction_sync) {
    ...
    "true");
  {
    for (int i=0; i<10; i++) {
	  ...
	  file.append(to_string(j));
          ...
          ASSERT_EQ(0, fs->open_for_write(dir, file, &h, false));
          ASSERT_NE(nullptr, h);
          auto sg = make_scope_guard([this, h] { this->fs->close_writer(h); });
          bufferlist bl;
...
TEST_F(BlueFSTest, test_simple_compaction_async) {
  g_ceph_context->_conf.set_val(
    "bluefs_compact_log_sync",
    "false");
  {
...
TEST_F(BlueFSTest, test_compaction_sync) {
  g_ceph_context->_conf.set_val(
    "bluefs_compact_log_sync",
    "true");
  {
...
TEST_F(BlueFSTest, test_compaction_async) {
  g_ceph_context->_conf.set_val(
    "bluefs_compact_log_sync",
    "false");
  {
