                                      ...
                                      coll_entry_t *entry, C_StatState *stat)
{
  ...
  dout(2) << __func__ << " coll " << entry->m_coll << " "
      << coll_t() << " /" << log_obj << " (" << bl.length() << ")" << dendl;

  if (m_do_stats && (stat != NULL))
      stat->written_data += bl.length();

  uint64_t s = pg_log_size[entry->m_coll];
  t->write(coll_t(), log_obj, s, bl.length(), bl);
...
{  
  ...
  t->remove(coll_t(), entry->m_meta_obj);
}
                                         ...
                                         C_StatState *stat)
{
  ...
  dout(2) << __func__ << " meta " << coll_t() << "/" << entry->m_meta_obj << dendl;
  t->touch(coll_t(), entry->m_meta_obj);
  return entry;
}
