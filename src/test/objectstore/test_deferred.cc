...
class TweakedAllocator : public AvlAllocator {
  public:
  TweakedAllocator(
    CephContext* _cct,
    int64_t _capacity,
    ...
    : AvlAllocator( _cct, _capacity, _block_size, name) {
      }
  ~TweakedAllocator() override {}
  ...
  int64_t allocate(
    uint64_t want_size, uint64_t block_size,
    uint64_t max_alloc_size, int64_t hint,
    PExtentVector *extents) override {
      called_allocate = true;
      int r = AvlAllocator::allocate(want_size, block_size, max_alloc_size, hint, extents);
      return r;
    };
...
void create_deferred_and_terminate() {
  ...
  {
    string cmd = string("rm -rf bluestore.test_temp_dir");
...
void mount_check_alloc()
{
  ...
  bool called_allocate = false;
  bs->debug_set_deferred_replay_start(
    [&](){
      std::cout << "action before deferred replay" << std::endl;
      Allocator* alloc = bs->debug_get_alloc();
      TweakedAllocator* ta = dynamic_cast<TweakedAllocator*>(alloc);
      ceph_assert(ta);
      ta->called_allocate = false;
      std::cout << "sleeping to give compaction a chance" << std::endl;
      sleep(10);
      std::cout << "sleep end" << std::endl;
    });
  bs->debug_set_deferred_replay_end(
    [&](){
      std::cout << "action after deferred replay" << std::endl;
      Allocator* alloc = bs->debug_get_alloc();
      TweakedAllocator* ta = dynamic_cast<TweakedAllocator*>(alloc);
      ceph_assert(ta);
      std::cout << "called_allocate=" << ta->called_allocate << std::endl;
      called_allocate = ta->called_allocate;
      bs->debug_set_deferred_replay_track(nullptr);
      bs->debug_set_deferred_replay_start(nullptr);
      bs->debug_set_deferred_replay_end(nullptr);
    });

  interval_set<uint64_t> not_onode_allocations;
  bs->debug_set_init_alloc_done(
    [&](){
      Allocator* alloc = bs->debug_get_alloc();
      alloc->foreach(
      ...
      bs->debug_set_init_alloc_done(nullptr);
    });
  interval_set<uint64_t> extents_sum;
  bs->debug_set_deferred_replay_track(
...
int main(int _argc, char **_argv) {
  ...
  if (first_test == 0) {
    ...
    if (child == 0) {
      auto cct = setup_env();
      g_ceph_context->_conf->bluestore_allocator = "tweaked";
      g_ceph_context->_conf->bluestore_rocksdb_options +=
          ",level0_file_num_compaction_trigger=4";
      create_deferred_and_terminate();
      ceph_assert(false && "should exit() earlier");
