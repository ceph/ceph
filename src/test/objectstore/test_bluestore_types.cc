...
TEST(ExtentMap, reshard_failure) {
    ...
    };
  {
    ...
    em.reshard(nullptr, nullptr,
               1024 /*segment size, doesn't matter*/,
      [&](uint32_t needs_reshard_start,
          uint32_t needs_reshard_end,
          unsigned, unsigned,
          uint32_t segment_size,
          std::vector<bluestore_onode_t::shard_info>& new_shard_info) -> std::tuple<uint32_t, uint32_t> {
            new_shard_info.emplace_back(0, 0);
            // shard before 3rd blob in the first extent
            new_shard_info.emplace_back(l1 + l2, 0);
            // and shard before 3rd blob in the second extent
            new_shard_info.emplace_back(blob_len + l1 + l2, 0);
            return {needs_reshard_start, needs_reshard_end};
          });
