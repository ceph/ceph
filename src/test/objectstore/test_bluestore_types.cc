...
//
// merge_blob() relocates csum data in whole csum chunk units, so every
// pextent it moves has to start and end on a csum chunk boundary.  A blob
// whose csum chunk is larger than min_alloc_size - which happens when the
// object carries a SEQUENTIAL_READ + IMMUTABLE/APPEND_ONLY alloc hint, see
// _choose_write_options() - does not necessarily satisfy that: _do_alloc_write()
// copies the allocator's min_alloc_size granular PExtentVector into the blob
// verbatim, so a fragmented allocation splits the blob mid csum chunk.
// can_merge_blob() used to accept such a blob, and merge_blob() asserted on
//   (len % (1 << csum_chunk_order)) == 0
TEST_F(ExtentMapFixture, merge_blob_csum_chunk_unaligned) {
  ...
  // Emulates a write of 'csum_chunk' bytes at blob offset 'b_off' of a blob
  // that starts at logical offset 'blob_start'.  'fragments' is how the
  // allocator handed the space out; they have to add up to csum_chunk.
