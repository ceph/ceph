...
TEST_F(CrushWrapperTest, weight_shift_default) {
  CrushWrapper c;
  c.create();
  // 1.0 == 1 TiB by default, as for every existing map
...
TEST_F(CrushWrapperTest, weight_shift_scales) {
  ...
  // too small for the coarser unit rounds to zero
...
TEST_F(CrushWrapperTest, weight_shift_preserves_placement) {
  ...
  // a weight that does not divide evenly has to round, which should barely
  // move anything
...
TEST_F(CrushWrapperTest, weight_shift_encode_decode) {
  ...
  // a peer without the feature gets no shift, but the same raw weights
  bufferlist old;
