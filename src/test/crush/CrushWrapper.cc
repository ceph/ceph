...
TEST_F(CrushWrapperTest, get_immediate_parent) {
...

  {
    ...
    EXPECT_EQ(0, c->insert_item(cct, item, 1.0, 0, 0,
				"osd.0", loc));
  }
...
TEST_F(CrushWrapperTest, swap_bucket) {
  ...
  int a, b;
  EXPECT_EQ(0, c->add_bucket(0, CRUSH_BUCKET_STRAW2, CRUSH_HASH_RJENKINS1,
			     HOST_TYPE, 0, NULL, NULL, NULL, NULL, &a));
  EXPECT_EQ(0, c->set_item_name(a, "a"));
  EXPECT_EQ(0, c->add_bucket(0, CRUSH_BUCKET_STRAW2, CRUSH_HASH_RJENKINS1,
			     HOST_TYPE, 0, NULL, NULL, NULL, NULL, &b));
  ...
  }
  {
    ...
    loc["root"] = "root";
    loc["host"] = "a";
    EXPECT_EQ(0, c->insert_item(cct, 0, 1.0, 0, 0, "osd.0", loc));
    EXPECT_EQ(0, c->insert_item(cct, 1, 1.0, 0, 0, "osd.1", loc));
    EXPECT_EQ(0, c->insert_item(cct, 2, 1.0, 0, 0, "osd.2", loc));
  }
  {
    map<string,string> loc;
    loc["host"] = "b";
    EXPECT_EQ(0, c->insert_item(cct, 3, 1.0, 0, 0, "osd.3", loc));
  }
...
TEST_F(CrushWrapperTest, check_item_loc) {
  ...
  // fail if loc is empty
  {
    float weight;
    map<string,string> loc;
    EXPECT_FALSE(c->check_item_loc(cct, item, loc, &weight, NULL, NULL));
  }
  ...
  const string OSD("osd.0");
  {
    map<string,string> loc;
    loc["root"] = "default";
    EXPECT_EQ(0, c->insert_item(cct, item, expected_weight, 0, 0,
				OSD, loc));
  }
  ...
  // succeed and retrieves the expected weight
  {
    ...
    map<string,string> loc;
    loc["root"] = "default";
    EXPECT_TRUE(c->check_item_loc(cct, item, loc, &weight, NULL, NULL));
...
TEST_F(CrushWrapperTest, update_item) {
  ...
  // fail if invalid names anywhere in loc
  {
    map<string,string> loc;
    loc["rack"] = "\001";
    EXPECT_EQ(-EINVAL, c->update_item(cct, item, 1.0, 0, 0,
  ...
  EXPECT_EQ(0, c->insert_item(cct, item, original_weight, 0, 0,
			      OSD0, loc));
  ...
  EXPECT_TRUE(c->check_item_loc(cct, item, loc, &weight, NULL, NULL));
  EXPECT_EQ(0, c->update_item(cct, item, original_weight, 0, 0,
			      OSD0, loc));
  EXPECT_EQ(OSD0, c->get_item_name(item));
  EXPECT_EQ(original_weight, c->get_item_weightf(item));
  EXPECT_TRUE(c->check_item_loc(cct, item, loc, &weight, NULL, NULL));
  ...
  EXPECT_EQ(original_weight, c->get_item_weightf(item));
  EXPECT_TRUE(c->check_item_loc(cct, item, loc, &weight, NULL, NULL));
  EXPECT_EQ(1, c->update_item(cct, item, modified_weight, 0, 0,
			      OSD1, loc));
  ...
  EXPECT_EQ(original_weight, c->get_item_weightf(item));
  EXPECT_TRUE(c->check_item_loc(cct, item, loc, &weight, NULL, NULL));
  EXPECT_FALSE(c->check_item_loc(cct, item, other_loc, &weight, NULL, NULL));
  EXPECT_EQ(1, c->update_item(cct, item, modified_weight, 0, 0,
			      OSD1, other_loc));
  EXPECT_EQ(OSD1, c->get_item_name(item));
  EXPECT_EQ(modified_weight, c->get_item_weightf(item));
  EXPECT_FALSE(c->check_item_loc(cct, item, loc, &weight, NULL, NULL));
  EXPECT_TRUE(c->check_item_loc(cct, item, other_loc, &weight, NULL, NULL));
}
...
TEST_F(CrushWrapperTest, adjust_item_weight) {
...

  {
    ...
    bloc["root"] = "default";
    EXPECT_EQ(0, c->insert_item(cct, host0, host_weight, 0, 0,
				HOST0, bloc));
  }

  {
    ...
    map<string,string> bloc;
    bloc["root"] = "default";
    EXPECT_EQ(0, c->insert_item(cct, hostfake, host_weight, 0, 0,
				FAKE, bloc));
  }
...
TEST_F(CrushWrapperTest, insert_item) {
  ...
  // invalid names anywhere in loc trigger an error
  {
    map<string,string> loc;
    loc["host"] = "\001";
    EXPECT_EQ(-EINVAL, c->insert_item(cct, item, 1.0, 0, 0,
				      "osd." + stringify(item), loc));
  }
  ...
  // insert an item in an existing bucket
  {
    ...
    EXPECT_EQ(-EEXIST, c->insert_item(cct, another_item, 1.0, 0, 0,
				      "osd." + stringify(item), loc));
  }
  ...
  //
  {
    ...
    EXPECT_EQ(-ELOOP, c->insert_item(cct, rootno, 1.0, 0, 0,
				     "default", loc));
  }
...
TEST_F(CrushWrapperTest, remove_item) {
  ...
  const int num_osd = 12;
  {
    ...
    for (int item = 0; item < num_osd; item++) {
      ASSERT_EQ(0, c->insert_item(cct, item, 1.0, 0, 0,
  ...
  float weight;
  EXPECT_FALSE(c->check_item_loc(cct, item_to_remove, loc, &weight, NULL, NULL));
}
...
TEST_F(CrushWrapperTest, distance) {
  ...
  loc["rack"] = "a";
  loc["root"] = "default";
  c.insert_item(cct, 0, 1, 0, 0, "osd.0", loc);

  loc.clear();
  loc["host"] = "a2";
  loc["rack"] = "a";
  loc["root"] = "default";
  c.insert_item(cct, 1, 1, 0, 0, "osd.1", loc);

  loc.clear();
  loc["host"] = "b1";
  loc["rack"] = "b";
  loc["root"] = "default";
  c.insert_item(cct, 2, 1, 0, 0, "osd.2", loc);

  loc.clear();
  loc["host"] = "b2";
  loc["rack"] = "b";
  loc["root"] = "default";
  c.insert_item(cct, 3, 1, 0, 0, "osd.3", loc);
...
TEST_F(CrushWrapperTest, choose_args_compat) {
  ...
  loc["root"] = "default";
  int item = 1;
  c.insert_item(cct, item, weight, 0, 0, "osd.1", loc);

  loc["host"] = "b2";
  loc["rack"] = "r12";
  loc["root"] = "default";
  item = 2;
  c.insert_item(cct, item, weight, 0, 0, "osd.2", loc);
...
TEST_F(CrushWrapperTest, device_class_clone) {
  ...
  int item_no_class = 2;
  c.insert_item(cct, item_no_class, weight, 0, 0, "osd.2", loc);
...
TEST_F(CrushWrapperTest, try_remap_rule) {
  ...
  loc["root"] = "default";
  c.insert_item(cct, 3, 1, 0, 0, "osd.3", loc);
  c.insert_item(cct, 4, 1, 0, 0, "osd.4", loc);
  c.insert_item(cct, 5, 1, 0, 0, "osd.5", loc);

  loc.clear();
  loc["host"] = "baz";
  loc["rack"] = "b";
  loc["root"] = "default";
  c.insert_item(cct, 6, 1, 0, 0, "osd.6", loc);
  c.insert_item(cct, 7, 1, 0, 0, "osd.7", loc);
  c.insert_item(cct, 8, 1, 0, 0, "osd.8", loc);

  loc.clear();
  loc["host"] = "qux";
  loc["rack"] = "b";
  loc["root"] = "default";
  c.insert_item(cct, 9, 1, 0, 0, "osd.9", loc);
  c.insert_item(cct, 10, 1, 0, 0, "osd.10", loc);
  c.insert_item(cct, 11, 1, 0, 0, "osd.11", loc);
  c.finalize();
  ...
  loc["rack"] = "c";
  loc["root"] = "default";
  c.insert_item(cct, 12, 1, 0, 0, "osd.12", loc);
  c.insert_item(cct, 13, 1, 0, 0, "osd.13", loc);
  c.insert_item(cct, 14, 1, 0, 0, "osd.14", loc);
  c.finalize();
  ...
  loc["rack"] = "c";
  loc["root"] = "default";
  c.insert_item(cct, 15, 1, 0, 0, "osd.15", loc);
  c.insert_item(cct, 16, 1, 0, 0, "osd.16", loc);
  c.insert_item(cct, 17, 1, 0, 0, "osd.17", loc);
  c.finalize();
