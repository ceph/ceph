...
public:
  journal::ObjectPlayer::ref create_object(const std::string &oid,
                                         uint8_t order) {
    auto object = journal::ObjectPlayer::create(
      m_ioctx, oid + ".", 0, *m_timer, m_timer_lock, order,
      T::max_fetch_bytes);
    return object;
  }

  int fetch(journal::ObjectPlayer::ref object_player) {
    while (true) {
      C_SaferCond ctx;
  ...
  int watch_and_wait_for_entries(journal::ObjectPlayer::ref object_player,
