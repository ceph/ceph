...
public:
  struct FlushHandler : public journal::FutureImpl::FlushHandler {
    uint64_t flushes = 0;
    void flush(const journal::FutureImpl::ref& future) override {
      ++flushes;
    }
  ...
  journal::FutureImpl::ref create_future(uint64_t tag_tid, uint64_t entry_tid,
                                       uint64_t commit_tid,
                                       journal::FutureImpl::ref prev = nullptr) {
    auto future = journal::FutureImpl::create(tag_tid, entry_tid, commit_tid);
    future->init(prev);
    return future;
  }

  void flush(const journal::FutureImpl::ref& future) {
  }
