...
TEST(test_fast_containers, vector_context) {
   for (int i = 0; i < 10; ++i) {
      vector<int> a;
      fast_vector<int,4> b,c;
      eq_elements(a,b);
...
TEST(test_fast_containers, set_context) {
   for (int i = 0; i < 10; ++i) {
      set<int> a;
      fast_set<int,4> b;
      do_insert(a,b,i,i);
 ...
 *   make unittest_fast_containers &&
 *   valgrind --tool=memcheck ./unittest_fast_containers --gtest_filter=*.*"
 * End:
 */
