...
TEST(PerfCounters, SinglePerfCounters) {
  ...
  oss << "{\"test_perfcounter_1\":{\"element1\":0,"
            "\"element2\":0.000000000,\"element3\":{\"avgcount\":0,\"sum\":0.000000000}}}" << '\n';
  ASSERT_EQ("", client.do_request("{ \"prefix\": \"perf dump\", \"format\": \"json\" }", &msg));
  ASSERT_EQ(sd(oss.str().c_str()), msg);
  ...
  oss1 << "{\"test_perfcounter_1\":{\"element1\":1,"
            "\"element2\":0.500000000,\"element3\":{\"avgcount\":1,\"sum\":100.000000000}}}" << '\n';
  ASSERT_EQ("", client.do_request("{ \"prefix\": \"perf dump\", \"format\": \"json\" }", &msg));
  ASSERT_EQ(sd(oss1.str().c_str()), msg);
  fake_pf->tinc(TEST_PERFCOUNTERS1_ELEMENT_3, utime_t());
  fake_pf->tinc(TEST_PERFCOUNTERS1_ELEMENT_3, utime_t(25,0));
  oss2 << "{\"test_perfcounter_1\":{\"element1\":1,\"element2\":0.500000000,"
            "\"element3\":{\"avgcount\":3,\"sum\":125.000000000}}}" << '\n';
  ASSERT_EQ("", client.do_request("{ \"prefix\": \"perf dump\", \"format\": \"json\" }", &msg));
  ASSERT_EQ(sd(oss2.str().c_str()), msg);

  fake_pf->reset();
  msg.clear();
  oss3 << "{\"test_perfcounter_1\":{\"element1\":1,"
            "\"element2\":0.000000000,\"element3\":{\"avgcount\":0,\"sum\":0.000000000}}}" << '\n';
  ASSERT_EQ("", client.do_request("{ \"prefix\": \"perf dump\", \"format\": \"json\" }", &msg));
  ASSERT_EQ(sd(oss3.str().c_str()), msg);
...
TEST(PerfCounters, MultiplePerfCounters) {
  ...
  std::ostringstream oss,oss1,oss2,oss3,oss4,oss5,oss6,oss7;
  ASSERT_EQ("", client.do_request("{ \"prefix\": \"perf dump\", \"format\": \"json\" }", &msg));
  oss << "{\"test_perfcounter_1\":{\"element1\":0,\"element2\":0.000000000,\"element3\":"
            "{\"avgcount\":0,\"sum\":0.000000000}},\"test_perfcounter_2\":{\"foo\":0,\"bar\":0.000000000}}" << '\n';
  ASSERT_EQ(sd(oss.str().c_str()), msg);

  fake_pf1->inc(TEST_PERFCOUNTERS1_ELEMENT_1);
  fake_pf1->inc(TEST_PERFCOUNTERS1_ELEMENT_1, 5);
  ASSERT_EQ("", client.do_request("{ \"prefix\": \"perf dump\", \"format\": \"json\" }", &msg));
  oss1 << "{\"test_perfcounter_1\":{\"element1\":6,\"element2\":0.000000000,\"element3\":"
            "{\"avgcount\":0,\"sum\":0.000000000}},\"test_perfcounter_2\":{\"foo\":0,\"bar\":0.000000000}}" << '\n';
  ASSERT_EQ(sd(oss1.str().c_str()), msg);

  coll->reset(string("test_perfcounter_1"));
  ASSERT_EQ("", client.do_request("{ \"prefix\": \"perf dump\", \"format\": \"json\" }", &msg));
  oss2 << "{\"test_perfcounter_1\":{\"element1\":6,\"element2\":0.000000000,\"element3\":"
            "{\"avgcount\":0,\"sum\":0.000000000}},\"test_perfcounter_2\":{\"foo\":0,\"bar\":0.000000000}}" << '\n';
  ASSERT_EQ(sd(oss2.str().c_str()), msg);
  ...
  ASSERT_EQ("", client.do_request("{ \"prefix\": \"perf dump\", \"format\": \"json\" }", &msg));
  oss3 << "{\"test_perfcounter_1\":{\"element1\":13,\"element2\":0.000000000,\"element3\":"
            "{\"avgcount\":0,\"sum\":0.000000000}},\"test_perfcounter_2\":{\"foo\":0,\"bar\":0.000000000}}" << '\n';
  ASSERT_EQ(sd(oss3.str().c_str()), msg);
  ...
  msg.clear();
  ASSERT_EQ("", client.do_request("{ \"prefix\": \"perf dump\", \"format\": \"json\" }", &msg));
  oss4 << "{\"test_perfcounter_1\":{\"element1\":13,\"element2\":0.000000000,\"element3\":"
            "{\"avgcount\":0,\"sum\":0.000000000}},\"test_perfcounter_2\":{\"foo\":0,\"bar\":0.000000000}}" << '\n';
  ASSERT_EQ(sd(oss4.str().c_str()), msg);

  coll->remove(fake_pf2);
  ASSERT_EQ("", client.do_request("{ \"prefix\": \"perf dump\", \"format\": \"json\" }", &msg));
  oss5 << "{\"test_perfcounter_1\":{\"element1\":13,\"element2\":0.000000000,"
            "\"element3\":{\"avgcount\":0,\"sum\":0.000000000}}}" << '\n';
  ASSERT_EQ(sd(oss5.str().c_str()), msg);
  ASSERT_EQ("", client.do_request("{ \"prefix\": \"perf schema\", \"format\": \"json\" }", &msg));
  oss6 << "{\"test_perfcounter_1\":{\"element1\":{\"type\":2},"
               "\"element2\":{\"type\":1},\"element3\":{\"type\":5}}}" << '\n';
  ASSERT_EQ(sd(oss6.str().c_str()), msg);

  coll->clear();
  ASSERT_EQ("", client.do_request("{ \"prefix\": \"perf dump\", \"format\": \"json\" }", &msg));
  oss7 << "{}" << '\n';
  ASSERT_EQ(sd(oss7.str().c_str()), msg);
}
