...
TEST(filepath, encode_decode) {
  ...
  filepath fp_snap("//a/b");
  ASSERT_EQ(fp_snap.get_path(), "//a/b"sv);
  ...
  // This validates that the snappath property is preserved across serialization.
  // NOTE: The current implementation may fail this test as _is_snappath is not
  // explicitly encoded and cannot be recovered from the encoded path string.
...
TEST(filepath, terminating_slash) {
  ...
  filepath fp1("a/b/");
  ASSERT_EQ(fp1.get_path(), "a/b/."sv);

  // A single slash is just the root path.
  filepath fp2("/");
  ASSERT_EQ(fp2.get_path(), ""sv);
  ASSERT_TRUE(fp2.absolute());
  ASSERT_EQ(fp2.get_ino(), 1);

  // Multiple leading slashes indicate a snappath, with an empty path.
  filepath fp3("//");
  ASSERT_EQ(fp3.get_path(), "//"sv);
  ASSERT_FALSE(fp3.absolute());
  ASSERT_TRUE(fp3.is_last_snap());
  ASSERT_EQ(fp3.get_ino(), 0);

  // Duplicate slashes in the middle are chomped, trailing slash is still significant.
  filepath fp4("a//b/");
  ASSERT_EQ(fp4.get_path(), "a/b/."sv);
}
