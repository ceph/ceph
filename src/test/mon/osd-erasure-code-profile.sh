...
function TEST_set() {
    ...
    ceph osd erasure-code-profile set $profile \
        key=value plugin=ec_example || return 1
    ceph osd erasure-code-profile get $profile | \
        grep -e key=value -e plugin=ec_example || return 1
    #
...
function TEST_format_plain() {
    ...
    expected='"plugin":"ec_example"'
    run_mon $dir a \
        --osd_pool_default_erasure-code-profile "plugin=ec_example" || return 1
    ceph --format json osd erasure-code-profile get default | \
        grep "$expected" || return 1
}
