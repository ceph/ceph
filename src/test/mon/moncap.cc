...
TEST(MonCap, ProfileBootstrapRGW) {
  ...
  struct {
    ...
    // the caps rgw keys were created with before the profile
    { "client.rgw.foo", "allow rw",    "allow rwx",   true },
    // the two caps can be migrated one at a time
    { "client.rgw.foo", "allow rw",    "profile rgw", true },
    { "client.rgw.foo", "profile rgw", "allow rwx",   true },
    { "client.rgw.foo", "allow *",     "profile rgw", false },
    { "client.rgw.foo", "profile rgw", "allow *",     false },
