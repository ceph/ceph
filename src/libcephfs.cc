...
extern "C" int ceph_do_snap_metadata_op(struct ceph_mount_info* cmount,
  const char* path, const char* md_key, const char* md_val,  int op_flag)
{
  if (!cmount->is_mounted())
    return -ENOTCONN;

  return cmount->get_client()->do_snap_metadata_op(path, md_key, md_val, op_flag,
                                                   cmount->default_perms);
}
