...
SeaStore::mount_ertr::future<> SeaStore::mount()
{
  ...
  co_await device->mount().handle_error(
    crimson::ct_error::assert_all{"Invalid error in SeaStore::mount"}
  );
...
SeaStore::base_ertr::future<> SeaStore::Shard::umount()
{
...
SeaStore::seastore_mkfs_ertr::future<>
SeaStore::Shard::mkfs_managers()
{
  ...
  co_await repeat_eagain([&]() -> SeaStore::seastore_mkfs_ertr_again::future<> {
    co_await with_trans_intr(*t, [&](auto &tr) -> SeaStore::mkfs_iertr::future<> {
