...
seastar::future<> SeaStore::report_stats()
{
  ...
  return shard_stores.invoke_on_all([this](const Shard &local_store) {
    ...
    cache_size_stats_t queue_sizes_ps = cache_total.queue_sizes;
    queue_sizes_ps.divide_by(seastar::smp::count);
    cache_io_stats_t queue_io_ps = cache_total.queue_io;
    queue_io_ps.divide_by(seastar::smp::count);
    INFO("cache lru: total{} {}; per-shard: total{} {}",
         cache_total.queue_sizes,
         cache_io_stats_printer_t{seconds, cache_total.queue_io},
         queue_sizes_ps,
