  ...
  uint32_t flags)
{
  ...
  DEBUGT("onode={}, expected_object_size={}, expected_write_size={}, flags={}",
	 *ctx.transaction, *onode, expected_object_size,
	 expected_write_size, flags);
  {
    ceph_assert(expected_object_size <= max_object_size);
    ceph_assert(expected_write_size <= max_object_size);
    auto &object_size = onode->get_mutable_layout(*ctx.transaction).size;
  ...
  return seastar::do_with(
	...
	static_cast<extent_len_t>(expected_object_size),
	static_cast<extent_len_t>(expected_write_size),
	flags);
    });
}
