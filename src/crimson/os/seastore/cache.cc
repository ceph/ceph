...
void Cache::register_metrics(unsigned int shard_index)
{
  ...
  for (auto& [src, src_label] : labels_by_src) {
    src_label.push_back(sm::label_instance("shard_store_index", std::to_string(shard_index)));
  }
  for (auto& [ext, ext_label] : labels_by_ext) {
    ext_label.push_back(sm::label_instance("shard_store_index", std::to_string(shard_index)));
  }
  /*
   * trans_created
   */
  ...
  auto register_tree_metrics = [&labels_by_src, &onode_label, &omap_label, shard_index, this](
    ...
    metrics.add_group(
      "cache",
      {
        sm::make_counter(
          ...
          {tree_label, sm::label_instance("shard_store_index", std::to_string(shard_index))}
        ),
	sm::make_counter(
	  "tree_extents_num",
	  tree_extents_num,
