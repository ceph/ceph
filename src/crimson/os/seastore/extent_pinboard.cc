...
public:
  ...
  void add(laddr_t laddr, extent_len_t length, extent_len_t access_end) {
    assert(laddr != L_ADDR_NULL);
    assert(!index.contains(laddr));
    assert(access_end != 0);
    index[laddr] = queue.emplace(queue.end(), laddr, length, access_end);
    current_size += length;
    trim_to(capacity);
  }
...
private:
  struct entry_t {
    entry_t(laddr_t laddr, extent_len_t length, extent_len_t access_end)
	: laddr(laddr), length(length), last_access_end(access_end) {}
