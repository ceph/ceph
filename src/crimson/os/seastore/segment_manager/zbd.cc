...
Device& ZBDSegmentManager::get_sharded_device(unsigned int shard_index)
{
...
ZBDSegmentManager::mount_ret ZBDSegmentManager::shard_mount()
{
  return open_device(
    ...
    if(seastar::this_shard_id() + seastar::smp::count * shard_index >= meta.shard_num) {
    ...
    shard_info = meta.shard_infos[seastar::this_shard_id() + seastar::smp::count * shard_index];
    metadata = meta;
    return mount_ertr::now();
  });
}
