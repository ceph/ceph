  ...
  const std::string &key, const ceph::bufferlist &value) 
{
  ...
  co_return co_await omap_set_keys(root, onode, t, std::move(kvs));
}
  ...
  LogNodeRef prev)
{
  ...
  auto l = (*last).empty() ? std::nullopt : last;
  std::map<std::string, bufferlist> kvs;
...
LogManager::omap_clear(omap_root_t &log_root, Onode &onode, Transaction &t)
{
  LOG_PREFIX(LogManager::omap_clear);
  DEBUGT("enter", t);
  auto omap_root = onode.get_root(omap_type_t::OMAP).get(
        onode.get_metadata_hint(tm.get_block_size()));
  co_await remove_kvs(t, log_root.addr,
