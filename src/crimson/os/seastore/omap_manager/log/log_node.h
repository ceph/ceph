...
public:
  ...
  std::optional<delta_t> get_last_added_delta() {
    delta_t ret;
    for (auto &i: buffer) {
      if (i.op == delta_t::op_t::APPEND ||
	  i.op == delta_t::op_t::OVERWRITE) {
	ret = i;
      }
    }
    return ret;
...
struct d_bitmap_t {
  ...
  void init() {
    set_bitmap_range(0, BITMAP_ARRAY_SIZE * BITS_PER_WORD);
  }
...
class LogKVNodeLayout {
  ...
  using L = absl::container_internal::Layout<ceph_le32, laddr_le_t, ceph_le32, d_bitmap_le_t, log_key_le_t>;
  static constexpr L layout{1, 1, 1, 1, 1};
public:
...
public:
  ...
  uint32_t capacity() const {
    return len
      - (reinterpret_cast<char*>(layout.template Pointer<4>(buf))
...
struct LogNode 
    ...
    std::optional<std::string> end)
  {
    ...
    uint32_t index = 0, s_index = NPOS, e_index = NPOS;
    std::string last;
    while(iter != iter_end()) {
      auto key = iter->get_key();
      if (key >= s && s_index == NPOS) {
	s_index = index;
      }
      if (key <= e) {
	e_index = index;
    ...
    if (s_index == NPOS || e_index == NPOS || s_index >= e_index) {
      return std::nullopt;
    }
    set_cur_bitmap(s_index, e_index);
    return last;
  }

  bool is_removable();

  bool has_less_than(std::string_view str) const {
    auto iter = iter_begin();
    while(iter != iter_end()) {
      if (iter->get_key() < str) {
	return true;
  ...
  bool has_between(std::optional<std::string> start,
    std::optional<std::string> end) {
    std::string_view s(*start);
    std::string_view e(*end);
    auto iter = iter_begin();
    while(iter != iter_end()) {
      if (iter->get_key() <= e && iter->get_key() >= s) {
	return true;
      }
      iter++;
    };
