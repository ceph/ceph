...
struct OMapInnerNode
  ...
  void do_on_rewrite(Transaction &t, LogicalCachedExtent &extent) final {
    ...
    this->init_meta(other.get_begin(), other.get_end());
  ...
  void do_on_replace_prior() final {
	...
	prior.base_child_t::has_parent_tracker());
    }
  }
  ...
  std::string end)
{
  ...
  ).si_then([begin=std::move(begin), end=std::move(end)]
	    (auto maybe_indirect_extent) mutable {
    assert(!maybe_indirect_extent.is_indirect());
    assert(!maybe_indirect_extent.is_clone);
    maybe_indirect_extent.extent->init_meta(std::move(begin), std::move(end));
