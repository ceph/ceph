  ...
  const std::string &key)
{
  ...
  return get_omap_root(
    get_omap_context(t, omap_root.hint, omap_root.get_type()),
    omap_root
  ).si_then([this, &t, &key, &omap_root](auto&& extent) {
    return extent->get_value(
      get_omap_context(t, omap_root.hint, omap_root.get_type()), key);
  }).si_then([](auto &&e) {
  ...
  const ceph::bufferlist &value)
{
  ...
  return get_omap_root(
    ...
    return root->insert(get_omap_context(
      t, omap_root.hint, omap_root.get_type()), key, value);
	...
	get_omap_context(t, omap_root.hint, omap_root.get_type()), omap_root, mresult);
    else
      return seastar::now();
  });
}
  ...
  const std::string &key)
{
  ...
  return get_omap_root(
    ...
    return root->rm_key(get_omap_context(t, omap_root.hint, omap_root.get_type()), key);
