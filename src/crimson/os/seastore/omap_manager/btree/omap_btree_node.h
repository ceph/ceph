...
struct OMapNode : LogicalChildNode {
  ...
  struct mutation_result_t {
    ...
    std::optional<OMapNodeRef> maybe_new_root;
    ...
    mutation_result_t(
      mutation_status_t s,
      std::optional<std::tuple<OMapNodeRef, OMapNodeRef, std::string>> tuple,
      std::optional<OMapNodeRef> n_merge,
      OMapNodeRef maybe_new_root)
    : status(s),
      split_tuple(tuple),
      need_merge(n_merge),
      maybe_new_root(maybe_new_root) {}
