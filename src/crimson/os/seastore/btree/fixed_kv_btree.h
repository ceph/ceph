...
public:
  ...
  template <typename T,
           std::enable_if_t<
             std::is_same_v<internal_node_t, T> ||
             std::is_same_v<leaf_node_t, T>, int> = 0>
...
private:
  ...
  static lookup_internal_level_ret lookup_internal_level(
    ...
    auto v = parent->template get_child<internal_node_t>(
      c.trans, node_iter.get_offset(), node_iter.get_key());
    ...
    iterator &iter)
  {
    ...
    if (split_from == iter.get_depth()) {
      ...
      nroot->set_cache_proxy(new CacheProxyImpl(c.cache));
    ...
    node_position_t<NodeType> &pos)
  {
    ...
    auto v = parent_pos.node->template get_child<NodeType>(
      c.trans, donor_iter.get_offset(), donor_iter.get_key());
    // checking the lba child must be atomic with creating
    // and linking the absent child
    if (v.has_child()) {
      return std::move(v.get_child_fut()
