...
struct fixed_kv_node_meta_t {
  ...
  std::pair<fixed_kv_node_meta_t, fixed_kv_node_meta_t> split_into(laddr_t pivot) const {
  ...
  rebalance(const fixed_kv_node_meta_t &lhs, const fixed_kv_node_meta_t &rhs, laddr_t pivot) {
    ceph_assert(lhs.depth == rhs.depth);
    return std::make_pair(
 ...
 * hook, the fixed_kv_node_meta_t representing the lba range covered by a node,
...
public:
  ...
  btree_range_pin_t<laddr_t>& get_range_pin() {
    return pin;
  }
