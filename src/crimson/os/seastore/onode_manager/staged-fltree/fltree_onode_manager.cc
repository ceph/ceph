  ...
  const ghobject_t &hoid)
{
  ...
  return tree.insert(
    ...
    return seastar::do_with(
      std::move(p),
      adjacent_onodes_t{},
      [&hoid, &trans, FNAME](auto &p, auto &onodes) {
	...
	if (!cursor.is_head()) {
	  ...
	  }).si_then([&onodes, &hoid, &trans](auto cursor) {
	    auto prev = cursor.get_ghobj();
            auto prev_snap_bak = prev.hobj.snap;
	    prev.hobj.snap = hoid.hobj.snap;
	    if (prev == hoid) {
              prev.hobj.snap = prev_snap_bak;
	      onodes.left = new FLTreeOnode(prev.hobj, cursor.value());
	    }
	    return cursor.get_next(trans);
	  });
	}
	return fut.si_then([&trans, &onodes, &hoid](auto cursor) mutable {
	  if (cursor.is_end()) {
	    return eagain_iertr::make_ready_future<
	      adjacent_onodes_t>(std::move(onodes));
	  }
	  return cursor.get_next(trans
	  ).si_then([&onodes, &hoid](auto cursor) {
	    if (!cursor.is_end()) {
	      auto next = cursor.get_ghobj();
              auto next_snap_bak = next.hobj.snap;
	      next.hobj.snap = hoid.hobj.snap;
	      if (next == hoid) {
                next.hobj.snap = next_snap_bak;
		onodes.right = new FLTreeOnode(next.hobj, cursor.value());
	      }
	    }
