...
class child_pos_t {
public:
  child_pos_t(TCachedExtentRef<ParentT> stable_parent, uint16_t pos)
...
public:
  ...
  virtual key_t get_begin() const = 0;
  virtual key_t get_end() const = 0;
protected:
...
class CacheProxy : public boost::intrusive_ref_counter<
		    CacheProxy, boost::thread_unsafe_counter> {
public:
  template <typename T>
  get_child_ifut<T> get_extent_viewable_by_trans(Transaction &t, T* ext) {
    return _get_extent_viewable_by_trans(t, ext
    ).si_then([](auto ext) {
      return get_child_iertr::make_ready_future<
	TCachedExtentRef<T>>(ext->template cast<T>());
    });
  }
  virtual void account_absent_access(Transaction& t) = 0;
  virtual bool is_viewable_extent_data_stable(Transaction &, CachedExtent*) = 0;
  virtual bool is_viewable_extent_stable(Transaction &, CachedExtent*) = 0;
  virtual ~CacheProxy() {}
protected:
  virtual get_child_iertr::future<CachedExtentRef> _get_extent_viewable_by_trans(
    Transaction &t,
    CachedExtentRef extent) = 0;
};
using CacheProxyRef = boost::intrusive_ptr<CacheProxy>;
...
public:
  ...
  void set_cache_proxy(CacheProxyRef cp) {
    cache = std::move(cp);
  }

  CacheProxyRef get_cache_proxy() {
    return cache;
  }
    ...
    node_key_t key)
  {
    ...
    if (is_valid_child_ptr(child)) {
      return cache->get_extent_viewable_by_trans<ChildT>(
  ...
  void link_child(BaseChildNode<T, node_key_t>* child, uint16_t pos) {
    auto &me = down_cast();
...
protected:
  ...
  void copy_children_from_stable_sources() {
    if (!copy_sources.empty()) {
      ...
      auto &cs = *it;
      uint16_t start_pos = cs->lower_bound(me.get_node_meta().begin).get_offset();
  ...
  bool validate_stable_children() {
    ...
    for (auto i : me) {
      ...
      if (is_valid_child_ptr(child) && child->get_begin() != i.get_key()) {
  ...
  bool _is_child_stable(
	...
	|| me.is_stable_written());
      if (data_only) {
	return cache->is_viewable_extent_data_stable(
	  t, dynamic_cast<CachedExtent*>(child));
      } else {
	return cache->is_viewable_extent_stable(
	  t, dynamic_cast<CachedExtent*>(child));
      }
    } else if (me.is_pending()) {
...
template <typename ParentT, typename ChildT, typename key_t>
class ChildNode : public BaseChildNode<ParentT, key_t> {
protected:
  void take_prior_parent_tracker() {
    auto &me = down_cast();
    auto &prior = static_cast<ChildT&>(*me.get_prior_instance());
  ...
  void set_parent_tracker_from_prior_instance() {
    ...
    auto off = parent->lower_bound(this->get_begin()).get_offset();
    assert(parent->iter_idx(off).get_key() == this->get_begin());
...
private:
  ...
  const ChildT& down_cast() const {
    return *static_cast<const ChildT*>(this);
  }
