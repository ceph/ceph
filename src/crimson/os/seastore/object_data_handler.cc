  ...
  return with_objects_data(
	...
	[ctx, FNAME](state_t &state) {
	  ...
	  return ctx.tm.find_region(
	    ...
	    state.dst_insert_iter = res.iter;
		...
		return seastar::futurize_invoke([ctx, &state, cur_is_direct] {
		  ...
		  if (cur_is_direct) {
			...
			false
		      ...
		      state.lba_iter = iter;
		      return clone_iertr::make_ready_future<LBAIter>(iter);
		    });
		    ...
		    state.indriect_entries.push_back(entry);
		    return clone_iertr::make_ready_future<
		      LBAIter>(state.lba_iter);
		  }
		}).si_then([ctx](LBAIter iter) {
		  ...
		  // in the future.
		  return ctx.tm.next_iterator(ctx.t, iter);
		}).si_then([&state](LBAIter iter) {
		  state.lba_iter = iter;
		});
	      });
	  }).si_then([ctx, &state] {
	    // The parent leaf node of state.dst_insert_iter might become
	    // invalid after the process of src mappings.
	    return ctx.tm.refresh_iterator(ctx.t, state.dst_insert_iter);
	    ...
	    state.dst.update_reserved(
	      res.laddr, state.src.get_reserved_data_len());
	    state.dst_base = res.laddr;
		...
		return ctx.tm.insert_mapping(
		  ...
		  if (!entry.update_refcount) {
		    ...
		    return ctx.tm.get_iterators(ctx.t, key, entry.value.len
		      ...
		      iter.val.refcount++;
		      return ctx.tm.change_mapping(
			ctx.t, iter, iter.key, iter.val, nullptr
		      ).discard_result();
		    });
		  }
		});
