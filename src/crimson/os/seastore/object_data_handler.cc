  ...
  extent_len_t len)
{
  ...
  return seastar::do_with(
	...
	return ctx.tm.reserve_region(
      ...
      return ctx.tm.move_and_clone_mapping<ObjectDataBlock>(
...
ObjectDataHandler::rename(context_t ctx)
{
  return with_objects_data(ctx, [ctx, this](object_data_t &src, object_data_t &dst) {
    ...
    ).si_then([ctx, &src](LBAMapping mapping) {
      return ctx.tm.next_mapping(ctx.t, mapping.duplicate()
      ).si_then([ctx, mapping=std::move(mapping), &src](auto insert_pos) mutable {
	auto dst_prefix = mapping.get_key();
	return ctx.tm.remove(ctx.t, std::move(mapping)
	).si_then([ctx, pos=std::move(insert_pos)](auto) mutable {
	  return ctx.tm.refresh_lba_mapping(ctx.t, std::move(pos));
	}).si_then([ctx, dst_prefix, &src](LBAMapping dst) {
	  return ctx.tm.get_pin(ctx.t, src.get_reserved_data_base(), false
	  ).si_then([ctx, dst_prefix, dst=std::move(dst)](LBAMapping src) mutable {
