...
std::ostream& operator<<(
  ...
  return out << "overwrite_range_t{"
    << "unaligned_len=" << std::hex << overwrite_range.unaligned_len
    << ", unaligned_begin=" << std::hex << overwrite_range.unaligned_begin
    << ", aligned_begin=" << std::hex << overwrite_range.aligned_begin
    << ", unaligned_end=" << std::hex << overwrite_range.unaligned_end
    << ", aligned_end=" << std::hex << overwrite_range.aligned_end
    << ", aligned_len=" << std::hex << overwrite_range.aligned_len
    << "}";
}

std::ostream& operator<<(std::ostream &out, const data_t &data) {
  return out << "data_t{"
    << "headbl=" << std::hex << (data.headbl ? data.headbl->length() : 0)
    << ", bl=" << std::hex << (data.bl ? data.bl->length() : 0)
    << ", tailbl=" << std::hex << (data.tailbl ? data.tailbl->length() : 0)
    << "}";
}
  ...
  data_t &data)
{
  ...
  return ctx.tm.alloc_data_extents<ObjectDataBlock>(
    ...
    auto left = overwrite_range.aligned_begin.template get_byte_distance<
      extent_len_t>(overwrite_range.aligned_end);
    bufferlist _bl;
    if (data.headbl) {
  ...
  edge_t edge)
{
  ...
  return fut.si_then([ctx, edge_mapping, &overwrite_range, edge] {
	...
	return ObjectDataHandler::base_iertr::make_ready_future<
      ...
      assert(edge == edge_t::RIGHT);
      assert(overwrite_range.aligned_end <
  ...
  op_type_t op_type)
{
  ...
  case edge_handle_policy_t::MERGE_FROM_PENDING:
    return do_merge_based_edge_punch(
      ctx, overwrite_range, data, std::move(edge_mapping), edge);
  case edge_handle_policy_t::MERGE_TO_PENDING:
  ...
  LBAMapping mapping)
{
  if (mapping.is_pending()) {
    // TODO: should merge the current overwrite into the pending mapping
    //       instead of removing and inserting the pending mapping
    return punch_hole_in_pending_mapping(
      ctx, overwrite_range, overwrite_data, std::move(mapping));
  } else { // REMAP policy
    auto fut = ObjectDataHandler::base_iertr::now();
    ...
    if (edge != edge_t::NONE) {
      fut = read_unaligned_edge_data(
	ctx, overwrite_range, overwrite_data, mapping, edge);
    }
    return fut.si_then([ctx, &overwrite_range, mapping] {
      auto len = overwrite_range.aligned_end.template get_byte_distance<
	extent_len_t>(overwrite_range.aligned_begin);
  ...
  op_type_t op_type)
{
  if (overwrite_range.is_range_in_mapping(left_mapping)) {
    return punch_single_mapping_hole(
      ctx, overwrite_range, data, std::move(left_mapping));
  } else {
  ...
  op_type_t op_type)
{
  ...
  auto do_overwrite = [ctx, &overwrite_range, &data](auto pos) {
    ...
    if (overwrite_range.aligned_begin.template get_byte_distance<
	  extent_len_t>(overwrite_range.aligned_end) == ctx.tm.get_block_size()
  ...
  LBAMapping first_mapping)
{
  ...
  return seastar::do_with(
	...
	data.bl.has_value() ? op_type_t::OVERWRITE : op_type_t::OVERWRITE);
    } else {
  ...
  context_t ctx, object_data_t &object_data, extent_len_t size)
{
  ...
  return ctx.tm.get_containing_pin(
    ...
    auto unaligned_end = data_base + data_len;
    return seastar::do_with(
      data_t{},
      overwrite_range_t{
	data_len - size,
	...
	unaligned_end,
	ctx.tm.get_block_size()},
      [this, ctx, mapping=std::move(mapping)]
      (auto &data, auto &overwrite_range) mutable {
      return punch_hole(
	ctx, overwrite_range, data, std::move(mapping), op_type_t::TRIM
      ).si_then([ctx, &overwrite_range, &data](auto pos) {
	assert(pos.is_end() || pos.get_key() >= overwrite_range.aligned_end);
	return do_zero(ctx, std::move(pos), overwrite_range, data);
      });
    });
  }).handle_error_interruptible(
  ...
  LBAMapping dest)
{
  ...
  return seastar::do_with(
    ...
    src_params,
    dest_params,
    [this, ctx](auto &src, auto &dest, const auto &src_params,
	       const auto &dest_params) {
    return src.refresh().si_then([&src_params, &src, ctx](auto s) {
      assert(!s.is_pending());
      // remap src if it represents an extent and crosses src_params' begin
      if (s.is_real() && src_params.begin > s.get_key()) {
	return ctx.tm.split_mapping<ObjectDataBlock>(
	  ctx.t, src_params.begin, std::move(s)
	...
	return move_mappings_iertr::now();
      ...
      return dest.refresh();
    }).si_then([&dest, ctx, this, &src, &src_params, &dest_params](auto d) {
      dest = std::move(d);
	...
	return trans_intr::repeat([&dest, &src, &src_params, &dest_params] {
	  ...
	  return dest.refresh();
	}).si_then([&dest, &src, ctx, this, &src_params, &dest_params](auto d) {
	  dest = std::move(d);
	  // move src into dest
	  auto src_off = src.get_key().template get_byte_distance<
	    extent_len_t>(src_params.begin);
