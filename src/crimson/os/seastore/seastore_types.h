 ...
 *
 * All bits set to 1 in the object info represent the invalid prefix, and rados
 * logical extents must not use it to avoid sharing the same prefix with
 * L_ADDR_NULL.
 *
 * There are two special rules for global metadata logical extents:
 * 1. RootMetaBlock and CollectionNode
 *    When a laddr is used to represent these two types of extents, all object
 ...
 * - 2^6-1=63 shards per pool for EC
 ...
 * object. As mentioned above, the local clone id bits must not be all 1.
 *
 * NOTE: Taking snapshots is a symmetric process for both the newly cloned onode
 * and the head onode. The direct mappings in head are moved to a new address
 * space, both head and clone onode will make indirect mappings pointed to them,
 * see ObjectDataHandler::clone().
 ...
 *
 * It's possible for OMapInnerNode that only stores the block offset field in each
 * extent rather than the full 128-bit laddr_t to locate its children, which would
...
public:
...
#else
  // NOTE: `const std::varaint<int, Unsigned>` is not nothrow move constructible
  // so that a struct contains this type should mark its move ctor with nothrow,
  // otherwise it could not be used in seastar::future.
  ...
  laddr_t(const laddr_t &) = default;
  laddr_t(laddr_t &&) = default;
  laddr_t &operator=(const laddr_t &) = default;
  laddr_t &operator=(laddr_t &&) = default;
  ...
  struct laddr_offset_t {
    ...
    friend laddr_offset_t operator+(const laddr_offset_t &laddr_offset,
      ...
      return laddr_offset.get_aligned_laddr(laddr_t::UNIT_SIZE)
	...
	return laddr_offset_t(
	  laddr_offset.get_aligned_laddr(laddr_t::UNIT_SIZE),
	  laddr_offset.get_offset() - offset);
      } else {
	// laddr_offset_t could access (laddr_t - loffset_t) overload.
	return laddr_offset.get_aligned_laddr(laddr_t::UNIT_SIZE)
  ...
  friend bool operator==(const laddr_t &laddr,
    ...
    return laddr == laddr_offset.get_aligned_laddr(laddr_t::UNIT_SIZE)
	&& 0 == laddr_offset.get_offset();
  }
  friend bool operator==(const laddr_offset_t &laddr_offset,
			 const laddr_t &laddr) {
    return laddr_offset.get_aligned_laddr(laddr_t::UNIT_SIZE) == laddr
