  ...
  laddr_t offset, extent_len_t length)
{
  ...
  return seastar::do_with(
    ...
    return trans_intr::make_interruptible(seastar::parallel_for_each(
      leaves,
      [&t](auto &leaf) {
	...
	t.add_to_read_set(leaf);
      ...
      return leaf->wait_io();
    })).then_interruptible([&leaves, offset, length] {
      lba_pin_list_t ret;
  ...
  laddr_t offset)
{
  ...
  if (extent && extent->is_valid()) {
    ...
    return get_mapping_iertr::future<>(extent->wait_io()
    ).si_then([this, &t, extent, offset]() mutable
      -> get_mapping_ret {
      LOG_PREFIX(BtreeLBAManager::get_mapping);
      if (!extent->should_contain(offset)) {
