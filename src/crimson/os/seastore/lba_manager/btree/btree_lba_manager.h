...
public:
  ...
  move_mapping_ret move_mapping(
    ...
    laddr_t dest_laddr,
    LBAMapping dest,
    LogicalChildNode *extent = nullptr) final;
...
private:
  ...
  struct update_mapping_ret_bare_t {
    ...
    bool is_alive_mapping() const {
      return ret.index() == 1 && std::get<1>(ret);
    }
	...
	return {v.laddr,
		...
		val.len,
		v.next->is_indirect()
		  ? LBAMapping::create_indirect(std::move(v.next))
	...
	return {c.get_laddr(), c.val->refcount, 
	  ...
	  c.is_indirect()
	    ? LBAMapping::create_indirect(take_cursor())
   ...
   *
   * hint is a non-null laddr hint for allocation. None of alloc_infos'
   * key are L_ADDR_NULL, each mapping's final laddr maintains same offset
   * to allocated laddr as original to hint.
   */
