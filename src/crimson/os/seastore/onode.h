...
public:
  ...
  void handle_siblings(Onode *left, Onode *right) {
    auto update_or_validate = [this](Onode *sibling) {
      if (!sibling) {
        return;
      }

      auto sibling_prefix = sibling->get_clone_prefix();
      ceph_assert(sibling_prefix);

      if (auto prefix = get_clone_prefix(); prefix) {
        ceph_assert(prefix->get_local_object_id() ==
                    sibling_prefix->get_local_object_id());
      } else if (sibling_object_id) {
        ceph_assert(*sibling_object_id ==
                    sibling_prefix->get_local_object_id());
      } else {
        sibling_object_id = sibling_prefix->get_local_object_id();
      }
    };
    update_or_validate(left);
    update_or_validate(right);
  }
