...
public:
    ...
    lextent_init_func_t<T> maybe_init = [](T&) {})
  {
    ...
    return lba_manager->refresh_lba_mapping(
      t, std::move(const_cast<LBAMapping&>(pin))
  ...
  move_mapping_ret move_and_clone_mapping(
    ...
    LBAMapping dest)
  {
    LOG_PREFIX(TransactionManager::move_and_clone_mapping);
	...
	src.get_length(),
	original_bptr).get();
      return lba_manager->move_and_clone_mapping(
