...
struct overwrite_range_t {
  ...
  overwrite_range_t(
    ...
    : unaligned_len(unaligned_len),
      ...
      aligned_len(
	aligned_begin.template get_byte_distance<
...
private:
    ...
    op_type_t op_type) const
  {
    ...
    if (edge_mapping.is_pending()) {
      ...
      //       mostly in the recovery case, which is relative rare compared to
      //       normal IO processing.
      if (edge_mapping.is_initial_pending()) {
	return edge_handle_policy_t::MERGE_TO_PENDING;
      } else {
	return edge_handle_policy_t::MERGE_FROM_PENDING;
      }
    }
  ...
  // punch a lba hole that falls in a single mapping
  ...
  // punch a lba hole whether is single-mapping or multi-mapping
