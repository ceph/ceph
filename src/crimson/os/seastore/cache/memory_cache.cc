...
class LRUMemoryCache : public MemoryCache {
  ...
  bool should_promote_extent(const CachedExtent &extent) {
	...
	ret = !lextent->get_laddr().is_shadow()
  ...
  void remove_impl(CachedExtent &extent, bool need_to_promote) {
    ...
    TRACE("extent {}, {}, need to promote: {}, promote_contents: {}",
	  extent, get_cache_state(extent), need_to_promote, promote_contents);
	...
	TRACE("remove extent {} from pending promote list", extent);
	promote_contents -= extent.get_length();
  ...
  void register_metrics() {
    ...
    metrics.add_group(
      "cache",
      {
        ...
        sm::make_counter(
          "promoted_count",
          [this] { return promoted_count; },
          sm::description("total extents promoted to the faster device")
        ),
        sm::make_counter(
          "promoted_bytes",
