...
RecordSubmitter::open(unsigned int shard_index, bool is_mkfs)
{
  return journal_allocator.open(is_mkfs
  ).safe_then([this, shard_index](journal_seq_t ret) {
    ...
    label_instances.push_back(sm::label_instance("shard_store_index", std::to_string(shard_index)));
