...
journal_seq_t JournalTrimmerImpl::get_trim_dirty_target() const
{
  assert(background_callback->is_ready());
  auto journal_dirty_bytes = get_journal_dirty_bytes();
  auto rewrite_target =
    journal_dirty_bytes - config.rewrite_dirty_bytes_per_cycle;
  auto ret = journal_head.add_offset(
      backend_type,
      -static_cast<device_off_t>(
	std::max(config.min_journal_bytes, rewrite_target)),
      roll_start,
      roll_size);
  return ret;
}
...
JournalTrimmerImpl::trim_dirty()
{
  ...
  auto target = get_trim_dirty_target();
