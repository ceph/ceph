...
seastar::future<T*> create_sharded(Args... args) {
  ...
  auto ptr_shard = co_await seastar::smp::submit_to(0, [=] {
    ...
    auto ptr = co_await sharded_obj->start(args...);
...
struct client_config {
  ...
  // TBC. can we generate json from this?
  ...
  std::string json_str() const {
    ...
    f->dump_unsigned("ramptime", ramptime);
    f->dump_unsigned("msgtime", msgtime);
    f->dump_unsigned("num_clients", num_clients);
    f->dump_unsigned("num_conns", num_conns);
      ...
      class TimerReport {
       ...
       public:
        ...
        seastar::future<> ticktock() {
          ...
          // to make sure elapsed time is correct 
          /*
            seastar::smp::count > 1 ? seastar::smp::invoke_on_all([] {
            return seastar::now();
          }) : seastar::now();
          */

        void report_header() const {
          // TBC. Produce output in json format
          std::ostringstream sout;
          sout << std::setfill(' ')
               << std::setw(6) << "sec"
               << std::setw(7) << "depth"
