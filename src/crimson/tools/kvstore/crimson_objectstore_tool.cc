    ...
    return app.run(
      ...
      [] () -> seastar::future<int> {
        try {
          ...
          co_await sharded_conf().start(EntityName{}, std::string_view{"ceph"});
          auto stop_conf = seastar::deferred_stop(sharded_conf());
          co_await local_conf().start();
          ...
          if (config.list_objects ||
            ...
            auto it = std::find_if(pgs.begin(), pgs.end(),
              [&coll](const auto& pg) { return pg.first == coll; });
            if (it == pgs.end()) {
          ...
          if (config.list_pgs) {
            ...
            for (auto pg : pgs) {
              logger.info("pg: {}, shard id: {}", pg.first, pg.second);
            }
          } else if (config.list_objects) {
            ghobject_t next;
            do {
              auto objs = co_await st.list_objects(coll, shard_id, next);
              next = std::get<1>(objs);
            ...
            try {
              FuturizedStore::Shard::omap_values_t omaps =
                co_await st.omap_get_values(coll, shard_id, ghobj, start);
              if (omaps.empty()) {
            ...
            if (success) {
              logger.info("set omap success: key={}, value={}",
                config.omap_key,
                config.omap_value);
            } else {
            ...
            try {
              std::string omap_value = co_await st.get_omap(
                coll, shard_id, ghobj, config.omap_key);
              if (!omap_value.empty()) {
                logger.info("{}", omap_value);
              } else {
            ...
            try {
              bool success = co_await st.remove_omap(
                coll, shard_id, ghobj, config.omap_key);
              if (success) {
                logger.info("remove omap success: key={}",
                  config.omap_key);
              } else {
