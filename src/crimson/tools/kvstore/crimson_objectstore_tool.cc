...
// Helper class for reading seastore metadata, inspired by OSDMeta pattern
class SeastoreMetaReader {
private:
  std::string data_path;
  
  // Get block size using POSIX stat, similar to what seastar does internally
  size_t get_filesystem_block_size(const std::string& path) {
    struct stat st;
    if (stat(path.c_str(), &st) == 0) {
      // st_blksize is the optimal block size for I/O operations
      return st.st_blksize;
    }
    // Fallback to 4096 if stat fails
    return 4096;
  }
  
public:
  explicit SeastoreMetaReader(const std::string& path) : data_path(path) {}
  
  // Read seastore superblock using pattern similar to BlockSegmentManager::read_superblock()
  std::optional<crimson::os::seastore::block_sm_superblock_t> load_seastore_superblock() {
    try {
      std::string block_path = data_path + "/block";
      
      // Get proper block size from filesystem, similar to seastar::file_stat
      size_t block_size = get_filesystem_block_size(block_path);
      
      // Use the same pattern as BlockSegmentManager::read_superblock()
      ...
      // Use the actual filesystem block size instead of hardcoded value
      ...
      // Decode seastore superblock using bufferlist (similar to OSDMeta approach)
      bufferlist bl;
      ...
      // Validate like the main code does
  ...
  // Extract shard count from superblock
                              ...
                              const std::string& store_type)
{
  ...
  // Use BlockSegmentManager-inspired pattern for metadata reading  
...
get_seastar_args_from_storage(const objectstore_config_t& config)
{
  ...
  if (!shard_count.has_value()) {
    if (config.store_type == "seastore") {
      // For seastore, we require reading shard config from storage
      fmt::print(std::cerr, 
      ...
      // For non-seastore types, use default seastar configuration
      fmt::print(std::cerr, 
                 "Using default seastar configuration for {} store type\n", 
                 config.store_type);
      return std::vector<std::string>{}; // Return empty vector for default config
    }
  }
...
int main(int argc, const char* argv[])
{
  ...
  try {
    ...
    std::cerr << "ObjectStore Tool Options:" << std::endl;
        ...
        try {
          ...
          if (objectstore_config.list_objects || 
            ...
            if (objectstore_config.list_omap || 
                objectstore_config.get_omap || 
                objectstore_config.set_omap) {
              if ((objectstore_config.get_omap || objectstore_config.set_omap) && 
                  objectstore_config.object.empty()) {
                fmt::print(std::cerr, 
                           "object name is required for get-omap and set-omap\n");
                return EXIT_FAILURE;
              }
