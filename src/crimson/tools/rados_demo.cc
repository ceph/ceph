...
int main(int argc, const char* argv[])
{
  ...
  if (!early_result.has_value()) {
    const int code = early_result.error();
    if (code == 0) {
      return EXIT_SUCCESS;
    }
    if (code == 1) {
    ...
    std::cerr << "get_early_config_client failed: " << code << std::endl;
  ...
  auto seastar_early_args = early_config.get_early_args();

  try {
    return app.run(
      seastar_early_args.size(),
      const_cast<char**>(seastar_early_args.data()),
      [&] {
