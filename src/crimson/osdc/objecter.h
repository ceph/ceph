...
struct OsdSession {
  ...
  /// Throttle for write bytes in flight (max set from objecter_inflight_op_bytes)
  crimson::common::Throttle write_throttle{0};
};
...
class Objecter : public crimson::net::Dispatcher,
                 public crimson::common::ConfigProxy::ConfigObserver {
public:
...
private:
  ...
  uint64_t write_throttle_bytes = 0;

  void update_write_throttle_from_config();
  void apply_write_throttle_max(OsdSession& session) const;
  OsdSession make_osd_session(int osd_id,
                              entity_addr_t addr,
                              crimson::net::ConnectionRef conn) const;
