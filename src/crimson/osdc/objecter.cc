...
void Objecter::update_write_throttle_from_config()
{
  write_throttle_bytes = local_conf()->objecter_inflight_op_bytes;
  for (auto& [_, session] : sessions) {
    apply_write_throttle_max(session);
  }
}

void Objecter::apply_write_throttle_max(OsdSession& session) const
{
  session.write_throttle.reset_max(write_throttle_bytes);
}

OsdSession Objecter::make_osd_session(
  int osd_id,
  entity_addr_t addr,
  crimson::net::ConnectionRef conn) const
{
  OsdSession session;
  session.osd_id = osd_id;
  session.addr = addr;
...
{
  return {"objecter_inflight_op_bytes"s};
}

void Objecter::handle_conf_change(const crimson::common::ConfigProxy& conf,
                                  const std::set<std::string>& changed)
{
  (void)changed;
  write_throttle_bytes = conf->objecter_inflight_op_bytes;
  for (auto& [_, session] : sessions) {
    apply_write_throttle_max(session);
  }
}
...
seastar::future<> Objecter::start()
{
  ...
  update_write_throttle_from_config();
  local_conf().add_observer(this);
...
seastar::future<> Objecter::stop()
{
  ...
  ceph_assert(started);
  started = false;
  local_conf().remove_observer(this);
...
Objecter::get_or_connect(const entity_addr_t& addr, int osd_id)
{
  ...
  auto fut = promise_it->second.get_shared_future();

  crimson::net::ConnectionRef conn = msgr.connect(
    addr, entity_name_t(CEPH_ENTITY_TYPE_OSD, osd_id));

  pend_it = pending_connects.find(osd_id);
  if (pend_it == pending_connects.end()) {
    // ms_handle_connect completed synchronously during connect()
    it = sessions.find(osd_id);
    ceph_assert(it != sessions.end() && it->second.conn);
    return seastar::make_ready_future<crimson::net::ConnectionRef>(it->second.conn);
  }

  if (conn->is_connected()) {
    if (it == sessions.end()) {
      sessions.emplace(osd_id, make_osd_session(osd_id, addr, conn));
    } else {
      it->second.conn = conn;
      it->second.addr = addr;
    }
    promise_it->second.set_value(conn);
    pending_connects.erase(osd_id);
  }
                                ...
                                seastar::shard_id prv_shard)
{
  ...
  if (it != sessions.end()) {
    ...
    sessions.emplace(osd_id,
                    make_osd_session(osd_id, conn->get_peer_addr(), conn));
  }
