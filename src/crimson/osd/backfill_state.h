...
private:
  struct backfill_cancel_state_t {
    bool cancelled = false;
    bool should_go_enqueuing = false;
  } backfill_cancel_state;
  bool is_cancelled() const {
    return backfill_cancel_state.cancelled;
  }
  void on_cancelled() {
    ceph_assert(!is_cancelled());
    backfill_cancel_state = {true, false};
  }
  bool on_resumed() {
    auto go_enqueuing = backfill_cancel_state.should_go_enqueuing;
    backfill_cancel_state = {false, false};
    return go_enqueuing;
  }
  void go_enqueuing_on_resume() {
    ceph_assert(is_cancelled());
    backfill_cancel_state.should_go_enqueuing = true;
  }
  hobject_t last_backfill_started;
