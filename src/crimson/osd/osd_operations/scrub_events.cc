...
ScrubSleep::ifut<> ScrubSleep::run(PG &pg)
{
  ...
  return interruptor::make_interruptible(seastar::sleep(sleep_time)
    ...
    DEBUGDPP("sleep complete, checking abort conditions", pg);
    if (!pg.scrubber.verify_against_abort(pg.get_osdmap_epoch())) {
      DEBUGDPP("aborting scrub after sleep", pg);
      pg.scrubber.machine.process_event(scrub::events::abort_t{});
    } else {
      pg.scrubber.machine.process_event(
