  ...
  const ghobject_t &obj)
{
  ...
  return interruptor::repeat(
	...
	DEBUGDPP("op: {}, obj: {}, progress: {} scanning omap keys",
        ...
        start_from.seek_position =  progress.next_key.has_value() ? progress.next_key.value() : std::string{};
        ...
        std::function<ObjectStore::omap_iter_ret_t(std::string_view, std::string_view)> callback =
          [&obj, &progress, &entry, &pg] (std::string_view key, std::string_view value) {
            bufferlist bl;
            encode(key, bl);
            encode(value, bl);
            progress.omap_hash << bl;
