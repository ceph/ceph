...
{
  logger().info("{}: queue_snap_retrims snaps={}", *this, snaps);
  if (!peering_state.is_primary() || !peering_state.is_active()) {
    logger().info("{}: not primary or active, skipping snap retrims", *this);
    return;
  }
  ...
  for (auto snap : snaps) {
    if (!snap_trimq.contains(snap)) {
      ...
      logger().info("{}: snap {} already in trimq", *this, snap);
    }
  }
