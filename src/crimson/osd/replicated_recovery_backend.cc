    ...
    PushOp* push_op)
{
  ...
  start_from.seek_position = new_progress.omap_recovered_to.empty() ?  std::string{} : new_progress.omap_recovered_to;
  ...
  std::function<ObjectStore::omap_iter_ret_t(std::string_view, std::string_view)> callback =
    [&new_progress, &max_len, push_op] (std::string_view key, std::string_view value) {
  ...
  return seastar::do_with(
    std::move(start_from),
    std::move(callback),
    [&new_progress, &max_len, push_op, &oid, this] (auto &start_from, auto &fun) {
    return shard_services.get_store().omap_iterate(
      coll, ghobject_t{oid}, start_from, fun)
