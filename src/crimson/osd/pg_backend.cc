...
maybe_do_omap_iterate(
  ...
  std::function<ObjectStore::omap_iter_ret_t(std::string_view, std::string_view)> &fun)
{
  if (oi.is_omap()) {
    return store->omap_iterate(coll, ghobject_t{oi.soid}, start_from, fun);
  } else {
    return crimson::ct_error::enodata::make();
  }
}
  ...
  object_stat_sum_t& delta_stats) const
{
  ...
  return seastar::do_with(
    ...
    [&, start_after, max_return, this] (auto &result, auto &num, auto &truncated, auto &start_from, auto &callback) {

      start_from.seek_position = start_after;
      start_from.seek_type = ObjectStore::omap_iter_seek_t::UPPER_BOUND;

      callback = [&result, &num, &truncated, max_return] (std::string_view key, std::string_view value) {
        encode(key, result);
        ++num;
        if (num >= max_return) {
          truncated = true;
  ...
  object_stat_sum_t& delta_stats) const
{
  ...
  return seastar::do_with(
    ...
    (auto &result, auto &num, auto &truncated, auto &start_from, auto &callback) {
      ...
      callback = [this, filter_prefix, max_return, &result, &num, &truncated]
