  ...
  object_stat_sum_t& delta_stats) const
{
  ...
  co_await [&os, &delta_stats, &osd_op, start_after, max_return, this]() -> ll_read_ierrorator::future<> {
    ...
    std::function<ObjectStore::omap_iter_ret_t(std::string_view, std::string_view)> callback =
      [&result, &num, &truncated, max_return, &start_from] (std::string_view key, std::string_view value) {
  ...
  object_stat_sum_t& delta_stats) const
{
  ...
  co_await [&os, &delta_stats, &osd_op, filter_prefix, start_after, max_return, this]() -> ll_read_ierrorator::future<> {
    ceph::bufferlist result;
    uint32_t num = 0;
    bool truncated = false;
