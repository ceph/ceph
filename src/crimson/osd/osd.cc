  ...
  const std::set <std::string> &changed)
{
  ...
  if (changed.contains("osd_op_complaint_time")) {
    local_conf().set_val(
      "osd_op_complaint_time", std::to_string(conf->osd_op_complaint_time)
    ).get();
  }
  if (changed.contains("osd_aggregated_slow_ops_logging")) {
    local_conf().set_val(
      "osd_aggregated_slow_ops_logging",
      conf->osd_aggregated_slow_ops_logging ? "true" : "false"
    ).get();
  }
}
