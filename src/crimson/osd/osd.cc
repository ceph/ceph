...
{
  return shard_stores.local().store->open_collection(
    coll_t::meta()
  ).then([this](auto ch) {
    pg_shard_manager.init_meta_coll(ch, *(shard_stores.local().store));
    return seastar::now();
  });
}
...
seastar::future<> OSD::start()
{
  ...
  return pg_shard_manager.start(
    whoami, *cluster_msgr,
    *public_msgr, *monc, *mgrc, shard_stores
  ).then([this] {
  ...
  auto get_weight = [this] {
	...
	w >= 0) {
      return seastar::make_ready_future<double>(w);
       ...
       return shard_stores.local().store->stat().then([](auto st) {
         auto total = st.total;
...
void OSD::update_stats()
{
  ...
  gate.dispatch_in_background("statfs", *this, [this] {
    (void) shard_stores.local().store->stat().then([this](store_statfs_t&& st) {
      osd_stat.statfs = st;
    });
  });
}
                                      ...
                                      Ref<MOSDMap> m)
{
  ...
  return seastar::do_with(ceph::os::Transaction{},
    ...
    return pg_shard_manager.store_maps(t, start, m).then([=, this, &t] {
      ...
      return shard_stores.local().store->do_transaction(
...
{
  return shard_stores.local().store->write_meta("require_osd_release",
