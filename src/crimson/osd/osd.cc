...
{
  LOG_PREFIX(OSD::deep_scrub_purged_snaps);
  DEBUG("deep_scrub_purged_snaps");
  ...
  if (next < now) {
    DEBUG("deep_scrub_snaps: last_scrub={} next={} ... now", last, next);
    return scrub_purged_snaps();
  }

  DEBUG("deep_scrub_snaps: last_scrub={} next={}", last, next);
  return seastar::now();
}
