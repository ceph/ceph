...
#define MIN_EXPIRE_S 120
  ...
  void RGWLibProcess::handle_request(const DoutPrefixProvider *dpp, RGWRequest* r, bool null_vid)
  {
    /*
    ...
    int ret = process_request(req, &io_ctx, null_vid);
    if (ret < 0) {
  ...
  int RGWLibProcess::process_request(RGWLibRequest* req, bool null_vid)
  {
    // XXX move RGWLibIO and timing setup into process_request
  ...
  int RGWLibProcess::process_request(RGWLibRequest* req, RGWLibIO* io, bool null_vid)
  {
    int ret = 0;
	...
	goto done;
      ...
      op->pre_exec();
      op->execute(null_yield, null_vid);
      op->complete();
