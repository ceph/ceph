...
int RGWBucketCreateLocalCR::Request::_send_request()
{
  ...
  if (existed) {
    if (info.owner != user) {
      ldpp_dout(this, 20) << "NOTICE: bucket already exists under a different user (bucket=" << bucket << " user=" << user << " bucket_owner=" << info.owner << dendl;
      return -EEXIST;
    }
    bucket = info.bucket;
  }
  ...
  if (ret && !existed && ret != -EEXIST) {
    ...
    if (r < 0) {
      ldpp_dout(this, 0) << "WARNING: failed to unlink bucket: ret=" << r << dendl;
    }
  ...
  if (ret < 0) {
    ldpp_dout(this, 0) << "ERROR: bucket creation (bucket=" << bucket << ") return ret=" << ret << dendl;
  }
