...
namespace rgw::dedup {
  ...
  //---------------------------------------------------------------------------
  bool cluster::all_shard_tokens_completed(rgw::sal::RadosStore *store,
                                           unsigned shards_count,
                                           const char *prefix,
                                  ...
                                  const DoutPrefixProvider *dpp)
  {
    ...
    auto rados_handle = store->getRados()->get_rados_handle();
    ldpp_dout(dpp, 5) <<__func__ << "::delete pool: " << DEDUP_POOL_NAME << dendl;
    rados_handle->pool_delete(DEDUP_POOL_NAME);
#endif
