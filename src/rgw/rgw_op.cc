...
int RGWPutObj::user_manifest_iterate_cb(rgw_bucket& bucket, RGWObjEnt& ent, RGWAccessControlPolicy *bucket_policy, off_t start_ofs, off_t end_ofs)
{
  ...
  int ret = get_obj_attrs(store, s, part, attrs);
  if (ret < 0) {
    return ret;
  }
...
void RGWPutObj::execute()
{
  ...
  if (obj_manifest) {
    ...
    RGWObjectCtx obj_ctx(store);
    ret = store->get_bucket_info(obj_ctx, manifest_bucket, bucket_info, NULL, NULL);
    if (ret < 0) {
...
void RGWPutMetadata::execute()
{
  ...
  if (is_object_op) {
    ...
    RGWObjVersionTracker *ptracker = &s->bucket_info.objv_tracker;
...
void RGWDeleteObj::execute()
{
  ...
  if (!s->object.empty()) {
    ...
    ACLOwner& obj_owner = s->owner;

    ret = store->delete_obj(*obj_ctx, s->bucket_owner.get_id(), obj, s->bucket_info.versioning_status(), &obj_owner);
  }
}
...
void RGWCompleteMultipart::execute()
{
  ...
  int r = store->delete_obj(*(RGWObjectCtx *)s->obj_ctx, s->bucket_owner.get_id(), meta_obj, 0, NULL);
  if (r < 0) {
...
void RGWAbortMultipart::execute()
{
  ...
  do {
    ...
    for (obj_iter = obj_parts.begin(); obj_iter != obj_parts.end(); ++obj_iter) {
      ...
      if (obj_part.manifest.empty()) {
        ...
        ret = store->delete_obj(*obj_ctx, owner, obj, 0, NULL);
        if (ret < 0 && ret != -ENOENT)
          return;
      } else {
        ...
        for (oiter = manifest.obj_begin(); oiter != manifest.obj_end(); ++oiter) {
          ...
          ret = store->delete_obj(*obj_ctx, owner, loc, 0, NULL);
          if (ret < 0 && ret != -ENOENT)
            return;
        }
      }
  ...
  meta_obj.init_ns(s->bucket, meta_oid, mp_ns);
  meta_obj.set_in_extra_data(true);
  ret = store->delete_obj(*obj_ctx, owner, meta_obj, 0, NULL);
  if (ret == -ENOENT) {
    ret = -ERR_NO_SUCH_BUCKET;
  }
}
...
void RGWDeleteMultiObj::execute()
{
  ...
  for (iter = multi_delete->objects.begin();
    ...
    obj_ctx->set_atomic(obj);
    ret = store->delete_obj(*obj_ctx, s->bucket_owner.get_id(), obj, s->bucket_info.versioning_status(), NULL);
    if (ret == -ENOENT) {
      ret = 0;
    }
