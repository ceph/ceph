...
static int nonreplicated_bucket_redirect(const DoutPrefixProvider* dpp,
                                         ...
                                         std::string& endpoint)
{
  ...
  if (zone == zonegroup.zones.end()) {
    ...
    error_message = fmt::format(
        "The requested non-replicated bucket {} belongs to zone id {} which "
...
int rgw_build_bucket_policies(const DoutPrefixProvider *dpp, rgw::sal::Driver* driver, req_state* s, optional_yield y)
{
  ...
  if (!s->bucket_name.empty()) {
    ...
    const std::string& redirect_zone_id = s->bucket->get_info().redirect_zone_id;
    ...
    if (!redirect_zone_id.empty() &&
        redirect_zone_id != local_zone_params.id &&
        !forwarded) {
      return nonreplicated_bucket_redirect(dpp, zonegroup, redirect_zone_id,
