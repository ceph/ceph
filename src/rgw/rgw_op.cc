                                                 ...
                                                 boost::asio::yield_context y)
{
  ...
  for (const auto& [_, objects] : grouped_objects) {
    for (size_t i = 0; i + 1 < objects.size(); ++i) { // skip the last element
      ...
      aio_count++;
      boost::asio::spawn(y, [this, &aio_count, &objects, i, &formatter_flush_cond] (boost::asio::yield_context yield) {
        handle_individual_object(objects[i], yield, &*formatter_flush_cond, true /* skip_olh_obj_update */);
        aio_count--;
