...
{
  if (timeout_ms <= 0 || (!waiter && !rgw::restore::g_restore_waiter_registry)) {
  ...
  if (!waiter) {
    waiter = rgw::restore::g_restore_waiter_registry->register_waiter(
      s->bucket->get_key(),
      s->object->get_key()
    );
    guard = std::make_unique<rgw::restore::WaiterGuard>(
      rgw::restore::g_restore_waiter_registry,
      waiter
    );
  }
  ...
  while (remaining_ms > 0) {
    ...
    bool notified;
    {
      ...
      notified = waiter->cv.wait_for(lock,
                                     std::chrono::milliseconds(wait_time_ms),
                                     [&waiter] { return waiter->completed.load(std::memory_order_acquire); });
    }
                         ...
                         bool read_through, optional_yield y)
{
  ...
  try { 
    ...
    if (restore_status == rgw::sal::RGWRestoreStatus::RestoreAlreadyInProgress) {
      if (read_through) {
        int64_t timeout_ms = s->cct->_conf.get_val<int64_t>("rgw_read_through_timeout_ms");
        return wait_for_restore_completion(s, dpp, timeout_ms, nullptr, y);
      } else {
	...
	ldpp_dout(dpp, -1) << "failed to fetch tier placement handle, ret = " << op_ret << dendl;
      ...
      if (read_through && rgw::restore::g_restore_waiter_registry) {
        int64_t timeout_ms = s->cct->_conf.get_val<int64_t>("rgw_read_through_timeout_ms");
        if (timeout_ms > 0) {
          waiter = rgw::restore::g_restore_waiter_registry->register_waiter(
            s->bucket->get_key(),
	...
	ldpp_dout(dpp, 0) << "Restore of object " << s->object->get_key() << " failed" << op_ret << dendl;
      ...
      if (read_through) {
        int64_t timeout_ms = s->cct->_conf.get_val<int64_t>("rgw_read_through_timeout_ms");
        return wait_for_restore_completion(s, dpp, timeout_ms, waiter, y);
      }
      return op_ret;
    }
