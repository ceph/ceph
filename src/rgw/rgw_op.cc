...
int RGWGetObj::get_lua_filter(std::unique_ptr<RGWGetObj_Filter>* filter, RGWGetObj_Filter* cb) {
  ...
  const auto rc = rgw::lua::read_script(s, s->penv.lua.manager.get(), s->bucket_tenant, s->yield, rgw::lua::context::getData, scripts_meta);
  if (rc == -ENOENT) {
  ...
  filter->reset(new rgw::lua::RGWGetObjFilter(s, scripts_meta, cb));
  return 0;
}
...
int RGWPutObj::get_lua_filter(std::unique_ptr<rgw::sal::DataProcessor>* filter, rgw::sal::DataProcessor* cb) {
  rgw::lua::LuaRuntimeMeta scripts_meta;
  const auto rc = rgw::lua::read_script(s, s->penv.lua.manager.get(), s->bucket_tenant, s->yield, rgw::lua::context::putData, scripts_meta);
  if (rc == -ENOENT) {
    // no script, nothing to do
    return 0;
  } else if (rc < 0) {
  ...
  filter->reset(new rgw::lua::RGWPutObjFilter(s, scripts_meta, cb));
  return 0;
}
