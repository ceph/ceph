...
public:
  ...
  virtual int error_handler(int err_no, std::string* error_content, optional_yield y, bool null_vid);
...
public:
  ...
  virtual void execute(optional_yield y, bool null_vid) = 0;
...
public:
  ...
  int read_user_manifest_part(
    ...
    const off_t end_ofs,
    bool swift_slo);
  int handle_user_manifest(const char *prefix, optional_yield y, bool null_vid);
  int handle_slo_manifest(bufferlist& bl, optional_yield y);

  int get_data_cb(bufferlist& bl, off_t ofs, off_t len);

  virtual int get_params(optional_yield y) = 0;
  virtual int send_response_data_error(optional_yield y, bool null_vid) = 0;
...
public:
  ...
  public:
    ...
    bool verify_permission(RGWBucketInfo& binfo,
			   ...
			   optional_yield y);
    bool delete_single(const acct_path_t& path, optional_yield y, bool null_vid);
    bool delete_chunk(const std::list<acct_path_t>& paths, optional_yield y, bool null_vid);
  };
...
public:
  ...
  void pre_exec() override;
  void execute(optional_yield y, bool null_vid) override;
  int handle_slo_manifest(bufferlist& bl, optional_yield y, bool null_vid);
...
public:
  ...
  int verify_permission(optional_yield y) override;
  void pre_exec() override;
  void execute(optional_yield, bool null_vid) override = 0;
...
public:
  ...
  void pre_exec();
  void execute(optional_yield y, bool null_vid);
