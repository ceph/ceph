...
int RGWRestoreObj_ObjStore_S3::get_params(optional_yield y)
{
  ...
  ldpp_dout(this, 10) << "I am here: " << dendl;
  try
  {
    RGWXMLDecoder::decode_xml("RestoreRequest", request, &parser);
    ldpp_dout(this, 10) << "RestoreObjectRequest: " << request.days << dendl;
  }
  catch (RGWXMLDecoder::err &err)
  {
  ...
  if (request.days)
  {
    ...
    if (request.output_location)
    {
      ...
      {
        bucket_name = request.output_location->s3.bucket_name;
...
void RGWRestoreObj_ObjStore_S3::send_response()
{
  ...
  set_req_state_err(s, op_ret);
  dump_errno(s);
  dump_header(s, "x-amz-restore-output-path", s->bucket_name);
  end_header(s, this);
}
