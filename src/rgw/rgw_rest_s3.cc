...
int RGWCopyObj_ObjStore_S3::get_params(optional_yield y)
{
  ...
  if (!s->bucket->get_info().obj_lock_enabled() && (obj_lock_mode_str || obj_lock_date_str || obj_legal_hold_str)) {
    s->err.message = "ERROR: object retention or legal hold can't be set if bucket object lock not configured";
    ldpp_dout(this,0) << s->err.message << dendl;
    return -ERR_INVALID_REQUEST;
  }
