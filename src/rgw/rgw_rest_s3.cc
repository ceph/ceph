  ...
  optional_yield y) const
{
  ...
  auto apl = apl_factory->create_apl_local(cct, s, user_info, k.subuser, boost::none, std::string(_access_key_id));
  return result_t::grant(std::move(apl), completer_factory(k.key));
}

rgw::auth::RemoteApplier::AuthInfo
rgw::auth::s3::STSEngine::get_creds_info(const STS::SessionToken& token, const std::string access_key_id) const noexcept
{
  using acct_privilege_t = \
    rgw::auth::RemoteApplier::AuthInfo::acct_privilege_t;
  ...
  optional_yield y) const
{
  ...
  if (token.acct_type == TYPE_KEYSTONE || token.acct_type == TYPE_LDAP) {
    auto apl = remote_apl_factory->create_apl_remote(cct, s, get_acl_strategy(),
                                            get_creds_info(token, std::string(_access_key_id)));
    return result_t::grant(std::move(apl), completer_factory(boost::none));
