...
void RGWOp_MDLog_Lock::execute(optional_yield y) {
  ...
  if (period.empty()) {
    ldpp_dout(s, 5) << "Missing period id trying to use current" << dendl;
    period = store->get_zone()->get_current_period_id();
  }
...
#define LARGE_ENOUGH_BUF (128 * 1024)
  ...
  char* buf = data.c_str();
  ldpp_dout(s, 20) << __func__ << "(): read data: " << buf << dendl;
...
void RGWOp_DATALog_List::execute(optional_yield y) {
  ...
  // entry listed
  op_ret = static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->datalog_rados->list_entries(this, shard_id,
						     max_entries, entries,
						     marker, &last_marker,
						     &truncated);
}
...
void RGWOp_DATALog_ShardInfo::execute(optional_yield y) {
  ...
  op_ret = static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->datalog_rados->get_info(this, shard_id, &info);
}
...
void RGWOp_DATALog_Delete::execute(optional_yield y) {
  ...
  op_ret = static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->datalog_rados->trim_entries(this, shard_id, marker);
}
...
void RGWOp_MDLog_Status::execute(optional_yield y)
{
  ...
  if (sync == nullptr) {
    ldpp_dout(s, 1) << "no sync manager" << dendl;
    op_ret = -ENOENT;
    return;
  }
...
void RGWOp_DATALog_Status::execute(optional_yield y)
{
  ...
  if (sync == nullptr) {
    ldpp_dout(s, 1) << "no sync manager for source-zone " << source_zone << dendl;
    op_ret = -ENOENT;
    return;
  }
  op_ret = sync->read_sync_status(this, &status);
