...
void RGWSTSGetSessionToken::execute(optional_yield y, bool null_verid)
{
...
void RGWSTSAssumeRoleWithWebIdentity::execute(optional_yield y, bool null_verid)
{
  if (op_ret = get_params(); op_ret < 0) {
    return;
  }
...
void RGWSTSAssumeRole::execute(optional_yield y, bool null_verid)
{
  if (op_ret = get_params(); op_ret < 0) {
    return;
  }
