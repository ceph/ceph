...
SIProvider_REST::SIProvider_REST(const DoutPrefixProvider *_dpp,
  ...
  sip_cr_mgr.reset(new SIProviderCRMgr_REST(_dpp->get_cct(), conn, http_manager));
...
{
  return cr_mgr->run(dpp, sip_cr_mgri->init_cr(dpp));
}
...
{
  int r = cr_mgr->run(dpp, sip_cr_mgri->get_next_stage_cr(dpp, sid, next_sid));
  if (r < 0) {
...
std::vector<SIProvider::stage_id_t> SIProvider_REST::get_stages(const DoutPrefixProvider *dpp)
{
  ...
  int r = cr_mgr->run(dpp, sip_cr_mgri->get_stages_cr(dpp, &stages));
  if (r < 0) {
...
{
  int r = cr_mgr->run(dpp, sip_cr_mgri->get_stage_info_cr(dpp, sid, sinfo));
  if (r < 0) {
...
{
  int r = cr_mgr->run(dpp, sip_cr_mgri->fetch_cr(dpp, sid, shard_id, marker, max, result));
  if (r < 0) {
    ldpp_dout(dpp, 0) << "ERROR: failed to fetch data from remote sip: r=" << r << dendl;
    return r;
  }
...
{
  rgw_sip_pos pos;
  int r = cr_mgr->run(dpp, sip_cr_mgri->get_start_marker_cr(dpp, sid, shard_id, &pos));
  if (r < 0) {
    ldpp_dout(dpp, 0) << "ERROR: failed to fetch stages: r=" << r << dendl;
    return r;
  }
...
{
  rgw_sip_pos pos;
  int r = cr_mgr->run(dpp, sip_cr_mgri->get_cur_state_cr(dpp, sid, shard_id, &pos, disabled));
  if (r < 0) {
    ldpp_dout(dpp, 0) << "ERROR: failed to fetch stages: r=" << r << dendl;
    return r;
  }
...
{
  int r = cr_mgr->run(dpp, sip_cr_mgri->trim_cr(dpp, sid, shard_id, marker));
  if (r < 0) {
    ldpp_dout(dpp, 0) << "ERROR: failed to fetch stages: r=" << r << dendl;
    return r;
  }
