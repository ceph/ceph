...
struct bucket_index_normal_layout {
  ...
  // specifies that the fewest number of shards this bucket layout
  // allows
...
inline uint32_t min_layout_shards(const bucket_index_normal_layout& index) {
  // old buckets used num_shards=0 to mean 1
  return index.min_num_shards > 0 ? index.min_num_shards : 1;
}
...
inline uint32_t current_min_layout_shards(const BucketLayout& layout) {
  if (layout.current_index.layout.type == BucketIndexType::Normal) {
    return min_layout_shards(layout.current_index.layout.normal);
  } else {
    return 1;
  }
}
