...
namespace rgw::dedup {
    ...
    if (n != std::string::npos) {
      ...
      if (copy_size <= MAX_PART_LEN) {
        ...
        if (nbytes <= MAX_UINT16_DIGITS) {
          if (dec2int(buff, buff+nbytes, &num_parts)) {
  ...
  void worker_stats_t::dump(Formatter *f) const
  {
...

    {
      ...
      if(this->ingress_failed_bad_etag) {
        f->dump_unsigned("Ingress failed bad etag", this->ingress_failed_bad_etag);
      }
    }
  }
  ...
  void md5_stats_t::dump(Formatter *f) const
  {
...

    {
      ...
      if(this->ingress_failed_bad_etag) {
        f->dump_unsigned("Failed bad etag",this->ingress_failed_bad_etag);
      }
