...
auto handle_head(boost::intrusive_ptr<rgw::h3::Connection> conn,
    ...
    -> asio::awaitable<void, rgw::h3::Listener::executor_type>
{
  std::cout << "HEAD sending content-length:" << data.size << '\n';

  http::fields response;
  response.set(":status", "200");
  response.set(http::field::server, "h3_zero");
  response.set(http::field::content_length, std::to_string(data.size));
...
auto handle_get(boost::intrusive_ptr<rgw::h3::Connection> conn,
    ...
    -> asio::awaitable<void, rgw::h3::Listener::executor_type>
{
  std::cout << "GET sending " << data.size << " bytes of response body...\n";
  ...
  bool fin = (data.size == 0); // no body?
  co_await conn->write_response(stream, response, fin);

  std::vector<unsigned char> zeroes;
  zeroes.resize(std::min<std::size_t>(data.size, 64*1024), '\0');
...
struct ZeroServer {
  ...
  ZeroServer(rgw::h3::Listener::executor_type ex) : ex(ex) {}
...
int main(int argc, char** argv)
{
  ...
  library_options.add_options()
      ...
      ("cc-alg", po::value(&o.cc_algorithm),
       "congestion control algorithm: cubic (default), reno, bbr")
      ;
  ...
  rgw::h3::StreamHandler on_new_stream = ZeroServer{ex};
  std::unique_ptr<rgw::h3::Listener> listener;
  try {
    listener = create_h3_listener(observer, *config, ex,
