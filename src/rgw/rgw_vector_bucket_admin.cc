                                             ...
                                             const DoutPrefixProvider* dpp)
{
  ...
  ret = get_index_cache_stats(dpp, op_state.uid.tenant, op_state.bucket_name,
                              index_cache_stats);
  if (ret == -ENOENT) {
    ret = 0;
  } else if (ret < 0) {
    ...
    ret = get_metadata_cache_stats(dpp, op_state.uid.tenant, op_state.bucket_name,
  ...
  encode_json("tenant", op_state.uid.tenant, formatter);
                                          ...
                                          const DoutPrefixProvider* dpp)
{
  ...
  for (const auto& bucket : listing.buckets) {
    ...
    ret = get_index_cache_stats(dpp, bucket.bucket.tenant, bucket.bucket.name,
                                index_cache_stats);
    if (ret == -ENOENT) {
      continue;
    }
    ...
    ret = get_metadata_cache_stats(dpp, bucket.bucket.tenant, bucket.bucket.name,
                                   metadata_cache_stats);
    if (ret < 0) {
      return ret;
    }
    ...
    formatter->open_object_section("");
    encode_json("tenant", bucket.bucket.tenant, formatter);
                                           ...
                                           optional_yield y)
{
  ...
  ret = delete_session(dpp, op_state.uid.tenant, op_state.bucket_name);
