...
public:
  ...
  /* Send the HTTP connection headers indicating whether to close or keep-alive
     the connection.

     Limitations:
     Some frontends like FCGI will have to use their own module
     system
   */
  virtual size_t send_connection_header(bool force_close) {
    return 0;
  }
...
public:
  ...
  size_t send_connection_header(bool force_close) {
    return get_decoratee().send_connection_header(force_close);
  }
