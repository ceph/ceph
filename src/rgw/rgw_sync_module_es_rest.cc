...
public:
  ...
  void pre_exec(const jaeger_tracing::jspan* const parent_span = nullptr) override;
...
void RGWMetadataSearchOp::pre_exec(const jaeger_tracing::jspan* const parent_span)
{
  rgw_bucket_object_pre_exec(s);
}

void RGWMetadataSearchOp::execute(const jaeger_tracing::jspan* const parent_span)
{
  op_ret = get_params();
  if (op_ret < 0)
    return;
