			   ...
			   RGWMPObj& mp_obj)
{
  ...
  do {
    ...
    if (ret < 0) {
      ldpp_dout(this, 20) << __func__ << ": list_multipart_parts returned " <<
	ret << dendl;
  ...
  if (ret < 0) {
    ldpp_dout(this, 5) << __func__ << ": gc->send_chain() returned " << ret << dendl;
    if (ret == -ENOENT) {
  ...
  if (ret < 0) {
    ldpp_dout(this, 20) << __func__ << ": del_op.delete_obj returned " <<
      ret << dendl;
  }
  return (ret == -ENOENT) ? -ERR_NO_SUCH_UPLOAD : ret;
}
