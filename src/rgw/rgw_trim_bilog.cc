...
RGWCoroutine* BucketTrimManager::create_bucket_trim_cr(RGWHTTPManager *http, const DoutPrefixProvider *dpp)
{
  return new BucketTrimPollCR(impl->store, http, impl->config,
                              impl.get(), impl->status_obj, dpp);
}

RGWCoroutine* BucketTrimManager::create_admin_bucket_trim_cr(RGWHTTPManager *http, const DoutPrefixProvider *dpp)
{
