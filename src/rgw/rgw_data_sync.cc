      ...
      do {
        ...
        for (; iter != entries.end(); ++iter) {
          ...
          if (!marker_tracker->start(iter->first, total_entries, entry_timestamp)) {
            ...
            ldout(cct, 4) << __PRETTY_FUNCTION__ << " call sync_single_entry() on: " 
              << iter->first << dendl;
      ...
      do {
        ...
        if (error_retry_time <= ceph::coarse_real_clock::now()) {
          ...
          for (; iter != error_entries.end(); ++iter) {
            ...
            ldout(cct, 4) << __PRETTY_FUNCTION__ << " call sync_single_entry() handle error key: " 
              << to_string(source_bs, gen) << dendl;
        ...
        for (log_iter = log_entries.begin(); log_iter != log_entries.end(); ++log_iter) {
          ...
          if (!marker_tracker->start(log_iter->log_id, 0, log_iter->log_timestamp)) {
            ...
            ldout(cct, 4) << __PRETTY_FUNCTION__ << " call sync_single_entry(). data log: " 
              << log_iter->log_id << dendl;
    ...
    reenter(this) {
      yield {
        ...
        if (sync_env->sync_module->should_full_sync()) {
          const auto max_marker = marker_mgr.get(sync_pair.dest_bs.shard_id, "");
      ...
      ldpp_dout(sync_env->dpp, 20) << "status: " << state_to_string(status.state) << " and marker: " <<
        status.inc_marker.position << " written to object: " << sync_status_oid << dendl;
      return set_cr_done();
    }
    return 0;
  }
...
int RGWReadBucketPipeSyncStatusCoroutine::operate()
{
  reenter(this) {
    ...
    ldout(sync_env->cct, 20) << "status from attributes for status object: '" << oid <<
      "' is: " << state_to_string(status->state) << dendl;
    return set_cr_done();
  }
  return 0;
}
...
class InitBucketShardStatusCR : public RGWCoroutine {
 ...
 public:
  InitBucketShardStatusCR(RGWDataSyncCtx* sc,
                         const rgw_bucket_sync_pair_info& pair,
                         const BucketIndexShardsManager& marker_mgr)
    : RGWCoroutine(sc->cct), sc(sc), pair(pair), marker_mgr(marker_mgr)
  {}
  int operate() {
    reenter(this) {
      ...
      if (retcode >= 0) {
        ldpp_dout(sc->env->dpp, 20) << "per shard status object created (exclusive). marker: " 
          << status.inc_marker.position << ". for: " << pair.source_bs.get_key() << dendl;
        return set_cr_done();
      ...
      while (--tries) {
        ...
        yield call(new InitCR(sc, pair, status, marker_mgr, objv, exclusive));
        if (retcode >= 0) {
          ldpp_dout(sc->env->dpp, 20) << "per shard status object created (inexclusive). marker: " 
            << status.inc_marker.position << ". for: " << pair.source_bs.get_key() << dendl;
          return set_cr_done();
...
class InitBucketShardStatusCollectCR : public RGWShardCollectCR {
 ...
 public:
  InitBucketShardStatusCollectCR(RGWDataSyncCtx* sc,
                                 ...
                                 int num_shards)
    : RGWShardCollectCR(sc->cct, max_concurrent_shards),
      sc(sc), sync_pair(sync_pair), marker_mgr(marker_mgr), num_shards(num_shards)
  {}
  ...
  bool spawn_next() override {
    ...
    spawn(new InitBucketShardStatusCR(sc, sync_pair, marker_mgr), false);
    return true;
  }
};
...
public:
  ...
  int operate() override {
    reenter(this) {
      ...
      if (retcode < 0) {
        ldpp_dout(sync_env->dpp, 0) << "ERROR: failed to parse bilog shard markers: "
            << cpp_strerror(retcode) << dendl;
        return set_cr_error(retcode);
      }
      ...
      if (status.state != BucketSyncState::Incremental) {
        ...
        yield call(new InitBucketShardStatusCollectCR(sc, sync_pair, marker_mgr, num_shards));
        if (retcode < 0) {
          ldpp_dout(sync_env->dpp, 20) << "ERROR: failed to collect init status. from: " << sync_pair << ". error:"
            << cpp_strerror(retcode) << dendl;
          return set_cr_error(retcode);
        }
        ldpp_dout(sync_env->dpp, 20) << "collected init status. from: " << sync_pair.source_bs.get_key() << dendl;
      ...
      if (retcode < 0) {
        ldout(cct, 20) << "failed to write bucket shard status to sync object: " 
          << status_obj.oid << ". error" 
          << cpp_strerror(retcode)  << dendl;
        return set_cr_error(retcode);
      }
      ldpp_dout(sync_env->dpp, 20) << "writing bucket shard status: " << status.state 
        <<  " to sync object: " << status_obj.oid << ". for: " << sync_pair.source_bs.get_key() << dendl;
      return set_cr_done();
    }
    return 0;
  }
...
int RGWBucketShardIncrementalSyncCR::operate()
{
  ...
  reenter(this) {
    do {
      ...
      ldpp_dout(sync_env->dpp, 20) << "listing bilog for incremental sync. marker: " << 
        sync_info.inc_marker.position << ". for: " << bs.get_key() << dendl;
      ...
      for (; entries_iter != entries_end; ++entries_iter) {
        ...
        if (e.op == RGWModifyOp::CLS_RGW_OP_SYNCSTOP) {
          tn->log(20, SSTR("syncstop at: " << e.timestamp));
          syncstopped = true;
          entries_end = std::next(entries_iter); // stop after this entry
          ldpp_dout(sync_env->dpp, 20) << "syncstop at: " << e.timestamp << " marker: " 
            << e.id << " end marker: " << entries_end->id << ". for: " << bs.get_key() << dendl;
          break;
        }
      ...
      for (; entries_iter != entries_end; ++entries_iter) {
        ...
        if (entry->op == RGWModifyOp::CLS_RGW_OP_SYNCSTOP || entry->op == RGWModifyOp::CLS_RGW_OP_RESYNC) {
          ldpp_dout(sync_env->dpp, 20) << "detected syncstop or resync on " << entries_iter->timestamp << ", skipping entry: " << entry->id << dendl;
  ...
  reenter(this) {
	...
	target_num_shards = siter->target.get_bucket_info().layout.current_index.layout.normal.num_shards;
      ...
      for (; num_shards > 0; --num_shards, ++cur_shard) {
        ...
        ldpp_dout(sync_env->dpp, 20) << __PRETTY_FUNCTION__ << " calling sync_bucket_shard_cr() on: " 
          << sync_pair << dendl;
...
{
  reenter(this) {
    yield call(new RGWReadBucketPipeSyncStatusCoroutine(sc, sync_pair, &sync_info, &objv_tracker));
    if (retcode < 0 && retcode != -ENOENT) {
      tn->log(0, SSTR("ERROR: failed to read sync status for bucket. error: " << retcode));
      return set_cr_error(retcode);
    }

    tn->log(20, SSTR("sync status for source bucket shard: " << to_string(sync_info.state)));
    sync_info.state = rgw_bucket_shard_sync_info::StateIncrementalSync;
    if (progress) {
      *progress = sync_info.inc_marker.timestamp;
    }
    ...
    tn->log(20, SSTR("sync status for source bucket changed after init to: " << to_string(sync_info.state)));
    ldpp_dout(sync_env->dpp, 20) <<  "sync status changed after init to: " << sync_info.state << " for: " 
      << sync_pair.source_bs.get_key() << dendl;
    if (sync_info.state == rgw_bucket_shard_sync_info::StateStopped) {
...
int RGWSyncBucketCR::operate()
{
  reenter(this) {
    ...
    if (retcode < 0) {
      tn->log(0, SSTR("ERROR: failed to retrieve source bucket info for bucket=" << bucket_str{sync_pair.source_bs.bucket}));
      ldpp_dout(env->dpp, 20) << "failed to retrieve source bucket info: " 
        << bucket_str{sync_pair.source_bs.bucket} << ". error: " << retcode << dendl;
      return set_cr_error(retcode);
    }
    ...
    if (retcode < 0) {
      ...
      ldpp_dout(env->dpp, 20) << "failed to retrieve destination bucket info. error: " 
        << retcode << dendl;
      return set_cr_error(retcode);
    }
    ...
    if (retcode == -ENOENT) {
      ldpp_dout(env->dpp, 20) << "create bucket sync status for: " << sync_pair.source_bs.get_key() << dendl;
      ...
      if (retcode == -EEXIST) {
        ldpp_dout(env->dpp, 20) << "bucket sync status creation race. for: " << sync_pair << dendl;
    ...
    if (retcode < 0) {
      ldpp_dout(env->dpp, 20) << "failed to read bucket sync status for: " 
        << sync_pair << ". error: " << retcode << dendl;
      return set_cr_error(retcode);
    }

    do {
      ldpp_dout(env->dpp, 20) << "sync status at begining of loop for: " 
        << sync_pair.source_bs.get_key()<< ". is: " << bucket_status.state << dendl;
      ...
      if (bucket_status.state == BucketSyncState::Init || 
        ...
        if (retcode < 0) {
          lderr(cct) << "failed to read remote bilog info. for: " << sync_pair.source_bs.bucket << ":" << sync_pair.source_bs.shard_id 
            << ". error: "  << retcode << dendl;
          return set_cr_error(retcode);
        }
        if (!bucket_lease_cr) {
          ...
          while (!bucket_lease_cr->is_locked()) {
            if (bucket_lease_cr->is_done()) {
              ...
              set_status("lease lock failed, early abort");
              drain_all();
              ldpp_dout(env->dpp, 20) << "failed to take lease for: " << sync_pair 
                << ". error: " << bucket_lease_cr->get_ret_status() << dendl;
          ...
          if (retcode < 0) {
            ...
            ldpp_dout(env->dpp, 20) << "failed to read sync status after lock is taken. for: " 
              << sync_pair << ". error: " << retcode << dendl;
            return set_cr_error(retcode);
          }
        }
        if (info.syncstopped) {
          ldout(cct, 20) << "remote bilog indicates that sync was stopped. for: " 
            << sync_pair.source_bs.get_key() << dendl;
          RELEASE_LOCK(bucket_lease_cr);
          return set_cr_done();
        }
        yield {
          ...
          call(new InitBucketFullSyncStatusCR(sc, sync_pair, status_obj, bucket_status, objv, num_shards, init_check_compat, info));
        }
        if (retcode < 0) {
          ldpp_dout(env->dpp, 20) << "failed to got sync status for: " << sync_pair 
            << ". error: " << retcode << dendl;
          tn->log(20, SSTR("failed to get status. error: " << retcode));
          RELEASE_LOCK(bucket_lease_cr);
          return set_cr_error(retcode);
        }
        ldpp_dout(env->dpp, 20) << "init sync status for: " << sync_pair.source_bs.get_key()
          << ". is: " << bucket_status.state << dendl;
      }
      ...
      if (bucket_status.state == BucketSyncState::Full) {
        ...
        if (retcode < 0) {
          RELEASE_LOCK(bucket_lease_cr);
          ldpp_dout(env->dpp, 20) << "failed full sync for: " << sync_pair 
            << ". error: " << retcode << dendl;
          return set_cr_error(retcode);
        }
        ldpp_dout(env->dpp, 20) << "full sync ok. for: " << sync_pair << dendl;
      }
      ...
      if (bucket_status.state == BucketSyncState::Incremental) {
        ...
        if (gen) {
          ...
          if (*gen > current_gen) {
            ...
            ldpp_dout(env->dpp, 20) << "requested sync of future generation for: " 
              << sync_pair << ". generation: " << *gen << " > " << current_gen << dendl;
            return set_cr_error(retcode);
          } else if (*gen < current_gen) {
            tn->log(10, SSTR("requested sync of past generation "
                             << *gen << " < " << current_gen
                             << ", returning success"));
            ldpp_dout(env->dpp, 20) << "requested sync of past generation for: " 
              << sync_pair << ". generation: " << *gen << " > " << current_gen << dendl;
            return set_cr_done();
          }
        }
        ...
        yield call(new RGWSyncBucketShardCR(sc, data_lease_cr, sync_pair,
                                            ...
                                            tn, progress));
        if (retcode < 0) {
          ldpp_dout(env->dpp, 20) << "failed incremental sync. for: " 
            << sync_pair << ". error: " << retcode << dendl;
          return set_cr_error(retcode);
        }
        ldpp_dout(env->dpp, 20) << "incremental sync ok. for: " << sync_pair.source_bs.get_key() << dendl;
      }
