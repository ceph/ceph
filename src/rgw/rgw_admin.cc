...
static void get_data_sync_status(const rgw_zone_id& source_zone, list<string>& status, int tab)
{
  ...
  if (!static_cast<rgw::sal::RadosStore*>(driver)->svc()->zone->find_data_provider(source_zone, &sz)) {
                                     ...
                                     int width, std::ostream& out)
{
  ...
  r = rgw_read_bucket_full_sync_status(dpp, driver, pipe, &full_status, &remote_markers, null_yield);
  if (r >= 0) {
