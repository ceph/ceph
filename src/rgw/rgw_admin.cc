...
next:
  ...
  if (opt_cmd == OPT_REPLICALOG_GET) {
    ...
    if (replica_log_type == ReplicaLog_Metadata) {
      ...
      ret = logger.get_bounds(bucket, bounds, replicalog_index_by_instance);
      if (ret < 0)
        return -ret;
  ...
  if (opt_cmd == OPT_REPLICALOG_DELETE) {
    if (replica_log_type == ReplicaLog_Metadata) {
      ...
      RGWReplicaBucketLogger logger(store);
      ret = logger.delete_bound(bucket, daemon_id, replicalog_index_by_instance, false);
      if (ret < 0)
        return -ret;
    }
  }
  ...
  if (opt_cmd == OPT_REPLICALOG_UPDATE) {
    ...
    if (replica_log_type == ReplicaLog_Metadata) {
      ...
      RGWReplicaBucketLogger logger(store);
      ret = logger.update_bound(bucket, daemon_id, marker, time, &entries, replicalog_index_by_instance);
      if (ret < 0) {
        cerr << "ERROR: failed to update bounds: " << cpp_strerror(-ret) << std::endl;
        return -ret;
      }
