...
void usage()
{
  ...
  cout << "  reshardlog list                  list bucket reshard newest generation log\n";
  cout << "  reshardlog purge                 trim all bucket resharding log\n";
 ...
 if (opt_cmd == OPT::RESHARD_STALE_INSTANCES_DELETE) {
  ...
  if (opt_cmd == OPT::RESHARDLOG_LIST) {
    ...
    for (; i < max_shards; i++) {
      ...
      do {
        entries.clear();
        ret = static_cast<rgw::sal::RadosStore*>(driver)->getRados()->reshard_log_list(bs, marker, max_entries,
                                                                                       index.layout.normal.reshard_gen,
                                                                                       &entries, &is_truncated, null_yield);
        if (ret < 0) {
          cerr << "ERROR: reshard_log_list(): " << cpp_strerror(-ret) << std::endl;
          return -ret;
        }
