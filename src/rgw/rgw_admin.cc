...
int check_obj_locator_underscore(rgw::sal::RGWObject* obj, bool fix, bool remove_bad, Formatter *f) {
  ...
  if ((needs_fixing || remove_bad) && fix) {
    ret = static_cast<rgw::sal::RGWRadosStore*>(store)->getRados()->fix_head_obj_locator(dpp(), obj->get_bucket()->get_info(), needs_fixing, remove_bad, obj->get_key());
    if (ret < 0) {
                         ...
                         Formatter *formatter, bool force)
{
  ...
  int ret = realm.init(dpp(), g_ceph_context, static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->sysobj, null_yield);
  if (ret < 0 ) {
  ...
  ret = period.init(dpp(), g_ceph_context, static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->sysobj, realm.get_id(), null_yield);
  if (ret < 0) {
                          ...
                          RGWPeriod *period)
{
  ...
  ret = period->init(dpp(), g_ceph_context, static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->sysobj, null_yield, false);
  if (ret < 0) {
...
static int trim_sync_error_log(int shard_id, const string& marker, int delay_ms)
{
  ...
  for (;;) {
    int ret = static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->cls->timelog.trim(dpp(), oid, {}, {}, {}, marker, nullptr,
					      null_yield);
    if (ret == -ENODATA) {
      return 0;
    }
...
public:
  ...
  int init() {
    int ret = zonegroup.init(dpp(), g_ceph_context, static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->sysobj, null_yield);
    if (ret < 0) {
...
int main(int argc, const char **argv)
{
  ...
  if (raw_storage_op) {
	...
	int ret = period.init(dpp(), g_ceph_context, static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->sysobj, null_yield);
	if (ret < 0) {
	  cerr << "period.init failed: " << cpp_strerror(-ret) << std::endl;
	  return -ret;
	}
	...
	RGWPeriod period(period_id, epoch);
	int ret = period.init(dpp(), g_ceph_context, static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->sysobj, realm_id,
    ...
    case OPT::PERIOD_LIST:
      {
	list<string> periods;
	int ret = static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->zone->list_periods(dpp(), periods);
	if (ret < 0) {
    ...
    case OPT::PERIOD_PULL:
      {
        ...
        if (url.empty()) {
          ...
          ret = current_period.init(dpp(), g_ceph_context, static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->sysobj, null_yield);
          if (ret < 0) {
    ...
    case OPT::GLOBAL_QUOTA_DISABLE:
      {
        ...
        int ret = period_config.read(dpp(), static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->sysobj, realm_id, null_yield);
        if (ret < 0 && ret != -ENOENT) {
        ...
        if (opt_cmd != OPT::GLOBAL_QUOTA_GET) {
          ...
          ret = period_config.write(dpp(), static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->sysobj, realm_id, null_yield);
          if (ret < 0) {
	...
	list<string> realms;
	ret = static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->zone->list_realms(dpp(), realms);
	if (ret < 0) {
	...
	list<string> periods;
	ret = static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->zone->list_periods(dpp(), period_id, periods, null_yield);
	if (ret < 0) {
    ...
    case OPT::REALM_PULL:
      {
        ...
        RGWRealm realm;
        realm.init(dpp(), g_ceph_context, static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->sysobj, null_yield, false);
        try {
	...
	ret = zone.init(dpp(), g_ceph_context, static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->sysobj, null_yield);
	if (ret < 0) {
	...
	RGWZoneGroup zonegroup(zonegroup_id, zonegroup_name);
	int ret = zonegroup.init(dpp(), g_ceph_context, static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->sysobj,
				 null_yield);
	if (ret < 0) {
	  cerr << "failed to init zonegroup: " << cpp_strerror(-ret) << std::endl;
	  return -ret;
	...
	ret = static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->zone->list_zonegroups(dpp(), zonegroups);
	if (ret < 0) {
	...
	RGWZoneGroup zonegroup;
	ret = zonegroup.init(dpp(), g_ceph_context, static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->sysobj,
			     null_yield, false);
	if (ret < 0) {
	  cerr << "failed to init zonegroup: " << cpp_strerror(-ret) << std::endl;
	  return -ret;
	...
	if (!zonegroup_id.empty() || !zonegroup_name.empty()) {
	  ret = zonegroup.init(dpp(), g_ceph_context, static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->sysobj, null_yield);
	  if (ret < 0) {
	...
	RGWZoneParams zone(zone_id, zone_name);
	ret = zone.init(dpp(), g_ceph_context, static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->sysobj, null_yield, false);
	if (ret < 0) {
	  cerr << "unable to initialize zone: " << cpp_strerror(-ret) << std::endl;
	  return -ret;
	}
	...
	RGWZoneParams zone(zone_id, zone_name);
	int ret = zone.init(dpp(), g_ceph_context, static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->sysobj, null_yield);
	if (ret < 0) {
	  cerr << "unable to initialize zone: " << cpp_strerror(-ret) << std::endl;
	  return -ret;
	}
    ...
    case OPT::ZONE_LIST:
      {
	list<string> zones;
	int ret = static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->zone->list_zones(dpp(), zones);
	if (ret < 0) {
  ...
  if (opt_cmd == OPT::LOG_LIST) {
    ...
    RGWAccessHandle h;
    int r = static_cast<rgw::sal::RGWRadosStore*>(store)->getRados()->log_list_init(dpp(), date, &h);
    if (r == -ENOENT) {
      // no logs.
    } else {
      if (r < 0) {
  ...
  if (opt_cmd == OPT::LOG_SHOW || opt_cmd == OPT::LOG_RM) {
    ...
    if (opt_cmd == OPT::LOG_SHOW) {
      ...
      int r = static_cast<rgw::sal::RGWRadosStore*>(store)->getRados()->log_show_init(dpp(), oid, &h);
      if (r < 0) {
...
next:
    ...
    if (opt_cmd == OPT::LOG_RM) {
      int r = static_cast<rgw::sal::RGWRadosStore*>(store)->getRados()->log_remove(dpp(), oid);
      if (r < 0) {
  ...
  if (opt_cmd == OPT::POOL_ADD) {
    ...
    int ret = static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->zone->add_bucket_placement(dpp(), pool, null_yield);
    if (ret < 0)
  ...
  if (opt_cmd == OPT::POOL_RM) {
    ...
    int ret = static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->zone->remove_bucket_placement(dpp(), pool, null_yield);
    if (ret < 0)
  ...
  if (opt_cmd == OPT::POOLS_LIST) {
    set<rgw_pool> pools;
    int ret = static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->zone->list_placement_set(dpp(), pools, null_yield);
    if (ret < 0) {
  ...
  if (opt_cmd == OPT::OLH_GET) {
    ...
    ret = static_cast<rgw::sal::RGWRadosStore*>(store)->getRados()->get_olh(dpp(), bucket->get_info(), obj, &olh);
    if (ret < 0) {
...
#define MAX_LC_LIST_ENTRIES 100
  ...
  if (opt_cmd == OPT::USER_STATS) {
	...
	return EINVAL;
      ...
      ret = static_cast<rgw::sal::RGWRadosStore*>(store)->ctl()->user->reset_stats(dpp(), user->get_id(), null_yield);
      if (ret < 0) {
    ...
    int ret = static_cast<rgw::sal::RGWRadosStore*>(store)->ctl()->user->read_stats(dpp(), user->get_id(), &stats, null_yield,
					     &last_stats_sync,
					     &last_stats_update);
    if (ret < 0) {
  ...
  if (opt_cmd == OPT::MDLOG_AUTOTRIM) {
    ...
    ret = crs.run(dpp(), create_admin_meta_log_trim_cr(dpp(), static_cast<rgw::sal::RGWRadosStore*>(store), &http, num_shards));
    if (ret < 0) {
 ...
 if (opt_cmd == OPT::BUCKET_SYNC_RUN) {
  ...
  if (opt_cmd == OPT::BILOG_LIST) {
    ...
    do {
      ...
      ret = static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->bilog_rados->log_list(dpp(), bucket->get_info(), shard_id, marker, max_entries - count, entries, &truncated);
      if (ret < 0) {
  ...
  if (opt_cmd == OPT::SYNC_ERROR_LIST) {
    ...
    for (; shard_id < ERROR_LOGGER_SHARDS; ++shard_id) {
      ...
      do {
        list<cls_log_entry> entries;
        ret = static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->cls->timelog.list(dpp(), oid, {}, {}, max_entries - count, entries, marker, &marker, &truncated,
					      null_yield);
	if (ret == -ENOENT) {
	  break;
        }
  ...
  if (opt_cmd == OPT::BILOG_TRIM) {
    ...
    ret = static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->bilog_rados->log_trim(dpp(), bucket->get_info(), shard_id, start_marker, end_marker);
    if (ret < 0) {
  ...
  if (opt_cmd == OPT::BILOG_STATUS) {
    ...
    ret = static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->bilog_rados->get_log_status(dpp(), bucket->get_info(), shard_id,
						    &markers, null_yield);
    if (ret < 0) {
  ...
  if (opt_cmd == OPT::DATALOG_STATUS) {
    ...
    for (; i < g_ceph_context->_conf->rgw_data_log_num_shards; i++) {
      ...
      static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->datalog_rados->get_info(dpp(), i, &info);
  ...
  if (opt_cmd == OPT::MFA_CREATE) {
    ...
    int ret = static_cast<rgw::sal::RGWRadosStore*>(store)->ctl()->meta.mgr->mutate(RGWSI_MetaBackend_OTP::get_meta_key(user->get_id()),
					     ...
					     MDLOG_STATUS_WRITE,
					     [&] {
      return static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->cls->mfa.create_mfa(dpp(), user->get_id(), config, &objv_tracker, mtime, null_yield);
    });
    if (ret < 0) {
 ...
 if (opt_cmd == OPT::MFA_REMOVE) {
    ...
    int ret = static_cast<rgw::sal::RGWRadosStore*>(store)->ctl()->meta.mgr->mutate(RGWSI_MetaBackend_OTP::get_meta_key(user->get_id()),
					     ...
					     MDLOG_STATUS_WRITE,
					     [&] {
      return static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->cls->mfa.remove_mfa(dpp(), user->get_id(), totp_serial, &objv_tracker, mtime, null_yield);
    });
    if (ret < 0) {
 ...
 if (opt_cmd == OPT::MFA_GET) {
    ...
    int ret = static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->cls->mfa.get_mfa(dpp(), user->get_id(), totp_serial, &result, null_yield);
    if (ret < 0) {
 ...
 if (opt_cmd == OPT::MFA_LIST) {
    ...
    int ret = static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->cls->mfa.list_mfa(dpp(), user->get_id(), &result, null_yield);
    if (ret < 0) {
 ...
 if (opt_cmd == OPT::MFA_CHECK) {
    ...
    list<rados::cls::otp::otp_info_t> result;
    int ret = static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->cls->mfa.check_mfa(dpp(), user->get_id(), totp_serial, totp_pin.front(), null_yield);
    if (ret < 0) {
 ...
 if (opt_cmd == OPT::MFA_RESYNC) {
    ...
    rados::cls::otp::otp_info_t config;
    int ret = static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->cls->mfa.get_mfa(dpp(), user->get_id(), totp_serial, &config, null_yield);
    if (ret < 0) {
      if (ret == -ENOENT || ret == -ENODATA) {
        cerr << "MFA serial id not found" << std::endl;
      } else {
    ...
    ret = static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->cls->mfa.otp_get_current_time(dpp(), user->get_id(), &now, null_yield);
    if (ret < 0) {
