...
int check_min_obj_stripe_size(rgw::sal::RGWRadosStore *store, RGWBucketInfo& bucket_info, rgw::sal::RGWObject* obj, uint64_t min_stripe_size, bool *need_rewrite)
{
  ...
  try {
    ...
    ldpp_dout(store->ctx(), 0) << "ERROR: failed to decode manifest" << dendl;
    return -EIO;
  }
			      ...
			      std::set<rgw_sync_bucket_entity> *hint_entities)
{
  for (auto& zone_id : zones) {
    for (auto& b : buckets) {
      ...
      if (ret < 0) {
	ldpp_dout(store->ctx(), 20) << "could not init bucket info for hint bucket=" << b << " ... skipping" << dendl;
	continue;
      }
...
static int sync_info(std::optional<rgw_zone_id> opt_target_zone, std::optional<rgw_bucket> opt_bucket, Formatter *formatter)
{
  ...
  for (auto& hint_entity : hint_entities) {
    ...
    if (r < 0) {
      ldpp_dout(store->ctx(), 20) << "could not get bucket sync policy handler for hint bucket=" << hint_bucket << " ... skipping" << dendl;
      continue;
    }
    ...
    if (rc != OATH_INVALID_OTP) {
      ...
      if (rc != OATH_INVALID_OTP) {
        *pofs = time_ofs - step_size + step_size * totp.window / 2;
        ldpp_dout(cct, 20) << "found at time=" << start_time - time_ofs << " time_ofs=" << time_ofs << dendl;
        return 0;
      }
    }
    sign = -sign;
...
next:
  ...
  if (opt_cmd == OPT::OBJECT_REWRITE) {
    ...
    if (min_rewrite_stripe_size > 0) {
      ...
      if (ret < 0) {
        ldpp_dout(store->ctx(), 0) << "WARNING: check_min_obj_stripe_size failed, r=" << ret << dendl;
      }
    }
    if (need_rewrite) {
      ...
      ldpp_dout(store->ctx(), 20) << "skipped object" << dendl;
    }
  }
	...
	--expansion_factor;
	  ...
	  rgw::sal::RGWRadosObject obj(store, key, &rbucket);
          ...
          if (min_rewrite_stripe_size > 0) {
            ...
            if (r < 0) {
              ldpp_dout(store->ctx(), 0) << "WARNING: check_min_obj_stripe_size failed, r=" << r << dendl;
            }
          }
          if (!need_rewrite) {
            formatter->dump_string("status", "Skipped");
