...
void RGWOp_Metadata_Get::execute(optional_yield y, bool null_verid) {
  string metadata_key;
...
void RGWOp_Metadata_Get_Myself::execute(optional_yield y, bool null_verid) {
  string owner_id;

  owner_id = s->owner.get_id().to_str();
  s->info.args.append("key", owner_id);

  return RGWOp_Metadata_Get::execute(y, null_verid);
}

void RGWOp_Metadata_List::execute(optional_yield y, bool null_verid) {
  string marker;
...
void RGWOp_Metadata_Put::execute(optional_yield y, bool null_verid) {
  bufferlist bl;
  string metadata_key;
...
void RGWOp_Metadata_Delete::execute(optional_yield y, bool null_verid) {
  string metadata_key;
