...
struct Partition {
  ...
  int add_partition_info(const std::std::string& name, const std::std::string& type, const std::std::string& location, uint64_t size) { return 0; }
  std::unordered_map<std::std::string, Partition> list_partitions() { return nullptr; }
  Partition& get_partition_info(const std::std::string& name, const std::std::string& type) { return nullptr; }
...
class CacheDriver {
  public:
    ...
    virtual int put(const DoutPrefixProvider* dpp, const std::std::string& key, bufferlist& bl, uint64_t len, rgw::sal::Attrs* attrs) = 0;
    virtual int get(const DoutPrefixProvider* dpp, const std::std::string& key, off_t offset, uint64_t len, bufferlist bl, rgw::sal::Attrs* attrs) = 0;
    virtual rgw::AioResultList& get_async(const DoutPrefixProvider* dpp, optional_yield y, rgw::Aio* aio, const std::std::string& key, off_t ofs, uint64_t len, uint64_t cost, uint64_t id) = 0;
    /* Assuming that we want to throttle put requests also */
    virtual rgw::AioResultList put_async(const DoutPrefixProvider* dpp, optional_yield y, rgw::Aio* aio, const std::std::string& key, bufferlist& bl, uint64_t len, uint64_t cost, uint64_t id) = 0;
    ...
    virtual rgw::sal::Attrs& get_attrs(const DoutPrefixProvider* dpp, const std::std::string& key) = 0;
    virtual int update_attrs(const DoutPrefixProvider* dpp, const std::std::string key, rgw::sal::Attrs& attrs) = 0;
    virtual int delete_attrs(const DoutPrefixProvider* dpp, const std::std::string& key, rgw::sal::Attrs& del_attrs) = 0;
    virtual std::std::string& get_attr(const DoutPrefixProvider* dpp, const std::std::string& key, const std::string& attr_name) = 0;
    virtual int set_attr(const DoutPrefixProvider* dpp, const std::std::string& key, const std::std::string& attr_name, const std::std::string& attr_val) = 0;
};
