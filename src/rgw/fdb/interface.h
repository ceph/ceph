...
inline void set(transaction_handle txn,
                const concepts::key_value_range auto& kvs,
                const commit_after_op commit_after)
{
 return set(txn,
...
inline void set(transaction_handle txn, const concepts::key_value_range auto& kvs)
{
 return set(txn, kvs, commit_after_op::no_commit);
}

inline void set(database_handle dbh, const concepts::key_value_range auto& kvs)
{
 return detail::in_transaction(dbh,
...
{
 const auto nkeys = std::size(keys);

 if (1 == nkeys) {
  return select(*std::begin(keys));
 }

 if (2 == nkeys) {
  return select(*std::begin(keys), *std::next(std::begin(keys)));
 }

 throw libfdb_exception("range selection initializer list requires one or two keys");
}

template <query::expression SelectionT, concepts::string_pair_output_iterator OutIterT>
                                                        ...
                                                        OutIterT& out_iter)
{
 ...
 query::for_each_interval(selection, [&](const ceph::libfdb::select& interval) {
  nread += detail::get_value_range_from_transaction(txn, interval, out_iter);
 });
...
{
 return detail::in_transaction(dbh,
          [&selection, out_iter](transaction_handle& txn) mutable {
            return get(txn, selection, out_iter, commit_after_op::no_commit);
          });
}
...
inline std::size_t get(ceph::libfdb::transaction_handle txn,
                       ...
                       const ceph::libfdb::commit_after_op commit_after)
{
 return get(txn, selection, std::inserter(out, std::end(out)), commit_after);
}
...
{
 return detail::in_transaction(dbh,
          [&selection, &out](transaction_handle& txn) {
            return get(txn, selection, out, commit_after_op::no_commit);
          });
}
...
inline auto selectors_for_selection(ceph::libfdb::select selector)
{
 return std::views::single(std::move(selector));
}

template <query::non_interval_expression QueryT>
inline auto selectors_for_selection(QueryT query)
{
 std::vector<ceph::libfdb::select> selectors;

 query::for_each_interval(query, [&selectors](ceph::libfdb::select selector) {
...
inline auto scan(ceph::libfdb::transaction_handle txn, SelectionT selection)
  -> std::generator<std::pair<std::string, ValueT>>
{
 for (auto& selector : detail::selectors_for_selection(std::move(selection))) {
  co_yield std::ranges::elements_of(detail::scan_selector<ValueT>(txn, std::move(selector)));
 }
}
 ...
 -> std::generator<AssocT>
{
 for (auto& selector : detail::selectors_for_selection(std::move(selection))) {
  co_yield std::ranges::elements_of(
   detail::blocks_selector<ValueT, AssocT>(dbh, std::move(selector)));
 }
}
