...
int read_limit(const select_options& options) noexcept;
int target_bytes(const select_options& options) noexcept;
FDBStreamingMode streaming_mode(const select_options& options) noexcept;
void read_limit(select_options& options, int limit) noexcept;

} // namespace detail

struct select_options final
{
    bool reverse_order = false; // should we return results in reverse order?

 private:
    struct db_options final
    {
        int read_limit_ = 0; // max result pairs per FDB read; 0 == unlimited
        int target_bytes_ = 0; // FDB byte target hint per range read; 0 == default
        ...
        FDBStreamingMode streaming_mode_ = FDB_STREAMING_MODE_ITERATOR;
    };

    db_options db;

 private:
    friend int detail::read_limit(const select_options& options) noexcept;
    friend int detail::target_bytes(const select_options& options) noexcept;
    friend FDBStreamingMode detail::streaming_mode(const select_options& options) noexcept;
    friend void detail::read_limit(select_options& options, int limit) noexcept;
};
...
inline int read_limit(const select_options& options) noexcept
{
    return options.db.read_limit_;
}

inline int target_bytes(const select_options& options) noexcept
{
    return options.db.target_bytes_;
}

inline FDBStreamingMode streaming_mode(const select_options& options) noexcept
{
    return options.db.streaming_mode_;
}

inline void read_limit(select_options& options, const int limit) noexcept
{
    options.db.read_limit_ = limit;
}
...
inline future_value get_range_future_from_transaction(ceph::libfdb::transaction& txn, const ceph::libfdb::select& selection, int iteration)
{
  ...
  return future_value(fdb_transaction_get_range(txn.raw_handle(),
                      ...
                      detail::read_limit(options),                              // limit (0 == unlimited)
                      detail::target_bytes(options),                            // target bytes (0 == unlimited)
                      detail::streaming_mode(options),                          // streaming mode (e.g.: FDB_STREAMING_MODE_WANT_ALL)
