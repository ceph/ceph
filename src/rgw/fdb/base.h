...
concept string_pair_output_range =
 ...
 requires(RangeT& out) {
  requires string_pair_output_iterator<decltype(std::inserter(out, std::end(out)))>;
 };
...
concept supported_invocation_result =
 std::is_void_v<T> or storable_invocation_result<T>;
...
inline std::size_t get_value_range_from_transaction(transaction& txn, const select& key_range, OutIterT& out_iter)
{
  ...
  *out_iter++ = to_decoded_kv_pair<std::string>(kv);
  ++nread;
 }
