...
RGWPeriodPusher::RGWPeriodPusher(const DoutPrefixProvider *dpp, rgw::sal::RGWStore* store,
				 optional_yield y)
  : cct(store->ctx()), store(store)
{
  ...
  RGWPeriod period;
  // XXX dang
  int r = period.init(dpp, cct, static_cast<rgw::sal::RGWRadosStore *>(store)->svc()->sysobj, realm_id, y, realm.get_name());
  if (r < 0) {
