			  ...
			  bool run_sync)
{
  ...
  sysobj_core->core_init(rgwrados, zone.get());
  if (have_cache) {
    sysobj_cache->init(rgwrados, zone.get(), notify.get());
    sysobj->init(rgwrados, sysobj_cache.get());
  } else {
    sysobj->init(rgwrados, sysobj_core.get());
  }
