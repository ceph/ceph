...
void RGWDeletePolicyVersion::execute(optional_yield y)
{
  std::string policy_name = arn.resource.substr(arn.resource.rfind('/') + 1);
  constexpr bool exclusive = false;
  op_ret = driver->delete_policy_version(this, y, arn.account, policy_name, version_id, exclusive);
  if(op_ret < 0) {
...
{
  std::string policy_name = arn.resource.substr(arn.resource.rfind('/') + 1);
  op_ret = driver->untag_policy(this, y, arn.account, policy_name, keys);
  if(op_ret < 0) {
...
void RGWListPolicyTags::execute(optional_yield y)
{
  ...
  listing.next_marker = marker;
  std::string policy_name = arn.resource.substr(arn.resource.rfind('/') + 1);
  op_ret = driver->list_policy_tags(this, y, arn.account, policy_name,
                      listing.next_marker, max_items, listing);
  if (op_ret == -ENOENT) {
    op_ret = 0;
  } else if (op_ret < 0) {
