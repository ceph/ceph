                       ...
                       std::string &err)
{
  ...
  if (account.max_policy < 0)
  { // max_roles < 0 means unlimited
    return 0;
  }

  uint32_t count = 0;
  r = driver->count_account_policy(dpp, y, account_id, count);
  if (r < 0) {
    ldpp_dout(dpp, 4) << "failed to count roles for iam account "
                      << account_id << ": " << cpp_strerror(r) << dendl;
    return r;
  }
  if(std::cmp_greater_equal(count, account.max_policy)) {
    err = fmt::format("Role limit {} exceeded", account.max_policy);
...
void RGWPutIAMPolicy::execute(optional_yield y) {
  ...
  int r = driver->load_account_by_id(this, y, account_id, account, attrs, objv);
  if (r < 0)
  {
  ...
  std::unique_ptr<rgw::sal::RGWCustomerManagedPolicy> policy = driver->get_policy(
      policy_name,
      user_tenant,
      account_id,
      policy_path,
      ...
      default_version,
      tags);
    //TODO: if site is master forward to master
  std::string policy_id = "";
  int op_ret = policy->create(s, policy_id, y, account, attrs, objv);
  if(op_ret == 0){
    // TODO: Formatter in success
    ldpp_dout(this, 5) << "RAJA RGWPutIAMPolicy::executed sucessfully. policy_id: " << policy_id << dendl;
  }
}
