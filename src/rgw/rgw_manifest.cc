...
rgw_raw_obj rgw_obj_select::get_raw_obj(const RGWZoneGroup& zonegroup,
                                        const RGWZoneParams& zone) const
{
  if (!is_raw) {
    rgw_raw_obj raw;
    obj.get_full_oid_and_loc(raw.oid, raw.loc);
    rgw_get_obj_data_pool(zonegroup, zone, placement_rule, obj, &raw.pool);
    return raw;
  }
  return raw_obj;
}

rgw_raw_obj rgw_obj_select::get_raw_obj(RGWSI_Zone* zone_svc) const
{
  return get_raw_obj(zone_svc->get_zonegroup(), zone_svc->get_zone_params());
}
                           ...
                           const RGWZoneParams& zone)
{
  if (explicit_objs || m.explicit_objs) {
    return append_explicit(m, zonegroup, zone);
  }
...
void RGWObjManifest::convert_to_explicit(const RGWZoneGroup& zonegroup,
                                         const RGWZoneParams& zone)
{
  if (explicit_objs) {
    return;
  ...
  while (iter != obj_end()) {
    ...
    const rgw_raw_obj& raw_loc = os.get_raw_obj(zonegroup, zone);
    part.loc_ofs = 0;
...
{
  if (!explicit_objs) {
    convert_to_explicit(zonegroup, zone);
  }
  if (!m.explicit_objs) {
    m.convert_to_explicit(zonegroup, zone);
  }
