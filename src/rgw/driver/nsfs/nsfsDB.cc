...
int NSFSUserDB::ProcessOp(const DoutPrefixProvider *dpp, string_view Op, DBOpParams *params) {
  int ret = -1;
...
int NSFSUserDB::Initialize(string logfile, int loglevel)
{
  int ret = -1;
  ...
  if (ret) {
    ldpp_dout(dpp, 0) <<"InitializeNSFSUserDBOps failed " << dendl;
    closeDB(dpp);
    db = NULL;
    return ret;
  }

  ldpp_dout(dpp, 0) << "NSFSUserDB successfully initialized - name:" \
    << db_name << "" << dendl;
...
int NSFSUserDB::Destroy(const DoutPrefixProvider *dpp)
{
  DB::Destroy(dpp);

  ldpp_dout(dpp, 20)<<"NSFSUserDB successfully destroyed - name:" \
    <<db_name << dendl;

  return 0;
}

int NSFSAccountDB::ProcessOp(const DoutPrefixProvider *dpp, string_view Op, DBOpParams *params) {
  int ret = -1;
  shared_ptr<class DBOp> db_op;
...
int NSFSAccountDB::Initialize(string logfile, int loglevel)
{
  int ret = -1;
  const DoutPrefixProvider *dpp = get_def_dpp();
  ...
  if (ret) {
    ldpp_dout(dpp, 0) <<"InitializeNSFSAccountDBOps failed " << dendl;
    closeDB(dpp);
    db = NULL;
    return ret;
  }

  ldpp_dout(dpp, 0) << "NSFSAccountDB successfully initialized - name:" \
    << db_name << "" << dendl;

  return ret;
}

int NSFSAccountDB::Destroy(const DoutPrefixProvider *dpp)
{
  DB::Destroy(dpp);

  ldpp_dout(dpp, 20)<<"NSFSAccountDB successfully destroyed - name:" \
    <<db_name << dendl;
