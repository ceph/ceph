...
//map<string, class ObjectOp*> POSIXUserDB::objectmap = {};

/*
map<string, class ObjectOp*> POSIXUserDB::getObjectMap() {
  return POSIXUserDB::objectmap;
}
*/
...
#if 0
int POSIXUserDB::createGC(const DoutPrefixProvider *dpp) {
  int ret = 0;
  /* create gc thread */

  gc_worker = std::make_unique<POSIXUserDB::GC>(dpp, this);
  gc_worker->create("db_gc");

  return ret;
}

int POSIXUserDB::stopGC() {
  if (gc_worker) {
...
std::shared_ptr<class DBOp> POSIXUserDB::getPOSIXUserDBOp(const DoutPrefixProvider *dpp, std::string_view Op,
                  const DBOpParams *params)
{
...

  {
    ...
    iter = POSIXUserDB::objectmap.find(params->op.bucket.info.bucket.name);
  }

  if (iter == POSIXUserDB::objectmap.end()) {
...
#if 0
int POSIXUserDB::objectmapInsert(const DoutPrefixProvider *dpp, string bucket, class ObjectOp* ptr)
{
  map<string, class ObjectOp*>::iterator iter;
  class ObjectOp *Ob;

  const std::lock_guard<std::mutex> lk(mtx);
  iter = POSIXUserDB::objectmap.find(bucket);
  ...
  Ob->InitializeObjectOps(getPOSIXUserDBname(), dpp);

  POSIXUserDB::objectmap.insert(pair<string, class ObjectOp*>(bucket, Ob));

  return 0;
}

int POSIXUserDB::objectmapDelete(const DoutPrefixProvider *dpp, string bucket)
{
  map<string, class ObjectOp*>::iterator iter;
  ...
  POSIXUserDB::objectmap.erase(iter);

  return 0;
}
#endif

int POSIXUserDB::InitializeParams(const DoutPrefixProvider *dpp, DBOpParams *params)
{
  int ret = -1;
...
int POSIXUserDB::ProcessOp(const DoutPrefixProvider *dpp, std::string_view Op, DBOpParams *params) {
  int ret = -1;
  shared_ptr<class DBOp> db_op;

  db_op = getPOSIXUserDBOp(dpp, Op, params);
...
int POSIXUserDB::get_user(const DoutPrefixProvider *dpp,
...
int POSIXUserDB::store_user(const DoutPrefixProvider *dpp,
...
int POSIXUserDB::remove_user(const DoutPrefixProvider *dpp,
...
#if 0
int POSIXUserDB::get_bucket_info(const DoutPrefixProvider *dpp, const std::string& query_str,
...
int POSIXUserDB::create_bucket(const DoutPrefixProvider *dpp,
    ...
    optional_yield y)
{
  ...
  if (info.bucket.marker.empty()) {
    ...
    string s = getPOSIXUserDBname() + "." + std::to_string(bid);
...
int POSIXUserDB::remove_bucket(const DoutPrefixProvider *dpp, const RGWBucketInfo info) {
  int ret = 0;
...
int POSIXUserDB::list_buckets(const DoutPrefixProvider *dpp, const std::string& query_str,
    std::string& owner,
    const string& marker,
...
int POSIXUserDB::update_bucket(const DoutPrefixProvider *dpp, const std::string& query_str,
    RGWBucketInfo& info,
    bool exclusive,
 ...
 * max, then truncated.
 */
int POSIXUserDB::Bucket::List::list_objects(const DoutPrefixProvider *dpp, int64_t max,
		           vector<rgw_bucket_dir_entry> *result,
		           map<string, bool> *common_prefixes, bool *is_truncated)
{
  int ret = 0;
  POSIXUserDB *store = target->get_store();
  int64_t count = 0;
  std::string prev_obj;
...
int POSIXUserDB::raw_obj::InitializeParamsfromRawObj(const DoutPrefixProvider *dpp,
                                            DBOpParams* params) {
  int ret = 0;
...
int POSIXUserDB::Object::InitializeParamsfromObject(const DoutPrefixProvider *dpp,
                                           DBOpParams* params) {
  int ret = 0;
...
int POSIXUserDB::Object::get_object_impl(const DoutPrefixProvider *dpp, DBOpParams& params) {
  int ret = 0;
...
int POSIXUserDB::Object::obj_omap_set_val_by_key(const DoutPrefixProvider *dpp,
...
int POSIXUserDB::Object::obj_omap_get_vals_by_keys(const DoutPrefixProvider *dpp,
...
int POSIXUserDB::Object::add_mp_part(const DoutPrefixProvider *dpp,
...
int POSIXUserDB::Object::get_mp_parts_list(const DoutPrefixProvider *dpp,
...
void POSIXUserDB::gen_rand_obj_instance_name(rgw_obj_key *target_key)
{
...
int POSIXUserDB::Object::obj_omap_get_all(const DoutPrefixProvider *dpp,
...
int POSIXUserDB::Object::obj_omap_get_vals(const DoutPrefixProvider *dpp,
...
int POSIXUserDB::Object::set_attrs(const DoutPrefixProvider *dpp,
...
int POSIXUserDB::Object::transition(const DoutPrefixProvider *dpp,
...
int POSIXUserDB::raw_obj::read(const DoutPrefixProvider *dpp, int64_t ofs,
...
int POSIXUserDB::raw_obj::write(const DoutPrefixProvider *dpp, int64_t ofs, int64_t write_ofs,
                       uint64_t len, bufferlist& bl)
{
  int ret = 0;
  DBOpParams params = {};
...
int POSIXUserDB::Object::list_versioned_objects(const DoutPrefixProvider *dpp,
...
int POSIXUserDB::Object::get_obj_state(const DoutPrefixProvider *dpp,
...
int POSIXUserDB::Object::get_state(const DoutPrefixProvider *dpp, RGWObjState** pstate, bool follow_olh)
{
  return get_obj_state(dpp, bucket_info, obj, follow_olh, pstate);
}

int POSIXUserDB::Object::Read::get_attr(const DoutPrefixProvider *dpp, const char *name, bufferlist& dest)
{
  RGWObjState* state;
...
int POSIXUserDB::Object::Read::prepare(const DoutPrefixProvider *dpp)
{
  POSIXUserDB *store = source->get_store();
...
int POSIXUserDB::Object::Read::range_to_ofs(uint64_t obj_size, int64_t &ofs, int64_t &end)
{
  if (ofs < 0) {
    ofs += obj_size;
    if (ofs < 0)
...
int POSIXUserDB::Object::Read::read(int64_t ofs, int64_t end, bufferlist& bl, const DoutPrefixProvider *dpp)
{
  POSIXUserDB *store = source->get_store();
...
static int _get_obj_iterate_cb(const DoutPrefixProvider *dpp,
    const POSIXUserDB::raw_obj& read_obj, off_t obj_ofs,
...
int POSIXUserDB::get_obj_iterate_cb(const DoutPrefixProvider *dpp,
...
int POSIXUserDB::Object::Read::iterate(const DoutPrefixProvider *dpp, int64_t ofs, int64_t end, RGWGetDataCB *cb)
{
  POSIXUserDB *store = source->get_store();
...
int POSIXUserDB::Object::iterate_obj(const DoutPrefixProvider *dpp,
    const RGWBucketInfo& bucket_info, const rgw_obj& obj,
    off_t ofs, off_t end, uint64_t max_chunk_size,
    iterate_obj_cb cb, void *arg)
{
  POSIXUserDB *store = get_store();
  uint64_t len;
  RGWObjState* astate;
...
int POSIXUserDB::Object::Write::prepare(const DoutPrefixProvider* dpp)
{
  POSIXUserDB *store = target->get_store();
...
/* writes tail objects */
int POSIXUserDB::Object::Write::write_data(const DoutPrefixProvider* dpp,
  ...
  /* as we are writing max_chunk_size at a time in sal_dbstore POSIXUserDBAtomicWriter::process(),
...
int POSIXUserDB::Object::Write::_do_write_meta(const DoutPrefixProvider *dpp,
...
int POSIXUserDB::Object::Write::write_meta(const DoutPrefixProvider *dpp, uint64_t size, uint64_t accounted_size,
    map<string, bufferlist>& attrs)
{
...
int POSIXUserDB::Object::Delete::delete_obj(const DoutPrefixProvider *dpp) {
  int ret = 0;
...
int POSIXUserDB::Object::Delete::delete_obj_impl(const DoutPrefixProvider *dpp,
                                        DBOpParams& del_params) {
  int ret = 0;
  POSIXUserDB *store = target->get_store();
 ...
 */
int POSIXUserDB::Object::Delete::create_dm(const DoutPrefixProvider *dpp,
                                             DBOpParams& del_params) {
...
int POSIXUserDB::get_entry(const std::string& oid, const std::string& marker,
...
int POSIXUserDB::get_next_entry(const std::string& oid, const std::string& marker,
                       rgw::sal::LCEntry& entry)
{
  int ret = 0;
  const DoutPrefixProvider *dpp = get_def_dpp();
...
int POSIXUserDB::set_entry(const std::string& oid, const rgw::sal::LCEntry& entry)
{
  int ret = 0;
  const DoutPrefixProvider *dpp = get_def_dpp();
...
int POSIXUserDB::list_entries(const std::string& oid, const std::string& marker,
...
int POSIXUserDB::rm_entry(const std::string& oid, const rgw::sal::LCEntry& entry)
{
  int ret = 0;
  const DoutPrefixProvider *dpp = get_def_dpp();
...
int POSIXUserDB::get_head(const std::string& oid, rgw::sal::LCHead& head)
{
  int ret = 0;
  const DoutPrefixProvider *dpp = get_def_dpp();
...
int POSIXUserDB::put_head(const std::string& oid, const rgw::sal::LCHead& head)
{
  int ret = 0;
  const DoutPrefixProvider *dpp = get_def_dpp();
...
int POSIXUserDB::delete_stale_objs(const DoutPrefixProvider *dpp, const std::string& bucket,
                          uint32_t min_wait) {
  DBOpParams params = {};
  int ret = -1;
...
void *POSIXUserDB::GC::entry() {
  do {
    std::unique_lock<std::mutex> lk(mtx);

    ldpp_dout(dpp, 2) << " POSIXUserDB GC started " << dendl;
    int max = 100;
    RGWUserBuckets buckets;
