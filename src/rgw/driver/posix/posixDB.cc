...
int POSIXDB::ProcessOp(const DoutPrefixProvider *dpp, string_view Op, DBOpParams *params) {
  int ret = -1;
...
int POSIXDB::Initialize(string logfile, int loglevel)
{
  int ret = -1;
  ...
  if (ret) {
    ldpp_dout(dpp, 0) <<"InitializePOSIXDBOps failed " << dendl;
    closeDB(dpp);
    db = NULL;
    return ret;
  }

  ldpp_dout(dpp, 0) << "POSIXDB successfully initialized - name:" \
    << db_name << "" << dendl;
...
int POSIXDB::Destroy(const DoutPrefixProvider *dpp)
{
  DB::Destroy(dpp);

  ldpp_dout(dpp, 20)<<"POSIXDB successfully destroyed - name:" \
    <<db_name << dendl;
