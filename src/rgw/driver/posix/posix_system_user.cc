...
int POSIXSystemManager::check_permissions(const DoutPrefixProvider *dpp, const rgw_user& ruser, rgw::sal::FSEnt* ent, struct stat& statbuf) {
  if (dpp->get_cct()->_conf->rgw_posix_user_mapping_type == "none")
    return 0;

  if (dpp->get_cct()->_conf->rgw_posix_user_mapping_type == "direct") {
    int ret = stat(ent->get_name().c_str(), &statbuf);
    if (ret < 0) {
      ret = errno;
