...
using list_bucket_each_t =
  const fu2::unique_function<int(const rgw_bucket_dir_entry&) const>;
...
public:
  ...
  static inline std::string concat_key(const rgw_obj_key& k) {
    std::string k_str;
    k_str.reserve(k.ns.size() + k.name.size() + k.instance.size());
    k_str += k.ns;
    k_str += k.name;
    k_str += k.instance;
    return k_str;
