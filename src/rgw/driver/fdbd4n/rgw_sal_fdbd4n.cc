...
FDBD4NFilterDriver::FDBD4NFilterDriver(Driver* _next, boost::asio::io_context& io_context, bool admin) : FilterDriver(_next),
												   io_context(io_context), 
												   y(null_yield)
{
  rgw::cache::Partition partition_info;
  partition_info.location = g_conf()->rgw_fdbd4n_l1_datacache_persistent_path;
  partition_info.name = "fdbd4n";
  partition_info.type = "read-cache";
  partition_info.reserve_size = g_conf()->rgw_fdbd4n_l1_datacache_disk_reserve;
  cacheDriver = std::make_unique<rgw::cache::SSDDriver>(partition_info, admin);
}

FDBD4NFilterDriver::~FDBD4NFilterDriver() = default;

int FDBD4NFilterDriver::initialize(CephContext *cct, const DoutPrefixProvider *dpp)
{
  namespace net = boost::asio;
  using boost::redis::config;

  conn = std::make_shared<connection>(boost::asio::make_strand(io_context));
  objDir = std::make_unique<rgw::fdbd4n::ObjectDirectory>(conn);
  blockDir = std::make_unique<rgw::fdbd4n::BlockDirectory>(conn);
  bucketDir = std::make_unique<rgw::fdbd4n::BucketDirectory>(conn);
  policyDriver = std::make_unique<rgw::fdbd4n::PolicyDriver>(conn,
							  cacheDriver.get(),
							  "lfuda",
                                                          this->y);

  std::string address = cct->_conf->rgw_fdbd4n_address;
  config cfg;
  cfg.addr.host = address.substr(0, address.find(":"));
  cfg.addr.port = address.substr(address.find(":") + 1, address.length());
  cfg.clientname = "FDBD4N.Filter";

  if (!cfg.addr.host.length() || !cfg.addr.port.length()) {
    ldpp_dout(dpp, 0) << "FDBD4NFilterDriver::" << __func__ << "(): Endpoint was not configured correctly." << dendl;
    return -EDESTADDRREQ;
  }
  ...
  std::shared_ptr<rgw::fdbd4n::RedisPool>redis_pool = nullptr;
  if(rgw_redis_connection_pool_size>0){
      redis_pool = std::make_shared<rgw::fdbd4n::RedisPool>(&io_context, cfg, rgw_redis_connection_pool_size);
...
std::unique_ptr<User> FDBD4NFilterDriver::get_user(const rgw_user &u)
{
  std::unique_ptr<User> user = next->get_user(u);

  return std::make_unique<FDBD4NFilterUser>(std::move(user), this);
}

std::unique_ptr<Object> FDBD4NFilterBucket::get_object(const rgw_obj_key& k)
{
  std::unique_ptr<Object> o = next->get_object(k);

  return std::make_unique<FDBD4NFilterObject>(std::move(o), this, filter);
}

std::unique_ptr<Bucket> FDBD4NFilterDriver::get_bucket(const RGWBucketInfo& i)
{
  return std::make_unique<FDBD4NFilterBucket>(next->get_bucket(i), this);
}

int FDBD4NFilterDriver::load_bucket(const DoutPrefixProvider* dpp, const rgw_bucket& b,
				 std::unique_ptr<Bucket>* bucket, optional_yield y)
{
  std::unique_ptr<Bucket> nb;
  const int ret = next->load_bucket(dpp, b, &nb, y);
  *bucket = std::make_unique<FDBD4NFilterBucket>(std::move(nb), this);
  return ret;
}

int FDBD4NFilterBucket::create(const DoutPrefixProvider* dpp,
...
int FDBD4NFilterBucket::list(const DoutPrefixProvider* dpp, ListParams& params, int max,
                          ListResults& results, optional_yield y)
{
  ldpp_dout(dpp, 20) << "FDBD4NFilterBucket::" << __func__ << " params.marker.name: " << params.marker.name << dendl;
  ldpp_dout(dpp, 20) << "FDBD4NFilterBucket::" << __func__ << " params.marker.instance: " << params.marker.instance << dendl;
  ldpp_dout(dpp, 20) << "FDBD4NFilterBucket::" << __func__ << " params.end_marker.key: " << params.end_marker.name << dendl;
  ldpp_dout(dpp, 20) << "FDBD4NFilterBucket::" << __func__ << " max: " << max << dendl;
  ...
  if (g_conf()->fdbd4n_writecache_enabled) {
    ...
    if (!params.prefix.empty()) {
      ...
      do {
        ...
        if (ret < 0 && ret != -ENOENT) {
          ldpp_dout(dpp, 0) << "FDBD4NFilterBucket::" << __func__ << " zscan failed with ret: " << ret << dendl;
          return ret;
        }
        ...
        for (auto it = temp_objects.begin(); it != temp_objects.end(); it++) {
          ...
          if (!params.marker.name.empty() && *it <= params.marker.name) {
            ...
            if (!params.delim.empty() && pos != std::string::npos) {
              ...
              ldpp_dout(dpp, 20) << "FDBD4NFilterBucket::" << __func__ << " delim_str: " << delim_str << dendl;
              ...
              if (params.list_versions) {
                ...
                rgw::fdbd4n::CacheObj dir_obj = rgw::fdbd4n::CacheObj{
                  .objName = objName,
                ...
                if (params.marker.instance.empty() || instance_marker_processed || params.marker.name.empty()) {
                  ...
                  if (!instance_marker_processed) {
                    ...
                    if (ret < 0) {
                      ldpp_dout(dpp, 0) << "FDBD4NFilterBucket::" << __func__ << " zrank failed with: " << ret << dendl;
                      return ret;
                    }
                    ldpp_dout(dpp, 20) << "FDBD4NFilterBucket::" << __func__ << " start: " << index << dendl;
                    start = index;
                ...
                if (ret < 0 && ret != -ENOENT) {
                  ldpp_dout(dpp, 0) << "FDBD4NFilterBucket::" << __func__ << " zrevrange failed with ret: " << ret << dendl;
                  return ret;
                }
              } //params.list_version
      ...
      do {
        std::vector<std::string> temp_objects;
        ldpp_dout(dpp, 20) << "FDBD4NFilterBucket::" << __func__ << " zrange start is: " << start << dendl;
        auto ret = bucketDir->zrange(dpp, this->get_bucket_id(), start, "+", 0, (max + 1), temp_objects, y);
        if (ret < 0 && ret != -ENOENT) {
          ldpp_dout(dpp, 0) << "FDBD4NFilterBucket::" << __func__ << " zrange failed with ret: " << ret << dendl;
          return ret;
        }
        ldpp_dout(dpp, 20) << "FDBD4NFilterBucket::" << __func__ << " params.delim: " << params.delim << dendl;
        std::string last_element_processed;
        for (auto it = temp_objects.begin(); it != temp_objects.end(); it++) {
          last_element_processed = *it;
            ...
            if (cache_results.common_prefixes.find(delim_str) == cache_results.common_prefixes.end()) {
              ...
              if (num_objs == max) {
                ...
                do {
                  ...
                  if (ret < 0 && ret != -ENOENT) {
                    ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << " zscan failed with ret: " << ret << dendl;
                    return ret;
                  }
                  if ((next_cursor == 0) || (ret == -ENOENT)) {
                    if (!delim_objects.empty()) {
                      ...
                      if (ret < 0 && ret != -ENOENT) {
                        ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << " zrange failed with ret: " << ret << dendl;
                        return ret;
                      }
        ...
        if ((num_objs == max) || (ret == -ENOENT) || temp_objects.empty()) {
          ldpp_dout(dpp, 20) << "FDBD4NFilterBucket::" << __func__ << " Breaking out! " << dendl;
          ldpp_dout(dpp, 20) << "FDBD4NFilterBucket::" << __func__ << " num_objs " << num_objs << dendl;
          ldpp_dout(dpp, 20) << "FDBD4NFilterBucket::" << __func__ << " ret " << ret << dendl;
          break;
        }
    ...
    rgw::fdbd4n::BlockDirectory* blockDir = this->filter->get_block_dir();
    ...
    while (remainder_size > 0) {
      ...
      std::vector<rgw::fdbd4n::CacheBlock> blocks(batch_size);
      start_j = j;
      for (auto i = 0; i < batch_size; i++) {
        ldpp_dout(dpp, 20) << "FDBD4NFilterBucket::" << __func__ << " objects[j]: " << entries[j].key.name << dendl;
        ldpp_dout(dpp, 20) << "FDBD4NFilterBucket::" << __func__ << " remainder_size: " << remainder_size << dendl;
        ...
        ldpp_dout(dpp, 20) << "FDBD4NFilterBucket::" << __func__ << " blocks[i].cacheObj.objName: " << blocks[i].cacheObj.objName << dendl;
      ...
      if (ret < 0) {
        ldpp_dout(dpp, 0) << "FDBD4NFilterBucket::" << __func__ << " blockDir->get() returned error: " << ret << dendl;
        return ret;
      }
      ...
      for (auto block : blocks) {
	    if (return_blocks) {
          if (i >= dir_blocks_objNames.size() || i >= block.cacheObj.objName.size()) {
	        ldpp_dout(dpp, 0) << "FDBD4NFilterBucket::" << __func__ << " Unable to construct list of cached blocks." << dendl;
            return -EINVAL;
          }
          if (dir_blocks_objNames[i] == block.cacheObj.objName) {
	        dir_blocks.insert(std::make_pair(dir_blocks_objNames[i], block));
          } else {
	        ldpp_dout(dpp, 0) << "FDBD4NFilterBucket::" << __func__ << " Error constructing list of cached blocks while handling object " << dir_blocks_objNames[i] << dendl;
            return -EINVAL;
          }
        }
        i++;
        ...
        try {
          ...
          ldpp_dout(dpp, 0) << "FDBD4NFilterBucket::" << __func__ << " Invalid value of time: " << block.cacheObj.creationTime << dendl;
        }
  ...
  } //fdbd4n_write_cache_enabled = true
  ...
  if (ret < 0) {
    ldpp_dout(dpp, 0) << "FDBD4NFilterBucket::" << __func__ << " next->list returned: " << ret << dendl;
    return ret;
  }
  ...
  if (!cache_results.objs.empty() && !store_results.objs.empty()) {
    ...
    while (elementsAdded < max && i < cache_results.objs.size() && j < store_results.objs.size()) {
      ...
      ldpp_dout(dpp, 20) << "FDBD4NFilterObject::" << __func__ << " key_name_in_cache: " << key_name_in_cache << dendl;
      ldpp_dout(dpp, 20) << "FDBD4NFilterObject::" << __func__ << " key_name_in_store: " << key_name_in_store << dendl;
...
int FDBD4NFilterBucket::remove(const DoutPrefixProvider* dpp,
			    ...
			    optional_yield y)
{
  ...
  auto objDir = this->filter->get_obj_dir();
  std::vector<rgw::fdbd4n::CacheBlock> blocks; 
  std::vector<rgw::fdbd4n::CacheObj> objects; 
  ...
  do {
    ...
    for (const auto& obj : results.objs) { 
      if (((PIPELINE_MAX - blocks.size()) <= OBJECT_LIST_VAL) || (blocks.size() > (PIPELINE_MAX - 1000))) { 
	    if ((ret = blockDir->del(dpp, blocks, y) < 0)) {
	      ldpp_dout(dpp, 10) << "FDBD4NFilterBucket::" << __func__ << "(): Failed to delete cached object in block directory, ret=" << ret << dendl;
	      return ret;
	    }
	    blocks.clear();
      }
      ...
      ldpp_dout(dpp, 20) << "FDBD4NFilterBucket::" << __func__ << "(): handling object=" << obj.key << dendl;

      rgw::fdbd4n::CacheObj object = rgw::fdbd4n::CacheObj{
        .objName = c_obj->get_name(),
        .bucketName = this->get_bucket_id(),
      };

      rgw::fdbd4n::CacheBlock block = rgw::fdbd4n::CacheBlock{
        .cacheObj = object,
        .blockID = 0,
        .size = 0,
      };
      ...
      ldpp_dout(dpp, 20) << "FDBD4NFilterBucket::" << __func__ << "(): Obj size=" << lst << dendl;
      ...
      if (it != dir_blocks.end() && it->second.cacheObj.dirty) {
	    if (!this->filter->get_policy_driver()->get_cache_policy()->invalidate_dirty_object(dpp, get_cache_block_prefix(c_obj.get(), it->second.version))) {
	      ldpp_dout(dpp, 10) << "FDBD4NFilterBucket::" << __func__ << "(): Failed to invalidate obj=" << c_obj->get_name() << " in cache" << dendl;
	      return -EINVAL;
	    }
       ...
       * later eviction calls. */
        ...
        ldpp_dout(dpp, 20) << "FDBD4NFilterBucket::" << __func__ << "(): Listing retrieved from backend for object " << c_obj->get_name() << dendl;
      }

      // Handle versioned head objects
      ldpp_dout(dpp, 20) << "FDBD4NFilterBucket::" << __func__ << "(): versioned oid: " << block.cacheObj.objName << dendl;
      blocks.push_back(block);

      // Handle data blocks
      ldpp_dout(dpp, 20) << "FDBD4NFilterBucket::" << __func__ << "(): Object size=" << lst << dendl;
      off_t fst = 0;
      do {
        ...
        ldpp_dout(dpp, 20) << "FDBD4NFilterBucket::" << __func__ << "(): handling object=" << obj.key << dendl;
        rgw::fdbd4n::CacheBlock data_block;
        if (fst >= lst) {
          break;
        }
        ...
        ldpp_dout(dpp, 20) << "FDBD4NFilterBucket::" << __func__ << "(): data_block=" << data_block.cacheObj.objName << dendl;
    ...
    if ((PIPELINE_MAX - objects.size()) <= OBJECT_LIST_VAL) {
      if ((ret = objDir->del(dpp, objects, y)) < 0) {
        ldpp_dout(dpp, 10) << "FDBD4NFilterBucket::" << __func__ << "(): Failed to delete bucket in bucket directory, ret=" << ret << dendl;
        return ret;
      }
      objects.clear();
    }
  ...
  ldpp_dout(dpp, 20) << "FDBD4NFilterBucket::" << __func__ << "(): calling next->remove" << dendl;
  return next->remove(dpp, delete_children, y);
}

int FDBD4NFilterBucket::check_empty(const DoutPrefixProvider* dpp, optional_yield y)
{
  ...
  if ((ret = this->filter->get_bucket_dir()->exist_key(dpp, this->get_bucket_id(), y)) < 0) {
    ldpp_dout(dpp, 10) << "FDBD4NFilterBucket::" << __func__ << "(): Failed to retrieve bucket in bucket directory, ret=" << ret << dendl;
    return ret;
  } if (ret == 0) {
    ldpp_dout(dpp, 20) << "FDBD4NFilterBucket::" << __func__ << "(): calling next->check_empty" << dendl;
...
std::unique_ptr<MultipartUpload> FDBD4NFilterBucket::get_multipart_upload(
				  ...
				  ACLOwner owner, ceph::real_time mtime)
{
  ...
  return std::make_unique<FDBD4NFilterMultipartUpload>(std::move(nmu), this, this->filter);
}

int FDBD4NFilterObject::copy_object(const ACLOwner& owner,
                              ...
                              optional_yield y)
{
  bool write_to_cache = g_conf()->fdbd4n_writecache_enabled;
  bool dirty{false};
  ...
  if (auto ret = read_op->prepare(y, dpp); ret < 0) {
    ldpp_dout(dpp, 10) << "FDBD4NFilterObject::" << __func__ << "(): prepare method failed with ret: " << ret << dendl;
  ...
  ldpp_dout(dpp, 20) << "FDBD4NFilterObject::" << __func__ << "(): is_multipart: " << is_multipart() << dendl;
  ...
  if (is_multipart() || !write_to_cache) {
    ...
    if (ret < 0) {
      ldpp_dout(dpp, 10) << "FDBD4NFilterObject::" << __func__ << "(): next->copy_object failed with ret: " << ret << dendl;
      return ret;
    }
  }
  ...
  FDBD4NFilterObject* fdbd4n_dest_object = dynamic_cast<FDBD4NFilterObject*>(dest_object);
  ...
  if (write_to_cache) {
    ...
    if (!dest_object->have_instance()) {
      if (dest_object->get_bucket()->versioned() && !dest_object->get_bucket()->versioning_enabled()) { //if versioning is suspended
        ...
        ldpp_dout(dpp, 10) << "FDBD4NFilterObject::" << __func__ << "(): generating version: " << version << dendl;
      }
    } else {
      dest_version = dest_object->get_instance();
    }
    fdbd4n_dest_object->set_object_version(dest_version);
    if (auto ret = read_op->iterate(dpp, 0, (this->get_size() - 1), nullptr, y); ret < 0) {
      ldpp_dout(dpp, 10) << "FDBD4NFilterObject::" << __func__ << "(): iterate method failed with ret: " << ret << dendl;
      return ret;
    }
    ...
    ldpp_dout(dpp, 20) << "FDBD4NFilterObject::" << __func__ << " size is: " << dest_object->get_size() << dendl;
    fdbd4n_dest_object->set_attrs_from_obj_state(dpp, y, baseAttrs, dirty);
  } else {
    auto o_attrs = baseAttrs; 
    dest_object->load_obj_state(dpp, y);
    baseAttrs = dest_object->get_attrs();
    fdbd4n_dest_object->set_attrs_from_obj_state(dpp, y, baseAttrs, dirty);
    fdbd4n_dest_object->calculate_version(dpp, y, dest_version, o_attrs);
    if (dest_version.empty()) {
      ldpp_dout(dpp, 10) << "FDBD4NFilterObject::" << __func__ << "(): version could not be calculated." << dendl;
    }
  }
  bufferlist bl_val;
  ...
  bufferlist bl_data;
  dest_version = fdbd4n_dest_object->get_object_version();
  ...
  if (ret == 0) {
    ...
    if (ret == 0) {
      ldpp_dout(dpp, 20) << "FDBD4NFilterObject::" << __func__ << " version stored in update method is: " << dest_version << dendl;
      bufferlist bl;
      driver->get_policy_driver()->get_cache_policy()->update(dpp, key, 0, bl.length(), dest_version, dirty, rgw::fdbd4n::RefCount::NOOP, y);
      fdbd4n_dest_object->set_object_version(dest_version);
      ret = fdbd4n_dest_object->set_head_obj_dir_entry(dpp, nullptr, y, true, dirty);
      if (ret < 0) {
        ldpp_dout(dpp, 10) << "FDBD4NFilterObject::" << __func__ << "(): BlockDirectory set method failed for head object with ret: " << ret << dendl;
        return ret;
      }
      if (dirty) {
        driver->get_policy_driver()->get_cache_policy()->update_dirty_object(dpp, key, dest_version, false, this->get_size(), creationTime, std::get<rgw_user>(dest_object->get_bucket()->get_owner()), *etag, dest_object->get_bucket()->get_name(), dest_object->get_bucket()->get_bucket_id(), dest_object->get_key(), rgw::fdbd4n::RefCount::NOOP, y);
      }
    }
  }
...
int FDBD4NFilterObject::load_obj_state(const DoutPrefixProvider *dpp, optional_yield y,
                             bool follow_olh)
{
  if (load_from_store) {
    if (cache_request) {
...
int FDBD4NFilterObject::set_obj_attrs(const DoutPrefixProvider* dpp, Attrs* setattrs,
                            Attrs* delattrs, optional_yield y, uint32_t flags)
{
  rgw::sal::Attrs attrs;
  std::string head_oid_in_cache;
  rgw::fdbd4n::CacheBlock block;
  ...
  if (check_head_exists_in_cache_get_oid(dpp, head_oid_in_cache, attrs, block, y)) {
    ...
    if (setattrs != nullptr) {
      ...
      if (this->driver->get_policy_driver()->get_cache_policy()->update_refcount_if_key_exists(dpp, head_oid_in_cache, rgw::fdbd4n::RefCount::INCR, y)) {
        auto ret = driver->get_cache_driver()->set_attrs(dpp, head_oid_in_cache, *setattrs, y);
        this->driver->get_policy_driver()->get_cache_policy()->update_refcount_if_key_exists(dpp, head_oid_in_cache, rgw::fdbd4n::RefCount::DECR, y);
        if (ret < 0) {
          ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): CacheDriver set_attrs method failed with ret: " << ret << dendl;
          return ret;
        }
      } else {
    ...
    if (delattrs != nullptr) {
      ...
      if (this->driver->get_policy_driver()->get_cache_policy()->update_refcount_if_key_exists(dpp, head_oid_in_cache, rgw::fdbd4n::RefCount::INCR, y)) {
        ...
        this->driver->get_policy_driver()->get_cache_policy()->update_refcount_if_key_exists(dpp, head_oid_in_cache, rgw::fdbd4n::RefCount::DECR, y);
        if (ret < 0) {
          ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): CacheDriver delete_attrs method failed with ret: " << ret << dendl;
          return ret;
        }
      } else {
        found_in_cache = false;
    ...
    if (block.deleteMarker) {
      ldpp_dout(dpp, 10) << "FDBD4NFilterObject::" << __func__ << "(): object " << this->get_name() << " does not exist." << dendl;
      return -ENOENT;
    }
  }
  ...
  if (!found_in_cache) {
    ...
    if (ret < 0) {
      ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): set_obj_attrs method of backend store failed with ret: " << ret << dendl;
      return ret;
    }
  }
  return 0;
...
int FDBD4NFilterObject::get_obj_attrs_from_cache(const DoutPrefixProvider* dpp, optional_yield y)
{
  ...
  if (block.deleteMarker) {
    ...
    ldpp_dout(dpp, 10) << "FDBD4NFilterObject::" << __func__ << "(): obj is: " << this->get_obj().key.name << dendl;
    std::string instance;
    for (auto& attr : attrs) {
      if (attr.second.length() > 0) {
        if (attr.first == RGW_CACHE_ATTR_MTIME) {
          ldpp_dout(dpp, 10) << "FDBD4NFilterObject::" << __func__ << "(): setting mtime." << dendl;
          ...
          ldpp_dout(dpp, 10) << "FDBD4NFilterObject::" << __func__ << "(): setting object_size to: " << size << dendl;
          ...
          ldpp_dout(dpp, 10) << "FDBD4NFilterObject::" << __func__ << "(): setting epoch." << dendl;
          ...
          ldpp_dout(dpp, 10) << "FDBD4NFilterObject::" << __func__ << "(): setting version_id to: " << instance << dendl;
        } else if (attr.first == RGW_CACHE_ATTR_SOURC_ZONE) {
          ldpp_dout(dpp, 10) << "FDBD4NFilterObject::" << __func__ << "(): setting source zone id." << dendl;
          ...
          ldpp_dout(dpp, 10) << "FDBD4NFilterObject::" << __func__ << "(): is_multipart: " << this->multipart << " multipart: " << multipart << dendl;
        } else {
          ldpp_dout(dpp, 20) << "FDBD4NFilterObject::" << __func__ << "(): Unexpected attribute; not locally set, attr name: " << attr.first << dendl;
        }
      }//end-if
    }//end-for
    if (!instance.empty()) {
    ...
    if (ret < 0) {
      ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): FDBD4NFilterObject set_attrs method failed." << dendl;
    }
...
int FDBD4NFilterObject::set_attr_crypt_parts(const DoutPrefixProvider* dpp, optional_yield y, rgw::sal::Attrs& attrs)
{
  if (attrs.count(RGW_ATTR_CRYPT_MODE)) {
    ...
    for (uint64_t part = 0; part < num_parts; part++) {
      ...
      ldpp_dout(dpp, 20) << "FDBD4NFilterObject::" << __func__ << "(): part_num: " << part << " part_len: " << part_len << dendl;
...
void FDBD4NFilterObject::set_attrs_from_obj_state(const DoutPrefixProvider* dpp, optional_yield y, rgw::sal::Attrs& attrs, bool dirty)
{
  bufferlist bl_val;
...
int FDBD4NFilterObject::calculate_version(const DoutPrefixProvider* dpp, optional_yield y, std::string& version, rgw::sal::Attrs& attrs)
{
  ...
  ldpp_dout(dpp, 10) << "FDBD4NFilterObject::" << __func__ << "(): object name: " << this->get_name() << " instance: " << this->have_instance() << dendl;
...
int FDBD4NFilterObject::create_delete_marker(const DoutPrefixProvider* dpp, optional_yield y)
{
  this->delete_marker = true;
  char buf[OBJ_INSTANCE_LEN + 1];
  gen_rand_alphanumeric_no_underscore(dpp->get_cct(), buf, OBJ_INSTANCE_LEN);
  this->version = buf;
  ldpp_dout(dpp, 20) << "FDBD4NFilterObject::" << __func__ << "(): generating delete marker: " << version << dendl;
  ...
  ldpp_dout(dpp, 20) << "FDBD4NFilterObject::" << __func__ << " size is: " << this->get_size() << dendl;
  rgw::sal::Attrs attrs;
  ...
  bufferlist bl;
  ldpp_dout(dpp, 20) << "FDBD4NFilterObject::" << __func__ << "(): key is: " << key << dendl;
  ...
  if (ret == 0) {
    ...
    if (ret == 0) {
      ldpp_dout(dpp, 20) << "FDBD4NFilterObject::" << __func__ << "(): version stored in update method is: " << version << dendl;
      driver->get_policy_driver()->get_cache_policy()->update(dpp, key, 0, bl.length(), version, true, rgw::fdbd4n::RefCount::NOOP, y);
      std::vector<std::string> exec_responses;
      ret = this->set_head_obj_dir_entry(dpp, &exec_responses , y, true, true);
      if (ret < 0) {
        ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): BlockDirectory set method failed for head object, ret=" << ret << dendl;
        return ret;
      }
      auto creationTime = ceph::real_clock::to_double(this->get_mtime());
      ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): key=" << key << dendl;
      std::string objEtag = "";
      driver->get_policy_driver()->get_cache_policy()->update_dirty_object(dpp, key, version, true, this->get_accounted_size(), creationTime, std::get<rgw_user>(this->get_bucket()->get_owner()), objEtag, this->get_bucket()->get_name(), this->get_bucket()->get_bucket_id(), this->get_key(), rgw::fdbd4n::RefCount::NOOP, y);
    } else { //if get_cache_driver()->put()
      ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): put failed for key, ret=" << ret << " key: " << key << dendl;
      return ret;
    }
  } else {
    ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): eviction failed for key, ret=" << ret << dendl;
    return ret;
  }
...
int FDBD4NFilterObject::set_head_obj_dir_entry(const DoutPrefixProvider* dpp, std::vector<std::string>* exec_responses, optional_yield y, bool is_latest_version, bool dirty)
{
  ldpp_dout(dpp, 10) << "FDBD4NFilterObject::" << __func__ << "(): object name: " << this->get_name() << " bucket name: " << this->get_bucket()->get_name() << dendl;
  rgw::fdbd4n::CacheBlock block; 
  rgw::fdbd4n::BlockDirectory* blockDir = this->driver->get_block_dir();
  ...
  ldpp_dout(dpp, 10) << "FDBD4NFilterObject::" << __func__ << "(): etag: " << etag << dendl;
  ...
  if (acl_it != attrs.end()) {
    ...
    try {
      ...
      ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): ERROR: could not decode policy, caught buffer::error" << dendl;
      return -EIO;
    }
    ACLOwner owner = policy.get_owner();
    rgw_user user = std::get<rgw_user>(owner.id);
    ldpp_dout(dpp, 20) << "FDBD4NFilterObject::" << __func__ << "(): INFO: user_d: " << user.to_str() << dendl;
    ldpp_dout(dpp, 20) << "FDBD4NFilterObject::" << __func__ << "(): INFO: display_name: " << owner.display_name << dendl;
    user_id = user.to_str();
  ...
  if (is_latest_version) {
    ...
    ldpp_dout(dpp, 10) << "FDBD4NFilterObject::" << __func__ << "(): objName after special Handling: " << objName << dendl;
    rgw::fdbd4n::CacheObj object = rgw::fdbd4n::CacheObj{
      .objName = objName,
      .bucketName = this->get_bucket()->get_bucket_id(),
      .creationTime = std::to_string(ceph::real_clock::to_double(this->get_mtime())),
      .dirty = dirty,
      .hostsList = { dpp->get_cct()->_conf->rgw_fdbd4n_local_rgw_address },
      .etag = etag,
    ...
    if (dirty) {
      ...
      rgw::fdbd4n::Pipeline p = rgw::fdbd4n::Pipeline(redis_conn, redis_pool);
      p.start();
      ...
      if (!(this->get_bucket()->versioned())) {
        ...
        if (ret < 0) {
          ldpp_dout(dpp, 10) << "FDBD4NFilterObject::" << __func__ << "(): BlockDirectory set method failed for null head object with ret: " << ret << dendl;
          return ret;
        }
      }
      ...
      ldpp_dout(dpp, 10) << "FDBD4NFilterObject::" << __func__ << "(): Score of object name: "<< this->get_name() << " version: " << object_version << " is: "  << score << ret << dendl;
      rgw::fdbd4n::ObjectDirectory* objDir = this->driver->get_obj_dir();
      ret = objDir->zadd(dpp, &object, score, object_version, y, &p);
      if (ret < 0) {
        ldpp_dout(dpp, 10) << "FDBD4NFilterObject::" << __func__ << "(): Failed to add version to ordered set with error: " << ret << dendl;
        return ret;
      }
      ...
      rgw::fdbd4n::BucketDirectory* bucketDir = this->driver->get_bucket_dir();
      ret = bucketDir->zadd(dpp, this->get_bucket()->get_bucket_id(), 0, this->get_name(), y, &p);
      if (ret < 0) {
        ldpp_dout(dpp, 10) << "FDBD4NFilterObject::" << __func__ << "(): Failed to add object to ordered set with error: " << ret << dendl;
        return ret;
      }
      p.execute(dpp, y);
    } else { //for clean/non-dirty objects
      rgw::fdbd4n::CacheBlock latest = block;
      ...
      if (ret == -ENOENT) {
        ...
        ldpp_dout(dpp, 10) << "FDBD4NFilterObject::" << __func__ << "(): BlockDirectory get method failed for head object with ret: " << ret << dendl;
        ...
        if (this->get_bucket()->versioned()) {
          ...
          if (ret < 0 && ret != -ENOENT) {
            ldpp_dout(dpp, 10) << "FDBD4NFilterObject::" << __func__ << "(): BlockDirectory del method failed for head object with ret: " << ret << dendl;
          }
        }
  ...
  if (this->get_bucket()->versioned()) {
    ...
    rgw::fdbd4n::CacheObj version_object = rgw::fdbd4n::CacheObj{
    .objName = objName,
    .bucketName = this->get_bucket()->get_bucket_id(),
    .creationTime = std::to_string(ceph::real_clock::to_double(this->get_mtime())),
    .dirty = dirty,
    ...
    version_object.hostsList.insert({ dpp->get_cct()->_conf->rgw_fdbd4n_local_rgw_address });

    rgw::fdbd4n::CacheBlock version_block = rgw::fdbd4n::CacheBlock{
    ...
    if (ret < 0) {
      ldpp_dout(dpp, 10) << "FDBD4NFilterObject::" << __func__ << "(): BlockDirectory set method failed for versioned head object with ret: " << ret << dendl;
      return ret;
    }
...
int FDBD4NFilterObject::set_data_block_dir_entries(const DoutPrefixProvider* dpp, optional_yield y, std::string& version, bool dirty)
{
  rgw::fdbd4n::BlockDirectory* blockDir = driver->get_block_dir();

  //update data block entries in directory
  off_t lst = this->get_size();
  ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): Object size =" << lst << dendl;
  off_t fst = 0;
  std::vector<rgw::fdbd4n::CacheBlock> blocks;
  do {
    rgw::fdbd4n::CacheBlock block;
  ...
  auto ret = blockDir->get(dpp, blocks, y);
  if (ret < 0) {
    ldpp_dout(dpp, 0) << "FDBD4NFilterWriter::" << __func__ << "(): BlockDirectory pipelined get() method failed, ret=" << ret << dendl;
    return ret;
  }

  for (auto& block : blocks) {
    block.cacheObj.dirty = dirty;
    block.cacheObj.hostsList.insert(dpp->get_cct()->_conf->rgw_fdbd4n_local_rgw_address);
    block.version = version;
  }
  if ((ret = blockDir->set(dpp, blocks, y)) < 0) {
    ldpp_dout(dpp, 0) << "FDBD4NFilterWriter::" << __func__ << "(): BlockDirectory pipelined set() method failed, ret=" << ret << dendl;
    return ret;
  }
...
int FDBD4NFilterObject::delete_cache_entry(const DoutPrefixProvider* dpp, const std::string key, optional_yield y) {
  int ret;
...
int FDBD4NFilterObject::delete_data_block_cache_entries(const DoutPrefixProvider* dpp, optional_yield y, std::string& version, bool dirty)
{
...
bool FDBD4NFilterObject::check_head_exists_in_cache_get_oid(const DoutPrefixProvider* dpp, std::string& head_oid_in_cache, rgw::sal::Attrs& attrs, rgw::fdbd4n::CacheBlock& blk, optional_yield y)
{
  rgw::fdbd4n::BlockDirectory* blockDir = this->driver->get_block_dir();
  std::string objName = this->get_oid();
  ...
  ldpp_dout(dpp, 20) << "FDBD4NFilterObject::" << __func__ << "(): objName: " << objName << dendl;
  rgw::fdbd4n::CacheObj object = rgw::fdbd4n::CacheObj{
  ...
  if ((ret = blockDir->get(dpp, &block, y)) == 0) {
    ...
    if (this->driver->get_policy_driver()->get_cache_policy()->update_refcount_if_key_exists(dpp, key, rgw::fdbd4n::RefCount::INCR, y)) {
      ldpp_dout(dpp, 10) << "FDBD4NFilterObject::" << __func__ << "(): Is block dirty: " << block.cacheObj.dirty << dendl;
      ldpp_dout(dpp, 10) << "FDBD4NFilterObject::" << __func__ << "(): version: " << block.version << dendl;
      head_oid_in_cache = get_cache_block_prefix(this, version);
      ldpp_dout(dpp, 10) << "FDBD4NFilterObject::" << __func__ << "(): Fetching attrs from cache for head obj id: " << head_oid_in_cache << dendl;
      auto ret = this->driver->get_cache_driver()->get_attrs(dpp, head_oid_in_cache, attrs, y);
      if (ret < 0) {
        found_in_cache = false;
        ldpp_dout(dpp, 10) << "FDBD4NFilterObject::" << __func__ << "(): CacheDriver get_attrs method failed." << dendl;
      }
      std::string key = head_oid_in_cache;
      this->driver->get_policy_driver()->get_cache_policy()->update(dpp, key, 0, 0, version, block.cacheObj.dirty, rgw::fdbd4n::RefCount::DECR, y);
    ...
    found_in_cache = false;
    ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): BlockDirectory get method failed, ret=" << ret << dendl;
  }
...
int FDBD4NFilterObject::get_obj_attrs(optional_yield y, const DoutPrefixProvider* dpp)
{
  ...
  if ((ret = get_obj_attrs_from_cache(dpp, y)) == -ENOENT) {
    ...
    if(perfcounter) {
      perfcounter->inc(l_rgw_fdbd4n_cache_misses);
    }
    std::string head_oid_in_cache;
    rgw::sal::Attrs attrs;
    std::string version;
    ldpp_dout(dpp, 10) << "FDBD4NFilterObject::" << __func__ << "(): Fetching attrs from backend store." << dendl;
    auto ret = next->get_obj_attrs(y, dpp);
    if (ret < 0) {
      ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): Failed to fetching attrs from backend store with ret: " << ret << dendl;
      return ret;
    }
    ...
    ldpp_dout(dpp, 10) << "FDBD4NFilterObject::" << __func__ << "(): this->obj oid is: " << this->obj.key.name << "instance is: " << this->obj.key.instance << dendl;
    attrs = this->get_attrs();
    this->set_attrs_from_obj_state(dpp, y, attrs);

    calculate_version(dpp, y, version, attrs);
    if (version.empty()) {
      ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): version could not be calculated." << dendl;
    }
    std::string objName = this->get_name();
    head_oid_in_cache = get_cache_block_prefix(this, version);
    if (this->driver->get_policy_driver()->get_cache_policy()->update_refcount_if_key_exists(dpp, head_oid_in_cache, rgw::fdbd4n::RefCount::INCR, y)) {
      ...
      if (ret == 0) {
        ...
        ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): Failed to evict data, ret=" << ret << dendl;
      }
    }
    if (ret == 0) {
      ldpp_dout(dpp, 20) << "FDBD4NFilterObject::" << __func__ << " version stored in update method is: " << this->get_object_version() << dendl;
      this->driver->get_policy_driver()->get_cache_policy()->update(dpp, head_oid_in_cache, 0, 0, version, false, rgw::fdbd4n::RefCount::DECR, y);
      ...
      this->driver->get_policy_driver()->get_cache_policy()->update_refcount_if_key_exists(dpp, head_oid_in_cache, rgw::fdbd4n::RefCount::DECR, y);
      ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): failed to cache head object in cache backend, ret=" << ret << dendl;
    }
  } else {
    if(perfcounter) {
      perfcounter->inc(l_rgw_fdbd4n_cache_hits);
    }
  }
...
int FDBD4NFilterObject::modify_obj_attrs(const char* attr_name, bufferlist& attr_val,
                               optional_yield y, const DoutPrefixProvider* dpp,  uint32_t flags)
{
  ...
  if (check_head_exists_in_cache_get_oid(dpp, head_oid_in_cache, attrs, block, y)) {
    if (auto ret = driver->get_cache_driver()->update_attrs(dpp, head_oid_in_cache, update, y); ret < 0) {
      ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): CacheDriver update_attrs method failed with ret: " << ret << dendl;
      return ret;
    }
  } else {
    if (cache_request) {
    ...
    if (ret < 0) {
      ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): modify_obj_attrs of backend store failed with ret: " << ret << dendl;
      return ret;
    }
  }
  return 0;
...
int FDBD4NFilterObject::delete_obj_attrs(const DoutPrefixProvider* dpp, const char* attr_name,
                               optional_yield y)
{
  buffer::list bl;
  std::string head_oid_in_cache;
  ...
  if (!found_in_cache) {
    ...
    if (auto ret = next->delete_obj_attrs(dpp, attr_name, y); ret < 0) {
      ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): delete_obj_attrs method of backend store failed with ret: " << ret << dendl;
      return ret;
    }
  }
...
std::unique_ptr<Object> FDBD4NFilterDriver::get_object(const rgw_obj_key& k)
{
  std::unique_ptr<Object> o = next->get_object(k);

  return std::make_unique<FDBD4NFilterObject>(std::move(o), this);
}

std::unique_ptr<Writer> FDBD4NFilterDriver::get_atomic_writer(const DoutPrefixProvider *dpp,
				  optional_yield y,
				  rgw::sal::Object* obj,
				  ...
				  const std::string& unique_tag)
{
  ...
  return std::make_unique<FDBD4NFilterWriter>(std::move(writer), this, obj, dpp, true, y);
}

void FDBD4NFilterDriver::shutdown()
{
...
std::unique_ptr<Object::ReadOp> FDBD4NFilterObject::get_read_op()
{
  std::unique_ptr<ReadOp> r = next->get_read_op();
  return std::make_unique<FDBD4NFilterReadOp>(std::move(r), this);
}

std::unique_ptr<Object::DeleteOp> FDBD4NFilterObject::get_delete_op()
{
  std::unique_ptr<DeleteOp> d = next->get_delete_op();
  return std::make_unique<FDBD4NFilterDeleteOp>(std::move(d), this);
}

int FDBD4NFilterObject::FDBD4NFilterReadOp::prepare(optional_yield y, const DoutPrefixProvider* dpp)
{
  ...
  if ((ret = source->get_obj_attrs_from_cache(dpp, y)) == -ENOENT) {
    ldpp_dout(dpp, 10) << "FDBD4NFilterObject::FDBD4NFilterReadOp::" << __func__ << "(): object " << source->get_name() << " does not exist." << dendl;
    return -ENOENT;
  } else if (!ret) {
    ...
    rgw::sal::Attrs attrs;
    std::string version;
    ldpp_dout(dpp, 10) << "FDBD4NFilterObject::" << __func__ << "(): fetching head object from backend store" << dendl;
    next->params = params;
    auto ret = next->prepare(y, dpp);
    if (ret < 0) {
      ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): next->prepare method failed, ret=" << ret << dendl;
      return ret;
    }
    ...
    if (ret == 0) {
      ...
      if (ret == 0) {
        ldpp_dout(dpp, 20) << "FDBD4NFilterObject::" << __func__ << " version stored in update method is: " << this->source->get_object_version() << dendl;
        source->driver->get_policy_driver()->get_cache_policy()->update(dpp, head_oid_in_cache, 0, bl.length(), version, false, rgw::fdbd4n::RefCount::NOOP, y);
        ret = source->set_head_obj_dir_entry(dpp, nullptr, y, is_latest_version);
        if (ret < 0) {
          ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): BlockDirectory set method failed for head object, ret=" << ret << dendl;
        }
      } else {
        ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): put for head object failed, ret=" << ret << dendl;
      }
    } else {
      ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): failed to cache head object during eviction, ret=" << ret << dendl;
    }
  } else {
    /* 
    ...
    */
    if (params.part_num || (!params.part_num && source->is_multipart())) {
      ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): source->is_multipart()= " << source->is_multipart() << dendl;
      if (params.part_num) { 
	ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): *(params.part_num)= " << *(params.part_num) << dendl;
      }
      if (!source->is_multipart()) {
        ...
        auto ret = next->prepare(y, dpp);
        if (ret < 0) {
          ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): next->prepare failed, ret=" << ret << dendl;
          return ret;
        }
        params.parts_count = next->params.parts_count;
        return 0;
...
void FDBD4NFilterObject::FDBD4NFilterReadOp::cancel() {
  aio->drain();
}

int FDBD4NFilterObject::FDBD4NFilterReadOp::drain(const DoutPrefixProvider* dpp, optional_yield y) {
  auto c = aio->drain();
  ...
  for (auto it : blocks_info) {
    ...
    source->driver->get_policy_driver()->get_cache_policy()->update_refcount_if_key_exists(dpp, oid_in_cache, rgw::fdbd4n::RefCount::DECR, y);
  }
  if (r < 0) {
    cancel();
    return r;
...
int FDBD4NFilterObject::FDBD4NFilterReadOp::flush(const DoutPrefixProvider* dpp, rgw::AioResultList&& results, optional_yield y) {
  ...
  ldpp_dout(dpp, 20) << "FDBD4NFilterObject::In flush:: " << dendl;

  while (!completed.empty() && completed.front().id == offset) {
    auto bl = std::move(completed.front().data);

    ldpp_dout(dpp, 20) << "FDBD4NFilterObject::flush:: calling handle_data for offset: " << offset << " bufferlist length: " << bl.length() << dendl;
    ...
    if (it != blocks_info.end()) {
      ...
      ldpp_dout(dpp, 20) << "FDBD4NFilterObject::" << __func__ << " calling update for offset: " << offset << " adjusted offset: " << ofs  << " length: " << len << " oid_in_cache: " << oid_in_cache << dendl;
      ldpp_dout(dpp, 20) << "FDBD4NFilterObject::" << __func__ << " version stored in update method is: " << version << " " << source->get_object_version() << dendl;
      source->driver->get_policy_driver()->get_cache_policy()->update(dpp, oid_in_cache, ofs, len, version, std::nullopt, rgw::fdbd4n::RefCount::DECR, y);
      blocks_info.erase(it);
      if (source->dest_object && source->dest_bucket) {
        FDBD4NFilterObject* fdbd4n_dest_object = dynamic_cast<FDBD4NFilterObject*>(source->dest_object);
        std::string dest_version = fdbd4n_dest_object->get_object_version();
        rgw::fdbd4n::CacheBlock dest_block;
        ...
        dest_block.size = len;
        dest_block.cacheObj.hostsList.insert(dpp->get_cct()->_conf->rgw_fdbd4n_local_rgw_address);
        ...
        if (ret == 0) {
          rgw::sal::Attrs attrs;
          ldpp_dout(dpp, 20) << "FDBD4NFilterObject::" << __func__ << " destination object version in update method is: " << dest_version << dendl;
          // destination key is the same as key
          ret = source->driver->get_cache_driver()->put(dpp, key, bl, bl.length(), attrs, y);
          if (ret == 0) {
            source->driver->get_policy_driver()->get_cache_policy()->update(dpp, key, ofs, bl.length(), dest_version, true, rgw::fdbd4n::RefCount::NOOP, y);
          }
          if (ret = source->driver->get_block_dir()->set(dpp, &dest_block, y); ret < 0){
            ldpp_dout(dpp, 20) << "FDBD4NFilterObject::" << __func__ << " BlockDirectory set failed with ret: " << ret << dendl;
          }
        } else {
          ldpp_dout(dpp, 20) << "FDBD4NFilterObject::" << __func__ << " eviction returned ret: " << ret << dendl;
        }
      }
    } else {
      ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << " offset not found: " << offset << dendl;
    }
  ...
  ldpp_dout(dpp, 20) << "FDBD4NFilterObject::returning from flush:: " << dendl;
  return 0;
}

int FDBD4NFilterObject::FDBD4NFilterReadOp::iterate(const DoutPrefixProvider* dpp, int64_t ofs, int64_t end,
                        RGWGetDataCB* cb, optional_yield y) 
{
  ...
  ldpp_dout(dpp, 20) << "FDBD4NFilterObject::iterate:: " << "prefix: " << prefix << dendl;
  ldpp_dout(dpp, 20) << "FDBD4NFilterObject::iterate:: " << "oid: " << source->get_key().get_oid() << " ofs: " << ofs << " end: " << end << dendl;
  ...
  ldpp_dout(dpp, 20) << "FDBD4NFilterObject::iterate:: " << " adjusted_start_offset: " << adjusted_start_ofs << dendl;
  ldpp_dout(dpp, 20) << "FDBD4NFilterObject::iterate:: " << " adjusted_end_ofs: " << adjusted_end_ofs << dendl;
  ldpp_dout(dpp, 20) << "FDBD4NFilterObject::iterate:: " << " adjusted_len: " << adjusted_len << dendl;
  ldpp_dout(dpp, 20) << "FDBD4NFilterObject::iterate:: " << " len: " << len << dendl;

  if ((params.part_num && !source->is_multipart()) || !params.part_num) {
    aio = rgw::make_throttle(window_size, y);

    ldpp_dout(dpp, 20) << "FDBD4NFilterObject::iterate:: " << "max_chunk_size " << max_chunk_size << " num_parts " << num_parts << dendl;
    ...
    do {
      ...
      ldpp_dout(dpp, 20) << "FDBD4NFilterObject::iterate:: " << __func__ <<  " " << __LINE__ << "(): READ FROM CACHE: oid=" << oid_in_cache << " length to read is: " << len_to_read << " part num: " << start_part_num << 
      ...
      if (policy->update_refcount_if_key_exists(dpp, oid_in_cache, rgw::fdbd4n::RefCount::INCR, y) > 0) {
        ldpp_dout(dpp, 20) << "FDBD4NFilterObject::iterate:: " << __func__ << "(): " << __LINE__ << ": READ FROM CACHE: oid_in_cache=" << oid_in_cache << dendl;
        // Read From Cache
        ...
        ldpp_dout(dpp, 20) << "FDBD4NFilterObject::iterate:: " << __func__ << "(): Info: flushing data for oid: " << oid_in_cache << dendl;
        auto r = flush(dpp, std::move(completed), y);
        if (r < 0) {
          drain(dpp, y);
          ldpp_dout(dpp, 0) << "FDBD4NFilterObject::iterate:: " << __func__ << "(): Error: failed to flush, ret=" << r << dendl;
          return r;
        }
        ...
        if ((ret = block_dir->get(dpp, &block, y)) == 0) {
          if (block.version != version) {
            ...
            ldpp_dout(dpp, 20) << "FDBD4NFilterObject::iterate:: " << __func__ << "(): Info: Version mismatch, draining data for oid: " << oid_in_cache << dendl;
            auto r = drain(dpp, y);
            if (r < 0) {
              ldpp_dout(dpp, 0) << "FDBD4NFilterObject::iterate:: " << __func__ << "(): Error: failed to drain, ret=" << r << dendl;
              return r;
            }
            break;
          } //end if block.version != version
          auto it = block.cacheObj.hostsList.find(dpp->get_cct()->_conf->rgw_fdbd4n_local_rgw_address);
          auto hostsListSize = block.cacheObj.hostsList.size();
          if (it != block.cacheObj.hostsList.end()) {
            if ((r = block_dir->remove_host(dpp, &block, dpp->get_cct()->_conf->rgw_fdbd4n_local_rgw_address, y)) < 0) {
              ldpp_dout(dpp, 10) << "FDBD4NFilterObject::iterate:: " << __func__ << "(): Error: failed to remove incorrect host from block with oid=" << oid_in_cache <<", ret=" << r << dendl;
              hostsListSize = hostsListSize - 1;
            }
          }
          if (hostsListSize > 0) { /* Remote copy */
            ldpp_dout(dpp, 20) << "FDBD4NFilterObject::iterate:: " << __func__ << "(): Block with oid=" << oid_in_cache << " found in remote cache." << dendl;
            // TODO: Retrieve remotely
            // Policy decision: should we cache remote blocks locally?
          } else {
            ldpp_dout(dpp, 20) << "FDBD4NFilterObject::iterate:: " << __func__ << "(): Info: draining data for oid: " << oid_in_cache << dendl;
            auto r = drain(dpp, y);
            if (r < 0) {
              ldpp_dout(dpp, 0) << "FDBD4NFilterObject::iterate:: " << __func__ << "(): Error: failed to drain, ret=" << r << dendl;
              return r;
          ...
          auto r = drain(dpp, y);
          if (r < 0) {
            ldpp_dout(dpp, 10) << "FDBD4NFilterObject::iterate:: " << __func__ << "(): Error: failed to drain, ret=" << r << dendl;
            return r;
          }
          break;
        }
  ...
  ldpp_dout(dpp, 20) << "FDBD4NFilterObject::iterate:: " << __func__ << "(): Fetching object from backend store" << dendl;
  ...
  ldpp_dout(dpp, 20) << "FDBD4NFilterObject::iterate:: " << __func__ << "(): adjusted_start_ofs: " << adjusted_start_ofs << " end: " << end << dendl;
  ...
  if(perfcounter) {
    ...
    perfcounter->inc(l_rgw_fdbd4n_cache_misses, num_blocks);
  }
  
  if (r < 0) {
    ldpp_dout(dpp, 0) << "FDBD4NFilterObject::iterate:: " << __func__ << "(): Error: failed to fetch object from backend store, ret=" << r << dendl;
    return r;
  }
...
int FDBD4NFilterObject::FDBD4NFilterReadOp::get_attr(const DoutPrefixProvider* dpp, const char* name, bufferlist& dest, optional_yield y)
{
  rgw::sal::Attrs& attrs = source->get_attrs();
  if (attrs.empty()) {
    auto ret = source->get_obj_attrs(y, dpp);
    if (ret < 0) {
      ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): Error: failed to fetch attrs, ret=" << ret << dendl;
      return ret;
    }
  ...
  if (it == attrs.end()) {
    ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): Attribute value NOT found for attr name= " << name << dendl;
...
int FDBD4NFilterObject::FDBD4NFilterReadOp::FDBD4NFilterGetCB::flush_last_part()
{
  last_part = true;
  return handle_data(bl_rem, 0, bl_rem.length());
}

int FDBD4NFilterObject::FDBD4NFilterReadOp::FDBD4NFilterGetCB::handle_data(bufferlist& bl, off_t bl_ofs, off_t bl_len)
{
  ...
  if (write_to_cache) {
    ...
    rgw::fdbd4n::CacheBlock block, dest_block;
    rgw::fdbd4n::BlockDirectory* blockDir = source->driver->get_block_dir();
    ...
    if (bl.length() > 0 && last_part) { // if bl = bl_rem has data and this is the last part, write it to cache
      ...
      if (!policy->exist_key(oid)) {
        ...
        if (ret == 0) {
          ...
          if (ret == 0) {
            std::string objEtag = "";
            policy->update(dpp, oid, adjusted_start_ofs, bl.length(), version, dirty, rgw::fdbd4n::RefCount::NOOP, *y);
            blocks.emplace_back(block);
          } else {
            ldpp_dout(dpp, 10) << "FDBD4NFilterObject::FDBD4NFilterReadOp::FDBD4NFilterGetCB::" << __func__ << "(): put() to cache backend failed, ret=" << ret << dendl;
          }
        } //end-if ret == 0
      } //end-if exist_key
      if (source->dest_object && source->dest_bucket) {
        ...
        if (ret == 0) {
          ...
          if (ret == 0) {
            policy->update(dpp, dest_oid, adjusted_start_ofs, bl.length(), dest_version, dirty, rgw::fdbd4n::RefCount::NOOP, *y);
      ...
      if (bl_rem.length() == rgw_max_chunk_size) {
        std::string oid = prefix + CACHE_DELIM + std::to_string(adjusted_start_ofs) + CACHE_DELIM + std::to_string(bl_rem.length());
          ...
          if (ret == 0) {
            ...
            if (ret == 0) {
              policy->update(dpp, oid, adjusted_start_ofs, bl_rem.length(), version, dirty, rgw::fdbd4n::RefCount::NOOP, *y);
              blocks.emplace_back(block);
            } else {
              ldpp_dout(dpp, 0) << "FDBD4NFilterObject::FDBD4NFilterReadOp::FDBD4NFilterGetCB::" << __func__ << "(): put() to cache backend failed, ret=" << ret << dendl;
            }
          } else {
            ldpp_dout(dpp, 0) << "FDBD4N Filter: " << __func__ << " An error occurred during eviction, ret=" << ret << dendl;
          }
        }
        ...
        if (source->dest_object && source->dest_bucket) {
          ...
          if (ret == 0) {
            ...
            if (ret == 0) {
              policy->update(dpp, dest_oid, adjusted_start_ofs, bl_rem.length(), dest_version, dirty, rgw::fdbd4n::RefCount::NOOP, *y);
              dest_blocks.emplace_back(dest_block);
            }
          }
        }
    ...
    if (last_part) {
      auto ret = blockDir->get(dpp, blocks, *y);
      if (ret < 0) {
        ldpp_dout(dpp, 10) << "FDBD4NFilterWriter::" << __func__ << "(): BlockDirectory pipelined get() method failed, ret=" << ret << dendl;
      }
      ...
      if ((ret = blockDir->set(dpp, blocks, *y)) < 0) {
        ldpp_dout(dpp, 10) << "FDBD4NFilterWriter::" << __func__ << "(): BlockDirectory pipelined set() method failed, ret=" << ret << dendl;
      }
      if (source->dest_object && source->dest_bucket) {
        if ((ret = blockDir->set(dpp, dest_blocks, *y)) < 0) {
          ldpp_dout(dpp, 10) << "FDBD4NFilterWriter::" << __func__ << "(): BlockDirectory pipelined set() method for dest blocks failed, ret=" << ret << dendl;
        }
      }
    }// if last_part
  }//if write_to_cache
...
int FDBD4NFilterObject::FDBD4NFilterDeleteOp::delete_obj(const DoutPrefixProvider* dpp,
                                                   optional_yield y, uint32_t flags)
{
  ...
  if (!source->check_head_exists_in_cache_get_oid(dpp, head_oid_in_cache, attrs, block, y) && !block.deleteMarker) {
    ...
    ldpp_dout(dpp, 10) << "FDBD4NFilterObject::" << __func__ << "(): head object not found; calling next->delete_obj" << dendl;
    next->params = params;
    ...
    if (objDirty && !cache_request) { // head object dirty flag represents object dirty flag
      ...
      if (!source->get_bucket()->versioned() || (block.cacheObj.objName != source->get_name())) {
        ldpp_dout(dpp, 10) << "FDBD4NFilterObject::" << __func__ << "(): calling invalidate_dirty_object for: " << head_oid_in_cache << dendl;
    ...
    if (source->get_bucket()->versioned()) {
        ...
        if (!objDirty) {
          ...
          rgw::fdbd4n::CacheBlock latest_block = block;
          ...
          while(retry) {
            ...
            if (ret < 0) {
              ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): Failed to get latest entry in block directory for: " << latest_block.cacheObj.objName << ", ret=" << ret << dendl;
              return ret;
            }
            ...
            if (block.cacheObj.objName == objName) {
              ...
              if (!latest_block.deleteMarker) {
                ...
                if (ret < 0) {
                  ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): Failed to create a delete marker for: " << block.cacheObj.objName << ", ret=" << ret << dendl;
              ...
              if (ret == 0) {
                ...
                if (latest_block.version == block.version) {
                  ...
                  if (ret < 0) {
                    ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): Failed to get the second latest version for: " << dir_obj.objName << ", ret=" << ret << dendl;
                    return ret;
                  }
                  //if there is a second latest version
                  if (members.size() == 2) {
                    rgw::fdbd4n::CacheBlock version_block = latest_block;
                    version_block.cacheObj.objName = "_:" + members[1] + "_" + source->get_name();
                    //get versioned entry
                    ret = blockDir->get(dpp, &version_block, y);
                    if (ret < 0) {
                      ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): Failed to get the versioned entry for: " << version_block.cacheObj.objName << ", ret=" << ret << dendl;
                      return 0;
                    }
                    //set versioned entry as the latest entry
                    version_block.cacheObj.objName = latest_block.cacheObj.objName;
                    ldpp_dout(dpp, 20) << "FDBD4NFilterObject::" << __func__ << "(): INFO: promoting latest version entry to version: " << version_block.version << ", ret=" << ret << dendl;
                    ret = blockDir->set(dpp, &version_block, y);
                    if (ret < 0) {
                      ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): Failed to set new latest entry for: " << version_block.cacheObj.objName << ", ret=" << ret << dendl;
                      return 0;
                    }
                    ...
                    if (ret < 0) {
                      ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): Failed to delete latest entry in block directory, when it is the same as version requested, for: " << block.cacheObj.objName << ", ret=" << ret << dendl;
                      return ret;
                    }
                    //delete entry from ordered set of objects
                    ret = bucketDir->zrem(dpp, source->get_bucket()->get_bucket_id(), source->get_name(), y);
                    if (ret < 0) {
                      ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): Failed to Queue zrem request in bucket directory for: " << source->get_name() << ", ret=" << ret << dendl;
                      return ret;
                    }
					if (cache_request) {
                ...
                if ((ret = blockDir->del(dpp, &block, y)) < 0 && ret != -ENOENT) {
                  ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): Failed to delete head object in block directory for: " << block.cacheObj.objName << ", ret=" << ret << dendl;
                  return ret;
                }
                //delete entry from ordered set of versions
                std::string version = source->get_instance();
                ldpp_dout(dpp, 20) << "FDBD4NFilterObject::" << __func__ << "(): Version to be deleted is: " << version << dendl;
                ret = objDir->zrem(dpp, &dir_obj, version, y);
                if (ret < 0) {
                  ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): Failed to Queue zrem request in object directory for: " << source->get_name() << ", ret=" << ret << dendl;
                  return ret;
                }
                if (ret < 0) {
                  ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): Failed to execute exec in block directory: " << "ret= " << ret << dendl;
                  return ret;
                }
				if (cache_request) {
          ...
          if (!transaction_success) {
            ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): Redis transaction failed after retrying! " << dendl;
    ...
    if (!source->get_bucket()->versioned()) {
      ...
      ret = blockDir->del(dpp, &block, y);
      if (ret < 0) {
        ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): Failed to Queue delete head object op in block directory for: " << block.cacheObj.objName << ", ret=" << ret << dendl;
        return ret;
      }
      if (cache_request) {
		if ((ret = source->delete_cache_entry(dpp, head_oid_in_cache, y)) < 0) {
      ...
      ret = blockDir->del(dpp, &block, y);
      if (ret < 0) {
        ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): Failed to Queue delete head object in block directory for: " << block.cacheObj.objName << ", ret=" << ret << dendl;
        return ret;
      }
    ...
    if (objDirty) {
      ...
      if (attrs.find(RGW_CACHE_ATTR_OBJECT_SIZE) != attrs.end()) {
        ...
        ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): Failed to retrieve size for for: " << block.cacheObj.objName << ", ret=" << ret << dendl;
        return -EINVAL;
      }
      size = stoi(size_str);
    ...
    ldpp_dout(dpp, 20) << "FDBD4NFilterObject::" << __func__ << "(): Size of object is: " << size << dendl;
    ...
    if ((!objDirty && source->get_bucket()->versioned() && source->have_instance()) ||
      ...
      do { // loop through the data blocks
        ...
        block.size = static_cast<uint64_t>(cur_len);
          ...
          if ((ret = blockDir->get(dpp, &block, y)) < 0) {
            if (ret == -ENOENT) {
              ldpp_dout(dpp, 10) << "FDBD4NFilterObject::" << __func__ << "(): Directory entry for: " << source->get_oid() << " blockid: " << fst << " block size: " << cur_len << " does not exist; continuing" << dendl;
              fst += cur_len;
              if (fst >= lst) {
                break;
              }
              ...
              ldpp_dout(dpp, 10) << "FDBD4NFilterObject::" << __func__ << "(): Failed to retrieve directory entry for: " << source->get_oid() << " blockid: " << fst << " block size: " << cur_len << ", ret=" << ret << dendl;
              return ret;
            }
          }
          ...
          if ((ret = blockDir->del(dpp, &block, y)) == -ENOENT) { 
            ...
            ldpp_dout(dpp, 0) << "FDBD4NFilterObject::" << __func__ << "(): Failed to delete directory entry for: " << source->get_name() << " blockid: " << fst << " block size: " << cur_len << ", ret=" << ret << dendl;
            return ret;
          }
    ...
    if (!objDirty) {
      ...
      next->params = params;
      ldpp_dout(dpp, 10) << "FDBD4NFilterObject::" << __func__ << "(): object is not dirty; calling next->delete_obj" << dendl;
      ret = next->delete_obj(dpp, y, flags);
      result = next->result;
      return ret;
    }
...
int FDBD4NFilterWriter::prepare(optional_yield y) 
{
  fdbd4n_writecache = g_conf()->fdbd4n_writecache_enabled;

  if (!fdbd4n_writecache) {
    if (object->is_cache_request()) {
      return -EINVAL;
    }
    ldpp_dout(dpp, 0) << "FDBD4NFilterWriter::" << __func__ << "(): calling next->prepare" << dendl;
    ...
    if (!object->get_bucket()->versioned() || (object->get_bucket()->versioned() && !object->get_bucket()->versioning_enabled())) {
      ...
      if (object->check_head_exists_in_cache_get_oid(dpp, prev_oid_in_cache, attrs, block, y)) {
        ldpp_dout(dpp, 20) << "FDBD4NFilterWriter::" << __func__ << "(): found in cache, prev_oid_in_cache=" << prev_oid_in_cache << dendl;
      }
  ...
  if (!object->have_instance()) {
    ...
    ldpp_dout(dpp, 20) << "FDBD4NFilterWriter::" << __func__ << "(): generating version: " << version << dendl;
  } else {
    ldpp_dout(dpp, 20) << "FDBD4NFilterWriter::" << __func__ << "(): version is: " << object->get_instance() << dendl;
...
int FDBD4NFilterWriter::process(bufferlist&& data, uint64_t offset)
{
    bufferlist bl = data;
    ...
    if (!fdbd4n_writecache) {
      ...
      ldpp_dout(dpp, 10) << "FDBD4NFilterWriter::" << __func__ << "(): calling next process" << dendl;
	...
	dirty = true;
      ...
      if (ret == 0) {     
        if (bl.length() > 0) {          
          ldpp_dout(dpp, 10) << "FDBD4NFilterWriter::" << __func__ << "(): oid_in_cache is: " << oid_in_cache << dendl;
	    ...
	    if (!object->is_cache_request()) {
	      ...
	      if (ret == 0) {
		driver->get_policy_driver()->get_cache_policy()->update(dpp, oid_in_cache, ofs, bl.length(), version, dirty, rgw::fdbd4n::RefCount::NOOP, y);
	      }
            } else {
	      driver->get_policy_driver()->get_cache_policy()->update(dpp, oid_in_cache, ofs, bl.length(), version, dirty, rgw::fdbd4n::RefCount::NOOP, y);
            }
          } else {
            ldpp_dout(dpp, 0) << "FDBD4NFilterWriter::" << __func__ << "(): ERROR: writing data to the cache failed, ret=" << ret << dendl;
            return ret;
          }
        }
      }
...
int FDBD4NFilterWriter::complete(size_t accounted_size, const std::string& etag,
                       ...
                       uint32_t flags)
{
  ...
  if (fdbd4n_writecache) {
    ...
    ldpp_dout(dpp, 20) << "FDBD4NFilterWriter::" << __func__ << " size is: " << object->get_size() << dendl;
    ...
    ret = next->complete(accounted_size, etag, mtime, set_mtime, attrs, cksum,
                            ...
                            canceled, rctx, flags);
    if (ret < 0) {
      ldpp_dout(dpp, 0) << "FDBD4NFilterWriter::" << __func__ << "(): writing to backend store failed, ret=" << ret << dendl;
      return ret;
    }
  ...
  bufferlist bl;
  //same as key, as there is no len or offset attached to head oid in cache
  ldpp_dout(dpp, 20) << "FDBD4NFilterWriter::" << __func__ << "(): key is: " << key << dendl;
  ...
  if (ret == 0) {
    ...
    object->set_object_version(version);
    if (ret == 0) {
      ldpp_dout(dpp, 20) << "FDBD4NFilterWriter::" << __func__ << "(): version stored in update method is: " << version << dendl;
      driver->get_policy_driver()->get_cache_policy()->update(dpp, key, 0, bl.length(), version, dirty, rgw::fdbd4n::RefCount::NOOP, y);
      ret = object->set_head_obj_dir_entry(dpp, nullptr, y, true, dirty);
      if (ret < 0) {
        ldpp_dout(dpp, 0) << "FDBD4NFilterWriter::" << __func__ << "(): BlockDirectory set method failed for head object, ret=" << ret << dendl;
        return ret;
      }
      if (dirty) {
        auto creationTime = ceph::real_clock::to_double(object->get_mtime());
        ldpp_dout(dpp, 20) << "FDBD4NFilterWriter::" << __func__ << "(): key=" << key << dendl;
        ldpp_dout(dpp, 20) << "FDBD4NFilterWriter::" << __func__ << "(): obj->get_key()=" << obj->get_key() << dendl;
        driver->get_policy_driver()->get_cache_policy()->update_dirty_object(dpp, key, version, false, accounted_size, creationTime, std::get<rgw_user>(obj->get_bucket()->get_owner()), objEtag, obj->get_bucket()->get_name(), obj->get_bucket()->get_bucket_id(), obj->get_key(), rgw::fdbd4n::RefCount::NOOP, y);
        if (!prev_oid_in_cache.empty()) {
          driver->get_policy_driver()->get_cache_policy()->invalidate_dirty_object(dpp, prev_oid_in_cache);
        }
      }
    } else { //if get_cache_driver()->put()
      ldpp_dout(dpp, 0) << "FDBD4NFilterWriter::" << __func__ << "(): put failed for head_oid_in_cache, ret=" << ret << dendl;
      return ret;
    }
  } else {
    ldpp_dout(dpp, 0) << "FDBD4NFilterWriter::" << __func__ << "(): eviction failed for head_oid_in_cache, ret=" << ret << dendl;
    return ret;
  }
  return 0;
}

int FDBD4NFilterMultipartUpload::complete(const DoutPrefixProvider *dpp,
            ...
            const char *if_nomatch)
{
  ...
  FDBD4NFilterObject* fdbd4n_target_obj = dynamic_cast<FDBD4NFilterObject*>(target_obj);
  /* we want to always load latest object state from store
     to avoid reading stale state in case of object overwrites. */
  fdbd4n_target_obj->set_load_obj_from_store(true);
  fdbd4n_target_obj->load_obj_state(dpp, y);
  rgw::sal::Attrs attrs = fdbd4n_target_obj->get_attrs();
  fdbd4n_target_obj->set_attrs_from_obj_state(dpp, y, attrs);
  bufferlist bl_val;
  ...
  std::string version;
  fdbd4n_target_obj->calculate_version(dpp, y, version, attrs);
  if (version.empty()) {
    ldpp_dout(dpp, 10) << "FDBD4NFilterObject::" << __func__ << "(): version could not be calculated." << dendl;
  }

  bufferlist bl;
  std::string head_oid_in_cache = get_cache_block_prefix(fdbd4n_target_obj, version);
  ...
  if (ret == 0) {
    ...
    if (ret == 0) {
      ldpp_dout(dpp, 20) << "FDBD4NFilterMultipartUpload::" << __func__ << " version stored in update method is: " << fdbd4n_target_obj->get_object_version() << dendl;
      driver->get_policy_driver()->get_cache_policy()->update(dpp, head_oid_in_cache, 0, bl.length(), version, false, rgw::fdbd4n::RefCount::NOOP, y);
      ret = fdbd4n_target_obj->set_head_obj_dir_entry(dpp, nullptr, y, true);
      if (ret < 0) {
        ldpp_dout(dpp, 0) << "FDBD4NFilterMultipartUpload::" << __func__ << "(): BlockDirectory set method failed for head object, ret=" << ret << dendl;
      }
    } else {
      ldpp_dout(dpp, 0) << "FDBD4NFilterMultipartUpload::" << __func__ << "(): put for head object failed, ret=" << ret << dendl;
      return ret;
    }
  } else {
    ldpp_dout(dpp, 0) << "FDBD4NFilterMultipartUpload::" << __func__ << "(): failed to cache head object during eviction, ret=" << ret << dendl;
    return ret;
  }
...
rgw::sal::Driver* newFDBD4NFilter(rgw::sal::Driver* next, void* io_context, bool admin)
{
  auto *driver = new rgw::sal::FDBFDBD4NFilterDriver(next, *static_cast<boost::asio::io_context *>(io_context), admin);
