			    ...
			    optional_yield y)
{
  ...
  do {
    ...
    for (const auto& obj : results.objs) { 
      ...
      std::string oid_version, version;
	...
	version = it->version;
	dirty = it->cacheObj.dirty;
        lst = it->cacheObj.size;
      } else {
	ldpp_dout(dpp, 10) << "D4NFilterBucket::" << __func__ << "(): Failed to retrieve block info for " << c_obj->get_name() << dendl;
        return -ENOENT;
      }
      if (dirty) {
	if (!this->filter->get_policy_driver()->get_cache_policy()->invalidate_dirty_object(dpp, get_cache_block_prefix(c_obj.get(), version))) {
      ...
      block.cacheObj.objName = "_:" + oid_version + "_" + c_obj->get_name();
      blocks.push_back(block);
     ...
     * in redis docs */
    if ((PIPELINE_MAX - blocks.size()) < 1000) {
    ...
    if ((objects.size() % PIPELINE_MAX) < 1000) {
