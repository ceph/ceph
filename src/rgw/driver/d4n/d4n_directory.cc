...
template <typename... Types>
void redis_exec_cp(std::shared_ptr<rgw::d4n::MultiRedisPoolManager> pool_manager,
                int index,
                boost::system::error_code& ec,
                const boost::redis::request& req,
...
void redis_exec_connection_pool(const DoutPrefixProvider* dpp,
				std::shared_ptr<MultiRedisPoolManager> redis_pool_manager,
        int index,
...
int DirectoryConnection::initialize(const DoutPrefixProvider* dpp)
{
  ...
  if (rgw_redis_connection_pool_size > 0) {
    try {
      redis_pool_manager = std::make_shared<MultiRedisPoolManager>(
              &io_context,
              hosts,
              ports,
...
std::pair<std::vector<std::string>, std::vector<std::string>> DirectoryConnection::parseHostPorts(const DoutPrefixProvider* dpp, std::string_view address)
{
...
void DirectoryConnection::redis_exec_connection_pool(const DoutPrefixProvider* dpp,
          ...
          optional_yield y)
{
  rgw::d4n::redis_exec_connection_pool(dpp, redis_pool_manager, index, connections, ec, req, resp, y);
}
