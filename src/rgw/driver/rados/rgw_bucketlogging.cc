...
private:
  ...
  EntryProcessingResult process_entry(
      ...
      int& ret) {
    ret = 0;
    commit_log_entry_t commit_entry;
    auto iter = entry.data.cbegin();
    try {
      decode(commit_entry, iter);
    ...
    // TODO: 
    // 1. get pool name from entry
    // 2. get rados io_ctx for the target pool
    // 3. copy the entry from the logging pool to the target pool and create the head object.
  ...
  void process_queue(const std::string& queue_name, boost::asio::yield_context yield) {
	...
	librados::ObjectReadOperation op;
        ...
        rados::cls::lock::assert_locked(&op, queue_name+"_lock",
          ...
          lock_cookie,
          "" /*no tag*/);
	cls_queue_list_entries_start(op, &obl, &rval, start_marker, max_elements);
        ...
        ret = cls_queue_list_entries_finish(obl, entries, &truncated, end_marker);
        if (ret < 0) {
      ...
      ldpp_dout(this, 20) << "INFO: found: " << total_entries << " entries in: "
                          << queue_name << ". end marker is: " << end_marker
                          << dendl;
      ...
      if (remove_entries) {
        ...
        if (ret < 0) {
          ldpp_dout(this, 1) << "ERROR: failed to remove entries and/or lock"
                             << " queue up to: " << end_marker << " from queue: "
                             << queue_name << ". error: " << ret << dendl;
          return;
        } else {
          ldpp_dout(this, 20) << "INFO: removed entries up to: " << end_marker
                              <<  " from queue: " << queue_name << dendl;
        }
      }
    }
...
constexpr size_t MAX_QUEUE_SIZE = 4*1000*1000; // 4MB
