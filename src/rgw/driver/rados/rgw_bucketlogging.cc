...
class BucketLoggingManager : public DoutPrefixProvider {
  ...
  static constexpr auto queues_update_retry = std::chrono::milliseconds(5000); // 1s
...
private:
  ...
  std::vector<std::string> entryProcessingResultString = {"Failure", "Successful", "Sleeping", "Expired"};
  ...
  void process_queue(const std::string& queue_name, boost::asio::yield_context yield) {
	...
	op.omap_get_keys2(start_after, max_chunk, &entries, &more, &rval);
        ...
        if (ret == -EBUSY) {
          ldpp_dout(this, 10) << "WARNING: queue: " << queue_name
