...
static std::string get_commit_list_name (rgw::sal::Bucket *log_bucket,
                                         const std::string& prefix) {
  return  log_bucket->get_tenant() + "/" + log_bucket->get_name() + "/"
...
int add_commit_target_entry(const DoutPrefixProvider* dpp,
  ...
  std::string target_obj_name = get_commit_list_name (log_bucket, prefix);
  {
    ...
    op.setxattr("usr.rgw.logging_temp_pool", pool_bl);
  ...
  auto ret = rgw_rados_operate(dpp, ioctx_logging, COMMIT_LIST_OBJECT_NAME,
                               std::move(op), y);
  if (ret < 0) {
    ldpp_dout(dpp, 1) << "ERROR: failed to add entry " << new_commit_list
                      << " to global bucket logging object: "
                      << COMMIT_LIST_OBJECT_NAME << ". ret = " << ret << dendl;
    return ret;
  }
  ldpp_dout(dpp, 1) << "INFO: added entry " << target_obj_name
                    << " to global bucket logging object: "
...
*/
  ...
  int process_entry(
    ...
    if (ret == 0) {
      ...
      ldpp_dout(this, 10) << "INFO: committed logging object : " << entry
                          << " to bucket " << bucket_name
                          << ", id: "<< bucket_id << dendl;
      return 0;
    }
    // The log bucket has been deleted. Clean up pending objects.
    // This is being done in case the bucket_deletion_cleanup failed to remove
    // the temp log objects.
    bucket_deleted = true;
    ...
    if (ret < 0 && ret != -ENOENT) {
      ldpp_dout(this, 10) << "ERROR: failed to delete pending logging object : "
  ...
  //  constexpr auto max_elements = 1024;
	...
	//op.omap_get_keys2(start_after, max_chunk, &entries, &more, &rval);
        ...
        op.getxattr("usr.rgw.logging_temp_pool", &pool_obl, &pool_rval);
      ...
      for (auto const& [key, val] : entries) {
        ...
        boost::asio::spawn(yield, std::allocator_arg, make_stack_allocator(),
          [this, &is_idle, &list_obj_name, total_entries, &remove_entries,
           ...
           token = std::move(token), key, temp_obj_name, temp_data_pool, &bucket_deleted,
            &skip_bucket_deletion]
           (boost::asio::yield_context yield) {
            auto result =
      ...
      if (bucket_deleted && !skip_bucket_deletion) {
        // TODO: Don't delete the bucket list if process_entry failed?
        librados::ObjectWriteOperation op;
        rados::cls::lock::assert_locked(&op, list_obj_name+"_lock",
          ClsLockType::EXCLUSIVE,
          m_lock_cookie,
          "" /*no tag*/);
        op.remove();
        // check ownership and delete the object in one batch
        auto ret = rgw_rados_operate(this, rados_ioctx, list_obj_name, std::move(op), yield);
        if (ret < 0 ) {
          ldpp_dout(this, 10) << "ERROR: failed to remove bucket logging commit list :"
