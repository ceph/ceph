                                                 ...
                                                 prefix_map_t& processed_prefixes)
{
  ...
  do {
    ...
    for (auto part_it = parts.begin(); part_it != parts.end(); ++part_it) {
      ...
      auto it = processed_prefixes.find(part->info.num);

      if (!part->info.manifest.empty()) {
        auto manifest_prefix = part->info.manifest.get_prefix();
        if (not manifest_prefix.empty() && it->second.find(manifest_prefix) == it->second.end()) {
          auto target = meta_obj->get_obj();
          store->getRados()->update_gc_chain(dpp, target, part->info.manifest, &chain);
          ...
          if (oiter != part->info.manifest.obj_end(dpp)) {
            ...
            head->get_key().get_index_key(&remove_key);
                                               ...
                                               boost::container::flat_set<std::string>& processed_prefixes)
{
  ...
  for (auto& ppfx : part->get_past_prefixes()) {
    if (processed_prefixes.find(ppfx) != processed_prefixes.end()) {
      continue;
    }
    processed_prefixes.emplace(ppfx);
