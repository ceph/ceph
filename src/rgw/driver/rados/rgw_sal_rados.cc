...
int RadosBucket::delete_all_objects_sync(const DoutPrefixProvider* dpp,
			...
			optional_yield y)
{
  ...
  ret = delete_all_objects_sync(dpp, delete_children, y);
...
int RadosBucket::delete_all_objects_aio(const DoutPrefixProvider* dpp,
					optional_yield y,
					int concurrent_max,
...
{
  int ret = delete_all_objects_aio(dpp, y, concurrent_max, keep_index_consistent);
  if (ret < 0) {
    ldpp_dout(dpp, -1) << "ERROR: delete_all_objects_aio returned " << ret << dendl;
    return ret;
  }
...
int RadosBucket::prune(const DoutPrefixProvider* dpp, bool delete_children, optional_yield y)
{
...
int RadosBucket::prune_bypass_gc(int concurrent_max,
					bool keep_index_consistent,
					optional_yield y,
					const DoutPrefixProvider* dpp)
{
  return delete_all_objects_aio(dpp, y, concurrent_max, keep_index_consistent);
}
