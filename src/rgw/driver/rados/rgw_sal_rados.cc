...
*/
  //auto ioctx_logging = store->getRados()->get_logging_pool_ctx();
  rgw_pool logging_pool = store->svc()->zone->get_zone_params().logging_pool;
...
// Trying with omaps instead of queues
    ...
    // Add entry to the log object commit list
...
*/
  auto io_ctx = store->getRados()->get_logging_pool_ctx();
  bufferlist bl;
  bl.append(record);
  bl.append("\n");
