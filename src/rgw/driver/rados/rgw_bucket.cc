...
int RGWBucketAdminOp::prune_bucket(rgw::sal::Driver* driver, RGWBucketAdminOpState& op_state,
				   optional_yield y, const DoutPrefixProvider *dpp,
				   bool bypass_gc, bool keep_index_consistent)
{
  ...
  if (bypass_gc)
    return bucket->prune_bypass_gc(op_state.get_max_aio(), keep_index_consistent, y, dpp);

  return bucket->prune(dpp, op_state.will_delete_children(), y);
}
