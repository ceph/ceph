...
public:
  RGWDataSyncProcessorThread(rgw::sal::RadosStore *_driver, RGWAsyncRadosProcessor *async_rados,
    ...
    : RGWSyncProcessorThread(_driver->getRados(), "data-sync"),
      ...
      sync(_driver, async_rados, zid, counters.get()),
      initialized(false) {}
...
int RGWRados::init_complete(const DoutPrefixProvider *dpp)
{
  ...
  if (run_data_sync_thread) {
    ...
    for (auto entry : svc.zone->get_data_sync_source_zones()) {
      ...
      auto *thread = new RGWDataSyncProcessorThread(this->driver, svc.rados->get_async_processor(), zid, zname);
      ret = thread->init(dpp);
      if (ret < 0) {
...
int RGWRados::init_begin(const DoutPrefixProvider *dpp)
{
  ...
  ctl.si.mgr->register_sip("bucket", "bucket", { SIProvider::StageType::FULL, SIProvider::StageType::INC }, std::make_shared<RGWSIPGen_BucketContainer>(cct, driver, ctl.bucket));
                               ...
                               const FetchRemoteObjParams& params)
{
  ...
  if (params.copy_if_newer) {
    ...
    ret = get_obj_state(dpp, &obj_ctx, dest_bucket->get_info(), dest_obj.get(), &dest_state, &manifest, false, null_yield);
    if (ret < 0)
      goto set_err_state;
...
set_err_state:
  if (params.copy_if_newer && ret == -ERR_NOT_MODIFIED) {
    ...
    if (params.olh_epoch && *params.olh_epoch > 0) {
      ...
      ret = set_olh(dpp, obj_ctx, dest_bucket->get_info(), dest_obj.get(), false, nullptr,
