               ...
               std::optional<uint64_t>* bytes_transferred)
{
  ...
  set_mtime_weight.high_precision = high_precision_time;
  int ret;
  auto get_obj_name = [&src_obj, &dest_obj]() -> std::string {
    return (std::stringstream{} << "object(src=" << src_obj << ", dest=" << dest_obj << ")").str();
  };
                    ...
                    [&](map<string, bufferlist>& obj_attrs) {
                      ...
                      int ret = filter->filter(cct,
					       ...
					       &override_owner,
                                               &ptail_rule);
                      if (ret < 0) {
                        ldpp_dout(rctx.dpp, 5) << "Aborting fetch: source " << get_obj_name()
  ...
  for (int tries = 0; tries < NUM_ENPOINT_IOERROR_RETRIES; tries++) {
    ...
    if (ret < 0) {
      if (ret == -EIO && tries < NUM_ENPOINT_IOERROR_RETRIES - 1) {
        ldpp_dout(rctx.dpp, 20) << __func__ << "(): failed to fetch " << get_obj_name()
  ...
  if (cb.get_data_len() != accounted_size) {
    ret = -EIO;
    ldpp_dout(rctx.dpp, 0) << "ERROR: " << get_obj_name()
  ...
  if (const auto& verifier_etag = cb.get_verifier_etag();
    ...
    if (verifier_etag != trimmed_etag) {
      ret = -EIO;
      ldpp_dout(rctx.dpp, 0) << "ERROR: source and destination objects don't match: " << get_obj_name()
...
#define MAX_COMPLETE_RETRY 100
  for (i = 0; i < MAX_COMPLETE_RETRY; i++) {
    ...
    if (copy_if_newer && canceled) {
      ldpp_dout(rctx.dpp, 20) << "raced with another write of obj: " << get_obj_name() << dendl;
      ...
      if (!dest_state->exists ||
        ...
        ldpp_dout(rctx.dpp, 20) << "retrying writing " << get_obj_name()
                                << " mtime=" << set_mtime << " dest_state->mtime=" << dest_state->mtime
                                << " dest_state->exists=" << dest_state->exists << dendl;
        continue;
      } else {
        ldpp_dout(rctx.dpp, 20) << "not retrying writing " << get_obj_name()
                                 << " mtime=" << set_mtime << " dest_state->mtime=" << dest_state->mtime
                                 << " dest_state->exists=" << dest_state->exists << dendl;
      }
    }
  ...
  if (i == MAX_COMPLETE_RETRY) {
    ldpp_dout(rctx.dpp, 0) << "ERROR: retried " << get_obj_name()
