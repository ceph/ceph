                                            ...
                                            bool log_op)
{
  ...
  ret = store->cls_obj_complete_add(dpp, target->get_bucket_info(), *bs, obj, optag, poolid, epoch, ent, category, remove_objs, bilog_flags, zones_trace, add_log);
  if (add_log) {
                                                ...
                                                bool log_op)
{
  ...
  ret = store->cls_obj_complete_del(dpp, target->get_bucket_info(), *bs, optag, poolid, epoch, obj, removed_mtime, remove_objs, bilog_flags, zones_trace, add_log);
                                          ...
                                          bool log_op)
{
  ...
  int ret = guard_reshard(dpp, obj, &bs, [&](BucketShard *bs) -> int {
				 return store->cls_obj_complete_cancel(dpp, target->get_bucket_info(), *bs, optag, obj, remove_objs,
...
static rgw_zone_set get_zones_trace(const RGWZone& zone,
                                    const rgw_bucket& bucket,
                                    const rgw_zone_set* maybe_zones_trace)
{
  rgw_zone_set zones_trace;
			 ...
			 const bool force)
{
  ...
  do {
    ...
    ret = apply_olh_log(dpp, obj_ctx, *state, bucket_info, obj,
			state->olh_tag, log, std::move(bilog_handler), &ver_marker, y,
		      ...
		      bool skip_olh_obj_update)
{
  ...
  const auto zones_trace = get_zones_trace(svc.zone->get_zone(), bucket_info.bucket, _zones_trace);
...
#define MAX_ECANCELED_RETRY 100
  return with_bilog<CLSRGWLinkOLH<DeleteMarkerV>>(
    ...
    if (cct->_conf->rgw_debug_inject_set_olh_err) {
      ...
      ret = bucket_index_link_olh<DeleteMarkerV>(op_issuer, dpp, bucket_info, *state, target_obj,
		                              op_tag, meta, olh_epoch, unmod_since,
		                              high_precision_time, y, _zones_trace, log_data_change);
    }
    if (ret < 0) {
      ...
      int r = update_olh(dpp, obj_ctx, state, bucket_info, olh_obj, std::move(bilog_handler), y, _zones_trace, log_data_change);
      if (r < 0 && r != -ECANCELED) {
  ...
  ret = update_olh(dpp, obj_ctx, state, bucket_info, olh_obj, std::move(bilog_handler), y, _zones_trace, log_data_change);
				  ...
				  const bool force)
{
  ...
  return with_bilog<CLSRGWUnlinkInstance>(
    ...
    ret = bucket_index_unlink_instance(dpp, op_issuer, bucket_info, target_obj,
				       op_tag, olh_tag, olh_epoch, y,
				       bilog_flags, _zones_trace, log_op);
    if (ret < 0) {
      olh_cancel_modification(dpp, bucket_info, *state, olh_obj, op_tag, y);
	...
	target_obj << " returned " << ret << dendl;
      ...
      int r = update_olh(dpp, obj_ctx, state, bucket_info, olh_obj,
			 std::move(bilog_handler), y, _zones_trace, null_verid, log_op, force);
      if (r < 0 && r != -ECANCELED) {
        ldpp_dout(dpp, 20) << "update_olh() target_obj=" << olh_obj << " returned " << r << dendl;
      }
      return ret;
			       ...
			       string *max_marker, bool *syncstopped)
{
  ...
  int r = svc.bi_rados->get_dir_headers(dpp, bucket_info, idx_layout, shard_id,
                                        &headers, y);
  if (r < 0) {
    return r;
  }
  ...
  if (max_marker) {
    return svc.bilog_rados->log_get_max_marker(dpp, bucket_info, headers,
                                               shard_id, max_marker, y);
  } else {
    return 0;
  }
}
...
int RGWRados::cls_obj_complete_op(const DoutPrefixProvider *dpp, const RGWBucketInfo& bucket_info, BucketShard& bs,
                                  ...
                                  rgw_zone_set *_zones_trace, bool log_op)
{
  ...
  const auto zones_trace = get_zones_trace(svc.zone->get_zone(), bs.bucket, _zones_trace);
  ...
  return with_bilog<CLSRGWBucketModifyOpT>(
    [&, this] (auto op_issuer, auto bilog_handler) {
      ...
      ver.epoch = epoch;
      cls_rgw_guard_bucket_resharding(o, -ERR_BUSY_RESHARDING);
      op_issuer.complete_op(o, ver, dir_meta, remove_objs, op_issuer.key.name); // is it ok to substitute rgw_obj_index_key name in place of locator?
      auto* const arg = index_completion_manager->create_completion(
        ...
        [ op_issuer = std::move(op_issuer),
          ...
          cls_rgw_guard_bucket_resharding(o, -ERR_BUSY_RESHARDING);
          op_issuer.complete_op(o, ver, dir_meta, &remove_objs, op_issuer.key.name);
        });
...
int RGWRados::cls_obj_complete_add(const DoutPrefixProvider *dpp, const RGWBucketInfo& bucket_info, BucketShard& bs,
                                   const rgw_obj& obj, string& tag,
                                   int64_t pool, uint64_t epoch,
                                   rgw_bucket_dir_entry& ent, RGWObjCategory category,
                                   list<rgw_obj_index_key> *remove_objs, uint16_t bilog_flags,
...
{
  return cls_obj_complete_op<CLSRGWCompleteModifyOp<CLS_RGW_OP_ADD>>(dpp, bucket_info, bs, obj, tag, pool, epoch,
...
int RGWRados::cls_obj_complete_cancel(const DoutPrefixProvider *dpp, const RGWBucketInfo& bucket_info, BucketShard& bs,
