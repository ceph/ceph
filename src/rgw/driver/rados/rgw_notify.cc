...
namespace rgw::notify
{
  ...
  private:
    ...
    public:
      ...
      void async_wait(boost::asio::yield_context yield)
      {
        ...
        ldpp_dout(dpp, 20) << "INFO: tokens waiter is waiting on " << pending_tokens << " tokens" << dendl;
      ...
      int &ret)
  {
    ...
    const auto topic_persistency_ttl = event_entry.time_to_live != DEFAULT_GLOBAL_VALUE ? event_entry.time_to_live : conf->rgw_topic_persistency_time_to_live;
    const auto topic_persistency_max_retries = event_entry.max_retries != DEFAULT_GLOBAL_VALUE ? event_entry.max_retries : conf->rgw_topic_persistency_max_retries;
    const auto topic_persistency_sleep_duration = event_entry.retry_sleep_duration != DEFAULT_GLOBAL_VALUE ? event_entry.retry_sleep_duration : conf->rgw_topic_persistency_sleep_duration;
  ...
  void cleanup_queue(const std::string &queue_name, boost::asio::yield_context yield)
  {
    ...
    while (!shutdown)
    {
      ...
      ldpp_dout(this, 20) << "INFO: performing stale reservation cleanup for queue: " << queue_name << ". next cleanup will happen at: " << std::ctime(&tp) << dendl;
                     ...
                     boost::asio::yield_context yield)
  {
    ...
    void process_queue(const std::string &queue_name, boost::asio::yield_context yield)
    {
      ...
      boost::asio::spawn(make_strand(io_context), std::allocator_arg, make_stack_allocator(), [this, queue_name](boost::asio::yield_context yield)
                         { cleanup_queue(queue_name, yield); }, [](std::exception_ptr eptr)
                         {
      ...
      while (!shutdown)
      {
        ...
        for (auto &entry : entries)
        {
          ...
          boost::asio::spawn(yield, std::allocator_arg, make_stack_allocator(), [this, &is_idle, &notifs_persistency_tracker, &queue_name, entry_idx, total_entries, &end_marker, &remove_entries, &stop_processing, token = std::move(token), &entry, &needs_migration_vector, push_endpoint = push_endpoint.get(), &topic_info](boost::asio::yield_context yield)
                             {
            ...
            stop_processing = true; }, [](std::exception_ptr eptr)
                             {
          if (eptr) std::rethrow_exception(eptr); });
          ++entry_idx;
        }
      ...
      while (!shutdown)
      {
        ...
        for (const auto &queue_name : queues)
        {
          ...
          if (owned_queues.insert(queue_name).second)
          {
            ...
            boost::asio::spawn(make_strand(io_context), std::allocator_arg, make_stack_allocator(), [this, &queue_gc, &queue_gc_lock, queue_name, &processed_queue_count](boost::asio::yield_context yield)
                               {
            ...
            --processed_queue_count;
            ldpp_dout(this, 10) << "INFO: queue: " << queue_name << " marked for removal" << dendl; }, [](std::exception_ptr eptr)
                               {
            if (eptr) std::rethrow_exception(eptr); });
          }
          else
  ...
  public:
    ...
    void init()
    {
      boost::asio::spawn(make_strand(io_context), std::allocator_arg, make_stack_allocator(), [this](boost::asio::yield_context yield)
                         { process_queues(yield); }, [](std::exception_ptr eptr)
                         {
          if (eptr) std::rethrow_exception(eptr); });
    ...
    Manager(CephContext * _cct, rgw::sal::RadosStore * store, const SiteConfig &site) : cct(_cct),
                      ...
                      const RGWObjTags *req_tags)
  {
        ...
        const reservation_t &res, rgw::sal::Object *obj)
    {
      ...
      if (src_obj->get_attrs().empty())
      {
        ...
        if (ret < 0)
        {
          ldpp_dout(res.dpp, 20) << "failed to get attributes from object: " << src_obj->get_key() << ". ret = " << ret << dendl;
          return nullptr;
        }
      }
      return src_obj;
                                      ...
                                      rgw_pubsub_s3_event &event)
    {
        ...
        if (bl.length() > res.size)
        {
          ...
          if (ret < 0)
          {
            ldpp_dout(dpp, 1) << "ERROR: failed to abort reservation: "
                              << topic.res_id << " when trying to make a larger reservation on queue: " << shard_name
                              << ". error: " << ret << dendl;
            return ret;
          }
      ...
      for (auto &bucket_topic : bucket_topics.topics)
      {
        ...
        for (auto &event_type : event_types)
        {
          ...
          const std::string &topic_tenant = std::visit(fu2::overload(
                                                           [](const rgw_user &u) -> std::string
                                                           { return u.tenant; },
                                                           [](const rgw_account_id &a) -> std::string
                                                           { return a; }),
                                                       topic_cfg.owner);
      ...
      if (ret < 0)
      {
        ldpp_dout(res.dpp, 1) << "ERROR: failed to abort reservation: "
                              << topic.res_id << " from queue: " << shard_name << ". error: " << ret << dendl;
        return ret;
      }
      ...
      reservation_t::reservation_t(const DoutPrefixProvider *_dpp,
                                   ...
                                   optional_yield y) : dpp(_dpp), store(_store), s(nullptr), size(0) /* XXX */,
