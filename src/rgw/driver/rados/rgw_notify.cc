...
static inline uint32_t get_target_shard(const DoutPrefixProvider* dpp, const std::string& bucket_name, const std::string& object_key, const uint32_t num_shards) {
...
static inline std::string get_shard_name(const std::string& topic_name, const uint32_t& shard_id) {
...
int publish_abort(reservation_t& res) {
  for (auto& topic : res.topics) {
    ...
    uint64_t target_shard = topic.shard_id;   
    const auto& shard_name = target_shard == 0 ? topic.cfg.dest.persistent_queue : topic.cfg.dest.persistent_queue + "." + std::to_string(target_shard);     
    ldpp_dout(res.dpp, 1) << "INFO: target_shard: " + shard_name <<dendl;
    librados::ObjectWriteOperation op;
    cls_2pc_queue_abort(op, topic.res_id);
