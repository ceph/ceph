...
public:
  RGWInitDataSyncStatusCoroutine(
    RGWDataSyncCtx* _sc, uint64_t instance_id,
...
int RGWRemoteDataLog::read_log_info(const DoutPrefixProvider *dpp, rgw_datalog_info *log_info)
{
  ...
  int ret = sc.conns.data->get_json_resource(dpp, "/admin/log", pairs, null_yield, *log_info);
  if (ret < 0) {
...
int RGWRemoteDataLog::read_recovering_shards(const DoutPrefixProvider *dpp, const int num_shards, set<int>& recovering_shards)
{
  ...
  int ret = local_call(dpp, [&](RGWCoroutinesManager& crs, RGWDataSyncCtx& sc_local) {
    ...
    return crs.run(dpp, new RGWReadDataSyncRecoveringShardsCR(&sc_local, max_entries, num_shards, omapkeys));
  });
...
int RGWRemoteDataLog::init_sync_status(const DoutPrefixProvider *dpp, int num_shards)
{
  int ret = local_call(dpp, [&](RGWCoroutinesManager& crs, RGWDataSyncCtx& sc_local) {
    ...
    return crs.run(dpp, new RGWRDL::DataSyncInitCR(&sc_local, num_shards,
  ...
  int operate(const DoutPrefixProvider *dpp) override {
    reenter(this) {
      yield {
        ...
        string p = "/admin/log/";
        call(new RGWReadRESTResourceCR<rgw_bucket_index_marker_info>(sync_env->cct, sc->conns.data, sync_env->http_manager, p, pairs, info));
      }
      if (retcode < 0) {
        return set_cr_error(retcode);
      }
	...
	truncated = result.truncated;
      ...
      if (!failed) {
        ...
        yield call(sync_env->error_logger->log_error_cr(
		     dpp, sc->conns.data->get_remote_id(), "data.init", "",
    ...
    reenter(this) {
      ...
      if (sync_status < 0) {
        ...
        if (sync_status != -EBUSY && sync_status != -EAGAIN) {
          yield call(sync_env->error_logger->log_error_cr(dpp, sc->conns.data->get_remote_id(), "data",
  ...
  int operate(const DoutPrefixProvider *dpp) override {
	...
	if (retcode < 0) {
      ...
      if ((rgw_data_sync_info::SyncState)sync_status.sync_info.state == rgw_data_sync_info::StateInit) {
        ...
        yield call(new RGWInitDataSyncStatusCoroutine(sc, instance_id, tn,
...
int RGWDataSyncStatusManager::init(const DoutPrefixProvider *dpp)
{
  ...
  int r = source_log.init(source_zone, conns, error_logger, driver->getRados()->get_sync_tracer(),
  ...
  int operate(const DoutPrefixProvider *dpp) override {
    reenter(this) {
      yield {
        ...
        call(new RGWReadRESTResourceCR<bucket_list_result>(sync_env->cct, sc->conns.data, sync_env->http_manager, p, pairs, result));
      }
      if (retcode < 0) {
        return set_cr_error(retcode);
      }
  ...
  int operate(const DoutPrefixProvider *dpp) override {
    reenter(this) {
      ...
      yield {
        ...
        call(new RGWReadRESTResourceCR<bilog_list_result>(sync_env->cct, sc->conns.data, sync_env->http_manager,
					    ...
					    std::ostream* ostr)
{
  ...
  for (auto& pipe : pipes) {
    ...
    auto conns = driver->ctl()->remote->zone_conns(szone);
    if (!conns) {
    ...
    sources.emplace_back(&sync_env, szone, *conns,
						...
						uint64_t* num_shards)
{
  ...
  auto r = rgw_read_remote_bilog_info(dpp, s.sc.conns.data, s.info.bucket,
                                    ...
                                    std::vector<rgw_bucket_shard_sync_info> *status)
{
  ...
  RGWDataSyncCtx sc;
  sc.init(&env, RGWRemoteCtl::Conns(), *pipe.source.zone);
