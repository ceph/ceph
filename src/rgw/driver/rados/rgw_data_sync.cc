...
int RGWBucketFullSyncCR::operate(const DoutPrefixProvider *dpp)
{
  reenter(this) {
    ...
    if (sync_result == 0) {
      if (no_lease) {
        // re-read objv. a background sync process may have advanced
        objv.clear();
        yield call(new RGWSimpleRadosReadCR<rgw_bucket_sync_status>(
                dpp, sync_env->driver, status_obj, &sync_status, false, &objv));
        if (retcode < 0) {
          tn->log(0, SSTR("ERROR: failed to re-read sync status before final write. retcode=" << retcode));
          return set_cr_error(retcode);
        }
      }
