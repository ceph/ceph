...
namespace rgw::dedup {
                       ...
                       std::string *oid)
  {
    ...
    *oid = build_oid(p_rec->bucket_id, p_rec->obj_name);
  ...
  {
    // TBD:
    // chose a better name for the tail-object
    //return "shadow_" + manifest.get_prefix() + "0";
    //return "shadow_" + manifest.get_prefix() + "000";
    const unsigned MAX_NAME = 1024;
    char buff[MAX_NAME];
    int n = snprintf(buff, sizeof(buff), "%s_%s000",
                     RGW_OBJ_NS_SHADOW, manifest.get_prefix().c_str());
  ...
  //---------------------------------------------------------------------------
  static inline bool should_split_head(uint64_t head_size, uint64_t obj_size)
  {
    // TBD: make max_obj_size_for_split configurable
    // max_obj_size_for_split of zero means don't split!
    return (head_size > 0 && max_obj_size_for_split && obj_size <= max_obj_size_for_split);
  }
                                            ...
                                            const RGWObjManifest &manifest)
  {
    ...
    for (auto p = manifest.obj_begin(dpp); p != manifest.obj_end(dpp); ++p, ++idx) {
      ...
      ObjectWriteOperation op;
      bool implicit_ref = true;
      cls_refcount_get(op, ref_tag, implicit_ref);
                                     ...
                                     bufferlist           &new_manifest_bl)
  {
    ...
    // Only the tail object are replaced:
    // Use SRC manifest description for the tail-objects
    // while maintaining the head-object attributes of the TGT manifest
    ...
    // TBD:
    // Might need to adjust rules for the first offset if head_size in non-zero
                               ...
                               const std::string            &tail_oid)
  {
    ...
    int ret = get_ioctx(dpp, driver, rados, p_src_rec, &src_ioctx, &src_oid);
    if (unlikely(ret != 0)) {
    ...
    ret = get_ioctx(dpp, driver, rados, p_tgt_rec, &tgt_ioctx, &tgt_oid);
    if (unlikely(ret != 0)) {
      ldpp_dout(dpp, 1) << __func__ << "::ERR: failed TGT get_ioctx()" << dendl;
      if (p_src_rec->s.flags.is_split_head()) {
        remove_created_tail_object(dpp, src_ioctx, tail_oid);
      }
      return ret;
    }
    ...
    librados::ObjectWriteOperation tgt_op;
    {
      ...
      // TBD: need to edit src manifest - obj and head_placement_rule
      bufferlist new_manifest_bl;
                                          ...
                                          md5_stats_t           *p_stats) /*IN-OUT*/
  {
    ...
    if (itr != attrs.end()) {
      ...
      // TBD: should we check that the placement_rule itself is valid??
                                        ...
                                        remapper_t       *remapper)
  {
    ...
    // reset flags
    p_rec->s.flags.clear();
    ret = add_obj_attrs_to_record(&b, p_rec, attrs, p_stats);
    if (unlikely(ret != 0)) {
                                      ...
                                      md5_stats_t *p_stats)
  {
    ...
    while (p != manifest.obj_end(dpp)) {
      ...
      if (offset == 0) {
        ...
        // TBD: maybe should take the ns from the first tail-object ?
        ...
        // at the RGWObjManifest::obj_iterator code it is clear obj is not raw.
                                    ...
                                    md5_stats_t *p_stats)
  {
    ...
    // TBD: are we guarantee to have a valid tail_placement ???
    const rgw_bucket_placement &tail_placement = src_manifest.get_tail_placement();
    const rgw_bucket *p_bucket = nullptr;
    if (!tail_placement.bucket.name.empty()) {
      p_bucket = &tail_placement.bucket;
    }
    else {
      const rgw_obj& head_obj = src_manifest.get_obj();
      p_bucket = &head_obj.bucket;
    }

    // TBD: should take the bucket from the tail
    tail_oid = build_oid(p_bucket->bucket_id, tail_name);
    ret = ioctx.create(tail_oid, exclusive);
    if (ret >= 0) {
    ...
    ret = ioctx.write_full(tail_oid, bl);
    if (unlikely(ret < 0)) {
      ...
      ldpp_dout(dpp, 10) << __func__ << "::wrote tail obj:" << tail_oid << "::ret="
                         << ret << dendl;
    }
                                      ...
                                      remapper_t      *remapper)
  {
    ...
    if (p_tgt_rec->s.flags.has_shared_manifest()) {
      ...
      ldpp_dout(dpp, 1) << __func__ << "::(3)skipped shared_manifest, block_id="
                        << block_id << "::rec_id=" << (int)rec_id << dendl;
      // TBD: replace with a stat-counter
      ceph_abort("unexpcted shared_manifest");
      return 0;
    }
#endif
    ...
    if (p_src_rec->obj_name == p_tgt_rec->obj_name && p_src_rec->bucket_name == p_tgt_rec->bucket_name) {
    ...
    if (unlikely(!success)) {
      if (p_src_rec->s.flags.hash_calculated() && !src_val.has_valid_hash()) {
        ...
        ret = write_hash_object_attribute(dpp, driver, rados, p_src_rec, p_stats);
        if (ret == 0) {
      ...
      if (p_tgt_rec->s.flags.hash_calculated()) {
        ...
        write_hash_object_attribute(dpp, driver, rados, p_tgt_rec, p_stats);
      }
      return 0;
    }
                                    ...
                                    md5_shard_t num_md5_shards)
  {
    ...
    //DEDUP_DYN_ALLOC
    dedup_table_t table(dpp, d_head_object_size, d_min_obj_size_for_dedup, raw_mem,
                        raw_mem_size);
