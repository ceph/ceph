...
// a EFwwwwwwwwwwwwww-aaaaaaaaaaaaaaao-]]]]]]]]]]oooooooooooooooo-a
...
try
{
 ...
 if(0 == offset && 0 == count) {
  ...
  msg("JFW: I was too lazy to implement offset and limit, but I know how and where we can do it");
  return -1;
 }
...
//JFW: TODO: magic goes here buffer_to_values(values_input_buffer, values);

 msg(fmt::format("JFW associative arrays aren't implemented; {} => {}", key, values_input_buffer));
...
{
get entry
write value with different key
}
catch(const lfdb::exception& e)
{
 log_error(dpp, "{}", e.what());
...
try
{
 // JFW: again, a bit unclear what this is for-- my bet is that we select the entries from key "object" and then delete the fields
