...
int RGWOrphanSearch::build_linked_oids_for_bucket(const DoutPrefixProvider *dpp, const string& bucket_instance_id, map<int, list<string> >& oids)
{
  ...
  if (ret < 0) {
    ldpp_dout(this, 0) << __func__ << " failed to parse bucket instance: "
  ...
  if (cur_bucket_info.bucket.bucket_id != orphan_bucket.bucket_id) {
    ldpp_dout(this, 0) << __func__ << ": Skipping stale bucket instance: "
                           << orphan_bucket.name << ": "
  ...
  if (cur_bucket_info.reshard_status == cls_rgw_reshard_status::IN_PROGRESS) {
    ldpp_dout(this, 0) << __func__ << ": reshard in progress. Skipping "
                           << orphan_bucket.name << ": "
                           << orphan_bucket.bucket_id << dendl;
    return 0;
  }
  ...
  ldpp_dout(this, 10) << "building linked oids for bucket instance: " << bucket_instance_id << dendl;
  ...
  do {
    ...
    for (vector<rgw_bucket_dir_entry>::iterator iter = result.begin(); iter != result.end(); ++iter) {
      ...
      if (entry.key.instance.empty()) {
        ldpp_dout(this, 20) << "obj entry: " << entry.key.name << dendl;
      } else {
        ldpp_dout(this, 20) << "obj entry: " << entry.key.name << " [" << entry.key.instance << "]" << dendl;
      }

      ldpp_dout(this, 20) << __func__ << ": entry.key.name=" << entry.key.name << " entry.key.instance=" << entry.key.instance << dendl;
...
int RGWOrphanSearch::build_linked_oids_index(const DoutPrefixProvider *dpp)
{
  ...
  for (; iter != buckets_instance_index.end(); ++iter) {
    ldpp_dout(this, 0) << "building linked oids index: " << iter->first << "/" << buckets_instance_index.size() << dendl;
    bool truncated;
    ...
    do {
      ...
      for (map<string, bufferlist>::iterator eiter = entries.begin(); eiter != entries.end(); ++eiter) {
        ldpp_dout(this, 20) << " indexed entry: " << eiter->first << dendl;
...
int RGWOrphanSearch::run(const DoutPrefixProvider *dpp)
{
  ...
  switch (search_stage.stage) {
    ...
    case ORPHAN_SEARCH_STAGE_INIT:
      ldpp_dout(this, 0) << __func__ << "(): initializing state" << dendl;
    ...
    case ORPHAN_SEARCH_STAGE_LSPOOL:
      ldpp_dout(this, 0) << __func__ << "(): building index of all objects in pool" << dendl;
    ...
    case ORPHAN_SEARCH_STAGE_LSBUCKETS:
      ldpp_dout(this, 0) << __func__ << "(): building index of all bucket indexes" << dendl;
    ...
    case ORPHAN_SEARCH_STAGE_ITERATE_BI:
      ldpp_dout(this, 0) << __func__ << "(): building index of all linked objects" << dendl;
...
{
  ldpp_dout(this, 10) << "RGWRadosList::" << __func__ <<
    ...
    for (std::vector<rgw_bucket_dir_entry>::iterator iter = result.begin();
	 ...
	 ++iter) {
      ...
      if (entry.key.instance.empty()) {
        ...
        ldpp_dout(this, 20) << "obj entry: " << entry.key.name <<
	  " [" << entry.key.instance << "]" << dendl;
      }

      ldpp_dout(this, 20) << __func__ << ": entry.key.name=" <<
  ...
  RGWBucketInfo& bucket_info)
{
  ...
  do {
    ...
    if (ret == -ENOENT) {
      ...
      ldpp_dout(this, 20) << "RGWRadosList::" << __func__ <<
