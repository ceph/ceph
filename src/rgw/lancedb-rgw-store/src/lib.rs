...
#[no_mangle]
pub unsafe extern "C" fn rgw_lancedb_create_provider(
...
///
/// # Safety
/// - `provider` must be a valid pointer returned by rgw_lancedb_create_provider
/// - Must not be called if provider was passed to lancedb_registry_insert_provider()
#[no_mangle]
pub unsafe extern "C" fn rgw_lancedb_free_provider(provider: *mut LanceDBObjectStoreProvider) {
    if !provider.is_null() {
...
#[no_mangle]
pub extern "C" fn lancedb_rgw_store_version() -> *const std::os::raw::c_char {
...
mod tests {
    ...
    #[test]
    fn test_version() {
        let version = lancedb_rgw_store_version();
    ...
    fn test_create_provider_null_args() {
        assert!(unsafe { rgw_lancedb_create_provider(std::ptr::null_mut(), std::ptr::null()) }.is_null());
    }

    #[test]
    fn test_provider_lifecycle() {
        let fake_driver = 0x1234usize as *mut RGWSalDriver;
        let fake_dpp = 0x5678usize as *const RGWDoutPrefix;

        let provider = unsafe { rgw_lancedb_create_provider(fake_driver, fake_dpp) };
        assert!(!provider.is_null());

        // Free should not crash
        unsafe { rgw_lancedb_free_provider(provider) };
    }

    #[test]
    fn test_provider_free_null_safe() {
        unsafe { rgw_lancedb_free_provider(std::ptr::null_mut()) };
    }
}
