...
async fn aros_multipart_out_of_order() {
    ...
    // Need to verify if RGW SAL backend supports out-of-order part uploads.
    // S3 spec allows this, but implementation behavior may vary.
    // TODO: Investigate RGW SAL multipart ordering requirements and enable if supported.
...
async fn aros_multipart_race_condition() {
    ...
    // to the same path simultaneously. Need to verify RGW SAL's behavior in this scenario:
    // - Does it handle concurrent multipart uploads to the same key safely?
    // - What are the consistency guarantees?
    // TODO: Review RGW SAL concurrency model before enabling this test.
...
async fn aros_put_get_attributes() {
    ...
    // RGW supports user metadata (x-amz-meta-* headers), but the SAL wrapper
    // does not yet expose this functionality through the Attributes API.
    // TODO: Add Attributes support to rgw_sal_wrapper.h and RGWObjectStore.
