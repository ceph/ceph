...
public:
  ...
  int set_obj_attrs(bc::flat_map<string, bufferlist>& rgw_attrs);
...
public:
  ...
  int send_ready(RGWAccessKey& key, bc::flat_map<string, bufferlist>& rgw_attrs, bool send);
  int send_ready(RGWAccessKey& key, const std::map<string, string>& http_attrs,
