...
int FilterLuaManager::list_scripts(const DoutPrefixProvider* dpp, optional_yield y, const std::string& list_meta_key,
        const std::string& unnamed_script_key, RGWObjVersionTracker* objv, std::vector<std::string>& scripts)
{
  return next->list_scripts(dpp, y, list_meta_key, unnamed_script_key, objv, scripts);
}
