...
int FilterStore::get_bucket(const DoutPrefixProvider* dpp, User* u, const rgw_bucket& b, std::unique_ptr<Bucket>* bucket, optional_yield y)
{
  ...
  if (u)
    nu = dynamic_cast<FilterUser*>(u)->get_next();
				  ...
				  uint64_t *cur_accounted_size)
{
  ...
  std::unique_ptr<Object> no =
    dynamic_cast<FilterObject*>(_head_obj.get())->get_next()->clone();
  ...
  return std::make_unique<FilterWriter>(std::move(writer));
}
...
{
  FilterUser* fu = dynamic_cast<FilterUser*>(user);
...
{
  bucket = b;
  next->set_bucket(dynamic_cast<FilterBucket*>(b)->get_next());
};
				    ...
				    rgw::sal::Object* target_obj)
{
  return next->complete(dpp, y, cct, part_etags, remove_objs, accounted_size,
			...
			dynamic_cast<FilterObject*>(target_obj)->get_next());
}
