...
int RGWGC::list(int *index, string& marker, uint32_t max, bool expired_only, std::list<cls_rgw_gc_obj_info>& result, bool *truncated, bool& processing_queue)
{
  ...
  for (; *index < max_objs && result.size() < max; (*index)++, marker.clear(), check_queue = false) {
    ...
    if (! transitioned_objects_cache[*index] && ! check_queue && ! processing_queue) {
      ...
      cls_rgw_gc_list(op, obj_names[*index], marker, max - result.size(), expired_only, entries, truncated, next_marker);
      if (ret != -ENOENT && ret < 0) {
        return ret;
      }
      obj_version objv;
      ...
      if (ret == -ENOENT || entries.size() == 0) {
        if (objv.ver == 0) {
          ...
          if (! expired_only) {
            ...
            cls_rgw_gc_list(op, obj_names[*index], marker, 1, false, non_expired_entries, truncated, next_marker);
                   ...
                   RGWGCIOManager& io_manager)
{
  ...
  do {
    ...
    if (! transitioned_objects_cache[index]) {
      librados::ObjectReadOperation op;
      cls_rgw_gc_list(op, obj_names[index], marker, max, expired_only, entries, &truncated, next_marker);
      ldpp_dout(this, 20) <<
