...
namespace rgw::flight {
  ...
  FlightStore::FlightStore(boost::intrusive_ptr<ceph::common::CephContext>& _cct) :
    cct(_cct),
  ...
  MemoryFlightStore::MemoryFlightStore(boost::intrusive_ptr<ceph::common::CephContext>& _cct) :
    FlightStore(_cct)
  { }
  ...
  FlightKey MemoryFlightStore::add_flight(FlightData&& flight) {
    ...
    {
      const std::lock_guard<decltype(mtx)> lock(mtx);
  ...
  FlightServer::FlightServer(boost::intrusive_ptr<ceph::common::CephContext>& _cct,
    ...
    cct(_cct),
    dp(_cct.get(), dout_subsys, "rgw arrow_flight: "),
    store(_store),
  ...
  arw::Status FlightServer::DoGet(const flt::ServerCallContext &context,
    ...
    std::unique_ptr<rgw::sal::User> user = store->get_user(fd.user_id);
    if (user->empty()) {
    ...
    ret = store->get_bucket(&dp, &(*user), fd.tenant_name, fd.bucket_name,
			    &bucket, null_yield);
    if (ret < 0) {
