...
public:
  ...
  int do_get(RGWSI_MetaBackend_Handler::Op *op, string& entry, RGWMetadataObject **obj) override {
    ...
    int ret = svc.bucket->read_bucket_entrypoint_info(ctx, entry, &be, &ot, &mtime, &attrs);
    if (ret < 0)
      return ret;
  ...
  int do_remove(RGWSI_MetaBackend_Handler::Op *op, string& entry, RGWObjVersionTracker& objv_tracker) override {
    ...
    int ret = svc.bucket->read_bucket_entrypoint_info(ctx, entry, &be, &objv_tracker, &orig_mtime, nullptr);
    if (ret < 0)
      return ret;
...
public:
  ...
  int do_remove(RGWSI_MetaBackend_Handler::Op *op, string& entry, RGWObjVersionTracker& objv_tracker) override {
    ...
    RGWBucketEntryPoint be;
    map<string, bufferlist> attrs;
    int ret = svc.bucket->read_bucket_entrypoint_info(ctx, entry, &be, &objv_tracker, &mtime, &attrs);
    if (ret < 0) {
        return ret;
    }
    ...
    ret = ctl.bucket->read_bucket_instance_info(be.bucket, &old_bi, RGWBucketCtl::BucketInstance::GetParams()
    ...
    ret = ctl.bucket->remove_bucket_instance_info(be.bucket, old_bi);
    if (ret < 0) {
...
#warning mtime?
    return svc.bucket->remove_bucket_instance_info(ctx, entry, objv_tracker);
  }
...
public:
  ...
  int do_get(RGWSI_MetaBackend_Handler::Op *op, string& entry, RGWMetadataObject **obj) override {
    ...
    int ret = svc.bucket->read_bucket_instance_info(ctx, entry, &bci.info, &mtime, &bci.attrs);
    if (ret < 0)
      return ret;
  ...
  int do_remove(RGWSI_MetaBackend_Handler::Op *op, string& entry, RGWObjVersionTracker& objv_tracker) override {
    ...
    int ret = read_bucket_instance_entry(ctx, entry, &bci, nullptr);
    if (ret < 0 && ret != -ENOENT)
      return ret;
                                            ...
                                            ceph::optional_ref_default<RGWBucketCtl::BucketInstance::PutParams> _params)
{
  return bmi_handler->call([&](RGWSI_Bucket_BI_Ctx& ctx) {
    return do_store_bucket_instance_info(ctx, bucket, info, _params);
  });
}
  ...
  return call([&](RGWSI_Bucket_X_Ctx& ctx) {
    ...
    if (!bucket_info.has_instance_obj) {
      /* an old bucket object, need to convert it */
        int ret = convert_old_bucket_info(ctx, bucket);
        if (ret < 0) {
...
{
  return call([&](RGWSI_Bucket_X_Ctx& ctx) {
    return svc.bucket->read_bucket_stats(ctx, bucket, result);
  });
}
...
{
  return call([&](RGWSI_Bucket_X_Ctx& ctx) {
    return svc.bucket->read_buckets_stats(ctx, m);
  });
}
