...
int RGWREST::preprocess(req_state *s, rgw::io::BasicClient* cio)
{
  ...
  for (const auto& [http_header, attr] : generic_attrs_map) {
    ...
    if (env) {
      s->generic_attrs[attr.rgw_attr] = env;
    }
  }
