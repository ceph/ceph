...
class RGWOp_link_user_account : public RGWRESTOp {
public:
  int check_caps(const RGWUserCaps& caps) override {
  ...
  const char* name() const override { return "link_user_account"; }
};


void RGWOp_link_user_account::execute()
{
  ...
  if (link) {
    http_ret = RGWUserAdminOp_User::link_account(store, op_state, flusher, s->yield);
  } else {
    http_ret = RGWUserAdminOp_User::unlink_account(store, op_state, flusher, s->yield);
  }
...
RGWOp *RGWHandler_User::op_put()
{
  ...
  if (s->info.args.sub_resource_exists("account"))
    return new RGWOp_link_user_account;
