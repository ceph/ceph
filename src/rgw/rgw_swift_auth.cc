...
void RGW_SWIFT_Auth_Get::execute(const jaeger_tracing::Span& parent_span)
{
  int ret = -EPERM;
  ...
  if (swift_key->key.compare(key) != 0) {
    dout(0) << "NOTICE: RGW_SWIFT_Auth_Get::execute(const jaeger_tracing::Span& parent_span): bad swift key" << dendl;
    ret = -EPERM;
    goto done;
  }
