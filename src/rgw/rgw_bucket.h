...
extern int rgw_remove_object(rgw::sal::RGWRadosStore *store, const RGWBucketInfo& bucket_info, const rgw_bucket& bucket, rgw_obj_key& key, const jaeger_tracing::jspan* const parent_span = nullptr);
...
public:
  ...
  int store_bucket_entrypoint_info(const rgw_bucket& bucket,
                                   RGWBucketEntryPoint& info,
                                   optional_yield y,
                                   const Bucket::PutParams& params = {}, const jaeger_tracing::jspan* const parent_span = nullptr);
  ...
  int store_bucket_instance_info(const rgw_bucket& bucket,
                                 RGWBucketInfo& info,
                                 optional_yield y,
                                 const BucketInstance::PutParams& params = {}, const jaeger_tracing::jspan* const parent_span = nullptr);
  ...
  int link_bucket(const rgw_user& user_id,
		  ...
		  optional_yield y,
                  ...
                  rgw_ep_info *pinfo = nullptr, const jaeger_tracing::jspan* const parent_span = nullptr);

  int unlink_bucket(const rgw_user& user_id,
                    const rgw_bucket& bucket,
		    optional_yield y,
                    bool update_entrypoint = true, const jaeger_tracing::jspan* const parent_span = nullptr);
...
private:
  ...
  int do_link_bucket(RGWSI_Bucket_EP_Ctx& ctx,
		     ...
		     optional_yield y,
                     ...
                     rgw_ep_info *pinfo, const jaeger_tracing::jspan* const parent_span = nullptr);

  int do_unlink_bucket(RGWSI_Bucket_EP_Ctx& ctx,
                       const rgw_user& user_id,
                       const rgw_bucket& bucket,
		       optional_yield y,
                       bool update_entrypoint, const jaeger_tracing::jspan* const parent_span = nullptr);
