...
namespace rgw {
  ...
  int RGWLibFS::unlink(RGWFileHandle* rgw_fh, const char* name, bool null_vid, uint32_t flags)
  {
    int rc = 0;
    BucketStats bs;
    ...
    if (parent->is_root()) {
      ...
      fhr = stat_bucket(parent, name, bs, (rgw_fh) ?
			...
			RGWFileHandle::FLAG_NONE, null_vid);
      bkt_fh = get<0>(fhr);
      if (unlikely(! bkt_fh)) {
	...
	rc = g_rgwlib->get_fe()->execute_req(&req, null_vid);
	 ...
	 * child cache */
	if (rgw_fh->has_children(null_vid)) {
	  rgw_fh->mtx.unlock();
	  unref(rgw_fh);
	  return(-ENOTEMPTY);
	}
  ...
  int RGWLibFS::rename(RGWFileHandle* src_fh, RGWFileHandle* dst_fh,
		       const char *_src_name, const char *_dst_name, bool null_vid)
  ...
  MkObjResult RGWLibFS::symlink(RGWFileHandle* parent, const char *name,
            const char* link_path, struct stat *st, uint32_t mask, uint32_t flags, bool null_vid)
  {
    int rc, rc2;
  ...
  int RGWLibFS::lsxattrs(
    ...
    void *cb_arg, uint32_t flags, bool null_vid)
  {
    /* cannot store on fs_root, should not on buckets? */
    if ((rgw_fh->is_bucket()) ||
	(rgw_fh->is_root()))  {
  ...
  void RGWLibFS::update_fh(RGWFileHandle *rgw_fh, bool null_vid)
  {
    int rc, rc2;
    string obj_name{rgw_fh->relative_object_name()};
    buffer::list ux_key, ux_attrs;
  ...
  bool RGWFileHandle::has_children(bool null_vid) const
  {
    if (unlikely(! is_dir()))
      return false;
...
int rgw_statfs(struct rgw_fs *rgw_fs,
	       ...
	       struct rgw_statvfs *vfs_st, uint32_t flags, bool null_vid)
{
  RGWLibFS *fs = static_cast<RGWLibFS*>(rgw_fs->fs_private);
	       ...
	       uint32_t flags, bool null_vid)
{
  ...
  MkObjResult fhr = fs->create(parent, name, st, mask, flags, null_vid);
        ...
        uint32_t flags, bool null_vid)
{
  ...
  MkObjResult fhr = fs->symlink(parent, name, link_path, st, mask, flags, null_vid);
  RGWFileHandle *nfh = get<0>(fhr); // nullptr if !success
	      ...
	      struct rgw_file_handle **fh, uint32_t flags, bool null_vid)
{
  ...
  MkObjResult fhr = fs->mkdir(parent, name, st, mask, flags, null_vid);
  RGWFileHandle *nfh = get<0>(fhr); // nullptr if !success
	       ...
	       uint32_t flags, bool null_vid)
{
  ...
  return fs->rename(src_fh, dst_fh, src_name, dst_name, null_vid);
}
...
int rgw_unlink(struct rgw_fs *rgw_fs, struct rgw_file_handle *parent_fh,
	       const char *name, uint32_t flags, bool null_vid)
{
  RGWLibFS *fs = static_cast<RGWLibFS*>(rgw_fs->fs_private);
  RGWFileHandle* parent = get_rgwfh(parent_fh);

  return fs->unlink(parent, name, null_vid);
}
	      ...
	      struct stat *st, uint32_t mask, uint32_t flags, bool null_vid)
{
  ...
  if (parent->is_root()) {
    ...
    if (unlikely((strcmp(path, "..") == 0) ||
      ...
      fhr = fs->stat_bucket(parent, path, bstat, RGWFileHandle::FLAG_NONE, null_vid);
      rgw_fh = get<0>(fhr);
      if (! rgw_fh)
	return -ENOENT;
    }
	...
	fhr = fs->stat_leaf(parent, path, null_vid, fh_type, sl_flags);
      }
      if (! get<0>(fhr)) {
		...
		uint32_t mask, uint32_t flags, bool null_vid)
{
  ...
  return fs->setattr(rgw_fh, st, mask, flags, null_vid);
}
		...
		uint32_t flags, bool null_vid)
{
  ...
  int rc = parent->readdir(rcb, cb_arg, offset, eof, flags, null_vid);
  return rc;
} /* rgw_readdir */
		 ...
		 uint32_t flags, bool null_vid)
{
  ...
  int rc = parent->readdir(rcb, cb_arg, name, eof, flags, null_vid);
  return rc;
} /* rgw_readdir2 */
	     ...
	     uint32_t flags, bool null_vid)
{
  ...
  return fs->read(rgw_fh, offset, length, bytes_read, buffer, flags, null_vid);
}
	     ...
	     uint32_t flags, bool null_vid)
{
  ...
  return fs->readlink(rgw_fh, offset, length, bytes_read, buffer, flags, null_vid);
}
...
int rgw_writev(struct rgw_fs *rgw_fs, struct rgw_file_handle *fh,
	      rgw_uio *uio, uint32_t flags, bool null_vid)
{
		  ...
		  uint32_t flags, bool null_vid)
{
  ...
  return fs->getxattrs(rgw_fh, attrs, cb, cb_arg, flags, null_vid);
}
		 ...
		 rgw_getxattr_cb cb, void *cb_arg, uint32_t flags, bool null_vid)
{
  ...
  return fs->lsxattrs(rgw_fh, filter_prefix, cb, cb_arg, flags, null_vid);
}
		  ...
		  rgw_xattrlist *attrs, uint32_t flags, bool null_vid)
{
  ...
  return fs->setxattrs(rgw_fh, attrs, flags, null_vid);
}
		 ...
		 rgw_xattrlist *attrs, uint32_t flags, bool null_vid)
{
  ...
  return fs->rmxattrs(rgw_fh, attrs, flags, null_vid);
}
