...
namespace rgw {
  ...
  int RGWWriteRequest::exec_finish(const DoutPrefixProvider *dpp)
  {
    ...
    op_ret = rgw_get_request_metadata(state->cct, state->info, attrs, dpp);
    if (op_ret < 0) {
      goto done;
    }
		...
		uint32_t flags)
{
  ...
  const char *prefix = "rgw mount: ";
  const DoutPrefix dp(rgwlib.get_store()->ctx(), dout_subsys, prefix);
