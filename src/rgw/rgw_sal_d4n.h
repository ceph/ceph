...
public:
  ...
  virtual std::unique_ptr<ZoneGroup> clone() override {
    ...
    return std::make_unique<D4NFilterZoneGroup>(std::move(nzg));
  }
};
...
class D4NFilterZone : public FilterZone {
  ...
  virtual ~D4NFilterZone() = default;

  virtual std::unique_ptr<Zone> clone() override {
    std::unique_ptr<Zone> nz = next->clone();
    return std::make_unique<D4NFilterZone>(std::move(nz));
  }
...
class D4NFilterUser : public FilterUser {
  ...
  public:
    ...
    virtual std::unique_ptr<User> clone() override {
      return std::make_unique<D4NFilterUser>(*this);
    }
...
class D4NFilterBucket : public FilterBucket {
  ...
  public:
    ...
    virtual std::unique_ptr<Bucket> clone() override {
      ...
      return std::make_unique<D4NFilterBucket>(std::move(nb), user);
    }
...
class D4NFilterObject : public FilterObject {
  ...
  public:
    ...
    struct D4NFilterDeleteOp : DeleteOp {
      std::unique_ptr<DeleteOp> next;
      D4NFilterObject* source;

      D4NFilterDeleteOp(std::unique_ptr<DeleteOp> _next, D4NFilterObject* _source) : next(std::move(_next)),
										     source(_source) {}
    ...
    virtual std::unique_ptr<Object> clone() override {
      return std::make_unique<D4NFilterObject>(*this);
    }
...
public:
  struct D4NFilterLCHead : LCHead {
    std::unique_ptr<LCHead> next;

    D4NFilterLCHead(std::unique_ptr<LCHead> _next) : next(std::move(_next)) {}
    virtual ~D4NFilterLCHead() = default;
  ...
  struct D4NFilterLCEntry : LCEntry {
    std::unique_ptr<LCEntry> next;

    D4NFilterLCEntry(std::unique_ptr<LCEntry> _next) : next(std::move(_next)) {}
    virtual ~D4NFilterLCEntry() = default;
