...
void RGWRESTConn::populate_connect_to(RGWEndpoint& endpoint, ResolvedEndpoint& resolved_endpoint)
{
  ...
  // All IPs are down - pick one anyway (best effort)
  size_t idx = resolved_endpoint.endpoint_ips_round_robin_counter++ % num_ips;
  endpoint.set_connect_to(resolved_endpoint.resolved_ips[idx].connect_to);
  ldout(cct, 5) << "All IPs down for endpoint, using " << resolved_endpoint.resolved_ips[idx].connect_to << dendl;
}
