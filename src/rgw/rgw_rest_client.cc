...
public:
  ...
  int handle_data(bufferlist& bl, off_t bl_ofs, off_t bl_len, const jspan* const parent_span = nullptr) override; /* callback for object iteration when sending data */
};

int RGWRESTStreamOutCB::handle_data(bufferlist& bl, off_t bl_ofs, off_t bl_len, const jspan* const parent_span)
{
...
public:
  ...
  int handle_data(bufferlist& inbl, off_t bl_ofs, off_t bl_len, const jspan* const parent_span = nullptr) override {
