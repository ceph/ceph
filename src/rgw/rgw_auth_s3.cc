...
AWSv4ComplMulti::ChunkMeta::create_next(AWSv4ComplMulti* completer,
					CephContext* const cct,
                                        ChunkMeta&& old,
					...
					uint32_t flags)
{
  ...
  if (expect_chunk_signature) {
    ...
    return std::make_pair(
	     ChunkMeta(completer, data_starts_in_stream,
		       data_length, signature, flags, ++old.cnt),
	     semicolon_pos + 83);
  } else {
    ...
    return std::make_pair(ChunkMeta(completer, data_starts_in_stream,
                                    data_length, "" /* signature */, flags,
                                    ++old.cnt),
                          consumed);
  } /* no-signature */
  ...
  char* const buf, const size_t buf_max, uint32_t cnt, bool& eof)
{
  ...
  if (chunk_meta.is_new_chunk_in_stream(stream_pos)) {
    ...
    std::tie(chunk_meta, consumed) = \
      ChunkMeta::create_next(
	 this, cct(), std::move(chunk_meta),
					   ...
					   const std::string_view last_chunk_sig)
{
  ...
  const auto string_to_sign = string_join_reserve("\n",
    ...
    date,
    credential_scope,
    last_chunk_sig /* chunk_meta.get_signature() */, // XXX calc real last-chunk signature
...
static inline ExtractResult extract_helper(const std::string_view& region, const std::string_view start) {
...
bool AWSv4ComplMulti::complete()
{
  ...
  if (is_signature_mismatched()) {
    ...
    const auto string_to_sign = string_join_reserve("\n",
    "AWS4-HMAC-SHA256-PAYLOAD", /* XXXX oops! this needs to be conditional (and also the trailer) */
    date,
    credential_scope,
    prev_chunk_signature,
    AWS4_EMPTY_PAYLOAD_HASH,
    ...
    while (ttotal < trailer_buf_size) {
      const size_t received =
          io_base_t::recv_body(trailer_vec.data() + tbuf_pos, trailer_buf_size);
    ...
    if (expect_trailer_signature() &&
        ((calculated_trailer_signature != expected_trailer_signature) &&
         (!calculated_trailer_signature.empty()))) {
      throw rgw::io::Exception(ERR_SIGNATURE_NO_MATCH, std::system_category());
    }
