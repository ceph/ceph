...
class RGWPutBucketLoggingOp : public RGWDefaultResponseOp {
  ...
  void execute(optional_yield y) override { 
    ...
    if (!configuration.enabled) {
      ...
      op_ret = retry_raced_bucket_write(this, s->bucket.get(), [this, &src_bucket, &old_conf, y] {
        auto& attrs = src_bucket->get_attrs();
        if (auto it = attrs.find(RGW_ATTR_BUCKET_LOGGING); it != attrs.end()) {
          try {
      ...
      if (old_conf) {
        // conf removed - do cleanup
        if (const auto ret = commit_logging_object(*old_conf, this, driver, s->bucket_tenant, y); ret < 0) {
          ldpp_dout(this, 1) << "WARNING: could not commit pending logging object when removing logging configuration from bucket '" << 
            src_bucket_id << "', ret = " << ret << dendl;
        } else {
          ldpp_dout(this, 20) << "INFO: committed pending logging object when removing logging configuration from bucket '" << 
            src_bucket_id << "'" << dendl;
        }
        ...
        if (const auto ret = rgw::bucketlogging::update_bucket_logging_sources(this, driver, old_target_bucket_id, src_bucket_id, false, y); ret < 0) {
          ldpp_dout(this, 1) << "WARNING: failed to remove source bucket '" << src_bucket_id << "' from logging sources of target bucket '" <<
            old_target_bucket_id << "', ret = " << ret << dendl;
        }
        ldpp_dout(this, 20) << "INFO: removed logging configuration from bucket '" << src_bucket_id << "'" << dendl;
        return;
      }
      ldpp_dout(this, 20) << "INFO: logging configuration of bucket '" << src_bucket_id << "' already removed" << dendl;
      return;
    }
