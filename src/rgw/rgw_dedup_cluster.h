...
namespace rgw::dedup {
  static constexpr const char* DEDUP_POOL_NAME     = "rgw.dedup";
  ...
  public:
    ...
    //---------------------------------------------------------------------------
    bool all_work_shard_tokens_completed(rgw::sal::RadosStore *store,
                                         work_shard_t num_work_shards,
                                         bool force)
    {
      bool success = all_shard_tokens_completed(store,
                                                num_work_shards,
                                                WORKER_SHARD_PREFIX,
                                                &d_num_completed_workers,
                                                d_completed_workers);
      if (!success && force) {
        ...
        for (unsigned shard = 0; shard < num_work_shards; shard++) {
          if ( (d_completed_workers[shard] == TOKEN_STATE_COMPLETED) ||
               (d_completed_workers[shard] == TOKEN_STATE_CORRUPTED) ||
               (d_completed_workers[shard] == TOKEN_STATE_TIMED_OUT) ) {
            // don't wait for timed-out and corrupted tokens
            count++;
          }
        }
        return count == num_work_shards;
      }
      else {
        return success;
      }
    ...
    //---------------------------------------------------------------------------
    bool all_md5_shard_tokens_completed(rgw::sal::RadosStore *store,
                                        md5_shard_t num_md5_shards)
    {
      return all_shard_tokens_completed(store,
                                        num_md5_shards,
                                        MD5_SHARD_PREFIX,
                                        &d_num_completed_md5,
                                        d_completed_md5);
  ...
  private:
    ...
    void clear();
    bool all_shard_tokens_completed(rgw::sal::RadosStore *store,
