...
1. **`get(O:)` → verify non-null and not fenced.**
    - If null, the object was deleted — abort.
    - If fenced (delete marker), the object is deleted from S3's perspective — abort (404). Fenced entries have no `ref_tag`.
...
1. **`get(V:<vid>)` → check result:**
   - If exists and is a delete marker → abort (405 MethodNotAllowed). Delete markers have no `ref_tag`.
   - If exists and is live → use V:<vid> as parent. Read `ref_tag` from value.
   - If null → version may have been promoted to current (DELETE Case 2). Fallback:
2. **Fallback: `get(O:)` → check if `O:.version_id == vid`:**
   - If match and O: is fenced (delete marker) → abort (405 MethodNotAllowed).
   - If match and O: is live → version was promoted to O:. Use O: as parent.
...
get(V:<vid>)
if exists:
  if delete_marker → abort (405 MethodNotAllowed)
  parent = V:<vid>
else:
  get(O:) → if O:.version_id == vid:
    if fenced → abort (405 MethodNotAllowed)
    parent = O:
  else:
    → abort (NoSuchVersion)
```
...
**Policy and quota enforcement:** Child operations do not read `B` inside the transaction and therefore do not re-check policies or quotas at commit time. Authorization and quota checks are performed at the request-handling layer before entering the operation protocol, using TTL-cached bucket metadata (3-second TTL). This matches the refresh model for cheap operations — see [Bucket-State-Management.md — Refresh Strategy](Bucket-State-Management.md#2-refresh-strategy-by-operation-type). A policy or quota change that occurs during an in-flight child operation takes effect on the next request after the cache refreshes.
...
- See [Bucket-State-Management.md — "O:/V: implies bucket exists" Invariant](Bucket-State-Management.md#ov-implies-bucket-exists-invariant).
...
txn {
  ...
  2. get(O:) → if null or fenced → abort
...
txn {
  ...
  2. get(O:) → if null, fenced, or ref_tag mismatch → self-cleanup abort (see below)
...
txn {
  1. get(P:A) → exists
  2. get(O:) → null, fenced, or ref_tag mismatch → invalid state
  3. put(G:A, {anno_ref_tag})  ← queue annotation blob for GC
  4. delete(P:A)
  commit  ← cleanup-abort:
    if null → return HTTP 404 (NoSuchKey)
    if fenced → return HTTP 404 (NoSuchKey, x-amz-delete-marker: true)
    if ref_tag mismatch → return HTTP 409 (Conflict — object was overwritten)
}
```

Live processes self-clean on abort: move P:A → G:A immediately. Sweeper only handles entries from crashed processes. See [Bucket-State-Management.md — Abort and Cleanup](Bucket-State-Management.md#11-abort-and-cleanup) for the general abort/GC model.
...
```
txn {
  1. get(O:) → if null or fenced → abort
     Read annotation_count → if annotation_count >= max → abort (limit exceeded)
     Record parent_ref_tag = O:.ref_tag
...
txn {
  1. Generate anno_ref_tag
  2. Resolve parent:
     get(V:<vid>) → if exists and delete_marker → abort (405)
                   → if exists and live: parent = V:<vid>
     if null: get(O:) → if O:.version_id == vid and fenced → abort (405)
                       → if O:.version_id == vid and live: parent = O:
...
txn {
  ...
  2. Resolve parent (same logic as Phase 1):
     get(V:<vid>) → if exists and delete_marker → self-cleanup abort (405)
                   → if exists and live: parent = V:<vid>
     if null: get(O:) → if O:.version_id == vid and fenced → self-cleanup abort (405)
                       → if O:.version_id == vid and live: parent = O:
...
txn {
  ...
  4. delete(P:A)
  commit  ← cleanup-abort:
    if not found → return HTTP 404 (NoSuchVersion)
    if delete_marker → return HTTP 405 (MethodNotAllowed)
    if ref_tag mismatch → return HTTP 409 (Conflict — version was overwritten)
}
```

Live processes self-clean on abort. Sweeper only handles entries from crashed processes. See [Bucket-State-Management.md — Abort and Cleanup](Bucket-State-Management.md#11-abort-and-cleanup).
...
```
txn {
  1. get(O:) → if null or fenced → abort; read ref_tag, current tag state (inline/external/none)
...
```
txn {
  1. get(O:) → if null or fenced → abort; read ref_tag, current tag state
...
```
txn {
  get(O:) → if null or fenced → 404; read ref_tag
...
```
txn {
  get(O:) → if null or fenced → 404; read parent_ref_tag
...
```
txn {
  get(O:) → if null or fenced → 404; read ref_tag, verify has_extended_value flag
...
|---|---|---|---|
| `P:O` | PUT Phase 1 | Sweeper → G:S → GC, Bucket-delete → G:S → GC | No (Phase 3 never committed → no children exist) |
| `P:A` | PutObjectAnnotation Phase 1 | Sweeper → G:A → GC, Bucket-delete → G:A → GC | No (Phase 3 never committed → no C:A created) |
...
- Moves P:O → G:S (or P:A → G:A) atomically in a single transaction
...
3. **Atomicity of sweeper move:** The sweeper's move is a single atomic transaction — `{get(P:O), put(G:S), delete(P:O)}` for object entries, `{get(P:A), put(G:A), delete(P:A)}` for annotation entries. This prevents the race where GC frees data while Phase 3 can still commit. After commit: the P: entry is gone AND the G: entry exists — Phase 3 sees null, GC can safely proceed.
