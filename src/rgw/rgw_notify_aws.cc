...
namespace rgw::aws {
  ...
  static const int STATUS_FULL = -0x1001;
  static const int STATUS_MANAGER_STOPPED = -0x1002;
  static const int STATUS_CLIENT_CLOSED = -0x1003;
  static const int MISSING_FUCTION_NAME = -0x1004;
  ...
  void invoke_callback(const Aws::Lambda::LambdaClient* client,
    ...
    const auto ctx = static_cast<const AsyncCallerContext*>(context.get());
    CephContext* cct = ctx->cct;
    if (outcome.IsSuccess()) {
      ldout(cct, 20)<< "Invoke AWS Lambda function: '" << request.GetFunctionName() << "' Status Code: '" << outcome.GetResult().GetStatusCode() << 
        "' Function Error: '" << outcome.GetResult().GetFunctionError() << "' Log Result: '" << outcome.GetResult().GetLogResult() << "'" << dendl;
      if(ctx && ctx->cb) {
        ctx->cb(STATUS_OK);
      }
    }
      ...
      ldout(cct, 20) << "AWS Lambda invocation failed. function: '" << request.GetFunctionName() << "' Exception Name: '" << error.GetExceptionName() << 
        "' Message: '" << error.GetMessage() << "' Error Type: '" << static_cast<int>(error.GetErrorType()) << 
        "' Response Code: '" << static_cast<int>(error.GetResponseCode()) << "'" << dendl;
      if(ctx && ctx->cb) {
  ...
  void publish_callback(const Aws::SNS::SNSClient* client,
    ...
    CephContext* cct = ctx->cct;
    if (outcome.IsSuccess()) {
      ldout(cct, 20) << "AWS SNS published to topic: '" << request.GetTopicArn() << "'" << dendl;
      if(ctx && ctx->cb) {
        ctx->cb(STATUS_OK);
      }
    }
      ...
      const auto& error = outcome.GetError();
      ldout(cct, 20) << "AWS SNS publish failed. topic: '" << request.GetTopicArn() << "' Exception Name: '" << error.GetExceptionName() << 
        "' Message: '" << error.GetMessage() << "' Error Type: '" << static_cast<int>(error.GetErrorType()) << 
        "' Response Code: '" << static_cast<int>(error.GetResponseCode()) << "'" << dendl;
      if(ctx && ctx->cb) {
        ctx->cb(-static_cast<int>(error.GetErrorType()));
  ...
  private:
    ...
    void publish_internal(Aws::SNS::SNSClient *client, const std::string &resource_arn, const std::string &payload, reply_callback_t cb) {
      ...
      const auto context = Aws::MakeShared<AsyncCallerContext>("PublishCtxTag", cb, cct);
    ...
    void publish_internal(Aws::Lambda::LambdaClient *client, const std::string &resource_arn, const std::string &payload, reply_callback_t cb) {
      ...
      if(!resourceParsed) {
        ldout(cct, 20) << "AWS Lambda: missing function name" << dendl;
        cb(MISSING_FUCTION_NAME);
      } else {
        invokeRequest.SetFunctionName(Aws::String(resourceParsed.get().resource));
      }
      std::shared_ptr<Aws::IOStream> body = Aws::MakeShared<Aws::StringStream>("InvokeBodyTag");
      *body << payload;
      invokeRequest.SetBody(body);
      invokeRequest.SetContentType("application/json");
      const auto context = Aws::MakeShared<AsyncCallerContext>("InvokeCtxTag", cb, cct);
  ...
  public:
    ...
    AwsClient connect(const std::string &access_key,
      ...
      if (rgw::parse_url_authority(endpoint, host, user, password)) {
        configuration.endpointOverride = Aws::String(host);
      }
      // if not provided use the default AWS endpoint
