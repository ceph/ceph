...
class AWSv4ComplMulti : public rgw::auth::Completer,
  ...
  class ChunkMeta {
    ...
    ChunkMeta(AWSv4ComplMulti* completer,
    ...
    explicit ChunkMeta(AWSv4ComplMulti* completer,
		       const std::string_view& signature, uint32_t _flags,
		       uint32_t _cnt)
      : completer(completer), signature(std::string(signature)), flags(_flags),
	cnt(_cnt)
    {}
  ...
  public:
     ...
     * in a stream. */
    static ChunkMeta create_first(AWSv4ComplMulti* completer,
				  const std::string_view& seed_signature,
				  uint32_t flags,
				  uint32_t cnt) {
      return ChunkMeta(completer, seed_signature, flags, cnt);
    }
    ...
    static std::pair<ChunkMeta, size_t> create_next(AWSv4ComplMulti* completer,
						    CephContext* cct,
                                                    ChunkMeta&& prev,
                                                    const char* metabuf,
                                                    size_t metabuf_len,
  ...
  AWSv4ComplMulti(const req_state* const s,
    ...
    : io_base_t(nullptr),
      ...
      chunk_meta(ChunkMeta::create_first(
		   this, seed_signature, flags, 0 /* first call in cycle */)),
      stream_pos(0),
