...
static void dump_subuser_usage_categories_info(Formatter *formatter,
  ...
  if (subuser_iter != entry.subuser_usage_map.end()) {
    for (const auto category_iter : subuser_iter->second) {
      if (categories && !categories->empty() &&
          !categories->count(category_iter.first))
        continue;
...
int RGWUsage::show(RGWRados *store, rgw_user& uid, const string& subuser,
                   ...
                   map<string, bool> *categories, RGWFormatterFlusher& flusher)
{
  ...
  if (!by_subuser)
    return RGWUsage::show(store, uid, start_epoch, end_epoch,
