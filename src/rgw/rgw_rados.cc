...
static RGWDataCache<RGWCache<RGWRados>> data_cached_rados_provider;
...
int get_obj_data::add_l2_request(struct L2CacheRequest **pc, bufferlist *pbl, std::string oid, std::string  dest, off_t obj_ofs, off_t read_ofs, size_t len, std::string key, librados::AioCompletion *lc)
{
...
int get_obj_data::add_l1_request(struct L1CacheRequest **pc, bufferlist *pbl, string oid, size_t len, off_t ofs, off_t read_ofs, string key, librados::AioCompletion *lc)
{
  ...
  string location = cct->_conf->rgw_data_cache_location + oid;
  ...
  cb->aio_buf = (volatile void *)malloc(len); 
  cb->aio_nbytes = len;
...
int get_obj_data::submit_l1_io_read(bufferlist *bl, size_t len, std::string oid) {
...
void _cache_aio_completion_cb(sigval_t sigval){
  L1CacheRequest *c = (L1CacheRequest *)sigval.sival_ptr;
  c->op_data->cache_aio_completion_cb(c);
}

void get_obj_data::cache_aio_completion_cb(CacheRequest *c){

  ldout(cct, 0) << "cache_aio_completion_cb, oid: "<< c->oid << ",size "<< c->len <<dendl;
  int status = c->status();
  if (status == ECANCEL) {
    ...
    //l2_lock.Lock();
    c->finish();
    rados->get_obj_aio_completion_cb(0, 0);
    //l2_lock.Unlock();
  }
}
...
void get_obj_data::add_io(off_t ofs, off_t len, string oid, bufferlist **pbl, AioCompletion **pc) {
  Mutex::Locker l(lock);
...
void get_obj_data::cancel_all_io() {
  ...
  Mutex::Locker l(lock);
  for (map<off_t, CacheRequest *>::iterator iter = cache_aio_map.begin(); 
       iter != cache_aio_map.end(); ++iter) {
    CacheRequest  *c = iter->second;
...
{
  int use_meta_cache = cct->_conf->rgw_cache_enabled;
  int use_data_cache = cct->_conf->rgw_enable_l1 || cct->_conf->rgw_enable_l2;
  RGWRados *store = NULL;
  if (!use_meta_cache && !use_data_cache) {
    store = new RGWRados;
  } else if (!use_data_cache) {
    store = new RGWCache<RGWRados>; 
  } else {
    store = new RGWDataCache<RGWCache<RGWRados>>;
  }
