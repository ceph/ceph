...
int RGWRados::init_complete()
{
  ...
  if (run_sync_thread) {
    ...
    for (auto iter : svc.zone->get_zone_data_sync_from_map()) {
      ...
      auto *thread = new RGWDataSyncProcessorThread(this, svc.rados->get_async_processor(), iter.first);
      ret = thread->init();
      if (ret < 0) {
...
int RGWRados::cls_bucket_list_unordered(RGWBucketInfo& bucket_info,
  ...
  *is_truncated = false;
  ...
  if (shard_id >= 0) {
    ...
    current_shard =
      svc.bi_rados->bucket_shard_index(my_start.name, num_shards);
  }
  ...
  while (count <= num_entries &&
    ...
    //         the AIO callback
    map<int, struct rgw_cls_list_ret> list_results;
    r = CLSRGWIssueBucketList(ioctx, my_start, prefix, num_entries,
			      list_versions, oids, list_results,
			      cct->_conf->rgw_bucket_index_max_aio)();
    if (r < 0)
      return r;
