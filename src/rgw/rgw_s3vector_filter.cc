...
namespace rgw::s3vector {
  ...
  std::optional<FilterExprs> build_field_expr(
    ...
    if (const auto result = validate_metadata_key_name(field_name); !result) {
      ldpp_dout(dpp, 1) << "ERROR: s3vector filter: field name '" << field_name << "' " << result.error() << dendl;
      errors.push_back({"filter", fmt::format("field name '{}' {}", field_name, result.error())});
      return std::nullopt;
    }
