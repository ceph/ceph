...
namespace rgw::dedup {
  ...
  int Background::watch_reload(const DoutPrefixProvider* dpp)
  {
    ...
    ldpp_dout(dpp, 1) << "dedup_bg::watch_reload(): ioctx="
                      << d_dedup_cluster_ioctx.get_instance_id() << dendl;
    ...
    if (ret >= 0) {
      ldpp_dout(dpp, 1) << "dedup_bg::watch_reload():" << oid
                        << " was created!" << dendl;
    }
    else if (ret == -EEXIST) {
    ...
    ldpp_dout(dpp, 1) << "dedup_bg::watch_reload(): Started watching "
  ...
  int Background::unwatch_reload(const DoutPrefixProvider* dpp)
  {
    ...
    ldpp_dout(dpp, 1) << "dedup_bg::unwatch_reload(): ioctx="
    ...
    ldpp_dout(dpp, 1) << "dedup_bg::unwatch_reload():Stopped watching "
  ...
  {
    ldpp_dout(dpp, 1) <<__func__ << "::dedup_bg shutdown()" << dendl;
  ...
  {
    ldpp_dout(dpp, 1) << "dedup_bg->pause() request: ioctx="
                      << d_dedup_cluster_ioctx.get_instance_id() << dendl;
    std::unique_lock cond_lock(d_cond_mutex);
    ...
    ldpp_dout(dpp, 1) << "dedup_bg paused" << dendl;
  }

  //---------------------------------------------------------------------------
  void Background::resume(rgw::sal::Driver* _driver)
  {
    ldpp_dout(dpp, 1) << "dedup_bg->resume()" << dendl;
    // use lock to prevent concurrent pause/resume requests
    std::unique_lock cond_lock(d_cond_mutex);

    if (!d_ctl.local_paused) {
      cond_lock.unlock();
      ldpp_dout(dpp, 1) << "dedup_bg::resume thread is not paused!" << dendl;
      if (_driver != driver) {
    ...
    d_cond.notify_all();
    ldpp_dout(dpp, 1) << "dedup_bg was resumed" << dendl;
  }
