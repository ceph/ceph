...
namespace rgw::dedup {
  ...
  void control_t::reset()
  {
    ...
    this->bucket_index_throttle.reset(0);
    this->metadata_access_throttle.reset(0);
  }
                                             ...
                                             RGWObjManifest &manifest)
  {
    ...
    for (auto p = manifest.obj_begin(dpp); p != manifest.obj_end(dpp); ++p, ++idx) {
      ...
      ldpp_dout(dpp, 20) << __func__ << "::removing tail object: "
                         << raw_obj.oid << dendl;
    ...
    case URGENT_MSG_THROTTLE:
      for (auto action : throttle_msg.vec) {
        if (action.op_type == BUCKET_INDEX_OP) {
          d_ctl.bucket_index_throttle.reset(action.limit);
        }
        else if (action.op_type == METADATA_ACCESS_OP) {
          d_ctl.metadata_access_throttle.reset(action.limit);
        }
