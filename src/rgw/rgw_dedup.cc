...
namespace rgw::dedup {
                                        ...
                                        disk_block_seq_t    *p_disk)
  {
    ...
    uint32_t size_4k_units = byte_size_to_disk_blocks(p_rec->s.obj_bytes_size);
    uint64_t on_disk_byte_size = disk_blocks_to_byte_size(size_4k_units);
    ...
    if (ret != 0) {
      ...
      p_stats->skipped_singleton_bytes += on_disk_byte_size;
    ...
    if (unlikely(ret < 0)) {
      ldpp_dout(dpp, 1) << __func__ << "::ERR: failed to stat object(" << p_rec->obj_name
    ...
    if (attrs.find(RGW_ATTR_CRYPT_MODE) != attrs.end()) {
      ...
      p_stats->ingress_skip_encrypted_bytes += on_disk_byte_size;
      return 0;
    }
    ...
    if (attrs.find(RGW_ATTR_COMPRESSION) != attrs.end()) {
      ...
      p_stats->ingress_skip_compressed_bytes += on_disk_byte_size;
      return 0;
    }
    ...
    if (itr != attrs.end()) {
      if (unlikely(!parse_etag_string(itr->second.to_str(), &parsed_etag))) {
        p_stats->ingress_failed_bad_etag++;
                                      ...
                                      md5_stats_t         *p_stats /* IN-OUT */)
  {
    ...
    if (ret == 0) {
      ...
      // This counter is using s3 byte size disregarding the on-disk size affected by block-size
      p_stats->deduped_objects_bytes += calc_deduped_bytes(d_head_object_size,
                                                           p_tgt_rec->s.num_parts,
                                                   ...
                                                   worker_stats_t             *p_worker_stats /*IN-OUT*/)
  {
    ...
    if (entry.meta.storage_class.empty()) {
      ...
      p_worker_stats->default_storage_class_objs_bytes += on_disk_byte_size;
    }
    else {
      ...
      p_worker_stats->non_default_storage_class_objs_bytes += on_disk_byte_size;
      return 0;
    }

    if (on_disk_byte_size <= d_min_obj_size_for_dedup) {
      if (parsed_etag.num_parts == 0) {
        // dedup only useful for objects bigger than 4MB
        p_worker_stats->ingress_skip_too_small++;
        p_worker_stats->ingress_skip_too_small_bytes += on_disk_byte_size;

        if (on_disk_byte_size >= 64*1024) {
          p_worker_stats->ingress_skip_too_small_64KB++;
          p_worker_stats->ingress_skip_too_small_64KB_bytes += on_disk_byte_size;
        }
        return 0;
      }
      else {
                                    ...
                                    uint64_t         *p_size)
  {
    ...
    if (is_layout_indexless(index)) {
      ldpp_dout(dpp, 1) << __func__ << "::ERR, indexless buckets do not maintain stats; bucket="
                        << bucket->get_name() << dendl;
      return -EINVAL;
    }
    ...
    if (ret < 0) {
      ldpp_dout(dpp, 1) << __func__ << "::ERR getting bucket stats bucket=" << bucket->get_name()
                        << " ret=" << ret << dendl;
      return ret;
    }
