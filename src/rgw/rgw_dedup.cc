...
namespace rgw::dedup {
  ...
  //---------------------------------------------------------------------------
  static int safe_pool_delete(RGWRados                 *rados,
                              const DoutPrefixProvider *dpp,
                              int64_t                   expected_pool_id)
  {
    ldpp_dout(dpp, 10) <<__func__ << "::" << DEDUP_POOL_NAME << dendl;

    auto rados_handle = rados->get_rados_handle();
    int64_t pool_id = rados_handle->pool_lookup(DEDUP_POOL_NAME);
    ...
    ldpp_dout(dpp, 10) <<__func__ << "::calling delete pool(" << DEDUP_POOL_NAME
                       << ")" << dendl;
    return rados_handle->pool_delete(DEDUP_POOL_NAME);
  }

  //---------------------------------------------------------------------------
  static int64_t create_pool(RGWRados                 *rados,
                             const DoutPrefixProvider *dpp,
                             const std::string        &pool_name)
  {
    ...
    return rados_handle->pool_lookup(DEDUP_POOL_NAME);
  }

  //---------------------------------------------------------------------------
  static int init_dedup_pool_ioctx(RGWRados                 *rados,
                                   const DoutPrefixProvider *dpp,
                                   bool                      create,
                                   librados::IoCtx          &ioctx)
  {
    ...
    if (pool_id >= 0) {
      ...
      pool_id = create_pool(rados, dpp, pool_name);
      if (pool_id >= 0) {
        ldpp_dout(dpp, 10) << __func__ << "::pool " << DEDUP_POOL_NAME
  ...
  int Background::init_rados_access_handles(bool init_pool)
  {
    ...
    if (init_pool) {
      int ret = init_dedup_pool_ioctx(rados, dpp, true, d_dedup_cluster_ioctx);
  ...
  //---------------------------------------------------------------------------
  static bool all_work_shards_completed(cluster *p_cluster,
                                        rgw::sal::RadosStore *store,
                                        work_shard_t num_work_shards,
                                        bool force)
  {
    return p_cluster->all_work_shard_tokens_completed(store, num_work_shards, force);
  }
  ...
  void Background::work_shards_barrier(work_shard_t num_work_shards)
  {
    ...
    // We can move to the next step even if some token are inb failed state

    // Don't wait forever for failed members !!
    const unsigned MAX_WAIT_SEC = 120; // wait 2 minutes for timed-out members
    unsigned ttl = 3;
    unsigned time_elapsed = 0;
    bool force = false;
    while (!all_work_shards_completed(&d_cluster, store, num_work_shards, force)) {
      ...
      time_elapsed += ttl;
      if (time_elapsed > MAX_WAIT_SEC) {
        ldpp_dout(dpp, 10) << __func__ << "::force mode: skip timed-out members"
                           << dendl;
        force = true;
      }
    }
  ...
  static bool all_md5_shards_completed(cluster *p_cluster,
                                       ...
                                       md5_shard_t num_md5_shards)
  {
    return p_cluster->all_md5_shard_tokens_completed(store, num_md5_shards);
  }
      ...
      if (d_ctl.dedup_exec) {
        ...
        if (!d_ctl.should_stop()) {
          ...
          if (!d_ctl.should_stop()) {
            ...
            safe_pool_delete(rados, dpp, pool_id);
          }
          else {
