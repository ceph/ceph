...
std::string incremental_string(const DoutPrefixProvider *dpp, std::optional<std::string> old_name) {
  ...
  try {
    ...
    ++counter;
    static const auto fmt = std::format("{{:0>{}}}", N);
    return std::vformat(fmt, std::make_format_args(counter));
...
int new_logging_object(const configuration& conf,
  ...
  auto ret = target_bucket->set_logging_object_name(obj_name, conf.target_prefix, y, dpp, (old_name == std::nullopt), objv_tracker);
...
int rollover_logging_object(const configuration& conf,
  ...
  const auto ret = new_logging_object(conf, target_bucket, obj_name, dpp, y, old_obj, objv_tracker);
  if (ret == -ECANCELED) {
