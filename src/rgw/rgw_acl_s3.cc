                                       ...
                                       std::string &err_msg)
{
  ...
  if (user_ctl->get_info_by_uid(dpp, owner->get_id(), &owner_info, null_yield) < 0) {
    ldpp_dout(this, 10) << "owner info does not exist" << dendl;
    err_msg = "Invalid id";
    return -EINVAL;
  }
  ...
  ldpp_dout(this, 20) << "owner id=" << owner->get_id() << dendl;
  ldpp_dout(this, 20) << "dest owner id=" << dest.get_owner().get_id() << dendl;
    ...
    case ACL_TYPE_EMAIL_USER:
      {
        ...
        if (!src_grant.get_id(u)) {
          ldpp_dout(this, 0) << "ERROR: src_grant.get_id() failed" << dendl;
          return -EINVAL;
        }
        email = u.id;
        ldpp_dout(this, 10) << "grant user email=" << email << dendl;
        if (user_ctl->get_info_by_email(dpp, email, &grant_user, null_yield) < 0) {
          ldpp_dout(this, 10) << "grant user email not found or other error" << dendl;
    ...
    case ACL_TYPE_CANON_USER:
      {
        ...
        if (grant_user.user_id.empty() && user_ctl->get_info_by_uid(dpp, uid, &grant_user, null_yield) < 0) {
          ...
          ldpp_dout(this, 10) << "new grant: " << new_id << ":" << grant_user.display_name << dendl;
        }
      }
      break;
    case ACL_TYPE_GROUP:
      {
        ...
        if (ACLGrant_S3::group_to_uri(src_grant.get_group(), uri)) {
          new_grant = src_grant;
          grant_ok = true;
          ldpp_dout(this, 10) << "new grant: " << uri << dendl;
        } else {
          ldpp_dout(this, 10) << "bad grant group:" << (int)src_grant.get_group() << dendl;
