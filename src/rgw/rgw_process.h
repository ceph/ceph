...
protected:
  ...
  struct RGWWQ : public DoutPrefixProvider, public ThreadPool::WorkQueue<RGWRequest> {
    ...
    void _process(RGWRequest *req, ThreadPool::TPHandle &, bool null_verid) override;
...
public:
  ...
  virtual void run() = 0;
  virtual void handle_request(const DoutPrefixProvider *dpp, RGWRequest *req, bool null_verid) = 0;
...
public:
  ...
  void run() override;
  void checkpoint();
  void handle_request(const DoutPrefixProvider *dpp, RGWRequest* req, bool null_verid) override;
