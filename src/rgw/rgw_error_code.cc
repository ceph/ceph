...
const char* rgw_error_category::message(int ev, char*,
  ...
  case rgw_errc::created:
    return "Created";
  case rgw_errc::accepted:
    return "Accepted";
  case rgw_errc::no_content:
    return "No Content";
  case rgw_errc::partial_content:
    return "Partial Content";
  case rgw_errc::redirect:
    return "Redirect";
  case rgw_errc::no_apply:
    return "No Apply";
  case rgw_errc::applied:
    return "Applied";
  }
...
bool rgw_error_category::equivalent(int ev, const bs::error_condition& c) const noexcept {
  ...
  case rgw_errc::requirement_exceeds_limit:
    if (c == ceph::errc::limit_exceeded ||
	c == bs::errc::resource_deadlock_would_occur)
      return true;

  case rgw_errc::placement_pool_missing: [[fallthrough]];
  case rgw_errc::no_bucket_for_bucket_operation:
  case rgw_errc::no_apply:
    if (c == bs::errc::io_error)
      return true;
...
bool rgw_error_category::failed(int ev) const noexcept {
  switch (static_cast<rgw_errc>(ev)) {
  case rgw_errc::created:
  case rgw_errc::accepted:
  case rgw_errc::no_content:
