...
class LockClient {
 public:
  ...
  virtual boost::asio::awaitable<void> acquire(ceph::timespan duration) = 0;
  /// Renew an acquired lock for the given duration, or throw on error.
  virtual boost::asio::awaitable<void> renew(ceph::timespan duration) = 0;
  /// Release an acquired lock, or throw on error.
  virtual boost::asio::awaitable<void> release() = 0;
};
