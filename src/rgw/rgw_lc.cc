...
static bool should_list_unordered(const rgw::bucket_index_layout_generation& current_index, uint64_t threshold) {
  if (threshold == 0 || current_index.layout.type != rgw::BucketIndexType::Normal) {
    return true;
  } else {
    uint32_t shard_count = rgw::num_shards(current_index.layout.normal);
    return (shard_count >= threshold);
  }
}
class LCObjsLister {
