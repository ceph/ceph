...
int RGWAsyncGetBucketInstanceInfo::_send_request()
{
  ...
  if (r < 0) {
    ldpp_dout(this, 0) << "ERROR: failed to get bucket instance info for "
        << bucket << dendl;
    return r;
  }
...
int RGWAsyncRemoveObj::_send_request()
{
  ...
  if (ret < 0) {
    ldpp_dout(this, 20) << __func__ << "(): get_obj_state() obj=" << obj << " returned ret=" << ret << dendl;
    return ret;
  }

  /* has there been any racing object write? */
  if (del_if_older && (state->mtime > timestamp)) {
    ldpp_dout(this, 20) << __func__ << "(): skipping object removal obj=" << obj << " (obj mtime=" << state->mtime << ", request timestamp=" << timestamp << ")" << dendl;
    return 0;
  }
  ...
  if (ret < 0) {
    ldpp_dout(this, 20) << __func__ << "(): delete_obj() obj=" << obj << " returned ret=" << ret << dendl;
  }
  return ret;
}
