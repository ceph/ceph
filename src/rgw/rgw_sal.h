 ...
 *
 * RGW can load and process Lua scripts.  This will handle loading/storing
 * scripts; adding, deleting, listing, and installing packages.
 */
class LuaManager {
public:
  ...
  /** Add a lua package */
  virtual int add_package(const DoutPrefixProvider* dpp, optional_yield y, const std::string& package_name, bool allow_compilation) = 0;
  ...
  virtual int install_packages(const DoutPrefixProvider* dpp, optional_yield y, rgw::lua::packages_t& failed_packages, std::string& output) = 0;
};
