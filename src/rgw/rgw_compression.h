...
public:
                 ...
                 optional_yield y) override
  {
    ...
    std::vector<uint32_t> decrypt_part_nums;
    if (decrypt_crypt) {
      ...
      /*
       * AEAD multipart objects also store the actual S3 part numbers
       * (e.g. {1,3,5}) for per-part IV derivation. Empty → single-part.
       */
      if (aead_src) {
    ...
    if (decrypt_crypt) {
      if (aead_src) {
        decrypt_filter = std::make_unique<RGWGetObj_BlockDecrypt>(
            ...
            std::move(decrypt_parts), std::move(decrypt_part_nums),
