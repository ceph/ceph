...
int rgw_store_user_info(RGWUserInfo& info, bool exclusive)
{
  ...
  for (iter = info.swift_keys.begin(); iter != info.swift_keys.end(); ++iter) {
    ...
    if (r >= 0 && inf.user_id.compare(info.user_id) != 0) {
      ldout(rgwstore->ctx(), 0) << "WARNING: can't store user info, swift id already mapped to another user" << dendl;
      return -EEXIST;
    }
  }
  ...
  if (info.access_keys.size()) {
    ...
    for (; iter != info.access_keys.end(); ++iter) {
      ...
      if (r >= 0 && inf.user_id.compare(info.user_id) != 0) {
        ldout(rgwstore->ctx(), 0) << "WARNING: can't store user info, access key already mapped to another user" << dendl;
        return -EEXIST;
      }
    }
  }
...
int rgw_get_user_info_from_index(string& key, rgw_bucket& bucket, RGWUserInfo& info)
{
  ...
  try {
    ...
    ldout(rgwstore->ctx(), 0) << "ERROR: failed to decode user info, caught buffer::error" << dendl;
    return -EIO;
  }
...
static int rgw_read_buckets_from_attr(string& user_id, RGWUserBuckets& buckets)
{
  ...
  try {
    ...
    ldout(rgwstore->ctx(), 0) << "ERROR: failed to decode buckets info, caught buffer::error" << dendl;
    return -EIO;
  }
  return 0;
}
...
int rgw_read_user_buckets(string user_id, RGWUserBuckets& buckets, bool need_stats)
{
  ...
  if (need_stats) {
   ...
   if (r < 0)
     ldout(rgwstore->ctx(), 0) << "ERROR: could not get stats for buckets" << dendl;
...
int rgw_add_bucket(string user_id, rgw_bucket& bucket)
{
  ...
  if (rgwstore->supports_omap()) {
    ...
    if (ret < 0) {
      ldout(rgwstore->ctx(), 0) << "ERROR: error adding bucket to directory: "
    ...
    case -ENODATA:
      ...
      break;
    default:
      ldout(rgwstore->ctx(), 10) << "rgw_write_buckets_attr returned " << ret << dendl;
      break;
    }
  }
...
int rgw_remove_user_bucket_info(string user_id, rgw_bucket& bucket)
{
  ...
  if (rgwstore->supports_omap()) {
    ...
    if (ret < 0) {
      ldout(rgwstore->ctx(), 0) << "ERROR: error removing bucket from directory: "
		 << cpp_strerror(-ret)<< dendl;
    }
  } else {
    RGWUserBuckets buckets;
...
int rgw_delete_user(RGWUserInfo& info) {
  ...
  for (; kiter != info.access_keys.end(); ++kiter) {
    ldout(rgwstore->ctx(), 10) << "removing key index: " << kiter->first << dendl;
    ret = rgw_remove_key_index(kiter->second);
    if (ret < 0 && ret != -ENOENT) {
      ldout(rgwstore->ctx(), 0) << "ERROR: could not remove " << kiter->first << " (access key object), should be fixed (err=" << ret << ")" << dendl;
      return ret;
    }
  }
  ...
  for (; siter != info.swift_keys.end(); ++siter) {
    RGWAccessKey& k = siter->second;
    ldout(rgwstore->ctx(), 10) << "removing swift subuser index: " << k.id << dendl;
    /* check if swift mapping exists */
    ret = rgw_remove_swift_name_index(k.id);
    if (ret < 0 && ret != -ENOENT) {
      ldout(rgwstore->ctx(), 0) << "ERROR: could not remove " << k.id << " (swift name object), should be fixed (err=" << ret << ")" << dendl;
      return ret;
    }
  }

  rgw_obj email_obj(ui_email_bucket, info.user_email);
  ldout(rgwstore->ctx(), 10) << "removing email index: " << info.user_email << dendl;
  ret = rgwstore->delete_obj(NULL, email_obj);
  if (ret < 0 && ret != -ENOENT) {
    ldout(rgwstore->ctx(), 0) << "ERROR: could not remove " << info.user_id << ":" << email_obj << ", should be fixed (err=" << ret << ")" << dendl;
    return ret;
  }
  ...
  ldout(rgwstore->ctx(), 10) << "removing user buckets index" << dendl;
  ret = rgwstore->delete_obj(NULL, uid_bucks);
  if (ret < 0 && ret != -ENOENT) {
    ldout(rgwstore->ctx(), 0) << "ERROR: could not remove " << info.user_id << ":" << uid_bucks << ", should be fixed (err=" << ret << ")" << dendl;
    return ret;
  }
  
  rgw_obj uid_obj(ui_uid_bucket, info.user_id);
  ldout(rgwstore->ctx(), 10) << "removing user index: " << info.user_id << dendl;
  ret = rgwstore->delete_obj(NULL, uid_obj);
  if (ret < 0 && ret != -ENOENT) {
    ldout(rgwstore->ctx(), 0) << "ERROR: could not remove " << info.user_id << ":" << uid_obj << ", should be fixed (err=" << ret << ")" << dendl;
    return ret;
  }
