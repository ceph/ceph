...
*/
  ...
  int create_bucket_key(const std::string& key_name)
  {
/*
	.data.ciphertext <- (to-be) named attribute
...
*/
  ...
  int delete_bucket_key(const std::string& key_name)
  {
/*
	/keys/<keyname>/config
                            ...
                            std::string& actual_key)
{
  ...
  ldout(cct, 0) << "ERROR: Invalid rgw_crypt_sse_s3_backend: " << kms_backend << dendl;
  return -EINVAL;
}
                            ...
                            std::string& actual_key)
{
  ...
  std::string kms_backend { kctx.backend() };
  if (RGW_SSE_KMS_BACKEND_VAULT != kms_backend) {
    ldout(cct, 0) << "ERROR: Unsupported rgw_crypt_sse_s3_backend: " << kms_backend << dendl;
    return -EINVAL;
  }
  return make_actual_key_from_vault(cct, kctx, attrs, actual_key);
}
...
int create_ss3_s3_bucket_key(CephContext *cct,
                                     const std::string& bucket_key)
{
  ...
  if (RGW_SSE_KMS_VAULT_SE_TRANSIT == secret_engine){
    TransitSecretEngine engine(cct, kctx, std::move(secret_engine_parms));
	return engine.create_bucket_key(bucket_key);
  }
  else {
    ldout(cct, 0) << "Missing or invalid secret engine" << dendl;
    return -EINVAL;
...
int remove_ss3_s3_bucket_key(CephContext *cct,
                                     const std::string& bucket_key)
{
  SseS3Context kctx { cct };
  std::string secret_engine_str = kctx.secret_engine();
  ...
  if (RGW_SSE_KMS_VAULT_SE_TRANSIT == secret_engine){
    TransitSecretEngine engine(cct, kctx, std::move(secret_engine_parms));
	return engine.delete_bucket_key(bucket_key);
  }
  else {
    ldout(cct, 0) << "Missing or invalid secret engine" << dendl;
    return -EINVAL;
