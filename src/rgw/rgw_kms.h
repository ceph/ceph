...
inline int get_actual_key_from_kms(CephContext *cct,
                            boost::string_view key_id,
                            boost::string_view key_selector,
                            std::string& actual_key)
{
        return get_actual_key_from_kms(cct,
                std::string_view(key_id.data(), key_id.size()),
                std::string_view(key_selector.data(), key_selector.size()),
                actual_key);
}
inline int get_actual_key_from_kms(CephContext *cct,
                            std::string& key_id,
