  ...
  bool process_entry(const cls_queue_entry& entry, spawn::yield_context yield) {
    ...
    try {
      ...
      const auto push_endpoint = RGWPubSubEndpoint::create(event_entry.push_endpoint, event_entry.arn_topic,
          RGWHTTPArgs(event_entry.push_endpoint_args, dpp), 
          cct);
      ldpp_dout(dpp, 20) << "INFO: push endpoint created: " << event_entry.push_endpoint <<
      ...
      if (ret < 0) {
        ldpp_dout(dpp, 5) << "WARNING: push entry: " << entry.marker << " to endpoint: " << event_entry.push_endpoint 
          << " failed. error: " << ret << " (will retry)" << dendl;
        return false;
      } else {
        ldpp_dout(dpp, 20) << "INFO: push entry: " << entry.marker << " to endpoint: " << event_entry.push_endpoint 
          << " ok" <<  dendl;
