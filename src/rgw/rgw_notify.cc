...
class Manager {
  ...
  bool process_entry(const cls_queue_entry& entry, spawn::yield_context yield) {
    ...
    try {
      ...
      ldpp_dout(dpp, 5) << "WARNING: failed to decode entry. error: " << err.what() << dendl;
      return false;
    }
    try {
      ...
      const auto push_endpoint = RGWPubSubEndpoint::create(record_with_endpoint.push_endpoint, record_with_endpoint.arn_topic,
          RGWHTTPArgs(record_with_endpoint.push_endpoint_args, dpp), 
          cct);
      ldpp_dout(dpp, 20) << "INFO: push endpoint created: " << record_with_endpoint.push_endpoint <<
      ...
      if (ret < 0) {
        ldpp_dout(dpp, 5) << "WARNING: push entry: " << entry.marker << " to endpoint: " << record_with_endpoint.push_endpoint 
          << " failed. error: " << ret << " (will retry)" << dendl;
        return false;
      } else {
        ldpp_dout(dpp, 20) << "INFO: push entry: " << entry.marker << " to endpoint: " << record_with_endpoint.push_endpoint 
          << " ok" <<  dendl;
        if (perfcounter) perfcounter->inc(l_rgw_pubsub_push_ok);
        return true;
      }
    } catch (const RGWPubSubEndpoint::configuration_error& e) {
      ldpp_dout(dpp, 5) << "WARNING: failed to create push endpoint: " 
  ...
  void cleanup_queue(const std::string& queue_name, spawn::yield_context yield) {
    while (true) {
      ldpp_dout(dpp, 20) << "INFO: trying to perform stale reservation cleanup for queue: " << queue_name << dendl;
      ...
      if (ret == -ENOENT) {
        // queue was deleted
        ldpp_dout(dpp, 5) << "INFO: queue: " 
          << queue_name << ". was removed. cleanup will stop" << dendl;
        return;
      }
      if (ret == -EBUSY) {
        ldpp_dout(dpp, 5) << "WARNING: queue: " << queue_name << " ownership moved to another daemon. processing will stop" << dendl;
        return;
      }
      if (ret < 0) {
        ldpp_dout(dpp, 5) << "WARNING: failed to cleanup stale reservation from queue and/or lock queue: " << queue_name
          << ". error: " << ret << dendl;
      }
  ...
  void process_queue(const std::string& queue_name, spawn::yield_context yield) {
    ...
    while (true) {
      ...
      auto total_entries = 0U;
      {
        ...
        if (ret < 0) {
          ldpp_dout(dpp, 5) << "WARNING: failed to get list of entries in queue and/or lock queue: " 
            << queue_name << ". error: " << ret << " (will retry)" << dendl;
          continue;
        }
        ret = cls_2pc_queue_list_entries_result(obl, entries, &truncated, end_marker);
        if (ret < 0) {
          ldpp_dout(dpp, 5) << "WARNING: failed to parse list of entries in queue: " 
            << queue_name << ". error: " << ret << " (will retry)" << dendl;
          continue;
        }
      }
      ...
      ldpp_dout(dpp, 20) << "INFO: found: " << total_entries << " entries in: " << queue_name <<
      ...
      for (auto& entry : entries) {
        ...
        spawn::spawn(yield, [this, &queue_name, entry_idx, total_entries, &end_marker, &remove_entries, &has_error, &waiter, entry](spawn::yield_context yield) {
            ...
            if (process_entry(entry, yield)) {
              ldpp_dout(dpp, 20) << "INFO: processing of entry: " << 
                entry.marker << " (" << entry_idx << "/" << total_entries << ") from: " << queue_name << " ok" << dendl;
              remove_entries = true;
            }  else {
              if (set_min_marker(end_marker, entry.marker) < 0) {
                ldpp_dout(dpp, 1) << "ERROR: cannot determin minimum between malformed markers: " << end_marker << ", " << entry.marker << dendl;
              } else {
                ldpp_dout(dpp, 20) << "INFO: new end marker for removal: " << end_marker << " from: " << queue_name << dendl;
              }
              has_error = true;
              ldpp_dout(dpp, 20) << "INFO: processing of entry: " << 
      ...
      if (remove_entries) {
        ...
        if (ret < 0) {
          ldpp_dout(dpp, 1) << "ERROR: failed to remove entries and/or lock queue up to: " << end_marker <<  " from queue: " 
            << queue_name << ". error: " << ret << dendl;
        } else {
          ldpp_dout(dpp, 20) << "INFO: removed entries up to: " << end_marker <<  " from queue: " 
          << queue_name << dendl;
        }
      }
  ...
  void process_queues(spawn::yield_context yield) {
    ...
    while (true) {
      ...
      ldpp_dout(dpp, 20) << "INFO: next queues processing will happen at: " << std::ctime(&tp)  << dendl;
      boost::system::error_code ec;
      timer.async_wait(yield[ec]);
      ...
      for (const auto& queue_name : queues) {
        ...
        if (ret == -EBUSY) {
          ...
          ldpp_dout(dpp, 20) << "INFO: queue: " << queue_name << " owned (locked) by another daemon" << dendl;
        ...
        if (ret == -ENOENT) {
          ...
          ldpp_dout(dpp, 10) << "INFO: queue: " << queue_name << " should not be locked - already deleted" << dendl;
          continue;
        }
        if (ret < 0) {
          // failed to lock for another reason, continue to process other queues
          ldpp_dout(dpp, 1) << "ERROR: failed to lock queue: " << queue_name << ". error: " << ret << dendl;
          has_error = true;
          continue;
        }
        // add queue to list of owned queues
        if (owned_queues.insert(queue_name).second) {
          ldpp_dout(dpp, 10) << "INFO: queue: " << queue_name << " now owned (locked) by this daemon" << dendl;
          ...
          spawn::spawn(io_context, [this, &queue_gc, &queue_gc_lock, queue_name](spawn::yield_context yield) {
            ...
            ldpp_dout(dpp, 10) << "INFO: queue: " << queue_name << " marked for removal" << dendl;
          });
        } else {
          ldpp_dout(dpp, 20) << "INFO: queue: " << queue_name << " ownership (lock) renewed" << dendl;
        }
      }
      // erase all queue that were deleted
      {
        ...
        std::for_each(queue_gc.begin(), queue_gc.end(), [this, &owned_queues](const std::string& queue_name) {
          ...
          ldpp_dout(dpp, 20) << "INFO: queue: " << queue_name << " removed" << dendl;
        });
        queue_gc.clear();
      }
    }
...
public:
  ...
  Manager(CephContext* _cct, uint32_t _max_queue_size, uint32_t _queues_update_period_ms, 
          ...
          uint32_t _worker_count, rgw::sal::RGWRadosStore* store, const DoutPrefixProvider *_dpp) : 
    ...
    dpp(_dpp)
    {
      ...
      ldpp_dout(dpp, 10) << "Started notification manager with: " << worker_count << " workers" << dendl;
    }
  ...
  int add_persistent_topic(const std::string& topic_name, optional_yield y) {
    ...
    if (ret == -EEXIST) {
      ...
      ldpp_dout(dpp, 20) << "INFO: queue for topic: " << topic_name << " already exists. nothing to do" << dendl;
      return 0;
    }
    if (ret < 0) {
      // failed to create queue
      ldpp_dout(dpp, 1) << "ERROR: failed to create queue for topic: " << topic_name << ". error: " << ret << dendl;
      return ret;
    }
    ...
    if (ret < 0) {
      ldpp_dout(dpp, 1) << "ERROR: failed to add queue: " << topic_name << " to queue list. error: " << ret << dendl;
      return ret;
    } 
    ldpp_dout(dpp, 20) << "INFO: queue: " << topic_name << " added to queue list"  << dendl;
    return 0;
  }
