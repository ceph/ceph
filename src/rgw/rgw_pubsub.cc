...
int RGWPubSub::SubWithEvents<EventType>::list_events(const DoutPrefixProvider *dpp, const string& marker, int max_events)
{
  ...
  if (ret < 0) {
    ldpp_dout(this, 1) << "ERROR: failed to read sub config: ret=" << ret << dendl;
    return ret;
  }
  ...
  if (ret < 0) {
    ldpp_dout(this, 1) << "ERROR: failed to read bucket info for events bucket: bucket=" << sub_conf.dest.bucket_name << " ret=" << ret << dendl;
    return ret;
  }
  ...
  if (ret < 0) {
    ldpp_dout(this, 1) << "ERROR: failed to list bucket: bucket=" << sub_conf.dest.bucket_name << " ret=" << ret << dendl;
    return ret;
  }
  ...
  for (auto& obj : objs) {
    ...
    try {
      ...
      ldpp_dout(this, 1) << "ERROR: failed to event (not a valid base64)" << dendl;
      continue;
    }
    EventType event;
    ...
    try {
      ...
      ldpp_dout(this, 1) << "ERROR: failed to decode event" << dendl;
      continue;
    };
...
int RGWPubSub::SubWithEvents<EventType>::remove_event(const DoutPrefixProvider *dpp, const string& event_id)
{
  ...
  if (ret < 0) {
    ldpp_dout(this, 1) << "ERROR: failed to remove event (obj=" << obj << "): ret=" << ret << dendl;
  }
  return 0;
}
