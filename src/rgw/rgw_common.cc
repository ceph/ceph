                              ...
                              const uint64_t op, bool* granted_by_acl)
{
  ...
  if (restrict_public_buckets && bucket_policy && rgw::IAM::is_public(dpp, *bucket_policy) && !s->identity->is_owner_of(s->bucket_info.owner)) {
                              ...
                              const uint64_t op, bool* granted_by_acl)
{
  ...
  if (restrict_public_buckets && bucket_policy && rgw::IAM::is_public(dpp, *bucket_policy) && !ps->identity->is_owner_of(ps->bucket_info.owner)) {
    ldpp_dout(dpp, 10) << __func__ << ": public policies are blocked by the RestrictPublicBuckets block public access setting" << dendl;
    return false;
  }
