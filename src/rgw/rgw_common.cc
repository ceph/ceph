...
void req_info::init_meta_info(req_state* s)
{
  x_meta_map.clear();
  ...
  for (const auto& kv: env->get_map()) {
    ...
    for (int prefix_num = 0; (prefix = meta_prefixes[prefix_num].str) != NULL; prefix_num++) {
      ...
      if (strncmp(p, prefix, len) == 0) {
        ldpp_dout(s, 10) << "meta>> " << p << dendl;
        const char *name = p+len; /* skip the prefix */
        int name_len = header_name.size() - len;

        if (s->has_bad_meta && strncmp(name, "_META_", name_len) == 0)
          s->has_bad_meta = true;
  ...
  for (const auto& kv: x_meta_map) {
    ldpp_dout(s, 10) << "x>> " << kv.first << ":" << rgw::crypt_sanitize::x_meta_map{kv.first, kv.second} << dendl;
  }
}
