...
using ShardNamesView = std::ranges::transform_view<std::ranges::iota_view<uint32_t, uint32_t>, std::function<std::string(uint32_t)>>; 
...
struct rgw_pubsub_dest {
  ...
  uint32_t num_shards; //defaults to a single shard for now, for backward compatibility
