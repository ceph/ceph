...
struct HTTPMetaTable : public EmptyMetaTable {
  using http_args_map = std::remove_cvref_t<
    decltype(std::declval<RGWHTTPArgs&>().get_params())>;
...
struct RequestMetaTable : public EmptyMetaTable {
  static int IndexClosure(lua_State* L) {
    ...
    const auto multi_delete_obj = reinterpret_cast<rgw::sal::Object*>(
      lua_touserdata(L, lua_upvalueindex(FOURTH_UPVAL)));
    request_lua_context ctx {*s, op_name, multi_delete_obj};
    ...
    static constexpr std::array fields {
      ...
      lua_callback("Object", [](lua_field_context field, request_lua_context& ctx) {
        create_metatable<ObjectMetaTable>(
          field.L, field.table_name, field.field_name, false,
          ctx.multi_delete_obj ? ctx.multi_delete_obj : ctx.s.object.get());
        return ONE_RETURNVAL;
      }),
...
void create_top_metatable(lua_State* L, req_state* s, const char* op_name,
  ...
  create_metatable<RequestMetaTable>(L, "", request_table_name, true, s,
    const_cast<char*>(op_name), const_cast<rgw::sal::Object*>(multi_delete_obj));
    ...
    const rgw::sal::Object* multi_delete_obj)
{
  ...
  return execute(
    rest, olog, s, op, code, dummy_script_return_code, multi_delete_obj);
}
