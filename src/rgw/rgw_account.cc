           ...
           optional_yield y)
{
  ...
  do {
    ...
    for (const auto& info : users.users) {
      ...
      ldpp_dout(dpp, 1) << "Deleted account user " << info.user_id.to_str() << dendl;
    }
  ...
  do {
    ...
    for (const auto& ent : buckets.buckets) {
      ...
      ldpp_dout(dpp, 1) << "Deleted account bucket " << ent.bucket.name << dendl;
    }
  ...
  do {
    ...
    for (const auto& info : roles.roles) {
      ...
      ldpp_dout(dpp, 1) << "Deleted account role " << info.arn << dendl;
    }
  ...
  do {
    ...
    for (const auto& info : groups.groups) {
      ...
      ldpp_dout(dpp, 1) << "Deleted account group " << info.id << dendl;
    }
  ...
  for (const auto& info : providers) {
    ...
    ldpp_dout(dpp, 1) << "Deleted account oidc provider " << info.provider_url << dendl;
  }
  ...
  do {
    ...
    for (const auto& topic_name : topics.topics) {
      ...
      ldpp_dout(dpp, 1) << "Deleted account topic " << topic_name << dendl;
    }
