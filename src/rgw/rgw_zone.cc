...
int RGWPeriod::update()
{
  ...
  int ret = ceph::from_error_code(zone_svc->list_zonegroups(zonegroups, null_yield));
  if (ret < 0) {
...
static int read_sync_status(rgw::sal::RGWRadosStore *store, rgw_meta_sync_status *sync_status)
{
  ...
  RGWMetaSyncStatusManager mgr(store, store->svc()->get_async_processor());
  int r = mgr.init();
  if (r < 0) {
    return r;
  }
...
int RGWZoneParams::fix_pool_names()
{
  ...
  int r = ceph::from_error_code(zone_svc->list_zones(zones, null_yield));
  if (r < 0) {
