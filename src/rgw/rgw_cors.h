...
static inline uint8_t get_multi_cors_method_flags(const char *req_meth) {
  uint8_t flags = 0;
  std::string allowed_methods(req_meth);

  if (allowed_methods.contains("GET") == 0) flags |= RGW_CORS_GET;
  else if (allowed_methods.contains("POST") == 0) flags |= RGW_CORS_POST;
  else if (allowed_methods.contains("PUT") == 0) flags |= RGW_CORS_PUT;
  else if (allowed_methods.contains("DELETE") == 0) flags |= RGW_CORS_DELETE;
  else if (allowed_methods.contains("COPY") == 0) flags |= RGW_CORS_COPY;
