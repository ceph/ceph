                              ...
                              const uint32_t perm)
{
  ...
  if (user_ctl->get_info_by_uid(dpp, user, &grant_user, null_yield) < 0) {
    ldpp_dout(this, 10) << "grant user does not exist: " << uid << dendl;
    /* skipping silently */
                                             ...
                                             const uint32_t perm)
{
  for (const auto& uid : uids) {
    ...
    ldpp_dout(this, 20) << "trying to add grant for ACL uid=" << uid << dendl;
                                         ...
                                         uint32_t& rw_mask)
{
  ...
  if (read_list) {
    ...
    if (r < 0) {
      ldpp_dout(this, 0) << "ERROR: parse_list for read returned r="
                    << r << dendl;
      return r;
    }
    ...
    if (r < 0) {
      ldpp_dout(this, 0) << "ERROR: add_grants for read returned r="
                    << r << dendl;
      return r;
    }
  ...
  if (write_list) {
    ...
    if (r < 0) {
      ldpp_dout(r, 0) << "ERROR: add_grants for write returned r="
                    << r << dendl;
      return r;
    }
                                              ...
                                              const std::string& acl_str)
{
  ...
  if (!parser.parse(acl_str.c_str(), acl_str.length())) {
    ldpp_dout(this, 0) << "ERROR: JSONParser::parse returned error=" << dendl;
    return false;
  }
  ...
  if (!iter.end() && (*iter)->is_array()) {
    ...
    ldpp_dout(this, 0) << "admins: " << admin << dendl;
  ...
  if (!iter.end() && (*iter)->is_array()) {
    ...
    ldpp_dout(this, 0) << "read-write: " << readwrite << dendl;
  ...
  if (!iter.end() && (*iter)->is_array()) {
    ...
    ldpp_dout(this, 0) << "read-only: " << readonly << dendl;
