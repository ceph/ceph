...
namespace rgw::s3vector {
  ...
  int delete_session(const DoutPrefixProvider* dpp, const std::string& tenant, const std::string& bucket_name);
  // get index cache stats for a session by bucket name
  int get_index_cache_stats(const DoutPrefixProvider* dpp, const std::string& tenant, const std::string& bucket_name, LanceDBSessionCacheStats& out_stats);
  // get metadata cache stats for a session by bucket name
  int get_metadata_cache_stats(const DoutPrefixProvider* dpp, const std::string& tenant, const std::string& bucket_name, LanceDBSessionCacheStats& out_stats);
}
