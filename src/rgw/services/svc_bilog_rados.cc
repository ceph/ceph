...
int RGWSI_BILog_RADOS::log_trim(const RGWBucketInfo& bucket_info, int shard_id, const bucket_log_layout_generation& log_layout, string& start_marker, string& end_marker)
{
  ...
  int r = svc.bi->open_bucket_index(bucket_info, shard_id, current_index, &index_pool, &bucket_objs, nullptr);
  if (r < 0) {
    return r;
  }
...
int RGWSI_BILog_RADOS::log_start(const RGWBucketInfo& bucket_info, int shard_id, const bucket_log_layout_generation& log_layout)
{
  RGWSI_RADOS::Pool index_pool;
  map<int, string> bucket_objs;

  bucket_index_layout_generation current_index;
  log_to_index_layout(log_layout, current_index);
...
int RGWSI_BILog_RADOS::log_stop(const RGWBucketInfo& bucket_info, int shard_id, const bucket_log_layout_generation& log_layout)
{
  RGWSI_RADOS::Pool index_pool;
  map<int, string> bucket_objs;
...
int RGWSI_BILog_RADOS::log_list(const RGWBucketInfo& bucket_info, int shard_id, const bucket_log_layout_generation log_layout, string& marker, uint32_t max,
                                std::list<rgw_bi_log_entry>& result, bool *truncated)
{
  ...
  int r = svc.bi->open_bucket_index(bucket_info, shard_id, current_index, &index_pool, &oids, nullptr);
  if (r < 0)
    return r;
...
int RGWSI_BILog_RADOS::get_log_status(const RGWBucketInfo& bucket_info,
                                      int shard_id, const bucket_log_layout_generation log_layout, 
				      ...
				      optional_yield y)
{
  ...
  int r = svc.bi->cls_bucket_head(bucket_info, shard_id, current_index, &headers, &bucket_instance_ids, y);
  if (r < 0)
    return r;
