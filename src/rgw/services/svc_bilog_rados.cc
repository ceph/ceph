...
int RGWSI_BILog_RADOS::log_trim(const RGWBucketInfo& bucket_info, const rgw::bucket_log_layout_generation& log_layout,
int shard_id, string& start_marker, string& end_marker)
{
...
int RGWSI_BILog_RADOS::log_start(const RGWBucketInfo& bucket_info, const rgw::bucket_log_layout_generation& log_layout, int shard_id)
{
  RGWSI_RADOS::Pool index_pool;
  map<int, string> bucket_objs;
...
int RGWSI_BILog_RADOS::log_stop(const RGWBucketInfo& bucket_info, const rgw::bucket_log_layout_generation& log_layout, int shard_id)
{
  RGWSI_RADOS::Pool index_pool;
  map<int, string> bucket_objs;
...
int RGWSI_BILog_RADOS::log_list(const RGWBucketInfo& bucket_info, const rgw::bucket_log_layout_generation& log_layout,
int shard_id, string& marker, uint32_t max, std::list<rgw_bi_log_entry>& result, bool *truncated)
{
				      ...
				      optional_yield y)
{
  ...
  int r = svc.bi->cls_bucket_head(bucket_info, current_index, shard_id, &headers, &bucket_instance_ids, y);
  if (r < 0)
    return r;
