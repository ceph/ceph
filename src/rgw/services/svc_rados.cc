...
int RGWSI_RADOS::open_pool_ctx(const rgw_pool& pool, librados::IoCtx& io_ctx,
                               const OpenParams& params, const Span& global_parent_span)
{
  return rgw_init_ioctx(get_rados_handle(), pool, io_ctx,
                        params.create,
                        params.mostly_omap, global_parent_span);
}
...
int RGWSI_RADOS::Pool::open(const OpenParams& params, const Span& global_parent_span)
{
  return rados_svc->open_pool_ctx(pool, state.ioctx, params, global_parent_span);
}
