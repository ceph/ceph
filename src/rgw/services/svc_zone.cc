  ...
  *zone_public_config = zone_iter->second;
  ...
  for (const auto& zgiter : current_period->get_map().zonegroups) {
    ...
    for (const auto& ziter : zg.zones) {
      ...
      RGWRESTConn *conn = new RGWRESTConn(cct, z.id, z.endpoints, zone_params->system_key, zone_id().id, zonegroup->api_name);
...
int RGWSI_Zone::init_zg_from_period(const DoutPrefixProvider *dpp, optional_yield y)
{
  ...
  for (iter = current_period->get_map().zonegroups.begin();
    ...
    add_new_connection_to_map(zonegroup_conn_map, zg, new RGWRESTConn(cct, zg.get_id(), endpoints, zone_params->system_key, zone_id().id, zg.api_name));
    if (!current_period->get_master_zonegroup().empty() &&
        zg.get_id() == current_period->get_master_zonegroup()) {
      rest_master_conn = new RGWRESTConn(cct, zg.get_id(), endpoints, zone_params->system_key, zone_id().id, zg.api_name);
    }
  }
...
int RGWSI_Zone::init_zg_from_local(const DoutPrefixProvider *dpp, optional_yield y)
{
  ...
  if (zonegroup->is_master_zonegroup()) {
    ...
    const auto& endpoints = master->second.endpoints;
    rest_master_conn = new RGWRESTConn(cct, zonegroup->get_id(), endpoints, zone_params->system_key, zone_id().id, zonegroup->api_name);
  }
