...
struct RGWSI_MBSObj_PutParams : public RGWSI_MetaBackend::PutParams {
  ...
  RGWSI_MBSObj_PutParams(boost::container::flat_map<string, bufferlist> *_pattrs,
...
public:
  ...
  RGWSI_MetaBackend_SObj(CephContext *cct, boost::asio::io_context& ioc);
  ...
  boost::system::error_code call_with_get_params(ceph::real_time *pmtime, std::function<boost::system::error_code(RGWSI_MetaBackend::GetParams&)> cb) override;

  boost::system::error_code pre_modify(RGWSI_MetaBackend::Context *ctx,
                 const string& key,
  ...
  post_modify(RGWSI_MetaBackend::Context *ctx,
  ...
  boost::system::error_code put_entry(RGWSI_MetaBackend::Context *ctx,
                ...
                RGWObjVersionTracker *objv_tracker,
                optional_yield y) override;
  boost::system::error_code list_init(RGWSI_MetaBackend::Context *_ctx,
                                      std::optional<string> marker) override;
  boost::system::error_code list_next(RGWSI_MetaBackend::Context *_ctx,
                                      int max, std::vector<string> *keys,
                                      bool *truncated) override;
  std::string list_get_marker(RGWSI_MetaBackend::Context *ctx) override;
  boost::system::error_code
  remove_entry(RGWSI_MetaBackend::Context *ctx,
               const string& key,
  ...
  boost::system::error_code call(std::optional<RGWSI_MetaBackend_CtxParams> opt,
                                 std::function<boost::system::error_code(RGWSI_MetaBackend::Context *)> f) override;
};


class RGWSI_MetaBackend_Handler_SObj : public RGWSI_MetaBackend_Handler {
  friend struct RGWSI_MetaBackend_SObj::Context_SObj;
