...
class RGWSI_MDLog_RADOS : public RGWSI_MDLog
{
...
public:
  RGWSI_MDLog_RADOS(CephContext *cct, bool run_sync);
  virtual ~RGWSI_MDLog_RADOS();
  ...
  RGWPeriodHistory::Cursor find_oldest_period(const DoutPrefixProvider *dpp, optional_yield y) override;

  /// initialize the oldest log period if it doesn't exist, and attach it to
  /// our current history
  RGWPeriodHistory::Cursor init_oldest_log_period(optional_yield y, const DoutPrefixProvider *dpp) override;

  /// read the oldest log period, and return a cursor to it in our existing
  /// period history
  RGWPeriodHistory::Cursor read_oldest_log_period(optional_yield y, const DoutPrefixProvider *dpp) const override;
  ...
  RGWCoroutine* read_oldest_log_period_cr(const DoutPrefixProvider *dpp, 
                                          ...
                                          RGWObjVersionTracker *objv) const override;
  ...
  int add_entry(const DoutPrefixProvider *dpp, const std::string& hash_key, const std::string& section, const std::string& key, bufferlist& bl) override;

  int get_shard_id(const std::string& hash_key, int *shard_id) override;

  RGWPeriodHistory *get_period_history() override {
    return period_history.get();
  }

  int pull_period(const DoutPrefixProvider *dpp, const std::string& period_id, RGWPeriod& period, optional_yield y) override;

  /// find or create the metadata log for the given period
  RGWMetadataLog* get_log(const std::string& period) override;
};
