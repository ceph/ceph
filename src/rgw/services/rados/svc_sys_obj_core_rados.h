...
class RGWSI_SysObj_Core_RADOS : public RGWSI_SysObj_Core
{
...
protected:
  ...
  using GetObjState = RGWSI_SysObj_Core_GetObjState_RADOS;
  ...
  int raw_stat(const DoutPrefixProvider *dpp, const rgw_raw_obj& obj, uint64_t *psize,
  ...
  int get_attr(const DoutPrefixProvider *dpp, const rgw_raw_obj& obj,
               const char *name, bufferlist *dest,
               optional_yield y) override;

  int set_attrs(const DoutPrefixProvider *dpp, const rgw_raw_obj& obj,
                std::map<std::string, bufferlist>& attrs,
...
public:
  RGWSI_SysObj_Core_RADOS(CephContext *cct): RGWSI_SysObj_Core(cct) {}
};
