...
class RGWSI_User_Module : public RGWSI_MBSObj_Handler_Module {
  ...
  const string prefix;
public:
  RGWSI_User_Module(RGWSI_User::Svc& _svc)
...
void RGWSI_User::init(RGWSI_RADOS *_rados_svc,
...
int RGWSI_User::do_start(optional_yield, const DoutPrefixProvider *dpp)
{
...
rgw_raw_obj RGWSI_User::get_buckets_obj(const rgw_user& user) const
{
                               ...
                               const DoutPrefixProvider *dpp)
{
  if(user.id == RGW_USER_ANON_ID) {
    ldpp_dout(dpp, 20) << "RGWSI_User::read_user_info(): anonymous user"
		       << dendl;
    return -ENOENT;
  }
  bufferlist bl;
...
int RGWSI_User::remove_key_index(const DoutPrefixProvider *dpp,
				 RGWSI_MetaBackend::Context *_ctx,
...
int RGWSI_User::remove_email_index(const DoutPrefixProvider *dpp,
				   RGWSI_MetaBackend::Context *_ctx,
				   const string& email,
				   optional_yield y)
{
...
int RGWSI_User::remove_swift_name_index(const DoutPrefixProvider *dpp,
					RGWSI_MetaBackend::Context *_ctx,
  ...
  for (; siter != info.swift_keys.end(); ++siter) {
    ...
    if (ret < 0 && ret != -ENOENT) {
      ldpp_dout(dpp, 0) << "ERROR: could not remove " << k.id
			<< " (swift name object), should be fixed (err="
			<< ret << ")" << dendl;
      return ret;
    }
  }
...
int RGWSI_User::remove_uid_index(RGWSI_MetaBackend::Context *ctx,
					 ...
					 const DoutPrefixProvider *dpp)
{
  ...
  try {
    ...
    int ret = read_user_info(ctx, uid.user_id,
                             &e.info, &e.objv_tracker, nullptr, &cache_info,
			     nullptr, y, dpp);
    if (ret < 0) {
      return ret;
    }
...
int RGWSI_User::get_user_info_by_email(RGWSI_MetaBackend::Context *ctx,
                                       ...
                                       const DoutPrefixProvider *dpp)
{
  return get_user_info_from_index(ctx, email,
...
int RGWSI_User::cls_user_update_buckets(const DoutPrefixProvider *dpp,
					rgw_raw_obj& obj,
...
int RGWSI_User::cls_user_add_bucket(const DoutPrefixProvider *dpp,
				    rgw_raw_obj& obj,
...
int RGWSI_User::cls_user_remove_bucket(const DoutPrefixProvider *dpp,
				       rgw_raw_obj& obj,
...
int RGWSI_User::add_bucket(const DoutPrefixProvider *dpp,
...
int RGWSI_User::remove_bucket(const DoutPrefixProvider *dpp,
			      RGWSI_MetaBackend::Context *ctx,
			      const rgw_user& user,
...
int RGWSI_User::cls_user_flush_bucket_stats(const DoutPrefixProvider *dpp,
...
int RGWSI_User::cls_user_list_buckets(const DoutPrefixProvider *dpp,
				      rgw_raw_obj& obj,
...
int RGWSI_User::list_buckets(const DoutPrefixProvider *dpp,
			     RGWSI_MetaBackend::Context *ctx,
			     const rgw_user& user,
			     const string& marker,
			     const string& end_marker,
			     ...
			     bool *is_truncated, optional_yield y)
{
  ...
  buckets->clear();
   if (user.id == RGW_USER_ANON_ID) {
    ldpp_dout(dpp, 20) << "RGWSI_User::list_buckets(): anonymous user" << dendl;
    *is_truncated = false;
    return 0;
  }
  rgw_raw_obj obj = get_buckets_obj(user);
...
int RGWSI_User::flush_bucket_stats(const DoutPrefixProvider *dpp,
				   RGWSI_MetaBackend::Context *ctx,
				   const rgw_user& user,
				   const RGWBucketEnt& ent,
				   optional_yield y)
...
int RGWSI_User::reset_bucket_stats(const DoutPrefixProvider *dpp,
				   RGWSI_MetaBackend::Context *ctx,
				   const rgw_user& user,
				   optional_yield y)
{
...
int RGWSI_User::cls_user_reset_stats(const DoutPrefixProvider *dpp,
...
int RGWSI_User::complete_flush_stats(const DoutPrefixProvider *dpp,
				     RGWSI_MetaBackend::Context *ctx,
				     const rgw_user& user, optional_yield y)
{
  rgw_raw_obj obj = get_buckets_obj(user);
...
int RGWSI_User::cls_user_get_header(const DoutPrefixProvider *dpp,
...
int RGWSI_User::cls_user_get_header_async(const DoutPrefixProvider *dpp,
...
int RGWSI_User::read_stats(const DoutPrefixProvider *dpp,
			   RGWSI_MetaBackend::Context *ctx,
...
int RGWSI_User::read_stats_async(const DoutPrefixProvider *dpp, RGWSI_MetaBackend::Context *ctx,
