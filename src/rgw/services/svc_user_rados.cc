...
public:  
  ...
  int put() {
    ...
    int ret = svc.meta_be->put(ctx, RGWSI_User::get_meta_key(info.user_id), params, &ot);
    if (ret < 0)
      return ret;
...
int RGWSI_User_RADOS::remove_uid_index(RGWSI_MetaBackend::Context *ctx, const RGWUserInfo& user_info, RGWObjVersionTracker *objv_tracker)
{
  ...
  int ret = svc.meta_be->remove(ctx, get_meta_key(user_info.user_id), params, objv_tracker);
