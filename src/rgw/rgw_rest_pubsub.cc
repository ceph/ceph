...
void RGWPSCreateNotif_ObjStore_S3::execute(const jaeger_tracing::jspan* const parent_span) {
...
void RGWPSDeleteNotif_ObjStore_S3::execute(const jaeger_tracing::jspan* const parent_span) {
  op_ret = get_params();
  if (op_ret < 0) {
    return;
  }
...
void RGWPSListNotifs_ObjStore_S3::execute(const jaeger_tracing::jspan* const parent_span) {
  ups.emplace(store, s->owner.get_id());
  auto b = ups->get_bucket(bucket_info.bucket);
  ceph_assert(b);
