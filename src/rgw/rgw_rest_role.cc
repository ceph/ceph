...
int RGWRestRole::verify_permission(optional_yield y, const jspan* const parent_span)
{
...
int RGWCreateRole::verify_permission(optional_yield y, const jspan* const parent_span)
{
  if (s->auth.identity->is_anonymous()) {
    return -EACCES;
  }
...
void RGWCreateRole::execute(optional_yield y, const jspan* const parent_span)
{
  op_ret = get_params();
  if (op_ret < 0) {
    return;
...
void RGWDeleteRole::execute(optional_yield y, const jspan* const parent_span)
{
  op_ret = get_params();
  if (op_ret < 0) {
    return;
...
int RGWGetRole::verify_permission(optional_yield y, const jspan* const parent_span)
{
  return 0;
}
...
void RGWGetRole::execute(optional_yield y, const jspan* const parent_span)
{
  op_ret = get_params();
  if (op_ret < 0) {
    return;
...
void RGWModifyRole::execute(optional_yield y, const jspan* const parent_span)
{
  op_ret = get_params();
  if (op_ret < 0) {
    return;
...
int RGWListRoles::verify_permission(optional_yield y, const jspan* const parent_span)
{
  if (s->auth.identity->is_anonymous()) {
    return -EACCES;
  }
...
void RGWListRoles::execute(optional_yield y, const jspan* const parent_span)
{
  op_ret = get_params();
  if (op_ret < 0) {
    return;
...
void RGWPutRolePolicy::execute(optional_yield y, const jspan* const parent_span)
{
  op_ret = get_params();
  if (op_ret < 0) {
    return;
...
void RGWGetRolePolicy::execute(optional_yield y, const jspan* const parent_span)
{
  op_ret = get_params();
  if (op_ret < 0) {
    return;
...
void RGWListRolePolicies::execute(optional_yield y, const jspan* const parent_span)
{
  op_ret = get_params();
  if (op_ret < 0) {
    return;
...
void RGWDeleteRolePolicy::execute(optional_yield y, const jspan* const parent_span)
{
  op_ret = get_params();
  if (op_ret < 0) {
    return;
