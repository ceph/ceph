...
void MgrClient::send_report()
{
        ...
        const PerfCountersCollection::CounterMap &by_path)
  {
	...
	session->declared.insert(path);
      ...
      ::encode(static_cast<uint64_t>(data.u64.load()), report->packed);
      if (data.type & PERFCOUNTER_LONGRUNAVG) {
        ::encode(static_cast<uint64_t>(data.avgcount.load()), report->packed);
        ::encode(static_cast<uint64_t>(data.avgcount2.load()), report->packed);
      }
    }
