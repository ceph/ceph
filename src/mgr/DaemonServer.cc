        ...
        if (osds.size() == max) {
          ...
          if (out_osd_report->ok_to_upgrade()) {
            ...
            dout(20) << "found " << osds.size() - to_upgrade.size()
                     << " additional OSDs that are safe to upgrade" << dendl;
          }
          return;  // yay, we hit the max
        }
      }
  ...
  std::shared_ptr<CommandContext>& cmdctx)
{
  ...
  if (prefix == "pg scrub" ||
    ...
    std::string crush_bucket;
    cmd_getval(cmdctx->cmdmap, "crush_bucket", crush_bucket);
    ...
    int r;
    std::set<int> osds;
    // Get osds in the crush bucket
    cluster_state.with_osdmap([&](const OSDMap& osdmap) {
        r = osdmap.get_osds_by_bucket_name(crush_bucket, &osds);
      });
    // Validate crush bucket
    if (r == -ENOENT) {
      ss << "\"" << crush_bucket << "\" does not exist";
    } else if (r < 0) {
      ss << "can not parse bucket name:\"" << crush_bucket << "\"";
    }
    if (r < 0) {
      cmdctx->reply(r, ss);
      return true;
    ...
    if ((int)osds.size() == 0) {
      ss << "no osds found in crush bucket:\"" << crush_bucket << "\"";
      r = -ENOENT;
      cmdctx->reply(r, ss);
      return true;
    }
