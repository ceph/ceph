...
bool DaemonServer::ms_handle_reset(Connection *con)
{
  ...
  if (con->get_peer_type() == CEPH_ENTITY_TYPE_OSD) {
    ...
    // session may be null if the connection reset before authentication
    // completed (no MgrSession was attached yet).  Skip osd_cons cleanup
    // in that case, but still fall through to daemon_connections below.
    if (session) {
  ...
  if (iter != daemon_connections.end()) {
    dout(10) << "removing from daemon_connections: " << con << dendl;
