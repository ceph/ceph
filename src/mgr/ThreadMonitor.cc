...
void ThreadMonitor::register_thread(pid_t thread_id,
  ...
  dout(20) << "ThreadMonitor: Registered new thread: '" << thread_name << "' (TID: " << thread_id << ") info: " 
           << "utime=" << info.last_snapshot.utime << ", "
           << "stime=" << info.last_snapshot.stime << ", "
           << "rss_pages=" << info.last_snapshot.rss_pages << dendl;
}
...
bool ThreadMonitor::read_thread_stat(pid_t tid, long long& utime, long long& stime) {
  ...
  // Now skip fields 3-13 (11 fields total)
  for (int i = 0; i < 11; ++i) {
    ss >> val;
  }
