...
public:
  ...
  version_t get_table_tid() { return table_tid; }
  ...
  MMDSSnapUpdate(inodeno_t i, version_t tid, int op) :
    Message(MSG_MDS_SNAPUPDATE), ino(i), table_tid(tid), snap_op(op) {}
private:
...
public:
  ...
  void print(ostream& o) const override {
    o << "snap_update(" << ino << " table_tid " << table_tid << ")";
  }

  void encode_payload(uint64_t features) override {
    ::encode(ino, payload);
    ::encode(table_tid, payload);
