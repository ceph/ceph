...
private:
 ...
 public:
  ...
  MOSDFailure() : MessageInstance<MOSDFailure, PaxosServiceMessage>(MSG_OSD_FAILURE, 0, HEAD_VERSION) { }
  MOSDFailure(const uuid_d &fs, int osd, const entity_addrvec_t& av,
	      int duration, epoch_t e)
    : MessageInstance<MOSDFailure, PaxosServiceMessage>(MSG_OSD_FAILURE, e, HEAD_VERSION, COMPAT_VERSION),
      fsid(fs),
      target_osd(osd),
      target_addrs(av),
      flags(FLAG_FAILED),
