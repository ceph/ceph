	...
	struct ceph_mount_info *cmount, struct ceph_ll_io_info *io_info)
{
	...
	struct ceph_ll_io_info *io_info_copy;
	int32_t i, err;
	...
	io_info_copy = proxy_malloc(sizeof(struct ceph_ll_io_info) +
				    io_info->iovcnt * sizeof(struct iovec));
	if (io_info_copy == NULL) {
		return -ENOMEM;
	}
	memcpy(io_info_copy, io_info, sizeof(struct ceph_ll_io_info));
	io_info_copy->iov = (struct iovec *)(io_info_copy + 1);
	memcpy((void *)io_info_copy->iov, io_info->iov,
	       sizeof(struct iovec) * io_info->iovcnt);

	req.info = ptr_checksum(&cmount->async.random, io_info_copy);
