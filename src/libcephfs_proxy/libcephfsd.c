					  ...
					  int32_t data_size)
{
	...
	err = ptr_check(&client->random, req->add_fscrypt_key.v0.cmount,
			(void **)&mount);
	if ((err >= 0) &&
	    (req->add_fscrypt_key.v0.kid != sizeof(kid))) {
		err = proxy_log(LOG_ERR, EINVAL, "Invalid size of key id");
	}
	if (err >= 0) {
		user = req->add_fscrypt_key.v0.user;

		CEPH_BUFF_ADD(ans, kid, sizeof(kid));

		err = ceph_add_fscrypt_key(
			proxy_cmount(mount), data, req->add_fscrypt_key.v0.key,
			(struct ceph_fscrypt_key_identifier *)kid, user);
					     ...
					     int32_t data_size)
{
	...
	err = ptr_check(&client->random, req->remove_fscrypt_key.v0.cmount,
			(void **)&mount);
	if ((err >= 0) &&
	    (req->remove_fscrypt_key.v0.arg !=
	     sizeof(struct fscrypt_remove_key_arg))) {
		err = proxy_log(LOG_ERR, EINVAL,
				"Invalid size of key remove arg");
	}
	if (err >= 0) {
		user = req->remove_fscrypt_key.v0.user;
						 ...
						 int32_t data_size)
{
	...
	err = ptr_check(&client->random, req->get_fscrypt_key_status.v0.cmount,
			(void **)&mount);
	if ((err >= 0) &&
	    (req->get_fscrypt_key_status.v0.arg !=
						   ...
						   int32_t data_size)
{
	...
	err = ptr_check(&client->random,
			req->ll_set_fscrypt_policy_v2.v0.cmount,
			(void **)&mount);
	if (err >= 0) {
		err = ptr_check(&client->random,
				req->ll_set_fscrypt_policy_v2.v0.inode,
				(void **)&inode);
	}
	if ((err >= 0) &&
	    (req->ll_set_fscrypt_policy_v2.v0.policy !=
						   ...
						   int32_t data_size)
{
	...
	err = ptr_check(&client->random,
			req->ll_get_fscrypt_policy_v2.v0.cmount,
			(void **)&mount);
	if (err >= 0) {
		err = ptr_check(&client->random,
				req->ll_get_fscrypt_policy_v2.v0.inode,
				(void **)&inode);
	}
	if ((err >= 0) &&
	    (req->ll_get_fscrypt_policy_v2.v0.policy !=
	     sizeof(struct fscrypt_policy_v2))) {
		err = proxy_log(LOG_ERR, EINVAL,
				"Invalid size of fscrypt_policy_v2");
	}
					  ...
					  int32_t data_size)
{
	...
	err = ptr_check(&client->random, req->ll_is_encrypted.v0.cmount,
			(void **)&mount);
	if (err >= 0) {
		err = ptr_check(&client->random, req->ll_is_encrypted.v0.inode,
				(void **)&inode);
	}
	if (err >= 0) {
		tag = (char *)CEPH_STR_GET(req->ll_is_encrypted, v0.tag, data);
