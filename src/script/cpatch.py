...
class Builder:
    ...
    def build(self):
        ...
        if os.path.isdir(self._workdir / "tmp_bin"):
            ...
            dlines.append("RUN rm -rf /usr/local/lib64 && ln -s /usr/lib64 /usr/local && ln -s /usr/share/ceph /usr/local/share")
            # locally built binaries assume libceph-common.so.2 is in /usr/lib64 - create link to library that was just copied
            dlines.append("RUN ln -s /usr/lib64/ceph/libceph-common.so.2 /usr/lib64/libceph-common.so.2")
