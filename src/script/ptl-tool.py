...
class AuditReport:
    ...
    def has_errors(self) -> bool:
        return len(self.issues) > 0 or self.interactive_failures > 0

    def get_consolidated_text(self) -> str:
        blocks = []
        for cat, text in self.issues:
            if text:
                blocks.append(text)
        return "\n\n---\n\n".join(blocks)

    def post_consolidated_review(self, session: requests.Session, pr: int, dry_run: bool = False):
        """
        ...
        if consolidated_text:
            ...
            if dry_run:
                ...
                session.post(endpoint, auth=gitauth(), json={'body': consolidated_text, 'event': 'REQUEST_CHANGES'})
...
def post_draft_review(session, pr, initial_text, base=None):
    ...
    """
    rfa_msg = "\n\nWhen you feel this PR is ready for a new audit check, please remove the `releng-audit-fail` label. If you need further help, please see [#ceph-upstream-releases channel on Slack](https://docs.ceph.com/en/latest/start/get-involved/) **and** request review from @Ceph Release Manager."
...
class CommitParityCheck(BaseAuditCheck):
    ...
    def run(self, ctx: AuditContext) -> None:
        ...
        for commit in invalid_format_commits:
            ...
            if args.ci_mode:
                report.add("Invalid Commit Format", md_text)
                continue
    
            while True:
                ans = input("Do you want to allow this commit anyway? [p/m/r/o/q] (p=proceed, m=skip to merge, r=add to review, o=open PR in browser, q=quit) ").strip().lower()
                if ans == 'o':
        ...
        if valid_ref and unmerged_cps:
            ...
            if args.ci_mode:
                report.add("Unmerged Cherry-Picks", md_text)
            else:
                while True:
        ...
        if missing_commits or extra_commits:
            ...
            if args.ci_mode:
                report.add("Parity Mismatch", md_text)
            else:
                while True:
            ...
            if len(found_prs) > 1:
                ...
                if args.ci_mode:
                    report.add("Multiple Source PRs", md_text)
                else:
                    while True:
            ...
            ctx.visualizer_md_table = visualizer_md_table
            ctx.found_prs = found_prs
                    ...
                    try:
                        ...
                        if issue.tracker.id != REDMINE_TRACKER_ID_BACKPORT and hasattr(issue, 'description') and issue.description:
                            ...
                            if match and int(match.group(1)) == orig_pr:
                                ...
                                ans = 'y' if args.ci_mode else input(f"Fix tracker #{issue.id} by moving PR link from description to 'Pull Request ID' field? [y/N/m]: ").strip().lower()
                                if ans == 'm':
                                    raise SkipToMerge()
                                elif ans == 'y':
                ...
                for bp_tracker in bp_trackers:
                    ...
                    if not cf_pr and hasattr(bp_tracker, 'description') and bp_tracker.description:
                        ...
                        if match:
                            ...
                            ans = 'y' if args.ci_mode else input(f"Fix tracker #{bp_tracker.id} by moving PR link from description to 'Pull Request ID' field? [y/N/m]: ").strip().lower()
                            if ans == 'm':
                                raise SkipToMerge()
                            elif ans == 'y':
...
def verify_pr_readiness(G, session, R, pr, pr_commits, tip, base, args):
    ...
    if report.has_errors():
        ...
        if args.ci_mode:
            report.post_consolidated_review(session, pr, dry_run=args.dry_run)
        else:
