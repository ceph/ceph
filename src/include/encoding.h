...
template<class T>
inline void CEPH_INLINE encode_raw(
  const T& t,
  ceph::bufferlist::contiguous_reserver& cr)
{
...
#define WRITE_RAW_ENCODER(etype)					\
  inline void encode(							\
    const etype& v,							\
    ceph::bufferlist& t,						\
    const uint64_t features=0)						\
  {									\
    ::ceph::encode_raw(v, t);						\
  }									\
  inline void CEPH_INLINE encode(					\
    const etype& v,							\
    ceph::bufferlist::contiguous_reserver& t,				\
    uint64_t features=0)						\
  {									\
    ::ceph::encode_raw(v, t);						\
  }									\
...
encode(const bool &v, ceph::bufferlist::contiguous_reserver& cr) {
  __u8 vv = v;
  encode_raw(vv, cr);
}
...
#define WRITE_CLASS_ENCODER(cl)						\
  ...
  inline void CEPH_INLINE encode(					\
    ...
    const uint64_t features=0)						\
  {									\
    ENCODE_DUMP_PRE(); c.encode(cr); ENCODE_DUMP_POST(cl);		\
  }									\
...
#define WRITE_CLASS_MEMBER_ENCODER(cl)					\
  ...
  inline void CEPH_INLINE encode(					\
    const cl& c,							\
    ::ceph::bufferlist::contiguous_reserver& cr) const			\
  {									\
    ENCODE_DUMP_PRE(); c.encode(cr); ENCODE_DUMP_POST(cl);		\
  }									\
  inline void decode(cl& c, ::ceph::bufferlist::const_iterator& p)	\
...
#define WRITE_CLASS_ENCODER_FEATURES(cl)				\
  ...
  {									\
    ENCODE_DUMP_PRE(); c.encode(cr, features); ENCODE_DUMP_POST(cl);	\
  }									\
  inline void decode(cl& c, ::ceph::bufferlist::const_iterator& p)	\
  {									\
    c.decode(p);							\
...
inline void CEPH_INLINE encode_nohead(
  std::string_view s,
  ceph::bufferlist::contiguous_reserver& cr)
{
  cr.append(s.data(), s.length());
}
...
inline void CEPH_INLINE encode_nohead(
  ...
  ceph::bufferlist::contiguous_reserver& cr)
{
  encode_nohead(std::string_view(s), cr);
}
...
inline void CEPH_INLINE encode_destructively(
  ceph::bufferlist& s,
  ceph::bufferlist::contiguous_reserver& cr)
{
  __u32 len = s.length();
  encode(len, cr);
  static_cast<bufferlist&>(cr).claim_append(s);
}
...
inline CEPH_INLINE std::enable_if_t<!traits::supported>
  encode(const std::list<T, Alloc>& ls,
	 ceph::bufferlist::contiguous_reserver& cr)
{
  __u32 n = (__u32)(ls.size());  // c++11 std::list::size() is O(1)
  encode(n, cr);
...
template<class T, class Alloc, typename traits>
inline CEPH_INLINE std::enable_if_t<!traits::supported>
  encode(const std::vector<T,Alloc>& v,
	 ceph::bufferlist::contiguous_reserver& bl,
	 const uint64_t features)
{
  __u32 n = (__u32)(v.size());
  encode(n, bl);
...
{							     \
  ...
  ::ceph::bufferlist::contiguous_reserver bl(_scoped_bl_ref);\
  __u8 struct_v = v;                                         \
