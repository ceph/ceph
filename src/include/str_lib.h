...
inline constexpr char split_strings_default_delims[] = ";,= \t";
...
//
//   auto strings = split_strings(input);
//
...
{
  ceph::for_each_substr(s, ceph::str_lib_detail::split_strings_default_delims,
                        std::forward<FnT>(fn));
}
...
std::ranges::enable_borrowed_range<ceph::split_view> = true;
...
constexpr std::vector<std::string> split_strings(
  ...
  std::string_view delims =
    ceph::str_lib_detail::split_strings_default_delims)
{
  const auto parts = ceph::split(str, delims);
  std::vector<std::string> strings;

  std::ranges::transform(parts, std::back_inserter(strings),
