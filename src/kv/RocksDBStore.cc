...
void RocksDBStore::RocksDBTransactionImpl::rm_range_keys(
  ...
  ldout(db->cct, 0) << __func__ << " key_count_hint=" << key_count_hint
  ...
  int64_t thresh = h_bound;

  if (db_delete_range_threshold_hint >= 0) {
    thresh = std::min(db_delete_range_threshold_hint, h_bound);
  }
  ...
  auto iterate_shard_cf = [this](const std::string &prefix,
                                 const std::string &start,
                                 const std::string &end, int64_t thresh,
    ...
    if (p_iter == db->cf_handles.end()) {
      ...
      for (auto cf : p_iter->second.handles) {
        if (key_count_hint_per_shard < std::max(thresh, l_bound)) {
          iterate_shard_cf(prefix, start, end, thresh, bounds, cf);
        } else {
          bat.DeleteRange(cf, rocksdb::Slice(start), rocksdb::Slice(end));
        }
      }
