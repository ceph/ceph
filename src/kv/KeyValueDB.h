...
public:
  ...
  public:
    ...
    // Remove Single Key which exists and was not overwritten.  This
    // API is only related to performance optimization, and should
    // only be re-implemented by log-insert-merge tree based keyvalue
    // stores(such as RocksDB).  If a key is overwritten (by calling
    // set multiple times), then the result of calling rm_single on
    // this key is undefined.
    virtual void rm_single(
      const std::string &prefix,   ///< [in] Prefix to search for
      const std::string &k	   ///< [in] Key to remove
      ) {
