...
public:
  ...
  /// See RocksDB merge operator definition, we support the basic associative merge only right now.
  class MergeOperator {
    public:
    /// Merge into a key that doesn't exist
    virtual void merge_nonexistant(const string& rhs,std::string& new_value) = 0;
    /// Merge into a key that does exist
    virtual void merge(const std::string &lhs,const string& rhs,std::string& new_value) = 0;
...
protected:
  std::vector<std::pair<std::string, std::shared_ptr<MergeOperator> > > merge_ops; /// List of matching prefixes and merge operators
