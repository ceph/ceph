...
static void alloc_aligned_buffer(bufferlist& data, unsigned len, unsigned off)
{
  ...
  if (off & ~CEPH_PAGE_MASK) {
    // head
    unsigned head = 0;
    head = std::min<unsigned>(CEPH_PAGE_SIZE - (off & ~CEPH_PAGE_MASK), left);
...
int Pipe::read_message(Message **pm, AuthSessionHandler* auth_handler)
{
  ...
  if (data_len) {
	...
	if (!newbuf.length()) {
      ...
      int read = std::min<int>(bp.length(), left);
...
int Pipe::write_message(const ceph_msg_header& header, const ceph_msg_footer& footer, bufferlist& blist)
{
  ...
  while (left > 0) {
    unsigned donow = std::min<unsigned>(left, pb->length()-b_off);
    if (donow == 0) {
...
ssize_t Pipe::buffered_recv(char *buf, size_t len, int flags)
{
  ...
  if (recv_len > recv_ofs) {
    int to_read = std::min<int>(recv_len - recv_ofs, left);
