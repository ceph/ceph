    ...
    : id(i), _netif(cct, dev, c), _dev(dev), _inet(cct, c, &_netif)
{
  ...
  auto ms_dpdk_host_ipv4_addr = cct->_conf->get_val<std::string>(
    "ms_dpdk_host_ipv4_addr");
  auto ms_dpdk_gateway_ipv4_addr = cct->_conf->get_val<std::string>(
    "ms_dpdk_gateway_ipv4_addr");
  auto ms_dpdk_netmask_ipv4_addr = cct->_conf->get_val<std::string>(
    "ms_dpdk_netmask_ipv4_addr");
  bool parsed = parse_available_address(ms_dpdk_host_ipv4_addr,
                                        ms_dpdk_gateway_ipv4_addr,
