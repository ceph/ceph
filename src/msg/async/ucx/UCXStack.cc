...
{
  ldout(cct, 25) << "Creating UCXWorker: " << worker_id << dendl;
}
                       ...
                       ConnectedSocket *peer_skt)
{
  ...
  ucx_peerskt->set_active_status(true);
...
UCXProEngine::UCXProEngine(CephContext *cct, ucp_worker_h ucp_worker)
  : cct(cct), ucp_worker(ucp_worker), engine_status(0)
{
}

UCXProEngine::~UCXProEngine()
{
  engine_status = -1;
  thread_engine.join();
  ldout(cct, 25) << dendl;
...
{
  ldout(cct, 25) << " ucp_worker_progress start " << dendl;
  engine_status = 1;
  while (true) {
    if (engine_status != 1) {
      break;
...
void UCXProEngine::add_connections(uint64_t conn_id, UCXConSktImpl* ucx_conn)
{
  ucx_connections[conn_id] = ucx_conn;
}
                               ...
                               const ucp_am_recv_param_t *param)
{
  ...
  auto ucx_conn = iter->second;
  self_this->dispatch_am_message(ucx_conn, header, header_length, data, param);

  return UCS_OK;
}

void UCXProEngine::dispatch_am_message(UCXConSktImpl *ucx_conn,
                         ...
                         void *data, const ucp_am_recv_param_t *param)
{
  ...
  ldout(cct, 30) << "got io (AM) message " << io_op_names[msg->op_code] << ", "
                 << "sn " << msg->sn << ", "
                 << "data size " << msg->data_size
                 << dendl;
  ceph_assert(msg->op_code == IO_WRITE);
  ucx_conn->handle_io_am_write_request(msg, data, param);
}
...
UCXStack::UCXStack(CephContext *cct)
  : NetworkStack(cct)
{
  ldout(cct, 25) << "creating UCXStack: " << this << dendl;
