...
bool NVMeofGwMon::encode_last_gw_version(MonSession *session , std::string gw_id) {
  ...
  dout(10) << "Versions: mon_last " << is_mon_last << " gw last "
      << is_gw_last << " GW " << gw_id << ", sub "
...
void NVMeofGwMon::check_sub(Subscription *sub)
{
  ...
  if (sub->next <= map.epoch)
  {
    ...
    dout(10) << "Sending map to subscriber "
        << sub->session->con->get_peer_addr()
        << " gmap version last" << gw_map_version_last  << dendl;
    sub->session->con->send_message2(make_message<MNVMeofGwMap>
                (map, gw_map_version_last));
...
bool NVMeofGwMon::preprocess_beacon(MonOpRequestRef op) {
  ...
  dout(15) << "beacon from " << m->get_type() << " GW : " << m->get_gw_id()
...
bool NVMeofGwMon::prepare_beacon(MonOpRequestRef op) {
  ...
  dout(20) << "availability " <<  m->get_availability() << " GW : " << m->get_gw_id() <<
      " osdmap_epoch " << m->get_last_osd_epoch() << " subsystems "
      << m->get_subsystems() << dendl;
  ...
  if (avail == gw_availability_t::GW_CREATED) {
    if (gw == group_gws.end()) {
      ...
      dout(10) << "Warning: GW " << gw_id << " group_key " << group_key
          << " was not found in the  map.Created_gws "<< map.created_gws <<dendl;
      goto set_propose;
    }
    else {
      dout(10) << "GW  prepares the full startup " << gw_id << " GW availability: "
          << pending_map.created_gws[group_key][gw_id].availability << dendl;
      if (pending_map.created_gws[group_key][gw_id].availability
          == gw_availability_t::GW_AVAILABLE)
      {
        dout(4) << " Warning :GW marked as Available in the NVmeofGwMon database, performed full startup - Force gw to exit!"
            << gw_id <<dendl;
  ...
  if (pending_map.created_gws[group_key][gw_id].last_gw_map_epoch_valid == false) {
    dout(20) <<  "map epoch of gw is not up-to-date " << gw_id << " epoch "
        << map.epoch << " beacon_epoch " << m->get_last_gwmap_epoch() <<  dendl;
  }
