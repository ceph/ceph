...
MonSession::MonSession(Connection *c)
  : RefCountedObject(g_ceph_context),
    con(c),
    item(this) { }

void MonSession::_ident(const entity_name_t& n, const entity_addrvec_t& av) {
...
bool MonSession::is_capable(std::string service, int mask) {
...
void MonSession::dump(ceph::Formatter *f) const {
...
void MonSessionMap::remove_session(MonSession *s) {
...
MonSession *MonSessionMap::new_session(const entity_name_t& n,
...
void MonSessionMap::add_session(MonSession *s) {
...
MonSession *MonSessionMap::get_random_osd_session(OSDMap *osdmap) {
...
void MonSessionMap::add_update_sub(MonSession *s, const std::string& what, version_t start, bool onetime, bool incremental_onetime) {
  Subscription *sub = 0;
...
void MonSessionMap::remove_sub(Subscription *sub) {
