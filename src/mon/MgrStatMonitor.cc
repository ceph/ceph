...
void MgrStatMonitor::calc_pool_availability()
{
  ...
  // recorded values
  if (reset_availability_last_uptime_downtime_val != utime_t(1, 2)) {
    for (const auto& i : pool_availability) {
      const auto& poolid = i.first;
      pool_availability[poolid].last_downtime = reset_availability_last_uptime_downtime_val;
      pool_availability[poolid].last_uptime = reset_availability_last_uptime_downtime_val;
    }
    dout(20) << __func__ << " reset last_uptime and last_downtime to " 
             << reset_availability_last_uptime_downtime_val << dendl;
    reset_availability_last_uptime_downtime_val = utime_t(1, 2);
  }
