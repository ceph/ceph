...
bool LogMonitor::preprocess_log(MonOpRequestRef op)
{
  ...
  if (last_to_remove >= trim_max * trim_max_multiplier) {
    dout(10) << "last trim quantities reached max, ignore this op to avoid db growing too quickly" << dendl;
    goto done;
  }

  if (is_insufficient_space) {
    dout(10) << "mon store.db space insufficient, ignore this op to avoid crash" << dendl;
    goto done;
  }
