...
struct NvmeGwMonState {
  ...
  NvmeGwMonState(NvmeAnaGrpId id): ana_grp_id(id), availability(gw_availability_t::GW_CREATED),
...
struct NqnState {
  ...
  NqnState(const std::string& _nqn, const SmState& sm_state,
      const NvmeGwMonState & gw_created, bool encode_gmap_last) : nqn(_nqn)
  {
    uint32_t i = 0;
    if (encode_gmap_last) {
      for (auto& state_itr: sm_state) {
        ...
        std::pair<gw_exported_states_per_group_t, epoch_t> state_pair;
        state_pair.first = (sm_state.at(state_itr.first)
...
#define MAX_SUPPORTED_ANA_GROUPS 16 // ths define is used for backward compatibility
      ...
      for (auto& state_itr: sm_state) {
        state_pair[state_itr.first].first = (sm_state.at(state_itr.first)
                                    == gw_states_per_group_t::GW_ACTIVE_STATE
            || sm_state.at(state_itr.first) == gw_states_per_group_t::GW_WAIT_BLOCKLIST_CMPL)
            ? gw_exported_states_per_group_t::GW_EXPORTED_OPTIMIZED_STATE
                : gw_exported_states_per_group_t::GW_EXPORTED_INACCESSIBLE_STATE;
...
struct NvmeGwClientState {
  ...
  NvmeGwClientState() : NvmeGwClientState(REDUNDANT_GW_ANA_GROUP_ID, 0,
