...
class MonitorDBStore
{
 ...
 public:
  ...
  int apply_transaction(MonitorDBStore::TransactionRef t) {
    ...
    if (do_dump) {
      if (!g_conf->get_val<bool>("mon_debug_dump_json")) {
        bufferlist bl;
        t->encode(bl);
  ...
  struct C_DoTransaction : public Context {
    ...
    void finish(int r) override {
       ...
       */
      double delay_prob = g_conf->get_val<double>("mon_inject_transaction_delay_probability");
      if (delay_prob && (rand() % 10000 < delay_prob * 10000.0)) {
        utime_t delay;
        double delay_max = g_conf->get_val<double>("mon_inject_transaction_delay_max");
  ...
  protected:
	...
	return false;
      ...
      if (g_conf->get_val<bool>("mon_sync_debug")) {
  ...
  void _open(string kv_type) {
    ...
    if (g_conf->get_val<bool>("mon_debug_dump_transactions")) {
      if (!g_conf->get_val<bool>("mon_debug_dump_json")) {
        dump_fd_binary = ::open(
          g_conf->get_val<std::string>("mon_debug_dump_location").c_str(),
        ...
        dump_fd_json.open(g_conf->get_val<std::string>("mon_debug_dump_location").c_str());
      }
      do_dump = true;
    }
    if (kv_type == "rocksdb")
  ...
  int create_and_open(ostream &out) {
    ...
    int r = read_meta("kv_backend", &kv_type);
    if (r < 0) {
      kv_type = g_conf->get_val<std::string>("mon_keyvaluedb");
      r = write_meta("kv_backend", kv_type);
      if (r < 0)
	return r;
    }
