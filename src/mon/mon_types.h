...
namespace ceph {
  namespace features {
    namespace mon {
      ...
      constexpr mon_feature_t FEATURE_KV_RANGE_OPS(   (1ULL << 33));
...
static inline const char *ceph::features::mon::get_feature_name(uint64_t b) {
  ...
  } else if (f == FEATURE_KV_RANGE_OPS) {
    return "kv_range_ops";
...
inline mon_feature_t ceph::features::mon::get_feature_by_name(const std::string &n) {
  ...
  } else if (n == "kv_range_ops") {
    return FEATURE_KV_RANGE_OPS;
  } else if (n == "reserved") {
