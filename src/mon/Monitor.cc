...
void Monitor::forward_request_leader(PaxosServiceMessage *req)
{
  ...
  if (req->session_mon >= 0) {
    ...
    encode_message(g_ceph_context, req, -1, rr->request_bl);   // for my use only; use all features
...
void Monitor::try_send_message(Message *m, entity_inst_t to)
{
  ...
  bufferlist bl;
  encode_message(g_ceph_context, m, -1, bl);  // fixme: assume peers have all features we do.
