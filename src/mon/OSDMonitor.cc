				      ...
				      const cmdmap_t& cmdmap)
{
  ...
  if (prefix == "osd setcrushmap" ||
    ...
    wait_for_finished_proposal(op, new Monitor::C_Command(mon, op, 0, rs,
					      get_last_committed() + 1));
    return true;
  } else if (prefix == "osd pool force-reremove-snap") {
    string poolstr;
    cmd_getval(cmdmap, "pool", poolstr);
    int64_t pool = osdmap.lookup_pg_pool_name(poolstr.c_str());
    if (pool < 0) {
    ...
    ss << "force reremoving snap ids in the range of [" << lower_snapid_bound << ","
    ...
    if (force_removed_snapids.size()) {
      ss << "reremoved snapids: " << force_removed_snapids;
    } else {
      ss << "no snapshots were reremoved";
    }
