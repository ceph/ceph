				      ...
				      map<string,cmd_vartype> &cmdmap)
{
  ...
  if (prefix == "osd setcrushmap" ||
    ...
    if (update) {
	  pending_inc.crush.clear();
	  newcrush.encode(pending_inc.crush);
      ss << "move updating item names '" << names << " to location " << loc << " to crush map";
	  getline(ss, rs);
      wait_for_finished_proposal(op, new Monitor::C_Command(mon, op, 0, rs, get_last_committed() + 1));
	  return true;
      } else {
      ss << "no need to move item names '" << names << "' to location " << loc << " in crush map";
	  err = 0;
    }
    ...
    if (cmd_getval(g_ceph_context, cmdmap, "ancestor", ancestor_str)) {
      if (!newcrush.name_exists(ancestor_str)) {
        err = -ENOENT;
        ss << "ancestor item '" << ancestor_str << "' does not appear in the crush map";
        goto reply;
      }
    } else
    ancestor_str.clear();
