...
struct cmd_osd_set_flag final : public OSDMonitor_Projection
{
    OSDMonitor& owner;
    MonOpRequestRef& op;

    static std::unordered_map<std::string, int> flag_kvs;

 public:
    cmd_osd_set_flag(OSDMonitor& owner, MonOpRequestRef& op)
     : owner(owner),
       op(op)
    {}

 public:
    auto set_flag(const std::string& key) {
        const auto kv = flag_kvs.find(key.c_str());

        return end(flag_kvs) != kv ? prepare_set_flag(owner, op, kv->second) : false;
    }

    auto unset_flag(const std::string& key) {
        const auto kv = flag_kvs.find(key.c_str());

        return end(flag_kvs) != kv ? prepare_unset_flag(owner, op, kv->second) : false;
    }
};

std::unordered_map<std::string, int> cmd_osd_set_flag::flag_kvs {
...
bool OSDMonitor::prepare_set_flags(MonOpRequestRef op, const std::vector<int>&& flags)
{
  op->mark_osdmon_event(__func__);
  ...
  for (std::vector<int>::size_type i = 0; i != flags.size(); ++i) {
      auto& flag = flags[i];
...
bool OSDMonitor::prepare_set_flag(MonOpRequestRef op, int flag)
{
  ...
  return prepare_set_flags(op, { flag });
}
				      ...
				      const cmdmap_t& cmdmap)
{
  ...
  if (prefix == "osd setcrushmap" ||
  ...
  } else if (prefix == "osd set" || prefix == "osd setall") { 
    ...
    vector<string> keys;
    cmd_getval(cct, cmdmap, "keys", keys);

    if (key == "recovery_deletes") {
    ...
    // At this point, any special flag has already been handled, so we can try to handle
    // any remaining key as a normal input:
    if (!key.empty()) {
        keys.push_back(key);
    }

    // Handle flags without special requirements:
    cmd_osd_set_flag sf(*this, op);

    for (const auto& flag : keys) {
        if (!sf.set_flag(flag)) {
            ss << "unrecognized flag '" << flag << '\'';
            err = -EINVAL;
        }
    }
  ...
  } else if (prefix == "osd unset" || prefix == "osd unsetall") { 
    string key;
    cmd_getval(cct, cmdmap, "key", key);
