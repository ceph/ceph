...
bool OSDMonitor::preprocess_command(MonOpRequestRef op)
{
  ...
  using OSDMonReadCommandRef = std::shared_ptr<OSDMonReadCommand>;
  list<OSDMonReadCommandRef> read_cmds = {
    OSDMonReadCommandRef(new OSDMonStat(mon, this, cct)),
    OSDMonReadCommandRef(new OSDMonDumpAndFriends(mon, this, cct)),
    OSDMonReadCommandRef(new OSDMonGetMaxOSD(mon, this, cct)),
    OSDMonReadCommandRef(new OSDMonGetOSDUtilization(mon, this, cct)),
    OSDMonReadCommandRef(new OSDMonFind(mon, this, cct))
  };
  ...
  for (auto &cmd : read_cmds) {
    ...
    return cmd->preprocess(op, cmdmap, osdmap);
  }

  // non-abstracted commands
  //
				      ...
				      const cmdmap_t& cmdmap)
{
  ...
  for (auto &cmd : write_cmds) {
    ...
    dout(10) << __func__ << " handling prefix '" << prefix << "'" << dendl;
    return cmd->prepare(op, cmdmap, pending_inc, osdmap);
  }
