...
void PGMapDigest::print_summary(Formatter *f, ostream *out) const
{
  ...
  if (!f) {
         ...
         ++p)
    {
      std::stringstream ss;
      ss << p->first;
      max_width = std::max<unsigned>(ss.str().size(), max_width);
    }
...
int64_t PGMap::get_rule_avail(const OSDMap& osdmap, int ruleno) const
{
  ...
  for (auto p = wm.begin(); p != wm.end(); ++p) {
	...
	(1.0 - fratio);
      double avail = std::max<double>(0.0, osd_info->second.kb_avail - unusable);
      avail *= 1024.0;
    ...
    Formatter *f)
{
  ...
  if (by_pg) {
    ...
    // by osd utilization
    int num_osd = std::max<int>(1, pgm.osd_stat.size());
