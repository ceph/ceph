  ...
  mempool::pgmap::list<pair<pool_stat_t,utime_t> > *delta_avg_list)
{
  ...
  *last_ts = ts;        // @p ts becomes the last timestamp we saw
  ...
  delta_t =
    std::min(delta_t, utime_t(2 * (
      cct ? cct->_conf->get_val<bool>("mon_delta_reset_interval") : 10), 0));
  ...
  health_check_map_t *checks) const
{
  ...
  // REQUEST_SLOW
  // REQUEST_STUCK
  auto warn_age = cct->_conf->get_val<double>("mon_osd_warn_op_age");
  auto err_age = cct->_conf->get_val<double>("mon_osd_err_op_age_ratio");
  if (warn_age > 0 &&
      !osd_sum.op_queue_age_hist.h.empty() &&
      osd_sum.op_queue_age_hist.upper_bound() / 1000.0 > warn_age) {
    list<string> warn_detail, error_detail;
    unsigned warn = 0, error = 0;
    float err_age = warn_age * err_age;
    ...
    if (!warn_detail.empty()) {
      ostringstream ss;
      ss << warn << " slow requests are blocked > " << warn_age << " sec";
  ...
  {
    auto warn_scrub = cct->_conf->get_val<int64_t>("mon_warn_not_scrubbed");
    auto warn_deep = cct->_conf->get_val<int64_t>("mon_warn_not_deep_scrubbed");
    auto scrub_ival = cct->_conf->get_val<int64_t>("mon_scrub_interval");
    auto deep_ival = cct->_conf->get_val<double>("osd_deep_scrub_interval");
    if (warn_scrub || warn_deep) {
      ...
      const double age = warn_scrub + scrub_ival;
      utime_t cutoff = now;
      cutoff -= age;
      const double deep_age = warn_deep + deep_ival;
      utime_t deep_cutoff = now;
      deep_cutoff -= deep_age;
      for (auto& p : pg_stat) {
        if (warn_scrub && p.second.last_scrub_stamp < cutoff) {
          if (detail_max > 0) {
            ostringstream ss;
        ...
        if (warn_deep && p.second.last_deep_scrub_stamp < deep_cutoff) {
    ...
    Formatter *f)
{
  ...
  if (by_pg) {
    ...
    if (!num_osds ||
        (num_pg_copies / num_osds < g_conf->get_val<uint64_t>("mon_reweight_min_pgs_per_osd"))) {
    ...
    if ((uint64_t)pgm.osd_sum.kb * 1024 / num_osd < reweight_min) {
      *ss << "Refusing to reweight: we only have " << pgm.osd_sum.kb
	  << " kb across all osds!\n";
      return -EDOM;
    }
    if ((uint64_t)pgm.osd_sum.kb_used * 1024 / num_osd < reweight_min) {
