...
public:
  ...
  ceph::unordered_map<pg_t,pg_stat_t> pg_stat;//每一个pg的状态
  ceph::unordered_map<int32_t,osd_stat_t> osd_stat;//每一个osd的状态
  set<int32_t> full_osds;//bigger than full_ratio的所有osd
  set<int32_t> nearfull_osds;//bigger than nearfull_osds的所有osd
  float full_ratio;//full时的ratio
  float nearfull_ratio;//近满的ratio

  // mapping of osd to most recently reported osdmap epoch
  ceph::unordered_map<int32_t,epoch_t> osd_epochs;//记录某个osd最近上报时对应的epoch
  ...
  ceph::unordered_map<int,int> num_pg_by_state;//处理某一具体状态的，pg数量
  int64_t num_pg, num_osd;//pg数量，osd数量
  ceph::unordered_map<int,pool_stat_t> pg_pool_sum;//针对pool id查pool状态
  pool_stat_t pg_sum;//所有pg统计信息
  osd_stat_t osd_sum;//所有osd统计信息
  mutable epoch_t min_last_epoch_clean;//最小的clean epoch{所有pg}
  ceph::unordered_map<int,int> blocked_by_sum;//记录被某一具体osd阻塞的pg数量
  ceph::unordered_map<int,set<pg_t> > pg_by_osd;//可针对osd查此osd对应(负责的）的所有pg列表
 ...
 public:
  ...
  set<pg_t> creating_pgs;//记录正处于creating阶段的pg
  map<int,map<epoch_t,set<pg_t> > > creating_pgs_by_osd_epoch;//osd（注）,epoch到正在创建的Ｐg的映射
  ...
  void set_full_ratios(float full, float nearfull) {//设置full,nearfull比率
  ...
  size_t get_num_pg_by_osd(int osd) const {//获取指定osd对应的pg数
  ...
  pool_stat_t get_pg_pool_sum_stat(int64_t pool) const {//给定pool获取pool对应的状态
  ...
  static void generate_test_instances(list<PGMap*>& o);//不看
};
