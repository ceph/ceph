...
bool HealthMonitor::check_member_health()
{
  ...
  // snapshot of usage
  DataStats stats;
  get_fs_stats(stats.fs_stats, g_conf->get_val<std::string>("mon_data").c_str());
  ...
  if (stats.fs_stats.avail_percent <= g_conf->get_val<int64_t>("mon_data_avail_crit")) {
    stringstream ss, ss2;
  ...
  } else if (stats.fs_stats.avail_percent <= g_conf->get_val<int64_t>("mon_data_avail_warn")) {
    stringstream ss, ss2;
  ...
  if (stats.store_stats.bytes_total >= g_conf->get_val<uint64_t>("mon_data_size_warn")) {
    stringstream ss, ss2;
	...
	<< " >= mon_data_size_warn ("
	<< prettybyte_t(g_conf->get_val<uint64_t>("mon_data_size_warn")) << ")";
    d.detail.push_back(ss2.str());
  }
  ...
  // OSD_NO_DOWN_OUT_INTERVAL
  {
    ...
    if (g_conf->get_val<bool>("mon_warn_on_osd_down_out_interval_zero") &&
        g_conf->get_val<int64_t>("mon_osd_down_out_interval") == 0) {
      ostringstream ss, ds;
