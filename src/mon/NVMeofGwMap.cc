...
void NVMeofGwMap::to_gmap(std::map<NvmeGroupKey, NvmeGwMonClientStates>& Gmap,
  ...
  for (const auto& created_map_pair: created_gws) {
    ...
    for (const auto& gw_created_pair: gw_created_map) {
      ...
      auto gw_state = NvmeGwClientState(gw_created.ana_grp_id, epoch,
          gw_created.availability);
...
void NVMeofGwMap::add_grp_id(const NvmeGwId &gw_id, const NvmeGroupKey& group_key,
...
void NVMeofGwMap::remove_grp_id(const NvmeGwId &gw_id, const NvmeGroupKey& group_key,
    const NvmeAnaGrpId grpid)
{
  ...
  for (auto& gws_states: created_gws[group_key]) {
    ...
    if (gws_states.first == gw_id) {
      ...
      for (auto& state_itr: created_gws[group_key][gw_id].sm_state) {
        bool modified;
        fsm_handle_gw_delete(gw_id, group_key,state_itr.second ,
            state_itr.first, modified);
      }
...
int NVMeofGwMap::process_gw_map_gw_down(const NvmeGwId &gw_id, const NvmeGroupKey& group_key,
  ...
  if (gw_state != gws_states.end()) {
    ...
    for (auto& state_itr: created_gws[group_key][gw_id].sm_state ) {
      fsm_handle_gw_down(gw_id, group_key, state_itr.second, state_itr.first,
          propose_pending);
...
void NVMeofGwMap::process_gw_map_ka(const NvmeGwId &gw_id, const NvmeGroupKey& group_key,
    epoch_t& last_osd_epoch, bool &propose_pending)
{
  ...
  if (st.availability == gw_availability_t::GW_CREATED) {
    ...
    for (auto& state_itr: created_gws[group_key][gw_id].sm_state) {
      fsm_handle_gw_alive(gw_id, group_key, gw_state->second, state_itr.second,
      ...
      if (state.availability == gw_availability_t::GW_UNAVAILABLE
        ...
        for (auto& gw_state2 : gws_states) {
          ...
          if (state2.availability == gw_availability_t::GW_AVAILABLE
              && state2.sm_state[state.ana_grp_id] == gw_states_per_group_t::GW_ACTIVE_STATE)
          {
...
void  NVMeofGwMap::set_failover_gw_for_ANA_group(const NvmeGwId &failed_gw_id,
   const NvmeGroupKey& group_key, const NvmeGwId &gw_id,  NvmeAnaGrpId ANA_groupid)
{
...
void  NVMeofGwMap::find_failback_gw(const NvmeGwId &gw_id,
      const NvmeGroupKey& group_key,   bool &propose)
{
  auto& gws_states = created_gws[group_key];
  ...
  if (do_failback == false) {
    ...
    dout(10)  << "Failback GW candidate was not found, just set Optimized to group "
        << gw_state.ana_grp_id << " to GW " << gw_id << dendl;
  ...
  for (auto& gw_state_it: gws_states) {
    ...
    if (st.sm_state[gw_state.ana_grp_id] == gw_states_per_group_t::GW_ACTIVE_STATE) {
      dout(10)  << "Found Failback GW " << failback_gw_id
          << " that previously took over the ANAGRP " << gw_state.ana_grp_id
          << " of the available GW " << gw_id << dendl;
...
void  NVMeofGwMap::find_failover_candidate(const NvmeGwId &gw_id, const NvmeGroupKey& group_key,
...
#define MIN_NUM_ANA_GROUPS 0xFFF
  ...
  if (gw_state->second.sm_state[grpid] == gw_states_per_group_t::GW_ACTIVE_STATE
    ...
    for (auto& found_gw_state: gws_states) { // for all the gateways of the subsystem
      ...
      if (st.availability == gw_availability_t::GW_AVAILABLE) {
        ...
        for (auto& state_itr: created_gws[group_key][gw_id].sm_state ) {
          ...
          if (st.sm_state[anagrp] == gw_states_per_group_t::GW_OWNER_WAIT_FAILBACK_PREPARED
              || st.sm_state[anagrp] == gw_states_per_group_t::GW_WAIT_FAILBACK_PREPARED
        ...
        if (min_num_ana_groups_in_gw > current_ana_groups_in_gw) {
          ...
          dout(10) << "choose: gw-id  min_ana_groups " << min_loaded_gw_id
              << current_ana_groups_in_gw << " min " << min_num_ana_groups_in_gw << dendl;
        }
      }
    }
...
void NVMeofGwMap::fsm_handle_gw_alive(const NvmeGwId &gw_id, const NvmeGroupKey& group_key,
  ...
  case gw_states_per_group_t::GW_WAIT_BLOCKLIST_CMPL:
  {
    ...
    if (gw_map.blocklist_data[grpid].osd_epoch <= last_osd_epoch) {
      dout(10) << "is-failover: " << gw_map.blocklist_data[grpid].is_failover
          << " osd epoch changed from " << gw_map.blocklist_data[grpid].osd_epoch << " to "<< last_osd_epoch
      ...
      dout(20) << "osd epoch not changed from " <<  gw_map.blocklist_data[grpid].osd_epoch
          << " to "<< last_osd_epoch << " Ana-grp: " << grpid
...
void NVMeofGwMap::fsm_handle_gw_down(const NvmeGwId &gw_id, const NvmeGroupKey& group_key,
    ...
    case gw_states_per_group_t::GW_WAIT_FAILBACK_PREPARED:
      ...
      for (auto& gw_st: created_gws[group_key]) {
        ...
        if (st.sm_state[grpid] == gw_states_per_group_t::GW_OWNER_WAIT_FAILBACK_PREPARED) { // found GW   that was intended for  Failback for this ana grp
          dout(4) << "Warning: Outgoing Failback when GW is down back - to rollback it"
              <<" GW "  <<gw_id << "for ANA Group " << grpid << dendl;
...
void NVMeofGwMap::fsm_handle_gw_delete(const NvmeGwId &gw_id, const NvmeGroupKey& group_key,
  ...
  case gw_states_per_group_t::GW_OWNER_WAIT_FAILBACK_PREPARED:
  {
    ...
    if (grpid == gw_state.ana_grp_id) {// Try to find GW that temporary owns my group - if found, this GW should pass to standby for  this group
      ...
      for (auto& gs: gateway_states) {
        if (gs.second.sm_state[grpid] == gw_states_per_group_t::GW_ACTIVE_STATE
            || gs.second.sm_state[grpid] == gw_states_per_group_t::GW_WAIT_FAILBACK_PREPARED) {
  ...
  case gw_states_per_group_t::GW_WAIT_FAILBACK_PREPARED:
  {
    ...
    for (auto& nqn_gws_state: created_gws[group_key]) {
      ...
      if (st.sm_state[grpid] == gw_states_per_group_t::GW_OWNER_WAIT_FAILBACK_PREPARED) { // found GW   that was intended for  Failback for this ana grp
        dout(4) << "Warning: Outgoing Failback when GW is deleted - to rollback it"
            << " GW " << gw_id << "for ANA Group " << grpid << dendl;
        st.standby_state(grpid);
        break;
      }
...
void NVMeofGwMap::fsm_handle_to_expired(const NvmeGwId &gw_id,
  ...
  if (fbp_gw_state.sm_state[grpid] == gw_states_per_group_t::GW_WAIT_FAILBACK_PREPARED) {
    for (auto& gw_state: created_gws[group_key]) {
      ...
      if (st.ana_grp_id == grpid) {// group owner
        ...
        cancel_timer(gw_id, group_key, grpid);
        map_modified = true;
        if (st.sm_state[grpid] == gw_states_per_group_t::GW_OWNER_WAIT_FAILBACK_PREPARED &&
            st.availability == gw_availability_t::GW_AVAILABLE )
        {
        ...
        else if (st.sm_state[grpid] == gw_states_per_group_t::GW_STANDBY_STATE  &&
            st.availability == gw_availability_t::GW_AVAILABLE)
        {
        ...
        dout(10) << "Failback unsuccessfull GW: " << gw_id << " becomes Standby for the ANA groupId "
                  << grpid  << dendl;
        map_modified = true;
        break;
      }
    ...
    dout(4) << "Warning: Expired GW_WAIT_FAILOVER_PREPARED timer from GW, Force exit the GW "
...
struct CMonRequestProposal : public Context {
  ...
  utime_t expires;
  CMonRequestProposal(NVMeofGwMap *mon , entity_addrvec_t addr_vector,
       utime_t until) : m(mon), addr_vect(addr_vector), expires (until)  {}
  void finish(int r) {
...
int NVMeofGwMap::blocklist_gw(const NvmeGwId &gw_id, const NvmeGroupKey& group_key,
        NvmeAnaGrpId grpid, epoch_t &epoch, bool failover)
{
  ...
  if (gw_map.nonce_map[grpid].size() > 0) {
    ...
    if (!mon->osdmon()->is_writeable()) {
      ...
      if (!mon->osdmon()->is_writeable()) {
        dout(10) << "osdmon is not writable after blocklist is done, waiting, epoch = "
            << epoch << dendl;
        mon->osdmon()->wait_for_writeable_ctx( new CMonRequestProposal(this, addr_vect, expires ));// return false;
      }
      else{
    ...
    dout(10) << str << " mon->osdmon()->blocklist: epoch : " << epoch <<  " address vector: "
        << addr_vect << " " << addr_vect.size() << dendl;
  }
  else{
        ...
        if (st.sm_state[ana_group] == gw_states_per_group_t::GW_ACTIVE_STATE) {
          ...
          if (count == 2) {
            dout(1) << "Critical Error : number active states per ana-group " <<
                 ana_group << "more than 1 in pool-group " << group_key << dendl;
    ...
    for (auto& gw_to: group_to.second) {
      ...
      for (auto &to_itr:to.data)
      {
        ...
        dout(20) << "Checking timer for GW " << gw_id << " ANA GRP " <<
            to_itr.first<<" value(seconds): "<< (int)to.data[to_itr.first].timer_value << dendl;
...
void NVMeofGwMap::start_timer(const NvmeGwId &gw_id,
     const NvmeGroupKey& group_key, NvmeAnaGrpId anagrpid, uint8_t value_sec)
{
  fsm_timers[group_key][gw_id].data[anagrpid].timer_started = 1;
  fsm_timers[group_key][gw_id].data[anagrpid].timer_value = value_sec;
  dout(10) << "start timer for ana " << anagrpid << " gw " << gw_id << "value sec "
      << (int)value_sec << dendl;
  const auto now = std::chrono::system_clock::now();
  fsm_timers[group_key][gw_id].data[anagrpid].end_time = now + std::chrono::seconds(value_sec);
}

int  NVMeofGwMap::get_timer(const NvmeGwId &gw_id, const NvmeGroupKey& group_key,
...
void NVMeofGwMap::cancel_timer(const NvmeGwId &gw_id,
    const NvmeGroupKey& group_key, NvmeAnaGrpId anagrpid) {
