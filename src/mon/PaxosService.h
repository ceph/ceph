...
public:
 ...
 public:
  ...
  /**
   * Update @p first_committed with on-disk contents
   */
  void update_first_committed() {
    cached_first_committed =
      mon->store->get(get_service_name(), first_committed_name);
  }
  /**
   * Update @p last_committed with on-disk contents
   */
  void update_last_committed() {
    cached_last_committed =
      mon->store->get(get_service_name(), last_committed_name);
  }
  /**
   * Update all cached versions.
   */
  void update_cached_versions() {
    update_first_committed();
