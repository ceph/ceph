...
void Replayer<I>::init(Context* on_finish) {
  ...
  ceph_assert(m_state_builder->remote_image_peer != nullptr);
  m_remote_image_peer = m_state_builder->remote_image_peer;
      ...
      if (mirror_ns->complete) {
        ...
        if (peer_uuids.empty() ||
            (mirror_ns->state == cls::rbd::MIRROR_SNAPSHOT_STATE_PRIMARY_DEMOTED &&
             peer_uuids.size() == 1 &&
             peer_uuids.count(m_remote_image_peer->uuid) == 1)) {
          // After relocation, the primary snapshots on the new secondary become
          // absolute snapshots. They can be pruned if they are already unlinked.
...
void Replayer<I>::handle_prune_snapshot(int r) {
  ...
  if (r < 0 && r != -ENOENT) {
    derr << "failed to prune snapshot: " << cpp_strerror(r)
         << dendl;
    handle_replay_complete(r, "failed to prune snapshot");
    return;
  }
