...
template <typename I>
void RefreshImagesRequest<I>::mirror_group_list() {
  dout(10) << dendl;
  ...
  librados::AioCompletion *aio_comp = create_rados_callback<
    RefreshImagesRequest<I>,
    &RefreshImagesRequest<I>::handle_mirror_group_list>(this);
  int r = m_remote_io_ctx.aio_operate(RBD_MIRRORING, aio_comp, &op, &m_out_bl);
  ceph_assert(r == 0);
  aio_comp->release();
}
...
template <typename I>
void RefreshImagesRequest<I>::handle_mirror_group_list(int r) {
  dout(10) << "r=" << r << dendl;
...
template <typename I>
void RefreshImagesRequest<I>::group_image_list() {
  if (m_groups.empty()) {
    finish(0);
    return;
  }
  ...
  librados::AioCompletion *aio_comp = create_rados_callback<
    RefreshImagesRequest<I>,
    &RefreshImagesRequest<I>::handle_group_image_list>(this);
...
template <typename I>
void RefreshImagesRequest<I>::handle_group_image_list(int r) {
  dout(10) << "r=" << r << dendl;
