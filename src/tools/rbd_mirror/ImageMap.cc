...
void ImageMap<I>::update_image_mapping(Updates &&map_updates,
  ...
  for (auto const &update : map_updates) {
    bufferlist bl;
    encode(image_map::PolicyData(update.entity.size, policy_meta), bl);
...
void ImageMap<I>::update_images_added(
  ...
  for (auto &entity : entities) {
    ...
    auto result = m_peer_map[global_id].insert(peer_uuid);
    if (result.second && m_peer_map[global_id].size() == 1) {
      if (m_policy->add_entity(global_id, entity.size)) {
        schedule_action(global_id);
      }
    }
  }
...
void ImageMap<I>::update_images(const std::string &peer_uuid,
...

  {
    ...
    if (!removed_entities.empty()) {
      update_images_removed(peer_uuid, removed_entities);
    }
    if (!added_entities.empty()) {
      update_images_added(peer_uuid, added_entities);
    }
  }
