...
// Should never happen
 ...
 // bootstrap again when the replayer is restarted
    finish_start(r, "remote is not ready yet");
    return;
  } else if (r == -EINVAL) {
    sync_group_names();
  } else if (r < 0) {
...
void GroupReplayer<I>::handle_stop_group_replayer(int r) {
  ...
  if (r != 0 ){
    dout(10) << "failed to shutdown the group_replayer=" << r << dendl;
    return;
  }
...
void GroupReplayer<I>::shut_down(int r) {
  ...
  Context *ctx = new LambdaContext(
    [this, r](int _r) {
      set_mirror_group_status_update(cls::rbd::MIRROR_GROUP_STATUS_STATE_STOPPED,
                                     "");
      handle_shut_down(r);
    });
