...
void PrepareRemoteGroupRequest<I>::handle_get_mirror_info(int r) {
 ...
 if (m_mirror_group.state == cls::rbd::MIRROR_GROUP_STATE_DISABLING) {
    dout(5) << "remote group mirroring is in disabling state" << dendl;
    finish(-ERESTART);
    return;
  }

  switch (m_mirror_group.mirror_image_mode) {
...
/*
 // We currently support images only in the same pool as the group

  int r = librbd::util::create_ioctx(m_io_ctx, "local image pool",
 ...
 auto &spec = m_images.front().spec;
  ...
  if (r < 0) {
    ...
    return;
      ...
      (*m_state_builder)->remote_images.insert({spec.pool_id, mirror_image.global_image_id});
  }
