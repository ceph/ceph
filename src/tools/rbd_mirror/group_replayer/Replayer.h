...
public:
  static Replayer* create(
    ...
    return new Replayer(threads, local_io_ctx, remote_io_ctx, global_group_id,
        local_mirror_uuid, local_mirror_peer_uuid, pool_meta_cache, local_group_id, remote_group_id,
 ...
 *                 |                                                              |                                     |                    | |
 *                 + <-------------------------------------- +                    v                                     v            error --+ |
 *                 v          yes                            ^          PRUNE_GROUP_SNAPSHOTS           LOCAL_GROUP_IMAGE_LIST_BY_ID --------> +
 *   IS_RESYNC_REQUESTED ---------------------> +            |                    |                                     |                      ^
 *                 |                            |            |                    v                                     v                      |
 *                 v          yes               v            |         PRUNE_USER_GROUP_SNAPSHOTS       PRUNE_ALL_IMAGE_SNAPSHOTS_BY_GSID      |
 *   IS_RENAME_REQUESTED ----------> HANDLE_REPLAY_COMPLETE  |                    |                                     |                      |
 *                 |                            ^            |                    v                                     v                      |
 *                 v                            |            |        PRUNE_MIRROR_GROUP_SNAPSHOT             PRUNE_IMAGE_SNAPSHOT             |
 *   CHECK_LOCAL_GROUP_SNAPSHOTS -------------> +            |                    |                                     |                      |
 *                 |                demoted/split-brain      |                    v                                     v                   ^  |
 *                 v   m_retry_validate_snap == true         |      MIRROR_GROUP_SNAPSHOT_UNLINK_PEER          GROUP_SNAP_REMOVE            |  |
 *                 + --------------------------------> +     |                    |                                     |                 --+  |
 *                 |                                   |     |                    v              if all_pruned == true, + -------------------> +
 *                 v                                   |     |            PRUNE_GROUP_SNAPSHOT                  m_check_creating_snaps = false ^
 *  SCAN_FOR_UNSYNCED_GROUP_SNAPSHOTS                  |     |                    |                                                            |
 *                 |                                   |     |                    v                                                            |
 *                 v                                   |     |        PRUNE_ALL_IMAGE_SNAPSHOTS                                                |
 ...
 *       CREATE_GROUP_SNAPSHOT                         |     + <---------- GROUP_SNAP_REMOVE ------------------------------------------------> +
