...
void PrepareLocalGroupRequest<I>::send() {
  ...
  m_images.clear();
  (*m_state_builder)->local_images.clear();
...
void PrepareLocalGroupRequest<I>::handle_get_mirror_info(int r) {
  ...
  if (m_mirror_group.state == cls::rbd::MIRROR_GROUP_STATE_CREATING) {
    dout(5) << "local group is still in creating state, issuing a removal"
            << dendl;
    remove_local_group();
    return;
  } else if (m_mirror_group.state == cls::rbd::MIRROR_GROUP_STATE_DISABLING) {
    dout(5) << "local group mirroring is in disabling state" << dendl;

    //TODO: We cannot guarantee a valid promotion state
    /*if (m_promotion_state == librbd::mirror::PROMOTION_STATE_PRIMARY) {
      finish(-ERESTART);
    } else {
      // Have the bootstrap remove the local group and images
      finish(-ENOLINK);
    }*/
    finish(-ERESTART);
    return;
...
//TODO: We need this information to delete the group.

  dout(10) << "local_group_id=" << m_local_group_id << ", "
           << "local_promotion_state=" << m_promotion_state
          // << "local_primary_mirror_uuid=" << m_primary_mirror_uuid
           << dendl;
/*
  (*m_state_builder)->local_group_id = m_local_group_id;
  (*m_state_builder)->local_promotion_state = m_promotion_state;
  (*m_state_builder)->group_name = *m_local_group_name;
  (*m_state_builder)->local_mirror_group_state = m_mirror_group.state;
*/
  list_group_images();
}
...
void PrepareLocalGroupRequest<I>::handle_get_mirror_images(int r) {
  ...
  if (r < 0) {
    ...
    return;
      ...
      //(*m_state_builder)->local_images.insert({mirror_image.global_image_id,
...
void PrepareLocalGroupRequest<I>::remove_local_group() {
  ...
  auto req = RemoveLocalGroupRequest<I>::create(
      m_io_ctx, m_global_group_id, m_local_group_id,
      *m_local_group_name, *m_state_builder,
