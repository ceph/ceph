...
public:
//TODO: Redo this. Ideally this should only take the global_group_id and get
//everything else itself. The statebuilder is being passed only for the images
// to speed this up.
  static RemoveLocalGroupRequest *create(
    ...
    return new RemoveLocalGroupRequest(io_ctx, global_group_id, group_id,
                                       group_name, state_builder, resync,
  ...
  RemoveLocalGroupRequest(
    ...
    : m_io_ctx(io_ctx), m_global_group_id(global_group_id),
      m_group_id(group_id), m_group_name(group_name),
      m_state_builder(state_builder), m_resync(resync),
...
//  librados::IoCtx m_image_io_ctx;
  ...
  void move_images_to_trash();
  void handle_move_images_to_trash(int r);
