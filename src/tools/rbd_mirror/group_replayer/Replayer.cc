...
void Replayer<I>::create_mirror_snapshot(
  ...
  cls::rbd::GroupSnapshotNamespaceMirror mirror_namespace;
  ...
  if (require_osd_release >= CEPH_RELEASE_TENTACLE) {
    ...
    mirror_namespace = cls::rbd::GroupSnapshotNamespaceMirror(snap_state,
  ...
  if (remote_snap.snaps.size() == local_image_snap_specs.size()) {
    ...
    if (mirror_namespace.complete ==
      ...
      dout(10) << "local group snap info: "
               ...
               << dendl;
      locker.unlock();
      handle_post_mirror_snapshot_complete(0, group_snap_id, local_snap_copy,
                                           on_finish);
      return;
    }
