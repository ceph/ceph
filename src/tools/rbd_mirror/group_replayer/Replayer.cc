...
void Replayer<I>::mirror_snapshot_complete(
  ...
  if (local_snap.state == cls::rbd::GROUP_SNAPSHOT_STATE_CREATED) {
    check_mirror_snapshot_sync_status(group_snap_id, local_snap, on_finish);
    return;
  }
  ...
  if (remote_snap.snaps.size() == local_image_snap_specs.size()) {
    ...
    if (mirror_namespace.complete ==
      ...
      dout(10) << "local group snap info: "
               ...
               << dendl;
      locker.unlock();
      check_mirror_snapshot_sync_status(group_snap_id, local_snap_copy, on_finish);
      return;
    }
...
template <typename I>
void Replayer<I>::check_mirror_snapshot_sync_status(
    const std::string &group_snap_id,
    const cls::rbd::GroupSnapshot &local_snap,
    Context *on_finish) {
  std::unique_lock locker{m_lock};
