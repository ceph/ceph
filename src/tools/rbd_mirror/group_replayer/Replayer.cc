...
const cls::rbd::GroupSnapshot* get_latest_complete_mirror_group_snapshot(
  ...
  for (auto it = gp_snaps.rbegin(); it != gp_snaps.rend(); ++it) {
    ...
    if (gp_snap_ns != nullptr) {
      if (gp_snap_ns->complete == cls::rbd::MIRROR_GROUP_SNAPSHOT_COMPLETE) {
        ceph_assert(it->state == cls::rbd::GROUP_SNAPSHOT_STATE_CREATED);
        return &*it;
      }
      if (gp_snap_ns->complete == cls::rbd::MIRROR_GROUP_SNAPSHOT_COMPLETE_IF_CREATED &&
          it->state == cls::rbd::GROUP_SNAPSHOT_STATE_CREATED) {
        return &*it;
      }
    }
  }
...
void Replayer<I>::validate_local_group_snapshots() {
  ...
  for (auto &local_snap : m_local_group_snaps) {
    ...
    if (local_snap.state == cls::rbd::GROUP_SNAPSHOT_STATE_CREATED) {
      ...
      if (is_group_snapshot_complete(local_snap.state, ns->complete)) {
        continue;
      }
    }
...
void Replayer<I>::handle_load_local_group_snapshots(int r) {
  ...
  if (ns.state != cls::rbd::MIRROR_SNAPSHOT_STATE_PRIMARY) {
    ...
    if (!is_group_snapshot_complete(last_local_snap->state, ns.complete)) {
      m_retry_validate_snap = true;
    }
    ...
    if (prev_snap_id.empty() ||
      ...
      if (snap != m_remote_group_snaps.end()) {
        ...
        if (next_remote_snap_ns == nullptr) {
          ...
          if(is_group_snapshot_complete(snap->state, next_remote_snap_ns->complete)) {
...
void Replayer<I>::create_group_snapshot(cls::rbd::GroupSnapshot snap,
  ...
  if (snap_type == cls::rbd::GROUP_SNAPSHOT_NAMESPACE_TYPE_MIRROR) {
    ...
    for (; next_remote_snap != m_remote_group_snaps.end(); ++next_remote_snap) {
      ...
      } else if (!is_group_snapshot_complete(next_remote_snap->state, next_remote_snap_ns->complete)) {
...
void Replayer<I>::mirror_snapshot_complete(
  ...
  if (local_snap.state == cls::rbd::GROUP_SNAPSHOT_STATE_CREATED) {
    handle_post_mirror_snapshot_complete(0, group_snap_id, local_snap,
                                         on_finish);
    return;
  }
  ...
  if (remote_snap.snaps.size() == local_image_snap_specs.size()) {
    ...
    auto comp = create_rados_callback(
      new LambdaContext([this, group_snap_id, local_snap_copy, on_finish](int r) {
        handle_post_mirror_snapshot_complete(r, group_snap_id, local_snap_copy,
            on_finish);
      }));
    int r = m_local_io_ctx.aio_operate(
        librbd::util::group_header_name(m_local_group_id), comp, &op);
...
void Replayer<I>::prune_mirror_group_snapshots(
  ...
  for (auto local_snap = m_local_group_snaps.begin();
    ...
    } else if (!is_group_snapshot_complete(local_snap->state, local_snap_ns->complete)) {
        break;
    }
    auto snap_type = cls::rbd::get_group_snap_namespace_type(
        local_snap->snapshot_namespace);
    if (snap_type != cls::rbd::GROUP_SNAPSHOT_NAMESPACE_TYPE_MIRROR) {
      ...
      if (prune_snap) { // snapshot before user snap exists ?
        ...
        if (next_local_snap_ns == nullptr) {
          ...
          !is_group_snapshot_complete(next_local_snap->state, next_local_snap_ns->complete)) {
