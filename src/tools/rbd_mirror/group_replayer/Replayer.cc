...
void Replayer<I>::create_group_snapshot(cls::rbd::GroupSnapshot snap,
  ...
  if (snap_type == cls::rbd::GROUP_SNAPSHOT_NAMESPACE_TYPE_MIRROR) {
    ...
    auto ctx = new LambdaContext([this, snap](int r) mutable {
      if (r < 0) {
        dout(10) << "create regular snapshot failed: " << cpp_strerror(r) << dendl;
        return;
      }
...
void Replayer<I>::mirror_snapshot_complete(
  ...
  auto ctx = new LambdaContext([this, group_snap_id, itl, itr, local_images_mirror, on_finish](int r) {
    if (r < 0) {
      derr << "failed to list local group images: " << cpp_strerror(r) << dendl;
      on_finish->complete(r);
      return;
    }
    handle_mirror_snapshot_image_list(group_snap_id, *itl, *itr, *local_images_mirror, on_finish);
  });
...
void Replayer<I>::regular_snapshot_complete(
  ...
  std::vector<cls::rbd::GroupImageStatus>* local_images_regular =
                                    new std::vector<cls::rbd::GroupImageStatus>();
  auto ctx = new LambdaContext([this, group_snap_id, itl, itr, local_images_regular, on_finish](int r) {
    if (r < 0) {
      derr << "failed to list group images: " << cpp_strerror(r) << dendl;
      on_finish->complete(r);
      return;
    }
    handle_regular_snapshot_image_list(group_snap_id, *itl, *itr, *local_images_regular, on_finish);
  });
