...
void Replayer<I>::validate_image_snaps_sync_complete(
  ...
  if (snap_type != cls::rbd::GROUP_SNAPSHOT_NAMESPACE_TYPE_MIRROR) {
    ...
    regular_snapshot_complete(locker, local_snap.id, ctx);
    ...
    mirror_snapshot_complete(locker, local_snap.id, ctx);
  }
  return;
}
...
void Replayer<I>::validate_next_local_group_snapshot(size_t index) {
  ...
  if (index >= m_local_group_snaps.size()) {
    dout(10) << "All local snapshots validation complete"
             << dendl;
    proceed_to_load_snapshots();
    return;
  }
...
void Replayer<I>::create_group_snapshot(cls::rbd::GroupSnapshot snap,
  ...
  if (snap_type == cls::rbd::GROUP_SNAPSHOT_NAMESPACE_TYPE_MIRROR) {
    ...
    create_mirror_snapshot(locker, &snap, snap_state, ctx);
