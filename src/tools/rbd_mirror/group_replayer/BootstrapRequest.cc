...
void BootstrapRequest<I>::handle_group_get_info(int r) {
  ...
  if (m_mirror_group.state == cls::rbd::MIRROR_GROUP_STATE_CREATING) {
    dout(10) << "local group is still in creating state" << dendl;
    prepare_local_group();
    return;
  }
...
void BootstrapRequest<I>::handle_prepare_local_group(int r) {
  ...
  bool local_group_snap_metadata_corrupt =
    (r == -ESTALE && *m_resync_requested &&
     m_remote_group_prepare_result == 0 &&
     (*m_state_builder)->is_remote_primary());

  if (local_group_snap_metadata_corrupt) {
    dout(5) << "local group snapshot metadata is corrupt, continuing resync "
               "from the remote primary" << dendl;
  } else if (r == -ENOENT) {
  ...
  if (*m_resync_requested) {
    ...
    } else if (m_remote_group_prepare_result != 0) {
...
void BootstrapRequest<I>::handle_prepare_remote_group(int r) {
  ...
  if (*m_resync_requested) {
    dout(10) << "resync requested, preparing local group before deciding "
                "whether to remove it" << dendl;
