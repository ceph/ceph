...
void BootstrapRequest<I>::handle_get_local_group_id(int r) {
  ...
  if (r == 0) {
    ...
    derr << "error decoding local group id: " << cpp_strerror(r) << dendl;
    finish(r);
  }
}
...
void BootstrapRequest<I>::handle_prepare_remote_group(int r) {
  ...
  // Prepare the local group even if remote preparation failed. This keeps the
  // local role available so a healthy secondary can still be promoted.
  ...
  if (*m_resync_requested && r == 0 &&
      (*m_state_builder)->is_remote_primary()) {
    dout(10) << "resync requested and remote is primary, skipping local "
                "group preparation" << dendl;
...
void BootstrapRequest<I>::prepare_local_group() {
  ...
  auto req = PrepareLocalGroupRequest<I>::create(
    ...
    &m_prepare_local_group_name, &m_mirror_group, m_state_builder,
...
void BootstrapRequest<I>::get_local_mirror_group() {
  ...
  // Local preparation is skipped for resync from a remote primary. Read the
  // mirror group needed by RemoveLocalGroupRequest without listing snapshots.
  librados::ObjectReadOperation op;
...
template <typename I>
void BootstrapRequest<I>::continue_bootstrap(int remote_result) {
  dout(10) << "remote_result=" << remote_result << dendl;
  ...
  if (remote_result == -ENOENT) {
    ...
    derr << "error preparing remote group: " << cpp_strerror(remote_result)
         << dendl;
    finish(remote_result);
    return;
  }
  ...
  if (m_local_group_id.empty()) {
    ...
    if (m_prepare_local_group_name != m_prepare_remote_group_name) {
      finish(-EREMCHG);
      return;
    }
  }
