...
//  ceph_assert(r < 0 || *m_state_builder != nullptr);
  ...
  if (r >= 0 && !m_prepare_local_group_name.empty()) {
...
void BootstrapRequest<I>::handle_prepare_remote_group(int r) {
  ...
  if (state_builder->is_local_primary()) {
	...
	return;
      ...
      //FIXME: Check if state == disabled /delete the local group?
      remove_local_group();
      return;
    }
  } else if (r < 0) {
...
//    }
  ...
  if (state_builder->local_group_id.empty()) {
    ...
    // Local group is secondary. See if rsync is set.
...
void BootstrapRequest<I>::create_local_group() {
  ...
  auto req = CreateLocalGroupRequest<I>::create(
    m_local_io_ctx, m_global_group_id,
    *m_state_builder, m_threads->work_queue, ctx);
  req->send();
}
...
void BootstrapRequest<I>::remove_local_group() {
  ...
  ceph_assert(!(*m_state_builder)->group_name.empty());

  dout(10) << "group_name=" << (*m_state_builder)->group_name
           << " , group_id=" << (*m_state_builder)->local_group_id << dendl;
  ...
  auto req = RemoveLocalGroupRequest<I>::create(
      ...
      (*m_state_builder)->group_name, *m_state_builder,
...
//    if (m_local_mirror_group.state == cls::rbd::MIRROR_GROUP_STATE_DISABLED) {
      ...
      *m_local_group_ctx = {(*m_state_builder)->group_name,
                            ...
                            m_global_group_id,
                            (*m_state_builder)->is_local_primary(), m_local_io_ctx};
      r = create_replayers();
    }
  }
...
/*
  ...
  if (!state_builder->remote_group_id.empty() && state_builder->is_remote_primary()) {
...
//      m_image_replayers2->emplace(image_replayer, local_io_ctx);
      //(*m_image_replayer_index)[{remote_pool_id, remote_image_id}] = image_replayer;
    }
  } else if ((*m_state_builder)->is_local_primary()) {
