...
void usage()
{
  cout << " usage: [--print] <mapfilename>\n";
  cout << "   --create-from-conf      creates an osd map with default configurations\n";
  cout << "   --createsimple <numosd> [--clobber] [--pg-bits <bitsperosd>] [--pgp-bits <bits>] creates a relatively generic OSD map with <numosd> devices\n";
  cout << "   --pgp-bits <bits>       pgp_num map attribute will be shifted by <bits>\n";
  cout << "   --pg-bits <bits>        pg_num map attribute will be shifted by <bits>\n";
  cout << "   --clobber               allows osdmaptool to overwrite <mapfilename> if it already exists\n";
  cout << "   --export-crush <file>   write osdmap's crush map to <file>\n";
  cout << "   --import-crush <file>   replace osdmap's crush map with <file>\n";
  cout << "   --health                dump health checks\n";
  ...
  cout << "   --test-random           do random placements\n";
  cout << "   --test-map-pg <pgid>    map a pgid to osds\n";
  cout << "   --test-map-object <objectname> [--pool <poolid>] map an object to osds\n";
  cout << "   --upmap-cleanup <file>  clean up pg_upmap[_items] entries, writing\n";
  cout << "                           commands to <file> [default: - for stdout]\n";
  cout << "   --upmap <file>          calculate pg upmap entries to balance pg layout\n";
  cout << "                           writing commands to <file> [default: - for stdout]\n";
  cout << "   --upmap-max <max-count> set max upmap entries to calculate [default: 10]\n";
  cout << "   --upmap-deviation <max-deviation>\n";
  cout << "                           max deviation from target [default: 5]\n";
  cout << "   --upmap-pool <poolname> restrict upmap balancing to 1 or more pools\n";
  cout << "   --upmap-active          Act like an active balancer, keep applying changes until balanced\n";
  cout << "   --dump <format>         displays the map in plain text when <format> is 'plain', 'json' if specified format is not supported\n";
