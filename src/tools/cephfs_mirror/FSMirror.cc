...
void FSMirror::handle_acquire_directory(string_view dir_path, string_view sync_from_snapshot) {
  dout(5) << ": dir_path=" << dir_path << ", sync_from_snapshot=" << sync_from_snapshot << dendl;

  {
    ...
    for (auto &[peer, peer_replayer] : m_peer_replayers) {
      ...
      peer_replayer->add_directory(dir_path, sync_from_snapshot);
    }
  }
  if (m_perf_counters) {
