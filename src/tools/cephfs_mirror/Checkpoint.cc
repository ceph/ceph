...
int write_checkpoint_metadata(CephContext *cct, MountRef mnt,
  ...
  // UPDATE fails with -EINVAL when the key is absent on the MDS. Treat that
  // as success: the checkpoint was removed (e.g. while sync was in progress)
  ...
  r = do_op(CHECKPOINT_UPDATED_AT_KEY,
            ...
            CEPH_SNAP_MD_OP_UPDATE);
  // Same as status: -EINVAL means the checkpoint key is gone on the MDS.
  if (r == -EINVAL) {
    return 0;
  }
  if (r < 0) {
