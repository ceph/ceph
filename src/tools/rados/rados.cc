                             ...
                             std::vector<const char*> &nargs)
{
  ...
  if (strcmp(nargs[0], "lspools") == 0) {
	...
	librados::NObjectIterator i_end;
	i = io_ctx.nobjects_begin();
	i_end = io_ctx.nobjects_end();
	bool stripedOnly = (opts.find("striper") != opts.end());
	for (; i != i_end; ++i) {
	  if (stripedOnly) {
	...
	*outstream << std::endl;
    ...
    if (!pool_name || nargs.size() < 3)
      usage_exit();
    ret = do_get(io_ctx, striper, nargs[1], nargs[2], op_size,
		 opts.find("striper") != opts.end());
    if (ret < 0) {
    ...
    if (!pool_name || nargs.size() < 3)
      usage_exit();
    ret = do_put(io_ctx, striper, nargs[1], nargs[2], op_size,
		 opts.find("striper") != opts.end());
    if (ret < 0) {
