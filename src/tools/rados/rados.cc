...
// Max num operations per single ObjectReadOperation
const size_t MAX_READ_SUB_OPS = 3;
...
protected:
  int completions_init(int concurrentios) override {
    ...
    more.resize(concurrentios);
    for (size_t i = 0; i < MAX_READ_SUB_OPS; i++) {
      ret_vals[i].resize(concurrentios);
    }
    return 0;
  }
  ...
  int aio_read(const std::string& oid, int slot,
    ...
    if (read_destination & OP_DEST_OBJ) {
      op.read(offset, len, pbl, &ret_vals[0][slot]);
    }
    if (read_destination & OP_DEST_OMAP) {
      std::set<std::string> keys;
      keys.emplace("bench-omap-key-" + stringify(offset));
      op.omap_get_vals_by_keys(keys, &omaps[slot], &ret_vals[1][slot]);
    }
