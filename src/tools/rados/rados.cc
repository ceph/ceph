...
"   cache-flush <obj-name>           attempt to flush cache pool object\n"
"   cache-evict <obj-name>           attempt to evict cache pool object\n"
"   cache-flush-evict-all            attempt to flush+evict all objects\n"
"\n"
"GLOBAL OPTIONS:\n"
...
static int do_cache_flush_evict_all(IoCtx& io_ctx)
{
  int errors = 0;
  try {
	...
	io_ctx.locator_set_key(string());
      ...
      int r = do_cache_flush(io_ctx, i->first);
      if (r < 0) {
	cerr << "failed to flus " << i->first << ": "
                             ...
                             std::vector<const char*> &nargs)
{
  ...
  if (strcmp(nargs[0], "lspools") == 0) {
    ...
    if (!pool_name)
      usage_exit();
    ret = do_cache_flush_evict_all(io_ctx);
    if (ret < 0) {
