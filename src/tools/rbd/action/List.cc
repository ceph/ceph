...
int do_list(librbd::RBD &rbd, librados::IoCtx& io_ctx, bool lflag,
  ...
  if (all_ns) {
    rbd.namespace_list(io_ctx, namespaces);
  } else {
    namespaces.insert(nspace);
  }

  for (std::set<std::string>::iterator i = namespaces.begin();
       i != namespaces.end(); ++i) {
    std::string ns = *i;
    io_ctx.set_namespace(*i);
    names.clear();
    r = rbd.list(io_ctx, names);
    if (r == -ENOENT)
      r = 0;
	...
	if (!lockers.empty())
	  ...
	  f->dump_string("namespace", ns == "" ? "default" : ns);
	f->close_section();
      } else {
	tbl << *i
...
int execute(const po::variables_map &vm) {
  ...
  librbd::RBD rbd;
  r = do_list(rbd, io_ctx, vm["long"].as<bool>(), nspace, vm["all"].as<bool>(), formatter.get());
  if (r < 0) {
