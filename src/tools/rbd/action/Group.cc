...
int execute_group_snap_list(const po::variables_map &vm,
  ...
  for (const auto& snap : snaps) {
    ...
    std::string snap_type = get_group_snap_namespace_name(snap.namespace_type);
    std::string state_string = get_group_snap_state_name(snap.state);
    if (snap.namespace_type == RBD_GROUP_SNAP_NAMESPACE_TYPE_MIRROR) {
      r = rbd.group_snap_get_mirror_namespace(
