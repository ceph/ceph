...
typedef struct {
  string snap_name;
  vector<librbd::group_image_snap_info_t> images_snap;
}group_snap_info;

typedef struct {
  string image_pool_name;
  librbd::Image image;
}image_export_info;
...
static int open_export_image(librbd::RBD &rbd, librados::IoCtx& group_ioctx,
                            ...
                            map<string, image_export_info> *images,
                            vector<librbd::RBD::AioCompletion*> &on_finishes)
{
  string key;
...
static int open_group_image(librbd::RBD &rbd, librados::IoCtx& group_ioctx,
                     ...
                     const vector<group_snap_info> &group_snaps,
                     const vector<librbd::group_image_info_t> &image_list,
                     map<string, image_export_info> *images)
{
  int open_ret = 0, r = 0;
  vector<librbd::RBD::AioCompletion*> on_finishes;
...
static int group_export_header(const string &flag, const uint64_t num, int fd)
{
  bufferlist bl;
  bl.append(flag);
...
static int group_export_image_diff_fd(librbd::Image &image,
                const string &fromsnapname, const librados::snap_t endsnapid,
...
static void group_encode_string(bufferlist &bl, const string &str, const __u8 tag,
...
static int group_export_image_diff(librados::IoCtx& group_ioctx,
                      ...
                      const int64_t end_pool_id, const string &end_image_name,
                      const snapid_t end_snap_id,
                      const map<string, image_export_info> &images,
                      const bool whole_object, int fd,
                      const int export_format, const bool no_progress)
{
  int64_t r = 0;
...
static int group_export(librados::IoCtx& group_ioctx, const char *group_name,
                     int fd, map<string, image_export_info> &images,
                     const bool no_progress, librbd::ProgressContext& pc)
{
  librbd::RBD rbd;

  /* snap list */
  vector<group_snap_info> snaps;
  int r = 0;
  {
    vector<librbd::group_snap_info_t> snaps_info;
  ...
  /* image list */
  vector<librbd::group_image_info_t> image_list;
  {
...
static int do_group_export_diff(librados::IoCtx& group_ioctx,
                const char *group_name, const string &fromsnapname,
                const string &endsnapname, const bool whole_object,
                const char *path, const bool no_progress)
{
  librbd::RBD rbd;
  vector<group_snap_info> snaps;
  ...
  vector<group_snap_info> group_snap;
  if (group_end_snap) {
...
{
  vector<librbd::group_snap_info_t> snaps;
...
static int group_import_snap_info(librbd::RBD &rbd, librados::IoCtx& group_ioctx,
                        const char *group_name, int fd,
                        string &to_snap, const int import_format)
{
...
static int group_image_import_diff(librbd::RBD &rbd,
                        ...
                        const char *group_name, const string &to_snap, int fd,
                        librbd::ImageOptions& opts, const size_t sparse_size,
                        map<string, librbd::Image> *images,
                        const vector<librbd::group_image_info_t> &images_list,
                        const int import_format, bool no_progress)
{
  string src_pool_name;
  string src_image_name;
                                 ...
                                 const char *group_name)
{
  ...
  vector<librbd::group_image_info_t> images_list;
  /* get image list */
