...
void get_arguments_add(po::options_description *positional,
  ...
  add_retention_policy_options("snapshots", options);
}
...
int execute_add(const po::variables_map &vm,
  ...
  r = get_retention_policy_args(vm, &args);
  if (r < 0) {
    return r;
  }
...
int execute_status(const po::variables_map &vm,
  ...
  std::stringstream out;
  r = utils::mgr_command(rados, "rbd snapshot schedule status", args, &out,
                         &std::cerr);
...
Shell::Action add_action(
  ...
  "Add snapshot schedule period.", "", &get_arguments_add, &execute_add);
Shell::Action remove_action(
  {"snapshot", "schedule", "period", "remove"},
  {"snapshot", "schedule", "period", "rm"},
  "Remove snapshot schedule period(s).", "", &get_arguments_remove,
  &execute_remove);
...
Shell::Action status_action(
  ...
  "Show snapshot schedule status.", "", &get_arguments_status, &execute_status);
