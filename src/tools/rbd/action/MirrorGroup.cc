...
int validate_mirroring_enabled(librados::IoCtx& io_ctx,
  ...
  if (info.state == RBD_MIRROR_GROUP_DISABLED) {
    std::cerr << "rbd: mirroring disabled on the group '" << group_name
              << "'" << std::endl;
    return -EINVAL;
  } else if (info.state != RBD_MIRROR_GROUP_ENABLED) {
    std::cerr << "rbd: mirroring not enabled on the group '" << group_name
  ...
  if (info.mirror_image_mode != RBD_MIRROR_IMAGE_MODE_SNAPSHOT) {
    std::cerr << "rbd: mirroring is enabled on group '" << group_name
              << "' but image mode is not snapshot-based"
              << std::endl;
    return -EINVAL;
  }
