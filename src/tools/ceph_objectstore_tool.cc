...
int mark_pg_for_removal(ObjectStore *fs, spg_t pgid, ObjectStore::Transaction *t)
{
  ...
  if (struct_v < 8) {
    ...
    cout << "remove " << coll_t() << " " << log_oid.hobj.oid << std::endl;
    t->remove(coll_t(), log_oid);
    cout << "remove " << coll_t() << " " << biginfo_oid.oid << std::endl;
    t->remove(coll_t(), biginfo_oid);
  } else {
    // new omap key
...
int get_osdmap(ObjectStore *store, epoch_t e, OSDMap &osdmap, bufferlist& bl)
{
  bool found = store->read(
      coll_t(), OSD::get_osdmap_pobject_name(e), 0, 0, bl) >= 0;
  if (!found) {
...
int main(int argc, char **argv)
{
  ...
  r = fs->read(coll_t(), OSD_SUPERBLOCK_POBJECT, 0, 0, bl);
  if (r < 0) {
