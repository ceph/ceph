...
int JournalScanner::scan_events(EventCallback cb)
{
  ...
  uint64_t num_events_valid_cb = 0;
  uint64_t num_objects_valid_cb = 0;
  ...
  dout(4) << "Scanned objects, " << objects_missing.size() << " missing, " << (cb ? num_objects_valid_cb : objects_valid.size()) << " valid" << dendl;
  dout(4) << "Events scanned, " << ranges_invalid.size() << " gaps" << dendl;
  dout(4) << "Found " << (cb ? num_events_valid_cb : events_valid.size()) << " valid events" << dendl;
  if (!cb) {
