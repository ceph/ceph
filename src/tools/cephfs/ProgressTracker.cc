...
{
  return std::format("{}_{}", operation_name, getpid());
}
...
ProgressTracker::get_progress_status() const
{
  ...
  if (current_total > 0) {
    ...
    std::string eta_str =
	(eta_seconds > 0)
	    ? std::format("{}m{:02}s", eta_seconds / 60, eta_seconds % 60)
	    : "calculating";
    ...
    return std::format("{} objects", current_processed);
  }
}
...
ProgressTracker::update_ceph_progress_internal(
    const std::string& message, bool force_update) const
{
  ...
  try {
    ...
    std::cerr
	<< "Warning: Failed to retrieve CEPH_CONF path in update_ceph_progress_internal."
	<< std::endl;
