...
static void *proxy_manager_main(void *arg)
{
	proxy_manager_t *manager;
	proxy_worker_t *worker;
...
int32_t proxy_manager_run(proxy_manager_t *manager, proxy_manager_main_t main)
{
	...
	err = proxy_thread_create(&manager->tid, proxy_manager_main, manager);
	if (err < 0) {
		goto done_signal;
	}
			     ...
			     proxy_worker_destroy_t destroy)
{
	...
	err = proxy_thread_create(&worker->tid, proxy_worker_main, worker);
	if (err < 0) {
