...
#else
    // This function runs on all the shards due to the parent handle_conf_change,
    // so it is safe to call the synchronous version of rm_val_sync here.
    cct->_conf.rm_val_sync(*key);
#endif
  }
}
