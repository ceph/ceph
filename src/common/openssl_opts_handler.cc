...
static void log_error(const string &err)
{
  derr << "Intended OpenSSL acceleration failed.\n"
       << "set by openssl_conf = "
       << g_ceph_context->_conf->openssl_conf
...
{
  string openssl_conf = g_ceph_context->_conf->openssl_conf;
  if (!g_ceph_context->_conf->rgw_openssl_conf.empty()) {
    openssl_conf = g_ceph_context->_conf->rgw_openssl_conf;
  }
