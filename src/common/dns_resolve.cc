...
# endif
    ...
    if (len < 0) {
      ...
      ldout(cct, 1) << "resolve_all_addrs: no address records for hostname "
  ...
  if (addrs->empty()) {
    ...
    ldout(cct, 1) << "resolve_all_addrs: no A/AAAA addresses found for hostname "
                  << hostname << dendl;
    return -1;
  }

  ldout(cct, 5) << "resolve_all_addrs: hostname \"" << hostname
