...
public:
  ...
  lock()
  {
    const auto wait_start_clock =
        unlikely(lockstat_detail::LockStat::is_lockstat_enabled())
            ...
            : mono_clock::zero();
    mutex_base::lock();
    if (unlikely(wait_start_clock != mono_clock::zero())) {
      record_wait_time(mono_clock::now() - wait_start_clock, LockMode::WRITE);
    }
  }
  ...
  try_lock()
  {
    const auto wait_start_clock =
        unlikely(
            g_start_cycles.load(std::memory_order_relaxed) != mono_clock::zero())
            ? mono_clock::now()
            : mono_clock::zero();
    if (mutex_base::try_lock()) {
      if (unlikely(wait_start_clock != mono_clock::zero())) {
        record_wait_time(
            mono_clock::now() - wait_start_clock, LockMode::TRY_WRITE);
      }
      return true;
    } else {
      return false;
...
public:
  ...
  lock_shared()
  {
    ...
    mutex_base::lock();
    if (unlikely(wait_start_clock != mono_clock::zero())) {
      record_wait_time(mono_clock::now() - wait_start_clock, LockMode::READ);
    }
  }
  ...
  try_lock_shared()
  {
    ...
    if (mutex_base::try_lock()) {
      if (unlikely(wait_start_clock != mono_clock::zero())) {
        record_wait_time(
            mono_clock::now() - wait_start_clock, LockMode::TRY_READ);
      }
      return true;
    } else {
      return false;
