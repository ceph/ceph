...
auto write(std::string_view str, Iterator out)
{
...
std::string_view name(const char* begin, const char* end)
{
  ...
  std::size_t len = std::distance(begin, pos);
  return {begin, len};
}
...
std::string_view labels(const char* begin, const char* end)
{
  ...
  std::advance(pos, 1); // skip delim
  std::size_t len = std::distance(pos, end);
  return {pos, len};
}
...
void label_iterator::advance(std::optional<iterator_state>& s)
{
  ...
  std::size_t len = std::distance(s->pos, d);
  s->label.first = std::string_view{s->pos, len};
  s->pos = std::next(d);
  ...
  len = std::distance(s->pos, d);
  s->label.second = std::string_view{s->pos, len};
  s->pos = std::next(d);
}
...
bool label_iterator::operator==(const label_iterator& rhs) const
{
...
bool label_iterator::iterator_state::operator==(const iterator_state& rhs) const
{
  return pos == rhs.pos;
}
