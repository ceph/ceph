 ...
 * queue and the lock is free.  A small RAII helper unlinks the waiter if
 * wait() returns early.  unlock() clears the held flag and, if the queue
 * is non-empty, wakes the head waiter with notify_one(), so there is no
 * thundering herd.
 *
...
public:
  ...
  lock()
  {
    ...
    if (locked || !waiters.empty()) {
      linked_waiter lw{waiters};
      lw.get().cv.wait(lock, [&] {
        return &waiters.front() == &lw.get() && !locked;
      });
    }
    locked = true;
    _set_locked_by();
...
private:
  ...
  class linked_waiter {
  public:
    explicit linked_waiter(waiter_list& waiters_) : waiters(waiters_)
    {
      waiters.push_back(w);
    }
    ...
    linked_waiter(const linked_waiter&) = delete;
    linked_waiter& operator=(const linked_waiter&) = delete;
