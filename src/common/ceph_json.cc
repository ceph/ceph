...
{
  if (!ceph_json::parse(json_buffer, data, options))
   return false;
...
{
  if (0 > len || static_cast<std::size_t>(len) > json_buffer.size()) {
    return false;
  }

  if (!ceph_json::parse(std::string_view {json_buffer}.substr(0, len), data, options))
   return false;
...
bool JSONParser::parse(std::string_view json_string_view) 
{
  ...
  if (!ceph_json::parse(json_string_view, data, options))
   return false; 
...
try
{
  const auto path = file_name.string();

  // Glaze reuses an existing object value, so start each document afresh:
  data = nullptr;

  const auto ec = options.validate_trailing_whitespace
    ? glz::read_file_json<glz::opts_validate {}>(data, path, std::string {})
    : glz::read_file_json(data, path, std::string {});
...
catch (const ceph_json::detail::duplicate_object_key&)
{
  return false;
}
