...
void JSONObj::handle_value(const ceph_json::value& v)
{
  ...
  auto elements = ceph_json::array_view(v);
  if (!elements.empty()) {
...
vector<string> JSONObj::get_array_elements()
{
  ...
  const auto elements = ceph_json::array_view(data);

  vector<string> decoded_elements;
  decoded_elements.reserve(elements.size());

  std::ranges::transform(elements, std::back_inserter(decoded_elements), [](const auto& element) {
    auto json_str = glz::write_json(element);
