    ...
    use_perf(_use_perf)
{
  ...
  if (cct->_conf->throttler_perf_counter) {
    ...
    logger->set(l_throttle_max, max.load());
  }
}
...
void Throttle::_reset_max(int64_t m)
{
  ...
  if (static_cast<int64_t>(max.load()) == m)
    return;
  if (!cond.empty())
    cond.front()->SignalOne();
  if (logger)
    logger->set(l_throttle_max, m);
  max.store(static_cast<unsigned>(m));
}
...
int64_t Throttle::take(int64_t c)
{
  ...
  if (logger) {
    ...
    logger->set(l_throttle_val, count.load());
  }
  return count.load();
}
...
int64_t Throttle::put(int64_t c)
{
  ...
  if (c) {
    if (!cond.empty())
      cond.front()->SignalOne();
    assert(((int64_t)count.load()) >= c); //if count goes negative, we failed somewhere!
    count -= c;
    if (logger) {
