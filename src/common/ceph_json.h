...
// Some supported platforms do not provide <format>. Use Glaze's fallback.
...
struct parse_options {
  // Reject non-whitespace after the first complete JSON value:
  bool validate_trailing_whitespace = true;
};

namespace detail {

struct duplicate_object_key {};

// Glaze normally treats a repeated object member as another assignment.
// ceph_json rejects ambiguous objects, so flag repeats during parsing:
template <typename T>
struct duplicate_rejecting_map : glz::ordered_small_map<T> {
  using base = glz::ordered_small_map<T>;

  template <typename KeyT>
  T& operator[](KeyT&& key)
  {
    const auto previous_size = base::size();
    auto& value = base::operator[](std::forward<KeyT>(key));

    if (previous_size == base::size()) {
      throw duplicate_object_key {};
    }
...
using value = glz::generic_json<glz::num_mode::u64, detail::duplicate_rejecting_map>;

namespace detail {

constexpr glz::error_ctx duplicate_key_error {
  .ec = glz::error_code::syntax_error,
  .custom_error_message = "duplicate object key"
};

inline glz::error_ctx read_json(std::string_view input, value& out, parse_options options)
try
{
  // Glaze reuses an existing object value, so start each document afresh:
  out = nullptr;

  if (options.validate_trailing_whitespace) {
    return glz::read<glz::opts_validate {}>(out, input);
  }

  return glz::read_json(out, input);
}
catch (const duplicate_object_key&)
{
  return duplicate_key_error;
}

inline std::string_view as_string_view(ceph::buffer::list& buffers)
{
  const auto length = buffers.length();

  if (0 == length) {
    return {};
  }

  auto input = std::string_view {buffers.c_str(), length};

  // Some callers include the terminator in the buffer length; this isn't valid
  // JSON, but the implementation has historically been forgiving of a single
...
inline bool parse(std::string_view input, value& out, parse_options options = {})
{
  return !detail::read_json(input, out, options);
}

inline value parse(std::string_view input, parse_options options = {})
{
  value out;

  if (auto ec = detail::read_json(input, out, options); ec) {
...
inline bool parse(ceph::buffer::list& buffers, value& out, parse_options options = {})
{
  return parse(detail::as_string_view(buffers), out, options);
}

inline value parse(ceph::buffer::list& buffers, parse_options options = {})
{
  return parse(detail::as_string_view(buffers), options);
}
...
class JSONParser final : public JSONObj
{
  ...
  ceph_json::parse_options options;

public:
  explicit JSONParser(ceph_json::parse_options options = {})
    : options(options) {}
  ...
  bool parse(const char *buffer, int length)
  {
    if (!buffer || 0 > length) {
      return false;
    }
    ...
    return parse(std::string_view {
      buffer, static_cast<std::string_view::size_type>(length)});
  }

  bool parse(ceph::buffer::list& buffers)
  {
    return parse(ceph_json::detail::as_string_view(buffers));
  }
...
public:
  ...
  JSONDecoder(ceph::buffer::list& buffers, ceph_json::parse_options options = {})
    : parser(options)
  {
    if (!parser.parse(buffers)) {
