...
// Some supported platforms still do not provide <format>; Ceph uses libfmt,
// but we need to let the Glaze internals know not to use std::format:
...
inline void read_json_or_throw(std::string_view input, value& data_out)
{
  if (auto ec = glz::read_json(data_out, input); ec) {
...
/*
 * public:
 * Prefer this API for new code or conversions; the older
 * JSONObj/Parser/Decoder remains below for existing callsites that use its
 * object model.
 */

inline bool parse(std::string_view input, value& data_out)
{
...
inline bool parse(ceph::buffer::list& bl, value& data_out)
{
  return parse(detail::as_string_view(bl), data_out);
}
...
{
  auto* members = object(json);
  if (!members) {
    return nullptr;
  }

  auto i = members->find(name);
  if (std::end(*members) == i) {
    return nullptr;
  }
...
template <typename ValueT>
bool decode_field(std::string_view name, ValueT& out, const value& json)
{
  auto* field = find_value(json, name);
  if (!field) {
...
{
  auto* item = find_value(json, name);
  if (!item) {
    return std::nullopt;
  }
...
template <std::default_initializable ValueT>
ValueT field_or(const value& json, std::string_view name, ValueT fallback)
{
...
template <typename ValueT>
void decode_required_field(std::string_view name, ValueT& out, const value& json)
{
  if (decode_field(name, out, json)) {
    return;
  }
...
inline std::span<const value> require_array(const value& json, std::string_view name)
{
  auto* out = find_value(json, name);
  if (!out) {
    throw JSONDecoderErr(fmt::format("missing mandatory field {}", name));
...
/*
 * detail:
 * Shared implementation constraints for the legacy compatibility API and the
 * formatter helpers below.
 */
 ...
 * These types preserve the older Ceph JSON object model. Prefer the
 * ceph_json::parse()/require()/field() family above when a caller can work
 * directly from the native value tree.
 */

class JSONObjIter final {

  using child_iter_t = std::vector<std::unique_ptr<JSONObj>>::iterator;

  child_iter_t cur;
  child_iter_t last;

private:
  JSONObjIter(child_iter_t cur_, child_iter_t end_)
   : cur(cur_),
     last(end_)
  {}
...
public:
  void set(child_iter_t cur_, child_iter_t end_) {
	cur = cur_;
	last = end_;
  }
  ...
  JSONObj *operator*() { return cur->get(); }
...
public:
  using native_handle_type = ceph_json::value;
...
protected:
  using child_seq_t = std::vector<std::unique_ptr<JSONObj>>;

  std::string name; // corresponds to obj_type in XMLObj

  ceph_json::value data;

  mutable data_val val;

  mutable bool data_value_ready = false;

  child_seq_t children;

  void cache_data_value(std::string_view s, bool quoted) const;
  void ensure_data_value() const;
  void handle_value(const ceph_json::value& v);

  static std::string_view child_name(const std::unique_ptr<JSONObj>& child) noexcept {
...
protected:
  static bool parse_json(std::string_view input, ceph_json::value& data_out) {
    try {
      ceph_json::detail::read_json_or_throw(input, data_out);
    } catch (const JSONDecoderErr& e) {
      throw std::runtime_error(fmt::format("JSON error:\n{}\n", e.what()));
    }
...
public:
  ...
  // Borrow access to the underlying Glaze value for performance-sensitive code.
  native_handle_type& native_handle() noexcept {
    data_value_ready = false;
    return data;
  }

  const native_handle_type& native_handle() const noexcept { return data; }
  ...
  JSONObjIter find(std::string_view name) {
    auto fst = std::ranges::lower_bound(children, name, std::less<>{}, child_name);

    if (std::end(children) != fst && name == (*fst)->get_name()) {
      return { fst, std::ranges::upper_bound(children, name, std::less<>{}, child_name) };
    }

    return { std::end(children), std::end(children) };
  }

  JSONObjIter find_first() {
    return { std::begin(children), std::end(children) };
  }
