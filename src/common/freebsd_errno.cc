...
#define H2C_ERRNO(a,b) conv[a] = b
#define C2H_ERRNO(a,b) conv[a] = b
...
static std::array<__s32, 256> make_ceph_to_hostos_conv() {
  std::array<__s32, 256> conv{};
...
//     Linux errno    FreeBSD errno
	...
	C2H_ERRNO(78,  ENOSYS);	 // EREMCHG	  /* Will give a different messages wehn printed, but closest as possible */
...
static const std::array<__s32, 256> ceph_to_hostos_conv = make_ceph_to_hostos_conv();

// Build a table with the FreeBSD error as index
// and the Linux error as value
// Use the fact that the arry is initialised per default on all 0's
// And we do not translate for 0's, but return the original value.
static std::array<__s32, 256> make_hostos_to_ceph_conv() {
  std::array<__s32, 256> conv{};
...
static const std::array<__s32, 256> hostos_to_ceph_conv = make_hostos_to_ceph_conv();
