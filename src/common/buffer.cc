...
public:
    ...
    size_t want, const char **data)
  {
    ...
    *data = p->c_str() + p_off;
    size_t l = std::min(static_cast<size_t>(p->length() - p_off), want);
    p_off += l;
    if (p_off == p->length()) {
      ++p;
      p_off = 0;
  ...
  {
    length = std::min(length, static_cast<size_t>(get_remaining()));
    while (length > 0) {
      const char *p;
...
int buffer::list::write_fd(int fd, uint64_t offset) const
{
  ...
  while (left_pbrs) {
    ssize_t bytes = 0;
    unsigned iovlen = 0;
    uint64_t size = std::min(left_pbrs, static_cast<uint64_t>(IOV_MAX));
    left_pbrs -= size;
    while (size > 0) {
      iov[iovlen].iov_base = (void *)p->c_str();
      iov[iovlen].iov_len = p->length();
