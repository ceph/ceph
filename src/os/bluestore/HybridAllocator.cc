...
int64_t HybridAllocator::allocate(uint64_t want, uint64_t unit,
  ...
  typedef std::function<int64_t(uint64_t, uint64_t, uint64_t, int64_t,
                                PExtentVector*)>
      alloc_fn;
  alloc_fn priA = [&](uint64_t _want, uint64_t _unit, uint64_t _max_alloc_size,
                      int64_t _hint, PExtentVector* _extents) {
    return _allocate(_want, _unit, _max_alloc_size, _hint, _extents);
  };
  alloc_fn secA = [&](uint64_t _want, uint64_t _unit, uint64_t _max_alloc_size,
                      int64_t _hint, PExtentVector* _extents) {
    return bmap_alloc ? bmap_alloc->allocate(_want, _unit, _max_alloc_size,
                                             _hint, _extents)
                      : 0;
  };
...
void HybridAllocator::init_rm_free(uint64_t offset, uint64_t length) {
  ...
  _try_remove_from_tree(
      offset, length, [&](uint64_t o, uint64_t l, bool found) {
        if (!found) {
          if (bmap_alloc) {
...
void HybridAllocator::_spillover_range(uint64_t start, uint64_t end) {
  auto size = end - start;
  dout(20) << __func__ << std::hex << " " << start << "~" << size << std::dec
           << dendl;
  ceph_assert(size);
  if (!bmap_alloc) {
    dout(1) << __func__ << " constructing fallback allocator" << dendl;
    bmap_alloc = new BitmapAllocator(cct, get_capacity(), get_block_size(),
                                     get_name() + ".fallback");
  }
