...
int BlueStore::_init_alloc()
{
  ...
  dout(1) << __func__
          ...
          << std::dec << dendl;
  if (debug_init_alloc_done) debug_init_alloc_done();
  return 0;
}
...
int BlueStore::_deferred_replay()
{
  ...
  if (debug_deferred_replay_start) debug_deferred_replay_start();
  ...
  for (it->lower_bound(string()); it->valid(); it->next(), ++count) {
    ...
    if (has_some) {
      if (debug_deferred_replay_track) debug_deferred_replay_track(*deferred_txn);
