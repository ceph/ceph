...
const std::string& BlueStore::Onode::get_omap_prefix(uint8_t flags)
{
  if (onode.is_pgmeta_omap(flags)) {
    return PREFIX_PGMETA_OMAP;
  }
  if (onode.is_perpg_omap(flags)) {
    return PREFIX_PERPG_OMAP;
  }
  if (onode.is_perpool_omap(flags)) {
...
// '-' < '.' < '~'
void BlueStore::Onode::get_omap_header(uint8_t flags, std::string* out)
{
  if (!onode.is_pgmeta_omap(flags)) {
    if (onode.is_perpg_omap(flags)) {
      _key_encode_u64(c->pool(), out);
      _key_encode_u32(oid.hobj.get_bitwise_key_u32(), out);
    } else if (onode.is_perpool_omap(flags)) {
      _key_encode_u64(c->pool(), out);
    }
  }
...
void BlueStore::Onode::get_omap_key(uint8_t flags,
...
void BlueStore::Onode::get_omap_tail(uint8_t flags, std::string* out)
{
  if (!onode.is_pgmeta_omap(flags)) {
    if (onode.is_perpg_omap(flags)) {
      _key_encode_u64(c->pool(), out);
  ...
  const BlueStore::FSCK_ObjectCtx& ctx)
{
  ...
  if (repairer &&
	...
	bluestore_onode_t::FLAG_PERPG_OMAP;
      const string& new_omap_prefix = o->get_omap_prefix(new_flags);
	...
	if (header.length()) {
	  string new_head;
	  o->get_omap_header(new_flags, &new_head);
      ...
      {
	string new_tail;
	o->get_omap_tail(new_flags, &new_tail);
	bufferlist empty;
	...
	txn_cost += new_tail.length() + new_tail.length();
      ...
      // values
      string final_key;
      o->get_omap_key(new_flags, string(), &final_key);
