...
#undef dout_prefix
#define dout_prefix *_dout << "bluestore.bnode_lru(" << this << ") "

void BlueStore::BnodeHashLRU::_touch(BnodeRef o)
{
  lru_list_t::iterator p = lru.iterator_to(*o);
  lru.erase(p);
  lru.push_front(*o);
...
BlueStore::BnodeRef BlueStore::BnodeHashLRU::lookup(uint32_t bnode_id)
{
  ...
  dout(30) << __func__ << " " << bnode_id << " hit " << p->second << dendl;
  _touch(p->second);
  return p->second;
}

void BlueStore::BnodeHashLRU::clear()
{
  std::lock_guard<std::mutex> l(lock);
  dout(10) << __func__ << dendl;
  lru.clear();
...
int BlueStore::BnodeHashLRU::_trim(int max)
{
  ...
  while (num > 0) {
    ...
    dout(30) << __func__ << "  trim " << b->bnode_id << dendl;
    if (p != lru.begin()) {
      lru.erase(p--);
    } else {
      lru.erase(p);
...
void BlueStore::_dump_onode(OnodeRef o, int log_level)
{
  ...
  uint64_t pos = 0;
  for (auto p = o->onode.lextents.begin();
       p != o->onode.lextents.end();
       ++p) {
  ...
  uint32_t fadvise_flags)
{
  ...
  dout(20) << __func__
	   ...
	   << " - have " << o->onode.size
	   << " bytes in " << o->onode.lextents.size()
	   << " extents" << dendl;
  _dump_onode(o);
  ...
  if (r >= 0 && orig_offset + orig_length > o->onode.size) {
