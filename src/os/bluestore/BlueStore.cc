...
{
  assert(key.size() > 5);
  const char *p = key.data() + key.size() - 5;
				  ...
				  bool force)
{
  if (o->onode.extent_map_shards.empty()) {
    if (inline_bl.length() == 0) {
      unsigned n;
      if (encode_some(0, 0xffffffff, inline_bl, &n)) {
	return true;
      }
...
void BlueStore::ExtentMap::reshard(Onode *o)
{
  ...
  while (ep != extent_map.end()) {
    ...
    if (estimate && estimate + extent_avg > target) {
      // new shard
      if (offset == 0) {
	new_shard_info.push_back(bluestore_onode_t::shard_info());
  ...
  if (!o->onode.extent_map_shards.empty()) {
	...
	dout(30) << __func__ << "  shard 0x" << std::hex << shard_start
      ...
      if (ep->blob->id < 0 &&
	  ep->blob_spans(shard_start, shard_end - shard_start)) {
	ep->blob->id = bid++;
				       ...
				       bufferlist& bl, unsigned *pn)
{
  ...
  for (auto p = start;
    ...
    if (p->blob->id < 0 && p->blob_spans(offset, length)) {
  ...
  for (auto p = start;
    ...
    if (p->blob->id >= 0) {
      ...
      blobid = p->blob->last_encoded_id << 4;
    }
...
/*  derr << __func__ << ":";
  ...
  */
  ...
  while (!p.end()) {
    ...
    le->length = prev_len;
    if (blobid & 8) {
      le->blob = get_spanning_blob(blobid >> 4);
    } else {
      blobid >>= 4;
      if (blobid) {
...
int BlueStore::fsck()
{
  ...
  if (it) {
	...
	used_nids.insert(o->onode.nid);
      ...
      o->extent_map.fault_range(db, 0, 0xffffffff);
      // shards
  ...
  if (it) {
	...
	for (auto &r : shared_blob.ref_map.ref_map) {
	  extents.push_back(bluestore_pextent_t(r.first, r.second.length));
	}
...
#warning what about write -> clone sequence?
  }
		      ...
		      OnodeRef& newo)
{
  ...
  if (g_conf->bluestore_clone_cow) {
	...
	nb = id_to_blob[e.blob->last_encoded_id];
      } else {
	// dup the blob
	const bluestore_blob_t& blob = e.blob->get_blob();
	...
	e.blob->dup(*nb);
	if (nb->id >= 0) {
	  newo->extent_map.spanning_blob_map.insert(*nb);
	  nb->get();
	}
	...
	for (auto& dc : txc->deferred_csum) {
	  if (dc.blob == e.blob) {
	    dout(20) << __func__ << "  duplicating deferred csum for blob "
		     << *e.blob << dendl;
	    txc->add_deferred_csum(nb, dc.b_off, dc.data);
	  }
	}
      }
      // dup extent
