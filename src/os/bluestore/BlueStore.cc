...
MEMPOOL_DEFINE_OBJECT_FACTORY(BlueStore::Extent, bluestore_extent,
                              bluestore_extent);
MEMPOOL_DEFINE_OBJECT_FACTORY(BlueStore::Blob, bluestore_blob, bluestore_blob);
...
const string PREFIX_PERPG_OMAP =
...
// Label offsets where they might be replicated. It is possible on previous
// versions where these offsets were already used so labels won't exist there.
static constexpr uint64_t _1G = uint64_t(1024) * 1024 * 1024;
const vector<uint64_t> bdev_label_positions = {BDEV_FIRST_LABEL_POSITION, _1G,
                                               10 * _1G, 100 * _1G, 1000 * _1G};
...
static void get_coll_range(const coll_t& cid, int bits, ghobject_t* temp_start,
                           ghobject_t* temp_end, ghobject_t* start,
                           ghobject_t* end, bool legacy) {
  spg_t pgid;
...
static int get_key_shared_blob(const string& key, uint64_t* sbid) {
  ...
  if (key.length() < sizeof(uint64_t)) return -1;
...
static int get_key_object(const S& key, ghobject_t* oid) {
  ...
  if (key.length() == ENCODED_KEY_PREFIX_LEN) return -2;
  const char* p = key.c_str();
...
static void _get_object_key(const ghobject_t& oid, S* key) {
  size_t max_len = ENCODED_KEY_PREFIX_LEN + (oid.hobj.nspace.length() * 3 + 1) +
...
void _dump_extent_map(CephContext* cct, const BlueStore::ExtentMap& em) {
  ...
  for (auto& e : em.extent_map) {
    ...
    if (blob.has_csum()) {
      ...
      for (unsigned i = 0; i < n; ++i) v.push_back(blob.get_csum_item(i));
...
ostream& operator<<(ostream& out, const BlueStore::Buffer& b) {
  ...
  if (b.flags) out << " " << BlueStore::Buffer::get_flag_name(b.flags);
  return out << ")";
}
...
std::ostream& operator<<(std::ostream& out,
                         const BlueStore::pool_fsck_stats_t& s) {
  out << "(" << s.num_objects << " objects, " << s.shared_blobs
      << " shared blobs, " << s.omaps << " omaps, " << s.omap_key_size
      << " bytes in omap keys, " << s.omap_val_size << " bytes in omap vals, "
      << s.stored << " bytes stored, " << s.allocated << " bytes allocated"
      << ")";
  return out;
}
...
class CollectionListIterator {
 public:
  CollectionListIterator(const KeyValueDB::Iterator& it) : m_it(it) {}
  ...
  bool is_ge(const ghobject_t& oid) const { return cmp(oid) >= 0; }

  bool is_lt(const ghobject_t& oid) const { return cmp(oid) < 0; }
...
class SimpleCollectionListIterator : public CollectionListIterator {
 public:
  ...
  bool valid() const override { return m_it->valid(); }
...
class SortedCollectionListIterator : public CollectionListIterator {
 public:
  ...
  bool valid() const override { return m_chunk_iter != m_chunk.end(); }
  ...
  void lower_bound(const ghobject_t& oid) override {
    ...
    if (this->oid().shard_id != oid.shard_id ||
        ...
        this->oid().hobj.get_bitwise_key_u32() !=
            oid.hobj.get_bitwise_key_u32()) {
      return;
    }
 ...
 private:
  ...
  bool get_next_chunk() {
    ...
    while (true) {
      ...
      ceph_assert(r == 0);
      if (next.shard_id != oid.shard_id || next.hobj.pool != oid.hobj.pool ||
...
struct LruOnodeCacheShard : public BlueStore::OnodeCacheShard {
  ...
  void _rm(BlueStore::Onode* o) override {
    ...
    dout(20) << __func__ << " " << this << " "
             << " " << o->oid << " removed, num=" << num << dendl;
  }

  void maybe_unpin(BlueStore::Onode* o) override {
    OnodeCacheShard* ocs = this;
    ocs->lock.lock();
    // It is possible that during waiting split_cache moved us to different
    // OnodeCacheShard.
  ...
  void _trim_to(uint64_t new_size) override {
    ...
    while (n-- > 0 && lru.size() > 0) {
      ...
      dout(20) << __func__ << "  rm " << o->oid << " " << o->nref << " "
               << o->cached << dendl;
...
BlueStore::OnodeCacheShard* BlueStore::OnodeCacheShard::create(
    CephContext* cct, string type, PerfCounters* logger) {
...
struct LruBufferCacheShard : public BlueStore::BufferCacheShard {
  ...
  explicit LruBufferCacheShard(BlueStore* store)
      : BlueStore::BufferCacheShard(store) {}
...
BlueStore::BufferCacheShard* BlueStore::BufferCacheShard::create(
    BlueStore* store, string type, PerfCounters* logger) {
...
void BlueStore::BufferSpace::_add_buffer(BufferCacheShard* cache, Buffer* b,
  ...
  if (b->is_writing()) {
    ceph_assert(b->txc);
    // we might get already cached data for which resetting mempool is
    // inppropriate hence calling try_assign_to_mempool
    if (b->txc->add_writing(&onode, b->offset, b->length)) {
      ...
      //  So we delete it.
      ldout(cache->cct, 20)
          << __func__ << " not added to writing, releasing " << b << dendl;
      delete b;
      b = nullptr;
      add_to_map = false;
    } else {
...
void BlueStore::BufferSpace::__rm_buffer(BufferCacheShard* cache, Buffer* b) {
  ceph_assert(b);
...
int BlueStore::BufferSpace::_discard(BufferCacheShard* cache, uint32_t offset,
  ...
  while (i != buffer_map.end()) {
    Buffer* b = &*i;
    // First iteration either finds a buffer that contains the offset or the
    // next buffer after it. Subsequent iterations are either buffers inside
    // range or after the range. If we already found a buffer that doesn't
    // overlaps with the range, we can break, as it must be next to the range.
    ...
    if (b->offset < offset) {
      ...
      if (b->end() > end) {
        ...
        if (b->data.length()) {
          ...
          _add_buffer(cache,
                      new Buffer(this, b->state, b->txc, end, bl, b->flags), 0,
                      0, b);
        } else {
          _add_buffer(cache,
    ...
    if (b->data.length()) {
      ...
      _add_buffer(cache, new Buffer(this, b->state, b->txc, end, bl, b->flags),
                  0, 0, b);
    } else {
      _add_buffer(cache,
                  new Buffer(this, b->state, b->txc, end, keep, b->flags), 0, 0,
                  b);
    }
    __rm_buffer(cache, &*i);
...
void BlueStore::BufferSpace::_finish_write(BufferCacheShard* cache,
                                           TransContext* txc, uint32_t offset,
                                           uint32_t len) {
  ldout(cache->cct, 10) << __func__ << " txc " << txc << std::hex << " 0x"
                        << offset << "~" << len << std::dec << dendl;
  ...
  while (i != buffer_map.end() && offset < end && i->offset < end) {
    ...
    if (b->txc == txc && b->is_writing()) {
      ldout(cache->cct, 20) << __func__ << " finish " << *b << dendl;
...
// TransContext
bool BlueStore::TransContext::add_writing(Onode* o, uint32_t off,
                                          uint32_t len) {
...
BlueStore::OnodeRef BlueStore::OnodeSpace::lookup(const ghobject_t& oid) {
...

  {
    ...
    if (p == onode_map.end()) {
      ...
      ldout(cache->cct, 30)
          << __func__ << " " << oid << " hit " << p->second << " "
          << p->second->nref << " " << p->second->cached << dendl;
...
void BlueStore::OnodeSpace::dump(CephContext* cct) {
  for (auto& i : onode_map) {
    ldout(cct, LogLevelV) << i.first << " : " << i.second << " "
                          << i.second->nref << " " << i.second->cached << dendl;
  }
}
...
void BlueStore::Blob::copy_from(CephContext* cct, const Blob& from,
                                uint32_t min_release_size, uint32_t start,
                                uint32_t len) {
  ...
  ceph_assert(
      !bfrom.is_compressed());  // not suitable for compressed (immutable) blobs
  ...
  dout(25) << __func__ << " extent split:" << std::hex << start_aligned << "~"
           << start_roundup << "~" << end_aligned << "~" << end_roundup
           << std::dec << dendl;
  ...
  if (bto.has_csum()) {
    ...
    size_t csd_item_start =
        p2align(start, uint32_t(1 << bto.csum_chunk_order)) >>
        bto.csum_chunk_order;
    size_t csd_item_end =
        p2roundup(start + len, uint32_t(1 << bto.csum_chunk_order)) >>
        bto.csum_chunk_order;
...
void BlueStore::Blob::copy_extents(CephContext* cct, const Blob& from,
                                   uint32_t start, uint32_t pre_len,
                                   uint32_t main_len, uint32_t post_len) {
  ...
  auto check_sane_need_copy = [&](const PExtentVector& from,
                                  const PExtentVector& to, uint32_t pos,
                                  uint32_t len) -> bool {
    uint32_t pto = pos;
    auto ito = to.begin();
  ...
  dout(20) << __func__ << " 0x" << std::hex << start << " " << pre_len << "/"
           << main_len << "/" << post_len << std::dec << dendl;
...
void BlueStore::Blob::copy_extents_over_empty(CephContext* cct,
                                              const Blob& from, uint32_t start,
                                              uint32_t len) {
  dout(20) << __func__ << " to=" << *this << " from=" << from << "[0x"
           << std::hex << start << "~" << len << std::dec << "]" << dendl;
  uint32_t padding;
  ...
  auto try_append = [&](PExtentVector::iterator& it, uint64_t disk_offset,
                        uint32_t disk_len) {
    if (prev != exto.end()) {
      if (prev->is_valid()) {
  ...
  if (ito == exto.end()) {
    ...
    ceph_assert(ito->length >=
                sto + len);  // for at least len, starting with remainder sto
    padding = ito->length - (sto + len);  // add this much after copying
    ito = exto.erase(ito);                // cut a hole
    if (sto > 0) {
      ito = exto.insert(
          ito, bluestore_pextent_t(bluestore_pextent_t::INVALID_OFFSET, sto));
      prev = ito;
      ++ito;
    }
  }
...
// The important (unchecked) condition is that both Blobs belong to the same
// object. Verifies if 'other' Blob can be deleted but its content moved to
// 'this' Blob. Requirements: 1) checksums: same type and size 2) tracker: same
// au size 3) extents: must be disjointed 4) unused: ignored, will be cleared
//
// Returns:
// false - Blobs are incompatible
// true - Blobs can be merged
//
// Returned blob_width is a distance between 'other' Blob's blob_start() and
// last logical_offset that can refer to 'other' Blob extents. It is used to
// limit iteration on ExtentMap.
bool BlueStore::Blob::can_merge_blob(const Blob* other,
                                     uint32_t& blob_width) const {
  const Blob* x = other;
  const Blob* y = this;
  // checksums
  ...
  auto skip_empty = [&](const PExtentVector& list,
                        PExtentVector::const_iterator& it, uint32_t& pos) {
...
uint32_t BlueStore::Blob::merge_blob(CephContext* cct, Blob* blob_to_dissolve) {
  ...
  // drop unused, do not recalc it, unlikely those chunks could be used in
  // future
  ...
  auto move_data = [&](uint32_t pos, uint32_t len) {
    if (src_blob.has_csum()) {
      ...
      ceph_assert(dst_blob.csum_data.length() >=
                  (item_no + item_cnt) * csum_value_size);
  ...
  if (pos < dst_blob.get_logical_length()) {
    ...
    tmp_extents.emplace_back(bluestore_pextent_t::INVALID_OFFSET,
                             dst_blob.get_logical_length() - pos);
  }
...
void BlueStore::Blob::decode(bufferptr::const_iterator& p, uint64_t struct_v,
                             uint64_t* sbid, bool include_ref_map,
                             Collection* coll) {
...
BlueStore::ExtentMap::ExtentMap(Onode* o, size_t inline_shard_prealloc_size)
    : onode(o), inline_bl(inline_shard_prealloc_size) {}
...
void BlueStore::ExtentMap::scan_shared_blobs(
  ...
  for (auto ep = ep_start; ep != extent_map.end(); ++ep) {
    ...
    // ep->blob_start() allows us to include blobs that do have some empty space
    // in the beginning
...
BlueStore::Blob* BlueStore::ExtentMap::find_mergable_companion(
  ...
  dout(30) << __func__ << std::hex << " blob_start=0x" << blob_start << std::dec
           << dendl;
  Blob* result = nullptr;
  for (auto it = candidates.find(blob_start);
       it != candidates.end() && it->first == blob_start; ++it) {
    dout(30) << __func__ << " trying " << it->second << dendl;
    if (it->second->can_merge_blob(blob_to_dissolve, blob_width)) {
      dout(20) << __func__ << " merging " << blob_to_dissolve << " to "
               << it->second << dendl;
      result = it->second;
      break;
    }
...
void BlueStore::ExtentMap::reblob_extents(uint32_t blob_start,
                                          uint32_t blob_end, BlobRef from_blob,
                                          BlobRef to_blob) {
...
void BlueStore::ExtentMap::make_range_shared_maybe_merge(TransContext* txc,
                                                         OnodeRef& onoderef,
                                                         uint64_t srcoff,
                                                         uint64_t length) {
  ...
  for (auto ep = seek_lextent(srcoff); ep != extent_map.end();) {
    auto& e = *ep;
    ...
    if (!blob.is_shared()) {
      ...
      if (b) {
        ...
        dirty_range_end = std::max<uint32_t>(dirty_range_end,
                                             e.blob_start() + b_logical_length);
        uint32_t goto_logical_offset = e.logical_offset + e.length;
        reblob_extents(e.blob_start(), e.blob_start() + blob_width, e.blob, b);
...
void BlueStore::ExtentMap::dup_esb(BlueStore* b, TransContext* txc,
  ...
  for (auto ep = seek_lextent(srcoff); ep != extent_map.end(); ++ep) {
    ...
    if (e.blob->get_blob().is_compressed()) {
      ...
      cb->get_ref(c.get(), e.blob_offset + skip_front,
                  e.length - skip_front - skip_back);
    } else if (e.blob_start() >= srcoff && e.blob_end() <= end) {
      // blob already copied
    } else {
      // copy part
...
void BlueStore::ExtentMap::update(KeyValueDB::Transaction t, bool force) {
  ...
  if (onode->onode.extent_map_shards.empty()) {
    ...
    while (shard != shards.end()) {
      ceph_assert(shard->shard_info->offset >=
                  previous_shard->shard_info->offset);
      ...
      if (encode_some(shard->shard_info->offset,
                      endoff - shard->shard_info->offset, bl,
                      &shard->extents)) {
        if (force) {
      ...
      if (!force) {
        ...
        // avoid resharding the trailing shard, even if it is small
          ...
          if (shard == shards.begin()) {
            ...
            if (previous_shard->shard_info->bytes >
                next_shard->shard_info->bytes) {
              request_reshard(shard->shard_info->offset, endoff + 1);
            } else {
...
bid_t BlueStore::ExtentMap::allocate_spanning_blob_id() {
  if (spanning_blob_map.empty()) return 0;
...
void BlueStore::ExtentMap::reshard(KeyValueDB* db, KeyValueDB::Transaction t) {
  ...
  if (!shards.empty()) {
    while (shard_index_begin + 1 < shards.size() &&
           shards[shard_index_begin + 1].shard_info->offset <=
               needs_reshard_begin) {
      ++shard_index_begin;
    }
    needs_reshard_begin = shards[shard_index_begin].shard_info->offset;
    for (shard_index_end = shard_index_begin; shard_index_end < shards.size();
         ++shard_index_end) {
    ...
    dout(20) << __func__ << "   shards [" << shard_index_begin << ","
             << shard_index_end << ")"
             << " over 0x[" << std::hex << needs_reshard_begin << ","
             << needs_reshard_end << ")" << std::dec << dendl;
  } else {
    // When sharding is not applied yet, it is an error to request reshard on
    // range. The problem is that reshard() function will not touch any extent
    // outside the range. Thus initial reshard() must encompass whole object.
  ...
  for (auto extent = extent_map.lower_bound(dummy); extent != extent_map.end();
       ++extent) {
    ...
    bool would_span =
        (extent->logical_offset < max_blob_end) || extent->blob_offset;
  ...
  if (new_shard_info.empty() &&
      (shard_index_begin > 0 || shard_index_end < shards.size())) {
  ...
  if (extent_map_shards.empty()) {
    ...
    extent_map_shards.erase(extent_map_shards.begin() + shard_index_begin,
                            extent_map_shards.begin() + shard_index_end);
    shards.erase(shards.begin() + shard_index_begin,
                 shards.begin() + shard_index_end);
    extent_map_shards.insert(extent_map_shards.begin() + shard_index_begin,
                             new_shard_info.begin(), new_shard_info.end());
    shards.insert(shards.begin() + shard_index_begin, new_shard_info.size(),
                  Shard());
  ...
  if (extent_map_shards.empty()) {
    ...
    while (spanning_blob_it != spanning_blob_map.end()) {
      ...
      dout(30) << __func__ << " un-spanning " << *spanning_blob_it->second
               << dendl;
    ...
    decltype(onode->c->onode_space.cache->dumped_onodes)::value_type* oid_slot =
        nullptr;
    decltype(onode->c->onode_space.cache
                 ->dumped_onodes)::value_type* oldest_slot = nullptr;

    for (auto extent = extent_map.lower_bound(Extent(needs_reshard_begin));
         extent != extent_map.end(); ++extent) {
      if (extent->logical_offset >= needs_reshard_end) {
        break;
      }
      dout(30) << " extent " << *extent << dendl;
...
bool BlueStore::ExtentMap::encode_some(uint32_t offset, uint32_t length,
  ...
  for (auto p = start; p != extent_map.end() && p->logical_offset < end;
    ...
    if (!must_reshard) {
      ...
      p->blob->bound_encode(bound, struct_v, p->blob->get_sbid(), false);
    }
  }
  if (must_reshard) {
    return true;
...
void BlueStore::ExtentMap::ExtentDecoder::decode_extent(Extent* le,
                                                        __u8 struct_v,
                                                        bptr_c_it_t& p,
                                                        Collection* c) {
  uint64_t blobid;
...
unsigned BlueStore::ExtentMap::ExtentDecoder::decode_some(const bufferlist& bl,
                                                          Collection* c) {
  __u8 struct_v;
  uint32_t num;
...
void BlueStore::ExtentMap::ExtentDecoder::decode_spanning_blobs(bptr_c_it_t& p,
                                                                Collection* c) {
  __u8 struct_v;
  denc(struct_v, p);
  // Version 2 differs from v1 in blob's ref_map
...
void BlueStore::ExtentMap::ExtentDecoderFull::consume_blobid(
  ...
  if (spanning) {
    ...
    le->blob->get_ref(extent_map.onode->c, le->blob_offset, le->length);
  }
}
...
void BlueStore::ExtentMap::ExtentDecoderFull::add_extent(
    BlueStore::Extent* le) {
...
BlueStore::Extent* BlueStore::ExtentMap::set_lextent(
    CollectionRef& c, uint64_t logical_offset, uint64_t blob_offset,
    uint64_t length, BlobRef b, old_extent_map_t* old_extents) {
...
BlueStore::ExtentMap::debug_list_disk_layout() {
  ...
  for (auto ep = extent_map.begin(); ep != extent_map.end(); ++ep) {
    ...
    if (bblob.has_csum()) {
      ...
      // size of first chunk
      length = p2align(ep->blob_offset + csum_chunk_size, csum_chunk_size) -
               ep->blob_offset;
      ...
      length =
          p2align(ep->blob_offset + chunk_size, chunk_size) - ep->blob_offset;
      length = std::min<uint32_t>(length_left, length);
    }
    ...
    while (length_left > 0) {
      ...
      bluestore_extent_ref_map_t::debug_len_cnt l_c = {
...
std::ostream& operator<<(std::ostream& out,
                         const BlueStore::ExtentMap::debug_au_vector_t& auv) {
  out << "[";
  for (size_t i = 0; i < auv.size(); ++i) {
    ...
    if (auv[i].disk_offset != -1ULL) {
      out << auv[i].disk_offset << "~" << auv[i].disk_length << "(" << std::dec
          << int32_t(auv[i].ref_cnts) << "):" << std::hex << auv[i].chksum;
    } else {
...
// The target ranges are no longer used, but are left there because they are
// necessary for successful decompression.
//
// In compressed blobs PExtentVector and csum refer to actually occupied disk
// space. Blob's logical length is larger then occupied disk space. Mapping from
// extent_map always uses offsets of decompressed data.
...
// '-' < '.' < '~'
void BlueStore::Onode::calc_omap_header(uint8_t flags, const Onode* o,
                                        std::string* out) {
  if (!bluestore_onode_t::is_pgmeta_omap(flags)) {
    if (bluestore_onode_t::is_perpg_omap(flags)) {
...
void BlueStore::Onode::calc_omap_key(uint8_t flags, const Onode* o,
...
void BlueStore::Onode::calc_omap_tail(uint8_t flags, const Onode* o,
                                      std::string* out) {
  if (!bluestore_onode_t::is_pgmeta_omap(flags)) {
    if (bluestore_onode_t::is_perpg_omap(flags)) {
      _key_encode_u64(o->c->pool(), out);
...
void BlueStore::Onode::decode_raw(
    BlueStore::Onode* on, const bufferlist& v,
...
BlueStore::Onode* BlueStore::Onode::create_decode(CollectionRef c,
...
void BlueStore::Onode::finish_write(TransContext* txc, uint32_t offset,
                                    uint32_t length) {
  while (true) {
    ...
    if (cache != c->cache) {
      ldout(cache->cct, 20)
          << __func__ << " raced with sb cache update, was " << cache
          << ", now " << c->cache << ", retrying" << dendl;
      continue;
    }
    ldout(c->store->cct, 10) << __func__ << " txc " << txc << std::hex << " 0x"
  ...
  while (p != onode_space.onode_map.end()) {
    ...
    if (!p->second->oid.match(destbits, destpg.pgid.ps())) {
      ...
      for (auto& b : o->bc.buffer_map) {
        ...
        ldout(store->cct, 1) << __func__ << "   moving " << b << dendl;
      ...
      for (auto& b : o->extent_map.spanning_blob_map) {
        ...
        if (tb->last_encoded_id == -1) {
          ...
          ldout(store->cct, 10)
              << __func__ << " spanning blob not in map " << *tb << dendl;
          rehome_blob(tb);
          tb->last_encoded_id = 0;
        }
      }
...
BlueStore::OmapIteratorImpl::OmapIteratorImpl(PerfCounters* _logger,
...
BlueStore::BlueStore(CephContext* cct, const string& path,
  ...
  bluestore_bdev_label_require_all =
      cct->_conf.get_val<bool>("bluestore_bdev_label_require_all");
}
...
void BlueStore::_set_csum() {
  ...
  if (t > Checksummer::CSUM_NONE) csum_type = t;
...
int BlueStore::_set_cache_sizes() {
  ...
  auto _set_bin = [&](std::string conf_name, std::vector<uint64_t>* intervals) {
    ...
    std::copy(std::istream_iterator<uint64_t>(interval_stream),
  ...
  cache_kv_onode_ratio =
      cct->_conf.get_val<double>("bluestore_cache_kv_onode_ratio");
  if (cache_kv_onode_ratio < 0 || cache_kv_onode_ratio > 1.0) {
    derr << __func__ << " bluestore_cache_kv_onode_ratio ("
         << cache_kv_onode_ratio << ") must be in range [0,1.0]" << dendl;
    return -EINVAL;
  }
  ...
  if (cache_meta_ratio + cache_kv_ratio + cache_kv_onode_ratio > 1.0) {
    derr << __func__ << " bluestore_cache_meta_ratio (" << cache_meta_ratio
         ...
         << ") = " << cache_meta_ratio + cache_kv_ratio + cache_kv_onode_ratio
         << "; must be <= 1.0" << dendl;
    return -EINVAL;
  }

  cache_data_ratio = (double)1.0 - (double)cache_meta_ratio -
                     (double)cache_kv_ratio - (double)cache_kv_onode_ratio;
  ...
  dout(1) << __func__ << " cache_size " << cache_size << " meta "
          << cache_meta_ratio << " kv " << cache_kv_ratio << " kv_onode "
          << cache_kv_onode_ratio << " data " << cache_data_ratio << dendl;
  return 0;
}
...
int BlueStore::write_meta(const std::string& key, const std::string& value) {
  ...
  if (bdev_label_valid_locations.empty()) {
    _read_multi_bdev_label(cct, bdev, p, fsid, &bdev_label,
  ...
  if (!bdev_label_valid_locations.empty()) {
    ...
    int r =
        _write_bdev_label(cct, bdev, p, bdev_label, bdev_label_valid_locations);
    ceph_assert(r == 0);
  }
  return ObjectStore::write_meta(key, value);
}
...
int BlueStore::read_meta(const std::string& key, std::string* value) {
  ...
  if (!local_bdev) {
    string p = path + "/block";
    local_bdev =
        BlockDevice::create(cct, p, nullptr, nullptr, nullptr, nullptr);
  ...
  string p = path + "/block";
  if (bdev_label_valid_locations.empty()) {
    _read_multi_bdev_label(cct, local_bdev, p, fsid, &bdev_label,
                           &bdev_label_valid_locations, &bdev_label_multi,
                           &bdev_label_epoch);
  }
  if (!bdev_label_valid_locations.empty()) {
...
int BlueStore::read_meta_conf_check_env() {
  ...
  if (r == 0) {
    ...
    if (r == -ENOENT) {
      dout(1) << __func__
              << " meta.elastic_shared_blobs not set, using legacy mode"
              << dendl;
...
void BlueStore::_init_logger() {
  ...
  b.add_u64(l_bluestore_allocated, "allocated", "Sum for allocated bytes",
            "al_b", PerfCountersBuilder::PRIO_CRITICAL, unit_t(UNIT_BYTES));
  ...
  b.add_u64(l_bluestore_fragmentation, "fragmentation_micros",
            ...
            "fbss", PerfCountersBuilder::PRIO_USEFUL);
  ...
  b.add_time_avg(l_bluestore_state_done_lat, "state_done_lat",
                 "Average done state latency", "sdnl",
                 PerfCountersBuilder::PRIO_USEFUL);
  ...
  b.add_time_avg(l_bluestore_state_deferred_aio_wait_lat,
                 ...
                 "Average aio_wait state latency", "sdal",
                 PerfCountersBuilder::PRIO_USEFUL);
  ...
  b.add_time_avg(l_bluestore_read_onode_meta_lat, "read_onode_meta_lat",
                 "Average read onode metadata latency", "roml",
                 PerfCountersBuilder::PRIO_USEFUL);
  b.add_time_avg(l_bluestore_read_wait_aio_lat, "read_wait_aio_lat",
                 "Average read I/O waiting latency", "rwal",
                 PerfCountersBuilder::PRIO_USEFUL);
  ...
  b.add_u64_counter(l_bluestore_write_big_bytes, "write_big_bytes",
                    "Large aligned writes into fresh blobs (bytes)", NULL,
                    PerfCountersBuilder::PRIO_DEBUGONLY, unit_t(UNIT_BYTES));
  b.add_u64_counter(l_bluestore_write_big_blobs, "write_big_blobs",
                    "Large aligned writes into fresh blobs (blobs)");
  b.add_u64_counter(l_bluestore_write_big_deferred, "write_big_deferred",
  ...
  b.add_u64_counter(l_bluestore_write_small_bytes, "write_small_bytes",
                    ...
                    NULL, PerfCountersBuilder::PRIO_DEBUGONLY,
                    unit_t(UNIT_BYTES));
  b.add_u64_counter(l_bluestore_write_small_unused, "write_small_unused",
                    "Small writes into unused portion of existing blob");
  b.add_u64_counter(l_bluestore_write_small_pre_read, "write_small_pre_read",
  ...
  b.add_u64_counter(l_bluestore_submitted_deferred_write_bytes,
                    ...
                    "Total bytes submitted to disk by deferred writes", NULL,
                    PerfCountersBuilder::PRIO_DEBUGONLY, unit_t(UNIT_BYTES));

  b.add_u64_counter(l_bluestore_write_big_skipped_blobs,
                    "write_big_skipped_blobs",
                    "Large aligned writes into fresh blobs skipped due to zero "
                    "detection (blobs)");
  b.add_u64_counter(l_bluestore_write_big_skipped_bytes,
                    "write_big_skipped_bytes",
                    "Large aligned writes into fresh blobs skipped due to zero "
                    "detection (bytes)");
  b.add_u64_counter(l_bluestore_write_small_skipped, "write_small_skipped",
                    "Small writes into existing or sparse small blobs skipped "
                    "due to zero detection");
  b.add_u64_counter(l_bluestore_write_small_skipped_bytes,
                    "write_small_skipped_bytes",
                    "Small writes into existing or sparse small blobs skipped "
                    "due to zero detection (bytes)");
  //****************************************
  ...
  b.add_time_avg(l_bluestore_compress_lat, "compress_lat",
                 "Average compress latency", "_cpl",
                 PerfCountersBuilder::PRIO_USEFUL);
  b.add_time_avg(l_bluestore_decompress_lat, "decompress_lat",
                 "Average decompress latency", "dcpl",
                 PerfCountersBuilder::PRIO_USEFUL);
  ...
  b.add_u64_counter(l_bluestore_onode_hits, "onode_hits",
                    "Count of onode cache lookup hits", "o_ht",
                    PerfCountersBuilder::PRIO_USEFUL);
  b.add_u64_counter(l_bluestore_onode_misses, "onode_misses",
                    "Count of onode cache lookup misses", "o_ms",
                    PerfCountersBuilder::PRIO_USEFUL);
  b.add_u64_counter(l_bluestore_onode_shard_hits, "onode_shard_hits",
                    "Count of onode shard cache lookups hits");
  b.add_u64_counter(l_bluestore_onode_shard_misses, "onode_shard_misses",
                    "Count of onode shard cache lookups misses");
  b.add_u64(l_bluestore_extents, "onode_extents", "Number of extents in cache");
  b.add_u64(l_bluestore_blobs, "onode_blobs", "Number of blobs in cache");
  //****************************************
  ...
  b.add_u64_counter(
      l_bluestore_extent_compress, "extent_compress",
  ...
  b.add_u64_counter(l_bluestore_omap_rmkey_ranges_count,
                    "omap_rmkey_range_count",
  ...
  b.add_time_avg(l_bluestore_clist_lat, "clist_lat",
                 "Average collection listing latency", "cl_l",
                 PerfCountersBuilder::PRIO_USEFUL);
  b.add_time_avg(l_bluestore_remove_lat, "remove_lat",
                 "Average removal latency", "rm_l",
                 PerfCountersBuilder::PRIO_USEFUL);
  b.add_time_avg(l_bluestore_truncate_lat, "truncate_lat",
                 "Average truncate latency", "tr_l",
                 PerfCountersBuilder::PRIO_USEFUL);
  //****************************************

  // slow op count
  //****************************************
  b.add_u64_counter(l_bluestore_slow_aio_wait_count, "slow_aio_wait_count",
                    "Slow op count for aio wait", "sawc",
                    PerfCountersBuilder::PRIO_USEFUL);
  b.add_u64_counter(l_bluestore_slow_committed_kv_count,
                    "slow_committed_kv_count", "Slow op count for committed kv",
                    "sckc", PerfCountersBuilder::PRIO_USEFUL);
  b.add_u64_counter(l_bluestore_slow_read_onode_meta_count,
                    "slow_read_onode_meta_count",
                    "Slow op count for read onode meta", "sroc",
                    PerfCountersBuilder::PRIO_USEFUL);
  b.add_u64_counter(l_bluestore_slow_read_wait_aio_count,
                    "slow_read_wait_aio_count",
                    "Slow op count for read wait aio", "srwc",
                    PerfCountersBuilder::PRIO_USEFUL);
  ...
  b.add_u64_counter_histogram(
      l_bluestore_allocate_hist, "allocate_histogram", alloc_hist_x_axis_config,
  ...
  b.add_time_avg(l_bluestore_allocator_lat, "allocator_lat",
                 "Average bluestore allocator latency", "bsal",
                 PerfCountersBuilder::PRIO_USEFUL);
...
int BlueStore::get_block_device_fsid(CephContext* cct, const string& path,
  ...
  unique_ptr<BlockDevice> bdev(
      BlockDevice::create(cct, bdev_path, nullptr, nullptr, nullptr, nullptr));
...
int BlueStore::_write_bdev_label(CephContext* cct, BlockDevice* bdev,
                                 const string& path,
                                 bluestore_bdev_label_t label,
  ...
  bl.rebuild_aligned_size_and_memory(BDEV_LABEL_BLOCK_SIZE,
                                     BDEV_LABEL_BLOCK_SIZE, IOV_MAX);
  int r = 0;
  ...
  for (uint64_t position : locations) {
    if (position + BDEV_LABEL_BLOCK_SIZE <= dev_size) {
      ...
      if (r == 0) {
        ...
        derr << __func__ << " failed to write to " << path << " at location 0x"
             << std::hex << position << std::dec << ": " << cpp_strerror(r)
             << dendl;
        failed_r = r;
      }
    }
...
/*
  ...
  <0 - error
*/
int BlueStore::_read_bdev_label(CephContext* cct, BlockDevice* bdev,
  ...
  int r =
      bdev->read_random(disk_position, BDEV_LABEL_BLOCK_SIZE, buf.get(), false);
  if (r < 0) {
    derr << __func__ << " failed to read from " << path << " at 0x" << std::hex
         << disk_position << std::dec << ": " << cpp_strerror(r) << dendl;
    return -EIO;
  }
  ...
  try {
    ...
    derr << __func__ << " unable to decode label " << path.c_str()
         << " at offset " << p.get_off() << ": " << e.what() << dendl;
    return -ENOENT;
  }
  if (crc != expected_crc) {
...
int BlueStore::_read_multi_bdev_label(
    CephContext* cct, BlockDevice* bdev, const string& path, uuid_d fsid,
    bluestore_bdev_label_t* out_label,
    std::vector<uint64_t>* out_valid_positions, bool* out_is_multi,
    int64_t* out_epoch) {
  dout(10) << __func__ << dendl;
  ceph_assert(bdev);
  ...
  for (uint64_t position : bdev_label_positions) {
    ...
    if (r == 0 && (fsid.is_zero() || label.osd_uuid == fsid || any_fsid)) {
      ...
      derr << __func__ << " label at 0x" << std::hex << position << std::dec
           << " correct, but osd_uuid=" << label.osd_uuid << " need=" << fsid
           << dendl;
...
void BlueStore::_main_bdev_label_try_reserve() {
  ...
  // We can do it here only because its only on init, otherwise it would be
  // unacceptable. Here we could use some API like: alloc->allocate_at(). When
  // we create it, replace code.
  ...
  for (size_t i = 0; i < candidate_positions.size(); i++) {
    uint64_t location = candidate_positions[i];
    derr << __func__ << " bdev label location 0x" << std::hex << location
         << std::dec
...
int BlueStore::_check_or_set_bdev_label(BlockDevice* bdev, const string& path,
...
int BlueStore::_set_main_bdev_label() {
  ...
  int r = _write_bdev_label(cct, bdev, path + "/block", label,
                            bdev_label_valid_locations);
  if (r < 0) return r;
  return 0;
}
...
int BlueStore::_check_main_bdev_label() {
  ...
  if (r < 0) return r;
  if (cct->_conf->bluestore_debug_permit_any_bdev_label) {
    dout(20) << __func__ << " bdev " << block_path << " fsid "
             << bdev_label.osd_uuid << " and fsid " << fsid << " check bypassed"
             << dendl;
  } else if (bdev_label.osd_uuid != fsid) {
    derr << __func__ << " bdev " << block_path << " fsid "
         << bdev_label.osd_uuid << " does not match our fsid " << fsid << dendl;
    return -EIO;
  }
...
int BlueStore::read_bdev_label(CephContext* cct, const std::string& path,
                               bluestore_bdev_label_t* out_label,
                               std::vector<uint64_t>* out_valid_positions,
                               bool* out_is_multi, int64_t* out_epoch) {
  unique_ptr<BlockDevice> bdev(
      BlockDevice::create(cct, path, nullptr, nullptr, nullptr, nullptr));
  int r = bdev->open(path);
  if (r < 0) return r;
  uuid_d fsid;
  r = BlueStore::_read_multi_bdev_label(cct, bdev.get(), path, fsid, out_label,
                                        out_valid_positions, out_is_multi,
                                        out_epoch);
  bdev->close();
  return r;
}
int BlueStore::write_bdev_label(CephContext* cct, const std::string& path,
                                const bluestore_bdev_label_t& label,
                                uint64_t disk_position) {
  unique_ptr<BlockDevice> bdev(
      BlockDevice::create(cct, path, nullptr, nullptr, nullptr, nullptr));
  int r = bdev->open(path);
  if (r < 0) return r;
  r = BlueStore::_write_bdev_label(cct, bdev.get(), path, label,
                                   {disk_position});
  bdev->close();
  return r;
}
...
int BlueStore::_open_fm(KeyValueDB::Transaction t, bool read_only,
  ...
  // when function is called in repair mode (to_repair=true) we skip
  // db->open()/create()
  bool can_have_null_fm = !is_db_rotational() && !read_only && db_avail &&
                          cct->_conf->bluestore_allocation_from_file;

  // When allocation-info is stored in a single file we set freelist_type to
  // "null"
  if (can_have_null_fm) {
    freelist_type = "null";
  ...
  if (t) {
    ...
    if (!bdev->is_smr() && freelist_type == "zoned") {
      derr << "non-SMR device (or SMR support not built-in) but freelist_type "
              "= zoned"
           << dendl;
      return -EINVAL;
    }
  ...
  dout(1) << __func__ << " effective freelist_type = " << freelist_type
          << std::hex << ", freelist_alloc_size = 0x" << fm->get_alloc_size()
          << ", min_alloc_size = 0x" << min_alloc_size << std::dec << dendl;
  if (!fm->validate(min_alloc_size)) {
    derr << __func__ << " freelist validation failed, unable to proceed."
         << dendl;
    ceph_assert(false);
  }
...
int BlueStore::_create_alloc() {
  ...
  alloc = Allocator::create(cct, allocator_type, bdev->get_size(), alloc_size,
                            "block");
  if (!alloc) {
    lderr(cct) << __func__ << " failed to create " << allocator_type
               << " allocator" << dendl;
    return -EINVAL;
  }
...
int BlueStore::_init_alloc() {
  ...
  if (!fm->is_null_manager()) {
    // This is the original path - loading allocation map from RocksDB and
    // feeding into the allocator
    dout(5) << __func__ << "::NCB::loading allocation from FM -> alloc"
            << dendl;
    ...
    dout(5) << __func__ << "::num_entries=" << num << " free_size=" << bytes
            << " alloc_size=" << alloc->get_capacity() - bytes
            << " time=" << duration << " seconds" << dendl;
  } else {
    // This is the new path reading the allocation map from a flat bluefs file
    // and feeding them into the allocator

    if (!cct->_conf->bluestore_allocation_from_file) {
      derr << __func__
           << "::NCB::cct->_conf->bluestore_allocation_from_file is set to "
              "FALSE with an active NULL-FM"
           << dendl;
      derr << __func__
           << "::NCB::Please change the value of "
              "bluestore_allocation_from_file to TRUE in your ceph.conf file"
           << dendl;
      return -ENOTSUP;  // Operation not supported
    }
    if (restore_allocator(alloc, &num, &bytes) == 0) {
      dout(5) << __func__
              << "::NCB::restore_allocator() completed successfully alloc="
              << alloc << dendl;
    } else {
      // This must mean that we had an unplanned shutdown and didn't manage to
      // destage the allocator
      dout(0) << __func__
              << "::NCB::restore_allocator() failed! Run Full Recovery from "
                 "ONodes (might take a while) ..."
              << dendl;
      ...
      if (read_allocation_from_drive_on_startup() != 0) {
        ...
        derr << __func__
             << "::NCB::Ceph-OSD won't start, make sure your drives are "
                "connected and readable"
             << dendl;
        derr << __func__
             << "::NCB::If no HW fault is found, please report failure and "
                "consider redeploying OSD"
             << dendl;
  ...
  dout(1) << __func__ << " loaded " << byte_u_t(bytes) << " in " << num
          << " extents" << std::hex << ", allocator type " << alloc->get_type()
          << ", capacity 0x" << alloc->get_capacity() << ", block size 0x"
          << alloc->get_block_size() << ", free 0x" << alloc->get_free()
          << ", fragmentation " << alloc->get_fragmentation() << std::dec
          << dendl;
...
void BlueStore::_post_init_alloc() {
  int r = 0;
  if (fm->is_null_manager()) {
    // Now that we load the allocation map we need to invalidate the file as new
    // allocation won't be reflected Changes to the allocation map
    // (alloc/release) are not updated inline and will only be stored on
    // umount() This means that we should not use the existing file on failure
    // case (unplanned shutdown) and must resort
...
bool BlueStore::is_rotational() {
  ...
  if (r < 0) goto out;
  r = _open_fsid(false);
  if (r < 0) goto out_path;
  r = _read_fsid(&fsid);
  if (r < 0) goto out_fsid;
  r = _lock_fsid();
  if (r < 0) goto out_fsid;
  r = _open_bdev(false);
  if (r < 0) goto out_fsid;
...
bool BlueStore::test_mount_in_use() {
  ...
  if (r < 0) goto out_path;
  r = _lock_fsid();
  if (r < 0) ret = true;  // if we can't lock, it is in use
  _close_fsid();
out_path:
  _close_path();
  return ret;
...
int BlueStore::_minimal_open_bluefs(bool create) {
  ...
  if (::stat(bfn.c_str(), &st) == 0) {
    ...
    if (bluefs->bdev_support_label(BlueFS::BDEV_DB)) {
      r = _check_or_set_bdev_label(bluefs->get_block_device(BlueFS::BDEV_DB),
                                   bfn, "bluefs db", create);
      if (r < 0) {
  ...
  if (::stat(bfn.c_str(), &st) == 0) {
    ...
    if (bluefs->bdev_support_label(BlueFS::BDEV_WAL)) {
      r = _check_or_set_bdev_label(bluefs->get_block_device(BlueFS::BDEV_WAL),
                                   bfn, "bluefs wal", create);
      if (r < 0) {
        derr << __func__ << " check block device(" << bfn
             << ") label returned: " << cpp_strerror(r) << dendl;
        goto free_bluefs;
...
int BlueStore::_open_bluefs(bool create, bool read_only) {
  ...
  if (bluefs_layout.shared_bdev == BlueFS::BDEV_SLOW ||
      cct->_conf->bluestore_volume_selection_policy ==
    ...
    if (!options_annex.empty()) {
      if (!options.empty() && *options.rbegin() != ',') {
        options += ',';
      }
    ...
    r = RocksDBStore::ParseOptionsFromStringStatic(cct, options, rocks_opts,
                                                   nullptr);
    if (r < 0) {
      return r;
    }
    if (cct->_conf->bluestore_volume_selection_policy == "fit_to_fast") {
      ...
      vselector = new RocksDBBlueFSVolumeSelector(
          ...
          cct->_conf->bluestore_volume_selection_policy.find(
              "use_some_extra") == 0);
    }
  }
  if (create) {
...
int BlueStore::_open_db_and_around(bool read_only, bool to_repair) {
  dout(5) << __func__ << "::NCB::read_only=" << read_only
          << ", to_repair=" << to_repair << dendl;
  {
    string type;
  ...
  // GBH: can probably skip open_db step in REad-Only mode when operating in
  // NULL-FM mode (might need to open if failed to restore from file)
  ...
  // when function is called in repair mode (to_repair=true) we skip
  // db->open()/create() we can't change bluestore allocation so no need to
  // invlidate allocation-file
  ...
  if (!is_db_rotational() && !read_only && !to_repair &&
    ...
    commit_to_null_manager();
    need_to_destage_allocation_file = true;
    dout(10) << __func__ << "::NCB::need_to_destage_allocation_file was set"
             << dendl;
  }
...
int BlueStore::open_db_environment(KeyValueDB** pdb, bool read_only,
                                   bool to_repair) {
  _kv_only = true;
...
BlueFS* BlueStore::get_bluefs() { return bluefs; }
...
int BlueStore::_open_db(bool create, bool to_repair_db, bool read_only) {
  ...
  if (r < 0) {
    derr << __func__ << " failed to prepare db environment: " << err.str()
         << dendl;
    return -EIO;
  }
  // if reached here then BlueFS is already opened
  db_was_opened_read_only = read_only;
  dout(10) << __func__ << "::db_was_opened_read_only was set to " << read_only
           << dendl;
  if (kv_backend == "rocksdb") {
  ...
  if (create) {
    ...
    r = read_only ? db->open_read_only(err, sharding_def)
  ...
  dout(1) << __func__ << " opened " << kv_backend << " path " << kv_dir_fn
...
void BlueStore::_close_db() {
  dout(10) << __func__ << ":read_only=" << db_was_opened_read_only
           ...
           << " pool stats=" << osd_pools.size() << dendl;
  ...
  if (do_destage && is_statfs_recoverable()) {
    ...
    if (per_pool_stat_collection) {
      KeyValueDB::Iterator it =
          db->get_iterator(PREFIX_STAT, KeyValueDB::ITERATOR_NOCACHE);
      uint64_t pool_id;
      for (it->upper_bound(string()); it->valid(); it->next()) {
        ...
        if (r >= 0) {
          ...
          derr << __func__ << " wiping invalid statfs key: " << it->key()
               << dendl;
        }
      ...
      for (auto& p : osd_pools) {
        ...
        if (!p.second.is_empty()) {
          ...
          dout(10) << __func__ << " persisting: " << p.first << "->" << s
                   << dendl;
        }
      }
    } else {
  ...
  if (do_destage && fm && fm->is_null_manager()) {
    ...
    // When we reach here it is either a graceful shutdown (so can drain the
    // full discards-queue)
    //   or it was a fast shutdown, but we already moved the main discards-queue
    //   to the allocator and only need to wait for the threads local
    //   discard_processing queues to drain
    bdev->discard_drain();
    int ret = store_allocator(alloc);
    if (unlikely(ret != 0)) {
      derr << __func__
           << "::NCB::store_allocator() failed (we will need to rebuild it on "
              "startup)"
           << dendl;
    }
  }
...
int BlueStore::_open_collections() {
  ...
  dout(10) << __func__ << " collections loaded: " << load_cnt << dendl;
  return 0;
}
...
void BlueStore::_fsck_collections(int64_t* errors) {
  if (collections_had_errors) {
    dout(10) << __func__ << dendl;
    KeyValueDB::Iterator it =
        db->get_iterator(PREFIX_COLL, KeyValueDB::ITERATOR_NOCACHE);
    for (it->upper_bound(string()); it->valid(); it->next()) {
      coll_t cid;
      if (!cid.parse(it->key())) {
        derr << __func__ << " unrecognized collection " << it->key() << dendl;
...
void BlueStore::_open_statfs() {
  ...
  dout(10) << __func__ << " statfs " << std::hex << vstatfs << std::dec
           << dendl;
}
...
int BlueStore::_setup_block_symlink_or_file(string name, string epath,
  ...
  if (epath.length()) {
    ...
    if (!epath.compare(0, strlen(SPDK_PREFIX), SPDK_PREFIX)) {
      ...
      // a transport id for tcp looks like: "trype:TCP adrfam:IPv4
      // traddr:172.31.89.152 trsvcid:4420"
...
int BlueStore::mkfs() {
  ...
  r = _open_fsid(true);
  if (r < 0) goto out_path_fd;

  r = _lock_fsid();
  if (r < 0) goto out_close_fsid;
  ...
  if (min_alloc_size % block_size != 0) {
    derr << __func__ << " min_alloc_size 0x" << std::hex << min_alloc_size
         << " is less or not aligned with block_size: 0x" << block_size
         << std::dec << dendl;
    r = -EINVAL;
    goto out_close_bdev;
  }
  ...
  r = _open_db(true);
  if (r < 0) goto out_close_alloc;

  {
    KeyValueDB::Transaction t = db->get_transaction();
    r = _open_fm(t, false, true);
    if (r < 0) goto out_close_db;
    {
      bufferlist bl;
...
int BlueStore::add_new_bluefs_device(int id, const string& dev_path) {
  ...
  dout(5) << __func__ << "::NCB::calling open_db_and_around(read-only)"
          << dendl;
  ...
  if (id == BlueFS::BDEV_NEWWAL) {
    ...
    if (bluefs->bdev_support_label(BlueFS::BDEV_NEWWAL)) {
      r = _check_or_set_bdev_label(
          bluefs->get_block_device(BlueFS::BDEV_NEWWAL), p, "bluefs wal", true);
      ceph_assert(r == 0);
    }
    ...
    if (bluefs->bdev_support_label(BlueFS::BDEV_NEWDB)) {
      r = _check_or_set_bdev_label(bluefs->get_block_device(BlueFS::BDEV_NEWDB),
                                   p, "bluefs db", true);
      ceph_assert(r == 0);
    }
    bluefs_layout.shared_bdev = BlueFS::BDEV_SLOW;
...
int BlueStore::migrate_to_existing_bluefs_device(const set<int>& devs_source,
  ...
  auto close_db = make_scope_guard([&] { _close_db_and_around(); });
...
int BlueStore::migrate_to_new_bluefs_device(const set<int>& devs_source, int id,
  ...
  if (id == BlueFS::BDEV_NEWWAL) {
    ...
    if (bluefs->bdev_support_label(BlueFS::BDEV_NEWWAL)) {
      r = _check_or_set_bdev_label(
          bluefs->get_block_device(BlueFS::BDEV_NEWWAL), dev_path, "bluefs wal",
          true);
      ceph_assert(r == 0);
    }
  } else if (id == BlueFS::BDEV_NEWDB) {
...
void BlueStore::set_cache_shards(unsigned num) {
  ...
  for (unsigned i = bold; i < num; ++i) {
    buffer_cache_shards[i] = BufferCacheShard::create(
        this, cct->_conf->bluestore_cache_type, logger);
  }
}
...
int BlueStore::_mount() {
  ...
  dout(5) << __func__ << "::NCB::calling open_db_and_around(read/write)"
          << dendl;
...
int BlueStore::cold_open() { return _open_db_and_around(true); }
...
#define fsck_dendl                                                     \
  dendl;                                                               \
  if (need_skip_print) derr << "more error lines skipped..." << dendl; \
  }
...
int BlueStore::_fsck_check_extents(
    std::string_view ctx_descr, const PExtentVector& extents, bool compressed,
    mempool_dynamic_bitset& used_blocks, uint64_t granularity,
    BlueStoreRepairer* repairer, store_statfs_t& expected_statfs,
    BlueStore::pool_fsck_stats_t& pool_fsck_stat, FSCKDepth depth) {
      ...
      apply_for_bitset_range(
          ...
          [&](uint64_t pos, mempool_dynamic_bitset& bs) {
            if (bs.test(pos)) {
              ...
              if (!already) {
                derr << __func__ << "::fsck error: " << ctx_descr << ", extent "
                     << e << " or a subset is already allocated (misreferenced)"
                     << dendl;
                ++errors;
                already = true;
              }
...
void BlueStore::_fsck_check_statfs(const store_statfs_t& expected_statfs,
  ...
  store_statfs_t s;
  {
    ...
    while (op != osd_pools.end()) {
      ...
      if (it_expected == my_expected_pool_statfs.end()) {
        ...
        derr << __func__ << "::fsck error: " << std::hex << "pool "
             << op0->first << " has got no statfs to match against: " << s
             << std::dec << dendl;
        ++errors;
        if (repairer) {
          osd_pools.erase(op0);
        ...
        if (!(s == it_expected->second)) {
          derr << "fsck error: actual " << s << " != expected "
               << it_expected->second << " for pool " << std::hex << op->first
               << std::dec << dendl;
          ++errors;
          if (repairer) {
            // repair in-memory in a hope this would be flushed properly on
            // shutdown
    ...
    for (auto& p : my_expected_pool_statfs) {
      ...
      get_pool_stat_key(p.first, &key);
      derr << __func__ << "::fsck error: " << std::hex << "pool " << p.first
           << " has got no actual statfs: " << std::dec << p.second << dendl;
      ++errors;
      if (repairer) {
  ...
  if (repairer) {
    ...
    if (!(s == expected_statfs)) {
      derr << "fsck error: actual " << s << " != expected " << expected_statfs
           << dendl;
      ++errors;
    }
  }
...
void BlueStore::_fsck_foreach_shared_blob(
    std::function<bool(coll_t, ghobject_t, uint64_t, const bluestore_blob_t&)>
        cb) {
...
void BlueStore::_fsck_repair_shared_blobs(
    BlueStoreRepairer& repairer, shared_blob_2hash_tracker_t& sb_ref_counts,
  ...
  _fsck_foreach_shared_blob([&](coll_t cid, ghobject_t oid, uint64_t sbid,
    ...
    for (auto& p : b.get_extents()) {
      if (p.is_valid() &&
        ...
        dout(20) << __func__ << " broken shared blob found for col:" << cid
  ...
  while (ref_it != refs_map.end()) {
    ...
    for (cnt = 0; cnt < max_transactions && ref_it != refs_map.end();
         ref_it++) {
      auto sbid = ref_it->first;
      dout(20) << __func__ << " repaired shared_blob 0x" << std::hex << sbid
               << std::dec << ref_it->second << dendl;
  ...
  sb_info.foreach_stray([&](const sb_info_t& sbi) {
    ...
    dout(20) << __func__ << " removing stray shared_blob 0x" << std::hex << sbid
             << std::dec << dendl;
    ...
    if (blob.is_shared()) {
      if (i.first->get_sbid() > blobid_max) {
        derr << "fsck error: " << oid << " blob " << blob << " sbid "
             << i.first->get_sbid() << " > blobid_max " << blobid_max << dendl;
        ++errors;
      ...
      errors += _fsck_check_extents(
          ctx_descr, blob.get_extents(), blob.is_compressed(), *used_blocks,
          fm->get_alloc_size(), repairer, *res_statfs, *pool_fsck_stat, depth);
    } else {
      errors += _fsck_sum_extents(blob.get_extents(), blob.is_compressed(),
                                  *res_statfs, *pool_fsck_stat);
    }
  }  // for (auto& i : ref_map)

  {
    ...
    for (auto it = sbm.begin(); it != sbm.end();) {
      ...
      if (ref_map.count(it1->second) == 0) {
        if (!broken) {
          ...
          ++errors;
          derr << "fsck error:"
               << " stray spanning blob found:" << it1->first << dendl;
        }
        broken++;
        if (repairer) {
          sbm.erase(it1);
    ...
    if (broken) {
      derr << "fsck error: " << oid << " - " << broken
           << " zombie spanning blob(s) found, the first one: " << *first_broken
           << dendl;
      if (repairer) {
        repairer->fix_spanning_blobs(
            db, [&](KeyValueDB::Transaction txn) { _record_onode(o, txn); });
      }
    }
  }
 ...
 public:
  ...
  void worker(ThreadPool::WorkThread* wt) override {
    ...
    while (!_stop) {
      ...
      if (item) {
        processing++;
        TPHandle tp_handle(cct, nullptr, wq->timeout_interval.load(),
  ...
  struct FSCKWorkQueue : public ThreadPool::WorkQueue_ {
    ...
    void _void_process(void* item, TPHandle& handle) override {
      ...
      BlueStore::FSCK_ObjectCtx ctx(
          ...
          nullptr, sb_info_lock, *sb_info, *sb_ref_counts,
          batch->expected_store_statfs, batch->expected_pool_statfs,
          batch->per_pool_fsck_stats, repairer);
    ...
    void finalize(ThreadPool& tp, BlueStore::FSCK_ObjectCtx& ctx) {
      ...
      for (size_t i = 0; i < batchCount; i++) {
        ...
        if (batch.entry_count) {
          TPHandle tp_handle(store->cct, nullptr, timeout_interval.load(),
        ...
        for (auto it = batch.per_pool_fsck_stats.begin();
             it != batch.per_pool_fsck_stats.end(); it++) {
...
void BlueStore::_fsck_check_object_omap(FSCKDepth depth, OnodeRef& o,
  ...
  if (!o->onode.is_perpool_omap() && !o->onode.is_pgmeta_omap()) {
    ...
    if (per_pool_omap == OMAP_PER_PG) {
      fsck_derr(errors, MAX_FSCK_ERROR_LINES)
          << "fsck error: " << o->oid
          << " has omap that is not per-pg or pgmeta" << fsck_dendl;
      ++errors;
    } else {
      const char* w;
      int64_t num;
  ...
  if (repairer && !o->onode.is_perpg_omap() && !o->onode.is_pgmeta_omap()) {
...
void BlueStore::_fsck_check_objects(FSCKDepth depth,
  ...
  if (it) {
    ...
    std::unique_ptr<WQ> wq(new WQ("FSCKWorkQueue", (thread_count ?: 1) * 32,
                                  this, sb_info_lock, sb_info, sb_ref_counts,
                                  repairer));
        ...
        for (auto& i : referenced) {
          ...
          if (blob.has_csum()) {
            ...
            for (unsigned p = 0; p < csum_count; ++p) {
              ...
              unsigned firstbit =
                  pos / unused_chunk_size;  // [firstbit,lastbit]
              unsigned lastbit =
...
int BlueStore::_fsck(BlueStore::FSCKDepth depth, bool repair) {
  dout(5) << __func__ << (repair ? " repair" : " check")
...
int BlueStore::_fsck_on_open(BlueStore::FSCKDepth depth, bool repair) {
  uint64_t sb_hash_size = uint64_t(
      ...
      cct->_conf.get_val<double>("bluestore_fsck_shared_blob_tracker_size"));

  dout(1) << __func__ << " <<<START>>>" << (repair ? " repair" : " check")
          << (depth == FSCK_DEEP      ? " (deep)"
              : depth == FSCK_SHALLOW ? " (shallow)"
                                      : " (regular)")
          << " start sb_tracker_hash_size:" << sb_hash_size << dendl;
  int64_t errors = 0;
  int64_t warnings = 0;
  unsigned repaired = 0;
  ...
  sb_info_space_efficient_map_t sb_info;
  shared_blob_2hash_tracker_t sb_ref_counts(sb_hash_size, min_alloc_size);
  size_t sb_ref_mismatches = 0;

  /// map of oid -> (first_)offset for each zone
  std::vector<std::unordered_map<ghobject_t, uint64_t>>
      zone_refs;  // FIXME: this may be a lot of RAM!
  ...
  if (bdev->supported_bdev_label() && bdev_label_multi &&
    ...
    derr << __func__ << " fsck error: no valid block device label found"
         << dendl;
    return -EIO;
  }
  ...
  if (bdev->supported_bdev_label() && bdev_label_multi) {
    for (size_t i = 0; i < bdev_label_positions.size(); i++) {
      ...
      if (std::find(bdev_label_valid_locations.begin(),
                    bdev_label_valid_locations.end(),
                    location) == bdev_label_valid_locations.end()) {
        derr << "fsck error: bdev label at 0x" << std::hex << location
             << std::dec << " corrupted" << dendl;
        errors++;
  ...
  if (bluefs) {
    ...
    bluefs->foreach_block_extents(
        bluefs_layout.shared_bdev, [&](uint64_t start, uint32_t len) {
  ...
  if (bdev->supported_bdev_label() && bdev_label_multi) {
    ...
    // If an object happens to be using it we will get an error and a repair
    // applied. We can move away data only if it was allocated for object in
    // BlueStore, we are unable to move away BlueFS data.
    ...
    for (size_t i = 0; i < bdev_label_positions.size(); i++) {
      ...
      if (position + length <= bdev->get_size()) {
        apply_for_bitset_range(
            position, length, alloc_size, used_blocks,
  ...
  apply_for_bitset_range(
      ...
      std::max<uint64_t>(min_alloc_size, SUPER_RESERVED), alloc_size,
      used_blocks,
      [&](uint64_t pos, mempool_dynamic_bitset& bs) { bs.set(pos); });

  if (repair) {
    repairer.init_space_usage_tracker(bdev->get_size(), min_alloc_size);
  }
  ...
  if (it) {
    for (it->lower_bound(string()); it->valid(); it->next()) {
      ...
      for (auto& r : shared_blob.ref_map.ref_map) {
        sb_ref_counts.inc_range(sbid, r.first, r.second.length, -r.second.refs);
      }
    }
  ...
  // walk PREFIX_OBJ
  {
    ...
    BlueStore::FSCK_ObjectCtx ctx(
        ...
        depth == FSCK_SHALLOW ? &sb_info_lock : nullptr, sb_info, sb_ref_counts,
        expected_store_statfs, expected_pool_statfs, per_pool_fsck_stats,
  ...
  if (it) {
    ...
    for (it->lower_bound(string()); it->valid(); it->next()) {
      ...
      if (p == sb_info.end()) {
        ...
        ppfs.shared_blobs++;
        errors += _fsck_check_extents(
            ss.str(), extents, sbi.allocated_chunks < 0, used_blocks,
            fm->get_alloc_size(), repair ? &repairer : nullptr,
            *expected_statfs, ppfs, depth);
      }
    }
      ...
      for (it->lower_bound(string()); it->valid() && !bypass_rest; it->next()) {
        ...
        for (auto b : blobs) {
          ...
          for (auto e = pextents.begin(); e != pextents.end(); e++) {
            ...
            PExtentVector exts;
            dout(5) << __func__ << "::NCB::(F)alloc=" << alloc
                    << ", length=" << e->length << dendl;
            int64_t alloc_len =
            ...
            b->get_blob().map_bl(b_off_cur, bl,
          ...
          if (b->get_blob().is_shared()) {
            ...
            if (sbi.allocated_chunks < 0) {
              // NB: it's crucial to use compressed_allocated_chunks from
              // sb_info_t as we originally used that value while accumulating
              // expected_statfs
              expected_statfs->allocated -= uint64_t(-sbi.allocated_chunks)
  ...
  _fsck_check_statfs(expected_store_statfs, expected_pool_statfs, errors,
                     warnings, repair ? &repairer : nullptr);
  if (depth != FSCK_SHALLOW) {
      ...
      for (it->lower_bound(string()); it->valid(); it->next()) {
        ...
        if (used_omap_head.count(omap_head) == 0 &&
          ...
          fsck_derr(errors, MAX_FSCK_ERROR_LINES)
              ...
              << url_escape(rk.second) << fsck_dendl;
          ++errors;
      ...
      for (it->lower_bound(string()); it->valid(); it->next()) {
        ...
        if (used_omap_head.count(omap_head) == 0 &&
          ...
          fsck_derr(errors, MAX_FSCK_ERROR_LINES)
              ...
              << " key " << pretty_binary_string(it->key()) << omap_head << " "
    ...
    if (!fm->is_null_manager()) {
      ...
      sum.insert(sum.end(), bdev_labels_in_repair.begin(),
                 bdev_labels_in_repair.end());
      for (uint64_t location : sum) {
        uint64_t length = std::max<uint64_t>(BDEV_LABEL_BLOCK_SIZE, alloc_size);
        if (location != BDEV_FIRST_LABEL_POSITION) {
          apply_for_bitset_range(
              location, length, alloc_size, used_blocks,
...
out_scan:
  ...
  dout(2) << __func__ << " Per-pool stats:" << dendl;
  for (auto& p : per_pool_fsck_stats) {
    dout(2) << __func__ << " pool " << p.first << " -> " << p.second << dendl;
  }
...
int BlueStore::get_shared_blob(const string& key, bufferlist& bl) {
...
void BlueStore::inject_stray_shared_blob_key(uint64_t sbid) {
  ...
  dout(20) << __func__ << " sbid " << sbid << " takes " << bl.length()
...
void BlueStore::inject_legacy_omap(coll_t cid, ghobject_t oid) {
  ...
  o->onode.clear_flag(bluestore_onode_t::FLAG_PERPG_OMAP |
                      bluestore_onode_t::FLAG_PERPOOL_OMAP |
...
void BlueStore::inject_bluefs_file(std::string_view dir, std::string_view name,
                                   size_t new_size) {
  ceph_assert(bluefs);
...
void BlueStore::collect_metadata(map<string, string>* pm) {
  ...
  (*pm)["bluestore_allocation_from_file"] =
      stringify(fm && fm->is_null_manager());
}
...
void BlueStore::prepare_for_fast_shutdown() { m_fast_shutdown = true; }
...
int BlueStore::get_devices(set<string>* ls) {
  ...
  auto close_path = make_scope_guard([&] { _close_path(); });
  if (int r = _open_fsid(false); r < 0) {
    return r;
  }
  auto close_fsid = make_scope_guard([&] { _close_fsid(); });
  ...
  auto close_bdev = make_scope_guard([&] { _close_bdev(); });
...
void BlueStore::_get_statfs_overall(struct store_statfs_t* buf) {
  ...
  auto prefix = per_pool_omap == OMAP_BULK       ? PREFIX_OMAP
                : per_pool_omap == OMAP_PER_POOL ? PREFIX_PERPOOL_OMAP
                                                 : PREFIX_PERPG_OMAP;
  buf->omap_allocated = db->estimate_prefix_size(prefix, string());
  ...
  if (bluefs) {
    ...
    buf->internal_metadata = bluefs->get_used() - buf->omap_allocated;
  }
  ...
  if (rc == 0) {
    ...
    buf->allocated =
        ebd_state.get_physical_total() - ebd_state.get_physical_avail();
    ;
  } else {
...
int BlueStore::pool_statfs(uint64_t pool_id, struct store_statfs_t* buf,
  ...
  *out_per_pool_omap = per_pool_omap != OMAP_BULK;
  ...
  if (*out_per_pool_omap && db) {
    auto prefix = per_pool_omap == OMAP_PER_POOL ? PREFIX_PERPOOL_OMAP
                                                 : PREFIX_PERPG_OMAP;
    ...
    if (per_pool_omap != OMAP_PER_POOL) {
      if (cct->_conf->bluestore_warn_on_no_per_pool_omap) {
        per_pool =
            ...
            "suggest to run store repair to benefit from per-pool omap usage "
            "statistics";
      }
    }
  }
...
BlueStore::CollectionRef BlueStore::_get_collection(const coll_t& cid) {
  ...
  if (cp == coll_map.end()) return CollectionRef();
  return cp->second;
}

BlueStore::CollectionRef BlueStore::_get_collection_by_oid(
    const ghobject_t& oid) {
  std::shared_lock l(coll_lock);
  ...
  for (auto& i : coll_map) {
    spg_t spgid;
    if (i.first.is_pg(&spgid) && i.second->contains(oid)) {
      return i.second;
    }
  }
...
int BlueStore::read(CollectionHandle& c_, const ghobject_t& oid,
  ...
  int r;
  {
    ...
    if (offset == length && offset == 0) length = o->onode.size;
...
void BlueStore::_read_cache(OnodeRef& o, uint64_t offset, size_t length,
  ...
  while (left > 0 && lp != o->extent_map.extent_map.end()) {
    ...
    o->bc.read(o->c->cache, pos, b_len, cache_res, cache_interval,
               read_cache_policy);
    dout(20) << __func__ << "  blob " << *bptr << std::hex << " need 0x" << pos
             << "~" << b_len << " cache has 0x" << cache_interval << std::dec
             << dendl;
    ...
    while (b_len > 0) {
      unsigned l;
      if (pc != cache_res.end() && pc->first == pos) {
        l = pc->second.length();
      ...
      for (auto& req : r2r) {
        ...
        for (const auto& r : req.regs) {
          if (buffered) {
            ...
            o->bc.did_read(o->c->cache, r.logical_offset,
...
int BlueStore::_do_read(Collection* c, OnodeRef& o, uint64_t offset,
  ...
  log_latency_fn(
      __func__, l_bluestore_read_wait_aio_lat, mono_clock::now() - start,
      cct->_conf->bluestore_log_op_age,
  ...
  r = _generate_read_result_bl(o, offset, length, ready_regions,
                               ...
                               buffered && !ioc.skip_cache(), &csum_error, bl);
  if (csum_error) {
...
int BlueStore::_fiemap(CollectionHandle& c_, const ghobject_t& oid,
  ...
  if (!c->exists) return -ENOENT;
  {
    ...
    if (offset >= o->onode.size) goto out;
...
int BlueStore::collection_list_legacy(CollectionHandle& c_,
...
int BlueStore::_collection_list(Collection* c, const ghobject_t& start,
  ...
  auto log_latency = make_scope_guard(
      [&, start_time = mono_clock::now(), func_name = __func__] {
        log_latency_fn(
            func_name, l_bluestore_clist_lat, mono_clock::now() - start_time,
  ...
  get_coll_range(c->cid, c->cnode.bits, &coll_range_temp_start,
                 ...
                 legacy);
  dout(20) << __func__ << " range " << coll_range_temp_start << " to "
           << coll_range_temp_end << " and " << coll_range_start << " to "
           << coll_range_end << " start " << start << dendl;

  // if specified start is not specifically in the pg normal range, we should
  // start with temp iter
  if ((start == ghobject_t() || start.hobj == hobject_t() ||
       start == c->cid.get_min_hobj() || start.hobj.is_temp()) &&
      coll_range_temp_start != coll_range_temp_end) {
    ranges.push_back(std::tuple(std::move(coll_range_temp_start),
                                std::move(coll_range_temp_end)));
  }
  // if end param is in temp section, then we do not need to proceed to the
  // normal section
  if (!end.hobj.is_temp()) {
    ranges.push_back(
        std::tuple(std::move(coll_range_start), std::move(coll_range_end)));
  }

  for (const auto& [cur_range_start, cur_range_end] : ranges) {
    dout(30) << __func__ << " cur_range " << cur_range_start << " to "
             << cur_range_end << dendl;
    ...
    dout(30) << __func__ << " kv_low_key: " << kv_low_key
             << " kv_high_key: " << kv_high_key << dendl;
    const KeyValueDB::IteratorBounds bounds = KeyValueDB::IteratorBounds{
        std::move(kv_low_key), std::move(kv_high_key)};
    if (legacy) {
...
int BlueStore::_onode_omap_get(
  ...
  o->flush();
  {
    ...
    o->get_omap_tail(&tail);
    KeyValueDB::Iterator it =
        db->get_iterator(prefix, 0, KeyValueDB::IteratorBounds{head, tail});
    it->lower_bound(head);
    while (it->valid()) {
      if (it->key() == head) {
        dout(30) << __func__ << "  got header" << dendl;
...
out:
  c->store->log_latency(__func__, l_bluestore_omap_get_keys_lat,
                        mono_clock::now() - start1,
                        c->store->cct->_conf->bluestore_log_omap_iterator_age);
...
out:
  c->store->log_latency(__func__, l_bluestore_omap_get_values_lat,
                        mono_clock::now() - start1,
                        c->store->cct->_conf->bluestore_log_omap_iterator_age);
...
ObjectMap::ObjectMapIterator BlueStore::get_omap_iterator(
  ...
  KeyValueDB::Iterator it =
      db->get_iterator(o->get_omap_prefix(), 0, std::move(bounds));
...
void BlueStore::_txc_update_store_statfs(TransContext* txc) {
  if (txc->statfs_delta.is_empty()) return;
  ...
  while (true) {
    ...
    switch (txc->get_state()) {
      ...
      case TransContext::STATE_IO_DONE:
        ceph_assert(
            ceph_mutex_is_locked(txc->osr->qlock));  // see _txc_finish_io
        if (txc->had_ios) {
        ...
        if (cct->_conf->bluestore_sync_submit_transaction) {
          if (txc->last_nid >= nid_max || txc->last_blobid >= blobid_max) {
            ...
            dout(20) << __func__ << " prior txc(s) with unstable ios "
              ...
              cct->_conf->bluestore_debug_randomize_serial_transaction &&
              rand() % cct->_conf
                           ->bluestore_debug_randomize_serial_transaction ==
                  0) {
            dout(20) << __func__
                     << " DEBUG randomly forcing submit via kv thread" << dendl;
          } else {
  ...
  ceph_assert(txc->get_state() == TransContext::STATE_KV_QUEUED);
  {
    ...
    if (txc->tracing) {
      tracepoint(bluestore, transaction_kv_submit_latency,
                 txc->osr->get_sequencer_id(), (uint64_t)txc,
...
void BlueStore::_txc_committed_kv(TransContext* txc) {
  ...
  log_latency_fn(
      __func__, l_bluestore_commit_lat, mono_clock::now() - txc->start,
      cct->_conf->bluestore_log_op_age,
...
void BlueStore::_kv_sync_thread() {
  ...
  while (true) {
    ...
    if (period && elapsed >= observation_period) {
      dout(5) << __func__ << " utilization: idle " << twait << " of " << elapsed
    ...
    if (kv_queue.empty() &&
      ...
      int r =
          db_was_opened_read_only || cct->_conf->bluestore_debug_omit_kv_commit
              ? 0
...
void BlueStore::_deferred_queue(TransContext* txc) {
...

  {
    ...
    // condition "tmp->txcs.size() == 1" mean deferred_pending was originally
    // empty. So we should add osr into deferred_queue.
...
bool BlueStore::_eliminate_outdated_deferred(
    bluestore_deferred_transaction_t* deferred_txn,
    interval_set<uint64_t>& bluefs_extents) {
  bool has_some = false;
  dout(30) << __func__ << " bluefs_extents: " << std::hex << bluefs_extents
           << std::dec << dendl;
  auto it = deferred_txn->ops.begin();
  while (it != deferred_txn->ops.end()) {
    // We process a pair of _data_/_extents_ (here: it->data/it->extents)
    // by eliminating _extents_ that belong to bluefs, removing relevant parts
    // of _data_ example:
    // +------------+---------------+---------------+---------------+
    ...
    for (auto& e : it->extents) {
      ...
      while (mi != bluefs_extents.end() &&
             mi.get_start() < e.offset + e.length) {
        // The interval_set does not like (asserts) when we erase interval that
        // does not exist. Hence we do we implement (region-mi) by
        // ((region+mi)-mi).
      ...
      // we trim coresponding parts from it->data (actally constructing new_data
      // / new_extents)
      for (auto ki = region.begin(); ki != region.end(); ki++) {
        ceph::buffer::list chunk;
        // A chunk from it->data; data_offset is a an offset where 'e' was
        // located; 'ki.get_start() - e.offset' is an offset of ki inside 'e'.
        chunk.substr_of(it->data, data_offset + (ki.get_start() - e.offset),
                        ki.get_len());
        new_data.claim_append(chunk);
        new_extents.emplace_back(
            bluestore_pextent_t(ki.get_start(), ki.get_len()));
      }
      data_offset += e.length;
    }
    dout(30) << __func__ << " output extents: " << new_extents << dendl;
    if (it->data.length() != new_data.length()) {
      dout(10) << __func__ << " trimmed deferred extents: " << it->extents
               << "->" << new_extents << dendl;
    }
...
void BlueStore::_txc_add_transaction(TransContext* txc, Transaction* t) {
  ...
  for (int pos = 0; i.have_op(); ++pos) {
    ...
    // no coll or obj
    if (op->op == Transaction::OP_NOP) continue;
...
void BlueStore::_pad_zeros(bufferlist* bl, uint64_t* offset,
  ...
  *_dout << dendl;
  ...
  bl->hexdump(*_dout);
  *_dout << dendl;
  if (pad_count) logger->inc(l_bluestore_write_pad_bytes, pad_count);
  ceph_assert(bl->length() == length);
}

void BlueStore::_do_write_small(TransContext* txc, CollectionRef& c,
  ...
  if (!cct->_conf->bluestore_zero_block_detection || !bl.is_zero()) {
    ...
    wctx->write(offset, b, alloc_len, b_off0, bl, b_off, length,
                min_alloc_size !=
                    block_size,  // use 'unused' bitmap when alloc granularity
                                 // doesn't match disk one only
                true);
  } else {  // if (bl.is_zero())
    dout(20) << __func__ << " skip small zero block " << std::hex << " (0x"
             << b_off0 << "~" << bl.length() << ")"
...
bool BlueStore::BigDeferredWriteContext::can_defer(
    BlueStore::extent_map_t::iterator ep, uint64_t prefer_deferred_size,
    uint64_t block_size, uint64_t offset, uint64_t l) {
  bool res = false;
...
void BlueStore::_do_write_big_apply_deferred(
    ...
    BlueStore::BigDeferredWriteContext& dctx, bufferlist::iterator& blp,
    WriteContext* wctx) {
  bufferlist bl;
  ...
  if (dctx.head_read) {
    int r =
        _do_read(c.get(), o, dctx.off - dctx.head_read, dctx.head_read, bl, 0);
  ...
  if (dctx.tail_read) {
    bufferlist tail_bl;
    int r =
        _do_read(c.get(), o, dctx.off + dctx.used, dctx.tail_read, tail_bl, 0);
  ...
  Extent* le =
      o->extent_map.set_lextent(c, dctx.off, dctx.off - dctx.blob_start,
                                dctx.used, b0, &wctx->old_extents);
...
void BlueStore::_do_write_big(TransContext* txc, CollectionRef& c, OnodeRef& o,
  ...
  while (length > 0) {
    ...
    if (!wctx->compress) {
      ...
      dout(20) << __func__ << " may be defer: 0x" << std::hex << offset << "~"
               << l << std::dec << dendl;
      ...
      if (prefer_deferred_size_snapshot &&
        ...
        // existing blobs than having 3 blobs: 0x0~10000, 0x10000~20000,
        // 0x30000~10000
        ...
        bool will_defer =
            ep != end ? head_info.can_defer(ep, prefer_deferred_size_snapshot,
                                            block_size, offset, l)
                      : false;
        ...
        if (will_defer && remaining) {
          ...
          if (remaining <= prefer_deferred_size_snapshot) {
            ...
            will_defer = ep_next == end
                             ? false
                             : tail_info.can_defer(
                                   ep_next, prefer_deferred_size_snapshot,
                                   block_size, offset_next, remaining);
            will_defer = will_defer && remaining == tail_info.used;
          }
        }
        if (will_defer) {
          dout(20) << __func__ << " " << *(head_info.blob_ref)
                   << " deferring big " << std::hex << " (0x" << head_info.b_off
                   << "~" << head_info.blob_aligned_len() << ")" << std::dec
                   << " write via deferred" << dendl;
          if (remaining) {
            dout(20) << __func__ << " " << *(tail_info.blob_ref)
                     << " deferring big " << std::hex << " (0x"
                     << tail_info.b_off << "~" << tail_info.blob_aligned_len()
        ...
        if (will_defer) {
          ...
          dout(20) << __func__ << " defer big: 0x" << std::hex << offset << "~"
                   << l << std::dec << dendl;
          offset += l;
          length -= l;
      ...
      do {
        any_change = false;
        if (ep != end && ep->logical_offset < offset + max_bsize) {
          dout(20) << __func__ << " considering " << *ep << " bstart 0x"
                   << std::hex << ep->blob_start() << std::dec << dendl;
        ...
        if (prev_ep != end && prev_ep->logical_offset >= min_off) {
          dout(20) << __func__ << " considering rev " << *prev_ep
                   << " bstart 0x" << std::hex << prev_ep->blob_start()
                   << std::dec << dendl;
    ...
    if (!cct->_conf->bluestore_zero_block_detection || !t.is_zero()) {
      ...
      dout(20) << __func__ << " schedule write big: 0x" << std::hex << offset
               << "~" << l << std::dec << (new_blob ? " new " : " reuse ") << *b
               << dendl;

      logger->inc(l_bluestore_write_big_blobs);
    } else {  // if (!t.is_zero())
      dout(20) << __func__ << " skip big zero block " << std::hex << " (0x"
               << b_off << "~" << t.length() << ")"
...
int BlueStore::_do_alloc_write(TransContext* txc, CollectionRef coll,
  ...
  log_latency("allocator@_do_alloc_write", l_bluestore_allocator_lat,
              mono_clock::now() - start, cct->_conf->bluestore_log_op_age);
  ...
  for (auto& wi : wctx->writes) {
    ...
    if (wi.compressed) {
      ...
      if (l->length() != wi.blob_length) {
        ...
        csum_order =
            std::min<unsigned>(wctx->csum_order, std::countr_zero(l->length()));
      }
    ...
    if (!g_conf()->bluestore_debug_omit_block_device_write) {
      if (data_size < prefer_deferred_size_snapshot) {
        dout(20) << __func__ << " deferring 0x" << std::hex << l->length()
                 << " write via deferred, pds=0x"
                 << prefer_deferred_size_snapshot << std::dec << dendl;
  ...
  while (oep != wctx->old_extents.end()) {
    ...
    if (!r.empty()) {
      ...
      if (blob.is_shared()) {
        ...
        for (auto e : r) {
          b->get_shared_blob()->put_ref(e.offset, e.length, &final,
                                        unshare_ptr);
        }
        if (unshare) {
...
int BlueStore::_do_write(TransContext* txc, CollectionRef& c, OnodeRef& o,
  ...
  dout(20) << __func__ << " " << o->oid << " 0x" << std::hex << offset << "~"
           ...
           << o->onode.size << ")"
           << " bytes" << std::hex << " fadvise_flags 0x" << fadvise_flags
...
int BlueStore::_truncate(TransContext* txc, CollectionRef& c, OnodeRef& o,
  ...
  log_latency_fn(
      __func__, l_bluestore_truncate_lat, mono_clock::now() - start_time,
      cct->_conf->bluestore_log_op_age, [&](const ceph::timespan& lat) {
        ostringstream ostr;
        ostr << ", lat = " << timespan_str(lat) << " cid =" << c->cid
             << " oid =" << o->oid;
        return ostr.str();
...
int BlueStore::_remove(TransContext* txc, CollectionRef& c, OnodeRef& o) {
  ...
  log_latency_fn(
      __func__, l_bluestore_remove_lat, mono_clock::now() - start_time,
      cct->_conf->bluestore_log_op_age, [&](const ceph::timespan& lat) {
        ostringstream ostr;
        ostr << ", lat = " << timespan_str(lat) << " cid =" << c->cid
             << " oid =" << o->oid;
...
int BlueStore::_rmattr(TransContext* txc, CollectionRef& c, OnodeRef& o,
  ...
  if (it == o->onode.attrs.end()) goto out;
...
int BlueStore::_rmattrs(TransContext* txc, CollectionRef& c, OnodeRef& o) {
  ...
  if (o->onode.attrs.empty()) goto out;
...
size_t BlueStore::_trim_slow_op_event_queue(mono_clock::time_point cur_time) {
  ...
  auto warn_duration =
      std::chrono::seconds(cct->_conf->bluestore_slow_ops_warn_lifetime);
  while (!slow_op_event_queue.empty() &&
         ((slow_op_event_queue.front() < cur_time - warn_duration) ||
          (slow_op_event_queue.size() >
           cct->_conf->bluestore_slow_ops_warn_threshold))) {
...
void BlueStore::log_latency_fn(
    ...
    std::function<string(const ceph::timespan& lat)> fn, int idx2) {
  logger->tinc(idx, l);
  if (lat_threshold > 0.0 && l >= make_timespan(lat_threshold)) {
    dout(0) << __func__ << " slow operation observed for " << name
...
void BlueStore::BlueStoreThrottle::emit_initial_tracepoint(
  ...
  if (should_trace(&started, &completed)) {
    ...
    tracepoint(
        ...
        (uint64_t)&txc, throttle_bytes.get_current(),
    ...
    tracepoint(bluestore, transaction_initial_state_rocksdb,
               txc.osr->get_sequencer_id(), (uint64_t)&txc, rocksdb_base_level,
               rocksdb_estimate_pending_compaction_bytes,
               rocksdb_cur_size_all_mem_tables, rocksdb_compaction_pending,
               rocksdb_mem_table_flush_pending, rocksdb_num_running_compactions,
               rocksdb_num_running_flushes, rocksdb_actual_delayed_write_rate);
...
mono_clock::duration BlueStore::BlueStoreThrottle::log_state_latency(
  ...
  if (txc.tracing && state >= l_bluestore_state_prepare_lat &&
    ...
    tracepoint(bluestore, transaction_state_duration,
               txc.osr->get_sequencer_id(), (uint64_t)&txc, state,
...
void BlueStore::generate_db_histogram(Formatter* f) {
  ...
  while (iter->valid()) {
    ...
    if (key.first == PREFIX_SUPER) {
      ...
      hist.update_hist_entry(hist.key_hist, PREFIX_PERPOOL_OMAP, key_size,
                             value_size);
      num_omap++;
    } else if (key.first == PREFIX_PERPG_OMAP) {
      hist.update_hist_entry(hist.key_hist, PREFIX_PERPG_OMAP, key_size,
                             value_size);
      num_omap++;
...
void BlueStore::_log_alerts(osd_alert_list_t& alerts) {
  ...
  if (used > 0) {
    ...
    ostringstream ss;
    ss << "spilled over " << byte_u_t(used) << " metadata from 'db' device ("
       << byte_u_t(db_used) << " used of " << byte_u_t(db_total)
       << ") to slow device";
  ...
  if (!spurious_read_errors_alert.empty() &&
    ...
    alerts.emplace("BLUESTORE_SPURIOUS_READ_ERRORS",
  ...
  if (!no_per_pg_omap_alert.empty()) {
    alerts.emplace("BLUESTORE_NO_PER_PG_OMAP", no_per_pg_omap_alert);
  }
...
bool BlueStoreRepairer::fix_shared_blob(KeyValueDB::Transaction txn,
                                        uint64_t sbid,
...
bool BlueStoreRepairer::fix_spanning_blobs(
    KeyValueDB* db, std::function<void(KeyValueDB::Transaction)> f) {
  std::lock_guard l(lock);
  if (!fix_onode_txn) {
  ...
  switch (hint) {
    case LEVEL_SLOW:
      ...
      if (db_avail4slow > 0) {
        ...
        max_db_use +=
            per_level_per_dev_max.at(BlueFS::BDEV_DB, LEVEL_LOG - LEVEL_FIRST);
        max_db_use +=
        ...
        uint64_t avail = min(db_avail4slow,
...
void RocksDBBlueFSVolumeSelector::dump(ostream& sout) {
  ...
  sout << ">>Settings<<"
       ...
       << ", l_multi=" << byte_u_t(level_multiplier) << std::endl;
...
BlueFSVolumeSelector* RocksDBBlueFSVolumeSelector::clone_empty() const {
  RocksDBBlueFSVolumeSelector* ns =
      new RocksDBBlueFSVolumeSelector(0, 0, 0, 0, 0, 0, 0, 0, false);
  return ns;
}

bool RocksDBBlueFSVolumeSelector::compare(BlueFSVolumeSelector* other) {
  RocksDBBlueFSVolumeSelector* o =
      dynamic_cast<RocksDBBlueFSVolumeSelector*>(other);
  ceph_assert(o);
  bool equal = true;
  for (size_t x = 0; x < BlueFS::MAX_BDEV + 1; x++) {
    for (size_t y = 0; y < LEVEL_MAX - LEVEL_FIRST + 1; y++) {
      equal &= (per_level_per_dev_usage.at(x, y) ==
                o->per_level_per_dev_usage.at(x, y));
    }
  }
...
//================================================================================================================
// BlueStore is committing all allocation information (alloc/release) into
// RocksDB before the client Write is performed. This cause a delay in write
// path and add significant load to the CPU/Memory/Disk. The reason for the
// RocksDB updates is that it allows Ceph to survive any failure without losing
// the allocation state.
//
// We changed the code skiping RocksDB updates on allocation time and instead
// performing a full desatge of the allocator object with all the OSD allocation
// state in a single step during umount(). This change leads to a 25% increase
// in IOPS and reduced latency in small random-write workload, but exposes the
// system to losing allocation info in failure cases where we don't call umount.
// We add code to perform a full allocation-map rebuild from information stored
// inside the ONode which is used in failure cases. When we perform a graceful
// shutdown there is no need for recovery and we simply read the allocation-map
// from a flat file where we store the allocation-map during umount().
//================================================================================================================
...
struct allocator_image_header {
  ...
  allocator_image_header(utime_t timestamp, uint32_t format_version,
                         uint32_t serial) {
  ...
  friend std::ostream& operator<<(std::ostream& out,
                                  const allocator_image_header& header) {
    out << "format_version  = " << header.format_version << std::endl;
    out << "valid_signature = " << header.valid_signature << "/"
        << ALLOCATOR_IMAGE_VALID_SIGNATURE << std::endl;
  ...
  int verify(CephContext* cct, const std::string& path) {
    if (valid_signature == ALLOCATOR_IMAGE_VALID_SIGNATURE) {
      ...
      derr << "Illegal Header - signature=" << valid_signature << "("
           << ALLOCATOR_IMAGE_VALID_SIGNATURE << ")" << dendl;
      return -1;
    }
  }
};
...
struct allocator_image_trailer {
  ...
  allocator_image_trailer(utime_t timestamp, uint32_t format_version,
                          uint32_t serial, uint64_t entries_count,
  ...
  friend std::ostream& operator<<(std::ostream& out,
                                  const allocator_image_trailer& trailer) {
    if (trailer.null_extent.offset || trailer.null_extent.length) {
      out << "trailer.null_extent.offset = " << trailer.null_extent.offset
          << std::endl;
      out << "trailer.null_extent.length = " << trailer.null_extent.length
          << std::endl;
    }
    out << "format_version  = " << trailer.format_version << std::endl;
    out << "valid_signature = " << trailer.valid_signature << "/"
        << ALLOCATOR_IMAGE_VALID_SIGNATURE << std::endl;
  ...
  int verify(CephContext* cct, const std::string& path,
             const allocator_image_header* p_header, uint64_t entries_count,
             uint64_t allocation_size) {
    if (valid_signature == ALLOCATOR_IMAGE_VALID_SIGNATURE) {
      // trailer must starts with null extents (both fields set to zero) [no
      // need to convert formats for zero)
      if (null_extent.offset || null_extent.length) {
        derr << "illegal trailer - null_extent = [" << null_extent.offset << ","
             << null_extent.length << "]" << dendl;
        return -1;
      }

      if (serial != p_header->serial) {
        derr << "Illegal trailer: header->serial(" << p_header->serial
             << ") != trailer->serial(" << serial << ")" << dendl;
        return -1;
      }

      if (format_version != p_header->format_version) {
        derr << "Illegal trailer: header->format_version("
             << p_header->format_version << ") != trailer->format_version("
      ...
      if (this->entries_count != entries_count) {
        derr << "Illegal trailer: entries_count(" << entries_count
             << ") != trailer->entries_count(" << this->entries_count << ")"
             << dendl;
        return -1;
      }

      if (this->allocation_size != allocation_size) {
        derr << "Illegal trailer: allocation_size(" << allocation_size
             << ") != trailer->allocation_size(" << this->allocation_size << ")"
             << dendl;
        return -1;
      }
      ...
      derr << "Illegal Trailer - signature=" << valid_signature << "("
           << ALLOCATOR_IMAGE_VALID_SIGNATURE << ")" << dendl;
      return -1;
    }
  }
...
//-------------------------------------------------------------------------------------
// invalidate old allocation file if exists so will go directly to recovery
// after failure we can safely ignore non-existing file
int BlueStore::invalidate_allocation_file_on_bluefs() {
  // mark that allocation-file was invalidated and we should destage a new copy
  // whne closing db
  need_to_destage_allocation_file = true;
  ...
  if (ret != 0) {
    dout(5) << __func__ << " allocator_file(" << allocator_file
            << ") doesn't exist" << dendl;
    // nothing to do -> return
    return 0;
  }
  ...
  if (ret != 0) {
    derr << __func__ << "::NCB:: Failed open_for_write with error-code " << ret
         << dendl;
    return -1;
  }
  ...
  if (ret != 0) {
    derr << __func__ << "::NCB:: Failed truncaste with error-code " << ret
         << dendl;
    bluefs->close_writer(p_handle);
    return -1;
  }
...
//-----------------------------------------------------------------------------------
int BlueStore::copy_allocator(Allocator* src_alloc, Allocator* dest_alloc,
  ...
  *p_num_entries = 0;
  ...
  try {
    ...
    derr << "****Failed dynamic allocation, num_entries=" << *p_num_entries
         << dendl;
    return -1;
  }
  ...
  auto copy_entries = [&](uint64_t extent_offset, uint64_t extent_length) {
    if (extent_length > 0) {
      ...
      derr << "zero length extent!!! offset=" << extent_offset
           << ", index=" << idx << dendl;
    }
  };
  ...
  if (idx > *p_num_entries) {
    derr << "****spillover, num_entries=" << *p_num_entries
         << ", spillover=" << (idx - *p_num_entries) << dendl;
...
//-----------------------------------------------------------------------------------
static uint32_t flush_extent_buffer_with_crc(BlueFS::FileWriter* p_handle,
                                             const char* buffer,
                                             const char* p_curr, uint32_t crc) {
...
int BlueStore::store_allocator(Allocator* src_allocator) {
  // when storing allocations to file we must be sure there is no background
  // compactions the easiest way to achieve it is to make sure db is closed
  ...
  BlueFS::FileWriter* p_handle = nullptr;
  ret = bluefs->open_for_write(allocator_dir, allocator_file, &p_handle,
                               overwrite_file);
  if (ret != 0) {
    derr << __func__ << "Failed open_for_write with error-code " << ret
         << dendl;
    return -1;
  }
  ...
  // store all extents (except for the bluefs extents we removed) in a single
  // flat file
  ...
  auto iterated_allocation = [&](uint64_t extent_offset,
                                 uint64_t extent_length) {
    if (extent_length == 0) {
      derr << __func__ << "" << extent_count << "::[" << extent_offset << ","
           << extent_length << "]" << dendl;
      ret = -1;
      return;
    }
    ...
    if (p_curr == p_end) {
      crc = flush_extent_buffer_with_crc(p_handle, (const char*)buffer,
                                         (const char*)p_curr, crc);
  ...
  {
    allocator_image_trailer trailer(timestamp, s_format_version, s_serial,
                                    extent_count, allocation_size);
  ...
  utime_t duration = ceph_clock_now() - start_time;
  dout(5) << "WRITE-extent_count=" << extent_count
          << ", allocation_size=" << allocation_size << ", serial=" << s_serial
          << dendl;
  dout(5) << "p_handle->pos=" << p_handle->pos << " WRITE-duration=" << duration
          << " seconds" << dendl;
...
Allocator* BlueStore::create_bitmap_allocator(uint64_t bdev_size) {
  ...
  uint64_t alloc_size = min_alloc_size;
  Allocator* alloc =
      Allocator::create(cct, "bitmap", bdev_size, alloc_size, "recovery");
  if (alloc) {
    return alloc;
  } else {
...
//-----------------------------------------------------------------------------------
int BlueStore::__restore_allocator(Allocator* allocator, uint64_t* num,
                                   uint64_t* bytes) {
  if (cct->_conf->bluestore_debug_inject_allocation_from_file_failure > 0) {
    boost::mt11213b rng(time(NULL));
    boost::uniform_real<> ur(0, 1);
    if (ur(rng) <
        cct->_conf->bluestore_debug_inject_allocation_from_file_failure) {
  ...
  int ret = bluefs->open_for_read(allocator_dir, allocator_file, &p_temp_handle,
                                  false);
  if (ret != 0) {
  ...
  uint64_t file_size = p_handle->file->fnode.size;
  dout(5) << "file_size=" << file_size
          << ",sizeof(extent_t)=" << sizeof(extent_t) << dendl;
  ...
  int header_size = calc_allocator_image_header_size();
  {
    ...
    int read_bytes =
        bluefs->read(p_handle.get(), offset, header_size, &temp_bl, nullptr);
    if (read_bytes != header_size) {
      derr << "Failed bluefs->read() for header::read_bytes=" << read_bytes
           << ", req_bytes=" << header_size << dendl;
      return -1;
    }
    ...
    crc_calc = header_bl.cbegin().crc32c(
        p.get_off(), crc_calc);  // crc from begin to current pos
    decode(crc, p);
    if (crc != crc_calc) {
      derr << "crc mismatch!!! crc=" << crc << ", crc_calc=" << crc_calc
           << dendl;
      derr << "header = \n" << header << dendl;
      return -1;
    }
  ...
  uint64_t extent_count = 0;
  uint64_t extents_bytes_left =
      file_size - (header_size + trailer_size + sizeof(crc));
  while (extents_bytes_left) {
    int req_bytes =
        std::min(extents_bytes_left, static_cast<uint64_t>(sizeof(buffer)));
    int read_bytes =
        bluefs->read(p_handle.get(), offset, req_bytes, nullptr, (char*)buffer);
    ...
    read_bytes =
        bluefs->read(p_handle.get(), offset, sizeof(crc), nullptr, (char*)&crc);
    if (read_bytes == sizeof(crc)) {
      crc = CEPHTOH_32(crc);
      if (crc != calc_crc) {
        derr << "data crc mismatch!!! crc=" << crc << ", calc_crc=" << calc_crc
             << dendl;
        derr << "extents_bytes_left=" << extents_bytes_left
             << ", offset=" << offset << ", extent_count=" << extent_count
             << dendl;
        return -1;
      }
      ...
      derr << "Failed bluefs->read() for crc::read_bytes=" << read_bytes
           << ", req_bytes=" << sizeof(crc) << dendl;
      return -1;
    }
  }

  // finally, read the trailer and verify it is in good shape and that we got
  // all the extents
  {
    bufferlist trailer_bl, temp_bl;
    int read_bytes =
        bluefs->read(p_handle.get(), offset, trailer_size, &temp_bl, nullptr);
    if (read_bytes != trailer_size) {
      derr << "Failed bluefs->read() for trailer::read_bytes=" << read_bytes
           << ", req_bytes=" << trailer_size << dendl;
      return -1;
    }
    offset += read_bytes;
    ...
    decode(trailer, p);
    if (trailer.verify(cct, path, &header, extent_count, read_alloc_size) !=
        0) {
      derr << "trailer=\n" << trailer << dendl;
      return -1;
    }

    crc_calc = trailer_bl.cbegin().crc32c(
        p.get_off(), crc_calc);  // crc from begin to current pos
    decode(crc, p);
    if (crc != crc_calc) {
      derr << "trailer crc mismatch!::crc=" << crc << ", crc_calc=" << crc_calc
           << dendl;
      derr << "trailer=\n" << trailer << dendl;
      return -1;
    }
  ...
  utime_t duration = ceph_clock_now() - start_time;
  dout(5) << "READ--extent_count=" << extent_count
          << ", read_alloc_size=  " << read_alloc_size
          << ", file_size=" << file_size << dendl;
  dout(5) << "READ duration=" << duration
          << " seconds, s_serial=" << header.serial << dendl;
  *num = extent_count;
...
//-----------------------------------------------------------------------------------
int BlueStore::restore_allocator(Allocator* dest_allocator, uint64_t* num,
                                 uint64_t* bytes) {
  utime_t start = ceph_clock_now();
  auto temp_allocator =
      unique_ptr<Allocator>(create_bitmap_allocator(bdev->get_size()));
  ...
  dout(5) << " calling copy_allocator(bitmap_allocator -> shared_alloc.a)"
          << dendl;
  copy_allocator(temp_allocator.get(), dest_allocator, &num_entries);
  utime_t duration = ceph_clock_now() - start;
  dout(5) << "restored in " << duration
          << " seconds, num_entries=" << num_entries << dendl;
  return ret;
}

//-----------------------------------------------------------------------------------
void BlueStore::set_allocation_in_simple_bmap(SimpleBitmap* sbmap,
                                              uint64_t offset,
                                              uint64_t length) {
  dout(30) << __func__ << " 0x" << std::hex << offset << "~" << length << " "
           << min_alloc_size_mask << dendl;
...
void BlueStore::ExtentDecoderPartial::consume_blobid(Extent* le, bool spanning,
                                                     uint64_t blobid) {
  [[maybe_unused]] auto cct = store.cct;
...
int BlueStore::read_allocation_from_onodes(SimpleBitmap* sbmap,
  ...
  if (it) {
    for (it->lower_bound(string()); it->valid(); it->next()) {
      ...
      dout(20) << __func__ << " decode sb " << pretty_binary_string(key)
               << dendl;
      uint64_t sbid = 0;
      if (get_key_shared_blob(key, &sbid) != 0) {
        derr << __func__ << " bad shared blob key '"
             << pretty_binary_string(key) << "'" << dendl;
      }
      bluestore_shared_blob_t shared_blob(sbid);
      bufferlist bl = it->value();
  ...
  ExtentDecoderPartial edecoder(*this, stats, *sbmap, sb_info,
                                min_alloc_size_order);

  // iterate over all ONodes stored in RocksDB
  for (it->lower_bound(string()); it->valid(); it->next(), kv_count++) {
    // trace an even after every million processed objects (typically every 5-10
    // seconds)
    ...
    auto key = it->key();
    auto okey = key;
    dout(20) << __func__ << " decode onode " << pretty_binary_string(key)
             << dendl;
    ghobject_t oid;
    if (!is_extent_shard_key(it->key())) {
      ...
      Onode dummy_on(cct);
      Onode::decode_raw(&dummy_on, it->value(), edecoder);
      ++stats.onode_count;
    } else {
      uint32_t offset;
  ...
  for (auto& p : stats.actual_pool_vstatfs) {
    ...
    p.second.publish(&s);
    dout(5) << __func__ << " recovered pool " << std::hex << p.first << "->"
            << s << std::dec << " per-pool:" << per_pool_stat_collection
            << dendl;
  }
  vstatfs = stats.actual_store_vstatfs;
  vstatfs.publish(&s);
  dout(5) << __func__ << " recovered " << s << dendl;
  return 0;
}

//---------------------------------------------------------
int BlueStore::reconstruct_allocations(SimpleBitmap* sbmap,
                                       read_alloc_stats_t& stats) {
...
//-----------------------------------------------------------------------------------
static void copy_simple_bitmap_to_allocator(SimpleBitmap* sbmap,
                                            Allocator* dest_alloc,
                                            uint64_t alloc_size) {
  int alloc_size_shift = std::countr_zero(alloc_size);
  uint64_t offset = 0;
  extent_t ext = sbmap->get_next_clr_extent(offset);
  while (ext.length != 0) {
    dest_alloc->init_add_free(ext.offset << alloc_size_shift,
                              ext.length << alloc_size_shift);
...
int BlueStore::read_allocation_from_drive_on_startup() {
  ...
  auto shutdown_cache = make_scope_guard([&] { _shutdown_cache(); });
  ...
  dout(1) << "::Allocation Recovery was completed in " << duration
          << " seconds, extent_count=" << stats.extent_count << dendl;
  return ret;
}

// Only used for debugging purposes - we build a secondary allocator from the
// Onodes and compare it to the existing one Not meant to be run by customers
...
//---------------------------------------------------------
int BlueStore::compare_allocators(Allocator* alloc1, Allocator* alloc2,
                                  uint64_t req_extent_count,
                                  uint64_t memory_target) {
  uint64_t allocation_size =
      std::min((req_extent_count) * sizeof(extent_t), memory_target / 3);
  uint64_t extent_count = allocation_size / sizeof(extent_t);
  dout(5) << "req_extent_count=" << req_extent_count
          << ", granted extent_count=" << extent_count << dendl;
  ...
  try {
    ...
    derr << "****Failed dynamic allocation, extent_count=" << extent_count
         << dendl;
    return -1;
  }

  // copy the extents from the allocators into simple array and then compare
  // them
  ...
  if (idx1 == idx2) {
    ...
    for (uint64_t i = 0; i < idx1; i++) {
      if (memcmp(arr1.get() + i, arr2.get() + i, sizeof(extent_t)) != 0) {
        derr << "!!!![" << i << "] arr1::<" << arr1[i].offset << ","
             << arr1[i].length << ">" << dendl;
        derr << "!!!![" << i << "] arr2::<" << arr2[i].offset << ","
             << arr2[i].length << ">" << dendl;
        return -1;
      }
    }
...
//---------------------------------------------------------
int BlueStore::add_existing_bluefs_allocation(Allocator* allocator,
                                              read_alloc_stats_t& stats) {
...
int BlueStore::read_allocation_from_drive_for_bluestore_tool() {
...

  {
    ...
    ret = compare_allocators(allocator.get(), alloc, stats.insert_count,
                             memory_target);
    if (ret == 0) {
      dout(5) << "Allocator drive - file integrity check OK" << dendl;
    } else {
      derr << "FAILURE. Allocator from file and allocator from metadata "
              "differ::ret="
           << ret << dendl;
    }
  }
...
Allocator* BlueStore::clone_allocator_without_bluefs(Allocator* src_allocator) {
  ...
  // BlueFS stores its internal allocation outside RocksDB (FM) so we should not
  // destage them to the allcoator-file we are going to hide bluefs allocation
  // during allocator-destage as they are stored elsewhere
  {
    bluefs->foreach_block_extents(bluefs_layout.shared_bdev,
                                  [&](uint64_t start, uint32_t len) {
...
//---------------------------------------------------------
static void clear_allocation_objects_from_rocksdb(KeyValueDB* db,
                                                  CephContext* cct,
                                                  const std::string& path) {
...
//---------------------------------------------------------
void BlueStore::copy_allocator_content_to_fm(Allocator* allocator,
                                             FreelistManager* real_fm) {
...
//---------------------------------------------------------
Allocator* BlueStore::initialize_allocator_from_freelist(
    FreelistManager* real_fm) {
...
int BlueStore::reset_fm_for_restore() {
  ...
  // this will mark the full device space as allocated (and not just the
  // reserved space)
...
int BlueStore::verify_rocksdb_allocations(Allocator* allocator) {
  ...
  uint64_t memory_target =
      cct->_conf.get_val<Option::size_t>("osd_memory_target");
  int ret = compare_allocators(allocator, temp_allocator, insert_count,
                               memory_target);
  ...
  if (ret == 0) {
    ...
    derr << "**** FAILURE compare(allocator, temp_allocator)::ret=" << ret
         << dendl;
    return -1;
  }
}
...
//---------------------------------------------------------
// convert back the system from null-allocator to using rocksdb to store
// allocation
int BlueStore::push_allocation_to_rocksdb() {
  if (cct->_conf->bluestore_allocation_from_file) {
    derr << "cct->_conf->bluestore_allocation_from_file must be cleared first"
         << dendl;
  ...
  // remove all objects of PREFIX_ALLOC_BITMAP from RocksDB to guarantee a clean
  // start
  ...
  // push the free-space from the allocator (shared-alloc without bfs) to
  // rocksdb
...
int BlueStore::commit_freelist_type() {
  ...
  // When allocation-info is stored in a single file we set freelist_type to
  // "null" This will direct the startup code to read allocation from file and
  // not RocksDB
  KeyValueDB::Transaction t = db->get_transaction();
  if (t == nullptr) {
...
#else
  // should check how long this step take on a big configuration as deletes are
  // expensive
  if (commit_freelist_type() == 0) {
    // remove all objects of PREFIX_ALLOC_BITMAP from RocksDB to guarantee a
    // clean start
    clear_allocation_objects_from_rocksdb(db, cct, path);
  }
#endif
