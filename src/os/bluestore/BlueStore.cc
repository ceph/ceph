  ...
  interval_set<uint32_t>& res_intervals)
{
...

  {
    ...
    for (auto i = _data_lower_bound(offset);
	  ...
	  uint32_t l = std::min(length, b->length - skip);
  ...
  KeyValueDB::Transaction t)
{
  ...
  unsigned extent_avg = bytes / std::max((unsigned)1, extents);
...
int BlueStore::_open_fm(bool create)
{
  ...
  if (create) {
    ...
    // bluefs_extents.
    uint64_t reserved = ROUND_UP_TO(std::max((uint64_t)SUPER_RESERVED, min_alloc_size),
				    min_alloc_size);
...
int BlueStore::_balance_bluefs_freespace(PExtentVector *extents)
{
  ...
  if (gift) {
    ...
    gift = std::min(gift, (uint64_t)1ull<<31);
  ...
  if (reclaim) {
    ...
    // hard cap to fit into 32 bits
    reclaim = std::min(reclaim, (uint64_t)1ull<<31);
...
int BlueStore::_fsck(bool deep, bool repair)
{
  ...
  apply(
    0, std::max(min_alloc_size, (uint64_t)(SUPER_RESERVED)), min_alloc_size, used_blocks,
    [&](uint64_t pos, mempool_dynamic_bitset &bs) {
      bs.set(pos);
    }
  );
  ...
  uint64_t chunk_size)
{
  ...
  *_dout << dendl;
  ...
  size_t pad_count = 0;
  if (front_pad) {
    size_t front_copy = std::min((unsigned)(chunk_size - front_pad), length);
                      ...
                      const string &prefix, size_t key_size, size_t value_size)
{
    ...
    key_hist[prefix][key_slab].max_len = std::max((uint32_t)key_size, key_hist[prefix][key_slab].max_len);
    key_hist[prefix][key_slab].val_map[value_slab].count++;
    key_hist[prefix][key_slab].val_map[value_slab].max_len =
                  std::max((uint32_t)value_size, key_hist[prefix][key_slab].val_map[value_slab].max_len);
}
