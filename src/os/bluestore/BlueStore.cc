  ...
  const pool_opts_t& opts)
{
  ...
  c->pool_opts = opts;
  c->segment_size = cct->_conf.get_val<Option::size_t>("bluestore_onode_segment_size");
  return 0;
}
  ...
  uint32_t fadvise_flags)
{
  ...
  if (c->segment_size != 0 && wctx.target_blob_size > c->segment_size) {
    wctx.target_blob_size = c->segment_size;
  }
  if (bl.length() != length) {
