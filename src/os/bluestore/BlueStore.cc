...
int BlueStore::_balance_bluefs_freespace(PExtentVector *extents)
{
  ...
  if (reclaim) {
    ...
    // hard cap to fit into 32 bits
    reclaim = std::min(reclaim, 1ul << 31);
...
int BlueStore::_fsck(bool deep, bool repair)
{
  ...
  apply(
    0, std::max(min_alloc_size, uint64_t(SUPER_RESERVED)), fm->get_alloc_size(), used_blocks,
    [&](uint64_t pos, mempool_dynamic_bitset &bs) {
	assert(pos < bs.size());
      bs.set(pos);
    }
  ...
  uint64_t chunk_size)
{
  ...
  *_dout << dendl;
  ...
  size_t pad_count = 0;
  if (front_pad) {
    size_t front_copy = std::min(chunk_size - front_pad, uint64_t(length));
