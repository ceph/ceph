    ...
    if (command == "bluestore allocator dump " + name) {
      ...
      if (alloc_unit == 0 ||
        ...
        ss << "Invalid allocation unit: '" << alloc_unit
           << ", to be aligned with: '" << alloc->get_block_size() << std::endl;
        return -EINVAL;
      }
      ...
      for (int i = 0; i < num_buckets; i++) {
        ...
        f->dump_unsigned("max_len", hist[i].get_max(i, num_buckets));
...
Allocator::Allocator(std::string_view name, int64_t _capacity,
                     int64_t _block_size)
...
const string& Allocator::get_name() const { return asok_hook->name; }
...
double Allocator::get_fragmentation_score() {
  ...
  // chunks larger then 128MB are large enough that should be counted without
  // penalty
  ...
  // for chunks 1MB - 128MB penalty coeffs are linearly weighted 1.2 (at small)
  // ... 1 (at huge)
...
void Allocator::build_free_state_histogram(
  ...
  size_t max = free_state_hist_bucket::get_max(num_buckets - 2, num_buckets);
