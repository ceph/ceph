...
StupidAllocator::StupidAllocator(CephContext *cct, int64_t capacity,
                                 int64_t _block_size, std::string_view name)
    : Allocator(name, capacity, _block_size), cct(cct), num_free(0), free(10) {
...
unsigned StupidAllocator::_choose_bin(uint64_t orig_len) {
  ...
  ldout(cct, 30) << __func__ << " len 0x" << std::hex << orig_len << std::dec
                 << " -> " << bin << dendl;
  return bin;
}
...
void StupidAllocator::_insert_free(uint64_t off, uint64_t len) {
  ...
  while (true) {
    ...
    if (newbin == bin) break;
    ldout(cct, 30) << __func__ << " promoting 0x" << std::hex << off << "~"
                   << len << std::dec << " to bin " << newbin << dendl;
    free[bin].erase(off, len);
    bin = newbin;
  }
}

int64_t StupidAllocator::allocate_int(uint64_t want_size, uint64_t alloc_unit,
                                      int64_t hint, uint64_t *offset,
                                      uint32_t *length) {
  std::lock_guard l(lock);
  ldout(cct, 10) << __func__ << " want_size 0x" << std::hex << want_size
                 << " alloc_unit 0x" << alloc_unit << " hint 0x" << hint
                 << std::dec << dendl;
...
found:
  *offset = p.get_start();
  *length = std::min(std::max(alloc_unit, want_size),
                     p2align(p.get_len(), alloc_unit));
  ...
  ldout(cct, 30) << __func__ << " got 0x" << std::hex << *offset << "~"
                 << *length << " from bin " << std::dec << bin << dendl;
  ...
  if (*offset && free[bin].contains(*offset - 1, &off, &len)) {
    ...
    if (newbin != bin) {
      ldout(cct, 30) << __func__ << " demoting 0x" << std::hex << off << "~"
                     << len << std::dec << " to bin " << newbin << dendl;
      free[bin].erase(off, len);
...
int64_t StupidAllocator::allocate(uint64_t want_size, uint64_t alloc_unit,
...
double StupidAllocator::get_fragmentation() {
  ...
  uint64_t intervals = 0;
  {
    std::lock_guard l(lock);
    max_intervals =
        p2roundup<uint64_t>(num_free, get_block_size()) / get_block_size();
...
void StupidAllocator::init_rm_free(uint64_t offset, uint64_t length) {
  ...
  for (unsigned i = 0; i < free.size() && !rm.empty(); ++i) {
    ...
    if (!overlap.empty()) {
      ldout(cct, 20) << __func__ << " bin " << i << " rm 0x" << std::hex
                     << overlap << std::dec << dendl;
      ...
      while (it != it_end) {
        ...
        free[i].erase(o, l, [&](uint64_t off, uint64_t len) {
          unsigned newbin = _choose_bin(len);
          if (newbin != i) {
            ldout(cct, 30) << __func__ << " demoting1 0x" << std::hex << off
                           << "~" << len << std::dec << " to bin " << newbin
                           << dendl;
            _insert_free(off, len);
            return true;
          }
...
void StupidAllocator::shutdown() { ldout(cct, 1) << __func__ << dendl; }
