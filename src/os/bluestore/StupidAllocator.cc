...
{
  uint64_t len = orig_len / cct->_conf->get_val<int64_t>("bdev_block_size");
  ...
  uint64_t *offset, uint32_t *length)
{
  ...
  *length = MIN(MAX(alloc_unit, want_size), P2ALIGN((p.get_len() - skew), alloc_unit));
  if (cct->_conf->get_val<int64_t>("bluestore_debug_small_allocations")) {
    uint64_t max =
      alloc_unit * (rand() % cct->_conf->get_val<int64_t>("bluestore_debug_small_allocations"));
    if (max && *length > max) {
