...
void bluestore_extent_ref_map_t::_check() const {
  ...
  for (const auto& p : ref_map) {
    if (p.first < pos) ceph_abort_msg("overlap");
    if (p.first == pos && p.second.refs == refs) ceph_abort_msg("unmerged");
...
void bluestore_extent_ref_map_t::get(uint64_t offset, uint32_t length) {
  ...
  if (p != ref_map.end()) _maybe_merge_left(p);
  //_check();
}
  ...
  while (length > 0) {
    ...
    if (length < p->second.length) {
      ...
      if (p->second.refs > 1) {
        ...
        if (release) release->push_back(bluestore_pextent_t(p->first, length));
        ref_map.erase(p);
      }
      goto out;
    }
...
bluestore_extent_ref_map_t::debug_len_cnt
bluestore_extent_ref_map_t::debug_peek(uint64_t offset) const {
  // locate offset
  auto p = ref_map.lower_bound(offset);
  if (p != ref_map.end() && p->first == offset) {
...
bool bluestore_extent_ref_map_t::contains(uint64_t offset,
  ...
  while (length > 0) {
    if (p == ref_map.end()) return false;
    if (p->first > offset) return false;
    if (p->first + p->second.length >= offset + length) return true;
...
bool bluestore_extent_ref_map_t::intersects(uint64_t offset,
  ...
  if (p == ref_map.end()) return false;
  if (p->first >= offset + length) return false;
...
bluestore_blob_use_tracker_t::bluestore_blob_use_tracker_t(
    ...
    : au_size{tracker.au_size}, num_au(0), alloc_au(0), bytes_per_au{nullptr} {
  if (tracker.num_au > 0) {
...
void bluestore_blob_use_tracker_t::release(uint32_t au_count, uint32_t* ptr) {
  if (au_count) {
    ...
    mempool::get_pool(
        mempool::pool_index_t(mempool::mempool_bluestore_cache_other))
        .adjust_count(-(int32_t)au_count,
                      -(int32_t)(sizeof(uint32_t) * au_count));
  }
}
...
void bluestore_blob_use_tracker_t::dup(const bluestore_blob_use_tracker_t& from,
  ...
  for (uint32_t i = start / au_size, pos = start; pos < end;
       i++, pos += au_size) {
    array[i] = afrom[i];
  }
}
...
void bluestore_blob_t::adjust_to(const bluestore_blob_t& other,
...
void bluestore_blob_t::dump(Formatter* f) const {
  ...
  for (unsigned i = 0; i < n; ++i) f->dump_unsigned("csum", get_csum_item(i));
  f->close_section();
...
ostream& operator<<(ostream& out, const bluestore_blob_t& o) {
  ...
  if (o.has_unused()) out << " unused=0x" << std::hex << o.unused << std::dec;
  out << ")";
  return out;
}
...
void bluestore_blob_t::dup(const bluestore_blob_t& from) {
  ...
  if (from.csum_data.length()) {
    csum_data =
        ceph::buffer::ptr(from.csum_data.c_str(), from.csum_data.length());
...
shared_blob_2hash_tracker_t::build_hash_input(uint64_t sbid,
                                              uint64_t offset) const {
  hash_input_t res = {sbid, offset >> au_void_bits,
                      ((sbid & 0xffffffff) << 32) +
                          ~(uint32_t((offset >> au_void_bits) & 0xffffffff))};
  return res;
}

void shared_blob_2hash_tracker_t::inc(uint64_t sbid, uint64_t offset, int n) {
  auto hash_input = build_hash_input(sbid, offset);
  ref_counter_2hash_tracker_t::inc((char*)hash_input.data(),
                                   get_hash_input_size(), n);
}

void shared_blob_2hash_tracker_t::inc_range(uint64_t sbid, uint64_t offset,
                                            uint32_t len, int n) {
...
bool shared_blob_2hash_tracker_t::test_hash_conflict(uint64_t sbid1,
                                                     uint64_t offset1,
                                                     uint64_t sbid2,
  ...
  return ref_counter_2hash_tracker_t::test_hash_conflict(
      (char*)hash_input1.data(), (char*)hash_input2.data(),
      get_hash_input_size());
}

bool shared_blob_2hash_tracker_t::test_all_zero(uint64_t sbid,
                                                uint64_t offset) const {
  auto hash_input = build_hash_input(sbid, offset);
  return ref_counter_2hash_tracker_t::test_all_zero((char*)hash_input.data(),
                                                    get_hash_input_size());
}

bool shared_blob_2hash_tracker_t::test_all_zero_range(uint64_t sbid,
                                                      uint64_t offset,
                                                      uint32_t len) const {
  uint32_t alloc_unit = 1 << au_void_bits;
  int64_t l = len;
