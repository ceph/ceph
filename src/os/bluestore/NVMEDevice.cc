...
class SharedDriverData {
  ...
  SharedDriverData(unsigned _id, const std::string &sn_tag,
    ...
    RTE_LCORE_FOREACH_SLAVE(i) {
      queues.push_back(new SharedDriverQueueData(this, ctrlr, ns, block_size, sn, sector_size, i, queue_index++));
   }
  ...
  SharedDriverQueueData *get_queue(uint32_t i) {
	return queues.at(i%(queue_index + 1));
  }
...
{
  dout(1) << __func__ << " YZYYYYYYYYYYYYY start" << queue_id << dendl;
...
void io_complete(void *t, const struct spdk_nvme_cpl *completion)
{
  ...
  if(queue_id == -1) {
    queue_id = pthread_self();
  }
  queue = driver->get_queue(queue_id);
