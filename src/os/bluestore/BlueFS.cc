...
static void wal_discard_cb(void *priv, void* priv2) {
  ...
  bluefs->handle_discard_completion(BlueFS::BDEV_WAL, *tmp);
}

static void db_discard_cb(void *priv, void* priv2) {
  BlueFS *bluefs = static_cast<BlueFS*>(priv);
  interval_set<uint64_t> *tmp = static_cast<interval_set<uint64_t>*>(priv2);
  bluefs->handle_discard_completion(BlueFS::BDEV_DB, *tmp);
}

static void slow_discard_cb(void *priv, void* priv2) {
  BlueFS *bluefs = static_cast<BlueFS*>(priv);
  interval_set<uint64_t> *tmp = static_cast<interval_set<uint64_t>*>(priv2);
  bluefs->handle_discard_completion(BlueFS::BDEV_SLOW, *tmp);
}
...
void BlueFS::handle_discard_completion(unsigned id, interval_set<uint64_t>& to_release)
{
  dout(10) << __func__ << " bdev " << id << dendl;
  _do_release(id, to_release);
}
...
void BlueFS::_release_pending_allocations(vector<interval_set<uint64_t>>& to_release)
{
  for (unsigned i = 0; i < to_release.size(); ++i) {
    ...
    if (!discard_queued) {
      _do_release(i, to_release[i]);
    }
  }
}

void BlueFS::_do_release(unsigned id, const interval_set<uint64_t>& to_release)
{
  ceph_assert(alloc[id]);
