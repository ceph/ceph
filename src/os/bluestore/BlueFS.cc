...
MEMPOOL_DEFINE_OBJECT_FACTORY(BlueFS::FileWriter, bluefs_file_writer,
                              bluefs_file_writer);
MEMPOOL_DEFINE_OBJECT_FACTORY(BlueFS::FileReaderBuffer,
                              bluefs_file_reader_buffer, bluefs_file_reader);
MEMPOOL_DEFINE_OBJECT_FACTORY(BlueFS::FileReader, bluefs_file_reader,
                              bluefs_file_reader);
  ...
  static BlueFS::SocketHook* create(BlueFS* bluefs) {
    ...
    if (admin_socket) {
      ...
      if (r != 0) {
        ...
        ceph_assert(r == 0);
        r = admin_socket->register_command(
            "bluefs debug_inject_read_zeros", hook,
...
void BlueFS::_init_logger() {
  ...
  b.add_u64_counter(l_bluefs_read_random_buffer_bytes,
                    ...
                    PerfCountersBuilder::PRIO_USEFUL, unit_t(UNIT_BYTES));
  b.add_time_avg(l_bluefs_read_lat, "read_lat", "Average bluefs read latency",
                 "rd_t", PerfCountersBuilder::PRIO_INTERESTING);
  b.add_u64_counter(l_bluefs_read_count, "read_count",
                    "buffered read requests processed", NULL,
                    PerfCountersBuilder::PRIO_USEFUL);
  ...
  b.add_time_avg(l_bluefs_wal_alloc_lat, "wal_alloc_lat",
                 "Average bluefs wal allocate latency", "bwal",
                 PerfCountersBuilder::PRIO_USEFUL);
  b.add_time_avg(l_bluefs_db_alloc_lat, "db_alloc_lat",
                 "Average bluefs db allocate latency", "bdal",
                 PerfCountersBuilder::PRIO_USEFUL);
  b.add_time_avg(l_bluefs_slow_alloc_lat, "slow_alloc_lat",
                 "Average allocation latency for primary/shared device", "bsal",
                 PerfCountersBuilder::PRIO_USEFUL);
  b.add_time(l_bluefs_wal_alloc_max_lat, "alloc_wal_max_lat",
             "Max allocation latency for wal device", "awxt",
             PerfCountersBuilder::PRIO_INTERESTING);
  b.add_time(l_bluefs_db_alloc_max_lat, "alloc_db_max_lat",
             "Max allocation latency for db device", "adxt",
             PerfCountersBuilder::PRIO_INTERESTING);
  b.add_time(l_bluefs_slow_alloc_max_lat, "alloc_slow_max_lat",
             "Max allocation latency for primary/shared device", "asxt",
             PerfCountersBuilder::PRIO_INTERESTING);
...
BlockDevice* BlueFS::get_block_device(unsigned id) const {
  if (id < bdev.size() && bdev[id]) return bdev[id];
  return nullptr;
}
...
uint64_t BlueFS::get_total(unsigned id) { return _get_total(id); }
...
void BlueFS::dump_block_extents(ostream& out) {
  for (unsigned i = 0; i < MAX_BDEV; ++i) {
    ...
    out << i << " : device size 0x" << std::hex << total << " : using 0x"
        << total - free << std::dec << "(" << byte_u_t(total - free) << ")";
    out << "\n";
  }
}

void BlueFS::foreach_block_extents(unsigned id,
...
int BlueFS::mkfs(uuid_d osd_uuid, const bluefs_layout_t& layout) {
  ...
  int r = _allocate(vselector->select_prefer_bdev(log_file->vselector_hint),
                    cct->_conf->bluefs_max_log_runway, 0, &log_file->fnode);
...
void BlueFS::_init_alloc() {
  ...
  auto change_alloc_size = [this](uint64_t& max_alloc_size, uint64_t new_alloc,
                                  bool& changed) {
    if (max_alloc_size == 0 ||
  ...
  if (bdev[BDEV_WAL]) {
    ...
    change_alloc_size(super.bluefs_max_alloc_size[BDEV_WAL], wal_alloc_size,
                      alloc_size_changed);
  }
  ...
  if (bdev[BDEV_SLOW]) {
    ...
    change_alloc_size(super.bluefs_max_alloc_size[BDEV_SLOW], shared_alloc_size,
                      alloc_size_changed);
  } else {
    alloc_size[BDEV_DB] = shared_alloc_size;
    alloc_size[BDEV_SLOW] = 0;
    change_alloc_size(super.bluefs_max_alloc_size[BDEV_DB], shared_alloc_size,
                      alloc_size_changed);
  }
  ...
  if (alloc_size_changed) {
    dout(1) << __func__ << " alloc_size changed, the new super is:" << super
            << dendl;
    _write_super(BDEV_DB);
  }
  ...
  for (unsigned id = 0; id < bdev.size(); ++id) {
    ...
    if (is_shared_alloc(id)) {
      dout(1) << __func__ << " shared, id " << id << std::hex << ", capacity 0x"
              << bdev[id]->get_size() << ", block size 0x" << alloc_size[id]
              << std::dec << dendl;
    } else {
      ...
      dout(1) << __func__ << " new, id " << id << std::hex
              << ", allocator name " << name << ", allocator type "
              << cct->_conf->bluefs_allocator << ", capacity 0x"
              << bdev[id]->get_size() << ", reserved 0x" << block_reserved[id]
              << ", block size 0x" << alloc_size[id] << ", max alloc size 0x"
              << super.bluefs_max_alloc_size[id] << std::dec << dendl;
      alloc[id] = Allocator::create(cct, cct->_conf->bluefs_allocator,
                                    bdev[id]->get_size(),
                                    super.bluefs_max_alloc_size[id], name);
...
int BlueFS::_read_and_check(uint8_t ndev, uint64_t off, uint64_t len,
                            ceph::buffer::list* pbl, IOContext* ioc,
                            bool buffered) {
  dout(10) << __func__ << " dev " << int(ndev) << ": 0x" << std::hex << off
           << "~" << len << std::dec << (buffered ? " buffered" : "") << dendl;
  int r;
  bufferlist bl;
  ...
  if (inject_read_zeros) {
    if (len >= block_size * 2) {
      derr << __func__ << " injecting error, zeros at " << int(ndev) << ": 0x"
           << std::hex << (off + len / 2) << "~" << (block_size * 2) << std::dec
           << dendl;
  ...
  if (all_zeros) {
    ...
    if (!bl.contents_equal(bl_reread)) {
      ...
      derr << __func__ << " initial read of " << int(ndev) << ": 0x" << std::hex
           << off << "~" << len << std::dec << ": different then re-read "
           << dendl;
...
int BlueFS::_read_random_and_check(uint8_t ndev, uint64_t off, uint64_t len,
...
int BlueFS::_bdev_read(uint8_t ndev, uint64_t off, uint64_t len,
  ...
  switch (ndev) {
    case BDEV_WAL:
      cnt = l_bluefs_read_disk_bytes_wal;
      break;
    case BDEV_DB:
      cnt = l_bluefs_read_disk_bytes_db;
      break;
    case BDEV_SLOW:
      cnt = l_bluefs_read_disk_bytes_slow;
      break;
  }
  if (cnt) {
    logger->inc(cnt, len);
...
int BlueFS::_bdev_read_random(uint8_t ndev, uint64_t off, uint64_t len,
  ...
  switch (ndev) {
    case BDEV_WAL:
      cnt = l_bluefs_read_random_disk_bytes_wal;
      break;
    case BDEV_DB:
      cnt = l_bluefs_read_random_disk_bytes_db;
      break;
    case BDEV_SLOW:
      cnt = l_bluefs_read_random_disk_bytes_slow;
      break;
  }
  if (cnt) {
    logger->inc(cnt, len);
...
int BlueFS::mount() {
  ...
  if (shared_alloc) {
    shared_alloc->need_init = false;
    dout(1) << __func__ << " shared_bdev_used = " << shared_alloc->bluefs_used
            << dendl;
  } else {
...
int BlueFS::prepare_new_device(int id, const bluefs_layout_t& layout) {
  ...
  if (id == BDEV_NEWDB) {
    ...
    _rewrite_log_and_layout_sync_LNF_LD(false, BDEV_NEWDB, new_log_dev_cur,
                                        new_log_dev_next, RENAME_DB2SLOW,
                                        layout);
  } else if (id == BDEV_NEWWAL) {
    _rewrite_log_and_layout_sync_LNF_LD(false, BDEV_DB, BDEV_NEWWAL, BDEV_WAL,
                                        REMOVE_WAL, layout);
  } else {
    assert(false);
  }
...
int BlueFS::_open_super() {
  ...
  r = _bdev_read(BDEV_DB, get_super_offset(), get_super_length(), &bl,
                 ioc[BDEV_DB], false);
  if (r < 0) return r;
...
int BlueFS::_check_allocations(
    const bluefs_fnode_t& fnode, boost::dynamic_bitset<uint64_t>* used_blocks,
  ...
  for (auto e : fnode_extents) {
    ...
    if (int r = _verify_alloc_granularity(id, e.offset, e.length, alloc_unit,
                                          op_name);
        r < 0) {
      return r;
    }
    ...
    apply_for_bitset_range(
        e.offset, e.length, alloc_unit, used_blocks[id],
    ...
    if (fail) {
      derr << __func__ << " " << op_name << " invalid extent " << int(e.bdev)
           ...
           << (is_alloc == true ? ": duplicate reference, ino "
                                : ": double free, ino ")
           << fnode.ino << dendl;
      return -EFAULT;
    }
...
int BlueFS::_replay(bool noop, bool to_stdout) {
  ...
  if (!noop) {
    if (cct->_conf->bluefs_log_replay_check_allocations) {
      ...
      int r = _check_allocations(log_file->fnode, used_blocks, true,
                                 "Log from super");
      if (r < 0) {
        return r;
      }
  ...
  while (true) {
    ...
    bufferlist bl;
    {
      ...
      if (r != (int)super.block_size && cct->_conf->bluefs_replay_recovery) {
        r += _do_replay_recovery_read(log_reader, pos, read_pos + r,
                                      super.block_size - r, &bl);
      }
    ...
    if (more) {
      ...
      if (r < (int)more) {
        dout(10) << __func__ << " 0x" << std::hex << pos << ": stop: len is 0x"
                 << bl.length() + more << std::dec << ", which is past eof"
                 << dendl;
        if (cct->_conf->bluefs_replay_recovery) {
          // try to search for more data
          r += _do_replay_recovery_read(log_reader, pos, read_pos + r, more - r,
                                        &t);
          if (r < (int)more) {
      ...
      switch (op) {
        ...
        case bluefs_transaction_t::OP_JUMP: {
          ...
          if (unlikely(to_stdout)) {
            std::cout << " 0x" << std::hex << pos << std::dec
                      << ":  op_jump seq " << next_seq << " offset 0x"
        ...
        case bluefs_transaction_t::OP_DIR_LINK: {
          ...
          if (!noop) {
            ...
            map<string, FileRef>::iterator r =
                q->second->file_map.find(filename);
        ...
        case bluefs_transaction_t::OP_FILE_UPDATE: {
          ...
          if (!noop) {
            ...
            if (cct->_conf->bluefs_log_replay_check_allocations) {
              int r = _check_allocations(f->fnode, used_blocks, false,
                                         "OP_FILE_UPDATE");
              if (r < 0) {
                return r;
              }
            ...
            if (cct->_conf->bluefs_log_replay_check_allocations) {
              int r = _check_allocations(f->fnode, used_blocks, true,
                                         "OP_FILE_UPDATE");
              if (r < 0) {
                return r;
              }
        ...
        case bluefs_transaction_t::OP_FILE_UPDATE_INC: {
          ...
          if (!noop) {
            ...
            if (cct->_conf->bluefs_log_replay_check_allocations) {
              int r = _check_allocations(fnode, used_blocks, false,
                                         "OP_FILE_UPDATE_INC");
              if (r < 0) {
                return r;
              }
        ...
        case bluefs_transaction_t::OP_FILE_REMOVE: {
          ...
          if (!noop) {
            ...
            if (cct->_conf->bluefs_log_replay_check_allocations) {
              int r = _check_allocations(p->second->fnode, used_blocks, false,
                                         "OP_FILE_REMOVE");
              if (r < 0) {
                return r;
              }
...
int BlueFS::device_migrate_to_existing(CephContext* cct,
  ...
  for (auto& [ino, file_ref] : nodes.file_map) {
    ...
    bool rewrite = std::any_of(file_ref->fnode.extents.begin(),
                               file_ref->fnode.extents.end(), [=](auto& ext) {
                                 return ext.bdev != dev_target &&
                                        devs_source.count(ext.bdev);
                               });
    if (rewrite) {
      ...
      for (const auto& old_ext : old_fnode_extents) {
        ...
        int r = _bdev_read_random(old_ext.bdev, old_ext.offset, old_ext.length,
      ...
      // write entire file
      auto l = _allocate(dev_target, bl.length(), 0, &file_ref->fnode, nullptr,
                         0, false);
      if (l < 0) {
  ...
  _rewrite_log_and_layout_sync_LNF_LD(
      false, (flags & REMOVE_DB) ? BDEV_SLOW : BDEV_DB, new_log_dev_cur,
...
int BlueFS::device_migrate_to_new(CephContext* cct, const set<int>& devs_source,
  ...
  _rewrite_log_and_layout_sync_LNF_LD(false, super_dev, new_log_dev_cur,
                                      new_log_dev_next, flags, layout);
  return 0;
}
...
In any case it is enough to hold File::lock to be sure fnode will not be
modified.
*/
...
void BlueFS::_drop_link_D(FileRef file) {
  dout(20) << __func__ << " had refs " << file->refs << " on "
           << lock_fnode_print(file) << dendl;
                             ...
                             char* out)      ///< [out] copy it here
{
  ...
  dout(10) << __func__ << " h " << h << " 0x" << std::hex << off << "~" << len
           << std::dec << " from " << lock_fnode_print(h->file) << dendl;
    ...
    char* out)          ///< [out] optional: or copy it here
{
  ...
  dout(10) << __func__ << " h " << h << " 0x" << std::hex << off << "~" << len
           << std::dec << " from " << lock_fnode_print(h->file)
    ...
    if (off < buf->bl_off || off >= buf->get_buf_end()) {
      ...
      if (off < buf->bl_off || off >= buf->get_buf_end()) {
        ...
        if (!cct->_conf->bluefs_check_for_zeros) {
          ...
          r = _read_and_check(p->bdev, p->offset + x_off, l, &buf->bl,
                              ioc[p->bdev], use_buffered_io);
        }
...
bool BlueFS::_should_start_compact_log_L_N() {
  ...
  dout(10) << __func__ << " current 0x" << std::hex << current << " expected "
           << expected << std::dec << " ratio " << ratio << dendl;
    ...
    if (bdev_update_flags) {
      for (auto& e : file_ref->fnode.extents) {
        ...
        if (bdev == BDEV_NEWDB) {
          ...
          ceph_assert(!(bdev_update_flags & REMOVE_DB) !=
                      !(bdev_update_flags & RENAME_DB2SLOW));
...
void BlueFS::_compact_log_sync_LNF_LD() {
  ...
  _rewrite_log_and_layout_sync_LNF_LD(true, BDEV_DB, prefer_bdev, prefer_bdev,
                                      0, super.memorized_layout);
...
void BlueFS::_rewrite_log_and_layout_sync_LNF_LD(
    bool permit_dev_fallback, int super_dev, int log_dev, int log_dev_new,
  ...
  dout(10) << __func__ << " super_dev:" << super_dev << " log_dev:" << log_dev
           << " log_dev_new:" << log_dev_new << " flags:" << flags
           << " seq:" << log.seq_live << dendl;
  ...
  log.t.clear();
  log.t.seq = log.seq_live;
  // From now on, no changes to log.t are permitted until we finish rewriting
  // log. Can allow dirty to remain dirty - log.seq_live will not change.
  ...
  for (auto p : fnode_starter.extents) {
    ...
    if (log_dev != log_dev_new) {
      dout(10) << __func__ << " renaming log extents to " << log_dev_new
               << dendl;
      p.bdev = log_dev_new;
    }
  ...
  _make_initial_transaction(starter_seq, fnode_persistent, starter_need,
                            &starter_bl);
  ...
  //
  {
    dout(10) << __func__ << " release old log extents " << old_log_fnode.extents
             << dendl;
    std::lock_guard dl(dirty.lock);
...
void BlueFS::_compact_log_async_LD_LNF_D()  // also locks FW for new_writer
{
  ...
  // Extend log in case of having a big transaction waiting before starting
  // compaction.
  _maybe_extend_log();
  ...
  // 2. Update op to log
  // 3. Jump op to log
  // During that, no one else can write to log, otherwise we risk jumping
  // backwards. We need to sync log, because we are injecting discontinuity, and
  // writer is not prepared for that.
  ...
  dout(10) << __func__ << " old_log_jump_to 0x" << std::hex << old_log_jump_to
           << " need 0x" << cct->_conf->bluefs_max_log_runway << std::dec
           << dendl;
  int r = _allocate(vselector->select_prefer_bdev(log_file->vselector_hint),
                    cct->_conf->bluefs_max_log_runway, 0, &fnode_tail);
  ceph_assert(r == 0);
  ...
  // 1.3 update the log file change and log a jump to the offset where we want
  // to write the new entries
  ...
  // we need to flush all bdev because we will be streaming all dirty files to
  // log
  // TODO - think - if _flush_and_sync_log_jump will not add dirty files nor
  // release pending allocations then flush_bdev() will not be necessary
  _flush_bdev();
           ...
           << dendl;
  {
    ...
    r = _allocate(vselector->select_prefer_bdev(log_file->vselector_hint),
                  compacted_meta_need, 0, &fnode_pre_tail);
    ceph_assert(r == 0);
  ...
  // 2.7 Encode compacted meta transaction,
  dout(20) << __func__ << " new_log jump seq " << seq_now << std::hex
           << " offset 0x" << starter_need + compacted_meta_need << std::dec
           << dendl;
...
void BlueFS::_pad_bl(bufferlist& bl, uint64_t pad_size) {
  ...
  if (partial) {
    dout(10) << __func__ << " padding with 0x" << std::hex << pad_size - partial
             << " zeros" << std::dec << dendl;
...
void BlueFS::_consume_dirty(uint64_t seq) {
  ...
  // log dirty files
  // we just incremented log_seq. It is now illegal to add to
  // dirty.files[log_seq]
  auto lsi = dirty.files.find(seq);
  if (lsi != dirty.files.end()) {
    dout(20) << __func__ << " " << lsi->second.size() << " dirty.files"
             << dendl;
...
int64_t BlueFS::_maybe_extend_log() {
  ...
  // increasing the size of the log involves adding a OP_FILE_UPDATE_INC which
  // its size will increase with respect the number of extents.
  // bluefs_min_log_runway should cover the max size a log can get. inject new
  // allocation in case log is too big
  ...
  runway = log.writer->file->fnode.get_allocated() -
           log.writer->get_effective_write_pos();
  return runway;
}
...
void BlueFS::_extend_log(uint64_t amount) {
  ...
  int r = _allocate(
      vselector->select_prefer_bdev(log.writer->file->vselector_hint), amount,
      0, &log.writer->file->fnode, [&](const bluefs_extent_t& e) {
  ...
  ceph_assert(allocated_before_extension >=
              log.writer->get_effective_write_pos());
...
void BlueFS::_flush_and_sync_log_core() {
  ...
  uint64_t runway = log.writer->file->fnode.get_allocated() -
                    log.writer->get_effective_write_pos();
  // ensure runway is big enough, this should be taken care of by
  // _maybe_extend_log, but let's keep this here just in case.
...
int BlueFS::_flush_and_sync_log_LD(uint64_t want_seq) {
  ...
  ceph_assert(
      want_seq == 0 ||
      want_seq <=
          dirty.seq_live);  // illegal to request seq that was not created yet
...
int BlueFS::_flush_and_sync_log_jump_D(uint64_t jump_to) {
  ...
  vselector->sub_usage(log.writer->file->vselector_hint,
                       log.writer->file->fnode.size);
  log.writer->file->fnode.size = jump_to;
  vselector->add_usage(log.writer->file->vselector_hint,
                       log.writer->file->fnode.size);
...
ceph::bufferlist BlueFS::FileWriter::flush_buffer(CephContext* const cct,
                                                  const bool partial,
...
int BlueFS::_signal_dirty_to_log_D(FileWriter* h) {
  ...
  if (h->file->dirty_seq <= dirty.seq_stable) {
    ...
    dout(20) << __func__ << " dirty_seq = " << dirty.seq_live << " (was clean)"
             << dendl;
  } else {
    if (h->file->dirty_seq != dirty.seq_live) {
      ...
      h->file->dirty_seq = dirty.seq_live;
      dirty.files[h->file->dirty_seq].push_back(*h->file);
      dout(20) << __func__ << " dirty_seq = " << dirty.seq_live << " (was "
...
void BlueFS::flush_range(FileWriter* h, uint64_t offset,
                         uint64_t length) /*_WF*/
{
...
int BlueFS::_flush_range_F(FileWriter* h, uint64_t offset, uint64_t length) {
  ...
  if (h->file->fnode.size < offset + length) {
    vselector->add_usage(h->file->vselector_hint,
...
void BlueFS::append_try_flush(FileWriter* h, const char* buf,
                              size_t len) /*_WF_LNF_NF_LD_D*/
{
  bool flushed_sum = false;
  {
    ...
    while (len > 0) {
      ...
      if (l0 < max_size) {
        ...
        len -= l;
        need_flush =
            h->get_buffer_length() >= cct->_conf->bluefs_min_flush_size;
      }
      if (need_flush) {
        bool flushed = false;
...
int BlueFS::fsync(FileWriter* h) /*_WF_WD_WLD_WLNF_WNF*/
{
  ...
  {
    dout(10) << __func__ << " " << h << " " << h->file->fnode << " dirty "
             << h->file->is_dirty << dendl;
    int r = _flush_F(h, true);
    if (r < 0) return r;
    _flush_bdev(h);
    if (h->file->is_dirty) {
...
int BlueFS::_allocate(uint8_t id, uint64_t len, uint64_t alloc_unit,
                      bluefs_fnode_t* node, update_fn_t cb,
                      size_t alloc_attempts, bool permit_dev_fallback) {
  dout(10) << __func__ << " len 0x" << std::hex << len << " au 0x" << alloc_unit
           << std::dec << " from " << (int)id << " cooldown "
           << cooldown_deadline << dendl;
  ceph_assert(id < alloc.size());
  int64_t alloc_len = 0;
  ...
  if (alloc_len < 0 || alloc_len < need) {
    if (alloc[id]) {
      ...
      if (!was_cooldown && shared) {
        ...
        cooldown_deadline = delay_s + duration_cast<seconds>(
                                          real_clock::now().time_since_epoch())
                                          .count();
        dout(1) << __func__ << " shared allocation cooldown set for " << delay_s
                << "s" << dendl;
      }
      dout(1) << __func__ << " unable to allocate 0x" << std::hex << need
              << " on bdev " << (int)id << ", allocator name "
              << alloc[id]->get_name() << ", allocator type "
              << alloc[id]->get_type() << ", capacity 0x"
              << alloc[id]->get_capacity() << ", block size 0x"
              << alloc[id]->get_block_size() << ", alloc unit 0x" << alloc_unit
              << ", free 0x" << alloc[id]->get_free() << ", fragmentation "
              << alloc[id]->get_fragmentation() << ", allocated 0x"
              << (alloc_len > 0 ? alloc_len : 0) << std::dec << dendl;
    } else {
      dout(20) << __func__ << " alloc-id not set on index=" << (int)id
               << " unable to allocate 0x" << std::hex << need << " on bdev "
               << (int)id << std::dec << dendl;
    }
    if (alloc[id] && shared && alloc_unit != shared_unit) {
      alloc_unit = shared_unit;
      dout(20) << __func__ << " fallback to bdev " << (int)id
               << " with alloc unit 0x" << std::hex << alloc_unit << std::dec
               << dendl;
      logger->inc(l_bluefs_alloc_shared_size_fallbacks);
      return _allocate(id, len, alloc_unit, node, cb, alloc_attempts,
                       permit_dev_fallback);
      ...
      return _allocate(id + 1, len,
                       0,  // back to default alloc unit
                       node, cb, alloc_attempts, permit_dev_fallback);
    } else {
...
int BlueFS::preallocate(FileRef f, uint64_t off, uint64_t len) /*_LF*/
{
  ...
  if (off + len > allocated) {
    ...
    int r = _allocate(vselector->select_prefer_bdev(f->vselector_hint), want, 0,
                      &f->fnode, [&](const bluefs_extent_t& e) {
...
int BlueFS::open_for_write(std::string_view dirname, std::string_view filename,
                           FileWriter** h, bool overwrite) /*_LND*/
{
  _maybe_check_vselector_LNF();
  FileRef file;
  bool create = false;
  mempool::bluefs::vector<bluefs_extent_t> pending_release_extents;
  {
    ...
    if (q == dir->file_map.end() || q->first != filename) {
      ...
      ++file->refs;
      create = true;
      vselector->add_usage(file->vselector_hint, file->fnode.size,
                           true);  // update file count
      logger->set(l_bluefs_num_files, nodes.file_map.size());
    } else {
      ...
      if (overwrite) {
        ...
        dout(20) << __func__ << " dir " << dirname << " (" << dir << ") file "
                 << filename << " already exists, truncate + overwrite"
                 << dendl;
        vselector->sub_usage(file->vselector_hint, file->fnode);
        file->fnode.size = 0;
        vselector->add_usage(file->vselector_hint, file->fnode.size,
                             true);  // restore file count
...
void BlueFS::_drain_writer(FileWriter* h) {
  ...
  if (h->file->fnode.size >= (1ull << 30)) {
    dout(10) << __func__ << " file is unexpectedly large:" << h->file->fnode
             << dendl;
  }
}
...
int BlueFS::open_for_read(std::string_view dirname, std::string_view filename,
                          FileReader** h, bool random) /*_N*/
{
  _maybe_check_vselector_LNF();
  std::lock_guard nl(nodes.lock);
 ...
 */
int BlueFS::_do_replay_recovery_read(FileReader* log_reader, size_t replay_pos,
  ...
  dout(2) << __func__
          << " log file encoded extents length = " << bin_extents.length()
          << dendl;
  ...
  int r = _bdev_read(e->bdev, e->offset + e_off, e->length - e_off, &fixed,
                     ioc[e->bdev], cct->_conf->bluefs_buffered_io);
  ceph_assert(r == 0);
  ...
  for (int dcnt = 0; dcnt < 3; dcnt++) {
    ...
    dout(5) << __func__ << " " << disk_regions_count << " regions to scan on "
            << get_device_name(dev) << dendl;
    ...
    for (size_t i = 0; i < disk_regions_count; reg++, i++) {
      ...
      while (len > last_32.length()) {
        ...
        dout(5) << __func__ << " read " << get_device_name(dev) << ":0x"
                << std::hex << pos << "+" << chunk_len << std::dec << dendl;
        r = _bdev_read_random(dev, pos, chunk_len, raw_data + page_size,
                              cct->_conf->bluefs_buffered_io);
        ceph_assert(r == 0);
        ...
        for (char* sp = search_b;; sp += last_32.length()) {
          sp = (char*)memmem(sp, search_e - sp, last_32.c_str(),
                             last_32.length());
          if (sp == nullptr) {
            break;
          }
          ...
          dout(5) << __func__ << " checking location 0x" << std::hex
                  << pos + (n - chunk_b) << std::dec << dendl;
          bufferlist test;
          ...
          if (extents_rejected.count(ne) != 0) {
            dout(5) << __func__ << " extent " << ne << " already refected"
                    << dendl;
            continue;
          }
          ...
          if (ne.bdev >= MAX_BDEV || bdev[ne.bdev] == nullptr ||
              ne.length > 16 * 1024 * 1024 || (ne.length & 4095) != 0 ||
          ...
          r = _bdev_read(ne.bdev, ne.offset, ne.length, &candidate,
                         ioc[ne.bdev], cct->_conf->bluefs_buffered_io);
          ceph_assert(r == 0);
          ...
          dout(5) << __func__ << " successful extension of log " << l << "/"
                  << read_len << dendl;
...
void FitToFastVolumeSelector::get_paths(const std::string& base,
                                        paths& res) const {
