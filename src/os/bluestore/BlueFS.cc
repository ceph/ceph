...
void BlueFS::_init_alloc()
{
  ...
  for (unsigned id = 0; id < bdev.size(); ++id) {
    ...
    if (is_shared_alloc(id)) {
      ...
      // temporary lock candidate regions to forbid their use
      bluefs_locked_extents_t res_la = locked.get_merged();
      if (res_la.head_length) {
        alloc[id]->init_rm_free(res_la.head_offset, res_la.head_length);
      }
      if (res_la.tail_length) {
        alloc[id]->init_rm_free(res_la.tail_offset, res_la.tail_length);
      }
    }
  }
}
...
int BlueFS::mount()
{
  ...
  // temporary unlock allocation regions to permit proper
  // marking from file map.
  for (uint8_t i = 0; i < MAX_BDEV; i++) {
    bluefs_locked_extents_t res_la = locked_alloc[i].get_merged();
    if (res_la.head_length) {
      alloc[i]->init_add_free(res_la.head_offset, res_la.head_length);
    }
    if (res_la.tail_length) {
      alloc[i]->init_add_free(res_la.tail_offset, res_la.tail_length);
    }
  }
