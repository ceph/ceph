...
BlueFS::BlueFS(CephContext* cct)
  : cct(cct),
    m_bluefs_allocator(
      cct->_conf->get_val<std::string>("bluefs_allocator")),
    m_bluefs_alloc_size(
      cct->_conf->get_val<uint64_t>("bluefs_alloc_size")),
    m_bluefs_buffered_io(
      cct->_conf->get_val<bool>("bluefs_buffered_io")),
    m_bluefs_min_flush_size(
      cct->_conf->get_val<uint64_t>("bluefs_min_flush_size")),
    m_bluefs_max_prefetch(
      cct->_conf->get_val<uint64_t>("bluefs_max_prefetch")),
    m_bluefs_max_log_runway(
      cct->_conf->get_val<uint64_t>("bluefs_max_log_runway")),
    m_bluefs_min_log_runway(
      cct->_conf->get_val<uint64_t>("bluefs_min_log_runway")),
    m_bluefs_preextend_wal_files(
      cct->_conf->get_val<bool>("bluefs_preextend_wal_files")),
    m_bluefs_sync_write(
      cct->_conf->get_val<bool>("bluefs_sync_write")),
    bdev(MAX_BDEV),
    ioc(MAX_BDEV),
    block_all(MAX_BDEV)
{
			   ...
			   AllocExtentVector *extents)
{
  ...
  int64_t got = alloc[id]->allocate(want, m_bluefs_alloc_size, 0, extents);
  assert(got != 0);
  if (got < (int64_t)want) {
...
int BlueFS::mkfs(uuid_d osd_uuid)
{
  ...
  int r = _allocate(log_file->fnode.prefer_bdev, m_bluefs_max_log_runway,
    &log_file->fnode);
  assert(r == 0);
...
void BlueFS::_init_alloc()
{
  ...
  for (unsigned id = 0; id < bdev.size(); ++id) {
    ...
    alloc[id] = Allocator::create(cct, m_bluefs_allocator, bdev[id]->get_size(),
      m_bluefs_alloc_size);
...
int BlueFS::_replay(bool noop, bool to_stdout)
{
  ...
  FileReader *log_reader = new FileReader(log_file, m_bluefs_max_prefetch,
    false,  // !random
    true);  // ignore eof
  while (true) {
...
{
  std::unique_lock<std::mutex> l(lock);
  auto compact_log_sync = cct->_conf->get_val<bool>("bluefs_compact_log_sync");
  if (compact_log_sync) {
     _compact_log_sync();
  } else {
...
bool BlueFS::_should_compact_log()
{
  ...
  dout(10) << __func__ << " current 0x" << std::hex << current
	   ...
	   << dendl;
  auto log_compact_min_size = cct->_conf->get_val<uint64_t>(
    "bluefs_log_compact_min_size");
  auto log_compact_min_ratio = cct->_conf->get_val<double>(
    "bluefs_log_compact_min_ratio");
  if (new_log || log_compact_min_size || ratio < log_compact_min_ratio) {
    return false;
  }
  return true;
}
...
void BlueFS::_compact_log_sync()
{
  ...
  uint64_t need = bl.length() + m_bluefs_max_log_runway;
...
void BlueFS::_compact_log_async(std::unique_lock<std::mutex>& l)
{
  ...
  uint64_t need = old_log_jump_to + m_bluefs_max_log_runway;
				...
				uint64_t jump_to)
{
  ...
  if (runway < (int64_t)m_bluefs_min_log_runway) {
    ...
    int r = _allocate(log_writer->file->fnode.prefer_bdev,
      m_bluefs_max_log_runway, &log_writer->file->fnode);
    assert(r == 0);
...
int BlueFS::_flush_range(FileWriter *h, uint64_t offset, uint64_t length)
{
  ...
  if (allocated < offset + length) {
    ...
    if (m_bluefs_preextend_wal_files && h->writer_type == WRITER_WAL) {
...
int BlueFS::_flush(FileWriter *h, bool force)
{
  ...
  if (!force && length < m_bluefs_min_flush_size) {
    dout(10) << __func__ << " " << h << " ignoring, length " << length
	     << " < min_flush_size " << m_bluefs_min_flush_size << dendl;
    return 0;
  }
  if (length == 0) {
		      ...
		      bluefs_fnode_t* node)
{
  ...
  uint64_t left = ROUND_UP_TO(len, m_bluefs_alloc_size);
  int r = -ENOSPC;
  if (alloc[id]) {
  ...
  int64_t alloc_len = alloc[id]->allocate(left, m_bluefs_alloc_size, hint,
                          &extents);
  if (alloc_len < (int64_t)left) {
    derr << __func__ << " allocate failed on 0x" << std::hex << left
	 << " min_alloc_size 0x" << m_bluefs_alloc_size 
...
void BlueFS::sync_metadata()
{
  ...
  if (_should_compact_log()) {
    auto compact_log_sync = cct->_conf->get_val<bool>(
  ...
  bool random)
{
  ...
  *h = new FileReader(
    file, random ? 4096 : m_bluefs_max_prefetch, random, false);
  dout(10) << __func__ << " h " << *h << " on " << file->fnode << dendl;
  return 0;
}
