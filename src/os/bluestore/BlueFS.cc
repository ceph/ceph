                             ...
                             bluefs_shared_alloc_context_t* _shared_alloc)
{
  ...
  if (trim) {
    ...
    in_discard += whole_device.size();
    if( !b->try_discard(whole_device, false)) {
      drop_in_discard(whole_device.size());
    }
  }
...
void BlueFS::drop_in_discard(uint64_t delta)
{
  ceph_assert(delta <= in_discard);
  in_discard -= delta;
  if (wait_discard && in_discard == 0) {
...
void BlueFS::_release_pending_allocations(vector<interval_set<uint64_t>>& to_release)
{
  for (unsigned i = 0; i < to_release.size(); ++i) {
    ...
    if (!discard_queued) {
      ...
      drop_in_discard(to_release[i].size());
    }
  }
}
