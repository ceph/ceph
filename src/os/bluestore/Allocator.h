...
public:
  ...
  */
   ...
   * 
   * Returned value is a cursor to potentially use in next get_free_extents() call:
   *   - if the cursor is >= range_end, the window has been fully enumerated;
   *   - otherwise the remaining window [cursor, range_end) might contain
   *     extents;
   *
  ...
  *     large (it is the clipped span of one free extent).
   *   - The cursor is an opaque offset; do not assume it lands on an extent
   *     boundary. Only the >= range_end termination test is meaningful.
   *
   * Preconditions: 
