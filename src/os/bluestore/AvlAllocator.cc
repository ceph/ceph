 ...
 */
uint64_t AvlAllocator::_pick_block_after(uint64_t* cursor, uint64_t size,
                                         uint64_t align) {
  ...
  if (*cursor == 0) {
    // If we already started from beginning, don't bother with searching from
    // beginning
    return -1ULL;
  }
...
void AvlAllocator::_add_to_tree(uint64_t start, uint64_t size) {
  ...
  if ((rs_before != range_tree.end() && rs_before->end > start) ||
    ...
    derr << __func__ << " inconsistent tree state " << std::hex << " 0x"
         << start << "~" << end - start << std::dec << dendl;
    _dump();
  }
...
void AvlAllocator::_process_range_removal(
    uint64_t start, uint64_t end, AvlAllocator::range_tree_t::iterator& rs) {
...
void AvlAllocator::_remove_from_tree(uint64_t start, uint64_t size) {
  ...
  if (rs == range_tree.end() || rs->start > start || rs->end < end) {
    derr << __func__ << " inconsistent tree state " << std::hex << " 0x"
         << rs->start << "~" << rs->end - rs->start << std::dec << dendl;
    _dump();
  }
...
int AvlAllocator::_allocate(uint64_t size, uint64_t unit, uint64_t* offset,
  ...
  uint64_t start = 0;
  // If we're running low on space, find a range by size by looking up in the
  // size sorted tree (best-fit), instead of searching in the area pointed by
  // cursor
  if (force_range_size_alloc || max_size < range_size_alloc_threshold ||
    ...
    dout(20) << __func__ << std::hex << " first fit params: 0x" << start << "~"
             << size << std::dec << dendl;
  }
  if (start == -1ULL) {
    do {
      start = _pick_block_fits(size, unit);
      dout(20) << __func__ << std::hex << " best fit params: 0x" << start << "~"
               << size << std::dec << dendl;
  ...
  dout(20) << __func__ << " allocated 0x" << std::hex << start << "~" << size
           << std::dec << dendl;
...
void AvlAllocator::_release(const interval_set<uint64_t>& release_set) {
  for (auto p = release_set.begin(); p != release_set.end(); ++p) {
    ...
    ldout(cct, 20) << __func__ << std::hex << " 0x" << offset << "~" << length
                   << std::dec << dendl;
...
void AvlAllocator::_release(const PExtentVector& release_set) {
  for (auto& e : release_set) {
    ldout(cct, 20) << __func__ << std::hex << " 0x" << e.offset << "~"
...
AvlAllocator::AvlAllocator(CephContext* cct, int64_t device_size,
                           int64_t block_size, uint64_t max_mem,
    ...
    : Allocator(name, device_size, block_size),
      ...
      max_search_count(cct->_conf.get_val<uint64_t>(
          "bluestore_avl_alloc_ff_max_search_count")),
      max_search_bytes(cct->_conf.get_val<Option::size_t>(
...
AvlAllocator::~AvlAllocator() { shutdown(); }

int64_t AvlAllocator::allocate(uint64_t want, uint64_t unit,
                               uint64_t max_alloc_size,
                               int64_t hint,  // unused, for now!
                               PExtentVector* extents) {
  ldout(cct, 10) << __func__ << std::hex << " 0x" << want << "/" << unit << ","
                 << max_alloc_size << "," << hint << std::dec << dendl;
...
void AvlAllocator::init_add_free(uint64_t offset, uint64_t length) {
  ldout(cct, 10) << __func__ << std::hex << " 0x" << offset << "~" << length
                 << std::dec << dendl;
  if (!length) return;
  std::lock_guard l(lock);
  ceph_assert(offset + length <= uint64_t(device_size));
