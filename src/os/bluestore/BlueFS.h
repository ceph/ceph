...
private:
  ...
  std::atomic<uint64_t> in_discard = 0;
  std::atomic<uint64_t> wait_discard = 0;
  ceph::condition_variable discard_cond;
  ceph::mutex discard_lock = ceph::make_mutex("BlueFS::discard_lock");
  ...
  void drop_in_discard(uint64_t delta);
