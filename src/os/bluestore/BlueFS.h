...
class BlueFSVolumeSelector {
 public:
  ...
  /**
   *  Increments stats for a given logical level using provided fnode as a
   * delta, Parameters: hint: logical level discriminator fnode: fnode metadata
   * to be used as a complex delta value:
  ...
  /**
   *  Increments stats for a given logical level using provided extent as a
   * delta, Parameters: hint: logical level discriminator extent: bluefs extent
   * to be used as a complex delta value:
  ...
  /**
   *  Decrements stats for a given logical level using provided extent as a
   * delta, Parameters: hint: logical level discriminator extent: bluefs extent
   * to be used as a complex delta value:
   *           (.bdev determines physical location, -length)
   *
   ...
   *
   */
  virtual void add_usage(void* hint, uint64_t fsize,
                         bool upd_files = false) = 0;
  /**
   ...
   *
   */
  virtual void sub_usage(void* hint, uint64_t fsize,
                         bool upd_files = false) = 0;
...
class BlueFS {
 public:
  ...
  struct File : public RefCountedObject {
    ...
    /* lock protects fnode and other the parts that can be modified during read
       & write operations. Does not protect values that are fixed Does not need
       to be taken when doing one-time operations: _replay,
       device_migrate_to_existing, device_migrate_to_new */
  ...
  struct FileWriter {
   ...
   public:
    ...
    void append(const std::byte* buf, size_t len) {
      // allow callers to use byte type instead of char* as we simply pass byte
      // array
 ...
 private:
  ...
  struct {
    ...
    // 2) we usually not remove extents from files. And when we do, we force
    // log-syncing.
  } dirty;
  ...
  int _allocate(uint8_t bdev, uint64_t len, uint64_t alloc_unit,
                bluefs_fnode_t* node, update_fn_t cb = nullptr,
                size_t alloc_attempts = 0, bool permit_dev_fallback = true);
  ...
  uint64_t _make_initial_transaction(uint64_t start_seq, bluefs_fnode_t& fnode,
  ...
  void _compact_log_dump_metadata_NF(uint64_t start_seq,
                                     bluefs_transaction_t* t, int flags,
  ...
  void _rewrite_log_and_layout_sync_LNF_LD(
      bool permit_dev_fallback, int super_dev, int log_dev, int new_log_dev,
 ...
 public:
  ...
  int device_migrate_to_new(CephContext* cct, const std::set<int>& devs_source,
  ...
  void foreach_block_extents(unsigned id,
                             std::function<void(uint64_t, uint32_t)> cb);

  int open_for_write(std::string_view dir, std::string_view file,
                     FileWriter** h, bool overwrite);

  int open_for_read(std::string_view dir, std::string_view file, FileReader** h,
                    bool random = false);
  ...
  int stat(std::string_view dirname, std::string_view filename, uint64_t* size,
           utime_t* mtime);
 ...
 private:
  ...
  int _read_random_and_check(uint8_t ndev, uint64_t off, uint64_t len,
                             char* buf, bool buffered);

  int _bdev_read(uint8_t ndev, uint64_t off, uint64_t len,
                 ceph::buffer::list* pbl, IOContext* ioc, bool buffered);
  int _bdev_read_random(uint8_t ndev, uint64_t off, uint64_t len, char* buf,
                        bool buffered);

  /// test and compact log, if necessary
  void _maybe_compact_log_LNF_NF_LD_D();
  int _do_replay_recovery_read(FileReader* log, size_t log_pos,
...
class FitToFastVolumeSelector : public OriginalVolumeSelector {
 public:
  FitToFastVolumeSelector(uint64_t _wal_total, uint64_t _db_total,
                          uint64_t _slow_total)
