  ...
  PExtentVector *extents)
{
  ...
  ldout(cct, 10) << __func__ << std::hex << " zone 0x" << zone_num
		 << " offset is now " << get_write_pointer(zone_num) << std::dec << dendl;

  ldout(cct, 10) << __func__ << " allocated 0x" << std::hex << want_size
