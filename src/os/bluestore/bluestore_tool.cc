...
void add_devices(BlueFS* fs, CephContext* cct, const vector<string>& devs) {
  ...
  for (auto e : got) {
    ...
    // We provide no shared allocator which prevents bluefs to operate in R/W
    // mode. Read-only mode isn't strictly enforced though
...
BlueFS* open_bluefs_readonly(CephContext* cct, const string& path,
                             const vector<string>& devs) {
...
static void bluefs_import(const string& input_file, const string& dest_file,
                          CephContext* cct, const string& path,
                          const vector<string>& devs) {
  int r;
...
int main(int argc, char** argv) {
  ...
  if (log_file.size()) {
    ...
    // do not write to default-named log "osd.x.log" if --log-file is not
    // provided
  ...
  if (action == "trim") {
    ...
    if (really.empty() ||
        strcmp(really.c_str(), "--yes-i-really-really-mean-it") != 0) {
  ...
  if (action == "restore_cfb") {
    ...
    auto [target_path,
          has_size_spec] = [&dev_target]() -> std::pair<string, bool> {
      if (dev_target.empty()) {
        return {"", false};
      }
      std::error_code ec;
    ...
    ceph::bufferlist in, out;
    ostringstream err;
    r = admin_socket->execute_command({"{\"prefix\": \"bluefs stats\"}"}, in,
                                      err, &out);
    if (r != 0) {
