...
BitmapAllocator::BitmapAllocator(CephContext* _cct, int64_t capacity,
                                 int64_t alloc_unit, std::string_view name)
    : Allocator(name, capacity, alloc_unit), cct(_cct) {
...
int64_t BitmapAllocator::allocate(uint64_t want_size, uint64_t alloc_unit,
  ...
  _allocate_l2(want_size, alloc_unit, max_alloc_size, hint, &allocated,
               extents);
  if (!allocated) {
    return -ENOSPC;
  }
  if (cct->_conf->subsys.should_gather<dout_subsys, 10>()) {
    for (auto i = old_size; i < extents->size(); ++i) {
      auto& e = (*extents)[i];
      ldout(cct, 10) << __func__ << " extent: 0x" << std::hex << e.offset << "~"
                     << e.length << "/" << alloc_unit << "," << max_alloc_size
                     << "," << hint << std::dec << dendl;
    }
  }
...
void BitmapAllocator::dump() {
  ...
  while (it != bins_overall.end()) {
    ldout(cct, 0) << __func__ << " bin " << it->first << "(< "
                  << byte_u_t((1 << (it->first + 1)) * get_min_alloc_size())
                  << ")"
                  << " : " << it->second << " extents" << dendl;
    ++it;
  }
}
