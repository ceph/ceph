...
void bluefs_super_t::dump(Formatter* f) const {
  ...
  for (auto& p : bluefs_max_alloc_size) f->dump_unsigned("max_alloc_size", p);
}
...
ostream& operator<<(ostream& out, const bluefs_super_t& s) {
  return out << "super(uuid " << s.uuid << " osd " << s.osd_uuid << " v "
             << s.version << " block_size 0x" << std::hex << s.block_size
             << " log_fnode 0x" << s.log_fnode << " max_alloc_size "
             << s.bluefs_max_alloc_size << std::dec << ")";
}
...
mempool::bluefs::vector<bluefs_extent_t>::iterator bluefs_fnode_t::seek(
  ...
  if (extents_index.size() > 4) {
    auto it =
        std::upper_bound(extents_index.begin(), extents_index.end(), offset);
...
bluefs_fnode_delta_t* bluefs_fnode_t::make_delta(bluefs_fnode_delta_t* delta) {
  ...
  if (allocated_commited < allocated) {
    ...
    if (x_off > 0) {
      ...
      delta->extents.emplace_back(p->bdev, p->offset + x_off,
                                  p->length - x_off);
      ++p;
    }
    while (p != extents.end()) {
...
void bluefs_fnode_t::dump(Formatter* f) const {
  ...
  for (auto& p : extents) f->dump_object("extent", p);
  f->close_section();
}
...
ostream& operator<<(ostream& out, const bluefs_fnode_t& file) {
  return out << "file(ino " << file.ino << " size 0x" << std::hex << file.size
             << std::dec << " mtime " << file.mtime << " allocated " << std::hex
             << file.allocated << std::dec << " alloc_commit " << std::hex
             << file.allocated_commited << std::dec << " extents "
             << file.extents << ")";
}
...
std::ostream& operator<<(std::ostream& out, const bluefs_fnode_delta_t& delta) {
  return out << "delta(ino " << delta.ino << " size 0x" << std::hex
             << delta.size << std::dec << " mtime " << delta.mtime << " offset "
             << std::hex << delta.offset << std::dec << " extents "
             << delta.extents << ")";
}
...
void bluefs_transaction_t::encode(bufferlist& bl) const {
  ...
  encode(uuid, bl);
  encode(seq, bl);
  // not using bufferlist encode method, as it merely copies the bufferptr and
  // not contents, meaning we're left with fragmented target bl
...
ostream& operator<<(ostream& out, const bluefs_transaction_t& t) {
  return out << "txn(seq " << t.seq << " len 0x" << std::hex << t.op_bl.length()
             << " crc 0x" << t.op_bl.crc32c(-1) << std::dec << ")";
}
