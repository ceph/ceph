 ...
 */
class WholeSpaceOmniIterator : public KeyValueDB::WholeSpaceIteratorImpl {
protected:
  OmniKVStore *db;
  bool ready;
...
public:
  WholeSpaceOmniIterator(OmniKVStore *db) : db(db), ready(false) { }
  virtual ~WholeSpaceOmniIterator() { }
...
int OmniKVStore::get(const string &prefix,
...
int OmniKVStore::get_keys(const string &prefix,
			       const std::set<string> &key,
			       std::set<string> *out) {
  if (!exists_prefix(prefix))
    return 0;
...
int OmniKVStore::set(const string &prefix,
			  const string &key,
			  const bufferlist &bl) {
...
int OmniKVStore::rmkey(const string &prefix,
			    const string &key) {
  db.erase(make_pair(prefix,key));
  return 0;
}

int OmniKVStore::rmkeys_by_prefix(const string &prefix) {
...
KeyValueDB::WholeSpaceIterator OmniKVStore::_get_iterator() {
  return ceph::shared_ptr<KeyValueDB::WholeSpaceIteratorImpl>(
    new WholeSpaceOmniIterator(this)
  );
}

class WholeSpaceSnapshotOmniIterator : public WholeSpaceOmniIterator {
public:
  ...
  WholeSpaceSnapshotOmniIterator(OmniKVStore *db) :
    WholeSpaceOmniIterator(db) { }
  ~WholeSpaceSnapshotOmniIterator() {
    delete db;
  }
};

KeyValueDB::WholeSpaceIterator OmniKVStore::_get_snapshot_iterator() {
  OmniKVStore *snap_db = new OmniKVStore(this);
  return ceph::shared_ptr<KeyValueDB::WholeSpaceIteratorImpl>(
    new WholeSpaceSnapshotOmniIterator(snap_db)
  );
}
