    ...
    const coll_t &cid, const ghobject_t &oid)
{
  ...
  while (1) {
    ...
    uint64_t seq = header_cache.get_seq(cid, oid);
    if (seq && header_cache.lookup(seq, header)) {
      break;
    }
    ...
    if (!try_again) {
      header_cache.add(header.seq, header);
      break;
    }
  }
...
GenericObjectMap::Header GenericObjectMap::lookup_parent(Header input)
{
  ...
  _Header header;
  if (header_cache.lookup(input->parent, header)) {
    goto out;
  }

  r = db->get(parent_seq_prefix(input->parent), keys, &out);
  if (r < 0) {
    assert(0);
    return Header();
  }
...

  {
    ...
    // Parent seq doesn't has corresponding ghobject_t, we won't search it so just
    // leave it blank
    header_cache.add(header.seq, header);
  }
