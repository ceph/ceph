    ...
    uint32_t op_flags)
{
  ...
  while (length > 0) {
	...
	if (stripe_off < stripe.length()) {
	  l = std::min<unsigned>(stripe.length() - stripe_off, swant);
	  bufferlist t;
		      ...
		      uint32_t fadvise_flags)
{
  ...
  while (length > 0) {
    ...
    bufferlist bl;
    if (offset_rem) {
      unsigned p = std::min<unsigned>(prev.length(), offset_rem);
      if (p) {
		  ...
		  uint64_t offset, size_t length)
{
  ...
  if (stripe_size) {
	...
	dout(30) << __func__ << " stripe " << pos - stripe_off << " got "
		 << stripe.length() << dendl;
	bufferlist bl;
	bl.substr_of(stripe, 0, std::min<unsigned>(stripe.length(), stripe_off));
...
int KStore::_do_truncate(TransContext *txc, OnodeRef o, uint64_t offset)
{
  ...
  if (stripe_size) {
	...
	dout(30) << __func__ << " stripe " << pos - stripe_off << " got "
		 << stripe.length() << dendl;
	bufferlist t;
	t.substr_of(stripe, 0, std::min<unsigned>(stripe_off, stripe.length()));
