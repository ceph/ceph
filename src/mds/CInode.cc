			     ...
			     int getattr_caps)
{
  ...
  if (session->info.has_feature(CEPHFS_FEATURE_REPLY_ENCODING)) {
    ENCODE_START(7, 1, bl);
    encode(std::tuple{oi->ino, snapid, oi->rdev, version, xattr_version}, bl, 0);
    encode(ecap, bl);
    {
    ...
    encode(std::tuple{any_i->ctime, file_i->mtime, file_i->atime, file_i->time_warp_seq, file_i->size, max_size, file_i->truncate_size, file_i->truncate_seq, auth_i->mode, (uint32_t)auth_i->uid, (uint32_t)auth_i->gid, link_i->nlink, file_i->dirstat.nfiles, file_i->dirstat.nsubdirs, file_i->rstat.rbytes, file_i->rstat.rfiles, file_i->rstat.rsubdirs, file_i->rstat.rctime}, bl, 0);
