...
static const char * quiesce_state_name(QuiesceState state) {
  switch (state) {
  case QS__INVALID:
    return "<invalid>";
  case QS_QUIESCING:
...
using QuiesceRoot = std::string; // TODO: file path?
using QuiesceDbVersion = uint32_t;
...
struct QuiesceSet {
  ...
  /// @param member_state the state of the member
  /// @param set_state the state of the set this member is from
  /// @return the effective member state
  QuiesceState get_effective_member_state(QuiesceState member_state) const
  {
    if (is_releasing()) {
      if (member_state >= QS_QUIESCED && member_state <= QS_RELEASED) {
        return QS_RELEASED;
      }
    }
    return std::max(member_state, rstate.state);
  }
...
struct QuiesceDbRequest {
  ...
  ///        Requires a set_id.
  std::optional<QuiesceDbVersion> if_version;
...
static QuiesceTimeInterval interval_saturate_add(QuiesceTimeInterval lhs, QuiesceTimeInterval rhs)
{
...
static QuiesceTimePoint interval_saturate_add_now(QuiesceTimeInterval interval) {
...
namespace QuiesceInterface {
  ...
  using AgentAck = std::function<int(const QuiesceMap&)>;
  /// @brief  Used by the leader to replicate the DB changes to its peers
  using DbPeerUpdate = std::function<int(mds_rank_t, const QuiesceDbListing&)>;
