...
public:
  ...
  CDentry* add_primary_dentry(std::string_view dname, CInode *in, mempool::mds_co::string alternate_name,
			      snapid_t first=2, snapid_t last=CEPH_NOSNAP);
  CDentry* add_remote_dentry(std::string_view dname, inodeno_t ino, inodeno_t referent_ino,
  ...
  void set_referent_inode(CDentry *dn, CInode *ref_in, bool link_referent_inode=true);
  void link_referent_inode(CDentry *dn, CInode *in, inodeno_t ino, inodeno_t referent_ino, unsigned char d_type);
