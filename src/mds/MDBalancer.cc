...
int MDBalancer::rank_mask_list_str_to_bitset(CInode *cur, std::string& rank_mask_list_str, std::bitset<MAX_MDS>& rank_mask_bitset, std::ostream& ss)
{
  ...
  std::bitset<MAX_MDS> temp_mask_bitset;
...
std::string MDBalancer::bitmask_to_str(std::bitset<MAX_MDS> &bitmask)
{
...
void MDBalancer::handle_heartbeat(const cref_t<MHeartbeat> &m)
{
  ...
  std::bitset<MAX_MDS> my_bal_rank_mask_bitset;
  std::vector<CDir*> authsubs = mds->mdcache->get_auth_subtrees();
  int overlap_count = 0;
  for (auto &cd : authsubs) {
    std::bitset<MAX_MDS> temp_bitset;
    ...
    if (temp_bitset.test(mds->get_nodeid())) {
      ...
      overlap_count++;
      if (overlap_count > 1)
        dout(10) << " rank_mask overlapped subdir " << *cd << dendl;
    }
  }

  if (overlap_count > 1) {
    dout(10) << " bal_rank_mask values of multiple subdirs have been overlapped, value: " << overlap_count << dendl;
  }
  ...
  if (num_mdss_in_rank_mask_bitset)
  {
    ...
    dout(10) << "Nothing to do because rank mast bitset are cleared "<< bitmask_to_str(bal_rank_mask_bitset) << dendl;
  }
}
