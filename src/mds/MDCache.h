...
class MDCache {
 public:
  ...
  void get_wouldbe_subtree_bound_inodes(CDir *root, std::set<CInode*>& bound_inodes);
 ...
 protected:
  ...
  std::set<mds_rank_t> peer_resolve_sent;
  std::set<mds_rank_t> peer_resolve_gather;
  std::set<mds_rank_t> peer_resolve_ack_gather;	// nodes i need a resolve_ack from
  bool subtree_resolves_pending = false;
  std::set<mds_rank_t> subtree_resolve_sent;	// nodes i need resolves from
  std::set<mds_rank_t> subtree_resolve_gather;	// nodes i need resolves from
  ...
  struct subtree_info_t {
    ...
    std::map<mds_rank_t, fragset_t> rank_frags;
  };
  std::map<inodeno_t, subtree_info_t> resolve_learned_subtrees;
