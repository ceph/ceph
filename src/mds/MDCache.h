...
class MDCache {
 public:
  ...
  typedef std::map<mds_rank_t, ceph::ref_t<MCacheExpire>> expiremap;
...
public:
  ...
  MDRequestRef request_start(const ceph::cref_t<MClientRequest>& req);
  MDRequestRef request_start_slave(metareqid_t rid, __u32 attempt, const ceph::cref_t<Message>& m);
...
protected:
  ...
  map<mds_rank_t, ceph::cref_t<MMDSResolve>> delayed_resolve;
  
  void handle_resolve(const ceph::cref_t<MMDSResolve>& m);
  void handle_resolve_ack(const ceph::cref_t<MMDSResolveAck>& m);
...
protected:
  ...
  map<client_t,pair<SessionRef,uint64_t> > rejoin_session_map;
  ...
  void rejoin_walk(CDir *dir, const ceph::ref_t<MMDSCacheRejoin>& rejoin);
  void handle_cache_rejoin(const ceph::cref_t<MMDSCacheRejoin>& m);
  void handle_cache_rejoin_weak(const ceph::cref_t<MMDSCacheRejoin>& m);
  CInode* rejoin_invent_inode(inodeno_t ino, snapid_t last);
  CDir* rejoin_invent_dirfrag(dirfrag_t df);
  void handle_cache_rejoin_strong(const ceph::cref_t<MMDSCacheRejoin>& m);
  void rejoin_scour_survivor_replicas(mds_rank_t from, const ceph::cref_t<MMDSCacheRejoin>& ack,
				      set<vinodeno_t>& acked_inodes,
				      set<SimpleLock *>& gather_locks);
  void handle_cache_rejoin_ack(const ceph::cref_t<MMDSCacheRejoin>& m);
...
public:
  ...
  void rejoin_open_sessions_finish(map<client_t,pair<SessionRef,uint64_t> >& session_map);
  bool process_imported_caps();
  void choose_lock_states_and_reconnect_caps();
  void prepare_realm_split(SnapRealm *realm, client_t client, inodeno_t ino,
			   map<client_t,ceph::ref_t<MClientSnap>>& splits);
  void prepare_realm_merge(SnapRealm *realm, SnapRealm *parent_realm, map<client_t,ceph::ref_t<MClientSnap>>& splits);
  void send_snaps(map<client_t,ceph::ref_t<MClientSnap>>& splits);
  Capability* rejoin_import_cap(CInode *in, client_t client, const cap_reconnect_t& icr, mds_rank_t frommds);
  void finish_snaprealm_reconnect(client_t client, SnapRealm *realm, snapid_t seq,
				  map<client_t,ceph::ref_t<MClientSnap>>& updates);
  Capability* try_reconnect_cap(CInode *in, const SessionRef& session);
  ...
  void do_cap_import(const SessionRef& session, CInode *in, Capability *cap,
...
protected:
  ...
  void _open_ino_fetch_dir(inodeno_t ino, const ceph::cref_t<MMDSOpenIno>& m, CDir *dir, bool parent);
  int open_ino_traverse_dir(inodeno_t ino, const ceph::cref_t<MMDSOpenIno>& m,
  ...
  void handle_open_ino(const ceph::cref_t<MMDSOpenIno>& m, int err=0);
  void handle_open_ino_reply(const ceph::cref_t<MMDSOpenInoReply>& m);
...
public:
  ...
  void handle_find_ino(const ceph::cref_t<MMDSFindIno>& m);
  void handle_find_ino_reply(const ceph::cref_t<MMDSFindInoReply>& m);
...
public:
  ...
  void handle_snap_update(const ceph::cref_t<MMDSSnapUpdate>& m);
...
protected:
  ...
  // == messages ==
 public:
  void dispatch(const ceph::cref_t<Message>& m);

 protected:
  // -- replicas --
  void handle_discover(const ceph::cref_t<MDiscover>& dis);
  void handle_discover_reply(const ceph::cref_t<MDiscoverReply>& m);
...
protected:
  void handle_dentry_link(const ceph::cref_t<MDentryLink>& m);
  void handle_dentry_unlink(const ceph::cref_t<MDentryUnlink>& m);
...
private:
  ...
  void handle_fragment_notify(const ceph::cref_t<MMDSFragmentNotify>& m);
  void handle_fragment_notify_ack(const ceph::cref_t<MMDSFragmentNotifyAck>& m);
...
public:
  ...
  void handle_dir_update(const ceph::cref_t<MDirUpdate>& m);

  // -- cache expiration --
  void handle_cache_expire(const ceph::cref_t<MCacheExpire>& m);
