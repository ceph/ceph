...
LogSegmentRef MDLog::_start_new_segment(SegmentBoundary* sb)
{
  ...
  // add a ref to the LogSegment since we have created a LogSegmentRef out of it
  ls->get();
...
void MDLog::_replay_thread()
{
  ...
  while (1) {
    ...
    if (auto sb = dynamic_cast<SegmentBoundary*>(le.get()); sb) {
      ...
      segments[event_seq]->get(); // add a ref since we are creating a LogSegmentRef out of it
      logger->set(l_mdl_seg, segments.size());
      if (sb->is_major_segment_boundary()) {
