...
void Migrator::handle_mds_failure_or_stop(mds_rank_t who)
{
  ...
  while (q != import_state.end()) {
    ...
    if (g_conf()->mds_kill_import_at == 10)
      _exit(120);
	}
	break;
...
void Migrator::dispatch_export_dir(const MDRequestRef& mdr, int count)
{
  ...
  if (g_conf()->mds_kill_export_at == 1)
    _exit(120);
  ...
  if (results.size() == 1 && results.front().first == dir) {
    ...
    if (g_conf()->mds_kill_export_at == 2)
      _exit(120);
...
void Migrator::handle_export_discover_ack(const cref_t<MExportDirDiscoverAck> &m)
{
  ...
  if (it == export_state.end() ||
    ...
    if (m->is_success()) {
      ...
      if (g_conf()->mds_kill_export_at == 3)
        _exit(120);
...
void Migrator::export_frozen(CDir *dir, uint64_t tid)
{
  ...
  if (g_conf()->mds_kill_export_at == 4)
    _exit(120);
...
void Migrator::handle_export_prep_ack(const cref_t<MExportDirPrepAck> &m)
{
  ...
  if (g_conf()->mds_kill_export_at == 5)
    _exit(120);
  // send warnings
  set<CDir*> bounds;
  mdcache->get_subtree_bounds(dir, bounds);
  ...
  if (g_conf()->mds_kill_export_at == 6)
    _exit(120);
  // nobody to warn?
...
void Migrator::export_go_synced(CDir *dir, uint64_t tid)
{
  ...
  if (g_conf()->mds_kill_export_at == 7)
    _exit(120);
  ...
  if (g_conf()->mds_kill_export_at == 8)
    _exit(120);
...
void Migrator::handle_export_ack(const cref_t<MExportDirAck> &m)
{
  ...
  if (g_conf()->mds_kill_export_at == 9)
    _exit(120);
  set<CDir*> bounds;
  mdcache->get_subtree_bounds(dir, bounds);
  ...
  if (g_conf()->mds_kill_export_at == 10)
    _exit(120);
...
void Migrator::export_logged_finish(CDir *dir)
{
  ...
  if (g_conf()->mds_kill_export_at == 11)
    _exit(120);
...
void Migrator::export_finish(CDir *dir)
{
  ...
  if (g_conf()->mds_kill_export_at == 12)
    _exit(120);
  map<CDir*,export_state_t>::iterator it = export_state.find(dir);
  if (it == export_state.end()) {
  ...
  if (g_conf()->mds_kill_export_at == 13)
    _exit(120);
...
void Migrator::handle_export_discover(const cref_t<MExportDirDiscover> &m, bool started)
{
  ...
  if (g_conf()->mds_kill_import_at == 1)
    _exit(120);
  ...
  if (g_conf()->mds_kill_import_at == 2)
    _exit(120);
}
...
void Migrator::handle_export_prep(const cref_t<MExportDirPrep> &m, bool did_assim)
{
  ...
  if (!did_assim) {
    ...
    if (g_conf()->mds_kill_import_at == 3)
      _exit(120);
  ...
  if (g_conf()->mds_kill_import_at == 4)
    _exit(120);
}
...
void Migrator::handle_export_dir(const cref_t<MExportDir> &m)
{
  ...
  if (g_conf()->mds_kill_import_at == 5)
    _exit(120);
  ...
  if (g_conf()->mds_kill_import_at == 6)
    _exit(120);
				   ...
				   map<client_t,pair<Session*,uint64_t> >& imported_session_map)
{
  ...
  if (g_conf()->mds_kill_import_at == 7)
    _exit(120);
  ...
  if (g_conf()->mds_kill_import_at == 8)
    _exit(120);
...
void Migrator::import_finish(CDir *dir, bool notify, bool last)
{
  ...
  if (g_conf()->mds_kill_import_at == 9)
    _exit(120);
