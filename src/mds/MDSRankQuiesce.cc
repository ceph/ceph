...
void MDSRank::quiesce_cluster_update() {
  ...
  // otherwise, the lowest rank that's standby
  auto less_leader = [](MDSMap::mds_info_t const* l, MDSMap::mds_info_t const* r) {
    ...
    if (l->rank == r->rank) {
      return l->state != MDSMap::STATE_ACTIVE;
    } else {
  ...
  for (auto&& [gid, info] : mdsmap->get_mds_info()) {
    // if it has a rank, it's good enough for us
    if (info.join_fscid == membership.fs_id && info.rank != MDS_RANK_NONE) {
  ...
  if (!is_active()) {
    quiesce_db_manager->reset_agent_callback([whoami = whoami, is_sr = is_standby_replay()](QuiesceMap& quiesce_map) {
      for (auto it = quiesce_map.roots.begin(); it != quiesce_map.roots.end();) {
        switch (it->second.state) {
        case QS_QUIESCING:
          if (is_sr) {
            it->second.state = QS_QUIESCED;
            dout(15) << "STANDBY REPLAY: reporting '" << it->first << "' as " << it->second.state << dendl;
          } else {
            it->second.state = QS_FAILED;
            dout(3) << "INACTIVE RESPONDER: reporting '" << it->first << "' as " << it->second.state << dendl;
          }
          it++;
          break;
        default:
