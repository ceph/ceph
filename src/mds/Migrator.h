...
protected:
  ...
  struct import_state_t {
    ...
    std::map<client_t,pair<ceph::ref_t<class Session>,uint64_t> > session_map;
  ...
  void import_logged_start(dirfrag_t df, CDir *dir, mds_rank_t from, std::map<client_t,pair<ref_t<class Session>,uint64_t>>& imported_session_map);
  ...
  void logged_import_caps(CInode *in,
			  mds_rank_t from,
			  std::map<client_t,pair<ceph::ref_t<class Session>,uint64_t> >& imported_session_map,
...
public:
  ...
  void finish_import_inode_caps(CInode *in, mds_rank_t from, bool auth_cap,
				const std::map<client_t,pair<ceph::ref_t<class Session>,uint64_t> >& smap,
