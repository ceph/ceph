...
public:
  ...
  void reset_inode(const inode_const_ptr& ptr) {
    inode = ptr;
  }
  void reset_xattrs(const xattr_map_const_ptr& ptr) {
    xattrs = ptr;
  }
  void reset_old_inodes(const old_inode_map_const_ptr& ptr) {
    old_inodes = ptr;
  }
...
public:
  mempool_inode* get_inode() const { return const_cast<mempool_inode*>(inode.get()); }
  mempool_xattr_map* get_xattrs() const { return const_cast<mempool_xattr_map*>(xattrs.get()); }
...
class CInode : public MDSCacheObject, public InodeStoreBase, public Counter<CInode> {
 public:
  ...
  struct projected_inode {
    ...
    explicit projected_inode(const inode_ptr& i, const xattr_map_ptr& x, sr_t *s) :
      inode(*i), xattrs(x.get()), snapnode(s) {}
  };
  ...
  inode_ptr _get_projected_inode() {
    ceph_assert(!projected_nodes.empty());
    return const_inode_cast(projected_nodes.back().inode);
  }
  ...
  inode_ptr _get_inode() const { return const_inode_cast(inode); }
