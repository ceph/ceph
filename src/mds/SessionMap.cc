...
void SessionMapStore::decode_legacy(bufferlist::const_iterator& p)
{
  ...
  if (pre == (uint64_t)-1) {
    ...
    while (n-- && !p.end()) {
      auto p2 = p;
      auto s = Session::create(ConnectionRef());
      s->info.decode(p);
...
void SessionMap::_mark_dirty(const Session::ref& s)
{
...
void SessionMap::mark_dirty(const Session::ref& s)
{
...
version_t SessionMap::mark_projected(const Session::ref& s)
{
  dout(20) << __func__ << " s=" << s << " name=" << s->info.inst.name
