				  ...
				  const file_layout_t *layout)
{
  ...
  // assign ino
  if (allow_prealloc_inos && (mdr->used_prealloc_ino = inode->ino = mdr->session->take_ino(useino))) {
    ...
    mdr->alloc_ino = 
      inode->ino = mds->inotable->project_alloc_id(useino);
...
void Server::handle_client_openc(MDRequestRef& mdr)
{
  ...
  if (cap && (cmode & CEPH_FILE_MODE_WR)) {
    ...
    inode->client_ranges[client].range.last = inode->layout.stripe_unit;
