 ...
 */
CDir* Server::try_get_complete_dirfrag(CInode *diri, frag_t fg, MDRequestRef& mdr)
{
...
void Server::_lookup_snap_ino(MDRequestRef& mdr)
{
  ...
  if (MDS_INO_IS_MDSDIR(ino))
    ...
    mdcache->open_ino(ino, mds->get_metadata_pool(), new C_MDS_LookupIno2(this, mdr), false);
}
...
void Server::handle_client_openc(MDRequestRef& mdr)
{
  ...
  if (mdr->session->info.has_feature(CEPHFS_FEATURE_DELEG_INO)) {
    ...
    if (client_async_dirop && delegate_inos_pct &&
	!req->is_queued_for_replay()) {
