...
class C_MDS_session_finish : public ServerLogContext {
  ...
  Context *fin;
public:
  C_MDS_session_finish(Server *srv, ref_t<Session> se, uint64_t sseq, bool s, version_t mv, Context *fin_ = NULL) :
    ServerLogContext(srv), session(std::move(se)), state_seq(sseq), open(s), cmapv(mv), inotablev(0), fin(fin_) { }
  C_MDS_session_finish(Server *srv, ref_t<Session> se, uint64_t sseq, bool s, version_t mv, interval_set<inodeno_t>& i, version_t iv, Context *fin_ = NULL) :
...
ref_t<Session> Server::find_session_by_uuid(std::string_view uuid)
{
  ref_t<Session> session;
...
void Server::reclaim_session(const ref_t<Session>& session, const cref_t<MClientReclaim> &m)
{
...
void Server::finish_reclaim_session(const ref_t<Session>& session, const ref_t<MClientReclaimReply> &reply)
{
...
void Server::find_idle_sessions()
{
  ...
  std::vector<ref_t<Session>> to_evict;
...
size_t Server::apply_blacklist(const std::set<entity_addr_t> &blacklist)
{
  ...
  std::vector<ref_t<Session>> victims;
  for (const auto& [c, s] : mds->sessionmap.get_sessions()) {
    if (!c.is_client()) {
...
std::pair<bool, uint64_t> Server::recall_client_state(MDSGatherBuilder* gather, RecallFlags flags)
{
  ...
  std::multimap<uint64_t, ref_t<Session>> caps_session;
 ...
 */
void Server::set_trace_dist(const ref_t<Session>& session, const ref_t<MClientReply>& reply,
