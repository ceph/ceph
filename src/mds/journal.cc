...
void EMetaBlob::fullbit::decode(bufferlist::const_iterator &bl) {
  ...
  inode = CInode::const_inode_cast(_inode);
  decode(*_inode, bl);
  {
...
{
  in->reset_inode(inode);
  in->reset_xattrs(xattrs);
  in->maybe_export_pin();
  if (in->is_dir()) {
  ...
  in->reset_old_inodes(old_inodes);
...
void EMetaBlob::replay(MDSRank *mds, LogSegment *logseg, MDSlaveUpdate *slaveup)
{
  ...
  for (const auto& lp : lump_order) {
	...
	in = new CInode(mds->mdcache, fb.dnfirst, fb.dnlast, flags);
