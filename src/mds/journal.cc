...
// EQuarantineStart
void EQuarantineStart::encode(bufferlist& bl, uint64_t features) const
{
  ENCODE_START(1, 1, bl);
  encode(subvol_ino, bl);
  encode(qtine_op, bl);
  ENCODE_FINISH(bl);
}

void EQuarantineStart::decode(bufferlist::const_iterator& bl)
{
  DECODE_START(1, bl);
  decode(subvol_ino, bl);
  decode(qtine_op, bl);
  DECODE_FINISH(bl);
}

void EQuarantineStart::dump(Formatter *f) const
{
  f->dump_int("subvol_ino", subvol_ino);
  f->dump_string("qtine_op", get_qtine_op_name());
}

std::list<EQuarantineStart> EQuarantineStart::generate_test_instances()
{
  std::list<EQuarantineStart> ls;
  ls.emplace_back();
  return ls;
}

void EQuarantineStart::replay(MDSRank *mds)
{
  mds->replay_record_quarantine_start(subvol_ino, qtine_op);
}

// EQuarantineFinish
void EQuarantineFinish::encode(bufferlist& bl, uint64_t features) const
{
  ENCODE_START(1, 1, bl);
  encode(subvol_ino, bl);
  encode(qtine_op, bl);
...
void EQuarantineFinish::decode(bufferlist::const_iterator& bl)
{
  DECODE_START(1, bl);
  decode(subvol_ino, bl);
  decode(qtine_op, bl);
...
void EQuarantineFinish::dump(Formatter *f) const
{
  f->dump_int("subvol_ino", subvol_ino);
  f->dump_string("qtine_op", get_qtine_op_name());
}

std::list<EQuarantineFinish> EQuarantineFinish::generate_test_instances()
{
  std::list<EQuarantineFinish> ls;
  ls.emplace_back();
  return ls;
}

void EQuarantineFinish::replay(MDSRank *mds)
{
  mds->replay_record_quarantine_finish(subvol_ino, qtine_op);
}

// EQuarantineCancel
void EQuarantineCancel::encode(bufferlist& bl, uint64_t features) const
{
  ENCODE_START(1, 1, bl);
  encode(subvol_ino, bl);
  encode(qtine_op, bl);
...
void EQuarantineCancel::decode(bufferlist::const_iterator& bl)
{
  DECODE_START(1, bl);
  decode(subvol_ino, bl);
  decode(qtine_op, bl);
...
void EQuarantineCancel::dump(Formatter *f) const
{
  f->dump_int("subvol_ino", subvol_ino);
  f->dump_string("qtine_op", get_qtine_op_name());
}

std::list<EQuarantineCancel> EQuarantineCancel::generate_test_instances()
{
  std::list<EQuarantineCancel> ls;
  ls.emplace_back();
  return ls;
}

void EQuarantineCancel::replay(MDSRank *mds)
{
  mds->replay_record_quarantine_cancel(subvol_ino, qtine_op);
}
