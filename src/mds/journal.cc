...
void EMetaBlob::remotebit::encode(bufferlist& bl) const
{
  ENCODE_START(3, 2, bl);
  encode(dn, bl);
  encode(std::tuple{dnfirst, dnlast, dnv, ino, d_type, dirty}, bl, 0);
  encode(alternate_name, bl);
  ENCODE_FINISH(bl);
}
...
void EMetaBlob::nullbit::encode(bufferlist& bl) const
{
  ENCODE_START(2, 2, bl);
  encode(dn, bl);
  encode(std::tuple{dnfirst, dnlast, dnv, dirty}, bl, 0);
  ENCODE_FINISH(bl);
}
...
void EMetaBlob::dirlump::encode(bufferlist& bl, uint64_t features) const
{
  ENCODE_START(2, 2, bl);
  encode(*fnode, bl);
  encode(std::tuple{state, nfull, nremote, nnull}, bl, 0);
...
void EMetaBlob::encode(bufferlist& bl, uint64_t features) const
{
  ...
  encode(std::tuple{opened_ino, allocated_ino, used_preallocated_ino}, bl, 0);
  encode(preallocated_inos, bl);
  encode(client_name, bl);
  encode(std::tuple{inotablev, sessionmapv}, bl, 0);
