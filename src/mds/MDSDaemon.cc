...
int MDSDaemon::ms_handle_authentication(Connection *con)
{
  ...
  if (!s) {
    ...
    s->info.inst.name = n;
    dout(10) << __func__ << " new session " << s << " for " << s->info.inst
	     << " con " << con << dendl;
    con->set_priv(s);
    s->connection = con;
  } else {
    dout(10) << __func__ << " existing session " << s << " for " << s->info.inst
	     << " existing con " << s->connection
  ...
  try {
    ...
    dout(20) << __func__ << "   parsing auth_cap_str='" << auth_cap_str
	     << "'" << dendl;
