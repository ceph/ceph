...
labels_t DaemonMetricCollector::get_extra_labels(std::string daemon_name) {
  ...
  if (daemon_name.find("radosgw") != std::string::npos) {
    ...
    std::cout << "daemon name: " << daemon_name << std::endl;
...
std::string DaemonMetricCollector::get_instance_id_from_rgw_socket_name(std::string &daemon_socket_file) {
  if (daemon_socket_file.find('.') != std::string::npos) {
    // Regular expression to match .<number>.<number> at the end
    std::regex pattern(R"(\.\d+\.\d+$)");
    auto daemon_name = std::regex_replace(daemon_socket_file, pattern, "");
