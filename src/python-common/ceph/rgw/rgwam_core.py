...
class RGWCmdBase:
    ...
    def run(self, cmd, stdin=None):
        ...
        if returncode != 0:
            cmd_str = " ".join(cmd)
            log.error(
                "ERROR: command exited with error status (%d): %s\nstdout=%s\nstderr=%s"
...
class ZonegroupOp:
    ...
    def create(
        self, realm: EntityKey, zg: EntityKey = None, endpoints=None, is_master=True
    ):
        ze = ZoneEnv(self.env, realm=realm).init_zg(zg, gen=True)

        params = ["zonegroup", "create"]
...
class PeriodOp:
    ...
    def update(
        self, realm: EntityKey, zonegroup: EntityKey, zone: EntityKey, commit=True
    ):
        ...
        master_zonegroup = (
            EntityName(master_zonegroup_info["name"])
            if master_zonegroup_info
            else zonegroup
        )
...
class UserOp:
    ...
    def create(
        ...
        params = ["user", "create", "--uid", u, "--display-name", dn]
...
class RGWAM:
    ...
    def create_zonegroup(
        self, realm, zonegroup_name, zonegroup_is_master, endpoints=None
    ):
        try:
            zg_info = self.zonegroup_op().create(
                realm,
    ...
    def create_system_user(self, realm, zonegroup, zone):
        try:
            ...
            logging.info(
                f"Created system user: {sys_user.uid} on"
    ...
    def create_normal_user(self, zg, zone, uid=None):
        try:
            ...
            logging.info(
                "Created regular user {user.uid} on"
                "{realm.name}/{zonegroup.name}/{zone.name}"
            )
            return user
        except RGWAMException as e:
    ...
    def realm_bootstrap(self, rgw_spec, start_radosgw=True):
        ...
        zonegroup = self.create_zonegroup(
            realm, zonegroup_name, zonegroup_is_master=True
        )
        ...
        self.zone_op().modify(
            zone, zonegroup, None, access_key, secret, endpoints=rgw_spec.zone_endpoints
        )
        self.update_period(realm, zonegroup)
        ...
        if start_radosgw and rgw_spec.zone_endpoints is None:
            ...
            realm_token = RealmToken(
                realm_name, realm.id, None, access_key, secret  # no endpoint
            )
    ...
    def realm_new_zone_creds(self, realm_name, endpoints, sys_uid):
        ...
        realm_token = RealmToken(
            realm_name, period.realm_id, ep, sys_access_key, sys_secret
        )
        ...
        return (
            0,
            "Realm Token: %s" % base64.b64encode(realm_token_b).decode("utf-8"),
            "",
        )
    ...
    def realm_rm_zone_creds(self, realm_token_b64):
        ...
        try:
            user_info = self.user_op().info(
                master_zone, master_zg, access_key=access_key
            )
        except RGWAMException as e:
        ...
        if only_key:
            ...
            success_message = (
                "Removed access key " + access_key + "(uid = " + user.uid + ")"
            )

        return (0, success_message, "")

    def zone_modify(
        self, realm_name, zonegroup_name, zone_name, endpoints, realm_token_b64
    ):
    ...
    def zone_create(self, rgw_spec, start_radosgw, secondary_zone_period_retry_limit=5):
        ...
        try:
            realm_info = self.realm_op().pull(
                EntityName(realm_token.realm_name),
                realm_token.endpoint,
                access_key,
                secret,
            )
        except RGWAMException as e:
        ...
        zone = self.create_zone(
            ...
            access_key,
            secret,
            endpoints=rgw_spec.zone_endpoints,
        )
        ...
        if start_radosgw and rgw_spec.zone_endpoints is None:
            secondary_realm_token = RealmToken(
                realm_name,
                realm_id,
                None,  # no endpoint
    ...
    def _get_daemon_eps(self, realm_name=None, zonegroup_name=None, zone_name=None):
        ...
        daemons = self.env.mgr.list_daemons(
            service_name,
            daemon_type,
            daemon_id=daemon_id,
            host=hostname,
        ...
        for s in daemons:
            for p in s.ports:
                ...
                if len(sp) < 2:
                    log.error(
                        "ERROR: service id cannot be parsed: (svc_id=%s)" % svc_id
                    )
                    continue
    ...
    def realm_reconcile(
        self, realm_name=None, zonegroup_name=None, zone_name=None, update=False
    ):
        ...
        for realm, realm_diff in diff.items():
            ...
            try:
                self.period_op().update(
                    EntityName(realm), EntityName(zg), EntityName(zone), True
                )
            except RGWAMException as e:
                raise RGWAMException("failed to update period", e)
