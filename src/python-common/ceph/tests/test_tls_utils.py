"""
Unit tests for ceph.deployment.tls_utils (Ceph issue #75710).

Pure parser/detector tests for the canonical PEM-handling utilities used
throughout cephadm and other consumers. No orchestrator, CertMgr, or
cephadm_module dependencies — this module only exercises tls_utils itself
against the stdlib + cryptography library.
"""
...
def _multi_block_no_cert_no_key() -> str:
    """Two PEM blocks, neither a CERTIFICATE nor a PRIVATE KEY.

    Used to prove that a multi-block blob containing zero usable certificate
    material is rejected (via parse_tls_pem_bundle's 'no CERTIFICATE blocks'
    check) rather than silently passed straight through to the caller as if
    it were valid cert data.
    """
    return "\n".join([
...
# ===========================================================================
# Tests: parse_tls_pem_bundle — canonical low-level parser
# ===========================================================================

class TestParseTlsPemBundle:
    """Unit tests for :func:`ceph.deployment.tls_utils.parse_tls_pem_bundle`,
    the single canonical parser used everywhere TLS PEM material needs to be
    parsed.
    """

    def setup_method(self):
        from ceph.deployment.tls_utils import parse_tls_pem_bundle
        self.parse = parse_tls_pem_bundle
    ...
    def test_pkcs1_key_with_three_cert_chain(self):
        ...
        cert_chain, key = self.parse(blob)
    ...
    def test_single_cert_no_key(self):
        ...
        cert_chain, key = self.parse(LEAF_CERT)
        assert LEAF_CERT.strip() in cert_chain
        assert key == ''
    ...
    def test_fullchain_with_crlf_line_endings(self):
        """Windows-style CRLF PEM input is accepted and split correctly."""
        blob = _fullchain_pkcs1().replace('\n', '\r\n')
        cert_chain, key = self.parse(blob)

        assert cert_chain.count('-----BEGIN CERTIFICATE-----') == 3
        assert '-----BEGIN RSA PRIVATE KEY-----' in key
        assert '-----END RSA PRIVATE KEY-----' in key
        assert '-----BEGIN RSA PRIVATE KEY-----' not in cert_chain
        assert '-----BEGIN CERTIFICATE-----' not in key

    def test_fullchain_with_openssl_bag_attributes_strips_metadata(self):
        """OpenSSL/PKCS#12 export metadata around PEM blocks is ignored.

        Bag Attributes, friendlyName, localKeyID, subject= and issuer= lines
        are metadata emitted by some OpenSSL export flows. They are not part
        of the PEM objects certmgr should persist.
        """
        blob = "\n".join([
            'Bag Attributes',
        ...
        for metadata in ['Bag Attributes', 'friendlyName', 'localKeyID', 'subject=', 'issuer=']:
    ...
    def test_raises_on_encrypted_private_key(self):
        ...
        with pytest.raises(SSLConfigException, match='Encrypted private keys are not supported'):
            self.parse(_encrypted_fullchain())
    ...
    def test_raises_on_plaintext_garbage(self):
        ...
        with pytest.raises(SSLConfigException):
            self.parse('this is not a PEM at all')
...
class TestGetCertificateInfoFullchain:
    ...
    def test_fullchain_returns_leaf_cert_info(self):
        from ceph.deployment.tls_utils import get_certificate_info
