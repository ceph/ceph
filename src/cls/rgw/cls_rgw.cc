...
{
  if (entry.exists) {
    std::string storage_class = entry.meta.storage_class;
...
int rgw_bucket_complete_op(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  ...
  if (op.op == CLS_RGW_OP_CANCEL) {
    ...
    std::string storage_class = meta.storage_class;
    if (storage_class.empty()){
      storage_class = RGW_STORAGE_CLASS_STANDARD;
    }
			    ...
			    bufferlist *in, bufferlist *out)
{
  ...
  while (!in_iter.end()) {
    ...
    if (cur_disk.pending_map.empty()) {
      ...
      if (cur_disk.exists) {
          std::string storage_class = cur_disk.meta.storage_class;
          if (storage_class.empty()){
              storage_class = RGW_STORAGE_CLASS_STANDARD;
          }
	...
	CLS_LOG_BITX(bitx_inst, 10, "INFO: %s: stats.num_entries: %ld -> %ld",
      ...
      std::string storage_class = cur_change.meta.storage_class;
      if (storage_class.empty()){
        storage_class = RGW_STORAGE_CLASS_STANDARD;
      }
      std::map<std::string, rgw_bucket_category_stats> storage_class_stats = header.storage_class_stats;
