...
struct rgw_usage_data {
  ...
  void aggregate(const rgw_usage_data& usage) {
    ...
    if (!storage_class and usage.storage_class){
...
struct rgw_usage_log_entry {
  ...
  std::map<std::string, std::map<std::string, rgw_usage_data>> usage_by_storage_class_map;
  ...
  void encode(ceph::buffer::list& bl) const {
   ...
   void decode(ceph::buffer::list::const_iterator& bl) {
    ...
    if (struct_v < 5) {
      usage_by_storage_class_map[""] = usage_map;
    } else {
