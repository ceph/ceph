                       ...
                       bufferlist *out)
{
  if (op.flags.is_first_part()) {
    if (unlikely(op.hash_state_bl.length() != 0)) {
      CLS_LOG(0, "ERROR: %s: Non empty hash_state_bl on first chunk (%u)",
              __func__, op.hash_state_bl.length());
      return -EOVERFLOW;
    }
  }
  else if (unlikely(op.hash_state_bl.length() > sizeof(blake3_hasher))) {
    CLS_LOG(0, "ERROR: %s: Overflow hash_state_bl len (%u/%lu)",
  ...
  if (op.flags.is_first_part()) {
    ...
    if (unlikely(hmac.cv_stack_len > (BLAKE3_MAX_DEPTH + 1) ||
      ...
      CLS_LOG(0, "ERROR: %s: bad hash_state_bl", __func__);
      return -EINVAL;
    }
  }

  uint64_t expected_chunk_counter = op.offset / BLAKE3_CHUNK_LEN;
  if (unlikely(expected_chunk_counter != hmac.chunk.chunk_counter)) {
    CLS_LOG(5, "ERROR: chunk_counter=%lu, expected_chunk_counter=%lu, offset=%lu",
            hmac.chunk.chunk_counter, expected_chunk_counter, op.offset);
  }
...
static int hash_data(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  ...
  default:
    CLS_LOG(0, "ERROR: %s: unexpcted hash_type (%u)", __func__, op.hash_type);
    return -EINVAL;
  }
}
