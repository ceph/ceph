...
bool MirrorImage::operator==(const MirrorImage &rhs) const {
  return mode == rhs.mode && global_image_id == rhs.global_image_id &&
         group_spec == group_spec && state == rhs.state;
}
...
void SnapshotNamespace::decode(bufferlist::const_iterator &p)
{
  ...
  boost::apply_visitor(DecodeSnapshotNamespaceVisitor(struct_v, p), *this);
  DECODE_FINISH(p);
}
...
GroupSnapshotNamespaceType get_group_snap_namespace_type(
  ...
  return static_cast<GroupSnapshotNamespaceType>(boost::apply_visitor(
    GetGroupSnapshotNamespaceTypeVisitor(), snapshot_namespace));
}

void GroupSnapshotNamespace::encode(bufferlist& bl) const {
  ENCODE_START(1, 1, bl);
  boost::apply_visitor(EncodeGroupSnapshotNamespaceVisitor(bl), *this);
  ENCODE_FINISH(bl);
}
...
void GroupSnapshotNamespace::decode(bufferlist::const_iterator &p)
{
  ...
  boost::apply_visitor(DecodeGroupSnapshotNamespaceVisitor(struct_v, p), *this);
  DECODE_FINISH(p);
}

void GroupSnapshotNamespace::dump(Formatter *f) const {
  boost::apply_visitor(
    DumpGroupSnapshotNamespaceVisitor(f, "snapshot_namespace_type"), *this);
}
...
void GroupSnapshot::dump(Formatter *f) const {
  f->dump_string("id", id);
  f->open_object_section("namespace");
  boost::apply_visitor(DumpGroupSnapshotNamespaceVisitor(f, "type"),
                       snapshot_namespace);
  f->close_section();
  f->dump_string("name", name);
  f->dump_stream("state") << state;
