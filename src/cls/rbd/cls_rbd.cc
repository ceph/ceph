			   ...
			   std::vector<cls::rbd::GroupSnapshot> *group_snaps)
{
  ...
  do {
    keys_read = cls_cxx_map_get_vals(hctx, last_read,
				     cls::rbd::RBD_GROUP_SNAP_KEY_PREFIX,
				     max_read, &vals);
  ...
  } while ((keys_read == RBD_MAX_KEYS_READ) && (group_snaps->size() < max_return));
		   ...
		   bufferlist *in, bufferlist *out)
{
  ...
  int r = check_duplicate_snap_name(hctx, group_snap.name);
  if (r < 0) {
    return r;
  }

  r = check_duplicate_snap_id(hctx, group_snap.snap_key());
  if (r < 0) {
    return r;
  }

  std::string key = group_snap.snap_key();
		      ...
		      bufferlist *in, bufferlist *out)
{
  ...
  bufferlist obl;
  ::encode(group_snap, obl);
  int r = cls_cxx_map_set_val(hctx, key, &obl);
		      ...
		      bufferlist *in, bufferlist *out)
{
  ...
  std::string snap_key = group_snap.snap_key();
			 ...
			 bufferlist *in, bufferlist *out)
{
  ...
  int r = cls_cxx_map_get_val(hctx, group_snap.snap_key(), &snapbl);
  if (r < 0) {
    return r;
  }
		    ...
		    bufferlist *in, bufferlist *out)
{
  ...
  std::vector<cls::rbd::GroupSnapshot> group_snaps;
  group_snap_list(hctx, start_after, max_return, &group_snaps);
