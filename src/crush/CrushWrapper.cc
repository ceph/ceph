				  ...
				  ostream *err)
{
  ...
  // validate first, so that a failure leaves the map untouched
  {
    vector<uint32_t> max_weight;
    if (new_shift < old_shift) {
      get_max_raw_weights(&max_weight);
  ...
  auto scale = [old_shift, new_shift](uint32_t w) -> uint32_t {
    ...
    if (new_shift > old_shift) {
      ...
      // a non-zero weight must not round to zero, which would drop the item
  ...
  // and the balancer's weight sets
  for (auto& i : choose_args) {
  ...
  // recompute bucket sums, straws and shadow trees from the scaled leaves
  reweight(cct);
				       ...
				       ostream *err)
{
  ...
  for (unsigned s = cur; s <= CRUSH_MAX_WEIGHT_SHIFT; ++s) {
    ...
    // rescaling rounds each leaf to nearest (and never to zero), so a bucket
    // can come out up to one raw unit per leaf heavier than weight >> (s - cur)
    const uint64_t slack = s == cur ? 0 : (uint64_t)crush->max_devices + 1;
    bool fits = true;
	...
	continue;
      ...
      uint64_t w = ((uint64_t)b->weight >> (s - cur)) + slack + add;
      if (w > max_weight[i]) {
	fits = false;
      }
    }
...
void CrushWrapper::encode(bufferlist& bl, uint64_t features) const
{
  ...
  if (HAVE_FEATURE(features, CRUSH_MSR)) {
    ...
    // keep last: the decoder finds trailing fields by position
