...
assert_namespaces_count() {
    ...
    for gw_ip in "${gateway_ips[@]}"; do
        ...
        if [ "$actual_count" -ne "$NVMEOF_SUBSYSTEMS_COUNT" ]; then
            ...
            echo "Expected count of namepaces not found via gateway $gw_ip, expected (per subsystem): $expected_count_per_subsys"
            return 1
        fi
    done
}
...
assert_host_connected() {
    ...
    if [ "$actual" != "$expected" ]; then
        ...
        echo "Expected host $host_nqn to be $(connected_or_disconnected "$expected") on $subsystem_nqn, found $(connected_or_disconnected "$actual")"
        return 1
    fi
}
...
assert_listener_exists() {
    ...
    for gw_ip in "${gateway_ips[@]}"; do
        ...
        if [ "$actual" != "$expected" ]; then
            ...
            echo "Expected listener $traddr:$trsvcid to be $(present_or_absent "$expected") on $subsystem_nqn via gateway $gw_ip, found $(present_or_absent "$actual")"
            return 1
        fi
    done
}
...
for i in $(seq 1 $NVMEOF_SUBSYSTEMS_COUNT); do
    ...
    for ns in $(seq 1 $NEW_NAMESPACES_COUNT); do
        ...
        ceph nvmeof --server-address $NVMEOF_DEFAULT_GATEWAY_IP_ADDRESS namespace add --nqn $subsystem_nqn --rbd-pool $RBD_POOL --rbd-image-name $image --load-balancing-group $(($image_index % $gateways_count + 1))
        ((image_index++))
    done
done
...
for i in $(seq 1 $NEW_SUBSYSTEMS_COUNT); do
    ...
    ceph nvmeof --server-address $NVMEOF_DEFAULT_GATEWAY_IP_ADDRESS namespace add --nqn $new_subsystem_nqn --rbd-pool $RBD_POOL --rbd-image-name "subsystest${i}" --create-image --rbd-image-size 16MB
done
...
for i in "${!gateway_ips[@]}"; do
    ...
    ceph nvmeof --server-address $gw_ip listener add --nqn $listener_test_subsystem_nqn --host-name $gw_name --traddr $gw_ip --trsvcid $NEW_LISTENER_PORT
done
...
for i in "${!gateway_ips[@]}"; do
    gw_ip="${gateway_ips[i]}"
    gw_name="${gateway_names[i]}"
    ceph nvmeof --server-address $gw_ip listener del --nqn $listener_test_subsystem_nqn --host-name $gw_name --traddr $gw_ip --trsvcid $NEW_LISTENER_PORT
done
...
ceph nvmeof --server-address $NVMEOF_DEFAULT_GATEWAY_IP_ADDRESS listener add --nqn $dummy_subsystem_nqn --host-name $NVMEOF_DEFAULT_GATEWAY_HOSTNAME --traddr $NVMEOF_DEFAULT_GATEWAY_IP_ADDRESS --trsvcid $NVMEOF_PORT
...
ceph nvmeof --server-address $NVMEOF_DEFAULT_GATEWAY_IP_ADDRESS listener add --nqn $listener_dummy_subsystem_nqn --host-name $NVMEOF_DEFAULT_GATEWAY_HOSTNAME --traddr $NVMEOF_DEFAULT_GATEWAY_IP_ADDRESS --trsvcid $NVMEOF_PORT
...
assert_command_fails ceph nvmeof --server-address $NVMEOF_DEFAULT_GATEWAY_IP_ADDRESS listener del --nqn $listener_dummy_subsystem_nqn --host-name $NVMEOF_DEFAULT_GATEWAY_HOSTNAME --traddr $NVMEOF_DEFAULT_GATEWAY_IP_ADDRESS --trsvcid $NVMEOF_PORT
assert_listener_exists $listener_dummy_subsystem_nqn $NVMEOF_DEFAULT_GATEWAY_IP_ADDRESS $NVMEOF_PORT "true"

# --force tears down the connection along with the listener
ceph nvmeof --server-address $NVMEOF_DEFAULT_GATEWAY_IP_ADDRESS listener del --nqn $listener_dummy_subsystem_nqn --host-name $NVMEOF_DEFAULT_GATEWAY_HOSTNAME --traddr $NVMEOF_DEFAULT_GATEWAY_IP_ADDRESS --trsvcid $NVMEOF_PORT --force
