...
# weight shift: rescaling changes the raw weights but not the ones the CLI
# shows
...
if ceph osd crush set-weight-shift 3 2> /tmp/ws.$$ ; then
    ...
    # reweight uses the same scale, and the largest single weight grows with
    # the shift: 32767 at shift 0, 8 times that at shift 3
    ceph osd crush reweight osd.$o6 100000
    expect_false ceph osd crush reweight osd.$o6 1000000
    ...
    # refused until require_osd_release supports weight_shift
    grep -q require_osd_release /tmp/ws.$$
fi
rm -f /tmp/ws.$$
