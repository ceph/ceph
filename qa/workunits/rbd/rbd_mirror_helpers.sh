...
wait_for_group_snapshot_uuid_removed()
{
    ...
    for s in 0.1 0.2 0.4 0.8 1 1 2 2 2 4 4 4 8 8 16; do
        ...
        local snap_exists
        snap_exists=$(xmlstarlet sel -t \
            -m "//group_snap[id='${group_snap_id}']" \
            -v id -n < "$CMD_STDOUT" || true)

        if [ -z "${snap_exists}" ]; then
            echo "group snapshot ${group_snap_id} does not exist"
            return 1
        fi

        local mirror_peer_uuids
        mirror_peer_uuids=$(xmlstarlet sel -t \
...
test_image_snapshots_uuid_removed()
{
    ...
    for image in ${images}; do
        ...
        local snap_exists
        snap_exists=$(xmlstarlet sel -t \
            -m "//snapshot[id='${snap_id}']" \
            -v id -n < "$CMD_STDOUT" || true)

        if [ -z "${snap_exists}" ]; then
            echo "snapshot ${snap_id} missing on image ${image}"
            return 1
        fi
