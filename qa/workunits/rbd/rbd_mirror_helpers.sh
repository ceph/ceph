...
test_group_snap_sync_state()
{
    ...
    snaps_synced=$(xmlstarlet sel -t -v "//group_snaps/group_snap[id='${group_snap_id}']/namespace/snaps_synced" < "$CMD_STDOUT")
    ...
    if [ -z "$snaps_synced" ]; then
        ...
        # Mirror snapshot
        test "$snaps_synced" = "$expected_state" || { fail; return 1; }
    fi
}
