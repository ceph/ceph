...
wait_for_group_snapshot_sync_complete()
{
    ...
    local snapshot_name=$(xmlstarlet sel -t -v "//group_snap/snapshot" < ${status_log})
    ...
    while true; do
        for s in 0.2 0.4 0.8 1.6 2 2 4 4 8 8 16 16 32 32; do
            ...
            local local_snapshot_name=$(xmlstarlet sel -t -v "//group_snap/snapshot" < ${local_status_log})

            test "${local_snapshot_name}" = "${snapshot_name}" && return 0
        done
