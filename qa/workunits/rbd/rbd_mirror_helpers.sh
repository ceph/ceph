...
mirror_group_disable_try()
{
    local cluster=$1
    local group_spec=$2

    try_cmd "rbd --cluster=${cluster} mirror group disable ${group_spec}"
}
...
backup_group_header()
{
    ...
    local group_id=$3
    local group_header_oid="rbd_group_header.${group_id}"
    local backup_oid="${group_header_oid}.backup"

    run_cmd "rados --cluster ${cluster} -p ${pool} cp ${group_header_oid} ${backup_oid}"
}

restore_group_header()
{
    local cluster=$1
    local pool=$2
    local group_id=$3
    ...
    run_cmd "rados --cluster ${cluster} -p ${pool} cp ${backup_oid} ${group_header_oid}"
    run_cmd "rados --cluster ${cluster} -p ${pool} rm ${backup_oid}"
}
