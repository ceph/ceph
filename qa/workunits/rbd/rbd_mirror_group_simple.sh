...
# test group snapshot with promote and demote
declare -a test_snapshot_with_promote_demote_1=("${CLUSTER2}" "${CLUSTER1}" "${pool0}" 3 2G)

test_snapshot_with_promote_demote_scenarios=1

test_snapshot_with_promote_demote()
{
  local primary_cluster=$1 ; shift
  local secondary_cluster=$1 ; shift
  local pool=$1 ; shift
...
# test snapshot sync to secondary cluster while restarting secondary cluster
declare -a test_multiple_snapshot_sync_while_restarting_secondary_1=("${CLUSTER2}" "${CLUSTER1}" "${pool0}" 3 2G)

test_multiple_snapshot_sync_while_restarting_secondary_scenarios=1

test_multiple_snapshot_sync_while_restarting_secondary()
{
  local primary_cluster=$1 ; shift
  local secondary_cluster=$1 ; shift
  local pool=$1 ; shift
