...
test_mirror_group_snapshot_schedule() {
    ...
    rbd group create rbd2/ns1/test1
    rbd group image add rbd2/ns1/test1 rbd2/ns1/img1
    rbd group image add rbd2/ns1/test1 rbd2/ns1/img2

    test "$(rbd mirror group status rbd2/ns1/test1 |
        grep -c mirror.primary)" = '0'

    rbd mirror group enable rbd2/ns1/test1 snapshot

    test "$(rbd mirror group status rbd2/ns1/test1 |
        grep -c mirror.primary)" = '1'

    rbd mirror group snapshot schedule add -p rbd2/ns1 --group test1 1m
    expect_fail rbd mirror group snapshot schedule ls
    rbd mirror group snapshot schedule ls -R | grep 'rbd2 *ns1 *test1 *every 1m'
    expect_fail rbd mirror group snapshot schedule ls -p rbd2
    rbd mirror group snapshot schedule ls -p rbd2 -R | grep 'rbd2 *ns1 *test1 *every 1m'
    expect_fail rbd mirror group snapshot schedule ls -p rbd2/ns1
    rbd mirror group snapshot schedule ls -p rbd2/ns1 -R | grep 'rbd2 *ns1 *test1 *every 1m'
    test "$(rbd mirror group snapshot schedule ls -p rbd2/ns1 --group test1)" = 'every 1m'
    ...
    test "$(rbd mirror group snapshot schedule status --format xml |
        xmlstarlet sel -t -v '//scheduled_groups/group/group')" = 'rbd2/ns1/test1'
    ...
    test "$(rbd mirror group snapshot schedule status -p rbd2/ns1 --format xml |
        xmlstarlet sel -t -v '//scheduled_groups/group/group')" = 'rbd2/ns1/test1'
    test "$(rbd mirror group snapshot schedule status -p rbd2/ns1 --group test1 --format xml |
        xmlstarlet sel -t -v '//scheduled_groups/group/group')" = 'rbd2/ns1/test1'

    rbd mirror group demote rbd2/ns1/test1
    for i in `seq 12`; do
        rbd mirror group snapshot schedule status | grep 'rbd2/ns1/test1' || break
        sleep 10
    done
    rbd mirror group snapshot schedule status | expect_fail grep 'rbd2/ns1/test1'

    rbd mirror group promote rbd2/ns1/test1
    for i in `seq 12`; do
        rbd mirror group snapshot schedule status | grep 'rbd2/ns1/test1' && break
        sleep 10
    done
    rbd mirror group snapshot schedule status | grep 'rbd2/ns1/test1'
    ...
    expect_fail rbd mirror group snapshot schedule add -p rbd2/ns1 --group test1 dummy
    expect_fail rbd mirror group snapshot schedule remove dummy
    expect_fail rbd mirror group snapshot schedule remove -p rbd2/ns1 --group test1 dummy
    test "$(rbd mirror group snapshot schedule ls)" = 'every 1h starting at 00:15:00'
    test "$(rbd mirror group snapshot schedule ls -p rbd2/ns1 --group test1)" = 'every 1m'

    rbd group rm rbd2/ns1/test1
    for i in `seq 12`; do
        rbd mirror group snapshot schedule status | grep 'rbd2/ns1/test1' || break
        sleep 10
    done
...
test_mirror_group_snapshot_schedule_recovery() {
    ...
    expect_fail rbd mirror group snapshot schedule add -p rbd2/ns1 --group test1 2m
    sleep 10
    for i in `seq 24`; do
        rbd mirror group snapshot schedule add -p rbd2/ns1 --group test1 2m && break
	sleep 10
    done

    rbd mirror group snapshot schedule ls -p rbd2/ns1 --group test1 | grep 'every 2m'
    # Verify that the schedule present before client blocklisting is preserved
    rbd mirror group snapshot schedule ls -p rbd2/ns1 --group test1 | grep 'every 1m'

    rbd mirror group snapshot schedule rm -p rbd2/ns1 --group test1 2m
    rbd mirror group snapshot schedule rm -p rbd2/ns1 --group test1 1m
    rbd mirror group snapshot schedule ls -p rbd2/ns1 --group test1 | expect_fail grep 'every 2m'
    rbd mirror group snapshot schedule ls -p rbd2/ns1 --group test1 | expect_fail grep 'every 1m'
