...
validate_subvol_cnt() {
  if [ $1 -eq 20 ]; then return 0; else return 1; fi
}
restart_mgr() {
  ceph mgr fail x
  timeout=30
...
#Create 20 subvolumes
for i in {1..20}; do ceph fs subvolume create cephfs sub_$i; done
...
for i in {1..20}; do ceph fs subvolume rm cephfs sub_$i; done
