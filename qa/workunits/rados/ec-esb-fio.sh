...
for OSD_ID in {0..6}; do
    echo "Processing osd.$OSD_ID"
    ceph-bluestore-tool --dev ./dev/osd$OSD_ID/block --command set-label-key --key elastic_shared_blobs --value 0 || true
    ceph-bluestore-tool --dev ./dev/osd$OSD_ID/block show-label || true
done
