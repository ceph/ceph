...
function test_sample_dedup()
{
  ...
  $DEDUP_DAEMON --pool $POOL --chunk-pool $CHUNK_POOL --chunk-dedup-threshold 3 --sampling-ratio 50 --run-once
  sleep 2
  ...
  RESULT=$($DEDUP_DAEMON --pool $POOL --chunk-pool $CHUNK_POOL --sampling-ratio 100 --chunk-dedup-threshold 2 --max-thread 1 --run-once)
  sleep 2
  PID=$(pidof ceph-dedup-daemon)
  COUNT=1
  while [ -n "$PID" ] && [ $COUNT -le 30 ]; do
...
function test_sample_dedup_snap()
{
  ...
  # Execute dedup crawler
  RESULT=$($DEDUP_DAEMON --pool $POOL --chunk-pool $CHUNK_POOL --sampling-ratio 100 --chunk-dedup-threshold 1 --run-once)
  sleep 2
  PID=$(pidof ceph-dedup-daemon)
  COUNT=1
...
function test_dedup_memory_limit()
{
  ...
  # Execute dedup crawler
  RESULT=$($DEDUP_DAEMON --pool $POOL --chunk-pool $CHUNK_POOL --sampling-ratio 100 --chunk-dedup-threshold 2 --run-once)
  sleep 2
  PID=$(pidof ceph-dedup-daemon)
  COUNT=1
  while [ -n "$PID" ] && [ $COUNT -le 30 ]; do
