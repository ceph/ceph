...
function TEST_ok_to_upgrade_replicated_pool() {
    ...
    local poolname="test"
    local OSDS=10
    local upgrade_version="01.00.00-gversion-test"
    ...
    local hostname=$(ceph osd tree | grep host | awk '{ print $4 }')
    local res=$(ceph osd ok-to-upgrade $hostname $upgrade_version --format=json)
    # Specifying hostname as the crush bucket with a 3x replicated pool on 10 OSDs
    # and with the default 'mgr_osd_upgrade_check_reduction_factor' would result
    ...
    local num_osds_upgradable=$(echo $res | jq '.ok_upgrade | length' | bc)
    test $exp_osds_upgradable = $num_osds_upgradable || return 1
    local num_osds_upgraded=$(echo $res | jq '.ok_upgraded | length' | bc)
    ...
    res=$(ceph osd ok-to-upgrade $hostname $upgrade_version --format=json)
    ...
    test $(echo $res | jq '.ok_to_upgrade') = false || return 1
    num_osds_upgradable=$(echo $res | jq '.ok_upgrade | length' | bc)
    test $num_osds_upgradable -eq 0 || return 1
    num_osds_upgraded=$(echo $res | jq '.ok_upgraded | length' | bc)
