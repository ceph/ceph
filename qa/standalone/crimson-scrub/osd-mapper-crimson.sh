...
function crimson_corrupt_snapmapper_sna() {
    ...
    # We match: SNA_<poolid>_0000000000000003_
    local KY
    KY=$(crimson-objectstore-tool --data-path $kvdir \
            --pgid $pgid "snapmapper" list-omap 2>/dev/null \
         | grep -a -i -E 'SNA_[0-9]+_0000000000000003_') || true

    if [[ -z "$KY" ]]; then
        echo "ERROR: no SNA_ key for snapid 3 found on osd.$osd_id"
        return 1
    fi
...
function TEST_truncated_sna_record() {
    ...
    num_sna_b4=$(crimson-objectstore-tool --data-path $dir/$osd \
        --pgid $pgid "snapmapper" list-omap 2>/dev/null \
      | grep -a -i -E 'SNA_[0-9]+_000000000000000[0-9a-f]_000000000000000' \
      | wc -l)
