...
#
# Crimson adaptation: Based on osd-recovery-scrub.sh
#
...
function TEST_recovery_scrub_1() {
    ...
    local ceph_osd_args="--osd_scrub_during_recovery=false "
    ceph_osd_args+="--osd-scrub-interval-randomize-ratio=0 "
    ...
    create_pool $poolname $PGS $PGS --autoscale_mode=off || return 1
...
main osd-recovery-scrub-crimson "$@"

# Local Variables:
# compile-command: "cd build ; make -j4 && \
#    ../qa/run-standalone.sh osd-recovery-scrub-crimson.sh"
# End: