    ...
    for (client,_) in config.items():
        ...
        ctx.cluster.only(client).run(
            args=['cd', '{tdir}/bin'.format(tdir=kafka_dir), run.Raw('&&'),
             ...
             '{tdir}/config/zookeeper1.properties'.format(tdir=kafka_dir), run.Raw('&')
            ],
        )
        ...
        ctx.cluster.only(client).run(
            args=['cd', '{tdir}/bin'.format(tdir=kafka_dir), run.Raw('&&'),
             './zookeeper-server-start.sh',
             '{tdir}/config/zookeeper2.properties'.format(tdir=kafka_dir), run.Raw('&')
            ],
        )
        # wait for zookeeper to start
        time.sleep(5)
        ...
        ctx.cluster.only(client).run(
            args=['cd', '{tdir}/bin'.format(tdir=kafka_dir), run.Raw('&&'),
             ...
             '{tdir}/config/kafka1.properties'.format(tdir=get_kafka_dir(ctx, config)), run.Raw('&')
            ],
        )
        ...
        ctx.cluster.only(client).run(
            args=['cd', '{tdir}/bin'.format(tdir=kafka_dir), run.Raw('&&'),
             './kafka-server-start.sh',
             '{tdir}/config/kafka2.properties'.format(tdir=get_kafka_dir(ctx, config)), run.Raw('&')
            ],
        )
        # wait for kafka to start
        time.sleep(5)
