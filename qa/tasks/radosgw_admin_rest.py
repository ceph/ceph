...
def task(ctx, config):
    ...
    assert bucket_quota["max_objects"] == 2000000
    ...
    (ret, out) = rgwadmin_rest(admin_conn, [('account', 'quota'), 'set'], {'id': account_id, 'quota-type' : 'bucket', 'max-size': 12345, 'max-objects' : 54321, 'enabled': False})
    assert ret == 200
    (ret, out) = rgwadmin_rest(admin_conn, ['account', 'info'], {'id' : account_id})
    assert ret == 200
