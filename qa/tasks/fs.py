...
def post_upgrade_checks(ctx, config):
    ...
    for fs in list(status.get_filesystems()):
        ...
        for i in range(pre_upgrade_epoch+1, mdsmap['epoch']):
            ...
            log.debug(f"epoch {i} : MDS not_joinable"
                      f" = {old_mdsmap['flags'] & CEPH_MDSMAP_NOT_JOINABLE} "
                      f"and max_mds = {old_mdsmap['max_mds']}")
            if not multiple_max_mds \
                    and (old_mdsmap['flags'] & CEPH_MDSMAP_NOT_JOINABLE):
                raise RuntimeError("mgr is doing fs fail when there is only "
                                   f"one rank in epoch {i}")
            elif multiple_max_mds \
                    and (old_mdsmap['flags'] & CEPH_MDSMAP_NOT_JOINABLE) \
                    and old_mdsmap['max_mds'] == 1:
                raise RuntimeError("fs fail happened along with reducing "
                                   f"max_mds in epoch {i}")
            if old_mdsmap['flags'] & CEPH_MDSMAP_NOT_JOINABLE:
                ...
                did_fail_fs = True
            elif multiple_max_mds and old_mdsmap['max_mds'] == 1:
        ...
        assert not multiple_max_mds or did_decrease_max_mds or did_fail_fs
