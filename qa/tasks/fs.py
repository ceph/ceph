...
def post_upgrade_checks(ctx, config):
    ...
    for fs in list(status.get_filesystems()):
        ...
        for i in range(pre_upgrade_epoch+1, mdsmap['epoch']):
            ...
            if multiple_max_mds and \
                    (old_mdsmap['flags'] & CEPH_MDSMAP_NOT_JOINABLE) and\
                    old_mdsmap['max_mds'] > 1:
        ...
        assert not multiple_max_mds or (did_decrease_max_mds != did_fail_fs)
