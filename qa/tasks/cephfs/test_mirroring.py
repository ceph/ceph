...
class TestMirroring(CephFSTestCase):
    ...
    def test_ceph_health_warning_on_init_failure(self):
        """Test MIRRORING_FAILURE warning in ceph health on init failure"""
    ...
    def test_ceph_health_warning_on_sync_failure(self):
        """
        That making changes to the remote .snap directory shows MIRRORING_FAILURE warning in ceph health
        """
        self.setup_mount_b(mds_perm='rwps')
        self.enable_mirroring(self.primary_fs_name, self.primary_fs_id)
        peer_spec = "client.mirror_remote@ceph"
        ...
        # remove the directory in the remote fs and check health warn goes away
        self.mount_b.run_shell(['sudo', 'rmdir', remote_snap_path], omit_sudo=False)
        self.wait_for_health("HEALTH_OK", 120)
        self.disable_mirroring(self.primary_fs_name, self.primary_fs_id)
