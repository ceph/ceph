...
class TestVolumesHelper(CephFSTestCase):
    ...
    def get_client_snapshot_visbility_flag(self, who: str):
        """
    ...
    def set_client_snapshot_visbility_flag(self, who: str, value: str):
        ...
        """
        self.get_ceph_cmd_stdout(f"config set {who} "
                                 f"{self.SNAPSHOT_VISIBILITY} {value}")
        self.assertEqual(self.get_client_snapshot_visbility_flag(who), value)
...
class TestSubvolumeSnapshotVisibilityBasic(TestVolumesHelper):
    ...
    def test_toggling_snapshotvisibility_vxattr_on_non_subvolume_path(self):
        """
        that toggling ceph.dir.subvolume.snaps.visible doesn't work on
        non-subvolume path
        """
    ...
    def test_snapshot_visibility_after_failing_mds(self):
        """
        that after setting the snapshot_visibility flag, and failing the MDS,
        ...
        self.set_subvolume_snapshot_visibility(self.volname, subvolume, "false")
        # fail rank 0 to enforce a failover to the standby MDS
        self.run_ceph_cmd("mds fail 0")
        # wait a while
        time.sleep(5)
...
class TestSubvolumeSnapshotVisibilityMgr(TestVolumesHelper):
    """
    That subvolume APIs adhere to the subvolume based flag snapshot_visibility
    and client config client_respect_subvolume_snapshot_visibility
    """
    CLIENTS_REQUIRED = 1
    ...
    def _test_snapshot_create(self, respect_client_config: bool,
                                grouped: bool):
        """
        that subvolume creation takes place according to the client config
        ...
        subvolume = self._gen_subvol_name()
        snapshot = self._gen_subvol_snap_name()
        snapshot1 = self._gen_subvol_snap_name() if not respect_client_config else ""
        ...
        if respect_client_config:
            ...
            # snapshot create should go through since it doesn't respect
            # the config
            self._fs_cmd_grouped("subvolume", "snapshot", "create",
                                 self.volname, subvolume, snapshot, group)
        ...
        self.set_subvolume_snapshot_visibility(self.volname, subvolume,
                                               "true", group)
        # since the snapshot visibility is true, no matter what the client
        # config is, snapshot create should go through
        if respect_client_config:
            self._fs_cmd_grouped("subvolume", "snapshot", "create",
                                 self.volname, subvolume, snapshot, group)
        else:
    ...
    def _test_snapshot_rm(self, respect_client_config: bool, grouped: bool):
        """
        that subvolume rm takes place according to the client config
        client_respect_subvolume_snapshot_visibility and subvolume flag
        snapshot_visibility
        """
        if respect_client_config:
        ...
        if respect_client_config:
            ...
            # snapshot rm should go through since it doesn't respect
            # the config
            self._fs_cmd_grouped("subvolume", "snapshot", "rm",
                                 self.volname, subvolume, snapshot, group)
        ...
        self.set_subvolume_snapshot_visibility(self.volname, subvolume,
                                               "true", group)
        # since the snapshot visibility is true, no matter what the client
        # config is, snapshot rm should go through
        if respect_client_config:
            self._fs_cmd_grouped("subvolume", "snapshot", "rm",
                                 self.volname, subvolume, snapshot, group)
        else:
    ...
    def test_snapshot_create_nogroup_no_respect_client_config(self):
        """
        that snapshot create works irrespective of the snapshot_visibility
        flag since client_respect_subvolume_snapshot_visibility is false
        for a non-grouped subvolume
        """
    ...
    def test_snapshot_create_nogroup_respect_client_config(self):
        """
        that snapshot create adheres snapshot_visibility flag since
    ...
    def test_snapshot_create_group_no_respect_client_config(self):
        """
        that snapshot create works irrespective of the snapshot_visibility
        flag since client_respect_subvolume_snapshot_visibility is false for
        a grouped subvolume
        """
    ...
    def test_snapshot_rm_nogroup_no_respect_client_config(self):
        """
        that snapshot rm works irrespective of the snapshot_visibility
        flag since client_respect_subvolume_snapshot_visibility is false
        for a non-grouped subvolume
        """
    ...
    def test_snapshot_rm_nogroup_respect_client_config(self):
        """
        that snapshot rm adheres snapshot_visibility flag since
        client_respect_subvolume_snapshot_visibility is true for a
        non-grouped subvolume
        """
