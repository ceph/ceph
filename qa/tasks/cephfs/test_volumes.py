...
class TestVolumesHelper(CephFSTestCase):
    ...
    def __check_clone_state(self, states, clones, clone_group=None,
                            max_count=120, sleep=1):
        if isinstance(states, str):
            states = (states, )
        if not isinstance(clones, (list, tuple)):
            clones = (clones, )
        ...
        for clone in clones:
            ...
            msg = (f'Executed cmd "{args}" {max_count} times; clone was never '
                   f'in "{states}" state(s).')

            with safe_while(sleep=sleep, tries=max_count, action=msg) as proceed:
                while proceed():
...
# random of files and due to multiple cloning jobs that are running
# simultaneously.
#
...
# time for test code can capture progress bar in desired state and finish
...
class TestCloneProgressReporter(TestVolumesHelper):
    ...
    def loop_cmd_if_eagain(self, cmdargs):
        i = 1
        ...
        while True:
            ...
            if p.exitstatus == 11:
                i += 1
                if i == 10:
                    raise RuntimeError(f'This cmd failed 10 times: {cmdargs}')
                time.sleep(2)
                continue
            elif p.exitstatus == 0:
                return p
    ...
    def wait_if_cloning_is_ongoing(self):
        with safe_while(tries=10, sleep=5) as proceed:
            while proceed():
                pevs = self.get_pevs_from_ceph_status(check=False)
                if len(pevs) == 0:
                    return

                #pev_msg = tuple(pevs.values())[0]['message'].lower()
                #if 'all cloning finished' in pev_msg:
                #    return
    ...
    def tearDown(self):
        ...
        # wait for ongoing clones and abort if they don't finish even after
        # waiting because deleting subvolumes doesn't work when clones are
        # ongoing.
        self.wait_if_cloning_is_ongoing()
        ...
        for i in subvols:
            sv = tuple(i.values())[0]
            if 'clone' in sv:
                self.loop_cmd_if_eagain(f'fs subvolume rm --force {v} {sv}')
                continue

            p = self.loop_cmd_if_eagain(f'fs subvolume snapshot ls {v} {sv} '
                                        '--format json')
            snaps = p.stdout.getvalue().strip()
            snaps = json.loads(snaps)
            for j in snaps:
                ss = tuple(j.values())[0]
                self.loop_cmd_if_eagain('fs subvolume snapshot rm --force '
    ...
    def wait_till_rbytes_is_right(self, v, sv, exp_size, sleep=2, max_count=60):
    ...
    def test_stats_are_printed(self):
        '''
        ...
        while p < 100:
            ...
            try:
                ...
                log.info(f'percentage cloned = {p}')
            except KeyError:
    ...
    def filter_out_gre_from_pevs(self, progress_events):
        '''
        ...
        This extra progress bar interferes with testing of progress bars for
        cloning. This helper methods removes (filters out) this extra event.
        '''
        ...
        for k, v in progress_events.items():
            if 'global recovery event' in v['message'].lower():
                gre_key = k
                # Don't break. Usually there is only one "Global Recovery
                # Event" in "progress_events" dictionary printed by "ceph
                # status --format json-pretty". But let the loop run and catch
                # and remove them in case there might be multiple of them here.
                # Else, it'll interfere and cause tests to fail.
                # break
    ...
    def _do_sv_io(self, sv):
        v = self.volname
        sv_path = self.get_ceph_cmd_stdout(f'fs subvolume getpath {v} {sv}')
        sv_path = sv_path[1:].rstrip() # remove "/" prefix and any trailing newline

        for i in range(1, 4):
            self.mount_a.run_shell(f'cp ~/1G.zero {sv_path}/file{i}')
    ...
    def get_pevs_from_ceph_status(self, clones=None, check=True):
        ...
        try:
            ...
            try:
                if check and clones:
                    self.__check_clone_state('completed', clone=clones, max_count=1)
            except:
        ...
        pevs = self.filter_out_gre_from_pevs(pevs)
    ...
    def test_progress_bar_for_1_clone(self):
        ...
        in output of command "ceph status" that shows progress of this clone.
        '''
        v = self.volname
        sv = 'sv1'
        ss = 'ss1'
    ...
    def test_progress_bars_for_4_clones(self):
        '''
        Test that one progress bar is printed when number of clone jobs is
        less than number of cloner threads.
        '''
        v = self.volname
        sv = 'sv1'
        ss = 'ss1'
    ...
    def wait_for_both_progress_bars_to_appear(self, sleep=1, iters=20):
        ...
        while len(pevs) < 2:
            if i > iters:
                raise RuntimeError(f'Waited for {iters*sleep} seconds but '
    ...
    def test_progress_bars_for_7_clones(self):
        '''
        Test that 2 progress bars are printed when number of clone jobs is
        greater than number of cloner threads. Also, test that one of these
        progress bars is for ongoing clones and other progress bar for
        ...
        with safe_while(tries=10, sleep=2) as proceed:
            while proceed():
                ...
                if ('ongoing clones' in pev1['message'].lower() and
                    'ongoing+pending clones' in pev2['message'].lower()):
                    break
                elif ('ongoing clones' in pev2['message'].lower() and
                      'ongoing+pending clones' in pev1['message'].lower()):
                    break
                else:
                    raise RuntimeError('messages for progress bars for '
                                       'snapshot cloning are not how they '
                                       'were expected')
                time.sleep(1)
    ...
    def test_progress_drops_when_new_jobs_are_added(self):
        ...
        with safe_while(tries=30, sleep=1) as proceed:
            while proceed():
    ...
    def test_when_1_clone_is_cancelled(self):
        ...
        pevs = self.get_pevs_from_ceph_status(c)
        if len(pevs) == 1:
            p_bar_msg = tuple(pevs.values())[0]['message'].lower()
            self.assertIn('all cloning finished', p_bar_msg)
        else:
            raise RuntimeError(
                '0 or 1 progress bar for cloning was expected in output of '
                f'"ceph status", found {len(pevs)} instead.')
    ...
    def test_when_7_clones_are_cancelled(self):
        ...
        with safe_while(tries=10, sleep=1) as proceed:
            while proceed():
                ...
                msg = ('"progress_events" dict in "ceph status --format json" '
                       f'output must have only two entries.\n{pevs}')
                self.assertEqual(len(pevs), 2, msg)
                pev1, pev2 = tuple(pevs.values())

                if ('all cloning finished' in pev1['message'].lower() and
                    'no pending clones left' in pev2['message'].lower()):
                    break
                elif ('all cloning finished' in pev2['message'].lower() and
                      'no pending clones left' in pev1['message'].lower()):
                    break
    ...
    def _test_rm_1_progress_bar_when_vol_is_deleted(self):
        ...
        self.run_ceph_cmd(f'fs subvolume create {v} {sv} --mode=777')
        #self._do_subvolume_io(sv, None, None, 3, 1024+1024*1024)
        self.run_ceph_cmd(f'fs subvolume snapshot create {v} {sv} {ss}')
        self.wait_till_rbytes_is_right(v, sv)
        ...
        try:
            ...
            self.cancel_clones_and_confirm(c)
    ...
    def _test_rm_2_progress_bars_when_vol_is_deleted(self):
        ...
        self.wait_for_both_progress_bars_to_appear()
