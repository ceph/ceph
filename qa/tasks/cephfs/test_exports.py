...
class TestKillPoints(CephFSTestCase):
    ...
    def _verify_data(self):
        ...
        if s1.isdisjoint(s2):
            log.info("Directory contents org: %s" %str(s2))
            log.info("Directory contents new: %s" %str(s1))
            return False
    ...
    def _run_export_dir(self, importv, exportv):
        ...
        icoredump = False if importv == 0 else True
        ecoredump = False if exportv == 0 else True

        log.info(f"icoredump:{icoredump}, ecoredump:{ecoredump}!")
        while True:
            time.sleep(5)
            ...
            if ecoredump and status.had_failover_rank(self.fs.id, 0, new_status):
                log.info("MDS %s crashed and active as MDS %s at type %s killpoint %d"
                         %(rank_0_name, rank_0_new_name, killpoint[0][0], killpoint[0][1]))
                if self.delete_mds_coredump(rank_0_name):
                    ecoredump = False

            if icoredump and status.had_failover_rank(self.fs.id, 1, new_status):
                log.info("MDS %s crashed and active as MDS %s at type %s killpoint %d"
                         %(rank_1_name, rank_1_new_name, killpoint[1][0], killpoint[1][1]))
                if self.delete_mds_coredump(rank_1_name):
                    icoredump = False

            active_mds = self.fs.get_active_names()
            if icoredump or ecoredump or len(active_mds) != 2:
                # One or more MDS did not come up
                elapsed += 1
                if elapsed < 120:
                    log.info(f"icoredump:{icoredump}, ecoredump:{ecoredump}, active_mds:{active_mds}, retry {elapsed}!")
                    continue
                return False
...
class TestKillPointsA(TestKillPoints):
    # The fist half of the killpiont tests
    pass

class TestKillPointsB(TestKillPoints):
    # The second half of the killpiont tests
    pass

class TestKillPointsC(TestKillPoints):
    # The third half of the killpiont tests
    pass
...
def setup_test_killpoints(TestClass, start, end):
    for import_killpoint in range(start, end):
        for export_killpoint in range(2, 14):
            test_export_killpoints = make_test_killpoints(import_killpoint, export_killpoint)
            setattr(TestClass, "test_export_killpoints_%d_%d" % (import_killpoint, export_killpoint), test_export_killpoints)

setup_test_killpoints(TestKillPointsA, 0, 4)
setup_test_killpoints(TestKillPointsB, 4, 8)
setup_test_killpoints(TestKillPointsC, 8, 10)
