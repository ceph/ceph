...
function ensure_min_npm_version {
  ...
  # The minimum node version required is 8.12.0 so that we can use yarn below
  UPDATE_NODE=false
  if [ $NODE_VER_MAJOR -lt 8 ]; then
    UPDATE_NODE=true
  elif [ $NODE_VER_MAJOR -eq 8 ] && [ $NODE_VER_MINOR -lt 12 ]; then
    UPDATE_NODE=true
  fi
  if $UPDATE_NODE; then
    $SUDO npm install -g n
    # installs nodejs version 8.12.0
    $SUDO n 8.12.0
